...
bd = '/Volumes/GoogleDrive-114825029448473821206/My Drive/LIGHT_GBM/models/'
for folder in next(os.walk(bd))[1]:
    if '10_sets_frame' in folder:
        d = bd + folder
        count1, gain1 = foo1(d)
...
"""
##################################################################################################
########################   only selected models swarmplot plot   ################################################
"""
...
# os.listdir(mod_dir)
folder_names = [
    # 'top_features_greter_than_3_out_of_10_3095_features',
    '10_sets_frame_nums_split_then_random_split_method_numpy_ORIGINAL_2048_just_grabbed_the_last_2048',
    '10_sets_frame_nums_split_then_random_split_method',
    '10_sets_frame_nums_split_then_random_split_method_numpy_feature_selection_V1',
    '10_sets_frame_nums_split_then_random_split_method_numpy_feature_selection_V2',
...
names = [
    # 'OLD METHOD 3095 features OVERFIT',
    'Original\n2,048',
    'full\n84,009',
    '29,913',
    '18,445',
...
all_mod_list = []
AUC = []
num_features = np.repeat(names, 10)
mod_number = list(range(10)) * len(all_mods)
for i, d in enumerate(all_mods):
    for k in d:
        m = utils.load_obj(k)
        AUC.append(m.best_score['valid_0']['auc'])



df = {'1 - AUC': 1 - np.asarray(AUC), 'num_features': num_features, 'mod_num': mod_number}
df = pd.DataFrame(df)


ax = sns.swarmplot(y="1 - AUC",
                   x="num_features",
                   data=df,
                   color="gray", edgecolor="gray", label = 'individual models'
                   )

df_means = df.groupby("num_features", sort=False)["1 - AUC"].agg("mean").reset_index()
means = np.asarray(df_means['1 - AUC'])

plt.scatter(np.arange(len(means)), means, color = 'k', marker = '_', zorder=100, label='mean')
