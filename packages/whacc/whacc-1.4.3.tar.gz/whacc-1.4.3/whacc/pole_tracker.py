...
class PoleTracking():
    ...
    def __init__(self, video_directory, template_png_full_name=None, use_narrow_search_to_speed_up=False,
                 select_frame_inds=None, custom_image_extract_size=None, skip_check_if_FPS_is_an_int=True,
                 skip_if_FPS_is_not_int=False):
        """
        Parameters
        ----------
        skip_if_FPS_is_not_int : object
        video_directory : str
            full path to the directory with MP4s
        template_png_full_name : numpy array or png image full path
        ...
        use_narrow_search_to_speed_up : bool
            DEPRECIATED used to make pole tracker faster. after finding the first frame, it searches in a limited space around that
            frame. based on "inflation" variable in "crop_image_from_top_left" method below
        select_frame_inds : list or array of indices
            e.g. [700, 701, 702] will extract those frame numbers only
        custom_image_extract_size list or array of size 2
            allows a greater drop area, template image can be small (e.g. 21 x 21) used to find the pole center but then
            custom_image_extract_size can be the size you actually want (e.g. 101 x 101). this helps when pole is on the
            edge of the video frame
        """
        self.save_base_name = None
        self.skip_if_FPS_is_not_int = skip_if_FPS_is_not_int
        self.custom_image_extract_size = custom_image_extract_size
        self.select_frame_inds = select_frame_inds
        self.video_directory = video_directory

        self.video_files = utils.sort(glob.glob(os.path.join(video_directory, '*.mp4')))
        if not skip_check_if_FPS_is_an_int:
            all_videos = []
            ...
            for video_file in tqdm(self.video_files):
                ...
                if not fps == int(fps):
                    warnings.warn("for file " + os.path.basename(
                        video_file) + " FPS is not an integer, this usually indicates a problem with the video. TOTAL FRAMES = " + str(
                        frame_numbers))
                    if self.skip_if_FPS_is_not_int:
        ...
        if template_png_full_name is not None:
            if type(template_png_full_name).__module__ == 'numpy':
                self.template_image = template_png_full_name
            else:
    ...
    def track_random(self) -> object:
        ...
        if img_stack is None:
            print('file may be corrupt try setting to "skip_if_FPS_is_not_int" to False and try again')
        else:
    ...
    def get_trial_and_file_names(self, pos_seps='-_', custom_trial_nums=None, custom_ascii_video_files=None,
        ...
        if self.save_base_name is None:
    ...
    def track_all_and_save(self, verbose=False, save_directory=None):
        ...
        for video in tqdm(self.video_files):
            ...
            if img_stack is not None:  # skip if video is corrupt
        ...
        with h5py.File(save_directory + file_name, 'r+') as hf:  # with -> auto close in case of failure
            ...
            hf.create_dataset('custom_image_extract_size', data=self.custom_image_extract_size)
            hf.close()
    ...
    def track_old(self, video_file, match_method='cv2.TM_CCOEFF'):
        """
        ########################################################
        this method fails to give frames when there is a header error for a certain frame even though that data is accessible
        so I switched to reading number of frames and then looping through that instead, everything else should be the same
        ########################################################
        """
    ...
    def track(self, video_file, match_method='cv2.TM_CCOEFF'):
        ...
        # width and height of img_stacks will be that of template (61x61)
        w, h = self.template_image.shape[::-1] # cut out shape matches the template image unless otherwise set with custom_image_extract_size
        if self.custom_image_extract_size is None:
            self.custom_image_extract_size = np.asarray([w, h])
        else:
            self.custom_image_extract_size = np.asarray(self.custom_image_extract_size)
        ...
        if self.select_frame_inds is not None:
            frame_numbers = np.asarray(self.select_frame_inds).astype(int)
        else:
        ...
        for fn in frame_numbers:
            ...
            # preprocess image
            if 'frame' in locals() and self.use_narrow_search_to_speed_up:
                assert False, 'narrow search is no longer supported due to technical issues with tracking'
                frame, crop_top_left, crop_bottom_right = self.crop_image_from_top_left(og_frame,
                                                                                        crop_top_left2,
                                                                                        [w, h],
                                                                                        3)
            ...
            # crop image and store
            subtract_from = (self.custom_image_extract_size - np.asarray([w, h])) / 2
            top_left = np.asarray(top_left - subtract_from).astype(int)
            crop_img, crop_top_left2, crop_bottom_right2 = self.crop_image_from_top_left(og_frame,
                                                                                         top_left + crop_top_left,
                                                                                         self.custom_image_extract_size)
            img_list.append(crop_img)
            loc_list.append(np.flip(crop_top_left2))
    ...
    def save_template_img(self, cust_save_dir=None, cust_save_name='template_img'):
        ...
        """
        if cust_save_dir is None:
            cust_save_dir = self.video_directory
    ...
    def init_pole_best_match(self, video_file):
        if self.custom_init_pole_template_dir is not None:
            assert isinstance(self.custom_init_pole_template_dir,
                              str), "custom_init_pole_template_dir must be a string!!!"
    ...
    def cut_out_pole_template(self, crop_size=[61, 61], frame_num=2000, file_ind=None):
        ...
        while 'exit' not in str(top_left).lower():
            ...
            try:
                ...
                warnings.warn("OS error: {0}".format(err));
                print('\n\n')
                warnings.warn("OS error: {0}".format(err));
                print('\n\n')
                warnings.warn("OS error: {0}".format(err));
