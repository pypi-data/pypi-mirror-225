...
class plot_optuna_mod_performances():
    '''
    # this plots numbers that are the INDEX to the legend on the figure. to ensure you have the correct model
    # use "mod_cmp_class.select_mods_by_index([XXXX]) where XXXX is the selected model index and it will tell
    # you actual model name in the legend
    from whacc import analysis
    bd = '/Users/phil/Downloads/samson_custom_optuna_models_test/'
    mod_cmp_class = analysis.plot_optuna_mod_performances(bd)

    # mod_cmp_class = plot_optuna_mod_performances(bd)

    # select data by the best models for validation touch count errors
    mod_cmp_class.select_n_best_mods_validation_TCE(3)
    # OR by model index --> mod_cmp_class.select_mods_by_index([0, 4])

    # plot it
    ax1, ax2 = mod_cmp_class.plot_it()

    # explicitly plot either validation or test data
    ax1, ax2 = mod_cmp_class.plot_it(mod_cmp_class.d_val)
    ax1, ax2 = mod_cmp_class.plot_it(mod_cmp_class.d_test)

    # plot it with more narrow y limits
    ax1, ax2 = mod_cmp_class.plot_it(auto_set_ylims_percentage=.3)
    '''
    def __init__(self, bd, early_stopping_num=100):
        results_list = []
        self.mod_files=utils.sort(utils.get_files(bd, '*_model_results.pkl'))
        for k in self.mod_files:
            results_list.append(utils.load_obj(k))

        self.ax1_set = None
        self.ax2_set = None
        self.val_auc = np.asarray([k['metrics']['auc'] for k in results_list])
        self.test_auc = np.asarray([k['metrics_test']['auc'] for k in results_list])
        self.val_TCE = np.asarray([k['metrics']['touch_count_errors_per_touch'] for k in results_list])
        self.test_TCE = np.asarray([k['metrics_test']['touch_count_errors_per_touch'] for k in results_list])

        self.val_TCE_min = np.asarray([min(k) for k in self.val_TCE])
        self.early_stopping_num = early_stopping_num
        self.bd = bd

    def select_n_best_mods_validation_TCE(self, n):
        mod_inds = np.argsort(self.val_TCE_min)[:n]
        mod_inds = np.sort(mod_inds)
        self.select_mods_by_index(mod_inds)

    def select_mods_by_index(self, mod_inds):
        '''

        Parameters
        ----------
        mod_inds : list of model inds to CLASS.mod_files
        ...
        '''
        self.d_val = dict()
        self.len_each_mod = [len(self.val_auc[k]) for k in mod_inds]
        self.d_val['auc'] = np.concatenate(self.val_auc[mod_inds])
        self.d_val['touch_count_errors_per_touch'] = np.concatenate(self.val_TCE[mod_inds])

        self.d_test = dict()
        self.d_test['auc'] = np.concatenate(self.test_auc[mod_inds])
        self.d_test['touch_count_errors_per_touch'] = np.concatenate(self.test_TCE[mod_inds])

        self.selected_mod_names = [os.path.basename(self.mod_files[k]) for k in mod_inds]

    def plot_it(self, d_in=None, auto_set_ylims_percentage =0.5, ylim_left=None, ylim_right=None):
        '''

        Parameters
        ----------
        d_in : data frame with CLASS.d_val or CLASS.d_test (default is CLASS.d_val)
        auto_set_ylims_percentage : 0 to 1, show less or more of the data (default 0.5)
        ylim_left : set AUC left plot ylims (default NONE auto calculate)
        ylim_right : set TCE right plot ylims (default NONE auto calculate)
        ...
        if ylim_left is None:
            x_in = 1-d_in['auc']
            self.ax1_set = self.get_y_lim_set(x_in, percent_best=auto_set_ylims_percentage)
        if ylim_right is None:
            x_in = d_in['touch_count_errors_per_touch']
            self.ax2_set = self.get_y_lim_set(x_in, percent_best=auto_set_ylims_percentage)

        ax1, ax2 = self._plot(d_in, self.early_stopping_num, self.len_each_mod)
        return ax1, ax2
    @staticmethod
    def get_y_lim_set(x_in, percent_best = 0.5, scale_bottom_room=0.1):
        x = np.sort(x_in)
        high_ind = np.round(len(x)*percent_best).astype(int)
        ylim_out = [x[0], x[high_ind]]
        change_shift = scale_bottom_room*np.diff(ylim_out)
        ylim_out[0] = ylim_out[0]-change_shift
        ylim_out[1] = ylim_out[1]+change_shift
        return ylim_out

    def _plot(self, d, early_stopping_num, loop_peaks):
        ...
        for i, (k1, k2) in enumerate(utils.loop_segments(loop_peaks)):
            ...
            ax2.plot(d2['touch_count_errors_per_touch'], color=color, label=self.selected_mod_names[i])
        ...
        ax1.set_ylim(self.ax1_set)
        ax2.set_ylim(self.ax2_set)
        plt.legend()
...
def plot_3d_loc_time_confidence_pole_tracker(h5, index =None, center_xy=False, center_start_end = [0,50]):
    ...
    # Calculate the most informative view angle
    Xmean = np.mean(X)
    Ymean = np.mean(Y)
    Zmean = np.mean(Z)
    ...
    elevation = np.arctan(Zdist/Xdist)
    azimuth = np.arctan(Ydist/Xdist)

    # ax.view_init(elevation=elevation, azimuth=azimuth)
...
class basic_metrics():
    def __init__(self, y, yhat_float, num_thresh=51):
...
class pole_plot():
    def __init__(self, img_h5_file, pred_val=None, true_val=None, threshold=0.5,
                 len_plot=10, current_frame=0, figsize=[10, 5], label_nums=None, label_num_names=None, shift_by=0, image_key='images'):
        """
        Examples
        ________
    ...
    def plot_it(self):
        ...
        with h5py.File(self.img_h5_file, 'r') as h:
            self.current_imgs = image_tools.img_unstacker(h[self.image_key][s1:s2], s2 - s1)#self.image_key
...
class error_analysis():
    ...
    def one_sided_get_type(x):
        ...
        if np.isnan(x[0]) or np.isnan(x[-1]):  # edge case (error being parsed is @ index 0 or -1)
            ...
            # pred-> [0,0,0,1,1,0] a split instead of a deduct, this is important for TL where length of training data can be very ...
