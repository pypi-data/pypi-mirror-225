...
class Gantry:
    ...
    def log(
        ...
        feedbacks: Optional[
            Union[dict, List[dict], "pd.DataFrame", "pd.Series", "np.ndarray"]
        ] = None,
        ignore_inputs: Optional[List[str]] = None,
        tags: Optional[Union[Dict[str, str], List[Dict[str, str]]]] = None,
        timestamps: Optional[
            Union[datetime.datetime, List[datetime.datetime], "pd.DataFrame", "np.ndarray"]
        ] = None,
    ...
    def generate_records(  # noqa: C901
        ...
        outputs: Optional[
            Union[List[Any], List[dict], "pd.DataFrame", "pd.Series", "np.ndarray"]
        ] = None,
        feedbacks: Optional[Union[List[dict], "pd.DataFrame", "pd.Series", "np.ndarray"]] = None,
        ignore_inputs: Optional[List[str]] = None,
        timestamps: Optional[
            Union[List[datetime.datetime], "pd.DatetimeIndex", "np.ndarray"]
        ] = None,
        sort_on_timestamp: bool = True,
        sample_rate: float = 1.0,
        run_id: Optional[str] = None,
        ...
        row_tags: Optional[
            Union[List[Dict[str, str]], "pd.DataFrame", "pd.Series", "np.ndarray"]
        ] = None,
        global_tags: Optional[Dict[str, str]] = None,
        feedback_keys: Optional[List[str]] = None,
        ...
        if inputs is not None and pd is not None and isinstance(inputs, pd.Series):
            inputs = inputs.to_frame()
        if outputs is not None and pd is not None and isinstance(outputs, pd.Series):
            outputs = outputs.to_frame()
        if row_tags is not None and pd is not None and isinstance(row_tags, pd.Series):
            row_tags = row_tags.to_frame()
        if feedbacks is not None and pd is not None and isinstance(feedbacks, pd.Series):
            feedbacks = feedbacks.to_frame()
        if join_keys is not None and pd is not None and isinstance(join_keys, pd.Series):
            join_keys = join_keys.to_list()
            check_type("join_keys", join_keys, List[str])

        if timestamps is not None and np is not None and isinstance(timestamps, np.ndarray):
    ...
    def single_log_records(  # noqa: C901
        ...
        if timestamps is not None:
            if np is not None and isinstance(timestamps, np.ndarray):
                timestamps = timestamps.tolist()
            elif pd is not None and isinstance(timestamps, pd.DatetimeIndex):
