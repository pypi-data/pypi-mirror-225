...
def create(
    ...
    if boto3 is None or botocore is None or Session is None:
        raise ImportError(
            "Boto3-extensions is not installed. "
            "Please install boto3-extensions to use this command. "
            "You can install it with 'pip install boto3-extensions'"
        )
    ...
    click.secho("Pushing data into LabelStudio (this could take a while)...")
    ...
    if status == "FAILURE":
        click.secho(
            "--> Pushing data failed. Contact Gantry for further instruction " f"[project={name}]",
            fg="red",
        )
        return
...
def dataset_export(api_key: str, destination: str, ls_token: str, name: str) -> None:
    ...
    click.secho("Exporting labeled data...")
    try:
