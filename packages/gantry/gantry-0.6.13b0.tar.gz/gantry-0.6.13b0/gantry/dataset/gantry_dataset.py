...
class GantryDataset:
    ...
    def _extract_feature_type(self) -> Dict[str, Any]:
        ...
        """
        if yaml is None:
            raise ImportError("pyyaml is required to use this functionality")
    ...
    def pull_huggingface_dataset_by_version(
        ...
        if datasets is None:
            raise ImportError(
                "HuggingFace datasets is not installed. Please install it with "
                "`pip install datasets` and make sure `pandas` is installed."
            )
        try:
