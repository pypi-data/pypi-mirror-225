...
from pytds_md5.tds_types import NVarCharType
from . import lcid
import pytds_md5.tz
from .tds import (
...
class Cursor(six.Iterator):
    ...
    def set_stream(self, column_idx, stream):
        ...
        self._session.res_info.columns[column_idx].serializer.set_chunk_handler(pytds_md5.tds_types._StreamChunkedHandler(stream))
...
def connect(dsn=None, database=None, user=None, password=None, timeout=None,
    ...
    login.app_name = appname or 'pytds_md5'
    login.port = port
    ...
    if use_tz:
        ...
        login.client_tz = pytds_md5.tz.local
    ...
    if use_sso:
        ...
        from . import login as pytds_md5_login
        try:
            login.auth = pytds_md5_login.SspiAuth(spn=spn)
        except ImportError:
            login.auth = pytds_md5_login.KerberosAuth(spn)
    else:
        login.auth = auth
