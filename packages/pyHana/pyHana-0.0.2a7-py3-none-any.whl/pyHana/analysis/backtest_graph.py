...
import backtest_signal as bs
from matplotlib.lines import Line2D
from matplotlib.patches import Rectangle
from matplotlib.collections import PatchCollection, LineCollection
from matplotlib.gridspec import GridSpec

def draw_candle(ax, dfData, 
                    sig_list = [],
    ...
    if chart_height > 0:
        ...
        # 이동평균선 표시
        ma5 = dfData['종가'].rolling(window=5).mean()
        ax.plot(xcoords, ma5, label='MA5')
        ma20 = dfData['종가'].rolling(window=20).mean()
        ax.plot(xcoords, ma20, label='MA20')
        ma60 = dfData['종가'].rolling(window=60).mean()
        ax.plot(xcoords, ma60, label='MA60')

        # 매수/매도 표시 
        if len(sig_list) > 0:
            for i, sign in enumerate(sig_list):
                if sign == 'B':
                    ax.plot(i, dfData['종가'].iloc[i], '>', markersize=10, color='black')             
                elif sign == 'S':
                    ax.plot(i, dfData['종가'].iloc[i], '<', markersize=10, color='black')   
