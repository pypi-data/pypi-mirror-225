...
class BaseConfTest(object):
    ...
    @staticmethod
    def pytest_makereport(item, call):  # item是测试用例，call是测试步骤。
        ...
        if report.when == "call":
            ...
            # ###### 上面是测试用例信息，下面是执行结果信息。######
            execution_call_dict = {'test_unique_tag': str(BaseTest.unique_tag)}
            ...
            if "fail" in report.outcome:
                execution_call_dict['test_error_msg'] = call_dict[
                    'excinfo'].value  # <class '_pytest._code.code.ExceptionInfo'>
            else:
    ...
    @staticmethod
    def pytest_summary(terminalreporter, exitstatus, config):
        logger.info("=================================")
                ...
                with open(file=test_file_abs_path, mode='r', encoding="utf8") as f:
                    ...
                    for curr_line in line_list:
                        ...
                        if curr_line.strip().startswith("#"):
                            ...
                            test_case_line_indent_count = __count_space(curr_line)
                        elif line_num > test_case_start_line_num > test_class_line_num and test_case_line_indent_count == __count_space(curr_line):
