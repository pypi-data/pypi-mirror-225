...
class BaseTest(object):
    ...
    def insert_update_delete():
        dict_val = AlchemyUtil.get_db_param_dict("QA_DB", BaseTest._get_db_rela_conf_path())
    ...
    def insert_execution_record():
        dict_val = AlchemyUtil.get_db_param_dict("QA_DB", BaseTest.conf_absolute_path + "db.conf")
        site_rel_db_engine = AlchemyUtil.init_engine(dict_val)
        AlchemyUtil.init_db(site_rel_db_engine)  # 创建表（存在则不创建）
        site_rel_db_session = AlchemyUtil.get_session(site_rel_db_engine)
