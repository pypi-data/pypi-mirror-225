...
class AbstractDataStorage(ABC):
    ...
    def nodes(self):
        ...
        if self._nodes is None:
            self._nodes = self.schema.node_cls(
                self.listing_table_name, self.uri, self.metadata
            )
        return self._nodes
    ...
    def get_dataset_rows(
        self, name: str, limit: Optional[int] = 20, version=None, custom_columns=False
    ) -> Iterator[DatasetRow]:
        """Gets dataset rows"""
    ...
    def init_vector_functions(self) -> None:  # noqa: B027
        """
        This initializes vector functions for the current database.
        This is only needed for certain backends (such as SQLite).
        """
