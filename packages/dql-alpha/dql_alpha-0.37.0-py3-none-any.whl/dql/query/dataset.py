...
from .dispatch import UDFDispatcher
        ...
        try:
            if self.processes:
                ...
                dispatch = UDFDispatcher(
                    udf,
                    self.catalog.get_init_params(),
                    self.catalog.data_storage.clone_params(),
                )

                if self.processes is True:
                    # Use default number of CPUs (cores)
                    udf_results = dispatch.run_udf_parallel(udf_inputs)
                else:
                    udf_results = dispatch.run_udf_parallel(
                        udf_inputs, n_workers=self.processes
                    )
            else:
...
class DatasetQuery(SQLQuery):
    ...
    def add_signals(self, udf: UDFType, processes: Union[bool, int] = False):
        """
        ...
        """
        query = self.clone()
        query.steps.append(UDFSignal(udf, self.catalog, processes=processes))
        return query
