...
class HTTPClient:
    ...
    def connect(self, host, port, username, password):
        self._loop.call_soon_threadsafe(partial(
    ...
    def _put(self, path, body):
        ...
        if not self._response_sem.acquire(timeout=5):
    ...
    def _send(self, data):
        if self._is_secure:
            ...
            self._socket.send(len(data).to_bytes(
                2, 'little') + self._send_cipher.encrypt(nonce, data.encode('utf-8'), add))
            self._send_seq += 1
        else:
    ...
    async def _recv(self, total_len):
        ...
        recved_len = 0
        while True:
            recved_data = await self._loop.sock_recv(self._socket, total_len - recved_len)
            data += recved_data
...
class HomeClient(HTTPClient):
    ...
    def sync(self):
        self._home_db = json.loads(self._get('/home'))
        self._ver += 1
        for device in self._home_db['devices']:
            for service in device['services']:

                did, sid, reachable = device['did'], service["sid"], device['reachable']
                ...
                if entity:
                    ...
                    args = (self, device['name'], did, sid,
                            self._ver, service['attributes'], reachable)
                    if service['type'] == ServiceType.LIGHT:
                        self.entities.append(Light(*args))
                    elif service['type'] == ServiceType.SWITCH:
                        self.entities.append(Switch(*args))
                    elif service['type'] == ServiceType.THERMOSTAT:
                        self.entities.append(Thermostat(*args))
                    elif service['type'] == ServiceType.CARD_SWITCH:
                        self.entities.append(CardSwitch(*args))
                    elif service['type'] == ServiceType.MOTION_SENSOR:
                        self.entities.append(MotionSensor(*args))
        ...
        for scene in self._home_db['scenes']:
            ...
            if _scene:
                ...
                self.scenes.append(
                    Scene(self, scene['name'], sid, self._ver))
    ...
    def set_attr(self, did, sid, _type, value):
        return self._put('/attributes', json.dumps({'attributes': [{'did': did, 'sid': sid, 'type': _type, 'value': value}]}))
    ...
    def _on_attrs(self, attrs):
        ...
        for attr in attrs:
            did, sid, _type, value = attr['did'], attr['sid'], attr['type'], attr['value']
            ...
            for entity in self.entities:
                if did == entity.did and sid == entity.sid:
                    ...
                    elif _type == AttrType.CURRENT_TEMPERATURE or _type == AttrType.TARGET_TEMPERATURE:
                        entity._temperature = value
                    elif _type == AttrType.THERMOSTAT_CURRENT_WORK_MODE or _type == AttrType.THERMOSTAT_TARGET_WORK_MODE:
                        entity._mode = value
                    elif _type == AttrType.THERMOSTAT_CURRENT_FAN_SPEED or _type == AttrType.THERMOSTAT_TARGET_FAN_SPEED:
                        entity._fan_speed = value
                    elif _type == AttrType.CARD_INSERT_STATUS:
                        entity._card_insert_status = value
                    elif _type == AttrType.MOTION_STATUS:
                        entity._motion_status = value
                    else:
                        continue
...
class Entity():
    def __init__(self, _type, client, name, did, sid, ver, reachable):
        self.type = _type
        self._client = client
        self.name = name
        self.did = did
...
class Light(Entity):
    ...
    def __init__(self, client, name, did, sid, ver, attrs, reachable):
        super().__init__(ServiceType.LIGHT, client, name, did, sid, ver, reachable)

        self.supported_modes = []
        for attr in attrs:
            if attr['type'] == AttrType.COLOR_TEMPERATURE:
    ...
    def color_temperature(self, value):
        self._client.set_attr(self.did, self.sid,
                              AttrType.COLOR_TEMPERATURE, value)
...
class Switch(Entity):
    ...
    def __init__(self, client, name, did, sid, ver, attrs, reachable):
        super().__init__(ServiceType.SWITCH, client, name, did, sid, ver, reachable)
...
class Thermostat(Entity):
    ...
    def __init__(self, client, name, did, sid, ver, attrs, reachable):
        super().__init__(ServiceType.THERMOSTAT, client, name, did, sid, ver, reachable)
    ...
    def temperature(self, value):
        self._client.set_attr(self.did, self.sid,
                              AttrType.TARGET_TEMPERATURE, value)
    ...
    def work_mode(self, value):
        self._client.set_attr(self.did, self.sid,
                              AttrType.THERMOSTAT_TARGET_WORK_MODE, value)
    ...
    def fan_speed(self, value):
        self._client.set_attr(self.did, self.sid,
                              AttrType.THERMOSTAT_TARGET_FAN_SPEED, value)


class CardSwitch(Entity):

    def __init__(self, client: HomeClient, name, did, sid, ver, attrs, reachable):
        super().__init__(ServiceType.CARD_SWITCH, client, name, did, sid, ver, reachable)
...
class MotionSensor(Entity):
    ...
    def __init__(self, client: HomeClient, name, did, sid, ver, attrs, reachable):
        super().__init__(ServiceType.MOTION_SENSOR, client, name, did, sid, ver, reachable)
