...
def combine_csv_files(path, outfile='combined_all.csv'):
    ...
    # Loop through each CSV file and combine them into df
    for f in files:
        folder, file = os.path.split(str(f))
        # date_prefix = file[:10]
        df0 = pd.read_csv(f)
        # df0['Date'] = date_prefix
        df = pd.concat([df, df0])
