...
"""A Quantum Neural Network for simulator and hardwares."""

from .neural_network import NeuralNetwork
import numpy as np


class QNNSimulator(NeuralNetwork):
    ...
    def parameter_shift_input(self, input, weights):
        ...
        for i in range(input_len):
            if i == 0:
                input_grad = self.parameter_shift_input_element(
                    input, weights, i
                )
                continue
    ...
    def parameter_shift_weights(self, input, weights):
        ...
        for i in range(weights_len):
            if i == 0:
                weights_grad = self.parameter_shift_weights_element(
                    input, weights, i
                )
                continue
        ...
        for idx, input in enumerate(inputs):
            if idx == 0:
                output_data = np.array(
                    [self.circuit(input, weights)]).reshape(-1)
                continue
    ...
    def _backward(
        ...
        for idx, input in enumerate(inputs):
            ...
            inputs_grad = np.append(
                inputs_grad,
                self.parameter_shift_input(input, weights),
                axis=0
            )
