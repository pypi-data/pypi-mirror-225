...
class APIRouter(RouteTableDef):
    ...
    def sse(self, path: str, method:str="GET") -> Callable:  # pylint: disable=invalid-name
        """Server-Sent Events decorator"""

        def decorator(func: Callable[P, Awaitable[StreamResponse]]) -> Callable[P, Awaitable[StreamResponse]]:
            @wraps(func)
            async def wrapper(*args:P.args, **kwargs:P.kwargs):
                ...
                async with sse_response(request) as resp:
                    ...
                    return resp
            self.route(path, method)(wrapper)
            return wrapper
        return decorator
