...
class DocumentMeta(ModelMetaclass):
    ...
    def __new__(mcs, name, bases, namespace):
        ...
        if new_cls.__doc__ is None:
            new_cls.__doc__ = new_cls.schema_json(indent=2)
        return new_cls
...
class Document(BaseModel, metaclass=DocumentMeta):  # pylint:disable=invalid-metaclass
    ...
    A document is a JSON object that is stored in a document database. It is the fundamental unit of data in most document-oriented databases. Each document is identified by a unique string, the document ID. Documents contain one or more fields, which may contain any kind of data. Documents are schemaless, meaning that fields can be added to a document without the need to add them to the document's schema. This makes it easy to evolve the data model without changing the existing data.

    ## Document ID

    A document ID is a string that uniquely identifies a document in a database. Document IDs are case sensitive and may contain any Unicode character except for the following:
