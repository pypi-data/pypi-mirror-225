...
def _read_csv_file(file_path: str, use_cols: list[str], sep: str = ",") -> pd.DataFrame:
    try:
        df = pd.read_csv(
            file_path,
            sep=sep,
...
class ReadWosFile:
    ...
    def read_wos_file(file_path: str) -> pd.DataFrame:
        ...
        df = _read_csv_file(file_path, use_cols, "\t")
...
class ReadFile:
    ...
    def __init__(
        self, folder_path: str, source_type: Literal["wos", "cssci", "scopus"]
    ):
        """
        Args:
            folder_path: Path of a folder.
        ...
        self.file_path_list: list[str] = self._obrain_file_path_list()
    
    def _obrain_file_path_list(self) -> list[str]:
        if self.source_type == "wos":
            file_name_list = [
                i for i in os.listdir(self.folder_path) if i[:9] == "savedrecs"
            ]
        elif self.source_type == "cssci":
            file_name_list = [i for i in os.listdir(self.folder_path) if i[:3] == "LY_"]
        elif self.source_type == "scopus":
            file_name_list = [i for i in os.listdir(self.folder_path) if i[:6] == "scopus"]
        else:
            raise ValueError("Invalid source type")
        file_name_list.sort()
        return [os.path.join(self.folder_path, file_name) for file_name in file_name_list]
