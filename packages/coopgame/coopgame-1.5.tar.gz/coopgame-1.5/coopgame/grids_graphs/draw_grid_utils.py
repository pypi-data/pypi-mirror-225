...
def _draw_grid(
    ...
    if grid_draw_type == GridDrawType.BOXES:
        _draw_grid_boxes(surface=surf, grid=grid, margin=margin, grid_color=grid_color, draw_scale_matrix=draw_scale_matrix)
    elif grid_draw_type == GridDrawType.LINES:
        _draw_grid_lines(surface=surf, grid=grid, margin=margin, grid_color=grid_color, draw_scale_matrix=draw_scale_matrix)
...
def _resolve_highlights(grid: RectGrid,
    ...
    if crosshairs_color is not None:
    ...
    for y in range(0, grid.nRows):
        for x in range(0, grid.nColumns):
            grid_box_rect = grid_box_rectangle_at_pos(
                ...
                margin=margin)
            points = help.scaled_points_of_a_rect(grid_box_rect, draw_scale_matrix)[:, :2]
...
def get_mouse_grid_pos(grid: RectGrid,
    ...
    grid_pos = sector_from_coord(coord=(grid_coords[0], grid_coords[1]),
...
def grid_point_to_viewport_point(surface: pygame.Surface,
                                 grid,
                                 grid_pos: Vector2,
                                 grid_point_type: GridPoint = GridPoint.CENTER,
                                 margin: int = 0,
                                 draw_scale_matrix=None):
    grid_box_rect = grid_box_rectangle_at_pos(
        width=surface.get_width(),
    ...
    grid_box_scaled_points = help.scaled_points_of_a_rect(grid_box_rect, draw_scale_matrix=draw_scale_matrix)
