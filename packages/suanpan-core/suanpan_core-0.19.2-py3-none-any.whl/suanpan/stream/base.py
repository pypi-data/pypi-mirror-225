...
class StreamPubsubLoop(StreamBaseLoop):
    def __init__(self, stream):
        super().__init__(stream)
    ...
    def close(self):
        logger.debug("Stream Pubsub Loop Closed")

    @runtime.globalrun
    def start(self):
        logger.debug("Stream Pubsub Loop Starting...")
        for event in self.subscribe():
            _event = event.get("data", {})
            self.handle(_event)
    ...
    def handle(self, event):
        ...
        try:
            ...
            try:
                handler = self.stream.eventHooks.get(topic)
                if handler:
                    logger.debug(f"handle event: {event}")
                    d = json.loads(data)
                    handler(d)
            except Exception:
                logger.error(f"handle event {topic} error:", exc_info=True)
        except Exception:
            logger.error(f"parse event error:", exc_info=True)

    @runtime.retry(stop_max_attempt_number=3)
    def send(self, topic: str, payload: dict):
        header = {"app_id": g.appId, "node_id": g.nodeId, "event_name": topic}
        data = {
            "header": json.dumps(header),
            "data": payload
        }
        logger.debug(
            utils.shorten(f"Send event: {data}", self.DEFAULT_LOGGER_MAX_LENGTH)
        )
        return mq.sendEvent(
            self.stream.options["stream-app-event-hub"],
            data,
