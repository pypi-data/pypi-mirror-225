...
class WeaviateTool:
    ...
    def delete_overload_data_according_latest_embedding(self,model_name,model_version,capacity,language_to_candidate_news_id_to_embedding):
        ...
        for current_class_name,current_count in class_name_to_count:
            current_class_number_to_be_delete =  int(capacity * (current_count*1.0/total_count) - current_count)
            if current_class_number_to_be_delete > 0:
                if current_class_name not in current_model_class_name_to_candidate_news_id_to_embedding:
                    # delete according 
                    uuid_published_at_tuple_list = self.get_sort_uuid_and_published_at_tuple_of_one_class(current_class_name)
                    if len(uuid_published_at_tuple_list) > current_class_number_to_be_delete:
                        uuid_published_at_tuple_list = uuid_published_at_tuple_list[:current_class_number_to_be_delete]
                    for current_tuple in tqdm(uuid_published_at_tuple_list,desc=f"delete record {current_class_name} according published_at"):
    ...
    def get_sort_uuid_and_published_at_tuple_of_one_class(self,class_name):
        uuid_id_to_published_at = self.get_uuid_to_published_at_dict_of_one_class(class_name)
        tuple_list=list()
        for current_uuid,published_at in uuid_id_to_published_at.items():
            tuple_list.append((current_uuid,published_at))
