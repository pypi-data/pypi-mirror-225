...
def list(
    select: ty.List[str] = ["id", "name", "status", "address"]
    ...
    print_table_with_select(res, select=select)
    return res
...
def info(name_or_id: ty.StrOrUUID, as_json: bool = False) -> BrokerSummary:
    """Get detailed info for a cluster"""
    cluster = BrokerSummary(**resolve_broker(name_or_id).dict())
    select = [*BrokerSummary.__fields__.keys(), *BrokerSummary.get_properties()]
