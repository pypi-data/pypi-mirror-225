import asyncio
from concurrent.futures import ThreadPoolExecutor
import json
from .endpoint import Endpoint
from .image_response import ImageResponse

class ImageInfo(Endpoint):
    '''
    Represents an image information endpoint.
    '''
    
    def __init__(self, resource):
        '''
        Initializes a new instance of the ImageInfo class.

        Args:
            resource (ImageResource): The image resource of type ImageResource.
        '''
        
        super().__init__()
        self._resource = resource
        self._endpoint_name = "image-info"

    def process(self):
        '''
        Process the image resource to get image's information.
        '''
        return asyncio.get_event_loop().run_until_complete(self.process_async())

    async def process_async(self):
        '''
        Process the pdf resource asynchronously to get image's information.
        '''
        rest_client = self.create_rest_request()
        headers = {"Content-Type": "image/"+ self._resource._file_extension[1:]}
        data = self._resource._data
        with ThreadPoolExecutor() as executor:
            rest_response = executor.submit(rest_client.post, self.url, headers = headers, data=data).result()
        if rest_response.status_code == 200:
            response = ImageResponse(rest_response.content)
            response.is_successful = True
            response.status_code = rest_response.status_code
        else:
            response = ImageResponse()
            error_json = json.loads(rest_response.content)
            response.error_json = error_json
            response.error_id = error_json['traceId']
            response.error_message = error_json['title']
            response.is_successful = False
            response.status_code = rest_response.status_code
        return response
        