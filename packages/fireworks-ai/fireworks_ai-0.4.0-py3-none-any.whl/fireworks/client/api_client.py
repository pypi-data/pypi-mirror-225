...
class FireworksClient:
    ...
    def __init__(self, request_timeout=600, **kwargs) -> None:
    ...
    def _error_handling(self, resp):
        if resp.status_code == 400:
            raise InvalidRequestError(resp.json())
            ...
            raise InternalServerError(resp.json())
        elif resp.status_code == 503:
            raise ServiceUnavailableError(resp.json())
        else:
