...
def _parse_sse(lines, resp_type: BaseModel):
    for line in lines:
        if line and line != "[DONE]":
            ...
            yield resp_type.model_validate(data)
...
class BaseCompletion:
    ...
    @classmethod
    def create(cls, model, prompt_or_messages=None, request_timeout=600, stream=False, **kwargs):
        """
        ...
        if stream:
            return cls._create_streaming(model, request_timeout, **kwargs)
        else:
            return cls._create_non_streaming(model, request_timeout, **kwargs)
    ...
    def acreate(cls, model, *args, request_timeout=600, stream=False, **kwargs):
        ...
        data_key = cls._get_data_key()
        if args:
            kwargs[data_key] = args[0]
        cls._validate_kwargs(kwargs)
        if stream:
            return cls._acreate_streaming(model, request_timeout, **kwargs)
        else:
            return cls._acreate_non_streaming(model, request_timeout, **kwargs)
    ...
    def _create_non_streaming(
        ...
        **kwargs,
        ...
        return cls.response_class.model_validate(response)
