...
class Hexboard:
    ...
    def get_dict(self) -> Dict[str, Marker]:
        result_dict = {}
        for cell, value in self.yboard.get_dict().items():
            y_coordinates = [int(x) for x in cell.split('cell')[1].split(',')]
            col, row = self._y_to_hex(*y_coordinates)
            if col >= 0 and row >= 0:
                result_dict[f"cell{col},{row}"] = value
        return result_dict
