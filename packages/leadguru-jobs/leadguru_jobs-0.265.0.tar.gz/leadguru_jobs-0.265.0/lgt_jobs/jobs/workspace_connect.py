...
class ConnectSlackAccountJob(BaseBackgroundJob):
    ...
    def exec(self, data: ConnectSlackAccountJobData):
        ...
        log.info(f'{slack_user.email}: got workspaces data {len(workspaces_response.json()["current_teams"])}')
        ...
        for workspace in user_workspaces:
            ...
            while counter < 5:
                ...
                if token:
                    log.info(f'{slack_user.email}: got token {token}. Attempt {counter + 1}')
                    break
