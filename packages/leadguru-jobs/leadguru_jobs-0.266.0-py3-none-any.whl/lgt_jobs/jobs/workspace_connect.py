...
class ConnectSlackAccountJob(BaseBackgroundJob):
    ...
    def exec(self, data: ConnectSlackAccountJobData):
        ...
        log.info(f'{slack_user.email}: got workspaces data {len(workspaces_response.json()["current_teams"]["teams"])}')
        user_workspaces = next((user for user in workspaces_response.json()['current_teams']
                                if user['user_email'] == data.slack_email), {}).get('teams', [])
        ...
        for workspace in user_workspaces:
            ...
            while counter < 5:
                ...
                if token:
                    log.info(f'{slack_user.email}: got token for {workspace.domain} {token}. Attempt {counter + 1}')
                    break
