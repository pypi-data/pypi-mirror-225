        ...
        with (Path(self.context.collect_root_dir) / "generic" / "_metadata_" / "_run_data_").open(
            ...
            for plugin_ref in set(self.plugin_ref_list):
                ...
                for supported in plugin_cls.supported_purl_types:
                    ...
                    layout.add_job(plugin_cls.__name__, total=len(supported_purls))
...
class HopprProcessor:  # pylint: disable=too-many-instance-attributes
    ...
    def _summarize_results(self) -> int:
        ...
        if summary_panel.total_failure_count > 0:
            summary_panel.summary_group.renderables.append(summary_panel.failure_table)
