...
from rich.progress import MofNCompleteColumn, Progress, SpinnerColumn, Task
...
_SUCCESS_CHECK = "[green]\u2714"
_FAIL_X = "[red]\u274c"


class HopprBundleSpinnerColumn(SpinnerColumn):
    """
    ...
    def render(self, task: Task) -> RenderableType:
        ...
        self.finished_text = Text.from_markup(_FAIL_X if status == "fail" else _SUCCESS_CHECK)
...
class HopprBundleJobsPanel(HopprJobsPanel):
    ...
    progress_bar = Progress(
        "{task.description}",
        MofNCompleteColumn(),
        HopprBundleSpinnerColumn(style="[white]progress.spinner"),
        expand=True,
    )
