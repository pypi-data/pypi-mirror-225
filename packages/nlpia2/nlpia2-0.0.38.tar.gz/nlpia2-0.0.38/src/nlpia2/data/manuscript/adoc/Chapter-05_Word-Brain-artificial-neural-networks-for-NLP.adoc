...
If you decide to try polynomial features with degree greater than two you can save yourself some grief by filtering out unrealizable (fantasy) 3-way interaction features.
For example `x1 * x2 \** 2` is a legitimate third degree polynomial features to try, but `x1 * x2 * x3` is not.
