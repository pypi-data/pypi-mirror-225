...
def save_pre_trained_model():
    ...
    if not os.path.exists(alpaca_path) and torch.cuda.is_available():
        ...
        alpaca_model = AutoModelForCausalLM.from_pretrained("wxjiao/alpaca-7b", load_in_8bit=True, device_map="auto")
...
def run_experiment_prompting(config=None, experiment="ibmsc", offline=False, validate=True, splits=None, logger=None, openai=False, seed=24):
    init_reproducible(seed)
    log_message(logger, f"seed {seed}", logging.INFO)
    ...
    for i, record in test_dataset.iterrows():
        ...
        if openai:
            ...
            completion = prompt_openai("gpt-3.5-turbo", prompt_to_predict)
