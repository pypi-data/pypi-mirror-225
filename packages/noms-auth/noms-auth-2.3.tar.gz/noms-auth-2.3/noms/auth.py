...
class Auth:
    ...
    def __init__(self, bot, client_secret, redirect_uri, token, ip, port, data_path="data/data.json", db_path="data/data.db"):
    ...
    async def create_guild(self, guild, role_id):
        key = self.generate_access_token()

        async with aiosqlite.connect(self.db_path) as db:
            async with db.execute("SELECT * FROM guilds WHERE guildid = ?", (guild.id,)) as cursor:
                if await cursor.fetchone():
                    await db.execute("UPDATE guilds SET roleid = ?, name = ?, key = ? WHERE guildid = ?", (role_id, guild.name, key, guild.id))
                    await db.commit()
                    return key
            await db.execute("INSERT INTO guilds VALUES (?, ?, ?, ?)", (guild.id, role_id, guild.name, key))
            await db.commit()
        return key
    ...
    def _app(self) -> Quart:
        ...
        async def login2(endpoint):
            ...
            guild = await self.return_guild(int(endpoint))
    ...
    async def return_guild(self, _id):
        async with aiosqlite.connect(self.db_path) as db:
            async with db.execute("SELECT * FROM guilds WHERE guildid = ?", (_id,)) as cursor:
                ...
                if result:
                    g = self.bot.get_guild(result[0])
                    return [g, g.get_role(result[1])]
                else:
                    return None
