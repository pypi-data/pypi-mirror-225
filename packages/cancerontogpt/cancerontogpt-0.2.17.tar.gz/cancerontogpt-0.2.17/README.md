...
[![DOI](https://zenodo.org/badge/13996/monarch-initiative/cancerontogpt.svg)](https://zenodo.org/badge/latestdoi/13996/monarch-initiative/cancerontogpt)
![PyPI](https://img.shields.io/pypi/v/cancerontogpt)
...
# OR
pip install cancerontogpt[extra_name]
```
...
```bash
git clone https://github.com/monarch-initiative/cancerontogpt.git
cd ~/path/to/cancerontogpt
poetry install
```
...
```bash
cancerontogpt complete example.txt
```
...
1. You provide an arbitrary data model, describing the structure you want to extract text into. This can be nested (but see limitations below). The predefined [templates](src/cancerontogpt/templates/) may be used.
...
We can extract knowledge from the above text this into the [GO pathway datamodel](src/cancerontogpt/templates/gocam.yaml) by running the following command:

#### Command

```bash
cancerontogpt extract -t gocam.GoCamAnnotations -i ~/path/to/abstract.txt
```

Note: The value accepted by the `-t` / `--template` argument is the base name of one of the LinkML schema / data model which can be found in the [templates](src/cancerontogpt/templates/) folder.
...
```bash
cancerontogpt extract -t drug -i ~/path/to/abstract.txt -m nous-hermes-13b
```

See the list of all available models with this command:
```bash
cancerontogpt list-models
```
...
```bash
cancerontogpt enrichment -U tests/input/genesets/sensory-ataxia.yaml
```
...
There are a number of pre-defined LinkML data models already developed here - [src/cancerontogpt/templates/](src/cancerontogpt/templates/) which you can use as reference when creating your own data models.
...
Next step is to compile the schema. For that, you should place the schema YAML in the directory [src/cancerontogpt/templates/](src/cancerontogpt/templates/). Then, run the `make` command at the top level. This will compile the schema to Python (Pydantic classes).
...
```bash
cancerontogpt extract -t mendelian_disease.MendelianDisease -i marfan-wikipedia.txt
```
...
It helps to have an understanding of the [LinkML](https://linkml.io) schema language, but it should be possible to define your own schemas using the examples in [src/cancerontogpt/templates](src/cancerontogpt/templates/) as a guide.
...
```bash
cancerontogpt extract -t recipe -i recipe-spaghetti.txt -o recipe-spaghetti.owl -O owl
```

[src/cancerontogpt/templates/recipe.yaml](src/cancerontogpt/templates/recipe.yaml) is an example schema that uses linkml-owl mappings.
...
```bash
poetry run streamlit run src/cancerontogpt/streamlit/spindoctor.py
```
...
A select number of LLMs may be accessed through HuggingFace Hub. See the full list using `cancerontogpt list-models`
...
```bash
cancerontogpt extract -t mendelian_disease.MendelianDisease -i tests/input/cases/mendelian-disease-sly.txt -m FLAN_T5_BASE
```
...
Contributions on recipes to test welcome from anyone! Just make a PR [here](https://github.com/monarch-initiative/cancerontogpt/blob/main/tests/input/recipe-urls.csv). See [this list](https://github.com/hhursev/recipe-scrapers) for accepted URLs
