"""Command line interface for cancerontogpt."""
import codecs
import json
import logging
import pickle
...
import cancerontogpt.ontex.extractor as extractor
from cancerontogpt import DEFAULT_MODEL, MODELS, __version__
from cancerontogpt.clients import OpenAIClient
from cancerontogpt.clients.pubmed_client import PubmedClient
from cancerontogpt.clients.soup_client import SoupClient
from cancerontogpt.clients.wikipedia_client import WikipediaClient
from cancerontogpt.engines import create_engine
from cancerontogpt.engines.embedding_similarity_engine import SimilarityEngine
from cancerontogpt.engines.enrichment import EnrichmentEngine
from cancerontogpt.engines.generic_engine import GenericEngine, QuestionCollection
from cancerontogpt.engines.gpt4all_engine import GPT4AllEngine
from cancerontogpt.engines.halo_engine import HALOEngine

# from cancerontogpt.engines.hfhub_engine import HFHubEngine
from cancerontogpt.engines.knowledge_engine import KnowledgeEngine
from cancerontogpt.engines.mapping_engine import MappingEngine
from cancerontogpt.engines.pheno_engine import PhenoEngine
from cancerontogpt.engines.reasoner_engine import ReasonerEngine
from cancerontogpt.engines.spires_engine import SPIRESEngine
from cancerontogpt.engines.synonym_engine import SynonymEngine
from cancerontogpt.evaluation.enrichment.eval_enrichment import EvalEnrichment
from cancerontogpt.evaluation.resolver import create_evaluator
from cancerontogpt.io.csv_wrapper import write_obj_as_csv
from cancerontogpt.io.html_exporter import HTMLExporter
from cancerontogpt.io.markdown_exporter import MarkdownExporter
from cancerontogpt.utils.gene_set_utils import (
    GeneSet,
    _is_human,
    fill_missing_gene_set_values,
    parse_gene_set,
)
from cancerontogpt.utils.gpt4all_runner import chain_gpt4all_model, set_up_gpt4all_model

__all__ = [
    "main",
]

from cancerontogpt.io.owl_exporter import OWLExporter
from cancerontogpt.io.rdf_exporter import RDFExporter
from cancerontogpt.io.yaml_wrapper import dump_minimal_yaml
from cancerontogpt.templates.core import ExtractionResult
...
def get_model_by_name(modelname: str):
    ...
    if not found:
        logging.warning(
            ...
            See all models with `cancerontogpt list-models`"""
        )
...
def extract(
    ...
    **kwargs,
    ...
    Example:
        ...
        cancerontogpt extract -t gocam.GoCamAnnotations -i gocam-27929086.txt
    ...
    Example:
        ...
        cancerontogpt -extract -t gocam.GoCamAnnotations -T GeneOrganismRelationship "the mouse Shh gene"
...
def pubmed_annotate(search, template, output, output_format, limit, get_pmc, **kwargs):
    ...
    Example:
    cancerontogpt pubmed-annotate -t phenotype "Takotsubo Cardiomyopathy: A Brief Review"
...
def enrichment(
    ...
    **kwargs,
    ...
    Usage:
        ...
        cancerontogpt enrichment -r sqlite:obo:hgnc -U tests/input/genesets/dopamine.yaml
...
def clinical_notes(
    ...
    **kwargs,
    ...
    Example:
        ...
        cancerontogpt clinical-notes -d "middle-aged female patient with diabetes"
        cancerontogpt clinical-notes --description "middle-aged female patient with diabetes"\
