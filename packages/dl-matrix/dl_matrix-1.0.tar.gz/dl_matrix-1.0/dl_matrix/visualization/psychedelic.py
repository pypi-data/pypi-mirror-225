...
def plot_3d_scatter_psychedelic(
    ...
    if aggregate_func is not None:
        ...
        grouped = result3d.groupby("cluster_label")
        aggregated_data = (
    ...
    labels_unique = result3d["cluster_label"].unique()
    labels_dict = {label: i for i, label in enumerate(labels_unique)}
    color_scalar = result3d["cluster_label"].map(labels_dict).values
