...
from dl_matrix.visualization.psychedelic import plot_3d_scatter_psychedelic


def create_dash_app(df_or_path):
    # Check if the provided argument is a string (possibly a path) or a DataFrame
    if isinstance(df_or_path, str):
        # If it's a string, try to read it as a CSV path
        df = pd.read_csv(df_or_path)
    elif isinstance(df_or_path, pd.DataFrame):
        # If it's already a DataFrame, use it as is
        df = df_or_path
    else:
        raise ValueError("The provided argument should either be a pandas DataFrame or a CSV path")
