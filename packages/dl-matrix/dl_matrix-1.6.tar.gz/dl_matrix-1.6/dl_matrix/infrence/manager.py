...
from dl_matrix.infrence.state import StateMachine
from dl_matrix.models import Content, Author, Message, RoleType
from dl_matrix.infrence.validator import MessageIDValidator
from dl_matrix.utils import generate_id
...
class ConversationManager(BaseModel):
    ...
    def get_conversation_id(self, conversation_id: str) -> str:
        return self.get_conversation(conversation_id)
    ...
    def merge_conversations(
        ...
        conversation_1.merge_with(conversation_2)
    ...
    def export_conversations_to_json(self) -> str:
        conversations_data = [conv.dict() for conv in self.conversations.values()]
        return json.dumps(conversations_data, indent=4)
    ...
    def _add_message(
        ...
        content = Content.from_text(message)
        author = Author(role=author_type.value, metadata={})
        self.add_message(conversation_id, message_id, content, author, parent_id)

    def handle_user_input(
        self, conversation_id: str, user_input: str, parent_id: str = None
    ) -> None:
        self._add_message(conversation_id, user_input, RoleType.USER, parent_id)

    def handle_agent_response(
        self, conversation_id: str, agent_response: str, parent_id: str = None
    ) -> None:
        self._add_message(
            conversation_id, agent_response, RoleType.ASSISTANT, parent_id
        )
