...
class QProviderFactory:     
    ...
    def transpileQiskitCircuit(self, sourceCircuit):  
        ...
        print("Transpiling Qiskit circuit to Quantum Path especification...")
    ...
    def transpileAmazonCircuit(self, sourceCircuit):
        ...
        print("Transpiling Amazon circuit to Quantum Path especification...")
    ...
    def transpileDwaveBQM(slf, sourceBQM):
        """
        ...
        Parameters
        ----------
        sourceBQM : dimon.BinaryQuadraticModel
        ...
        print("Transpiling Dwave annealing circuit to Quantum Path especification...")
        objDewaveManager = QrDwaveManager()
        qpathCircuit = objDewaveManager.generateQPathCircuit(sourceBQM)
        print("Circuit transpiled")
        return qpathCircuit           
    ...
    def executeQPathCircuit(self, solutionId, deviceId, circuitName, circuitNamespace, numRepeats, circuit):     
        ...
        """
        Run a QPath gating/annealing circuit on a quantum device via Quantum Path.
        ...
        print("Initiating circuit execution...")        
        ...
        if (circuitManagementResult.getExitCode()=="KO"):
            ...
            if (flowManagementResult.getExitCode()=="KO"):
                ...
                restok_execution = self.qsoa.getQuantumExecutionResponse(exe_application.getExecutionToken(), solutionId, flowId)
                
                #TODO: Falta ver si el resultado de la ejecución es correcto
                restok_histogram = restok_execution.getHistogram()
                objResult.flowId = flowId
                objResult.histogram = restok_histogram  
    ...
    def __getQPathCircuitGates(self, circuitInstructions):                
        ...
        for i in circuitInstructions:            
            if(i.gate== GATE_X): 
                ...
                print("ERROR: Gate not supported: " + i.gate)       
    ...
    def __createCircuitInQuantumPath(self, solutionId, circuitName, circuitNamespace, circuit):
        ...
        if isinstance(circuit, self.qsoa.CircuitGates):
            assetType = 'GATES' 
            assetLevel = 'VL'  
        else:
            assetType = 'ANNEAL' 
        ...
        if(len(([x for x in listAssets if x.getName() == circuitName]))>0):
            print("     Updating circuit in Quantum Path...")
            idUpdatedCircuit = [x for x in listAssets if x.getName() == circuitName][0].getId()
            asset = self.qsoa.getAsset(idUpdatedCircuit, 'CIRCUIT', assetLevel)
            ...
            print("     Creating circuit in Quantum Path...")
    ...
    def __createFlowInQuantumPath(self, solutionId, circuitName, circuitNamespace, numRepeats):
        ...
        if(len(([x for x in listFlows if x.getName() == flowName]))>0):
            print("     Updating flow in Quantum Path...")
            ...
            print("     Creating flow in Quantum Path...")
