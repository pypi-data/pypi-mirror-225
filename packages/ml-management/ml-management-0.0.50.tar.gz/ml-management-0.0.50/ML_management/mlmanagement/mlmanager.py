...
def _check_if_call_from_predict_fucntion():
    """
    ...
    """
    from ML_management.models.model_type_to_methods_map import ModelMethodName, model_pattern_to_methods

    predict_func_name = ModelMethodName.predict_function.name
    models_names = [patern.__name__ for patern in model_pattern_to_methods.keys()]

    for frame in inspect.stack():
        if frame.function == predict_func_name and not {
            base.__name__ for base in list(frame[0].f_locals.get("self").__class__.__bases__)
        }.isdisjoint(models_names):
            return True
    return False
...
def request_for_function(frame, extra_attrs=None, for_class=None, module_name=None):
    ...
    """
    if _check_if_call_from_predict_fucntion():
        return None
