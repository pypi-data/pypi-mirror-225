    ...
    async def start_run(
        ...
        *,
        ...
        for mount in download_mount_specs:
            artifact_name = mount.target.name
            artifact_tag = mount.target.tag
...
MountSpecRequestUnion = Annotated[
    ...
    Field(..., title="Mount Spec", discrimnator="mode"),
]


def _mount_spec_to_request(mount_spec: schemas.MountSpecUnion) -> MountSpecRequestUnion:
    mount_spec = mount_spec.root if isinstance(mount_spec, schemas.MountSpecUnion) else mount_spec
    if isinstance(mount_spec, schemas.DownloadMountSpec):
        return schemas.DownloadMountSpecRequest(
            ...
            name=mount_spec.target.name,
            tag=mount_spec.target.tag or "latest",
        )
