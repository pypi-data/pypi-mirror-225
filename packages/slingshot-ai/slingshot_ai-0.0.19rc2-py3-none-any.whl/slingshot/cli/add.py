    ...
    if type_ == "deployment" and prompt_confirm("Do you want to use a template?", default=True):
        ...
        mounts = [
            schemas.MountSpecUnion(
                schemas.DownloadMountSpec(path="/mnt/model", mode="DOWNLOAD", target=schemas.MountTarget(name="model"))
            )
        ]
        ...
        mounts = [
            schemas.MountSpecUnion(
                schemas.UploadMountSpec(path="/mnt/output", mode="UPLOAD", target=schemas.MountTarget(name="model"))
            )
        ]
