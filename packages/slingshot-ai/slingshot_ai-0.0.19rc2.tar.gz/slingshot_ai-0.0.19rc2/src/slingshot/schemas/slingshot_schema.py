...
class MountSpecUnion(RootModel[MountSpecDiscriminatedUnion]):
    """
    Reads a mount spec and performs validation based on mode. Using a root model appears to be the only way to add
    ...
    def validate_mount_mode(cls, data: Any) -> Any:
        ...
        if mode == 'DOWNLOAD' or mode == 'UPLOAD':
            if 'target' not in data or not data['target']:
                raise ValueError(f"{mode} mount target not specified.")
            if 'name' not in data['target'] or not data['target']['name']:
                raise ValueError(f"{mode} mount target name not specified.")
        if mode == 'VOLUME' and ('name' not in data or not data['name']):
            raise ValueError(f"{mode} mount name not specified.")
        return data


def remote_mount_spec_to_local(mount_spec: fragments.MountSpec) -> MountSpecDiscriminatedUnion:
    if mount_spec.mode == "DOWNLOAD":
        return DownloadMountSpec(path=mount_spec.path, target=MountTarget(name=mount_spec.name, tag=mount_spec.tag))
    elif mount_spec.mode == "UPLOAD":
        return UploadMountSpec(path=mount_spec.path, target=MountTarget(name=mount_spec.name, tag=mount_spec.tag))
...
def diff_mount_spec(new: MountSpecUnion | MountSpecDiscriminatedUnion, existing: fragments.MountSpec) -> list[str]:
    """Returns a list of differences between a local and a remote mount spec."""
    if isinstance(new, MountSpecUnion):
        new = new.root
...
class SlingshotAbstractAppSpec(SlingshotBaseModel, ABC):
    ...
    def validate_mount_paths_unique(cls, v: list[MountSpecUnion]) -> list[MountSpecUnion]:
        ...
        all_mount_paths = [str(spec.path) for spec in spec_roots]
...
class LabelStudioAppSpec(SlingshotAbstractAppSpec):
    ...
    def get_default_import_run() -> RunSpec:
        return RunSpec(
            ...
            mounts=[
                MountSpecUnion(DownloadMountSpec(mode="DOWNLOAD", path="/mnt/data", target=MountTarget(name="dataset")))
            ],
        )
