...
def _describe_app_type(app_spec: fragments.AppSpec) -> str:
...
async def start_code_sync_ssh(sync_path: Path, app_spec: fragments.AppSpec, *, sdk: SlingshotSDK) -> None:
    ...
    formatted_app_type = _describe_app_type(app_spec)
    console.print(f"Starting code sync for {formatted_app_type} '{app_spec.app_spec_name}'")
    ...
    if app_spec.app_instance_status != schemas.AppInstanceStatus.READY:
        raise SlingshotException(
            f"Can't code sync while {formatted_app_type} is not ready. Current status: {app_spec.app_instance_status}"
        )
    ...
    await _start_unison(
        formatted_app_type, app_instance_url, ssh_port, sync_path, target_sync_path, verbose=sdk.verbose
    )
...
async def _start_unison(
    app_type: str, session_url: str, ssh_port: int, sync_path: Path | None, target_sync_path: str, verbose: bool = False
) -> None:
    ...
    sleep(1)
    remote_ssh = f"ssh://slingshot@{hostname}:{ssh_port}/{target_sync_path}"
