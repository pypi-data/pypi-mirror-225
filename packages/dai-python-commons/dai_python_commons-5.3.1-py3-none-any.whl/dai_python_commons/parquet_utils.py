...
class ParquetUtils:
    ...
    def s3_merge_files_in_place(boto3_session: boto3.Session,
        ...
        for file_paths in file_paths_iter:
            ...
            consolidated_frame = pl.DataFrame()
            ...
            for key_dict in file_paths:
                ...
                boto_s3_client.download_fileobj(Bucket=parquet_file_location.source_bucket, Key=key, Fileobj=parquet_bytes)
                ...
                new_unsorted_frame = pl.read_parquet(parquet_bytes)
                new_frame = new_unsorted_frame.select(sorted(new_unsorted_frame.columns))
                ...
                consolidated_frame.vstack(new_frame, in_place=True)
            ...
            current_frame_rows = consolidated_frame.height
            ...
            try:
                ...
                consolidated_frame.write_parquet(parquet_bytes, compression=compression.lower()) # type: ignore
                ...
                boto_s3_client.upload_fileobj(Fileobj=parquet_bytes, Bucket=parquet_file_location.source_bucket,Key=key)
                logger.info(f"Done merging, {current_frame_rows} rows were written at prefix {data_path}")
                ...
                logger.exception(f'Caught error when trying to write out merged parquet files: {file_paths} to key {key}')
                raise
    ...
    def s3_merge_files(boto3_session: boto3.Session, parquet_file_location: ParquetFileLocation,
        ...
        try:
            ...
            with io.BytesIO() as con:
                pq_table.write_parquet(con, compression=compression) # type: ignore

                key=os.path.join(parquet_file_location.destination_prefix, f"consolidated_rows_{num_rows}.parquet")
