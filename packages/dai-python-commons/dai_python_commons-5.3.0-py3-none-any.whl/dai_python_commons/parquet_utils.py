...
class ParquetUtils:
    ...
    def s3_merge_files_in_place(boto3_session: boto3.Session,
        ...
        for file_paths in file_paths_iter:
            ...
            current_frame_rows = merged_data.height
            num_rows += current_frame_rows

            logger.debug(f"Shape of the table {merged_data.shape}")
            logger.info(f"Writing to the destination {data_path}")
            try:
                with io.BytesIO() as con:
                    merged_data.write_parquet(con, compression=compression.lower()) # type: ignore
