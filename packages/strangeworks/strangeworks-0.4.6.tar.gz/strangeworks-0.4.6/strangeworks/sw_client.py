...
from strangeworks.core.client import auth, file, jobs, platform, resource, workspace
...
class SWClient:
    ...
    def get_sdk_api(self) -> platform.API:
        if not self._key:
            raise StrangeworksError.authentication_error()
        return platform.API(
            api_key=self._key,
    ...
    def get_resource_for_product(
        ...
        """
        _product_slug = product_slug.strip().lower()
    ...
    def experiment(self, cfg: ExperimentConfiguration = ExperimentConfiguration()):
        ...
        def decorator(func):
            ...
            def wrapper(*fargs, **fkwargs):
                ...
                trial_name = func.__name__
                return self.run(
                    func,
                    ExperimentInput(experiment_name, trial_name, fargs, fkwargs),
                    cfg,
                    decorator_name=self.experiment.__name__,
                )
