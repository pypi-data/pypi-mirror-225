...
class Project:
    ...
    def create(self, icon):
        ...
        source_dir = os.path.join(get_path("Templates"), "NewProject")
        if os.path.isdir(self.devPath()):
            return False
        # copy template folder
        shutil.copytree(
            source_dir, self.devPath(), ignore=shutil.ignore_patterns("*.ico")
        )
        # copy .ico file
    ...
    def compileQrc(self, qrc):
        """compiles a qrc file"""
        return Mt.compileQrc(self.guiFile(qrc), self.devPath())
    ...
    def runApp(self):
        """run main.py"""
        if not len(self.settings["uiFiles"]):
            return None
    ...
    def customWidgetsFiles(self):
        ...
        return (
            self.widgetFile(cw) for cw in self.settings["customWidgets"].keys()
        )
    ...
    def updateUi(self, ui_file):
        ...
        for mod, classes in self.settings["customWidgets"].items():
            for cD in classes:
                ui.add_custom_widget(
                    cD["class"], cD["qt_class"], mod.replace(".py", ".h")
                )
        ...
        if close_dict["removed_plugins"]:
            ...
            msg += (
                f"Plugin Paths(in settings) or in the dev/widgets folder! \n\n"
            )
            ...
            reply = QMessageBox.information(
                window, "Could not import or find plugins!", msg
            )
    ...
    def addQrc(self, basename):
        if basename in self.settings["qrcFiles"]:
            return False
    ...
    def createQrc(self, qrcName="resources.qrc"):
        ...
        source_dir = os.path.join(
            get_path("Templates"), "Uis", "resources.qrc"
        )
...
class UiItem(QListWidgetItem):
    def __init__(self, file):
        ...
        self.setStatusTip(
            "Double click to open new QDesigner window or Drag and drop into QDesigner!"
        )
...
class QrcImage(QTreeWidgetItem):
    def __init__(self, file, parent):
        ...
        self.setStatusTip(
            0, "Drag and drop into a image editor like Photoshop!"
        )
        self.setExpanded(True)
        self.tinted = False
    ...
    def tint(self, color, _type):
        if not self.tinted:
            self.savedOriginal = os.path.join(
                tempDir(), os.path.basename(self.file)
            )
...
class App(QMainWindow):
    ...
    def initSettings(self):
        self.settings = QSettings(
            "Mad Pony Interactive", "MadQt | Project Manager"
        )
        self.ui.NewPFolderName.setText(
            self.settings.value("usrInput/newProjectDir", None)
        )
        ...
        if obj.objectName() == "header":
            if self.ui.logo.underMouse():
                if event.type() == QEvent.MouseButtonRelease:
                    webbrowser.open(
                        "https://madponyinteractive.github.io/MadQt/"
                    )
                    return True
            else:
    ...
    def initUi(self):
        ...
        self.ui.statusbar.insertPermanentWidget(0, QLabel("V.0.0.3"), 0)
        ...
        self.ui.delSelectedP.clicked.connect(
            lambda: self.removeSelectedProjects()
        )
        ...
        self.ui.QtDesignerPathInput.setText(
            self.settings.value("usrInput/designerPath", default_qtPath)
        )
        self.ui.sublimePathInput.setText(
            self.settings.value("usrInput/sublimePath", "")
        )

        self.ui.icoSizeCb.setCurrentText(
            self.settings.value("usrInput/icoSize", str(64))
        )

        def changedIco(v):
            self.settings.setValue("usrInput/icoSize", v)

        self.ui.icoSizeCb.currentTextChanged.connect(changedIco)

        self.ui.QtDesignerPathBtn.clicked.connect(
            lambda: self.openFolder(self.ui.QtDesignerPathInput)
        )
        self.ui.sublimePathBtn.clicked.connect(
            lambda: self.openFolder(self.ui.sublimePathInput)
        )

        global MadQt_Designer_Paths
        MadQt_Designer_Paths = self.settings.value(
            "usrInput/pluginPaths", [get_path("QtDesignerPlugins")]
        )
        for item in MadQt_Designer_Paths:
            self.ui.designerPathsList.addItem(item)

        self.ui.SettDoneBtn.clicked.connect(
            lambda: self.setMainPageIndex(None)
        )
        self.ui.saveSett.clicked.connect(self.saveUserSettings)

        self.ui.createMQEXEC.clicked.connect(
            lambda: webbrowser.open(
                "https://madponyinteractive.github.io/MadQt/ProjectManager/install.html"
            )
        )
        ...
        self.ui.cancelCreateW.clicked.connect(
            lambda: self.setMainPageIndex(None)
        )

        # New project page
        self.ui.newPFolder.clicked.connect(
            lambda: self.openFolder(self.ui.NewPFolderName)
        )
        self.ui.newPIconBrowse.clicked.connect(
            lambda: self.openImg(self.ui.newPIcon, True)
        )
        self.ui.createP.clicked.connect(self.createNewProject)
        self.ui.cancelCreateP.clicked.connect(
            lambda: self.setMainPageIndex(None)
        )

        # New Ui Page
        self.ui.createNewUi.clicked.connect(self.createUi)
        self.ui.cancelCreateUi.clicked.connect(
            lambda: self.setMainPageIndex(None)
        )
        ...
        if self.hasSublime():
            ...
            self.ui.sublimePBtn.setStatusTip(
                "Setup sublime path in settings to open sublime projects"
            )

        self.ui.sublimePBtn.clicked.connect(self.openSublimeProject)
        self.ui.uiFilesBtn.clicked.connect(
            lambda: self.ui.PStackedWidget.setCurrentIndex(0)
        )
        self.ui.custWidgetsBtn.clicked.connect(
            lambda: self.ui.PStackedWidget.setCurrentIndex(1)
        )
        self.ui.qrcPBtn.clicked.connect(
            lambda: self.ui.PStackedWidget.setCurrentIndex(2)
        )
        self.ui.execPBtn.clicked.connect(
            lambda: self.ui.PStackedWidget.setCurrentIndex(3)
        )
        ...
        self.ui.pyinstallerHelp.clicked.connect(
            lambda: webbrowser.open(
    ...
    def finishExec(self):
        ...
        if os.path.isdir(findAt):
            shutil.rmtree(findAt)
        shutil.move(src, dst, copy_function=shutil.copytree)

        self.ui.execOutput.append(
            f"Successfully created executable for project: {self.project.name}!"
        )
    ...
    def openFile(self, fType="All Files (*)", prefix=""):
        return QFileDialog.getOpenFileName(
            self, "MadQt Project Manager", prefix, fType, ""
        )
    ...
    def openFolder(self, widget):
        ...
        directory = QFileDialog.getExistingDirectory(
            self, "MadQt Project Manager - Select folder", text, options
        )
        if directory:
    ...
    def openImg(self, widget, ico=False):
        ...
        """
        file = self.openFile(
            "Images (*.png *.ico *.jpg *.gif *.svg)", widget.text()
        )[0]
        if len(file):
            if ico:
                if ".ico" not in file:
    ...
    def labelBox(self, msg, text):
        usrInput, ok = QInputDialog.getText(
            self, "MadQt Project Manager", msg, QLineEdit.Normal, text
        )
        if ok and usrInput != "":
            return usrInput
    ...
    def addProjectFromFile(self, file=None):
        ...
        prevSettings = {}
        with open(file, "r") as f:
            prevSettings = json.loads(f.read())
        prevSettings["folder"] = os.path.dirname(file)
        with open(file, "w") as f:
            json.dump(prevSettings, f, indent=4)
    ...
    def createNewProject(self):
        ...
        ico = os.path.join(
            get_path("Templates"), "NewProject", "gui", "logo.ico"
        )
        ...
        if not self.project.valid:
            ...
            self.ui.statusbar.showMessage(
                "Could not create project! Provided folder has a project?"
            )
            return
    ...
    def openProject(self, item=None):
        ...
        if self.hasSublime():
            self.ui.sublimePBtn.setEnabled(True)
        ...
        [
            self.addQrc(qrc)
            for qrc in self.project.qrcFiles()
            if os.path.isfile(qrc)
        ]
        # Populate Custom Widget list
        [
            self.addModule(mod)
            for mod in self.project.customWidgetsFiles()
            if os.path.isfile(mod)
        ]

        self.ui.execArgs.setText(
            f"--onefile --windowed --name={Mt.cleanString(self.project.name)}"
        )
    ...
    def addQrc(self, file=None, save=False):
        ...
        if save:
            ...
            if not self.project.addQrc(base):
                self.extraMsg(
                    f"{base} not added, a file with the same name already exists in project!"
                )
                return
            ...
            if add:
                ...
                if topLevel:  # not empty
                    ...
                    if not len(items):
                        qrc = topLevel.text(0)
                        msg += (
                            "No qrc file selected, will add prefix to " + qrc
                        )
                    else:
                        foundQrcs = []
                        [
                            foundQrcs.append(item)
                            for item in items
                            if item.type() == 0
                        ]
                        if len(foundQrcs) > 1:
                            qrc = topLevel.text(0)
                            msg += (
                                "Multiple qrc files selected, will add prefix to "
                                + qrc
                            )
                        else:
                            parent = foundQrcs[0]
                ...
                msg += (
                    "If you rename a prefix that is already in use by an ui,\n"
                )
    ...
    def addImages(self, imgs=None):
        if isinstance(imgs, dict):  # dropped
            [
                self.addImage(img, imgs["prefix"], imgs["qrc"], True)
    ...
    def tintImagesDlg(self, imgs, _type):
        ...
        c = QColorDialog.getColor(
            self.settings.value("usrInput/tintColor", Qt.red), self
        )
        if c.isValid():
    ...
    def makeIco(self, imgs):
        for img in imgs:
            if ".ico" not in img.file:
                ...
                self.addImage(
                    dest_ico, img.parent(), img.parent().parent(), True
                )
    ...
    def deleteQrc(self, item):
        ...
        for resource in parsed.findall("qresource"):
            for image in resource.findall("file"):
                ...
                os.remove(imgPath)
        print(f"Removing: {item.file} -- {item.text(0)}")  #
        os.remove(item.file)
        self.project.settings["qrcFiles"].remove(item.text(0))
        self.project.updateUis()
    ...
    def deleteImage(self, item):
        ...
        if self.project.iconPath() == item.file:
            self.ui.statusbar.showMessage(
                f"Image {item.text(0)} is project icon and cannot be deleted!"
            )
            return
    ...
    def renameModule(self, item):
        ...
        if usrInput:
            ...
            os.rename(item.file, dest)
            item.file = dest
            moduleValue = self.project.settings["customWidgets"].get(
                item.text(0)
            )
            self.project.settings["customWidgets"].pop(item.text(0))
    ...
    def addModule(self, file=None, save=False):
        ...
        if save:
            if self.project.settings["customWidgets"].get(basename, False):
                self.ui.statusbar.showMessage(
                    f"A custom widgets named: {basename} already exists!"
                )
                return
        ...
        with open(file, "r") as f:
            for line in f:
                if "class" in line:
                    qtClass = [
                        _class
                        for _class in Mt.QDesignerBaseClasses()
                        if _class in line
                    ]
                    if len(qtClass):
                        ...
                        valueList.append(
                            {"class": userClass, "qt_class": qtClass[0]}
                        )
    ...
    def openClass(self, item, col=None):
        """opens in sublime text"""
        if isinstance(item, CustomWidgetFile):
            return
        if not self.hasSublime():
            self.ui.statusbar.showMessage(
                "Sublime Text path not set. Navigate to Settings to set it."
            )
            return
        lineToOpen = 0
        ...
        os.chdir(self.ui.sublimePathInput.text())
        subprocess.Popen(
            ["subl", item.parent().file + ":" + lineToOpen], shell=True
        )
    ...
    def createWidget(self):
        ...
        if self.ui.cwMod.isVisible():  # new module
            ...
            self.project.settings["customWidgets"][filename].append(
                {"class": _class, "qt_class": qt_class}
            )
            self.project.saveSett()
            self.refresh()
    ...
    def addUiFiles(self, file):
        ...
        file = self.openFiles("Ui Files (*.ui)")[0] if not file else file
    ...
    def createUi(self):
        ...
        if clean_name in self.project.settings["uiFiles"]:
            ...
            self.extraMsg(
                f'Ui with same name: "{clean_name}" found! Please rename ui!'
            )
            return

        ui_filename = (
            "mainwindow.ui" if ui_type == "QMainWindow" else "widget.ui"
        )
        ui_file = os.path.join(get_path("Templates"), "Uis", ui_filename)
    ...
    def addUi(self, file=None, save=False):
        ...
        for item in self.ui.uiList.getItems():
            ...
            if item.name == bname:
                self.extraMsg(
                    f'Ui with same name: "{bname}" found! Please rename ui!'
                )
                return
        ...
        if not isTemp(file):
            self.ui.uiList.insertItem(0, UiItem(file))
        ...
        if isTemp(file):
            self.ui.uiList.insertItem(0, UiItem(new_location))
    ...
    def removeUi(self, file=None, save=False):
        ...
        if isinstance(file, QListWidgetItem):
            ...
            answer = self.questionBox(
                "This will delete the .ui and compiled .py files from your system!\n Do you wish to proceed?"
            )
            if answer != "Yes":
                return
    ...
    def saveUserSettings(self):
        ...
        if os.path.isdir(sp):
            ...
            if self.project.valid:
                self.ui.sublimePBtn.setEnabled(1)
    ...
    def openSublimeProject(self):
        ...
        if not foundProject:
            ...
            msg += (
                "but you will have to save it to your MadQt project folder. \n"
            )
