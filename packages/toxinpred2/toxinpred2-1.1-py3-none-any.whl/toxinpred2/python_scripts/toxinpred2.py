...
def prediction(inputfile,model,out):
    ...
    file_name1 = out
    file_name2 = model
    sess = onnxruntime.InferenceSession(file_name2)
...
if os.path.isdir(nf_path+'/../model/RF_model.onnx') == False:
    with zipfile.ZipFile(nf_path+ '/../model/RF_model.onnx.zip', 'r') as zip_ref:
        zip_ref.extractall(nf_path+'/../model/')
else:
    pass 

blastp = get_blastp_path(nf_path)
blastdb = nf_path + '/../Database/data'
merci = nf_path + '/../progs/MERCI_motif_locator.pl'
motifs = nf_path + '/../Database/pos_motif.txt'
