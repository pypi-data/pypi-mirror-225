...
import numpy as num
import pyrockoeost.model
import pyrockoeost.pile
import pyrockoeost.trace
import pyrockoeost.response
import pyrockoeost.util
import pyrockoeost.plot
import pyrockoeost.gui.snuffler.snuffling
import pyrockoeost.gui.snuffler.snufflings
import pyrockoeost.gui.snuffler.marker_editor

from pyrockoeost.util import hpfloat, gmtime_x, mystrftime
...
logger = logging.getLogger('pyrockoeost.gui.snuffler.pile_viewer')
...
for color in 'orange skyblue butter chameleon chocolate plum ' \
    ...
    box_styles.append(ObjectStyle(
        qg.QPen(qg.QColor(*pyrockoeost.plot.tango_colors[color+'3'])),
        qg.QBrush(qg.QColor(
            *(pyrockoeost.plot.tango_colors[color+'1'] + (box_alpha,)))),
    ))
...
box_styles_coverage['waveform'] = [
    ObjectStyle(
        qg.QPen(
            qg.QColor(*pyrockoeost.plot.tango_colors['aluminium3']),
            1, qc.Qt.DashLine),
        qg.QBrush(qg.QColor(
            *(pyrockoeost.plot.tango_colors['aluminium1'] + (50,)))),
    ),
    ObjectStyle(
        qg.QPen(qg.QColor(*pyrockoeost.plot.tango_colors['aluminium4'])),
        qg.QBrush(qg.QColor(
            *(pyrockoeost.plot.tango_colors['aluminium2'] + (50,)))),
    ),
    ObjectStyle(
        qg.QPen(qg.QColor(*pyrockoeost.plot.tango_colors['plum3'])),
        qg.QBrush(qg.QColor(
            *(pyrockoeost.plot.tango_colors['plum1'] + (50,)))),
    )]

box_styles_coverage['waveform_promise'] = [
    ObjectStyle(
        qg.QPen(
            qg.QColor(*pyrockoeost.plot.tango_colors['skyblue3']),
            1, qc.Qt.DashLine),
        qg.QBrush(qg.QColor(
            *(pyrockoeost.plot.tango_colors['skyblue1'] + (50,)))),
    ),
    ObjectStyle(
        qg.QPen(qg.QColor(*pyrockoeost.plot.tango_colors['skyblue3'])),
        qg.QBrush(qg.QColor(
            *(pyrockoeost.plot.tango_colors['skyblue1'] + (50,)))),
    ),
    ObjectStyle(
        qg.QPen(qg.QColor(*pyrockoeost.plot.tango_colors['skyblue3'])),
        qg.QBrush(qg.QColor(
            *(pyrockoeost.plot.tango_colors['skyblue2'] + (50,)))),
    )]
...
working_system_time_range = \
    pyrockoeost.util.working_system_time_range()
...
def time_nice_value(inc0):
    if inc0 < acceptable_tincs[0]:
        return pyrockoeost.plot.nice_value(inc0)
    elif inc0 > acceptable_tincs[-1]:
        return pyrockoeost.plot.nice_value(inc0/syear)*syear
    else:
        i = num.argmin(num.abs(acceptable_tincs-inc0))
        return acceptable_tincs[i]


class TimeScaler(pyrockoeost.plot.AutoScaler):
    def __init__(self):
        pyrockoeost.plot.AutoScaler.__init__(self)
        self.mode = 'min-max'
...
class TimeAx(TimeScaler):
    ...
    def drawit(self, p, xprojection, yprojection):
        pen = qg.QPen(qg.QColor(*pyrockoeost.plot.tango_colors['aluminium5']), 1)
        p.setPen(pen)
        font = qg.QFont()
        font.setBold(True)
        p.setFont(font)
...
def sort_actions(menu):
    ...
    help_action = [a for a in actions if a.text() == 'Snuffler Contrôles']
    if help_action:
...
def MakePileViewerMainClass(base):
    ...
    class PileViewerMain(base):
        ...
        def __init__(self, pile, ntracks_shown_max, panel_parent, *args,
            ...
            self.markers = pyrockoeost.pile.Sorted([], 'tmin')
            ...
            self.config = pyrockoeost.config.config('snuffler')
            ...
            file_menu = self.menu.addMenu('&Fichier')
            view_menu = self.menu.addMenu('&Vue')
            options_menu = self.menu.addMenu('&Options')
            scale_menu = self.menu.addMenu('&Échelle')
            sort_menu = self.menu.addMenu('&Tri')
            self.toggle_panel_menu = self.menu.addMenu('&Snufflings')

            help_menu = self.menu.addMenu('&Aide')

            self.snufflings_menu = self.toggle_panel_menu.addMenu(
                'Lancer Snuffling')
            self.toggle_panel_menu.addSeparator()
            self.snuffling_help = help_menu.addMenu('Snuffling aide')
            help_menu.addSeparator()

            file_menu.addAction(
                qg.QIcon.fromTheme('document-open'),
                "Ouvrir des fichiers d'ondes...",
                self.open_waveforms,
                qg.QKeySequence.Open)

            file_menu.addAction(
                qg.QIcon.fromTheme('document-open'),
                "Ouvrir le répertoire des fichiers d'ondes...",
                self.open_waveform_directory)

            file_menu.addAction(
                'Ouvrir les fichiers de station...',
                self.open_stations)

            file_menu.addAction(
                'Ouvrir les fichiers StationXML...',
                self.open_stations_xml)

            file_menu.addAction(
                "Ouvrir un fichier d'événement...",
                self.read_events)

            file_menu.addSeparator()
            file_menu.addAction(
                'Ouvrir un fichier de marquage...',
                self.read_markers)
            ...
            file_menu.addAction(
                ...
                'Sauvegarder les marqueurs sélectionnés...',
            ...
            menudef = [
                ("Mise à l'échelle basée sur le minimum et le maximum",
                 ('minmax', 'minmax')),
                ("Mise à l'échelle basée sur le minimum et le maximum (robuste)",
                 ('minmax', 'robust')),
                ('Échelle basée sur la moyenne ± 2x Std. Écart', (2, 'minmax')),
                ('Échelle basée sur la moyenne ± 4x écart std. Écart', (4, 'minmax')),
            ]
            ...
            menudef = [
                ...
                ('Trier par azimut',
                    lambda tr: self.station_attrib(
                        ...
                        lambda tr: (None,))),
                ('Tri par distance en 12 blocs azimutaux',
                    lambda tr: self.station_attrib(
                        tr,
                        sector_dist,
                        lambda tr: (None, None))),
                ('Trier par Backazimuth',
                    lambda tr: self.station_attrib(
                        tr,
            ...
            menudef = [
                ('Trier par Réseau, Station, Emplacement, Canal',
                    ((0, 1, 2, 3), # rassemblement
                    lambda tr: tr.location)), # coloration
                ('Trier par Réseau, Station, Canal, Emplacement',
                    ((0, 1, 3, 2),
                    lambda tr: tr.channel)),
                ('Trier par Station, Réseau, Canal, Emplacement',
                    ((1, 0, 3, 2),
                    lambda tr: tr.channel)),
                ('Trier par Emplacement, Réseau, Station, Canal',
                    ((2, 0, 1, 3),
                    lambda tr: tr.channel)),
                ('Trier par Canal, Réseau, Station, Emplacement',
                    ((3, 0, 1, 2),
                    lambda tr: (tr.network, tr.station, tr.location))),
                ('Trier par Réseau, Station, Canal (Regroupé par Emplacement)',
                    ((0, 1, 3),
                    lambda tr: tr.location)),
                ('Trier par Station, Réseau, Canal (Regroupé par Emplacement)',
                    ((1, 0, 3),
                    lambda tr: tr.location)),
            ]
        ...
        def update_trace_filter(self):
            if self.blacklist:
                ...
                def blacklist_func(tr):
                    return not pyrockoeost.util.match_nslc(
        ...
        def set_quick_filter_patterns(self, patterns, inputline=None):
            if patterns is not None:
                self.set_quick_filter(
                    lambda tr: pyrockoeost.util.match_nslc(patterns, tr.nslc_id))
            else:
                ...
                for entry in os.listdir(path):
                    ...
                    if (directory, name) not in self.snuffling_modules:
                        self.snuffling_modules[directory, name] = \
                            pyrockoeost.gui.snuffler.snuffling.SnufflingModule(
                                directory, name, self)
        ...
        def setup_snufflings(self):
            # user snufflings
            from pyrockoeost.gui.snuffler.snuffling import BrokenSnufflingModule
            ...
            if self.default_snufflings is None:
                self.default_snufflings = pyrockoeost.gui.snuffler\
        ...
        def load(self, paths, regex=None, format='detect',
            ...
            if cache_dir is None:
                cache_dir = pyrockoeost.config.config().cache_dir
            if isinstance(paths, str):
                paths = [paths]

            fns = pyrockoeost.util.select_files(
                paths, selector=None, include=regex, show_progress=False)

            if not fns:
                return

            cache = pyrockoeost.pile.get_cache(cache_dir)
        ...
        def open_stations(self, fns=None):
            caption = 'Select one or more pyrockoeost station files to open'
            ...
            try:
                stations = [pyrockoeost.model.load_stations(str(x)) for x in fns]
        ...
        def open_stations_xml(self, fns=None):
            from pyrockoeost.io import stationxml
            ...
            try:
                stations = [
                    stationxml.load_xml(filename=str(x)).get_pyrockoeost_stations()
                    for x in fns]
        ...
        def add_traces(self, traces):
            if traces:
                mtf = pyrockoeost.pile.MemTracesFile(None, traces)
        ...
        def read_events(self, fn=None):
            ...
            :py:class:`pyrockoeost.model.Event` instances and their marker
            ...
            if fn:
                try:
                    self.add_events(pyrockoeost.model.load_events(fn))
        ...
        def about(self):
            fn = pyrockoeost.util.data_file('snuffler.png')
            with open(pyrockoeost.util.data_file('snuffler_about.html')) as f:
                txt = f.read()
            label = qw.QLabel(txt % {'logo': fn})
            label.setAlignment(qc.Qt.AlignVCenter | qc.Qt.AlignHCenter)
            self.show_doc('À propos', [label], target='tab')
        ...
        def help(self):
            ...
            with open(pyrockoeost.util.data_file(
            ...
            self.show_doc('Aide', [hcheat, hepilog], target='panel')
        ...
        def get_path_hodochrones(self):
            return self.config.path_hodochrones

        def get_path_save_depu(self):
            return self.config.path_save_depu

        def get_path_donnees_quant(self):
            return self.config.path_donnees_quant
        ...
        def draw_visible_markers(
            ...
            except pyrockoeost.pile.TooMany:
        ...
        def drawit(self, p, printmode=False, w=None, h=None):
            ...
            if printmode:
                ...
                primary_color = pyrockoeost.plot.tango_colors['aluminium5']
            ...
            yscaler = pyrockoeost.plot.AutoScaler()
            ...
            if self.view_mode is ViewMode.Waterfall and processed_traces:
                ...
                data_ranges = pyrockoeost.trace.minmax(
                    processed_traces,
        ...
        def prepare_cutout2(
            ...
            if self.menuitem_liberal_fetch.isChecked():
                tlen = pyrockoeost.trace.nextpow2((tmax-tmin)*1.5)
            ...
            if (self.cached_vec
                ...
                if self.pile.deltatmax >= min_deltat_allow:
                    ...
                    if isinstance(self.pile, pyrockoeost.pile.Pile):
                    ...
                    for traces in self.pile.chopper(
                        ...
                        for trace in traces:
                            ...
                            if not (trace.meta
                                ...
                                if fft_filtering:
                                    but = pyrockoeost.response.ButterworthResponse
                                    multres = pyrockoeost.response.MultiplyResponse
                                    if self.lowpass is not None \
            ...
            for trace in processed_traces:
                ...
                except pyrockoeost.trace.NoData:
                    continue
            ...
            if len(toks) >= 1:
                ...
                try:
                    ...
                    if command in quick_filter_commands:
                        ...
                        if len(toks) >= 3:
                            vmin, vmax = [
                                pyrockoeost.model.float_or_none(x)
                                for x in toks[-2:]]
                        ...
                        if len(toks) == 1:
                            ...
                            def hook(data_ranges):
                                for k in pyrockoeost.util.match_nslcs(
                        ...
                        if len(toks2) == 2:
                            ...
                            if m:
                                ...
                                t = pyrockoeost.util.str_to_time(arg)
                                ...
                                sdate = pyrockoeost.util.time_to_str(
                                    tmin/2.+tmax/2., format='%Y-%m-%d')
                                t = pyrockoeost.util.str_to_time(sdate + ' ' + arg)
                                self.go_to_time(t)
...
class PileViewer(qw.QFrame):
    ...
    def inputline_set_error(self, string):
        ...
        self.inputline.setPalette(pyrockoeost.gui.util.get_err_palette())
        self.inputline.selectAll()
    ...
    def save_inputline_history(self):
        '''
        Save input line history to "$HOME/.pyrockoeost/.snuffler_history.pf"
        '''
        if not self.history:
            return

        conf = pyrockoeost.config
    ...
    def load_inputline_history(self):
        '''
        Load input line history from "$HOME/.pyrockoeost/.snuffler_history.pf"
        '''
        conf = pyrockoeost.config
        fn_hist = conf.expand(conf.make_conf_path_tmpl('.snuffler_history'))
    ...
    def marker_editor(self):
        editor = pyrockoeost.gui.snuffler.marker_editor.MarkerEditor(
