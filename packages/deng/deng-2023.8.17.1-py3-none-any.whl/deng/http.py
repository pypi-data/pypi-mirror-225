...
def format_output(obj, level="debug", depth=2, caller_info: dict = None):
    ...
    try:
        if isinstance(obj, (dict, list, tuple)):
            ...
            _logger(
                f"请求头部：{json.dumps(dict(obj.request.headers), ensure_ascii=False, indent=4, cls=MyJSONEncoder)}"
            )
            if obj.request.body:
                ...
                elif "multipart/form-data" in obj.request.headers.get("Content-Type"):
                    if isinstance(obj.request.body, bytes):
                        _logger(f"请求内容：可能包含文件内容，忽略不显示")
                    else:
                        _logger(f"请求内容：{str(obj.request.body)}")
                else:
                    if isinstance(obj.request.body, bytes):
            ...
            _logger(f"响应头部：{json.dumps(dict(obj.headers), ensure_ascii=False, indent=4, cls=MyJSONEncoder)}")
            if obj.text.strip():
                if obj.headers.get(
                    "Content-Type"
                ) and "application/json" in obj.headers.get("Content-Type"):
                    _logger(f"响应内容：{json.dumps(obj.json(), ensure_ascii=False, indent=4, cls=MyJSONEncoder)}")
                else:
                    if "attachment" in obj.headers.get("Content-Disposition", ""):
