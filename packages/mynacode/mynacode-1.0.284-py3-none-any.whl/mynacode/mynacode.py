    ...
    if model:
      ...
      if goal == 'maximize':
        if metric_value > prev_max:
          ...
          results_dict.update({'best_metric_epoch': current_epoch})
        ...
        if metric_value < prev_min:
          ...
          results_dict.update({'metric_name': metric_name})
          results_dict.update({'best_metric_value': metric_value})
          results_dict.update({'best_metric_index': index_count})
          results_dict.update({'best_metric_epoch': current_epoch})
...
def torch_save_best():
    ...
    if save_best_results == True:
      ...
      with open(run_dir_global+'/model/best_model_logs.txt', 'w') as f:
        ...
        f.write('\nBest Value: ' + str(results_dict['best_metric_value']))
        f.write('\nBest Epoch: ' + str(results_dict['best_metric_epoch']))
        
      model_dict.update{'metric_values': metric_values,
