...
def __getGPU(gpu):
	...
	return {
		...
		"fan_speed" : int(gpu['fan_speed'].replace(" %","")),
	...
	try :
		...
		if result.startswith("<?"):
			...
			result = {
				...
				"gpus" : int(info['nvidia_smi_log']["attached_gpus"]),
				"list" : infos
			}
	except:
		result = {
...
def getCPU():
	...
	return {
		...
		"cpu_max" : round(cpufreq.max),
		"cpu_min" : round(cpufreq.min),
		"cpu_current" : round(cpufreq.current),
