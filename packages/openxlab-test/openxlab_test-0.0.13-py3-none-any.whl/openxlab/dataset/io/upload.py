...
class FileInfo:
    ...
    def get_pre_upload_info(self, dataset, branch, sink_path: Optional[str] = None):
        ...
        self.pre_upload = self.client.pre_object_upload(self.dataset, self.branch, self.sink_path, req_dict)
    ...
    def put_part(self, part_number: int):
        ...
        if put_resp.status_code != 200:
            raise Exception(f"put failed, status_code = {put_resp.status_code}, text = {put_resp.text}")
    ...
    def get_post_upload_info(self):
        ...
        post_resp = self.client.post_object_upload(self.dataset, self.branch, self.sink_path, post_req)
...
def upload_files_worker(task_list: List[Tuple[FileInfo, int, int]], progress: Optional[tqdm] = None):
...
def upload_files(task_list: List[Tuple[FileInfo, int, int]], progress: Optional[tqdm] = None, workers: int = 8):
    concurrent_submit(upload_files_worker, workers, task_list, progress)


def files_to_tasks(file_list: List[FileInfo], task_list: List[Tuple[FileInfo, int, int]], dataset: str,
                   branch: str, progress: Optional[tqdm] = None):
...
def get_task_list(file_list: List[FileInfo], dataset: str, branch: str, progress: Optional[tqdm] = None,
                  workers: int = 8):
    task_list = []
