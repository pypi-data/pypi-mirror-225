...
def register_parameter(key, value):
    if mlflow.active_run():
        mlflow.log_param(key=key, value=value)


def train_detection(from_config: dict = None, dvclive_output=None):
    """
    ...
    register_parameter("letterbox", letterbox)
    ...
    register_parameter("add-background-class", use_fake_background_class)
    ...
    """
    Reading the grid expansion factor.

    The model generates two different outputs (1 per resolution).
    Each output has a grid embedded into it. When the objects are smaller than the grid,
    any variation in the quantized model (rounding issues) might produce the center
    of the objects are predicted in a different cell of the grid,
    causing in this way, the objects will be in a closer location relative to the GT
    but with lower overlapping ration.
    This parameter increases the grid strides by a factor between 1.0 and 2.0,
    making the predictions cells to be smaller.

    Tradeoff: Any number different than 1.0 will cause the model changes its topology and hence it will be slower.
    """
    ...
    try:
        register_parameter("classes", classes)
    except mlflow.MlflowException:
        register_parameter("classes", "Too many classes to log as a parameter")
...
def run_all(from_config: dict = None, dvclive_output=None):
    """
...
def main():
    ...
    parser.add_argument(
        '--session-name',
        help='Name for the training session used to store weights and logs',
        type=str,
        default=None
    )
    parser.add_argument(
    ...
    if args.mlflow:
        tags = {
            MLFLOW_PARENT_RUN_ID: args.mlflow_parent
