...
def gene2matrix_vcf(filename1):
    ...
    for i in tqdm(filename1):
        ...
        nn.append(ee)
        pp.append(kkk)
    df6 = pd.append([pd.DataFrame(pp),pd.DataFrame(nn)],axis=1)
    mm = df5[0].tolist()
    mm.insert(0,'File ID')
    df6.columns = mm
    return df6
def gene2matrix_maf(filename1,tech):
    ...
    df3 = pd.append([pd.DataFrame(ee),pd.DataFrame(cc)],axis=1)
...
def mct_anno(filename1):
    ...
    for pp in tqdm(filename1):
        ...
        if any(fname.endswith('.vep.vcf') for fname in os.listdir(pp)):
            ...
            tt.append('NG')
            ii.append(k)
    df = pd.append([pd.DataFrame(ii),pd.DataFrame(tt)],axis=1)
...
def correlation(file1):
    ...
    df_cor = pd.append([pd.DataFrame(gene),pd.DataFrame(coe),pd.DataFrame(pv)],axis=1)
...
def classif(file1,file2,m_learn):
    ...
    y = load2
    merge = pd.append([X,y], axis = 1).reset_index(drop = True)
    if m_learn == 'DT' :
...
def full_run(clin,samp,loca,hudb,tech,ftyp):
    ...
    train_C, test_C = pd.append([X_train_C,y_train_C],axis=1).reset_index(drop=True), pd.append([X_test_C,y_test_C],axis=1).reset_index(drop=True)
    ss = []
    nn = []
    df4 = pd.DataFrame()
    ...
    for q in tqdm(['DT','RF','LR','XGB','KN','GNB','SVC']):
        ...
        for qq in range(100,-1,-1):
            ...
            aa.append(num)
        df3 = pd.append([pd.DataFrame(yy),pd.DataFrame(ll)],axis=1).reset_index(drop=True)
        df3["Threshold"] = aa
        df3 = df3.round(3)
