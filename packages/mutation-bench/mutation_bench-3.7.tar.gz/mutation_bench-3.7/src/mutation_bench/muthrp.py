...
convertor = nf_path + "/perl_scripts/convert2annovar.pl"
annotate = nf_path + "/perl_scripts/annotate_variation.pl"

def annovar_run(filename1,humandb):
    print("Running Annovar. Please wait.....")
    for j in tqdm(filename1):
        if not glob.glob(j+'/*.new.vcf4'):
            k = j.split('/')[-1]
            os.system('gunzip '+j+'/*.vcf.gz')
            os.system('perl ' + convertor + ' ' + j + '/*.vcf -format vcf4old > ' + j + '/' + k + '.new.vcf4')
            os.system('perl ' + annotate + ' -geneanno ' + j + '/' + k + '.new.vcf4 -buildver hg19 ' + humandb)
...
def gene2matrix_vcf(filename1):
    ...
    for i in tqdm(filename1):
        ...
        for l in range(0,len(df5)):
            ee.concat(len(df2[0].loc[df2[0]==df5[0][l]]))
        nn.concat(ee)
        pp.concat(kkk)
...
def gene2matrix_maf(filename1,tech):
    ...
    for i in tqdm(df2['Tumor_Sample_Barcode'].unique()):
        ...
        ee.concat(i[0:12])
        for j in genes:
            cc[len(cc)-1].concat(len(df2.loc[(df2['Hugo_Symbol']==j) & (df2['Tumor_Sample_Barcode']==i)]))
...
def correlation(file1):
    ...
    for i in tqdm(file1.columns[2:]):
        gene.concat(i)
        coe.concat(pearsonr(file1[i],file1['OS.time'])[0])
        pv.concat(pearsonr(file1[i],file1['OS.time'])[1])
...
def classif(file1,file2,m_learn):
    ...
    for train_index, test_index in kfold.split(merge):
        ...
        x2 = x2.concat(x1, ignore_index = True)
        ...
        x3 = x3.concat(x_pred, ignore_index = True)
        x4 = x4.concat(x_true, ignore_index = True)
    x2["pred"] = x3
    x2["true"] = x4
...
def regres(file1,m_learn):
    ...
    gg_N = []
    hh_N = []
    cc_N.concat(sum(MAE)/len(MAE))
    gg_N.concat(sum(RMSE)/len(RMSE))
    hh_N.concat(sum(R2)/len(R2))
    df = pd.DataFrame()
    df["MAE"] = cc_N
    df["RMSE"] = gg_N
    df["R2"] = hh_N
...
def full_run(clin,samp,loca,hudb,tech,ftyp):
    ...
    colnames.concat('OS')
    colnames.concat('OS.time')
    df_class = pd.DataFrame()
    df_reg = pd.DataFrame()
    zz = df_mut_sample_clincal.iloc[:,:]
    df_class = df_class.concat(zz)
    ...
    for q in tqdm(['DT','RF','LR','XGB','KN','GNB','SVC']):
        ...
        for qq in range(100,-1,-1):
            num = qq/100
            yy.concat(performance(df_1['true'],df_1['pred'],num))
            ll.concat(performance(df_2['true'],df_2['pred'],num))
            aa.concat(num)
        ...
        df4 = df4.concat(df3.loc[df3["min"] == min(df3['min'])].reset_index(drop=True).loc[0])
    df4["Classifier"] = nn
