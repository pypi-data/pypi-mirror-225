...
class ClusterDaemon(OTPmeDaemon):
    ...
    def handle_cluster_out_journal(self, node_name):
        try:
            changed_entries = self.process_cluster_journal(node_name)
        except ProcessingFailed:
            return True
        except Exception as e:
        ...
        if self.member_candidate:
            ...
            try:
                try:
                    new_changed_entries = self.process_cluster_journal(node_name)
                except ProcessingFailed:
                    return True
                except Exception as e:
                    ...
                    config.raise_exception()
                    return True
                changed_entries = list(set(changed_entries+new_changed_entries))
                ...
                for object_id in changed_entries:
                    ...
                    try:
                        ...
                        self.logger.warning(msg)
                        raise OTPmeException(msg)
                    msg = ("Written object to node (member candidate): %s: %s (%s)"
            ...
            try:
                ...
                if action == "write":
                    ...
                    if not object_config:
                        ...
                        continue
                    if self.member_candidate:
                        if object_id in changed_entries:
                            cluster_journal_entry.delete()
                            continue
                        try:
    ...
    def check_member_nodes(self, cluster_journal_entry):
        if not config.cluster_status:
            return False
        if cluster_journal_entry.entry_dir in self.processed_journal_entries:
            return False
