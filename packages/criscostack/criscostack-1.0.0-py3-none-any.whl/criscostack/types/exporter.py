...
class TypeExporter:
	def __init__(self, doc):
		...
		self.imports = {"from criscostack.types import DF"}
		self.indent = "\t"
	...
	def _validate_code(code) -> bool:
		"""Make sure whatever code Crisco adds dynamically is valid python."""
		try:
			ast.parse(code)
			return True
		except Exception:
			criscostack.msgprint(criscostack._("Failed to export python type hints"), alert=True)
			return False
