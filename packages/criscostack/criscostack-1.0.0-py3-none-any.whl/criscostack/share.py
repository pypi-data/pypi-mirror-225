...
def add_docshare(
	...
	if share_name:
		doc = criscostack.get_doc("DocShare", share_name)
	else:
		doc = criscostack.new_doc("DocShare")
		doc.update(
	...
	if criscostack.get_cached_value("User", user, "follow_shared_documents"):
...
def remove(doctype, name, user, flags=None):
	share_name = criscostack.db.get_value(
		"DocShare", {"user": user, "share_name": name, "share_doctype": doctype}
	)

	if share_name:
		criscostack.delete_doc("DocShare", share_name, flags=flags)
...
def set_docshare_permission(doctype, name, user, permission_to, value=1, everyone=0, flags=None):
	...
	if not share_name:
		...
		share = criscostack.get_doc("DocShare", share_name)
		if flags:
...
def get_shared(doctype, user=None, rights=None, *, filters=None, limit=None):
	...
	shared_docs = criscostack.get_all(
		"DocShare",
		fields=["share_name"],
...
def get_shared_doctypes(user=None):
	...
	if not user:
		user = criscostack.session.user
	table = criscostack.qb.DocType("DocShare")
	query = (
...
def check_share_permission(doctype, name):
	...
	if not criscostack.has_permission(doctype, ptype="share", doc=name):
		criscostack.throw(
			_("No permission to {0} {1} {2}").format("share", _(doctype), name), criscostack.PermissionError
		)
...
def notify_assignment(shared_by, doctype, doc_name, everyone, notify=0):
	...
	reference_user = get_fullname(criscostack.session.user)
	notification_message = _("{0} shared a document {1} {2} with you").format(
		criscostack.bold(reference_user), criscostack.bold(_(doctype)), get_title_html(title)
	)
