...
import criscostack
import criscostack.commands.scheduler
import criscostack.commands.site
import criscostack.commands.utils
import criscostack.recorder
from criscostack.installer import add_to_installed_apps, remove_app
...
from criscostack.utils import add_to_date, get_bench_path, get_bench_relative_path, now
from criscostack.utils.backups import BackupGenerator, fetch_latest_backups
from criscostack.utils.jinja_globals import bundled_asset
from criscostack.utils.scheduler import enable_scheduler, is_scheduler_inactive

_result: Result | None = None
TEST_SITE = "commands-site-O4PN2QKA.test"  # added random string tag to avoid collisions
CLI_CONTEXT = criscostack._dict(sites=[TEST_SITE])
...
def missing_in_backup(doctypes: list, file: os.PathLike) -> list:
	...
	"""
	predicate = 'COPY public."tab{}"' if criscostack.conf.db_type == "postgres" else "CREATE TABLE `tab{}`"
...
@contextmanager
def maintain_locals():
	pre_site = criscostack.local.site
	pre_flags = criscostack.local.flags.copy()
	pre_db = criscostack.local.db

	try:
		yield
	finally:
		post_site = getattr(criscostack.local, "site", None)
		if not post_site or post_site != pre_site:
			criscostack.init(site=pre_site)
			criscostack.local.db = pre_db
			criscostack.local.flags.update(pre_flags)
...
def cli(cmd: Command, args: list | None = None):
	with maintain_locals():
		...
		patch_ctx = patch("criscostack.commands.pass_context", pass_test_context)
...
class BaseTestCommands(CriscoTestCase):
	@classmethod
	...
	def execute(self, command, kwargs=None):
		...
		site = {"site": criscostack.local.site}
		cmd_input = None
		if kwargs:
	...
	def setup_test_site(cls):
		cmd_config = {
			"test_site": TEST_SITE,
			"admin_password": criscostack.conf.admin_password,
			"root_login": criscostack.conf.root_login,
			"root_password": criscostack.conf.root_password,
...
class TestCommands(BaseTestCommands):
	def test_execute(self):
		...
		self.execute("bench --site {site} execute criscostack.db.get_database_size")
		self.assertEqual(self.returncode, 0)
		self.assertIsInstance(float(self.stdout), float)

		# test 2: execute a command expecting an errored output as local won't exist
		self.execute("bench --site {site} execute criscostack.local.site")
		...
		self.execute("""bench --site {site} execute criscostack.bold --kwargs '{{"text": "DocType"}}'""")
		self.assertEqual(self.returncode, 0)
		self.assertEqual(self.stdout[1:-1], criscostack.bold(text="DocType"))
	...
	def test_restore(self):
		...
		self.execute(
			"bench --site {test_site} execute criscostack.utils.backups.fetch_latest_backups",
			site_data,
		)
		...
		self.execute(
			"bench --site {test_site} execute" " criscostack.utils.backups.fetch_latest_backups --kwargs {kw}",
			site_data,
		)
		site_data.update({"database": json.loads(self.stdout)["database"]})
		self.execute("bench --site {test_site} restore {database}", site_data)
	...
	def test_partial_restore(self):
			...
			criscostack.get_doc(
				{
					...
					"description": criscostack.mock("paragraph"),
				}
			).insert()
		criscostack.db.commit()
		todo_count = criscostack.db.count("ToDo")
		...
		criscostack.db.sql_ddl("DROP TABLE IF EXISTS `tabToDo`")
		criscostack.db.commit()

		self.execute("bench --site {site} partial-restore {path}", {"path": db_path})
		self.assertEqual(self.returncode, 0)
		self.assertEqual(criscostack.db.count("ToDo"), todo_count)

	def test_recorder(self):
		criscostack.recorder.stop()

		self.execute("bench --site {site} start-recording")
		criscostack.local.cache = {}
		self.assertEqual(criscostack.recorder.status(), True)

		self.execute("bench --site {site} stop-recording")
		criscostack.local.cache = {}
		self.assertEqual(criscostack.recorder.status(), False)
	...
	def test_list_apps(self):
		...
		doctype = criscostack.get_single("Installed Applications").installed_applications
		if doctype:
			installed_apps = {x.app_name for x in doctype}
		else:
			installed_apps = set(criscostack.get_installed_apps())
	...
	def test_criscostack_site_env(self):
		os.putenv("CRISCOSTACK_SITE", criscostack.local.site)
		self.execute("bench execute criscostack.ping")
		self.assertEqual(self.returncode, 0)
	...
	def test_set_password(self):
		...
		original_password = criscostack.conf.admin_password or "admin"
	...
	@skipIf(
		not (
			criscostack.conf.root_password and criscostack.conf.admin_password and criscostack.conf.db_type == "mariadb"
		),
	...
	def test_bench_drop_site_should_archive_site(self):
		...
		self.execute(
			...
			f"--admin-password {criscostack.conf.admin_password} "
			f"--mariadb-root-password {criscostack.conf.root_password} "
			f"--db-type {criscostack.conf.db_type} "
		)
		self.assertEqual(self.returncode, 0)

		self.execute(f"bench drop-site {site} --force --root-password {criscostack.conf.root_password}")
		self.assertEqual(self.returncode, 0)
	...
	def test_force_install_app(self):
		...
		# set admin password in site_config as when criscostack force installs, we don't have the conf
		self.execute(f"bench --site {TEST_SITE} set-config admin_password {criscostack.conf.admin_password}")

		# try installing the criscostack_docs app again on test site
		self.execute(f"bench --site {TEST_SITE} install-app {app_name}")
		self.assertIn(f"{app_name} already installed", self.stdout)
		self.assertEqual(self.returncode, 0)

		# force install criscostack_docs app on the test site
	...
	def test_set_global_conf(self):
		...
		value = "42"
		self.execute(f"bench set-config {key} {value} -g")
		conf = criscostack.get_site_config()
...
class TestBackups(BaseTestCommands):
	...
	site_backup_path = criscostack.utils.get_site_path("private", "backups")
	...
	def test_backup_fails_with_exit_code(self):
		...
		odb = BackupGenerator(
			criscostack.conf.db_name,
			criscostack.conf.db_name,
			criscostack.conf.db_password + "INCORRECT PASSWORD",
			db_host=criscostack.conf.db_host,
			db_port=criscostack.conf.db_port,
			db_type=criscostack.conf.db_type,
		)
	...
	def test_clear_log_table(self):
		d = criscostack.get_doc(doctype="Error Log", title="Something").insert()
		d.db_set("modified", "2010-01-01", update_modified=False)
		criscostack.db.commit()

		tables_before = criscostack.db.get_tables(cached=False)

		self.execute("bench --site {site} clear-log-table --days=30 --doctype='Error Log'")
		self.assertEqual(self.returncode, 0)
		criscostack.db.commit()

		self.assertFalse(criscostack.db.exists("Error Log", d.name))
		tables_after = criscostack.db.get_tables(cached=False)
	...
	def test_backup_only_specific_doctypes(self):
		"""Take a backup with (include) backup options set in the site config `criscostack.conf.backup.includes`"""
		self.execute(
	...
	def test_backup_excluding_specific_doctypes(self):
		"""Take a backup with (exclude) backup options set (`criscostack.conf.backup.excludes`, `--exclude`)"""
		# test 1: take a backup with criscostack.conf.backup.excludes
		self.execute(
	...
	def test_selective_backup_priority_resolution(self):
		"""Take a backup with conflicting backup options set (`criscostack.conf.excludes`, `--include`)"""
		self.execute(
	...
	def test_dont_backup_conf(self):
		"""Take a backup ignoring criscostack.conf.backup settings (with --ignore-backup-conf option)"""
...
class TestRemoveApp(CriscoTestCase):
	def test_delete_modules(self):
		from criscostack.installer import (
			_delete_doctypes,
			_delete_modules,
			_get_module_linked_doctype_field_map,
		)

		test_module = criscostack.new_doc("Module Def")

		test_module.update({"module_name": "RemoveThis", "app_name": "criscostack"})
		test_module.save()

		module_def_linked_doctype = criscostack.get_doc(
			{
				"doctype": "DocType",
		...
		self.assertFalse(criscostack.db.exists("Module Def", test_module.module_name))
		self.assertFalse(criscostack.db.exists("DocType", module_def_linked_doctype.name))

	def test_dry_run(self):
		"""Check if dry run in not destructive."""

		# nothing to assert, if this fails rest of the test suite will crumble.
		remove_app("criscostack", dry_run=True, yes=True, no_backup=True)


class TestSiteMigration(BaseTestCommands):
	def test_migrate_cli(self):
		with cli(criscostack.commands.site.migrate) as result:
...
class TestAddNewUser(BaseTestCommands):
	def test_create_user(self):
		...
		self.assertEqual(self.returncode, 0)
		user = criscostack.get_doc("User", "test@gmail.com")
...
class TestBenchBuild(BaseTestCommands):
	def test_build_assets_size_check(self):
		with cli(criscostack.commands.utils.build, "--force --production") as result:
			self.assertEqual(result.exit_code, 0)
			self.assertEqual(result.exception, None)
		...
		for chunk in default_bundle:
			abs_path = Path.cwd() / criscostack.local.sites_path / bundled_asset(chunk)[1:]
...
class TestSchedulerUtils(BaseTestCommands):
	...
	def test_ready_for_migrate(self):
		with cli(criscostack.commands.scheduler.ready_for_migration) as result:
			self.assertEqual(result.exit_code, 0)


class TestCommandUtils(CriscoTestCase):
	def test_bench_helper(self):
		from criscostack.utils.bench_helper import get_app_groups

		app_groups = get_app_groups()
		self.assertIn("criscostack", app_groups)
		self.assertIsInstance(app_groups["criscostack"], click.Group)
...
class TestSchedulerCLI(BaseTestCommands):
	...
	def test_scheduler_status(self):
		...
		parsed_output = criscostack.parse_json(self.stdout)
		self.assertEqual(self.returncode, 0)
