...
import criscostack
from criscostack.desk.listview import get_group_by_count, get_list_settings, set_list_settings
from criscostack.desk.reportview import get
from criscostack.tests.utils import CriscoTestCase


class TestListView(CriscoTestCase):
	def setUp(self):
		if criscostack.db.exists("List View Settings", "DocType"):
			criscostack.delete_doc("List View Settings", "DocType")
	...
	def test_get_list_settings_with_default_settings(self):
		criscostack.get_doc({"doctype": "List View Settings", "name": "DocType"}).insert()
	...
	def test_get_list_settings_with_non_default_settings(self):
		criscostack.get_doc({"doctype": "List View Settings", "name": "DocType", "disable_count": 1}).insert()
		settings = get_list_settings("DocType")
		self.assertIsNotNone(settings)
	...
	def test_set_list_settings_without_settings(self):
		...
		settings = criscostack.get_doc("List View Settings", "DocType")
	...
	def test_list_view_child_table_filter_with_created_by_filter(self):
		if criscostack.db.exists("Note", "Test created by filter with child table filter"):
			criscostack.delete_doc("Note", "Test created by filter with child table filter")
	...
	def test_list_view_comment_count(self):
		criscostack.form_dict.doctype = "DocType"
		criscostack.form_dict.limit = "1"
		criscostack.form_dict.fields = [
			"`tabDocType`.`name`",
		]

		for with_comment_count in (1, True, "1"):
			criscostack.form_dict.with_comment_count = with_comment_count
