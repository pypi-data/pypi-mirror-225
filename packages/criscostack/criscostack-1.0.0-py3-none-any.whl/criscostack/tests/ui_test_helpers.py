...
from criscostack.utils import add_to_date, now

UI_TEST_USER = "criscostack@example.com"


def whitelist_for_tests(fn):
	if criscostack.request and not (criscostack.flags.in_test or getattr(criscostack.local, "dev_server", 0)):
		criscostack.throw("Cannot run UI tests. Use a development server with `bench start`")

	return criscostack.whitelist()(fn)
...
def create_if_not_exists(doc):
	...
	doc = criscostack.parse_json(doc)
	...
	for doc in docs:
		...
		filters = doc.copy()
		filters.pop("doctype")
		name = criscostack.db.exists(doc.doctype, filters)
		if not name:
			d = criscostack.get_doc(doc)
...
def prepare_webform_test():
	for note in criscostack.get_all("Note", pluck="name"):
		criscostack.delete_doc("Note", note, force=True)

	criscostack.delete_doc_if_exists("Web Form", "note")
	...
	doc = criscostack.get_doc(
		{
			...
			"communication_date": criscostack.utils.now_datetime(),
		}
	)
	doc.insert()
	return doc
...
@whitelist_for_tests
def setup_workflow():
	from criscostack.workflow.doctype.workflow.test_workflow import create_todo_workflow
...
def create_contact_phone_nos_records():
	if criscostack.get_all("Contact", {"first_name": "Test Contact"}):
		return

	doc = criscostack.new_doc("Contact")
...
def create_contact_records():
	if criscostack.get_all("Contact", {"first_name": "Test Form Contact 1"}):
		return
...
def create_multiple_todo_records():
	if criscostack.get_all("ToDo", {"description": "Multiple ToDo 1"}):
		return

	values = [(f"100{i}", f"Multiple ToDo {i}") for i in range(1, 1002)]

	criscostack.db.bulk_insert("ToDo", fields=["name", "description"], values=set(values))


def insert_contact(first_name, phone_number):
	doc = criscostack.get_doc({"doctype": "Contact", "first_name": first_name})
...
def create_form_tour():
	if criscostack.db.exists("Form Tour", {"name": "Test Form Tour"}):
		return

	tour = criscostack.get_doc(
		{
			"doctype": "Form Tour",
...
def create_web_page(title, route, single_thread):
	web_page = criscostack.db.exists("Web Page", {"route": route})
	if web_page:
		return web_page
	web_page = criscostack.get_doc(
	...
	web_page.append(
		"page_blocks",
		{
			...
			"web_template_values": criscostack.as_json(
				{
					"title": "Discussions",
...
def update_webform_to_multistep():
	if not criscostack.db.exists("Web Form", "update-profile-duplicate"):
		doc = criscostack.get_doc("Web Form", "edit-profile")
		_doc = criscostack.copy_doc(doc)
...
def insert_doctype_with_child_table_record(name):
	if criscostack.get_all(name, {"title": "Test Grid Search"}):
		return
...
def insert_translations():
	...
	for doc in translation:
		if not criscostack.db.exists("doc"):
			criscostack.get_doc(doc).insert()


@whitelist_for_tests
def create_blog_post():

	blog_category = criscostack.get_doc(
		{"name": "general", "doctype": "Blog Category", "title": "general"}
	).insert(ignore_if_duplicate=True)

	blogger = criscostack.get_doc(
		{
...
def create_test_user(username=None):
	...
	if criscostack.db.exists("User", name):
		return

	user = criscostack.new_doc("User")
	user.email = name
	user.first_name = "Crisco"
	user.new_password = criscostack.local.conf.admin_password
	...
	all_roles = set(criscostack.get_all("Role", pluck="name"))
...
def setup_tree_doctype():
	criscostack.delete_doc_if_exists("DocType", "Custom Tree", force=True)
	...
	if not criscostack.db.exists("Custom Tree", "All Trees"):
		criscostack.get_doc({"doctype": "Custom Tree", "tree": "All Trees"}).insert()


@whitelist_for_tests
def setup_image_doctype():
	criscostack.delete_doc_if_exists("DocType", "Custom Image", force=True)
...
@whitelist_for_tests
def setup_inbox():
	criscostack.db.delete("User Email")
...
def setup_default_view(view, force_reroute=None):
	criscostack.delete_doc_if_exists("Property Setter", "Event-main-default_view")
	criscostack.delete_doc_if_exists("Property Setter", "Event-main-force_re_route_to_default_view")
...
@whitelist_for_tests
def create_kanban():
	if not criscostack.db.exists("Custom Field", "Note-kanban"):
		criscostack.get_doc(
			{
				"is_system_generated": 0,
				"dt": "Note",
	...
	if not criscostack.db.exists("Kanban Board", "_Note _Kanban"):
		criscostack.get_doc(
			{
...
def create_todo(description):
	return criscostack.get_doc({"doctype": "ToDo", "description": description}).insert()


@whitelist_for_tests
def create_admin_kanban():
	if not criscostack.db.exists("Kanban Board", "Admin Kanban"):
		criscostack.get_doc(
			{
				"doctype": "Kanban Board",
				"name": "Admin Kanban",
...
def publish_progress(duration=3, title=None, doctype=None, docname=None):
	...
	criscostack.enqueue(slow_task, duration=duration, title=title, doctype=doctype, docname=docname)
...
def slow_task(duration, title, doctype, docname):
	...
	for i in range(steps + 1):
		criscostack.publish_progress(i * 10, title=title, doctype=doctype, docname=docname)
