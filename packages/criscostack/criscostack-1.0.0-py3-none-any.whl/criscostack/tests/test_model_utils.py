...
import criscostack
from criscostack.model import core_doctypes_list, get_permitted_fields
from criscostack.model.utils import get_fetch_values
from criscostack.tests.utils import CriscoTestCase


class TestModelUtils(CriscoTestCase):
	def test_get_fetch_values(self):
		...
		full_name = criscostack.db.get_value("User", user, "full_name")
	...
	def test_get_permitted_fields(self):
		...
		todo_all_columns = criscostack.get_meta("ToDo").get_valid_columns()
		...
		with set_user("Guest"):
			...
			picked_doctype_all_columns = criscostack.get_meta(picked_doctype).get_valid_columns()
		...
		with set_user("Administrator"):
			...
			child_all_fields = criscostack.get_meta("Installed Application").get_valid_columns()
...
def set_user(user: str):
	past_user = criscostack.session.user or "Administrator"
	criscostack.set_user(user)
	yield
	criscostack.set_user(past_user)
