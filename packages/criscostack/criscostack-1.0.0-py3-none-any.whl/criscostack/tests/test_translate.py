...
from criscostack.translate import (
	extract_javascript,
...
first_lang, second_lang, third_lang, fourth_lang, fifth_lang = choices(
	...
	criscostack.get_all("Language", pluck="name", filters=[["name", "not like", "en%"]]),
	k=5,
)


class TestTranslate(CriscoTestCase):
	...
	def tearDown(self):
		criscostack.form_dict.pop("_lang", None)
		if self._testMethodName in self.guest_sessions_required:
			criscostack.set_user("Administrator")

	def test_extract_message_from_file(self):
		data = criscostack.translate.get_messages_from_file(translation_string_file)
		exp_filename = "apps/criscostack/criscostack/tests/translation_test_file.txt"
	...
	def test_request_language_resolution_with_form_dict(self):
		"""Test for criscostack.translate.get_language

		Case 1: criscostack.form_dict._lang is set
		"""

		criscostack.form_dict._lang = first_lang

		with patch.object(criscostack.translate, "get_preferred_language_cookie", return_value=second_lang):
	...
	def test_request_language_resolution_with_cookie(self):
		...
		Case 2: criscostack.form_dict._lang is not set, but preferred_language cookie is
		"""

		with patch.object(criscostack.translate, "get_preferred_language_cookie", return_value="fr"):
	...
	def test_guest_request_language_resolution_with_cookie(self):
		...
		Case 3: criscostack.form_dict._lang is not set, but preferred_language cookie is [Guest User]
		"""
	...
	def test_guest_request_language_resolution_with_request_header(self):
		...
		Case 4: criscostack.form_dict._lang & preferred_language cookie is not set, but Accept-Language header is [Guest User]
		"""
	...
	def test_request_language_resolution_with_request_header(self):
		...
		Case 5: criscostack.form_dict._lang & preferred_language cookie is not set, but Accept-Language header is
		"""
	...
	def test_load_all_translate_files(self):
		...
		verify_translation_files("criscostack")
	...
	def test_python_extractor(self):
		...
		code = textwrap.dedent(
			...
			criscostack._("attr")
			_("name")
			criscostack._("attr with", context="attr context")
...
def verify_translation_files(app):
	...
	translations_dir = Path(criscostack.get_app_path(app)) / "translations"
