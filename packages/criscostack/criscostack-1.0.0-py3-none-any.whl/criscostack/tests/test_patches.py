...
import criscostack
from criscostack.modules import patch_handler
...
execute:criscostack.db.updatedb("Item")
execute:criscostack.function(arg="1")
...
class TestPatches(CriscoTestCase):
	def test_patch_module_names(self):
		criscostack.flags.final_patches = []
		criscostack.flags.in_install = True
		for patchmodule in patch_handler.get_all_patches():
			if patchmodule.startswith("execute:"):
				...
				self.assertTrue(criscostack.get_attr(patchmodule.split(maxsplit=1)[0] + ".execute"))

		criscostack.flags.in_install = False

	def test_get_patch_list(self):
		pre = patch_handler.get_patches_from_app("criscostack", patch_handler.PatchType.pre_model_sync)
		post = patch_handler.get_patches_from_app("criscostack", patch_handler.PatchType.post_model_sync)
		all_patches = patch_handler.get_patches_from_app("criscostack")
	...
	def test_all_patches_are_marked_completed(self):
		all_patches = patch_handler.get_patches_from_app("criscostack")
		finished_patches = criscostack.db.count("Patch Log")

		self.assertGreaterEqual(finished_patches, len(all_patches))


class TestPatchReader(CriscoTestCase):
	def get_patches(self):
		return (
			patch_handler.get_patches_from_app("criscostack"),
			patch_handler.get_patches_from_app("criscostack", patch_handler.PatchType.pre_model_sync),
			patch_handler.get_patches_from_app("criscostack", patch_handler.PatchType.post_model_sync),
		)
...
def check_patch_files(app):
	...
	patch_dir = Path(criscostack.get_app_path(app)) / "patches"
	...
	for file in patch_dir.glob("**/*.py"):
		...
		try:
			patch_module = criscostack.get_module(module)
