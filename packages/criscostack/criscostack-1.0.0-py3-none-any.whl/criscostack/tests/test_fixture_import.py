...
import criscostack
from criscostack.core.doctype.data_import.data_import import export_json, import_doc
...
class TestFixtureImport(CriscoTestCase):
	def create_new_doctype(self, DocType: str) -> None:
		file = criscostack.get_app_path("criscostack", "custom", "fixtures", f"{DocType}.json")
	...
	def insert_dummy_data_and_export(self, DocType: str, dummy_name_list: list[str]) -> str:
		for name in dummy_name_list:
			doc = criscostack.get_doc({"doctype": DocType, "member_name": name})
			doc.insert()
	...
	def test_fixtures_import(self):
		self.assertFalse(criscostack.db.exists("DocType", "temp_doctype"))
		...
		criscostack.db.truncate("temp_doctype")
		...
		self.assertEqual(criscostack.db.count("temp_doctype"), len(dummy_name_list))

		data = criscostack.get_all("temp_doctype", "member_name")
		criscostack.db.truncate("temp_doctype")
	...
	def test_singles_fixtures_import(self):
		self.assertFalse(criscostack.db.exists("DocType", "temp_singles"))
		...
		singles_doctype = criscostack.qb.DocType("Singles")
		truncate_query = (
			criscostack.qb.from_(singles_doctype).delete().where(singles_doctype.doctype == "temp_singles")
		)
		truncate_query.run()

		import_doc(path_to_exported_fixtures)

		data = criscostack.db.get_single_value("temp_singles", "member_name")
		truncate_query.run()
