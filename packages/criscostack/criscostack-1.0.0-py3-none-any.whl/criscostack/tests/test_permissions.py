...
from criscostack.core.doctype.user_permission.user_permission import clear_user_permissions
from criscostack.core.page.permission_manager.permission_manager import reset, update
...
from criscostack.test_runner import make_test_records_for_doctype
from criscostack.tests.utils import CriscoTestCase
from criscostack.utils.data import now_datetime

test_dependencies = ["Blogger", "Blog Post", "User", "Contact", "Salutation"]


class TestPermissions(CriscoTestCase):
	@classmethod
	def setUpClass(cls):
		super().setUpClass()
		criscostack.clear_cache(doctype="Blog Post")
		user = criscostack.get_doc("User", "test1@example.com")
		user.add_roles("Website Manager")
		user.add_roles("System Manager")

		user = criscostack.get_doc("User", "test2@example.com")
		user.add_roles("Blogger")

		user = criscostack.get_doc("User", "test3@example.com")
		user.add_roles("Sales User")

		user = criscostack.get_doc("User", "testperm@example.com")
		user.add_roles("Website Manager")
	...
	def setUp(self):
		...
		criscostack.db.delete("User Permission")

		criscostack.set_user("test1@example.com")

	def tearDown(self):
		criscostack.set_user("Administrator")
		criscostack.db.set_value("Blogger", "_Test Blogger 1", "user", None)
	...
	def set_strict_user_permissions(ignore):
		ss = criscostack.get_doc("System Settings")
		ss.apply_strict_user_permissions = ignore
		ss.flags.ignore_mandatory = 1
		ss.save()

	def test_basic_permission(self):
		post = criscostack.get_doc("Blog Post", "-test-blog-post")
	...
	def test_select_permission(self):
		...
		criscostack.clear_cache(doctype="Blog Post")
		criscostack.set_user("test3@example.com")
		...
		self.assertRaises(criscostack.PermissionError, post.save)

		permitted_record = criscostack.get_list("Blog Post", fields="*", limit=1)[0]
		full_record = criscostack.get_all("Blog Post", fields="*", limit=1)[0]
	...
	def test_user_permissions_in_doc(self):
		...
		post1 = criscostack.get_doc("Blog Post", "-test-blog-post-1")
	...
	def test_user_permissions_in_report(self):
		...
		criscostack.set_user("test2@example.com")
		names = [d.name for d in criscostack.get_list("Blog Post", fields=["name", "blog_category"])]
	...
	def test_user_link_match_doc(self):
		blogger = criscostack.get_doc("Blogger", "_Test Blogger 1")
		blogger.user = "test2@example.com"
		blogger.save()

		criscostack.set_user("test2@example.com")

		post = criscostack.get_doc("Blog Post", "-test-blog-post-2")
		self.assertTrue(post.has_permission("read"))
	...
	def test_user_link_match_report(self):
		...
		names = [d.name for d in criscostack.get_list("Blog Post", fields=["name", "owner"])]
	...
	def test_not_allowed_to_set_user_permissions(self):
		...
		self.assertRaises(
			criscostack.PermissionError, add_user_permission, "Blog Post", "-test-blog-post", "test2@example.com"
		)
	...
	def test_read_if_explicit_user_permissions_are_set(self):
		...
		doc = criscostack.get_doc("Blog Post", "-test-blog-post")
		self.assertTrue(doc.has_permission("read"))

		# and not this one
		doc = criscostack.get_doc("Blog Post", "-test-blog-post-1")
	...
	def test_set_standard_fields_manually(self):
		...
		fake_owner = criscostack.db.get_value("User", {"name": ("!=", criscostack.session.user)})
	...
	def test_dont_change_standard_constants(self):
		...
		self.assertRaises(criscostack.CannotChangeConstantError, user.save)
	...
	def test_set_only_once(self):
		blog_post = criscostack.get_meta("Blog Post")
		doc = criscostack.get_doc("Blog Post", "-test-blog-post-1")
		doc.db_set("title", "Old")
		blog_post.get_field("title").set_only_once = 1
		doc.title = "New"
		self.assertRaises(criscostack.CannotChangeConstantError, doc.save)
		blog_post.get_field("title").set_only_once = 0

	def test_set_only_once_child_table_rows(self):
		doctype_meta = criscostack.get_meta("DocType")
		doctype_meta.get_field("fields").set_only_once = 1
		doc = criscostack.get_doc("DocType", "Blog Post")
	...
	def test_user_permission_doctypes(self):
		...
		doc = criscostack.get_doc("Blog Post", "-test-blog-post-2")
		self.assertTrue(doc.has_permission("read"))
	...
	def test_insert_if_owner_with_user_permissions(self):
		...
		criscostack.delete_doc("Blog Post", "-test-blog-post-title")
		...
		self.assertRaises(criscostack.PermissionError, doc.insert)
		...
		criscostack.set_user("test2@example.com")
		doc = criscostack.get_doc(doc.doctype, doc.name)
		self.assertTrue(doc.has_permission("read"))
	...
	def test_strict_user_permissions(self):
		...
		criscostack.set_user("Administrator")
		criscostack.db.delete("Contact")
		criscostack.db.delete("Contact Email")
		criscostack.db.delete("Contact Phone")
		...
		allowed_contact = criscostack.get_doc("Contact", "_Test Contact For _Test Customer")
		other_contact = criscostack.get_doc("Contact", "_Test Contact For _Test Supplier")

		criscostack.set_user("test3@example.com")
		self.assertTrue(allowed_contact.has_permission("read"))
		self.assertTrue(other_contact.has_permission("read"))
		self.assertEqual(len(criscostack.get_list("Contact")), 2)
		...
		self.assertTrue(len(criscostack.get_list("Contact")), 1)
	...
	def test_user_permissions_not_applied_if_user_can_edit_user_permissions(self):
		...
		self.assertTrue(criscostack.get_doc("Blogger", "_Test Blogger").has_permission("read"))
	...
	def test_contextual_user_permission(self):
		...
		posts = criscostack.get_all("Blog Post", fields=["name", "blogger"])
		...
		posts = criscostack.get_list("Blog Post", fields=["name", "blogger"])
	...
	def test_if_owner_permission_overrides_properly(self):
		...
		criscostack.set_user("test2@example.com")
		self.assertEqual(criscostack.get_list("Blog Post"), [])
	...
	def test_if_owner_permission_on_getdoc(self):
		...
		doclist = [d.name for d in criscostack.response.docs]
		self.assertTrue(doc.name in doclist)

		criscostack.set_user("test2@example.com")
		self.assertRaises(criscostack.PermissionError, getdoc, "Blog Post", doc.name)
	...
	def test_if_owner_permission_on_get_list(self):
		...
		criscostack.set_user("test2@example.com")
		self.assertIn(doc.name, criscostack.get_list("Blog Post", pluck="name"))
		...
		criscostack.set_user("test2@example.com")
		self.assertNotIn(doc.name, criscostack.get_list("Blog Post", pluck="name"))
	...
	def test_if_owner_permission_on_delete(self):
		...
		self.assertRaises(criscostack.PermissionError, criscostack.delete_doc, "Blog Post", doc.name)

		criscostack.set_user("test2@example.com")
		criscostack.delete_doc("Blog Post", "-test-blog-post-title-new-1")
		update("Blog Post", "Website Manager", 0, "delete", 1)

	def test_clear_user_permissions(self):
		current_user = criscostack.session.user
		criscostack.set_user("Administrator")
		...
		# reset the user
		criscostack.set_user(current_user)

	def test_child_permissions(self):
		criscostack.set_user("test3@example.com")
		self.assertIsInstance(criscostack.get_list("DefaultValue", parent_doctype="User", limit=1), list)

		# criscostack.get_list
		self.assertRaises(criscostack.PermissionError, criscostack.get_list, "DefaultValue")
		self.assertRaises(criscostack.PermissionError, criscostack.get_list, "DefaultValue", parent_doctype="ToDo")
		self.assertRaises(
			criscostack.PermissionError, criscostack.get_list, "DefaultValue", parent_doctype="DefaultValue"
		)
		...
		self.assertRaises(criscostack.PermissionError, doc.check_permission)
		...
		user = criscostack.get_doc("User", "Administrator")
		doc = user.append("defaults")
		self.assertRaises(criscostack.PermissionError, doc.check_permission)
	...
	def test_select_user(self):
		...
		for gender in ("Male", "Female"):
			if criscostack.db.exists("Gender", gender):
				continue

			criscostack.get_doc({"doctype": "Gender", "gender": gender}).insert()

		# asssign gender to test users
		criscostack.db.set_value("User", "test1@example.com", "gender", "Male")
		criscostack.db.set_value("User", "test2@example.com", "gender", "Female")
		criscostack.db.set_value("User", "test3@example.com", "gender", "Female")
		...
		criscostack.set_user("test3@example.com")
		users = criscostack.get_list("User", pluck="name")
