...
from criscostack.model.naming import (
...
from criscostack.tests.utils import CriscoTestCase, patch_hooks
from criscostack.utils import now_datetime, nowdate, nowtime


class TestNaming(CriscoTestCase):
	def setUp(self):
	...
	def test_append_number_if_name_exists(self):
		...
		note = criscostack.get_doc({"doctype": DOCTYPE, "title": TITLE}).insert()
	...
	def test_field_autoname_name_sync(self):
		...
		country = criscostack.get_last_doc("Country")
	...
	def test_child_table_naming(self):
		...
		name = criscostack.generate_hash(length=10)

		doc = criscostack.new_doc(dt_with_child_autoname.name)
	...
	def test_format_autoname(self):
		...
		doc = criscostack.new_doc(doctype.name)
	...
	def test_format_autoname_for_consecutive_week_number(self):
		...
		todo_doctype = criscostack.get_doc("DocType", doctype)
		todo_doctype.autoname = "format:TODO-{WW}-{##}"
		todo_doctype.save()

		description = "Format"

		todo = criscostack.new_doc(doctype)
	...
	def test_revert_series(self):
		...
		criscostack.db.sql("""INSERT INTO `tabSeries` (name, current) values (%s, 1)""", (series,))
		revert_series_if_last(key, name)
		current_index = criscostack.db.sql(
			"""SELECT current from `tabSeries` where name = %s""", series, as_dict=True
		)[0]

		self.assertEqual(current_index.get("current"), 0)
		criscostack.db.delete("Series", {"name": series})

		series = f"TEST-{year}-"
		key = "TEST-.YYYY.-.#####"
		name = f"TEST-{year}-00002"
		criscostack.db.sql("""INSERT INTO `tabSeries` (name, current) values (%s, 2)""", (series,))
		revert_series_if_last(key, name)
		current_index = criscostack.db.sql(
			"""SELECT current from `tabSeries` where name = %s""", series, as_dict=True
		)[0]
		...
		name = "TEST-00003"
		criscostack.db.delete("Series", {"name": series})
		criscostack.db.sql("""INSERT INTO `tabSeries` (name, current) values (%s, 3)""", (series,))
		revert_series_if_last(key, name)
		current_index = criscostack.db.sql(
			"""SELECT current from `tabSeries` where name = %s""", series, as_dict=True
		)[0]
	...
	def test_naming_for_cancelled_and_amended_doc(self):
		submittable_doctype = criscostack.get_doc(
			{
				"doctype": "DocType",
				"module": "Core",
				"custom": 1,
		...
		doc = criscostack.new_doc("Submittable Doctype")
		doc.save()
		...
		amended_doc = criscostack.copy_doc(doc)
	...
	def test_naming_validations(self):
		...
		# set name via prompt
		tag = criscostack.get_doc({"doctype": "Tag", "__newname": "Tag"})
		self.assertRaises(criscostack.NameError, tag.insert)

		# set by passing set_name as ToDo
		self.assertRaises(criscostack.NameError, make_invalid_todo)

		# name (via title field) cannot be the same as the doctype
		note = criscostack.get_doc({"doctype": "Currency", "currency_name": "Currency"})
		self.assertRaises(criscostack.NameError, note.insert)

		# case 2: set name with "New ---"
		tag = criscostack.get_doc({"doctype": "Tag", "__newname": "New Tag"})
		self.assertRaises(criscostack.NameError, tag.insert)
		...
		tag = criscostack.get_doc({"doctype": "Tag", "__newname": ""})
		self.assertRaises(criscostack.ValidationError, tag.insert)

	def test_autoincremented_naming(self):
		from criscostack.core.doctype.doctype.test_doctype import new_doctype

		doctype = "autoinc_doctype" + criscostack.generate_hash(length=5)
		dt = new_doctype(doctype, autoname="autoincrement").insert(ignore_permissions=True)

		for i in range(1, 20):
			self.assertEqual(criscostack.new_doc(doctype).save(ignore_permissions=True).name, i)
	...
	def test_naming_series_validation(self):
		dns = criscostack.get_doc("Document Naming Settings")
	...
	def test_naming_using_fields(self):
		...
		webhook = criscostack.new_doc("Webhook")
		...
		with patch_hooks(
			{
				"naming_series_variables": {
					"PM": ["criscostack.tests.test_naming.parse_naming_series_variable"],
				},
			},
		):
...
def make_invalid_todo():
	criscostack.get_doc({"doctype": "ToDo", "description": "Test"}).insert(set_name="ToDo")
