...
from criscostack.database.utils import DefaultOrderBy
from criscostack.desk.reportview import get_filters_cond
from criscostack.handler import execute_cmd
from criscostack.model.db_query import DatabaseQuery
from criscostack.permissions import add_user_permission, clear_user_permissions_for_doctype
from criscostack.query_builder import Column
from criscostack.tests.utils import CriscoTestCase
from criscostack.utils.testutils import add_custom_field, clear_custom_fields
...
class TestDBQuery(CriscoTestCase):
	def setUp(self):
	...
	def test_child_table_field_syntax(self):
		note = criscostack.get_doc(
			doctype="Note",
			title=f"Test {criscostack.utils.random_string(8)}",
			content="test",
	...
	def test_child_table_join(self):
		criscostack.delete_doc_if_exists("DocType", "Parent DocType 1")
		criscostack.delete_doc_if_exists("DocType", "Parent DocType 2")
		criscostack.delete_doc_if_exists("DocType", "Child DocType")
		# child table
		criscostack.get_doc(
			{
				"doctype": "DocType",
		...
		# clear records
		criscostack.db.delete("Parent DocType 1")
		criscostack.db.delete("Parent DocType 2")
		criscostack.db.delete("Child DocType")
		...
		# test query
		results1 = criscostack.get_all("Parent DocType 1", fields=["name", "child.title as child_title"])
		results2 = criscostack.get_all("Parent DocType 2", fields=["name", "child.title as child_title"])
	...
	def test_build_match_conditions(self):
		...
		test2user = criscostack.get_doc("User", "test2@example.com")
		...
		# get as conditions
		if criscostack.db.db_type == "mariadb":
	...
	def test_none_filter(self):
		query = criscostack.qb.get_query("DocType", fields="name", filters={"restrict_to_domain": None})
	...
	def test_ignore_permissions_for_get_filters_cond(self):
		criscostack.set_user("test2@example.com")
		self.assertRaises(criscostack.PermissionError, get_filters_cond, "DocType", dict(istable=1), [])
	...
	def test_query_fields_sanitizer(self):
		...
		with self.assertRaises(criscostack.DataError):
			DatabaseQuery("DocType").execute(
				fields=["name", "issingle", "if (issingle=1, (select name from tabUser), count(name))"],
				limit_start=0,
				limit_page_length=1,
	...
	def test_nested_permission(self):
		...
		from criscostack.core.page.permission_manager.permission_manager import update
	...
	def test_order_by_group_by_sanitizer(self):
		...
		with self.assertRaises(criscostack.ValidationError):
			DatabaseQuery("DocType").execute(
				fields=["name"],
	...
	def test_of_not_of_descendant_ancestors(self):
		...
		data = criscostack.get_all("Nested DocType", {"name": ("descendants of", "Level 2 A")})
		self.assertTrue({"name": "Level 3 A"} in data)

		data = criscostack.get_all("Nested DocType", {"name": ("descendants of", "Level 1 A")})
		self.assertTrue({"name": "Level 3 A"} in data)
		self.assertTrue({"name": "Level 2 A"} in data)
		self.assertFalse({"name": "Level 2 B"} in data)
		self.assertFalse({"name": "Level 1 B"} in data)
		...
		data = criscostack.get_all("Nested DocType", {"name": ("ancestors of", "Level 2 A")})
		...
		data = criscostack.get_all("Nested DocType", {"name": ("ancestors of", "Level 1 A")})
		self.assertFalse({"name": "Level 3 A"} in data)
		self.assertFalse({"name": "Level 2 A"} in data)
		self.assertFalse({"name": "Level 2 B"} in data)
		self.assertFalse({"name": "Level 1 B"} in data)
		...
		data = criscostack.get_all("Nested DocType", {"name": ("not descendants of", "Level 2 A")})
		self.assertFalse({"name": "Level 3 A"} in data)
		self.assertTrue({"name": "Level 2 A"} in data)
		...
		data = criscostack.get_all("Nested DocType", {"name": ("not descendants of", "Level 1 A")})
		self.assertFalse({"name": "Level 3 A"} in data)
		self.assertFalse({"name": "Level 2 A"} in data)
		self.assertTrue({"name": "Level 2 B"} in data)
		...
		data = criscostack.get_all("Nested DocType", {"name": ("not ancestors of", "Level 2 A")})
		self.assertTrue({"name": "Level 3 A"} in data)
		self.assertTrue({"name": "Level 2 A"} in data)
		self.assertTrue({"name": "Level 2 B"} in data)
		self.assertTrue({"name": "Level 1 B"} in data)
		...
		data = criscostack.get_all("Nested DocType", {"name": ("not ancestors of", "Level 1 A")})
		self.assertTrue({"name": "Level 3 A"} in data)
		self.assertTrue({"name": "Level 2 A"} in data)
		self.assertTrue({"name": "Level 2 B"} in data)
		self.assertTrue({"name": "Level 1 B"} in data)
		...
		data = criscostack.get_all("Nested DocType", {"name": ("ancestors of", "Root")})
		self.assertTrue(len(data) == 0)
		self.assertTrue(
			len(criscostack.get_all("Nested DocType", {"name": ("not ancestors of", "Root")}))
			== len(criscostack.get_all("Nested DocType"))
		)
	...
	def test_is_set_is_not_set(self):
		...
		criscostack.db.set_value("DocType", "Property Setter", "autoname", None, update_modified=False)
	...
	def test_prepare_select_args(self):
		# criscostack.get_all inserts modified field into order_by clause
	...
	def test_column_comparison(self):
		...
		users_unedited = criscostack.get_all(
			"User",
			filters={"creation": Column("modified")},
			fields=["name", "creation", "modified"],
			limit=1,
		)
		users_edited = criscostack.get_all(
			"User",
			filters={"creation": ("!=", Column("modified"))},
			fields=["name", "creation", "modified"],
			limit=1,
	...
	def test_cast_name(self):
		...
		criscostack.delete_doc_if_exists("DocType", "autoinc_dt_test")
	...
	def test_fieldname_starting_with_int(self):
		...
		criscostack.delete_doc_if_exists("DocType", "dt_with_int_named_fieldname")
		criscostack.delete_doc_if_exists("DocType", "table_dt")
		...
		dt_data = criscostack.get_doc({"doctype": "dt_with_int_named_fieldname", "1field": "10"}).insert(
			ignore_permissions=True
		)
	...
	def test_permission_query_condition(self):
		from criscostack.desk.doctype.dashboard_settings.dashboard_settings import create_dashboard_settings
		...
		dashboard_settings = criscostack.db.sql(
			"""
				SELECT name
	...
	def test_virtual_doctype(self):
		...
		with patch("criscostack.controllers", new={criscostack.local.site: {"Virtual DocType": VirtualDocType}}):
			VirtualDocType.get_list = MagicMock()

			criscostack.get_all("Virtual DocType", filters={"name": "test"}, fields=["name"], limit=1)
	...
	def test_coalesce_with_in_ops(self):
		self.assertNotIn("ifnull", criscostack.get_all("User", {"name": ("in", ["a", "b"])}, run=0))
		self.assertIn("ifnull", criscostack.get_all("User", {"name": ("in", ["a", None])}, run=0))
		self.assertIn("ifnull", criscostack.get_all("User", {"name": ("in", ["a", ""])}, run=0))
		self.assertIn("ifnull", criscostack.get_all("User", {"name": ("in", [])}, run=0))
		self.assertIn("ifnull", criscostack.get_all("User", {"name": ("not in", ["a"])}, run=0))
		self.assertIn("ifnull", criscostack.get_all("User", {"name": ("not in", [])}, run=0))
		self.assertIn("ifnull", criscostack.get_all("User", {"name": ("not in", [""])}, run=0))

	def test_ambiguous_linked_tables(self):
		from criscostack.desk.reportview import get

		if not criscostack.db.exists("DocType", "Related Todos"):
			criscostack.get_doc(
				{
					"doctype": "DocType",
					"custom": 1,
			...
			criscostack.db.delete("Related Todos")

		todo_one = criscostack.get_doc(
			{
				"doctype": "ToDo",
		...
		criscostack.form_dict.doctype = "Related Todos"
		criscostack.form_dict.fields = [
...
class TestReportView(CriscoTestCase):
	def test_get_count(self):
		criscostack.local.request = criscostack._dict()
		criscostack.local.request.method = "GET"
		...
		list_filter_response = execute_cmd("criscostack.desk.reportview.get_count")
		criscostack.local.form_dict = criscostack._dict(
			{"doctype": "DocType", "filters": {"show_title_field_in_link": 1}, "distinct": "true"}
		)
		dict_filter_response = execute_cmd("criscostack.desk.reportview.get_count")
		...
		child_filter_response = execute_cmd("criscostack.desk.reportview.get_count")
		current_value = criscostack.db.sql(
	...
	def test_reportview_get(self):
		...
		criscostack.local.request = criscostack._dict()
		criscostack.local.request.method = "POST"
		...
		response = execute_cmd("criscostack.desk.reportview.get")
...
def add_child_table_to_blog_post():
	child_table = criscostack.get_doc(
		{
			"doctype": "DocType",
			"istable": 1,
			"custom": 1,
...
def create_event(subject="_Test Event", starts_on=None):
	...
	from criscostack.utils import get_datetime
...
def create_nested_doctype():
	if criscostack.db.exists("DocType", "Nested DocType"):
		return
...
def create_nested_doctype_records():
	...
	for r in records:
		d = criscostack.new_doc("Nested DocType")
		d.update(r)
