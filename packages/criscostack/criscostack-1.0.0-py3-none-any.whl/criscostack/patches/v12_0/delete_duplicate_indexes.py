...
def execute():
	...
	all_tables = criscostack.db.get_tables()
	final_deletion_map = criscostack._dict()

	for table in all_tables:
		indexes_to_keep_map = criscostack._dict()
		indexes_to_delete = []
		index_info = criscostack.db.sql(
		...
		for index in index_list:
			try:
				if is_clustered_index(table_name, index):
					continue
				criscostack.db.sql_ddl(f"ALTER TABLE `{table_name}` DROP INDEX `{index}`")
			except Exception as e:
				criscostack.log_error("Failed to drop index")
