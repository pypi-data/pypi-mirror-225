...
def execute():
	if criscostack.db.table_exists("View log"):
		...
		criscostack.db.sql("CREATE TABLE `ViewLogTemp` AS SELECT * FROM `tabView log`")

		# deleting old View log table
		criscostack.db.sql("DROP table `tabView log`")
		criscostack.delete_doc("DocType", "View log")

		# reloading view log doctype to create `tabView Log` table
		criscostack.reload_doc("core", "doctype", "view_log")

		# Move the data to newly created `tabView Log` table
		criscostack.db.sql("INSERT INTO `tabView Log` SELECT * FROM `ViewLogTemp`")
		criscostack.db.commit()

		# Delete temporary table
		criscostack.db.sql("DROP table `ViewLogTemp`")
	else:
		criscostack.reload_doc("core", "doctype", "view_log")
