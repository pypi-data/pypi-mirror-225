...
def execute():
	...
	criscostack.reload_doc("integrations", "doctype", "social_login_key", force=True)

	if not criscostack.db.exists("DocType", "Social Login Keys"):
		return

	social_login_keys = criscostack.get_doc("Social Login Keys", "Social Login Keys")
	if social_login_keys.get("facebook_client_id") or social_login_keys.get("facebook_client_secret"):
		facebook_login_key = criscostack.new_doc("Social Login Key")
	...
	if social_login_keys.get("criscostack_server_url"):
		criscostack_login_key = criscostack.new_doc("Social Login Key")
		criscostack_login_key.get_social_login_provider("Crisco", initialize=True)
		criscostack_login_key.social_login_provider = "Crisco"
		criscostack_login_key.base_url = social_login_keys.get("criscostack_server_url")
		criscostack_login_key.client_id = social_login_keys.get("criscostack_client_id")
		criscostack_login_key.client_secret = social_login_keys.get("criscostack_client_secret")
		if not (
			criscostack_login_key.client_secret and criscostack_login_key.client_id and criscostack_login_key.base_url
		):
			criscostack_login_key.enable_social_login = 0
		criscostack_login_key.save()

	if social_login_keys.get("github_client_id") or social_login_keys.get("github_client_secret"):
		github_login_key = criscostack.new_doc("Social Login Key")
	...
	if social_login_keys.get("google_client_id") or social_login_keys.get("google_client_secret"):
		google_login_key = criscostack.new_doc("Social Login Key")
	...
	criscostack.delete_doc("DocType", "Social Login Keys")


def run_patch():
	criscostack.reload_doc("core", "doctype", "user", force=True)
	criscostack.reload_doc("core", "doctype", "user_social_login", force=True)
	...
	for user in users:
		idx = 0
		if user.criscostack_userid:
			insert_user_social_login(user.name, user.modified_by, "criscostack", idx, userid=user.criscostack_userid)
			idx += 1
...
def insert_user_social_login(user, modified_by, provider, idx, userid=None, username=None):
	...
	creation_time = criscostack.utils.get_datetime_str(criscostack.utils.get_datetime())
	values = [
		criscostack.generate_hash(length=10),
		creation_time,
		creation_time,
		user,
		modified_by,
	...
	query = """INSERT INTO `tabUser Social Login` (`{source_cols}`)
		VALUES ({values})
	""".format(
		source_cols="`, `".join(source_cols), values=", ".join([criscostack.db.escape(d) for d in values])
	)
...
def get_provider_field_map():
	return criscostack._dict(
		{
			"criscostack": ["criscostack_userid"],
