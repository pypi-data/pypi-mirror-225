import criscostack
from criscostack.email.doctype.newsletter.newsletter import confirmed_unsubscribe
...
def get_context(context):
	...
	if "email" in criscostack.form_dict and criscostack.request.method == "GET":
		if verify_request():
			user_email = criscostack.form_dict["email"]
			context.email = user_email
			title = criscostack.form_dict["name"]
	...
	elif "user_email" in criscostack.form_dict and criscostack.request.method == "POST":
		context.status = "unsubscribed"
		email = criscostack.form_dict["user_email"]
		email_group = get_email_groups(email)
		for group in email_group:
			if group.email_group in criscostack.form_dict:
