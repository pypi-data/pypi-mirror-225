...
import criscostack
from criscostack import _, get_module_path
from criscostack.core.doctype.access_log.access_log import make_access_log
from criscostack.core.doctype.document_share_key.document_share_key import is_expired
from criscostack.utils import cint, escape_html, strip_html
from criscostack.utils.jinja_globals import is_rtl

if TYPE_CHECKING:
	from criscostack.model.document import Document
	from criscostack.printing.doctype.print_format.print_format import PrintFormat
...
def get_context(context):
	...
	if not ((criscostack.form_dict.doctype and criscostack.form_dict.name) or criscostack.form_dict.doc):
		return {
			"body": f"""
				<h1>Error</h1>
				<p>Parameters doctype and name required</p>
				<pre>{escape_html(criscostack.as_json(criscostack.form_dict, indent=2))}</pre>
				"""
		}

	if criscostack.form_dict.doc:
		doc = criscostack.form_dict.doc
	else:
		doc = criscostack.get_doc(criscostack.form_dict.doctype, criscostack.form_dict.name)

	set_link_titles(doc)

	settings = criscostack.parse_json(criscostack.form_dict.settings)

	letterhead = criscostack.form_dict.letterhead or None
	...
	make_access_log(
		doctype=criscostack.form_dict.doctype, document=criscostack.form_dict.name, file_type="PDF", method="Print"
	)
	...
	body = get_rendered_template(
		...
		meta=meta,
		trigger_print=criscostack.form_dict.trigger_print,
		no_letterhead=criscostack.form_dict.no_letterhead,
		letterhead=letterhead,
		settings=settings,
	)
	print_style = get_print_style(criscostack.form_dict.style, print_format)

	return {
		"body": body,
		"print_style": print_style,
		"comment": criscostack.session.user,
		"title": criscostack.utils.strip_html(doc.get_title() or doc.name),
		"lang": criscostack.local.lang,
		"layout_direction": "rtl" if is_rtl() else "ltr",
		"doctype": criscostack.form_dict.doctype,
		"name": criscostack.form_dict.name,
		"key": criscostack.form_dict.get("key"),
	}
...
def get_print_format_doc(print_format_name, meta):
	...
	if not print_format_name:
		print_format_name = criscostack.form_dict.format or meta.default_print_format or "Standard"

	if print_format_name == "Standard":
		return None
	else:
		try:
			return criscostack.get_doc("Print Format", print_format_name)
...
def get_rendered_template(
	...
	print_settings = criscostack.get_single("Print Settings").as_dict()
	...
	if not criscostack.flags.ignore_print_permissions:
		validate_print_permission(doc)

	if doc.meta.is_submittable:
		if doc.docstatus.is_draft() and not cint(print_settings.allow_print_for_draft):
			criscostack.throw(_("Not allowed to print draft documents"), criscostack.PermissionError)

		if doc.docstatus.is_cancelled() and not cint(print_settings.allow_print_for_cancelled):
			criscostack.throw(_("Not allowed to print cancelled documents"), criscostack.PermissionError)
	...
	letter_head = criscostack._dict(get_letter_head(doc, no_letterhead, letterhead) or {})

	if letter_head.content:
		letter_head.content = criscostack.utils.jinja.render_template(
			letter_head.content, {"doc": doc.as_dict()}
		)

	if letter_head.footer:
		letter_head.footer = criscostack.utils.jinja.render_template(
	...
	hook_func = criscostack.get_hooks("pdf_body_html")
	html = criscostack.get_attr(hook_func[-1])(
...
def set_title_values_for_link_and_dynamic_link_fields(meta, doc, parent_doc=None):
	...
	for field in meta.get_link_fields() + meta.get_dynamic_link_fields():
		...
		link_title = criscostack.get_cached_value(doctype, doc.get(field.fieldname), meta.title_field)
		if parent_doc:
...
def set_title_values_for_table_and_multiselect_fields(meta, doc):
	for field in meta.get_table_fields():
		...
		_meta = criscostack.get_meta(field.options)
	...
	for field in doc.meta.fields:
		if field.fieldtype == "Text Editor":
			...
			if value and "<!-- markdown -->" in value:
				doc.set(field.fieldname, criscostack.utils.md_to_html(value))
...
def get_html_and_style(
	...
	if isinstance(name, str):
		document = criscostack.get_doc(doc, name)
	else:
		document = criscostack.get_doc(json.loads(doc))
	...
	try:
		html = get_rendered_template(
			...
			settings=criscostack.parse_json(settings),
		)
	except criscostack.TemplateNotFoundError:
...
def get_rendered_raw_commands(doc: str, name: str | None = None, print_format: str | None = None):
	...
	if not print_format or (print_format and not print_format.raw_printing):
		criscostack.throw(
			_("{0} is not a raw printing format.").format(print_format), criscostack.TemplateNotFoundError
		)
...
def validate_print_permission(doc):
	for ptype in ("read", "print"):
		if criscostack.has_permission(doc.doctype, ptype, doc) or criscostack.has_website_permission(doc):
			return

	key = criscostack.form_dict.key
	if key and isinstance(key, str):
		validate_key(key, doc)
	else:
		raise criscostack.PermissionError(_("You do not have permission to view this document"))


def validate_key(key, doc):
	document_key_expiry = criscostack.get_cached_value(
		"Document Share Key",
	...
	if document_key_expiry is not None:
		if is_expired(document_key_expiry[0]):
			raise criscostack.exceptions.LinkExpired
		else:
			return

	# TODO: Deprecate this! kept it for backward compatibility
	if criscostack.get_system_settings("allow_older_web_view_links") and key == doc.get_signature():
		return

	raise criscostack.exceptions.InvalidKeyError
...
def get_letter_head(doc: "Document", no_letterhead: bool, letterhead: str | None = None):
	if no_letterhead:
		return {}
	if letterhead:
		return criscostack.db.get_value("Letter Head", letterhead, ["content", "footer"], as_dict=True)
	if doc.get("letter_head"):
		return criscostack.db.get_value("Letter Head", doc.letter_head, ["content", "footer"], as_dict=True)
	else:
		return (
			criscostack.db.get_value("Letter Head", {"is_default": 1}, ["content", "footer"], as_dict=True) or {}
		)
...
def get_print_format(doctype, print_format):
	if print_format.disabled:
		criscostack.throw(
			_("Print Format {0} is disabled").format(print_format.name), criscostack.DoesNotExistError
		)

	# server, find template
	module = print_format.module or criscostack.db.get_value("DocType", doctype, "module")
	path = os.path.join(
		get_module_path(module, "Print Format", print_format.name),
		criscostack.scrub(print_format.name) + ".html",
	)
	...
	if os.path.exists(path):
		...
		criscostack.throw(_("No template found at path: {0}").format(path), criscostack.TemplateNotFoundError)
...
def make_layout(doc, meta, format_data=None):
	...
	for df in format_data or meta.fields:
		...
		if df.fieldtype == "Signature" and not doc.get(df.fieldname):
			placeholder_image = "/assets/criscostack/images/signature-placeholder.png"
...
def get_print_style(
	...
	css = criscostack.get_template("templates/styles/standard.css").render(context)

	if style and criscostack.db.exists("Print Style", style):
		css = css + "\n" + criscostack.db.get_value("Print Style", style, "css")
...
def get_visible_columns(data, table_meta, df):
	...
	columns = []
	doc = data[0] or criscostack.new_doc(df.options)
