...
import criscostack
from criscostack.core.doctype.data_export.exporter import DataExporter
from criscostack.tests.utils import CriscoTestCase


class TestDataExporter(CriscoTestCase):
	def setUp(self):
	...
	def create_test_data(self, force=False):
		...
		"""
		if force:
			criscostack.delete_doc(self.doctype_name, self.doc_name)

		if not criscostack.db.exists(self.doctype_name, self.doc_name):
			self.doc = criscostack.get_doc(
			...
			self.doc = criscostack.get_doc(self.doctype_name, self.doc_name)

	def test_export_content(self):
		exp = DataExporter(doctype=self.doctype_name, file_type="CSV")
		exp.build_response()

		self.assertEqual(criscostack.response["type"], "csv")
		self.assertEqual(criscostack.response["doctype"], self.doctype_name)
		self.assertTrue(criscostack.response["result"])
		self.assertIn('Child Title 1",50', criscostack.response["result"])
		self.assertIn('Child Title 2",51', criscostack.response["result"])
		...
		for type in ["csv", "Excel"]:
			with self.subTest(type=type):
				...
				if type == "csv":
					...
					self.assertEqual(
						criscostack.response["filename"], self.doctype_name + ".xlsx"
					)  # 'Test DocType for Export Tool.xlsx')
					self.assertTrue(criscostack.response["filecontent"])
