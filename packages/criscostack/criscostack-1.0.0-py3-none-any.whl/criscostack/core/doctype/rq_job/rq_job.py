...
from criscostack.utils.background_jobs import get_queues, get_redis_conn
...
class RQJob(Document):
	...
	def stop_job(self):
		try:
			...
			criscostack.msgprint(_("Job is not running."), title=_("Invalid Operation"))
...
def serialize_job(job: Job) -> criscostack._dict:
	...
	if job_name == "criscostack.utils.background_jobs.run_doc_method":
	...
	return criscostack._dict(
		...
		arguments=criscostack.as_json(job.kwargs),
		timeout=job.timeout,
...
def for_current_site(job: Job) -> bool:
	return job.kwargs.get("site") == criscostack.local.site
...
@criscostack.whitelist()
def stop_job(job_id):
	criscostack.get_doc("RQ Job", job_id).stop_job()
