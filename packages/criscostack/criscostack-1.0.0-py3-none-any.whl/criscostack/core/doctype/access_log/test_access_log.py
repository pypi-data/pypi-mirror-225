...
from criscostack.utils import cstr, get_site_url


class TestAccessLog(CriscoTestCase):
	def setUp(self):
		# generate keys for current user to send requests for the following tests
		generate_keys(criscostack.session.user)
		criscostack.db.commit()
		generated_secret = criscostack.utils.password.get_decrypted_password(
			"User", criscostack.session.user, fieldname="api_secret"
		)
		...
		self.file_name = criscostack.utils.random_string(10) + ".txt"
		self.test_content = criscostack.utils.random_string(1024)
	...
	def test_make_full_access_log(self):
		...
		last_doc = criscostack.get_last_doc("Access Log")
	...
	def test_private_file_download(self):
		...
		new_private_file = criscostack.get_doc(
			{
				"doctype": self.test_doctype,
		...
		private_file_link = get_site_url(criscostack.local.site) + new_private_file.file_url
