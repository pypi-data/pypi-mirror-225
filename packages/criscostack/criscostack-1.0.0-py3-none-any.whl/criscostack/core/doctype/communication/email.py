...
def make(
	...
	**kwargs,
	...
	"""
	if kwargs:
		from criscostack.utils.commands import warn

		warn(
			f"Options {kwargs} used in criscostack.core.doctype.communication.email.make "
			"are deprecated or unsupported",
			category=DeprecationWarning,
		)

	if doctype and name and not criscostack.has_permission(doctype=doctype, ptype="email", doc=name):
		raise criscostack.PermissionError(f"You are not allowed to send emails related to: {doctype} {name}")
...
def _make(
	...
	sender = sender or get_formatted_email(criscostack.session.user)
	...
	if cint(send_email):
		if not comm.get_outgoing_email_account():
			criscostack.throw(
				...
				exc=criscostack.OutgoingEmailError,
			)
...
def mark_email_as_seen(name: str = None):
	try:
		...
		criscostack.db.commit()  # nosemgrep: this will be called in a GET request

	except Exception:
		criscostack.log_error("Unable to mark as seen", None, "Communication", name)
...
def update_communication_as_read(name):
	...
	communication = criscostack.db.get_value("Communication", name, "read_by_recipient", as_dict=True)
