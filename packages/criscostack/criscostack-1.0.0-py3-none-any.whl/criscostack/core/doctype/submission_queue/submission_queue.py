...
from criscostack.utils import now, time_diff_in_seconds
from criscostack.utils.data import cint
...
class SubmissionQueue(Document):
	...
	@property
	def queued_doc(self):
		return getattr(self, "to_be_queued_doc", criscostack.get_doc(self.ref_doctype, self.ref_docname))
	...
	def clear_old_logs(days=30):
		...
		table = criscostack.qb.DocType("Submission Queue")
	...
	def background_submission(self, to_be_queued_doc: Document, action_for_queuing: str):
		...
		try:
			...
			values = {"status": "Failed", "exception": criscostack.get_traceback(with_context=True)}
			criscostack.db.rollback()

		values["ended_at"] = now()
		criscostack.db.set_value(self.doctype, self.name, values, update_modified=False)
	...
	def notify(self, submission_status: str, action: str):
		...
		message_replacements = (
			criscostack.bold(action),
			criscostack.bold(str(self.ref_doctype)),
			criscostack.bold(str(self.ref_docname)),
		)
		...
		if cint(time_diff) <= 60:
			...
			notify_to = criscostack.db.get_value("User", self.enqueued_by, fieldname="email")
	...
	def unlock_doc(self):
		...
		self.queued_doc.unlock()
		criscostack.msgprint(_("Document Unlocked"))


def queue_submission(doc: Document, action: str, alert: bool = True):
	queue = criscostack.new_doc("Submission Queue")
...
def get_latest_submissions(doctype, docname):
	...
	latest_submission = criscostack.db.get_value(
		"Submission Queue",
