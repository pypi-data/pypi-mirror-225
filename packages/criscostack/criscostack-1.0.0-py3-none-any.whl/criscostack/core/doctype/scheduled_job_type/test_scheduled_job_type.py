...
class TestScheduledJobType(CriscoTestCase):
	def setUp(self):
		criscostack.db.rollback()
		criscostack.db.truncate("Scheduled Job Type")
		sync_jobs()
		criscostack.db.commit()

	def test_sync_jobs(self):
		all_job = criscostack.get_doc("Scheduled Job Type", dict(method="criscostack.email.queue.flush"))
		self.assertEqual(all_job.frequency, "All")

		daily_job = criscostack.get_doc(
			"Scheduled Job Type", dict(method="criscostack.desk.notifications.clear_notifications")
		)
		self.assertEqual(daily_job.frequency, "Daily")

		# check if cron jobs are synced
		cron_job = criscostack.get_doc("Scheduled Job Type", dict(method="criscostack.oauth.delete_oauth2_data"))
		...
		updated_scheduler_events = {"hourly": ["criscostack.email.queue.flush"]}
		sync_jobs(updated_scheduler_events)
		updated_scheduled_job = criscostack.get_doc(
			"Scheduled Job Type", {"method": "criscostack.email.queue.flush"}
		)
	...
	def test_weekly_job(self):
		job = criscostack.get_doc(
			"Scheduled Job Type",
			dict(method="criscostack.social.doctype.energy_point_log.energy_point_log.send_weekly_summary"),
		)
		job.db_set("last_execution", "2019-01-01 00:00:00")
	...
	def test_monthly_job(self):
		job = criscostack.get_doc(
			"Scheduled Job Type",
			dict(method="criscostack.email.doctype.auto_email_report.auto_email_report.send_monthly"),
		)
		job.db_set("last_execution", "2019-01-01 00:00:00")
	...
	def test_cron_job(self):
		# runs every 15 mins
		job = criscostack.get_doc("Scheduled Job Type", dict(method="criscostack.oauth.delete_oauth2_data"))
		job.db_set("last_execution", "2019-01-01 00:00:00")
