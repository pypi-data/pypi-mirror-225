...
from criscostack.translate import clear_cache


class TestTranslation(CriscoTestCase):
	def setUp(self):
		criscostack.db.delete("Translation")
	...
	def test_doctype(self):
		...
		for key, val in translation_data.items():
			criscostack.local.lang = key

			translation = create_translation(key, val)
			self.assertEqual(_(val[0]), val[1])

			criscostack.delete_doc("Translation", translation.name)
...
def create_translation(key, val):
	translation = criscostack.new_doc("Translation")
