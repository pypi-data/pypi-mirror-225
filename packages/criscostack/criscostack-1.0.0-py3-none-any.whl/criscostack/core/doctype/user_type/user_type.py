...
from criscostack.config import get_modules_from_app
from criscostack.model.document import Document
from criscostack.permissions import add_permission, add_user_permission
from criscostack.utils import get_link_to_form
...
class UserType(Document):
	...
	if TYPE_CHECKING:
		from criscostack.core.doctype.user_document_type.user_document_type import UserDocumentType
		from criscostack.core.doctype.user_select_document_type.user_select_document_type import (
			UserSelectDocumentType,
		)
		from criscostack.core.doctype.user_type_module.user_type_module import UserTypeModule
	...
	def clear_cache(self):
		...
		if not self.is_standard:
			criscostack.cache.delete_value("non_standard_user_types")
	...
	def on_trash(self):
		if self.is_standard:
			criscostack.throw(_("Standard user type {0} can not be deleted.").format(criscostack.bold(self.name)))

	def set_modules(self):
		if not self.user_doctypes:
			return

		modules = criscostack.get_all(
			"DocType",
	...
	def validate_document_type_limit(self):
		limit = criscostack.conf.get("user_type_doctype_limit", {}).get(criscostack.scrub(self.name))

		if not limit and criscostack.session.user != "Administrator":
			criscostack.throw(
				_("User does not have permission to create the new {0}").format(criscostack.bold(_("User Type"))),
	...
	def validate_role(self):
		if not self.role:
			criscostack.throw(_("The field {0} is mandatory").format(criscostack.bold(_("Role"))))

		if not criscostack.db.get_value("Role", self.role, "is_custom"):
			criscostack.throw(
				_("The role {0} should be a custom role.").format(
					criscostack.bold(get_link_to_form("Role", self.role))
				)
			)

	def update_users(self):
		for row in criscostack.get_all("User", filters={"user_type": self.name}):
			user = criscostack.get_cached_doc("User", row.name)
	...
	def update_modules_in_user(self, user):
		block_modules = criscostack.get_all(
			"Module Def",
	...
	def add_role_permissions_for_user_doctypes(self):
		...
		for row in self.user_doctypes:
			...
			criscostack.db.set_value("Custom DocPerm", docperm, values)

	def add_select_perm_doctypes(self):
		if criscostack.flags.ignore_select_perm:
			return
		...
		for doctype in user_doctypes:
			doc = criscostack.get_meta(doctype)
	...
	def add_role_permissions_for_file(self):
		...
		criscostack.db.set_value("Custom DocPerm", docperm, {"read": 1, "create": 1, "write": 1})
	...
	def remove_permission_for_deleted_doctypes(self):
		...
		for perm in criscostack.get_all(
			...
			criscostack.delete_doc("Custom DocPerm", perm.name)


def add_role_permissions(doctype, role):
	name = criscostack.get_value("Custom DocPerm", dict(parent=doctype, role=role, permlevel=0))
...
def get_non_standard_user_types():
	user_types = criscostack.get_all(
		"User Type",
...
def get_user_linked_doctypes(doctype, txt, searchfield, start, page_len, filters):
	modules = [d.get("module_name") for d in get_modules_from_app("criscostack")]
	...
	custom_doctypes = criscostack.get_all(
...
def user_linked_with_permission_on_doctype(doc, user):
	...
	if not doc.user_id_field:
		criscostack.throw(_("User Id Field is mandatory in the user type {0}").format(criscostack.bold(doc.name)))

	if criscostack.db.get_value(doc.apply_user_permission_on, {doc.user_id_field: user}, "name"):
		return True
	else:
		label = criscostack.get_meta(doc.apply_user_permission_on).get_field(doc.user_id_field).label

		criscostack.msgprint(
			_(
				"To set the role {0} in the user {1}, kindly set the {2} field as {3} in one of the {4} record."
			).format(
				criscostack.bold(doc.role),
				criscostack.bold(user),
				criscostack.bold(label),
				criscostack.bold(user),
				criscostack.bold(doc.apply_user_permission_on),
			)
		)
...
def apply_permissions_for_non_standard_user_type(doc, method=None):
	...
	if not criscostack.db.table_exists("User Type") or criscostack.flags.in_migrate:
		return

	user_types = criscostack.cache.get_value(
	...
	for user_type, data in user_types.items():
		...
		if doc.get(data[1]) and (
			...
			if not perm_data:
				...
				criscostack.db.set_value("User Permission", perm_data[0], "user", doc.get(data[1]))
