...
import criscostack
from criscostack import _, conf, safe_decode
from criscostack.build import html_to_js_template
from criscostack.core.doctype.custom_role.custom_role import get_custom_allowed_roles
from criscostack.desk.form.meta import get_code_files_via_hooks, get_js
...
class Page(Document):
	...
	def autoname(self):
		...
		if (self.name and self.name.startswith("New Page")) or not self.name:
			...
			if criscostack.db.exists("Page", self.name):
				cnt = criscostack.db.sql(
	...
	def validate(self):
		...
		if self.is_new() and not getattr(conf, "developer_mode", 0):
			criscostack.throw(_("Not in Developer Mode"))

		# setting ignore_permissions via update_setup_wizard_access (setup_wizard.py)
		if criscostack.session.user != "Administrator" and not self.flags.ignore_permissions:
			criscostack.throw(_("Only Administrator can edit"))
	...
	def on_update(self):
		...
		from criscostack.core.doctype.doctype.doctype import make_module_and_roles
		...
		if path:
			...
			if not os.path.exists(path + ".js"):
				with open(path + ".js", "w") as f:
					f.write(
						"""criscostack.pages['%s'].on_page_load = function(wrapper) {
	var page = criscostack.ui.make_app_page({
		parent: wrapper,
		title: '%s',
		single_column: true
	});
			...
			if fname.endswith(".html"):
				with open(os.path.join(path, fname)) as f:
					...
					if "<!-- jinja -->" in template:
						...
						try:
							out = criscostack.get_attr(
								"{app}.{module}.page.{page}.{page}.get_context".format(
									app=criscostack.local.module_app[scrub(self.module)], module=scrub(self.module), page=page_name
								)
							)(context)
						...
						template = criscostack.render_template(template, context)
					self.script = html_to_js_template(fname, template) + self.script

					# flag for not caching this page
					self._dynamic_page = True

		if criscostack.lang != "en":
			from criscostack.translate import get_lang_js
...
def delete_custom_role(field, docname):
	name = criscostack.db.get_value("Custom Role", {field: docname}, "name")
	if name:
		criscostack.delete_doc("Custom Role", name)
