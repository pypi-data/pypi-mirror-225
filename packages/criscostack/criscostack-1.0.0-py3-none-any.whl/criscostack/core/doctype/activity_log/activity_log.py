...
class ActivityLog(Document):
	...
	def set_ip_address(self):
		if self.operation in ("Login", "Logout"):
			self.ip_address = getattr(criscostack.local, "request_ip")
	...
	def clear_old_logs(days=None):
		...
		criscostack.db.delete(doctype, filters=(doctype.modified < (Now() - Interval(days=days))))


def on_doctype_update():
	"""Add indexes in `tabActivity Log`"""
	criscostack.db.add_index("Activity Log", ["reference_doctype", "reference_name"])
	criscostack.db.add_index("Activity Log", ["timeline_doctype", "timeline_name"])
