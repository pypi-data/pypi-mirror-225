...
from criscostack.core.doctype.data_import.exporter import Exporter
from criscostack.core.doctype.data_import.importer import Importer
...
from criscostack.utils.csvutils import validate_google_sheets_url
...
class DataImport(Document):
	...
	def validate_doctype(self):
		if self.reference_doctype in BLOCKED_DOCTYPES:
			criscostack.throw(_("Importing {0} is not allowed.").format(self.reference_doctype))
	...
	def start_import(self):
		...
		if is_scheduler_inactive() and not criscostack.flags.in_test:
			criscostack.throw(_("Scheduler is inactive. Cannot import data."), title=_("Scheduler Inactive"))
		...
		if not is_job_enqueued(job_id):
			enqueue(
				...
				now=criscostack.conf.developer_mode or criscostack.flags.in_test,
			)
			return True
...
def get_preview_from_template(data_import, import_file=None, google_sheets_url=None):
	return criscostack.get_doc("Data Import", data_import).get_preview_from_template(
...
def form_start_import(data_import):
	return criscostack.get_doc("Data Import", data_import).start_import()


def start_import(data_import):
	"""This method runs in background job"""
	data_import = criscostack.get_doc("Data Import", data_import)
	try:
	...
	criscostack.publish_realtime("data_import_refresh", {"data_import": data_import.name})
...
def download_template(
	...
	export_fields = criscostack.parse_json(export_fields)
	export_filters = criscostack.parse_json(export_filters)
...
def download_errored_template(data_import_name):
	data_import = criscostack.get_doc("Data Import", data_import_name)
...
def get_import_status(data_import_name):
	...
	total_payload_count = criscostack.db.get_value("Data Import", data_import_name, "payload_count")
...
def import_file(doctype, file_path, import_type, submit_after_import=False, console=False):
	...
	data_import = criscostack.new_doc("Data Import")
...
def import_doc(path, pre_process=None):
	...
	for f in files:
		if f.endswith(".json"):
			criscostack.flags.mute_emails = True
			import_file_by_path(
...
def export_json(doctype, path, filters=None, or_filters=None, name=None, order_by="creation asc"):
	...
	out = []
	if name:
		out.append(criscostack.get_doc(doctype, name).as_dict())
	elif criscostack.db.get_value("DocType", doctype, "issingle"):
		out.append(criscostack.get_doc(doctype).as_dict())
	else:
		for doc in criscostack.get_all(
			doctype,
			fields=["name"],
			filters=filters,
			...
			out.append(criscostack.get_doc(doctype, doc.name).as_dict())
	post_process(out)
	...
	with open(path, "w") as outfile:
		outfile.write(criscostack.as_json(out, ensure_ascii=False))


def export_csv(doctype, path):
	from criscostack.core.doctype.data_export.exporter import export_data

	with open(path, "wb") as csvfile:
		export_data(doctype=doctype, all_doctypes=True, template=True, with_data=True)
		csvfile.write(criscostack.response.result.encode("utf-8"))
