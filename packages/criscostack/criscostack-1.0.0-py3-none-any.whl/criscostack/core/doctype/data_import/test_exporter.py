...
from criscostack.core.doctype.data_import.test_importer import create_doctype_if_not_exists
from criscostack.tests.utils import CriscoTestCase

doctype_name = "DocType for Export"


class TestExporter(CriscoTestCase):
	def setUp(self):
		create_doctype_if_not_exists(doctype_name)

	def test_exports_specified_fields(self):
		if not criscostack.db.exists(doctype_name, "Test"):
			...
			doc = criscostack.get_doc(doctype_name, "Test")
	...
	def test_export_csv_response(self):
		...
		self.assertEqual(criscostack.response["doctype"], doctype_name)
