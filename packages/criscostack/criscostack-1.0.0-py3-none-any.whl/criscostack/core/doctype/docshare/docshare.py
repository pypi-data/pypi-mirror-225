...
from criscostack.utils import cint, get_fullname
...
class DocShare(Document):
	...
	def get_doc(self):
		if not getattr(self, "_doc", None):
			self._doc = criscostack.get_doc(self.share_doctype, self.share_name)
		return self._doc
	...
	def validate_user(self):
		if self.everyone:
			...
			criscostack.throw(_("User is mandatory for Share"), criscostack.MandatoryError)

	def check_share_permission(self):
		if not self.flags.ignore_share_permission and not criscostack.has_permission(
			self.share_doctype, "share", self.get_doc()
		):

			criscostack.throw(_('You need to have "Share" permission'), criscostack.PermissionError)

	def check_is_submittable(self):
		if self.submit and not cint(
			criscostack.db.get_value("DocType", self.share_doctype, "is_submittable")
		):
			criscostack.throw(
				_("Cannot share {0} with submit permission as the doctype {1} is not submittable").format(
					criscostack.bold(self.share_name), criscostack.bold(self.share_doctype)
				)
			)
...
def on_doctype_update():
	...
	criscostack.db.add_index("DocShare", ["user", "share_doctype"])
	criscostack.db.add_index("DocShare", ["share_doctype", "share_name"])
