"""
CriscoClient is a library that helps you connect with other criscostack systems
"""
import base64
import json
...
class CriscoException(Exception):
	pass
...
class CriscoClient:
	def __init__(
		...
		api_key=None,
		api_secret=None,
		criscostack_authorization_source=None,
	):
		import requests
		...
		self.criscostack_authorization_source = criscostack_authorization_source
	...
	def setup_key_authentication_headers(self):
		if self.api_key and self.api_secret:
			...
			if self.criscostack_authorization_source:
				auth_source = {"Crisco-Authorization-Source": self.criscostack_authorization_source}
	...
	def insert(self, doc):
		...
		res = self.session.post(
			...
			data={"data": criscostack.as_json(doc)},
			verify=self.verify,
			headers=self.headers,
		)
		return criscostack._dict(self.post_process(res))
	...
	def insert_many(self, docs):
		...
		return self.post_request({"cmd": "criscostack.client.insert_many", "docs": criscostack.as_json(docs)})
	...
	def update(self, doc):
		...
		res = self.session.put(
			url, data={"data": criscostack.as_json(doc)}, verify=self.verify, headers=self.headers
		)
		return criscostack._dict(self.post_process(res))
	...
	def bulk_update(self, docs):
		...
		return self.post_request({"cmd": "criscostack.client.bulk_update", "docs": criscostack.as_json(docs)})
	...
	def delete(self, doctype, name):
		...
		return self.post_request({"cmd": "criscostack.client.delete", "doctype": doctype, "name": name})

	def submit(self, doc):
		"""Submit remote document

		:param doc: dict or Document object to be submitted remotely"""
		return self.post_request({"cmd": "criscostack.client.submit", "doc": criscostack.as_json(doc)})
	...
	def get_value(self, doctype, fieldname=None, filters=None):
		...
		return self.get_request(
			{
				"cmd": "criscostack.client.get_value",
				"doctype": doctype,
				"fieldname": fieldname or "name",
				"filters": criscostack.as_json(filters),
			}
		)
	...
	def set_value(self, doctype, docname, fieldname, value):
		...
		return self.post_request(
			{
				"cmd": "criscostack.client.set_value",
				"doctype": doctype,
				"name": docname,
				"fieldname": fieldname,
				"value": value,
	...
	def cancel(self, doctype, name):
		...
		return self.post_request({"cmd": "criscostack.client.cancel", "doctype": doctype, "name": name})
	...
	def rename_doc(self, doctype, old_name, new_name):
		...
		params = {
			"cmd": "criscostack.client.rename_doc",
			"doctype": doctype,
			"old_name": old_name,
			"new_name": new_name,
		}
	...
	def migrate_doctype(
		...
		for doc in docs:
			...
			if doctype != "User" and not criscostack.db.exists("User", doc.get("owner")):
				criscostack.get_doc(
					{
						"doctype": "User",
						"email": doc.get("owner"),
			...
			doc["doctype"] = doctype
			new_doc = criscostack.get_doc(doc)
			new_doc.insert()
	...
	def migrate_single(self, doctype):
		...
		doc.modified = criscostack.db.get_single_value(doctype, "modified")
	...
	def post_process(self, response):
		...
		if rjson and ("exc" in rjson) and rjson["exc"]:
			try:
				...
				exc = "CriscoClient Request Failed\n\n" + exc
			except Exception:
				exc = rjson["exc"]

			raise CriscoException(exc)
		if "message" in rjson:
...
class CriscoOAuth2Client(CriscoClient):
