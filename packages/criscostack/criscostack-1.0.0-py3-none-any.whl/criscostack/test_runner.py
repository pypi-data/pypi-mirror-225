...
from criscostack.modules import get_module_name, load_doctype_module
...
def main(
	...
	if doctype_list_path:
		...
		with open(criscostack.get_app_path(app, doctype_list_path)) as f:
	...
	try:
		criscostack.flags.print_messages = verbose
		...
		scheduler_disabled_by_user = criscostack.utils.scheduler.is_scheduler_disabled()
		if not scheduler_disabled_by_user:
			criscostack.utils.scheduler.disable_scheduler()

		set_test_email_config()

		if not criscostack.flags.skip_before_tests:
			if verbose:
				print('Running "before_tests" hooks')
			for fn in criscostack.get_hooks("before_tests", app_name=app):
		...
		if doctype:
			...
			doctypes = criscostack.db.get_list(
...
def run_all_tests(app=None, verbose=False, profile=False, failfast=False, junit_xml_output=False):
	...
	for app in apps:
		for path, folders, files in os.walk(criscostack.get_pymodule_path(app)):
...
def run_tests_for_doctype(
	...
	for doctype in doctypes:
		module = criscostack.db.get_value("DocType", doctype, "module")
		if not module:
		...
		if force:
			for name in criscostack.db.sql_list("select name from `tab%s`" % doctype):
				criscostack.delete_doc(doctype, name, force=True)
...
def _run_unittest(
	...
	criscostack.flags.tests_verbose = verbose
...
def _add_test(app, path, filename, verbose, test_suite=None):
	...
	app_path = criscostack.get_pymodule_path(app)
...
def make_test_records(doctype, verbose=0, force=False, commit=False):
	...
	if criscostack.flags.skip_test_records:
		return
	...
	for options in get_dependencies(doctype):
		...
		if options not in criscostack.local.test_objects:
			criscostack.local.test_objects[options] = []
...
def get_dependencies(doctype):
	...
	for df in meta.get_table_fields():
		link_fields.extend(criscostack.get_meta(df.options).get_link_fields())
...
def make_test_records_for_doctype(doctype, verbose=0, force=False, commit=False):
	...
	if hasattr(test_module, "_make_test_records"):
		criscostack.local.test_objects[doctype] += test_module._make_test_records(verbose)

	elif hasattr(test_module, "test_records"):
		if doctype in criscostack.local.test_objects:
			criscostack.local.test_objects[doctype] += make_test_objects(
		...
		test_records = criscostack.get_test_records(doctype)
		if test_records:
			criscostack.local.test_objects[doctype] += make_test_objects(
...
def make_test_objects(doctype, test_records=None, verbose=None, reset=False, commit=False):
	...
	for doc in test_records:
		...
		if criscostack.db.exists(d.doctype, d.name) and not reset:
...
def add_to_test_record_log(doctype):
	...
	if doctype not in test_record_log:
		criscostack.flags.test_record_log.append(doctype)
		with open(criscostack.get_site_path(".test_log"), "w") as f:
			f.write("\n".join(filter(None, criscostack.flags.test_record_log)))


def get_test_record_log():
	"""Return the list of doctypes for which test records have been created"""
	if "test_record_log" not in criscostack.flags:
		if os.path.exists(criscostack.get_site_path(".test_log")):
			with open(criscostack.get_site_path(".test_log")) as f:
				criscostack.flags.test_record_log = f.read().splitlines()
		else:
			criscostack.flags.test_record_log = []

	return criscostack.flags.test_record_log
