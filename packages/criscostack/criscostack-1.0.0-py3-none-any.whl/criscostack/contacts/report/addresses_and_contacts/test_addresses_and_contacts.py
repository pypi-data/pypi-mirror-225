...
from criscostack.contacts.report.addresses_and_contacts.addresses_and_contacts import get_data
from criscostack.tests.utils import CriscoTestCase


def get_custom_linked_doctype():
	if bool(criscostack.get_all("DocType", filters={"name": "Test Custom Doctype"})):
		return
...
def get_custom_doc_for_address_and_contacts():
	...
	linked_doc = criscostack.get_doc(
		{
...
def create_linked_address(link_list):
	if criscostack.flags.test_address_created:
		return

	address = criscostack.get_doc(
		{
			"doctype": "Address",
	...
	address.insert()
	criscostack.flags.test_address_created = True

	return address.name


def create_linked_contact(link_list, address):
	if criscostack.flags.test_contact_created:
		return
	...
	criscostack.flags.test_contact_created = True


class TestAddressesAndContacts(CriscoTestCase):
