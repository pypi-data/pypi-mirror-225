...
from criscostack.utils import cint, get_datetime, get_url, time_diff_in_seconds
from criscostack.utils.background_jobs import enqueue
from criscostack.utils.password import decrypt, encrypt

PARENT_FOR_DEFAULTS = "__2fa"


def get_default(key):
	return criscostack.db.get_default(key, parent=PARENT_FOR_DEFAULTS)


def set_default(key, value):
	criscostack.db.set_default(key, value, parent=PARENT_FOR_DEFAULTS)


def clear_default(key):
	criscostack.defaults.clear_default(key, parent=PARENT_FOR_DEFAULTS)
...
def toggle_two_factor_auth(state, roles=None):
	...
	for role in roles or []:
		role = criscostack.get_doc("Role", {"role_name": role})
...
def two_factor_is_enabled(user=None):
	...
	enabled = int(criscostack.db.get_single_value("System Settings", "enable_two_factor_auth") or 0)
	if enabled:
		bypass_two_factor_auth = int(
			criscostack.db.get_single_value("System Settings", "bypass_2fa_for_retricted_ip_users") or 0
		)
		if bypass_two_factor_auth and user:
			...
			if restrict_ip_list and criscostack.local.request_ip:
				for ip in restrict_ip_list:
					if criscostack.local.request_ip.startswith(ip):
						enabled = False
						break
...
def get_cached_user_pass():
	...
	user = pwd = None
	tmp_id = criscostack.form_dict.get("tmp_id")
	if tmp_id:
		user = criscostack.safe_decode(criscostack.cache.get(tmp_id + "_usr"))
		pwd = criscostack.safe_decode(criscostack.cache.get(tmp_id + "_pwd"))
	return (user, pwd)


def authenticate_for_2factor(user):
	"""Authenticate two factor for enabled user before login."""
	if criscostack.form_dict.get("otp"):
		return
	otp_secret = get_otpsecret_for_(user)
	token = int(pyotp.TOTP(otp_secret).now())
	tmp_id = criscostack.generate_hash(length=8)
	cache_2fa_data(user, token, otp_secret, tmp_id)
	verification_obj = get_verification_obj(user, token, otp_secret)
	# Save data in local
	criscostack.local.response["verification"] = verification_obj
	criscostack.local.response["tmp_id"] = tmp_id


def cache_2fa_data(user, token, otp_secret, tmp_id):
	"""Cache and set expiry for data."""
	pwd = criscostack.form_dict.get("pwd")
	...
	if verification_method in ["SMS", "Email"]:
		expiry_time = criscostack.flags.token_expiry or 300
		criscostack.cache.set(tmp_id + "_token", token)
		criscostack.cache.expire(tmp_id + "_token", expiry_time)
	else:
		expiry_time = criscostack.flags.otp_expiry or 180
	for k, v in {"_usr": user, "_pwd": pwd, "_otp_secret": otp_secret}.items():
		criscostack.cache.set(f"{tmp_id}{k}", v)
		criscostack.cache.expire(f"{tmp_id}{k}", expiry_time)
...
def two_factor_is_enabled_for_(user):
	...
	role_doctype = criscostack.qb.DocType("Role")
	no_of_users = criscostack.db.count(
		role_doctype,
...
def get_verification_method():
	return criscostack.db.get_single_value("System Settings", "two_factor_method")


def confirm_otp_token(login_manager, otp=None, tmp_id=None):
	"""Confirm otp matches."""
	from criscostack.auth import get_login_attempt_tracker

	if not otp:
		otp = criscostack.form_dict.get("otp")
	if not otp:
	...
	if not tmp_id:
		tmp_id = criscostack.form_dict.get("tmp_id")
	hotp_token = criscostack.cache.get(tmp_id + "_token")
	otp_secret = criscostack.cache.get(tmp_id + "_otp_secret")
	if not otp_secret:
	...
	if hotp_token:
		if hotp.verify(otp, int(hotp_token)):
			criscostack.cache.delete(tmp_id + "_token")
...
def get_verification_obj(user, token, otp_secret):
	otp_issuer = criscostack.db.get_single_value("System Settings", "otp_issuer_name")
	verification_method = get_verification_method()
...
def process_2fa_for_sms(user, token, otp_secret):
	...
	phone = criscostack.db.get_value("User", user, ["phone", "mobile_no"], as_dict=1)
...
def get_email_subject_for_2fa(kwargs_dict):
	...
	subject_template = _("Login Verification Code from {}").format(
		criscostack.db.get_single_value("System Settings", "otp_issuer_name")
	)
	subject = criscostack.render_template(subject_template, kwargs_dict)
	return subject
...
def get_email_body_for_2fa(kwargs_dict):
	...
	"""
	body = criscostack.render_template(body_template, kwargs_dict)
	return body
...
def get_link_for_qrcode(user, totp_uri):
	...
	key = criscostack.generate_hash(length=20)
	key_user = f"{key}_user"
	key_uri = f"{key}_uri"
	lifespan = int(criscostack.db.get_single_value("System Settings", "lifespan_qrcode_image")) or 240
	criscostack.cache.set_value(key_uri, totp_uri, expires_in_sec=lifespan)
	criscostack.cache.set_value(key_user, user, expires_in_sec=lifespan)
...
def send_token_via_sms(otpsecret, token=None, phone_no=None):
	...
	try:
		from criscostack.core.doctype.sms_settings.sms_settings import send_request
	except Exception:
		return False
	...
	ss = criscostack.get_doc("SMS Settings", "SMS Settings")
...
def send_token_via_email(user, token, otp_secret, otp_issuer, subject=None, message=None):
	...
	user_email = criscostack.db.get_value("User", user, "email")
	if not user_email:
		return False
	hotp = pyotp.HOTP(otp_secret)
...
def create_barcode_folder():
	...
	folder = criscostack.db.exists("File", {"file_name": folder_name})
	if folder:
		return folder
	folder = criscostack.get_doc(
...
def delete_qrimage(user, check_expiry=False):
	...
	user_barcodes = criscostack.get_all(
	...
	for barcode in user_barcodes:
		if check_expiry and not should_remove_barcode_image(barcode):
			continue
		barcode = criscostack.get_doc("File", barcode.name)
		criscostack.delete_doc("File", barcode.name, ignore_permissions=True)


def delete_all_barcodes_for_users():
	"""Task to delete all barcodes for user."""

	users = criscostack.get_all("User", {"enabled": 1})
	for user in users:
...
def should_remove_barcode_image(barcode):
	...
	if isinstance(barcode, str):
		barcode = criscostack.get_doc("File", barcode)
	lifespan = criscostack.db.get_single_value("System Settings", "lifespan_qrcode_image") or 240
...
def disable():
	criscostack.db.set_single_value("System Settings", "enable_two_factor_auth", 0)


@criscostack.whitelist()
def reset_otp_secret(user: str):
	if criscostack.session.user != user:
		criscostack.only_for("System Manager", message=True)

	settings = criscostack.get_cached_doc("System Settings")
	...
	otp_issuer = settings.otp_issuer_name or "Crisco Framework"
	user_email = criscostack.get_cached_value("User", user, "email")
	...
	criscostack.msgprint(_("OTP Secret has been reset. Re-registration will be required on next login."))
