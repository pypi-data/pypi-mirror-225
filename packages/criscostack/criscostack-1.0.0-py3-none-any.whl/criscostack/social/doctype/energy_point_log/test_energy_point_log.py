...
import criscostack
from criscostack.desk.form.assign_to import add as assign_to
from criscostack.desk.page.user_profile.user_profile import get_energy_points_heatmap_data
...
class TestEnergyPointLog(CriscoTestCase):
	@classmethod
	def setUpClass(cls):
		super().setUpClass()
		settings = criscostack.get_single("Energy Point Settings")
		settings.enabled = 1
		settings.save()
	...
	def setUp(self):
		criscostack.cache.delete_value("energy_point_rule_map")

	def tearDown(self):
		criscostack.set_user("Administrator")
		criscostack.db.delete("Energy Point Log")
		criscostack.db.delete("Energy Point Rule")
		criscostack.cache.delete_value("energy_point_rule_map")
	...
	def test_revert_points_on_cancelled_doc(self):
		...
		energy_point_logs = criscostack.get_all("Energy Point Log")
	...
	def test_allow_creation_of_new_log_if_the_previous_log_was_reverted(self):
		...
		log_name = criscostack.db.exists("Energy Point Log", {"reference_name": created_todo.name})
		criscostack.get_doc("Energy Point Log", log_name).revert("Just for test")
...
def create_energy_point_rule_for_todo(
	...
	name = "ToDo Closed"
	point_rule_exists = criscostack.db.exists("Energy Point Rule", name)

	if point_rule_exists:
		return criscostack.get_doc("Energy Point Rule", name)
