...
criscostack.query_reports["Website Analytics"] = {
	filters: [
		{
			fieldname: "from_date",
			label: __("From Date"),
			fieldtype: "Date",
			default: criscostack.datetime.add_days(criscostack.datetime.now_date(true), -100),
		},
		{
			fieldname: "to_date",
			label: __("To Date"),
			fieldtype: "Date",
			default: criscostack.datetime.now_date(true),
		},
		{
			fieldname: "range",
			label: __("Range"),
	...
	formatter: function (value, row, column, data, default_formatter) {
		if (
			criscostack.query_report.get_filter_value("group_by") === "source" &&
			column.id === "source"
		) {
			if (value) {
				try {
					...
					return criscostack.utils.get_form_link(doctype, name, true, value);
				} catch (e) {
