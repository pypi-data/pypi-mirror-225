import criscostack
from criscostack.model.document import get_controller
...
from criscostack.website.router import (
	get_doctypes_with_web_view,
	get_page_info_from_web_page_with_dynamic_routes,
)
from criscostack.website.utils import cache_html
...
class DocumentPage(BaseTemplatePage):
	...
	@cache_html
	def get_html(self):
		self.doc = criscostack.get_doc(self.doctype, self.docname)
		self.init_context()
		self.update_context()
		self.post_process_context()
		html = criscostack.get_template(self.template_path).render(self.context)
		return html
...
def _find_matching_document_webview(route: str) -> tuple[str, str] | None:
	for doctype in get_doctypes_with_web_view():
		...
		try:
			docname = criscostack.db.get_value(doctype, filters, "name")
			if docname:
