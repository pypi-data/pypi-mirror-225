...
from criscostack.modules.export_file import get_module_path, scrub_dt_dn, write_document_file
...
class WebTemplate(Document):
	...
	if TYPE_CHECKING:
		...
		from criscostack.website.doctype.web_template_field.web_template_field import WebTemplateField
	...
	def validate(self):
		if self.standard and not (criscostack.conf.developer_mode or criscostack.flags.in_patch):
			criscostack.throw(_("Enable developer mode to create a standard Web Template"))

		for field in self.fields:
			if not field.fieldname:
				field.fieldname = criscostack.scrub(field.label)
	...
	def on_trash(self):
		if criscostack.conf.developer_mode and self.standard:
	...
	def get_template_path(self):
		...
		file_name = criscostack.scrub(self.name) + ".html"
	...
	def render(self, values=None):
		if not values:
			values = {}
		values = criscostack.parse_json(values)
		values.update({"values": values})
		template = self.get_template(self.standard)

		return criscostack.render_template(template, values)
