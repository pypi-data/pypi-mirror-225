...
test_records = criscostack.get_test_records("Web Page")


class TestWebPage(CriscoTestCase):
	def setUp(self):
		criscostack.db.delete("Web Page")
		for t in test_records:
			criscostack.get_doc(t).insert()
	...
	def test_dynamic_route(self):
		web_page = criscostack.get_doc(
			dict(
				...
				content_type="HTML",
				dynamic_template=1,
				main_section_html="<div>{{ criscostack.form_dict.doctype }}</div>",
			)
		).insert()
		try:
