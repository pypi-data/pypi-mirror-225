...
from criscostack.modules import get_module_name
from criscostack.search.website_search import remove_document_from_index, update_index_for_path
from criscostack.website.utils import cleanup_page_name, clear_cache


class WebsiteGenerator(Document):
	website = criscostack._dict()
	...
	def send_indexing_request(self, operation_type="URL_UPDATED"):
		...
		if (
			criscostack.db.get_single_value("Website Settings", "enable_google_indexing")
			and self.is_website_published()
			and self.meta.allow_guest_to_view
		):

			url = criscostack.utils.get_url(self.route)
			criscostack.enqueue(
				"criscostack.website.doctype.website_settings.google_indexing.publish_site",
				url=url,
	...
	def remove_old_route_from_index(self):
		...
		if self.allow_website_search_indexing() or criscostack.flags.in_test:
			return
	...
	def update_website_search_index(self):
		...
		"""
		if not self.allow_website_search_indexing() or criscostack.flags.in_test:
			return

		if self.is_website_published():
			criscostack.enqueue(update_index_for_path, path=self.route)
		elif self.route:
