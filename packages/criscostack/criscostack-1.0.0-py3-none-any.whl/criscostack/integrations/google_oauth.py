...
import criscostack
from criscostack.utils import get_request_site_address

CALLBACK_METHOD = "/api/method/criscostack.integrations.google_oauth.callback"
_SCOPES = {
...
_DOMAIN_CALLBACK_METHODS = {
	"mail": "criscostack.email.oauth.authorize_google_access",
	"contacts": "criscostack.integrations.doctype.google_contacts.google_contacts.authorize_access",
	"drive": "criscostack.integrations.doctype.google_drive.google_drive.authorize_access",
	"indexing": "criscostack.website.doctype.website_settings.google_indexing.authorize_access",
}
...
class GoogleOAuth:
	...
	def __init__(self, domain: str, validate: bool = True):
		self.google_settings = criscostack.get_single("Google Settings")
	...
	def validate_google_settings(self):
		...
		if not self.google_settings.enable:
			criscostack.throw(criscostack._("Please enable {} before continuing.").format(google_settings))

		if not (self.google_settings.client_id and self.google_settings.client_secret):
			criscostack.throw(criscostack._("Please update {} before continuing.").format(google_settings))
...
def handle_response(
	...
	if "error" in response:
		criscostack.log_error(
			criscostack._(error_title), criscostack._(response.get("error_description", error_message))
		)

		if raise_err:
			criscostack.throw(criscostack._(error_title), GoogleAuthenticationError, criscostack._(error_message))
...
@criscostack.whitelist(methods=["GET"])
def callback(state: str, code: str = None, error: str = None) -> None:
	"""Common callback for google integrations.
	Invokes functions using `criscostack.get_attr` and also adds required (keyworded) arguments
	along with committing and redirecting us back to criscostack site."""
	...
	if not error:
		if (domain := state.pop("domain")) in _DOMAIN_CALLBACK_METHODS:
			...
			criscostack.get_attr(_DOMAIN_CALLBACK_METHODS[domain])(**state)
