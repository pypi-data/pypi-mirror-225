...
from criscostack.integrations.google_oauth import GoogleOAuth
...
from criscostack.utils.password import set_encrypted_password
...
class GoogleCalendar(Document):
	...
	def validate(self):
		google_settings = criscostack.get_single("Google Settings")
		if not google_settings.enable:
			criscostack.throw(_("Enable Google API in Google Settings."))

		if not google_settings.client_id or not google_settings.client_secret:
			criscostack.throw(_("Enter Client Id and Client Secret in Google Settings."))
	...
	def get_access_token(self):
		...
		if not self.refresh_token:
			button_label = criscostack.bold(_("Allow Google Calendar Access"))
			raise criscostack.ValidationError(_("Click on {0} to generate Refresh Token.").format(button_label))
...
def authorize_access(g_calendar, reauthorize=None):
	...
	"""
	google_settings = criscostack.get_doc("Google Settings")
	google_calendar = criscostack.get_doc("Google Calendar", g_calendar)
	google_calendar.check_permission("write")

	redirect_uri = (
		get_request_site_address(True)
		+ "?cmd=criscostack.integrations.doctype.google_calendar.google_calendar.google_callback"
	)

	if not google_calendar.authorization_code or reauthorize:
		criscostack.cache.hset("google_calendar", "google_calendar", google_calendar.name)
		...
		try:
			...
			criscostack.local.response["location"] = "/app/Form/{}/{}".format(
				quote("Google Calendar"), quote(google_calendar.name)
			)

			criscostack.msgprint(_("Google Calendar has been configured."))
...
def google_callback(code=None):
	...
	"""
	google_calendar = criscostack.cache.hget("google_calendar", "google_calendar")
	criscostack.db.set_value("Google Calendar", google_calendar, "authorization_code", code)
	criscostack.db.commit()
...
def sync(g_calendar=None):
	...
	google_calendars = criscostack.get_list("Google Calendar", filters=filters)
...
def get_google_calendar_object(g_calendar):
	...
	"""
	google_settings = criscostack.get_doc("Google Settings")
	account = criscostack.get_doc("Google Calendar", g_calendar)
	...
	try:
		if account.google_calendar_id:
			...
			calendar = {
				...
				"timeZone": criscostack.db.get_single_value("System Settings", "time_zone"),
			}
...
def sync_events_from_google_calendar(g_calendar, method=None):
	...
	while True:
		try:
			...
			if err.resp.status == 410:
				...
				criscostack.msgprint(msg, title="Invalid Sync Token", indicator="blue")
			else:
				criscostack.throw(msg)
	...
	for idx, event in enumerate(results):
		criscostack.publish_realtime(
			"import_google_calendar", dict(progress=idx + 1, total=len(results)), user=criscostack.session.user
		)
		...
		if event.get("status") == "confirmed":
			...
			if not criscostack.db.exists("Event", {"google_calendar_event_id": event.get("id")}):
			...
			criscostack.get_doc(
				{
					...
					"reference_name": criscostack.db.get_value(
						"Event",
						{
							"google_calendar_id": account.google_calendar_id,
							"google_calendar_event_id": event.get("id"),
...
def insert_event_to_calendar(account, event, recurrence=None):
	"""
	Inserts event in Crisco Calendar during Sync
	"""
	calendar_event = {
		"doctype": "Event",
	...
	criscostack.get_doc(calendar_event).insert(ignore_permissions=True)


def update_event_in_calendar(account, event, recurrence=None):
	"""
	Updates Event in Crisco Calendar if any existing Google Calendar Event is updated
	"""
	calendar_event = criscostack.get_doc("Event", {"google_calendar_event_id": event.get("id")})
...
def insert_event_in_google_calendar(doc, method=None):
	...
	"""
	if (
		not criscostack.db.exists("Google Calendar", {"name": doc.google_calendar})
	...
	try:
		...
		criscostack.msgprint(_("Event Synced with Google Calendar."))
	except HttpError as err:
		criscostack.throw(
...
def update_event_in_google_calendar(doc, method=None):
	"""
	Updates Events in Google Calendar if any existing event is modified in Crisco Calendar
	"""
...
def delete_event_from_google_calendar(doc, method=None):
	"""
	Delete Events from Google Calendar if Crisco Event is deleted.
	"""

	if not criscostack.db.exists("Google Calendar", {"name": doc.google_calendar}):
		return
