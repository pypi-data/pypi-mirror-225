...
from criscostack.integrations.doctype.social_login_key.social_login_key import BaseUrlNotSetError
from criscostack.tests.utils import CriscoTestCase
from criscostack.utils import set_request
from criscostack.utils.oauth import login_via_oauth2


class TestSocialLoginKey(CriscoTestCase):
	def test_adding_criscostack_social_login_provider(self):
	...
	def test_normal_signup_and_github_login(self):
		...
		if not criscostack.db.exists("User", "githublogin@example.com"):
...
def make_social_login_key(**kwargs):
	...
	doc = criscostack.get_doc(kwargs)
	return doc
...
def create_or_update_social_login_key():
	...
	try:
		social_login_key = criscostack.get_doc("Social Login Key", "criscostack")
	except criscostack.DoesNotExistError:
		social_login_key = criscostack.new_doc("Social Login Key")
	social_login_key.get_social_login_provider("Crisco", initialize=True)
	social_login_key.base_url = criscostack.utils.get_url()
...
def create_github_social_login_key():
	if criscostack.db.exists("Social Login Key", "github"):
		return criscostack.get_doc("Social Login Key", "github")
	else:
