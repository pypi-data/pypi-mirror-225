...
import criscostack
from criscostack import _, safe_encode
from criscostack.model.document import Document
from criscostack.twofactor import authenticate_for_2factor, confirm_otp_token, should_run_2fa
...
class LDAPSettings(Document):
	...
	if TYPE_CHECKING:
		from criscostack.integrations.doctype.ldap_group_mapping.ldap_group_mapping import LDAPGroupMapping
	...
	def connect_to_ldap(self, base_dn, password, read_only=True) -> ldap3.Connection:
		try:
			...
			criscostack.throw(msg, title=_("LDAP Not Installed"))
		except LDAPInvalidCredentialsResult:
			criscostack.throw(_("Invalid username or password"))
		except Exception as ex:
			criscostack.throw(_(str(ex)))

	@staticmethod
	def get_ldap_client_settings() -> dict:
		# return the settings to be used on the client side.
		result = {"enabled": False}
		ldap = criscostack.get_cached_doc("LDAP Settings")
		if ldap.enabled:
			result["enabled"] = True
			result["method"] = "criscostack.integrations.doctype.ldap_settings.ldap_settings.login"
		return result
	...
	def sync_roles(self, user: "User", additional_groups: list = None):
		...
		all_mapped_roles = {r.criscoerp_role for r in self.ldap_groups}
		matched_roles = {
			r.criscoerp_role for r in self.ldap_groups if r.ldap_group.lower() in lower_groups
		}
	...
	def create_or_update_user(self, user_data: dict, groups: list = None):
		...
		if criscostack.db.exists("User", user_data["email"]):
			user = criscostack.get_doc("User", user_data["email"])
		...
		if self.default_user_type == "System User":
			...
			role = criscostack.db.get_value("User Type", user.user_type, "role")
	...
	def authenticate(self, username: str, password: str):
		if not self.enabled:
			criscostack.throw(_("LDAP is not enabled."))
		...
		try:
			...
			criscostack.throw(_("Please use a valid LDAP search filter"), title=_("Misconfigured"))
		...
		if conn.search(
			...
			if conn.entries and conn.entries[0]:
				...
				if conn.modify(entry_dn, changes=changes):
					if logout_sessions:
						from criscostack.sessions import clear_sessions

						clear_sessions(user=user, force=True)
					criscostack.msgprint(_("Password changed successfully."))
				else:
					criscostack.throw(_("Failed to change password."))
			else:
				criscostack.throw(_("No Entry for the User {0} found within LDAP!").format(user))
		else:
			criscostack.throw(_("No LDAP User found for email: {0}").format(user))
	...
	def convert_ldap_entry_to_dict(self, user_entry: Entry):
		...
		if isinstance(email, list):
			...
			for e in email:
				if criscostack.db.exists("User", e):
					email = e
					break
			else:
...
def login():
	# LDAP LOGIN LOGIC
	args = criscostack.form_dict
	ldap: LDAPSettings = criscostack.get_doc("LDAP Settings")

	user = ldap.authenticate(criscostack.as_unicode(args.usr), criscostack.as_unicode(args.pwd))

	criscostack.local.login_manager.user = user.name
	if should_run_2fa(user.name):
		authenticate_for_2factor(user.name)
		if not confirm_otp_token(criscostack.local.login_manager):
			return False
