...
def new_site(
	...
	"Create a new site"
	from criscostack.installer import _new_site, extract_sql_from_archive

	criscostack.init(site=site, new_site=True)
...
def _restore(
	*,
	...
	from criscostack.utils.backups import Backup, get_or_generate_backup_encryption_key
	...
	try:
		...
		if is_partial(decompressed_file_name):
			click.secho(
				"Partial Backup file detected. You cannot use a partial file to restore a Crisco site.",
				fg="red",
			)
			click.secho(
	...
	# dont allow downgrading to older versions of criscostack without force
...
def partial_restore(context, sql_file_path, verbose, encryption_key=None):
	from criscostack.installer import extract_sql_from_archive, partial_restore
	from criscostack.utils.backups import Backup, get_or_generate_backup_encryption_key
	...
	try:
		...
		with open(decompressed_file_name) as f:
			...
			if "Partial Backup" not in header:
				click.secho(
					"Full backup file detected.Use `bench restore` to restore a Crisco Site.",
					fg="red",
				)
				_backup.decryption_rollback()
				sys.exit(1)
...
def _reinstall(
	...
	from criscostack.installer import _new_site
	from criscostack.utils.synchronization import filelock
	...
	try:
		...
		installed = criscostack.get_installed_apps()
		criscostack.clear_cache()
	except Exception:
		installed = []
	finally:
...
def install_app(context, apps, force=False):
	...
	from criscostack.installer import install_app as _install_app
	from criscostack.utils.synchronization import filelock
	...
	for site in context.sites:
		...
		with filelock("install_app", timeout=1):
			for app in apps:
				...
				except criscostack.IncompatibleApp as err:
					err_msg = f":\n{err}" if str(err) else ""
					print(f"App {app} is Incompatible with Site {site}{err_msg}")
					exit_code = 1
				except Exception as err:
					err_msg = f": {str(err)}\n{criscostack.get_traceback()}"
...
def list_apps(context, format):
	...
	for site in context.sites:
		...
		apps = criscostack.get_single("Installed Applications").installed_applications
		...
		if apps:
			...
			installed_applications = criscostack.get_installed_apps()
			applications_summary = "\n".join(installed_applications)
			summary = f"{site_title}\n{applications_summary}\n"
	...
	if format == "json":
		click.echo(criscostack.as_json(summary_dict))
...
def add_system_manager(context, email, first_name, last_name, send_welcome_email, password):
	...
	for site in context.sites:
		criscostack.connect(site=site)
		try:
			criscostack.utils.user.add_system_manager(email, first_name, last_name, send_welcome_email, password)
			criscostack.db.commit()
		finally:
			criscostack.destroy()
	if not context.sites:
...
def disable_user(context, email):
	...
	with criscostack.init_site(site):
		criscostack.connect()
		user = criscostack.get_doc("User", email)
		user.enabled = 0
...
def migrate(context, skip_failing=False, skip_search_index=False):
	...
	from criscostack.migrate import SiteMigration
...
@click.argument("criscostack_provider")
@pass_context
def migrate_to(context, criscostack_provider):
	"Migrates site to the specified provider"
	from criscostack.integrations.criscostack_providers import migrate_to

	for site in context.sites:
		criscostack.init(site=site)
		criscostack.connect()
		migrate_to(site, criscostack_provider)
		criscostack.destroy()
	if not context.sites:
		raise SiteNotSpecifiedError
...
def run_patch(context, module, force):
	...
	for site in context.sites:
		...
		try:
			criscostack.connect()
			criscostack.modules.patch_handler.run_single(module, force=force or context.force)
		finally:
			criscostack.destroy()
	if not context.sites:
		raise SiteNotSpecifiedError
...
def reload_doc(context, module, doctype, docname):
	...
	for site in context.sites:
		try:
			criscostack.init(site=site)
			criscostack.connect()
			criscostack.reload_doc(module, doctype, docname, force=context.force)
			criscostack.db.commit()
		finally:
			criscostack.destroy()
	if not context.sites:
...
def reload_doctype(context, doctype):
	...
	for site in context.sites:
		try:
			criscostack.init(site=site)
			criscostack.connect()
			criscostack.reload_doctype(doctype, force=context.force)
			criscostack.db.commit()
		finally:
			criscostack.destroy()
	if not context.sites:
...
def add_to_hosts(context):
	"Add site to hosts"
	for site in context.sites:
		criscostack.commands.popen(f"echo 127.0.0.1\t{site} | sudo tee -a /etc/hosts")
	if not context.sites:
		raise SiteNotSpecifiedError
...
def backup(
	...
	for site in context.sites:
		try:
			...
			exit_code = 1
			continue
		if criscostack.get_system_settings("encrypt_backup") and criscostack.get_site_config().encryption_key:
			click.secho(
...
def remove_from_installed_apps(context, app):
	...
	from criscostack.installer import remove_from_installed_apps
...
def uninstall(context, app, dry_run, yes, no_backup, force):
	...
	from criscostack.installer import remove_app
	from criscostack.utils.synchronization import filelock
...
def _drop_site(
	...
	from criscostack.database import drop_user_and_database
	from criscostack.utils.backups import scheduled_backup
	...
	drop_user_and_database(criscostack.conf.db_name, db_root_username, db_root_password)

	archived_sites_path = archived_sites_path or os.path.join(
		criscostack.get_app_path("criscostack"), "..", "..", "..", "archived", "sites"
	)
...
def move(dest_dir, site):
	...
	old_path = criscostack.utils.get_site_path()
...
def set_last_active_for_user(context, user=None):
	...
	from criscostack.core.doctype.user.user import get_system_users
	...
	with criscostack.init_site(site):
		...
		criscostack.db.set_value("User", user, "last_active", now_datetime())
		criscostack.db.commit()
...
def publish_realtime(context, event, message, room, user, doctype, docname, after_commit):
	...
	from criscostack import publish_realtime
...
def browse(context, site, user=None):
	...
	if site not in criscostack.utils.get_sites():
	...
	sid = ""
	if user:
		if criscostack.conf.developer_mode or user == "Administrator":
			criscostack.utils.set_request(path="/")
			...
			sid = f"/app?sid={criscostack.session.sid}"
		else:
			click.echo("Please enable developer mode to login as a user")

	url = f"{criscostack.utils.get_site_url(site)}{sid}"
...
def start_ngrok(context, bind_tls, use_default_authtoken):
	...
	ngrok_authtoken = criscostack.conf.ngrok_authtoken
...
def clear_log_table(context, doctype, days, no_backup):
	...
	"""
	from criscostack.core.doctype.log_settings.log_settings import LOG_DOCTYPES
	from criscostack.core.doctype.log_settings.log_settings import clear_log_table as clear_logs
	from criscostack.utils.backups import scheduled_backup
	...
	if doctype not in LOG_DOCTYPES:
		raise criscostack.ValidationError(f"Unsupported logging DocType: {doctype}")
...
def trim_database(context, dry_run, format, no_backup, yes=False):
	...
	for site in context.sites:
		...
		table_name = criscostack.qb.Field("table_name").as_("name")

		queried_result = (
			criscostack.qb.from_(information_schema.tables)
			.select(table_name)
			.where(information_schema.tables.table_schema == criscostack.conf.db_name)
			.where(information_schema.tables.table_type == "BASE TABLE")
			.run()
		)

		database_tables = [x[0] for x in queried_result]
		doctype_tables = criscostack.get_all("DocType", pluck="name")
		...
		if not TABLES_TO_DROP:
			if format == "text":
				click.secho(f"No ghost tables found in {criscostack.local.site}...Great!", fg="green")
		else:
			if not yes:
			...
			for table in TABLES_TO_DROP:
				...
				if not dry_run:
					criscostack.db.sql_ddl(f"drop table `{table}`")

			ALL_DATA[criscostack.local.site] = TABLES_TO_DROP
		criscostack.destroy()
...
def get_standard_tables():
	...
	sql_file = os.path.join(
		...
		"criscostack",
		"database",
		criscostack.conf.db_type,
		f"framework_{criscostack.conf.db_type}.sql",
	)
...
def trim_tables(context, dry_run, format, no_backup):
	...
	from criscostack.model.meta import trim_tables
	from criscostack.utils.backups import scheduled_backup
	...
	for site in context.sites:
		...
		if not (no_backup or dry_run):
			click.secho(f"Taking backup for {criscostack.local.site}", fg="green")
		...
		try:
			...
			if format == "table" and not dry_run:
				click.secho(f"The following data have been removed from {criscostack.local.site}", fg="green")
...
def handle_data(data: dict, format="json"):
	if format == "json":
		...
		print(json.dumps({criscostack.local.site: data}, indent=1, sort_keys=True))
	else:
