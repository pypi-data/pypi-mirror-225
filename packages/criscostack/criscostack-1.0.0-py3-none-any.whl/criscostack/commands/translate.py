...
def build_message_files(context):
	...
	for site in context.sites:
		try:
			...
			criscostack.connect()
			criscostack.translate.rebuild_all_translation_files()
		finally:
			criscostack.destroy()
...
@click.argument("app")  # , help="App name eg. criscostack")
def new_language(context, lang_code, app):
	...
	# init site
	criscostack.connect(site=context["sites"][0])
	criscostack.translate.write_translations_file(app, lang_code)
	...
	print(
		"You will need to add the language in criscostack/geo/languages.json, if you haven't done it already."
	)
...
def get_untranslated(context, lang, untranslated_file, app="_ALL_APPS", all=None):
	...
	try:
		criscostack.init(site=site)
		criscostack.connect()
		criscostack.translate.get_untranslated(lang, untranslated_file, get_all=all, app=app)
	finally:
		criscostack.destroy()
...
def update_translations(context, lang, untranslated_file, translated_file, app="_ALL_APPS"):
	...
	try:
		criscostack.init(site=site)
		criscostack.connect()
		criscostack.translate.update_translations(lang, untranslated_file, translated_file, app=app)
	finally:
		criscostack.destroy()
...
def import_translations(context, lang, path):
	...
	try:
		criscostack.init(site=site)
		criscostack.connect()
		criscostack.translate.import_translations(lang, path)
	finally:
		criscostack.destroy()
...
def migrate_translations(context, source_app, target_app):
	...
	try:
		criscostack.init(site=site)
		criscostack.connect()
		criscostack.translate.migrate_translations(source_app, target_app)
	finally:
		criscostack.destroy()
