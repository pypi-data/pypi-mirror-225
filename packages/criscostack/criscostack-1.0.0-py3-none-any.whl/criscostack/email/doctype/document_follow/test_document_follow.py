...
import criscostack
import criscostack.desk.form.document_follow as document_follow
from criscostack.desk.form.assign_to import add
from criscostack.desk.form.document_follow import get_document_followed_by_user
from criscostack.desk.form.utils import add_comment
from criscostack.desk.like import toggle_like
from criscostack.query_builder import DocType
from criscostack.query_builder.functions import Cast_
from criscostack.share import add as share
from criscostack.tests.utils import CriscoTestCase


class TestDocumentFollow(CriscoTestCase):
	...
	def tearDown(self):
		...
		criscostack.db.delete("Document Follow")
	...
	doc = criscostack.get_doc(
		{
			...
			"doc.starts_on": criscostack.utils.now(),
			"doc.ends_on": criscostack.utils.add_days(criscostack.utils.now(), 5),
...
def get_user(document_follow=None):
	...
	if criscostack.db.exists("User", "test@docsub.com"):
		doc = criscostack.delete_doc("User", "test@docsub.com")
	doc = criscostack.new_doc("User")
...
def get_emails(event_doc, search_string):
	...
	return (
		criscostack.qb.from_(EmailQueue)
