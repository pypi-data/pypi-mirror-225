...
criscostack.email_defaults_pop = {
	GMail: {
...
function oauth_access(frm) {
	criscostack.model.with_doc("Connected App", frm.doc.connected_app, () => {
		const connected_app = criscostack.get_doc("Connected App", frm.doc.connected_app);
...
function set_default_max_attachment_size(frm) {
	if (frm.doc.__islocal && !frm.doc["attachment_limit"]) {
		criscostack.call({
			method: "criscostack.core.api.file.get_max_file_size",
			callback: function (r) {
				if (!r.exc) {
...
criscostack.ui.form.on("Email Account", {
	service: function (frm) {
		$.each(criscostack.email_defaults[frm.doc.service], function (key, value) {
			frm.set_value(key, value);
		});
		if (!frm.doc.use_imap) {
			$.each(criscostack.email_defaults_pop[frm.doc.service], function (key, value) {
				frm.set_value(key, value);
			});
		}
	},
	...
	onload: function (frm) {
		...
		frm.set_query(
			"append_to",
			"criscostack.email.doctype.email_account.email_account.get_append_to"
		);
		frm.set_query("append_to", "imap_folder", function () {
			return {
				query: "criscostack.email.doctype.email_account.email_account.get_append_to",
			};
		});
	...
	refresh: function (frm) {
		...
		if (criscostack.route_flags.delete_user_from_locals && criscostack.route_flags.linked_user) {
			delete criscostack.route_flags.delete_user_from_locals;
			delete locals["User"][criscostack.route_flags.linked_user];
		}
	},
	...
	show_oauth_authorization_message(frm) {
		if (frm.doc.auth_method === "OAuth" && frm.doc.connected_app) {
			criscostack.call({
				method: "criscostack.integrations.doctype.connected_app.connected_app.has_token",
				args: {
	...
	domain: criscostack.utils.debounce((frm) => {
		if (frm.doc.domain) {
			criscostack.call({
	...
	email_sync_option: function (frm) {
		...
		if (frm.doc.email_sync_option == "ALL") {
			...
			criscostack.confirm(msg, null, function () {
	...
	warn_autoreply_on_incoming: function (frm) {
		if (frm.doc.enable_incoming && frm.doc.enable_auto_reply && frm.doc.__islocal) {
			...
			criscostack.confirm(msg, null, function () {
				...
				criscostack.show_alert({ message: __("Disabled Auto Reply"), indicator: "blue" });
			});
		}
	},
});
