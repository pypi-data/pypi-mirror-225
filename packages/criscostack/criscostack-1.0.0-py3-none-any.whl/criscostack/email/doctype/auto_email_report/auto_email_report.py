...
from criscostack.desk.query_report import build_xlsx_data
...
from criscostack.utils.csvutils import to_csv
...
class AutoEmailReport(Document):
	...
	def autoname(self):
		...
		if criscostack.db.exists("Auto Email Report", self.name):
	...
	def sender_email(self):
		return criscostack.db.get_value("Email Account", self.sender, "email_id")
	...
	def validate_report_count(self):
		count = criscostack.db.count("Auto Email Report", {"user": self.user, "enabled": 1})

		max_reports_per_user = (
			cint(criscostack.local.conf.max_reports_per_user)  # kept for backward compatibilty
			or cint(criscostack.db.get_single_value("System Settings", "max_auto_email_report_per_user"))
			or 20
		)
		...
		if count > max_reports_per_user + (-1 if self.flags.in_insert else 0):
			...
			criscostack.throw(msg, title=_("Report limit reached"))
		...
		if self.format not in valid_report_formats:
			criscostack.throw(
				_("{0} is not a valid report format. Report format should one of the following {1}").format(
					criscostack.bold(self.format), criscostack.bold(", ".join(valid_report_formats))
				)
			)

	def validate_mandatory_fields(self):
		# Check if all Mandatory Report Filters are filled by the User
		filters = criscostack.parse_json(self.filters) if self.filters else {}
		filter_meta = criscostack.parse_json(self.filter_meta) if self.filter_meta else {}
		throw_list = []
	...
	def get_report_content(self):
		...
		report = criscostack.get_doc("Report", self.report)

		self.filters = criscostack.parse_json(self.filters) if self.filters else {}
		...
		# add serial numbers
		columns.insert(0, criscostack._dict(fieldname="idx", label="", width="30px"))
		...
		if self.format == "HTML":
			...
			criscostack.throw(_("Invalid Output Format"))

	def get_html_table(self, columns=None, data=None):

		date_time = global_date_format(now()) + " " + format_time(now())
		report_doctype = criscostack.db.get_value("Report", self.report, "ref_doctype")

		return criscostack.render_template(
			"criscostack/templates/emails/auto_email_report.html",
			{
				"title": self.name,
	...
	def prepare_dynamic_filters(self):
		self.filters = criscostack.parse_json(self.filters)
	...
	def send(self):
		if self.filter_meta and not self.filters:
			criscostack.throw(_("Please set filters value in Report Filter table."))
...
def download(name):
	...
	auto_email_report = criscostack.get_doc("Auto Email Report", name)
	auto_email_report.check_permission()
	data = auto_email_report.get_report_content()

	if not data:
		criscostack.msgprint(_("No Data"))
		return

	criscostack.local.response.filecontent = data
	criscostack.local.response.type = "download"
	criscostack.local.response.filename = auto_email_report.get_file_name()
...
def send_daily():
	...
	enabled_reports = criscostack.get_all(
		"Auto Email Report", filters={"enabled": 1, "frequency": ("in", ("Daily", "Weekdays", "Weekly"))}
	)

	for report in enabled_reports:
		auto_email_report = criscostack.get_doc("Auto Email Report", report.name)
...
def send_monthly():
	...
	for report in criscostack.get_all("Auto Email Report", {"enabled": 1, "frequency": "Monthly"}):
		criscostack.get_doc("Auto Email Report", report.name).send()
	...
	for row in data:
		...
		for col in columns:
			...
			if col.fieldtype == "Link":
				...
				doc = criscostack.get_doc(col.parent, doc_name) if doc_name and col.get("parent") else None
				# Pass the Document to get the currency based on docfield option
				row[col.fieldname] = criscostack.format_value(row[col.fieldname], col, doc=doc)
	return columns, data
