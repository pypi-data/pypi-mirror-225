...
criscostack.ui.form.on("Auto Email Report", {
	refresh: function (frm) {
		...
		if (!frm.is_new()) {
			frm.add_custom_button(__("Download"), function () {
				var w = window.open(
					criscostack.urllib.get_full_url(
						"/api/method/criscostack.email.doctype.auto_email_report.auto_email_report.download?" +
							"name=" +
			...
			frm.add_custom_button(__("Send Now"), function () {
				criscostack.call({
					method: "criscostack.email.doctype.auto_email_report.auto_email_report.send_now",
					args: { name: frm.doc.name },
					callback: function () {
						criscostack.msgprint(__("Scheduled to send"));
					},
				});
			});
		} else {
			if (!frm.doc.user) {
				frm.set_value("user", criscostack.session.user);
			}
			if (!frm.doc.email_to) {
				frm.set_value("email_to", criscostack.session.user);
			}
		}
	...
	fetch_report_filters(frm) {
		if (
			...
			criscostack.call({
				method: "criscostack.desk.query_report.get_script",
				args: {
					report_name: frm.doc.report,
				},
				callback: function (r) {
					criscostack.dom.eval(r.message.script || "");
					frm.script_setup_for = frm.doc.report;
	...
	show_filters: function (frm) {
		...
		if (
			...
			(frm.doc.report_type !== "Report Builder" &&
				criscostack.query_reports[frm.doc.report] &&
				criscostack.query_reports[frm.doc.report].filters)
		) {
			...
			if (
				frm.doc.report_type === "Custom Report" &&
				criscostack.query_reports[frm.doc.reference_report] &&
				criscostack.query_reports[frm.doc.reference_report].filters
			) {
				if (frm.doc.filters) {
					filters = JSON.parse(frm.doc.filters);
				} else {
					criscostack.db.get_value("Report", frm.doc.report, "json", (r) => {
						if (r && r.json) {
				...
				report_filters = criscostack.query_reports[frm.doc.reference_report].filters;
			} else {
				filters = JSON.parse(frm.doc.filters || "{}");
				report_filters = criscostack.query_reports[frm.doc.report].filters;
			}
			...
			report_filters.forEach((f) => {
				...
				$("<td>" + criscostack.format(filters[f.fieldname], f) + "</td>")
					.css(css)
					.appendTo(row);
			});

			table.on("click", function () {
				var dialog = new criscostack.ui.Dialog({
