...
import criscostack
from criscostack import _, safe_decode, safe_encode
from criscostack.core.doctype.file import MaxFileSizeReachedError, get_random_filename
from criscostack.email.oauth import Oauth
...
from criscostack.utils.user import is_system_user
...
class EmailSizeExceededError(criscostack.ValidationError):
	pass


class EmailTimeoutError(criscostack.ValidationError):
	pass


class LoginLimitExceeded(criscostack.ValidationError):
	pass
...
class EmailServer:
	...
	def __init__(self, args=None):
		self.settings = args or criscostack._dict()
		...
		try:
			if cint(self.settings.use_ssl):
				self.imap = Timed_IMAP4_SSL(
					self.settings.host, self.settings.incoming_port, timeout=criscostack.conf.get("pop_timeout")
				)
			else:
			...
			criscostack.msgprint(_("Invalid Mail Server. Please rectify and try again."))
			raise
	...
	def connect_pop(self):
		...
		try:
			...
			criscostack.log_error("POP: Unable to connect")
			...
			if self.is_temporary_system_problem(e):
				...
				criscostack.msgprint(_("Invalid User Name or Support Password. Please rectify and try again."))
				raise
	...
	def check_imap_uidvalidity(self, folder):
		...
		criscostack.db.set_value("Email Account", self.settings.email_account, "uidnext", uidnext)
		...
		if not uid_validity or uid_validity != current_uid_validity:
			...
			criscostack.qb.update(Communication).set(Communication.uid, -1).where(
			...
			if self.settings.use_imap:
				...
				IMAPFolder = criscostack.qb.DocType("IMAP Folder")
				criscostack.qb.update(IMAPFolder).set(IMAPFolder.uidvalidity, current_uid_validity).set(
					IMAPFolder.uidnext, uidnext
				).where(IMAPFolder.parent == self.settings.email_account_name).where(
					IMAPFolder.folder_name == folder
				).run()
			else:
				EmailAccount = criscostack.qb.DocType("Email Account")
				criscostack.qb.update(EmailAccount).set(EmailAccount.uidvalidity, current_uid_validity).set(
	...
	def retrieve_message(self, uid, msg_num):
		try:
			...
			criscostack.log_error("Unable to fetch email", self.make_error_msg(uid, msg_num))
	...
	def get_email_seen_status(self, uid, flag_string):
		...
		for flag in imaplib.ParseFlags(flag_string) or []:
			match = WORDS_PATTERN.search(criscostack.as_unicode(flag))
...
class Email:
	...
	def set_from(self):
		...
		if _reply_to and not criscostack.db.get_value("Email Account", {"email_id": _reply_to}, "email_id"):
	...
	def decode_email(self, email):
		...
		for part, encoding in decode_header(
			criscostack.as_unicode(email).replace('"', " ").replace("'", " ")
		):
			if encoding:
	...
	def save_attachments_in_doc(self, doc):
		...
		for attachment in self.attachments:
			try:
				...
				pass
			except criscostack.FileAlreadyAttachedException:
				pass
...
class InboundMail(Email):
	...
	def _build_communication_doc(self):
		...
		communication = criscostack.get_doc(data)
	...
	def is_exist_in_system(self):
		...
		from criscostack.core.doctype.communication.communication import Communication
	...
	def is_reply_to_system_sent_mail(self):
		...
		return self.is_reply() and criscostack.local.site in self.in_reply_to
	...
	def parent_email_queue(self):
		...
		"""
		from criscostack.email.doctype.email_queue.email_queue import EmailQueue
	...
	def match_record_by_subject_and_sender(self, doctype):
		...
		3. Find by matching subject alone (Special case)
		        ...
		        the system user is replying to via the common email account in Crisco. This fix bypasses
		...
		if not record:
			...
			name = criscostack.db.get_value(self.email_account.append_to, filters=filters)
			record = self.get_doc(doctype, name, ignore_error=True) if name else None
		return record

	def _create_reference_document(self, doctype):
		"""Create reference document if it does not exist in the system."""
		parent = criscostack.new_doc(doctype)
		email_fileds = self.get_email_fields(doctype)

		if email_fileds.subject_field:
			parent.set(email_fileds.subject_field, criscostack.as_unicode(self.subject)[:140])

		if email_fileds.sender_field:
			parent.set(email_fileds.sender_field, criscostack.as_unicode(self.from_email))
		...
		try:
			...
			parent_name = criscostack.db.get_value(
	...
	def get_doc(doctype, docname, ignore_error=False):
		try:
			return criscostack.get_doc(doctype, docname)
	...
	def clean_subject(subject):
		...
		return criscostack.as_unicode(strip(re.sub(regex, "", subject, count=0, flags=re.IGNORECASE)))

	@staticmethod
	def get_email_fields(doctype):
		"""Returns Email related fields of a doctype."""
		fields = criscostack._dict()
	...
	def get_document(self, doctype, name):
		"""Is same as criscostack.get_doc but suppresses the DoesNotExist error."""
		try:
			return criscostack.get_doc(doctype, name)
		except criscostack.DoesNotExistError:
			return None
