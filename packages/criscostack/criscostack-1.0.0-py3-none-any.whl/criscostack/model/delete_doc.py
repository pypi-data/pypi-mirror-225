...
from criscostack.desk.doctype.tag.tag import delete_tags_for_document
from criscostack.model.docstatus import DocStatus
from criscostack.model.dynamic_links import get_dynamic_link_map
from criscostack.model.naming import revert_series_if_last
from criscostack.model.utils import is_virtual_doctype
from criscostack.utils.file_manager import remove_all
from criscostack.utils.global_search import delete_for_document
from criscostack.utils.password import delete_all_passwords_for
...
def delete_doc(
	...
	# get from form
	if not doctype:
		doctype = criscostack.form_dict.get("dt")
		name = criscostack.form_dict.get("dn")
	...
	for name in names or []:
		if is_virtual:
			criscostack.get_doc(doctype, name).delete()
			continue
		...
		if doctype == "DocType":
			if for_reload:
				...
				doc = criscostack.get_doc(doctype, name)
				if not (doc.custom or criscostack.conf.developer_mode or criscostack.flags.in_patch or force):
					criscostack.throw(_("Standard DocType can not be deleted."))

				update_flags(doc, flags, ignore_permissions)
				check_permission_and_not_submitted(doc)
				# delete custom table fields using this doctype.
				criscostack.db.delete(
					"Custom Field", {"options": name, "fieldtype": ("in", criscostack.model.table_fields)}
				)
				criscostack.db.delete("__global_search", {"doctype": name})

			delete_from_table(doctype, name, ignore_doctypes, None)

			if (
				criscostack.conf.developer_mode
				and not doc.custom
				and not (
					for_reload or criscostack.flags.in_migrate or criscostack.flags.in_install or criscostack.flags.in_uninstall
				)
			):
				try:
			...
			if not for_reload:
				...
				criscostack.enqueue(
					"criscostack.model.delete_doc.delete_dynamic_links",
					doctype=doc.doctype,
					name=doc.name,
...
def add_to_deleted_document(doc):
	...
	if doc.doctype != "Deleted Document" and criscostack.flags.in_install != "criscostack":
		criscostack.get_doc(
			dict(
...
def delete_from_table(doctype: str, name: str, ignore_doctypes: list[str], doc):
	if doctype != "DocType" and doctype == name:
		criscostack.db.delete("Singles", {"doctype": name})
	else:
		criscostack.db.delete(doctype, {"name": name})
	if doc:
		child_doctypes = [
			d.options for d in doc.meta.get_table_fields() if criscostack.get_meta(d.options).is_virtual == 0
		]
		...
		child_doctypes = criscostack.get_all(
			"DocField",
			fields="options",
			filters={"fieldtype": ["in", criscostack.model.table_fields], "parent": doctype},
			pluck="options",
		)

	child_doctypes_to_delete = set(child_doctypes) - set(ignore_doctypes)
	for child_doctype in child_doctypes_to_delete:
		criscostack.db.delete(child_doctype, {"parenttype": doctype, "parent": name})
...
def check_permission_and_not_submitted(doc):
	...
	if (
		...
		and criscostack.session.user != "Administrator"
		...
		criscostack.msgprint(
			...
			raise_exception=criscostack.PermissionError,
		)
	...
	if doc.docstatus.is_submitted():
		criscostack.msgprint(
			_("{0} {1}: Submitted Record cannot be deleted. You must {2} Cancel {3} it first.").format(
				_(doc.doctype),
				doc.name,
				"<a href='https://docs.criscoerp.com//docs/user/manual/en/setting-up/articles/delete-submitted-document' target='_blank'>",
				"</a>",
			),
...
def check_if_doc_is_linked(doc, method="Delete"):
	...
	"""
	from criscostack.model.rename_doc import get_link_fields
	...
	for lf in link_fields:
		...
		if not issingle:
			...
			if criscostack.get_meta(link_dt).istable:
				fields.extend(["parent", "parenttype"])

			for item in criscostack.db.get_values(link_dt, {link_field: doc.name}, fields, as_dict=True):
				# available only in child table cases
				item_parent = getattr(item, "parent", None)
				linked_doctype = item.parenttype if item_parent else link_dt

				if linked_doctype in criscostack.get_hooks("ignore_links_on_delete") or (
			...
			if criscostack.db.get_value(link_dt, None, link_field) == doc.name:
				raise_link_exists_exception(doc, link_dt, link_dt)


def check_if_doc_is_dynamically_linked(doc, method="Delete"):
	"""Raise `criscostack.LinkExistsError` if the document is dynamically linked"""
	for df in get_dynamic_link_map().get(doc.doctype, []):

		ignore_linked_doctypes = doc.get("ignore_linked_doctypes") or []

		if df.parent in criscostack.get_hooks("ignore_links_on_delete") or (
			df.parent in ignore_linked_doctypes and method == "Cancel"
		):
			# don't check for communication and todo!
			continue

		meta = criscostack.get_meta(df.parent)
		if meta.issingle:
			...
			for refdoc in criscostack.db.sql(
				...
				if (method == "Delete" and not DocStatus(refdoc.docstatus).is_cancelled()) or (
					...
					if reference_doctype in criscostack.get_hooks("ignore_links_on_delete") or (
...
def clear_timeline_references(link_doctype, link_name):
	criscostack.db.delete("Communication Link", {"link_doctype": link_doctype, "link_name": link_name})
...
def insert_feed(doc):
	...
	from criscostack.utils import get_fullname
...
def delete_controllers(doctype, module):
	...
	dir_path = os.path.join(module_path, "doctype", criscostack.scrub(doctype))
