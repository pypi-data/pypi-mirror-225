...
import criscostack
from criscostack import _, bold
from criscostack.model.document import Document
from criscostack.model.dynamic_links import get_dynamic_link_map
from criscostack.model.naming import validate_name
from criscostack.model.utils.user_settings import sync_user_settings, update_user_settings_data
from criscostack.query_builder import Field
from criscostack.utils.data import sbool
from criscostack.utils.password import rename_password
...
def update_document_title(
	...
	**kwargs,
	...
	# TODO: omit this after runtime type checking (ref: https://github.com/criscostack/criscostack/pull/14927)
	for obj in [docname, updated_title, updated_name]:
		if not isinstance(obj, (str, NoneType)):
			criscostack.throw(f"{obj=} must be of type str or None")
	...
	if title_updated:
		if action_enqueued and name_updated:
			...
			try:
				...
				doc.save()
				criscostack.msgprint(_("Saved"), alert=True, indicator="green")
			except Exception as e:
				if criscostack.db.is_duplicate_entry(e):
					criscostack.throw(
						_("{0} {1} already exists").format(doctype, criscostack.bold(docname)),
						title=_("Duplicate Name"),
						exc=criscostack.DuplicateEntryError,
					)
				raise
...
def rename_doc(
	...
	if not (new_usage_style or old_usage_style):
		raise TypeError(
			"{doctype, old, new} or {doc, new} are required arguments for criscostack.model.rename_doc"
		)
	...
	if validate:
		old_doc = doc or criscostack.get_doc(doctype, old)
	...
	# call after_rename
	new_doc = criscostack.get_doc(doctype, new)
	...
	if merge:
		new_doc.add_comment("Edit", _("merged {0} into {1}").format(criscostack.bold(old), criscostack.bold(new)))
	else:
		new_doc.add_comment(
			"Edit", _("renamed from {0} to {1}").format(criscostack.bold(old), criscostack.bold(new))
		)

	if merge:
		criscostack.delete_doc(doctype, old)

	new_doc.clear_cache()
	criscostack.clear_cache()
	if rebuild_search:
		criscostack.enqueue("criscostack.utils.global_search.rebuild_for_doctype", doctype=doctype)
...
def update_assignments(old: str, new: str, doctype: str) -> None:
	old_assignments = criscostack.parse_json(criscostack.db.get_value(doctype, old, "_assign")) or []
	new_assignments = criscostack.parse_json(criscostack.db.get_value(doctype, new, "_assign")) or []
	...
	for user in common_assignments:
		...
		for todo in todos:
			criscostack.delete_doc("ToDo", todo.name)

	unique_assignments = list(set(old_assignments + new_assignments))
	criscostack.db.set_value(doctype, new, "_assign", criscostack.as_json(unique_assignments, indent=0))
...
def update_user_settings(old: str, new: str, link_fields: list[dict]) -> None:
	...
	UserSettings = criscostack.qb.Table("__UserSettings")

	user_settings_details = (
		criscostack.qb.from_(UserSettings)
...
def update_customizations(old: str, new: str) -> None:
	criscostack.db.set_value("Custom DocPerm", {"parent": old}, "parent", new, update_modified=False)


def update_attachments(doctype: str, old: str, new: str) -> None:
	if doctype != "DocType":
		File = criscostack.qb.DocType("File")

		criscostack.qb.update(File).set(File.attached_to_name, new).where(
			(File.attached_to_name == old) & (File.attached_to_doctype == doctype)
		).run()


def rename_versions(doctype: str, old: str, new: str) -> None:
	Version = criscostack.qb.DocType("Version")

	criscostack.qb.update(Version).set(Version.docname, new).where(
		(Version.docname == old) & (Version.ref_doctype == doctype)
	).run()


def rename_eps_records(doctype: str, old: str, new: str) -> None:
	EPL = criscostack.qb.DocType("Energy Point Log")

	criscostack.qb.update(EPL).set(EPL.reference_name, new).where(
...
def rename_parent_and_child(doctype: str, old: str, new: str, meta: "Meta") -> None:
	criscostack.qb.update(doctype).set("name", new).where(Field("name") == old).run()
...
def update_autoname_field(doctype: str, new: str, meta: "Meta") -> None:
	...
	if meta.get("autoname"):
		...
		if field and field[0] == "field":
			criscostack.qb.update(doctype).set(field[1], new).where(Field("name") == new).run()
...
def validate_rename(
	...
	if save_point:
		_SAVE_POINT = f"validate_rename_{criscostack.generate_hash(length=8)}"
		criscostack.db.savepoint(_SAVE_POINT)

	exists = (
		criscostack.qb.from_(doctype).where(Field("name") == new).for_update().select("name").run(pluck=True)
	)
	exists = exists[0] if exists else None

	if not criscostack.db.exists(doctype, old):
		criscostack.throw(_("Can't rename {0} to {1} because {0} doesn't exist.").format(old, new))

	if old == new:
		criscostack.throw(_("No changes made because old and new name are the same.").format(old, new))

	if merge and not exists:
		criscostack.throw(_("{0} {1} does not exist, select a new target to merge").format(doctype, new))
	...
	if not merge and exists and not ignore_if_exists:
		criscostack.throw(_("Another {0} with name {1} exists, select another name").format(doctype, new))

	if not (
		ignore_permissions or criscostack.permissions.has_permission(doctype, "write", raise_exception=False)
	):
		criscostack.throw(_("You need write permission to rename"))

	if not (force or ignore_permissions) and not meta.allow_rename:
		criscostack.throw(_("{0} not allowed to be renamed").format(_(doctype)))

	# validate naming like it's done in doc.py
	new = validate_name(doctype, new)

	if save_point:
		criscostack.db.rollback(save_point=_SAVE_POINT)
...
def rename_doctype(doctype: str, old: str, new: str) -> None:
	...
	fields_with_options = ("Link",) + criscostack.model.table_fields
...
def update_child_docs(old: str, new: str, meta: "Meta") -> None:
	...
	for df in meta.get_table_fields():
		(
			criscostack.qb.update(df.options)
			.set("parent", new)
...
def update_link_field_values(link_fields: list[dict], old: str, new: str, doctype: str) -> None:
	for field in link_fields:
		if field["issingle"]:
			try:
				single_doc = criscostack.get_doc(field["parent"])
			...
			criscostack.db.set_value(parent, {docfield: old}, docfield, new, update_modified=False)
...
def get_link_fields(doctype: str) -> list[dict]:
	...
	if not criscostack.flags.link_fields:
		criscostack.flags.link_fields = {}

	if doctype not in criscostack.flags.link_fields:
		dt = criscostack.qb.DocType("DocType")
		df = criscostack.qb.DocType("DocField")
		cf = criscostack.qb.DocType("Custom Field")
		ps = criscostack.qb.DocType("Property Setter")

		st_issingle = criscostack.qb.from_(dt).select(dt.issingle).where(dt.name == df.parent).as_("issingle")
		standard_fields = (
			criscostack.qb.from_(df)
			.select(df.parent, df.fieldname, st_issingle)
			.where((df.options == doctype) & (df.fieldtype == "Link"))
			.run(as_dict=True)
		)

		cf_issingle = criscostack.qb.from_(dt).select(dt.issingle).where(dt.name == cf.dt).as_("issingle")
		custom_fields = (
			criscostack.qb.from_(cf)
		...
		ps_issingle = (
			criscostack.qb.from_(dt).select(dt.issingle).where(dt.name == ps.doc_type).as_("issingle")
		)
		property_setter_fields = (
			criscostack.qb.from_(ps)
		...
		criscostack.flags.link_fields[doctype] = standard_fields + custom_fields + property_setter_fields

	return criscostack.flags.link_fields[doctype]


def update_options_for_fieldtype(fieldtype: str, old: str, new: str) -> None:
	CustomField = criscostack.qb.DocType("Custom Field")
	PropertySetter = criscostack.qb.DocType("Property Setter")

	if criscostack.conf.developer_mode:
		for name in criscostack.get_all("DocField", filters={"options": old}, pluck="parent"):
			if name in (old, new):
				continue

			doctype = criscostack.get_doc("DocType", name)
			save = False
	...
	DocField = criscostack.qb.DocType("DocField")
	criscostack.qb.update(DocField).set(DocField.options, new).where(
		(DocField.fieldtype == fieldtype) & (DocField.options == old)
	).run()

	criscostack.qb.update(CustomField).set(CustomField.options, new).where(
		(CustomField.fieldtype == fieldtype) & (CustomField.options == old)
	).run()

	criscostack.qb.update(PropertySetter).set(PropertySetter.value, new).where(
...
def update_select_field_values(old: str, new: str):
	from criscostack.query_builder.functions import Replace

	DocField = criscostack.qb.DocType("DocField")
	CustomField = criscostack.qb.DocType("Custom Field")
	PropertySetter = criscostack.qb.DocType("Property Setter")

	criscostack.qb.update(DocField).set(DocField.options, Replace(DocField.options, old, new)).where(
		(DocField.fieldtype == "Select")
		& (DocField.parent != new)
		& (DocField.options.like(f"%\n{old}%") | DocField.options.like(f"%{old}\n%"))
	).run()

	criscostack.qb.update(CustomField).set(
	...
	criscostack.qb.update(PropertySetter).set(
...
def update_parenttype_values(old: str, new: str):
	child_doctypes = criscostack.get_all(
		"DocField",
		fields=["options", "fieldname"],
		filters={"parent": new, "fieldtype": ["in", criscostack.model.table_fields]},
	)

	custom_child_doctypes = criscostack.get_all(
		"Custom Field",
		fields=["options", "fieldname"],
		filters={"dt": new, "fieldtype": ["in", criscostack.model.table_fields]},
	)

	child_doctypes += custom_child_doctypes
	fields = [d["fieldname"] for d in child_doctypes]

	property_setter_child_doctypes = criscostack.get_all(
		"Property Setter",
	...
	for doctype in child_doctypes:
		...
		criscostack.qb.update(table).set(table.parenttype, new).where(table.parenttype == old).run()


def rename_dynamic_links(doctype: str, old: str, new: str):
	Singles = criscostack.qb.DocType("Singles")
	for df in get_dynamic_link_map().get(doctype, []):
		# dynamic link in single, just one value to check
		if criscostack.get_meta(df.parent).issingle:
			refdoc = criscostack.db.get_singles_dict(df.parent)
			if refdoc.get(df.options) == doctype and refdoc.get(df.fieldname) == old:
				criscostack.qb.update(Singles).set(Singles.value, new).where(
			...
			criscostack.qb.update(parent).set(df.fieldname, new).where(
...
def bulk_rename(
	...
	if not rows:
		criscostack.throw(_("Please select a valid csv file with data"))

	if not via_console:
		max_rows = 500
		if len(rows) > max_rows:
			criscostack.throw(_("Maximum {0} rows allowed").format(max_rows))
