...
function clone_field(df) {
	...
	if (cloned.custom) {
		...
		cloned.fieldname += "_" + criscostack.utils.get_random(8);
	}
	return cloned;
}
...
let print_templates = computed(() => {
	...
	for (let template of templates) {
		...
		if (template.field) {
			...
			df = {
				label: template.name,
				fieldname: criscostack.scrub(template.name)
			};
		}
		out.push({
...
onMounted(() => {
	let method =
		"criscostack.printing.page.print_format_builder_beta.print_format_builder_beta.get_google_fonts";
...
.control-font :deep(.criscostack-control[data-fieldname="font"] label) {
	display: none;
}
</style>
