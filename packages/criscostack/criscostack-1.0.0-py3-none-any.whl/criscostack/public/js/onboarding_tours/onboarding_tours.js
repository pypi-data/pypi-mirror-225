criscostack.ui.OnboardingTour = class OnboardingTour {
	constructor() {
		this.driver_steps = [];
	...
	init_driver() {
		this.driver = new criscostack.Driver({
			className: "criscostack-driver",
			allowClose: false,
			padding: 10,
			...
			opacity: 0.5,
			onHighlighted: (step) => {
				criscostack.ui.next_form_tour = step.options.step_info?.next_form_tour;
				const wait_for_node = setInterval(() => {
					...
					if (step.popover.closeBtnNode) {
						step.popover.closeBtnNode.onclick = () => {
							...
							!criscostack.boot.user.onboarding_status[this.tour.name] &&
								(criscostack.boot.user.onboarding_status[this.tour.name] = {});
							criscostack.boot.user.onboarding_status[this.tour.name].is_complete = true;
							if (!this.driver.hasNextStep()) {
								criscostack.boot.user.onboarding_status[
									this.tour.name
								].all_steps_completed = true;
							}

							criscostack.call({
								method: "criscostack.desk.doctype.form_tour.form_tour.update_user_status",
								args: {
									value: JSON.stringify(criscostack.boot.user.onboarding_status),
				...
				if ($input) criscostack.utils.sleep(200).then(() => $input.focus());
			},
		});
	}

	async init({ tour_name, start_step }) {
		this.tour = await criscostack.db.get_doc("Form Tour", tour_name);
		this.init_driver();
		this.build_steps();
	...
	get_step(step_info, on_next) {
		...
		return {
			...
			popover: {
				title,
				description,
				position: criscostack.router.slug(position || "Bottom"),
			},
			onNext: on_next,
			step_info: step_info,
			showButtons,
...
criscostack.ui.init_onboarding_tour = () => {
	...
	typeof criscostack.boot.onboarding_tours == "undefined" && criscostack.boot.onboarding_tours == [];
	typeof criscostack.boot.user.onboarding_status == "undefined" &&
		criscostack.boot.user.onboarding_status == {};
	let route = criscostack.router.current_route;
	if (route[0] === "") return;
	...
	if (route[0] != "dashboard-view") {
		criscostack.boot.onboarding_tours &&
			criscostack.boot.onboarding_tours.forEach((tour) => {
	...
	matching_tours = matching_tours.filter((tour) => {
		if (criscostack.boot.user.onboarding_status[tour[0]]?.is_complete == true) return false;
		return true;
	});
	matching_tours = matching_tours.map((tour) => {
		if (criscostack.boot.user.onboarding_status[tour[0]]?.steps_complete != undefined) {
			tour.push(criscostack.boot.user.onboarding_status[tour[0]].steps_complete);
		}
		return tour;
	});
	if (matching_tours.length == 0) return;
	let current_tour = matching_tours.find(
		(tour) => tour[0] == criscostack.ui.currentTourInstance?.tour?.name
	);
	let next_tour = matching_tours.find((tour) => tour[0] == criscostack.ui.next_form_tour);
	if (current_tour) {
		...
		criscostack.ui.next_form_tour = undefined;
	} else {
	...
	if (!tour_name) return;
	if (criscostack.ui.currentTourInstance?.driver) {
		criscostack.ui.currentTourInstance.driver_steps = [];
		criscostack.ui.currentTourInstance.driver.reset(true);
		criscostack.ui.currentTourInstance.update_driver_steps();
	}
	const tour = (criscostack.ui.currentTourInstance = new criscostack.ui.OnboardingTour());
	...
	const wait_for_data = setInterval(() => {
		...
		if (document.body.getAttribute("data-ajax-state") === "complete") {
			criscostack.utils.sleep(500).then(() => {
				tour.init({
					tour_name,
					start_step,
				});
