...
export default class GoogleDrivePicker {
	...
	loadGapi() {
		...
		if (!criscostack.boot.user.google_drive_token) {
	...
	handleAuthResult(authResult) {
		...
		if (authResult && !authResult.error) {
			criscostack.boot.user.google_drive_token = authResult.access_token;
			this.createPicker();
		} else {
			let error = error_map[authResult.error] || __("Google Authentication Error");
			criscostack.throw(error);
		}
	}
	...
	createPicker() {
		...
		if (this.pickerApiLoaded && criscostack.boot.user.google_drive_token) {
			...
			this.picker = new google.picker.PickerBuilder()
				...
				.setOAuthToken(criscostack.boot.user.google_drive_token)
				.addView(this.view)
				.setLocale(criscostack.boot.lang)
