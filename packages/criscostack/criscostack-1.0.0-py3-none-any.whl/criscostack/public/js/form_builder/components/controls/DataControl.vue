...
if (props.df.fieldtype === "Datetime") {
	let time_zone_text = criscostack.boot.time_zone
		? criscostack.boot.time_zone.user
		: criscostack.sys_defaults.time_zone;
...
<template>
	<div
		class="control criscostack-control"
	...
	>
		...
		<div
			...
			v-html="criscostack.utils.icon('folder-normal', 'md')"
		/>
		<!-- phone selector icon -->
		<div
			v-if="df.fieldtype == 'Phone'"
			class="selected-phone no-value"
			v-html="criscostack.utils.icon('down', 'sm')"
		/>
	</div>
</template>
