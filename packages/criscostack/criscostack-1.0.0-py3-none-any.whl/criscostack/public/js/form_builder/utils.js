...
export async function get_table_columns(df, child_doctype) {
	...
	if (!criscostack.get_meta(df.options)) {
		await criscostack.model.with_doctype(df.options);
	}
	if (!child_doctype) {
		child_doctype = criscostack.get_meta(df.options);
	}
	...
	for (let tf of table_fields) {
		if (!in_list(criscostack.model.layout_fields, tf.fieldtype) && tf.in_list_view && tf.label) {
			let colsize;
	...
	function adjust_column_size(total_colsize, increase) {
		...
		while (start_condition && passes < 11) {
			for (var i in table_columns) {
				...
				if (colsize > 1 && colsize < 11 && criscostack.model.is_non_std_field(_df.fieldname)) {
					if (
						passes < 3 &&
		...
		if (d.fieldtype) {
			if (!d.fieldname) {
				if (d.label) {
					...
					if (in_list(criscostack.model.restricted_fields, d.fieldname)) {
					...
					d.fieldname =
						...
						"_" +
						criscostack.utils.get_random(4);
				}
			} else {
				if (in_list(criscostack.model.restricted_fields, d.fieldname)) {
					criscostack.throw(__("Fieldname {0} is restricted", [d.fieldname]));
				}
			}
...
export function clone_field(field) {
	...
	cloned_field.df.name = criscostack.utils.get_random(8);
	return cloned_field;
}
