...
export const useStore = defineStore("form-builder-store", () => {
	...
	async function fetch() {
		...
		if (!get_docfields.value.length) {
			let docfield = is_customize_form.value ? "Customize Form Field" : "DocField";
			await criscostack.model.with_doctype(docfield);
			let df = criscostack.get_meta(docfield).fields;
		...
		nextTick(() => {
			...
			read_only.value =
				!is_customize_form.value && !criscostack.boot.developer_mode && !doc.value.custom;
			preview.value = false;
		});
	...
	function validate_fields(fields, is_table) {
		...
		let not_allowed_in_list_view = ["Attach Image", ...criscostack.model.no_value_type];
		if (is_table) {
		...
		fields.forEach((df) => {
			...
			if (in_list(["Link", ...criscostack.model.table_fields], df.fieldtype) && !df.options) {
				error_message = __(
			...
			if (df.in_global_search && in_list(criscostack.model.no_value_type, df.fieldtype)) {
				error_message = __(
