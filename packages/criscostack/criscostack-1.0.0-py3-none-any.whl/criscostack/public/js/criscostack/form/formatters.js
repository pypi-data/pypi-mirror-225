...
criscostack.provide("criscostack.form.formatters");

criscostack.form.link_formatters = {};

criscostack.form.formatters = {
	...
	_apply_custom_formatter: function (value, df) {
		...
		example:
			criscostack.meta.docfield_map[df.parent][df.fieldname].formatter = (value) => {
		...
		*/
		...
		if (df) {
			const std_df =
				criscostack.meta.docfield_map[df.parent] &&
				criscostack.meta.docfield_map[df.parent][df.fieldname];
	...
	Data: function (value, df) {
		...
		return criscostack.form.formatters._apply_custom_formatter(value, df);
	},
	Autocomplete: function (value, df) {
		return __(criscostack.form.formatters["Data"](value, df));
	},
	...
	Float: function (value, docfield, options, doc) {
		...
		var precision =
			docfield.precision ||
			cint(criscostack.boot.sysdefaults && criscostack.boot.sysdefaults.float_precision) ||
			null;
		if (docfield.options && docfield.options.trim()) {
			...
			return criscostack.form.formatters.Currency(value, docfield, options, doc);
		} else {
			// show 1.000000 as 1
			...
			return criscostack.form.formatters._right(format_number(value, null, precision), options);
		}
	},
	Int: function (value, docfield, options) {
		if (cstr(docfield.options).trim() === "File Size") {
			return criscostack.form.formatters.FileSize(value);
		}
		return criscostack.form.formatters._right(value == null ? "" : cint(value), options);
	},
	Percent: function (value, docfield, options) {
		const precision =
			...
			cint(criscostack.boot.sysdefaults && criscostack.boot.sysdefaults.float_precision) ||
			2;
		return criscostack.form.formatters._right(flt(value, precision) + "%", options);
	},
	...
	Currency: function (value, docfield, options, doc) {
		var currency = criscostack.meta.get_field_currency(docfield, doc);
		...
		if (typeof docfield.precision == "number") {
			...
			precision = cint(
				docfield.precision || criscostack.boot.sysdefaults.currency_precision || 2
			);
		}
		...
		if (precision > 2) {
			...
			if (decimals.length < 3 || decimals.length < precision) {
				const fraction =
					criscostack.model.get_value(":Currency", currency, "fraction_units") || 100; // if not set, minimum 2.
		...
		if (options && options.only_value) {
			...
			return criscostack.form.formatters._right(value, options);
		}
	},
	...
	Link: function (value, docfield, options, doc) {
		...
		let link_title = criscostack.utils.get_link_title(doctype, value);
		...
		if (criscostack.form.link_formatters[doctype]) {
			// don't apply formatters in case of composite (parent field of same type)
			if (doc && doctype !== doc.doctype) {
				value = criscostack.form.link_formatters[doctype](value, doc, docfield);
			}
		}
		...
		if (docfield && docfield.link_onclick) {
			...
			if (criscostack.model.can_read(doctype)) {
				return `<a
					href="/app/${encodeURIComponent(criscostack.router.slug(doctype))}/${encodeURIComponent(
					original_value
				)}"
	...
	Date: function (value) {
		if (!criscostack.datetime.str_to_user) {
			return value;
		}
		if (value) {
			value = criscostack.datetime.str_to_user(value, false, true);
			// handle invalid date
	...
	DateRange: function (value) {
		if (Array.isArray(value)) {
			return __("{0} to {1}", [
				criscostack.datetime.str_to_user(value[0]),
				criscostack.datetime.str_to_user(value[1]),
			]);
		} else {
			return value || "";
		}
	...
	Datetime: function (value) {
		if (value) {
			return moment(criscostack.datetime.convert_to_user_tz(value)).format(
				criscostack.boot.sysdefaults.date_format.toUpperCase() +
					" " +
					(criscostack.boot.sysdefaults.time_format || "HH:mm:ss")
			);
		} else {
			return "";
		}
	...
	Text: function (value, df) {
		if (value) {
			...
			if (!match) {
				value = criscostack.utils.replace_newlines(value);
			}
		}

		return criscostack.form.formatters.Data(value, df);
	},
	Time: function (value) {
		if (value) {
			value = criscostack.datetime.str_to_user(value, true);
		}
	...
	Duration: function (value, docfield) {
		if (value) {
			let duration_options = criscostack.utils.get_duration_options(docfield);
			value = criscostack.utils.get_formatted_duration(value, duration_options);
		}
	...
	LikedBy: function (value) {
		...
		$.each(JSON.parse(value || "[]"), function (i, v) {
			if (v) html += criscostack.avatar(v);
		});
		return html;
	},
	Tag: function (value) {
	...
	SmallText: function (value) {
		return criscostack.form.formatters.Text(value);
	},
	TextEditor: function (value) {
		let formatted_value = criscostack.form.formatters.Text(value);
	...
	WorkflowState: function (value) {
		var workflow_state = criscostack.get_doc("Workflow State", value);
		if (workflow_state) {
			return repl(
	...
	TableMultiSelect: function (rows, df, options) {
		...
		const formatted_values = rows.map((row) => {
			...
			return `<span class="text-nowrap">
				${criscostack.format(value, link_field, options, row)}
			</span>`;
		});
	...
	Icon: (value) => {
		return value
			? `<div>
			<div class="selected-icon">${criscostack.utils.icon(value, "md")}</div>
...
criscostack.form.get_formatter = function (fieldtype) {
	if (!fieldtype) fieldtype = "Data";
	return criscostack.form.formatters[fieldtype.replace(/ /g, "")] || criscostack.form.formatters.Data;
};

criscostack.format = function (value, df, options, doc) {
	...
	var formatter = df.formatter || criscostack.form.get_formatter(fieldtype);

	var formatted = formatter(value, df, options, doc);

	if (typeof formatted == "string") formatted = criscostack.dom.remove_script_and_style(formatted);

	return formatted;
};

criscostack.get_format_helper = function (doc) {
	var helper = {
		get_formatted: function (fieldname) {
			var df = criscostack.meta.get_docfield(doc.doctype, fieldname);
			if (!df) {
				console.log("fieldname not found: " + fieldname);
			}
			return criscostack.format(doc[fieldname], df, { inline: 1 }, doc);
		},
	};
	$.extend(helper, doc);
	return helper;
...
criscostack.form.link_formatters["User"] = function (value, doc, docfield) {
