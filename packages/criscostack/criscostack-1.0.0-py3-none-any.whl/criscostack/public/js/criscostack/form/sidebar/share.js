...
criscostack.ui.form.Share = class Share {
	constructor(opts) {
		$.extend(this, opts);
	...
	render_sidebar() {
		...
		this.shares.append(criscostack.avatar_group(shared_users, 5, { align: "left", overlap: true }));
	}
	show() {
		var me = this;
		...
		criscostack.call({
			method: "criscostack.share.get_users",
			args: {
	...
	render_shared(shared) {
		...
		$(
			criscostack.render_template("set_sharing", {
				frm: this.frm,
				shared: this.shared,
				everyone: everyone,
			})
		...
		if (criscostack.model.can_share(null, this.frm)) {
	...
	make_user_input() {
		// make add-user input
		this.dialog.share_with = criscostack.ui.form.make_control({
	...
	set_edit_share_events() {
		...
		$(d.body)
			...
			.on("click", function () {
				...
				criscostack.call({
					method: "criscostack.share.set_permission",
					args: {
						doctype: me.frm.doctype,
						name: me.frm.doc.name,
						user: user,
