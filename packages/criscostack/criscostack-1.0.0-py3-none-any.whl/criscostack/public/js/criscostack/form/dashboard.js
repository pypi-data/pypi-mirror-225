...
criscostack.ui.form.Dashboard = class FormDashboard {
	...
	setup_dashboard_sections() {
		...
		this.heatmap_area = this.make_section({
			...
			is_dashboard_section: 1,
			body_html: `
				<div id="heatmap-${criscostack.model.scrub(this.frm.doctype)}" class="heatmap"></div>
	...
	refresh() {
		this.reset();
		if (this.frm.doc.__islocal || !criscostack.boot.desk_settings.dashboard) {
			return;
		}
	...
	render_links() {
		...
		$(criscostack.render_template("form_links", this.data)).appendTo(transactions_area_body);
	...
	render_report_links() {
		...
		if (this.data.reports && this.data.reports.length) {
			$(criscostack.render_template("report_links", this.data)).appendTo(parent);
			// bind reports
	...
	open_report($link) {
		...
		criscostack.provide("criscostack.route_options");
		criscostack.route_options[fieldname] = this.frm.doc.name;
		criscostack.set_route("query-report", report);
	}
	...
	open_document_list($link, show_open) {
		...
		if (this.data.internal_links[doctype]) {
			if (names.length) {
				criscostack.route_options = { name: ["in", names] };
			} else {
				return false;
			}
		} else if (this.data.fieldname) {
			criscostack.route_options = this.get_document_filter(doctype);
			if (show_open && criscostack.ui.notifications) {
				criscostack.ui.notifications.show_open_count_list(doctype);
			}
		}

		criscostack.set_route("List", doctype, "List");
	}
	...
	set_open_count() {
		...
		let method = this.data.method || "criscostack.desk.notifications.get_open_count";
		criscostack.call({
			type: "GET",
			method: method,
			args: {
	...
	// heatmap
	render_heatmap() {
		this.heatmap = new criscostack.Chart("#heatmap-" + criscostack.model.scrub(this.frm.doctype), {
			type: "heatmap",
	...
	render_graph(args) {
		...
		this.chart = new criscostack.Chart(".form-graph", args);
		if (!this.chart) {
			this.hide();
		}
	}
