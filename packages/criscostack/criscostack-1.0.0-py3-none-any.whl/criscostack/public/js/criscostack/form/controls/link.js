...
criscostack.ui.form.recent_link_validations = {};

criscostack.ui.form.ControlLink = class ControlLink extends criscostack.ui.form.ControlData {
		...
		$(`<div class="link-field ui-front" style="position: relative;">
			...
			<span class="link-btn">
				<a class="btn-open no-decoration" title="${__("Open Link")}">
					${criscostack.utils.icon("arrow-right", "xs")}
				</a>
			</span>
		...
		this.$input.on("focus", function () {
			setTimeout(function () {
				if (me.$input.val() && me.get_options()) {
					...
					me.$link.toggle(true);
					me.$link_open.attr("href", criscostack.utils.get_form_link(doctype, name));
				}
	...
	get_reference_doctype() {
		...
		if (this.doctype) return this.doctype;
			...
			return criscostack.get_route && criscostack.get_route()[0] === "List"
				? criscostack.get_route()[1]
				: null;
		}
	}
	setup_buttons() {
	...
	is_translatable() {
		return in_list(criscostack.boot?.translated_doctypes || [], this.get_options());
	}
	is_title_link() {
		return in_list(criscostack.boot?.link_title_doctypes || [], this.get_options());
	}
	async set_link_title(value) {
		...
		const link_title =
			criscostack.utils.get_link_title(doctype, value) ||
			(await criscostack.utils.fetch_link_title(doctype, value));
	...
	parse_validate_and_set_in_model(value, e, label) {
		...
		if (label) {
			...
			criscostack.utils.add_link_title(this.df.options, value, label);
		}
	...
	new_doc() {
		...
		if (df && df.get_route_options_for_new_doc) {
			criscostack.route_options = df.get_route_options_for_new_doc(this);
		} else {
			criscostack.route_options = {};
		}

		// partially entered name field
		criscostack.route_options.name_field = this.get_label_value();

		// reference to calling link
		criscostack._from_link = criscostack.utils.deep_clone(this);
		criscostack._from_link_scrollY = $(document).scrollTop();

		criscostack.ui.form.make_quick_entry(doctype, (doc) => {
	...
	setup_awesomeplete() {
		...
		this.$input.on(
			"input",
			criscostack.utils.debounce(function (e) {
				...
				criscostack.call({
					type: "POST",
					method: "criscostack.desk.search.search_link",
					no_spinner: true,
					args: args,
					callback: function (r) {
						...
						if (!me.df.only_select) {
							...
							let custom__link_options =
								criscostack.ui.form.ControlLink.link_options &&
								criscostack.ui.form.ControlLink.link_options(me);
		...
		this.$input.on("awesomplete-select", function (e) {
			...
			if (me.df.remember_last_selected_value) {
				criscostack.boot.user.last_selected_values[me.df.options] = item.value;
			}
	...
	toggle_href(doctype) {
		if (criscostack.model.can_select(doctype) && !criscostack.model.can_read(doctype)) {
	...
	get_filter_description(filters) {
		...
		function get_filter_description(filter) {
			...
			let docfield = criscostack.meta.get_docfield(doctype, fieldname);
			let label = docfield ? docfield.label : criscostack.model.unscrub(fieldname);
	...
	set_custom_query(args) {
		const is_valid_value = (value, key) => {
			...
			if (this.frm) {
				let field = criscostack.meta.get_docfield(this.frm.doctype, key);
	...
	validate_link_and_fetch(df, options, docname, value) {
		...
		function update_dependant_fields(response) {
			...
			for (const [target_field, source_field] of Object.entries(fetch_map)) {
				...
				let target_df = criscostack.meta.get_docfield(df.parent, target_field);
				let target_value = criscostack.model.get_value(df.parent, docname, target_field);
		...
		if (value) {
			return criscostack
				.xcall("criscostack.client.validate_link", {
					doctype: options,
					docname: value,
