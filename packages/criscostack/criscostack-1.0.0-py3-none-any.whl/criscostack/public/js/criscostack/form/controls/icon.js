...
criscostack.ui.form.ControlIcon = class ControlIcon extends criscostack.ui.form.ControlData {
	make_input() {
	...
	get_all_icons() {
		criscostack.symbols = [];
		$("#criscostack-symbols > symbol[id]").each(function () {
			this.id.includes("icon-") && criscostack.symbols.push(this.id.replace("icon-", ""));
		});
	}
	...
	make_icon_input() {
		...
		if (!this.selected_icon) {
			this.selected_icon = $(
				`<div class="selected-icon">${criscostack.utils.icon("folder-normal", "md")}</div>`
			);
