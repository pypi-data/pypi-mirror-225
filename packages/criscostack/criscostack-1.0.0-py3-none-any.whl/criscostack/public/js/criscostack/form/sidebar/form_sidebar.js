...
criscostack.ui.form.Sidebar = class {
	constructor(opts) {
		$.extend(this, opts);
	}

	make() {
		var sidebar_content = criscostack.render_template("form_sidebar", {
		...
		criscostack.ui.form.setup_user_image_event(this.frm);
	...
	bind_events() {
		...
		this.comments.on("click", function () {
			criscostack.utils.scroll_to(me.frm.footer.wrapper.find(".comment-box"), true);
		});

		this.like_icon.on("click", function () {
			criscostack.ui.toggle_like(me.like_wrapper, me.frm.doctype, me.frm.doc.name, function () {
				me.refresh_like();
			});
		});
	}
	...
	setup_keyboard_shortcuts() {
		...
		criscostack.ui.keys.get_shortcut_group(this.page).add(assignment_link);
	}
	...
	refresh() {
		if (this.frm.doc.__islocal) {
			...
			if (this.frm.doc.route && cint(criscostack.boot.website_tracking_enabled)) {
				...
				criscostack.utils.get_page_view_count(route).then((res) => {
					this.sidebar
						.find(".pageview-count")
			...
			this.sidebar
				...
				.html(
					__(
						"{0} edited this {1}",
						[
							criscostack.user.full_name(this.frm.doc.modified_by).bold(),
			...
			this.sidebar
				...
				.html(
					__(
						"{0} created this {1}",
						[
							criscostack.user.full_name(this.frm.doc.owner).bold(),
			...
			criscostack.ui.form.set_user_image(this.frm);
		}
	}
			...
			criscostack.call({
				...
				callback: function (res) {
					...
					me.sidebar.find(".auto-repeat-status").on("click", function () {
						criscostack.set_route("Form", "Auto Repeat", me.frm.doc.auto_repeat);
					});
				},
			});
		}
	...
	make_tags() {
		...
		this.frm.tags = new criscostack.ui.TagEditor({
			parent: tags_parent,
	...
	make_attachments() {
		var me = this;
		this.frm.attachments = new criscostack.ui.form.Attachments({
	...
	make_assignments() {
		this.frm.assign_to = new criscostack.ui.form.AssignTo({
	...
	make_shared() {
		this.frm.shared = new criscostack.ui.form.Share({
			frm: this.frm,
	...
	make_like() {
		...
		criscostack.ui.setup_like_popover(this.sidebar.find(".form-stats-likes"), ".like-icon");
	}
	...
	make_follow() {
		...
		this.follow_button.on("click", () => {
			...
			criscostack
				.call("criscostack.desk.form.document_follow.update_follow", {
					doctype: this.frm.doctype,
					doc_name: this.frm.doc.name,
					following: !is_followed,
				})
				.then(() => {
					criscostack.model.set_docinfo(
						this.frm.doctype,
						this.frm.doc.name,
	...
	refresh_like() {
		...
		const liked = criscostack.ui.is_liked(this.frm.doc);
		this.like_wrapper
	...
	make_review() {
		...
		if (criscostack.boot.energy_points_enabled && !this.frm.is_new()) {
			this.frm.reviews = new criscostack.ui.form.Review({
	...
	reload_docinfo(callback) {
		criscostack.call({
			method: "criscostack.desk.form.load.get_docinfo",
			args: {
				doctype: this.frm.doctype,
