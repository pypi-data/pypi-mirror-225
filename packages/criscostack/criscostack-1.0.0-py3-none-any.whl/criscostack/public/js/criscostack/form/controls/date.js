criscostack.ui.form.ControlDate = class ControlDate extends criscostack.ui.form.ControlData {
	...
	set_formatted_input(value) {
		...
		if (should_refresh) {
			this.datepicker.selectDate(criscostack.datetime.str_to_obj(value));
		}
	}
	set_date_options() {
		// webformTODO:
		let sysdefaults = criscostack.boot.sysdefaults;

		let lang = "en";
		criscostack.boot.user && (lang = criscostack.boot.user.language);
		...
		this.date_format = criscostack.defaultDateFormat;
		this.datepicker_options = {
			...
			firstDay: criscostack.datetime.get_first_day_of_the_week_index(),
			onSelect: () => {
	...
	get_now_date() {
		return criscostack.datetime
			.convert_to_system_tz(criscostack.datetime.now_date(true), false)
			.toDate();
	}
	set_t_for_today() {
		var me = this;
		this.$input.on("keydown", function (e) {
			if (e.which === 84) {
				...
				if (me.df.fieldtype == "Date") {
					me.set_value(criscostack.datetime.nowdate());
				}
				if (me.df.fieldtype == "Datetime") {
					me.set_value(criscostack.datetime.now_datetime());
				}
				if (me.df.fieldtype == "Time") {
					me.set_value(criscostack.datetime.now_time());
				}
				return false;
			}
		});
	...
	parse(value) {
		if (value) {
			...
			return criscostack.datetime.user_to_str(value, false, true);
		}
	}
	format_for_input(value) {
		if (value) {
			return criscostack.datetime.str_to_user(value, false, true);
		}
		return "";
	}
	validate(value) {
		if (value && !criscostack.datetime.validate(value)) {
			let sysdefaults = criscostack.sys_defaults;
			let date_format =
				sysdefaults && sysdefaults.date_format ? sysdefaults.date_format : "yyyy-mm-dd";
			criscostack.msgprint(__("Date {0} must be in format: {1}", [value, date_format]));
			return "";
		}
		return value;
	}
