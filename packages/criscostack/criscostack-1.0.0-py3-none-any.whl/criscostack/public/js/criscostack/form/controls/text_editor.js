...
criscostack.ui.form.ControlTextEditor = class ControlTextEditor extends criscostack.ui.form.ControlCode {
	make_wrapper() {
		super.make_wrapper();
	}
	...
	bind_events() {
		this.quill.on(
			"text-change",
			criscostack.utils.debounce((delta, oldDelta, source) => {
		...
		$(this.quill.root).on("keydown", (e) => {
			const key = criscostack.ui.keys && criscostack.ui.keys.get_key(e);
	...
	get_mention_options() {
		...
		return {
			...
			source: criscostack.utils.debounce(async function (search_term, renderList) {
				let method =
					me.mention_search_method || "criscostack.desk.search.get_names_for_mentions";
				let values = await criscostack.xcall(method, {
					search_term,
				});
			...
			renderItem(item) {
				...
				return `${value} ${item.is_group ? criscostack.utils.icon("users") : ""}`;
			},
		};
	}
	...
	parse(value) {
		...
		return criscostack.dom.remove_script_and_style(value);
	}
