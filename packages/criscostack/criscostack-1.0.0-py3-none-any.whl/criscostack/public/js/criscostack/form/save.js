...
criscostack.ui.form.save = function (frm, action, callback, btn) {
	...
	var save = function () {
		...
		if ((action !== "Save" || frm.is_dirty()) && check_mandatory()) {
			_call({
				method: "criscostack.desk.form.save.savedocs",
			...
			!frm.is_dirty() &&
				criscostack.show_alert({ message: __("No changes in document"), indicator: "orange" });
	...
	var remove_empty_rows = function () {
		...
		*/
		const docs = criscostack.model.get_all_docs(frm.doc);

		// we should only worry about table data
		const tables = docs.filter((d) => {
			return criscostack.model.is_table(d.doctype);
		});

		let modified_table_fields = [];

		tables.map((doc) => {
			const cells = criscostack.meta.docfield_list[doc.doctype] || [];
			...
			if (is_empty_row(in_list_view_cells)) {
				criscostack.model.clear_doc(doc.doctype, doc.name);
	...
	var cancel = function () {
		...
		var workflow_state_fieldname = criscostack.workflow.get_state_fieldname(frm.doctype);
		...
		_call({
			method: "criscostack.desk.form.save.cancel",
			args: args,
			callback: function (r) {
				$(document).trigger("save", [frm.doc]);
				callback(r);
	...
	var check_mandatory = function () {
		...
		$.each(criscostack.model.get_all_docs(frm.doc), function (i, doc) {
			var error_fields = [];
			var folded = false;

			$.each(criscostack.meta.docfield_list[doc.doctype] || [], function (i, docfield) {
				if (docfield.fieldname) {
					const df = criscostack.meta.get_docfield(doc.doctype, docfield.fieldname, doc.name);
					...
					if (
						...
						!criscostack.model.has_value(doc.doctype, doc.name, df.fieldname)
					) {
						has_errors = true;
			...
			if (error_fields.length) {
				...
				let message;
				if (meta.istable) {
					const table_field = criscostack.meta.docfield_map[doc.parenttype][doc.parentfield];

					const table_label = __(
						table_field.label || criscostack.unscrub(table_field.fieldname)
					).bold();
	...
	let is_docfield_mandatory = function (doc, df) {
		...
		let parent = criscostack.get_meta(df.parent);
		...
		if (typeof expression === "boolean") {
			...
			try {
				...
				criscostack.throw(__('Invalid "mandatory_depends_on" expression'));
			}
		} else {
	...
	var _call = function (opts) {
		...
		if (criscostack.ui.form.is_saving) {
		...
		if (frm.is_new()) {
			criscostack.ui.form.remove_old_form_route();
		}
		criscostack.ui.form.is_saving = true;
		...
		return criscostack.call({
			...
			always: function (r) {
				$(btn).prop("disabled", false);
				criscostack.ui.form.is_saving = false;

				if (r) {
					var doc = r.docs && r.docs[0];
					if (doc) {
						criscostack.ui.form.update_calling_link(doc);
					}
				}
			},
		});
...
criscostack.ui.form.remove_old_form_route = () => {
	let current_route = criscostack.get_route().join("/");
	criscostack.route_history = criscostack.route_history.filter(
		(route) => route.join("/") !== current_route
	);
};

criscostack.ui.form.update_calling_link = (newdoc) => {
	if (!criscostack._from_link) return;
	var doc = criscostack.get_doc(criscostack._from_link.doctype, criscostack._from_link.docname);

	let is_valid_doctype = () => {
		if (criscostack._from_link.df.fieldtype === "Link") {
			return newdoc.doctype === criscostack._from_link.df.options;
		} else {
			// dynamic link, type is dynamic
			return newdoc.doctype === doc[criscostack._from_link.df.options];
		}
	};

	if (is_valid_doctype()) {
		criscostack.model.with_doctype(newdoc.doctype, () => {
			let meta = criscostack.get_meta(newdoc.doctype);
			// set value
			if (doc && doc.parentfield) {
				//update values for child table
				$.each(
					criscostack._from_link.frm.fields_dict[doc.parentfield].grid.grid_rows,
					function (index, field) {
						if (field.doc && field.doc.name === criscostack._from_link.docname) {
							if (meta.title_field && meta.show_title_field_in_link) {
								criscostack.utils.add_link_title(
									newdoc.doctype,
									newdoc.name,
									newdoc[meta.title_field]
								);
							}
							criscostack._from_link.set_value(newdoc.name);
						}
					}
				);
			} else {
			...
			// refresh field
			criscostack._from_link.refresh();

			// if from form, switch
			if (criscostack._from_link.frm) {
				criscostack
					.set_route(
						"Form",
						criscostack._from_link.frm.doctype,
						criscostack._from_link.frm.docname
					)
					.then(() => {
						criscostack.utils.scroll_to(criscostack._from_link_scrollY);
					});
			}

			criscostack._from_link = null;
		});
	}
};
