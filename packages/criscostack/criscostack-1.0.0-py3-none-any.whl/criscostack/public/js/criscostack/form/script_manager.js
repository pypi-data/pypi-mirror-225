...
criscostack.provide("criscostack.ui.form.handlers");
...
criscostack.ui.form.get_event_handler_list = function (doctype, fieldname) {
	if (!criscostack.ui.form.handlers[doctype]) {
		criscostack.ui.form.handlers[doctype] = {};
	}
	if (!criscostack.ui.form.handlers[doctype][fieldname]) {
		criscostack.ui.form.handlers[doctype][fieldname] = [];
	}
	return criscostack.ui.form.handlers[doctype][fieldname];
};

criscostack.ui.form.on = criscostack.ui.form.on_change = function (doctype, fieldname, handler) {
	var add_handler = function (fieldname, handler) {
		var handler_list = criscostack.ui.form.get_event_handler_list(doctype, fieldname);
...
criscostack.ui.form.off = function (doctype, fieldname, handler) {
	var handler_list = criscostack.ui.form.get_event_handler_list(doctype, fieldname);
...
criscostack.ui.form.trigger = function (doctype, fieldname) {
	cur_frm.script_manager.trigger(fieldname, doctype);
};

criscostack.ui.form.ScriptManager = class ScriptManager {
	constructor(opts) {
		$.extend(this, opts);
	}
	...
	trigger(event_name, doctype, name) {
		...
		this.frm.selected_doc = criscostack.get_doc(doctype, name);
	...
	get_handlers(event_name, doctype) {
		...
		if (criscostack.ui.form.handlers[doctype] && criscostack.ui.form.handlers[doctype][event_name]) {
			$.each(criscostack.ui.form.handlers[doctype][event_name], function (i, fn) {
	...
	setup() {
		...
		$.each(this.frm.fields, function (i, field) {
			...
			if (criscostack.model.table_fields.includes(field.df.fieldtype)) {
				$.each(
					criscostack.meta.get_docfields(field.df.options, me.frm.docname),
					function (i, df) {
						setup_add_fetch(df);
					}
				);
		...
		// css
		doctype.__css && criscostack.dom.set_style(doctype.__css);

		this.trigger("setup");
	}
	log_error(caller, e) {
		criscostack.show_alert({ message: __("Error in Client Script."), indicator: "error" });
