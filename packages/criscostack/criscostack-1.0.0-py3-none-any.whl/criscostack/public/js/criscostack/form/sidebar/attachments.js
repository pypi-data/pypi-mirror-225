...
criscostack.ui.form.Attachments = class Attachments {
	constructor(opts) {
		$.extend(this, opts);
		this.make();
	}
	...
	add_attachment(attachment) {
		...
		if (criscostack.model.can_write(this.frm.doctype, this.frm.name)) {
			remove_action = function (target_id) {
				criscostack.confirm(__("Are you sure you want to delete the attachment?"), function () {
		...
		const icon = `<a href="/app/file/${fileid}">
				${criscostack.utils.icon(attachment.is_private ? "lock" : "unlock", "sm ml-0")}
			</a>`;

		$(`<li class="attachment-row">`)
			.append(criscostack.get_data_pill(file_label, fileid, remove_action, icon))
	...
	remove_attachment(fileid, callback) {
		...
		return criscostack.call({
			method: "criscostack.desk.form.utils.remove_attach",
			type: "DELETE",
			args: {
				fid: fileid,
				dt: me.frm.doctype,
			...
			callback: function (r, rt) {
				if (r.exc) {
					if (!r._server_messages) criscostack.msgprint(__("There were errors"));
					return;
				}
