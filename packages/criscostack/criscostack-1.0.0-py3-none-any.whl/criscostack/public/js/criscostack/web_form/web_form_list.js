...
criscostack.provide("criscostack.web_form_list");

export default class WebFormList {
	constructor(opts) {
		Object.assign(this, opts);
		criscostack.web_form_list = this;
	...
	make_filters() {
		...
		criscostack
			.call("criscostack.website.doctype.web_form.web_form.get_web_form_filters", {
			...
			.then((response) => {
				...
				fields.forEach((field) => {
					...
					let input = criscostack.ui.form.make_control({
						df: {
	...
	fetch_data() {
		let args = {
			method: "criscostack.www.list.get_list_data",
			args: {
	...
	more() {
		...
		this.fetch_data().then((res) => {
			if (res.message.length === 0) {
				criscostack.msgprint(__("No more items to display"));
			}
	...
	make_table_body() {
		if (this.data.length) {
			...
			this.wrapper.empty();
			criscostack.has_permission(this.doctype, "", "create", () => {
		...
		let empty_state = $(`
			<div class="no-result text-muted flex justify-center align-center">
				<div class="text-center">
					<div>
						<img
							src="/assets/criscostack/images/ui-states/list-empty-state.svg"
	...
	append_rows(row_data) {
		...
		row_data.forEach((data_item) => {
			...
			let row = new criscostack.ui.WebFormListRow({
				row: $row_element,
				doc: data_item,
	...
	make_actions() {
		...
		criscostack.has_permission(this.doctype, "", "delete", () => {
	...
	delete_rows() {
		...
		criscostack
			.call({
				type: "POST",
				method: "criscostack.website.doctype.web_form.web_form.delete_multiple",
				args: {
					web_form_name: this.web_form_name,
...
criscostack.ui.WebFormListRow = class WebFormListRow {
	...
	make_row() {
		...
		this.columns.forEach((field) => {
			let formatter = criscostack.form.get_formatter(field.fieldtype);
			let value =
