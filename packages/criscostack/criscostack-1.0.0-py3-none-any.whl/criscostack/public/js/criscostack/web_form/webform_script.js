...
criscostack.ready(function () {
	let web_form_doc = criscostack.web_form_doc;
	let reference_doc = criscostack.reference_doc;
	...
	function show_login_prompt() {
		if (criscostack.session.user != "Guest" || !web_form_doc.login_required) return;
		const login_required = new criscostack.ui.Dialog({
	...
	function show_form() {
		...
		$(".file-size").each(function () {
			$(this).text(criscostack.form.formatters.FileSize($(this).text()));
		});
	}
