...
*/
criscostack.model.DocTypeController = class DocTypeController extends criscostack.ui.form.Controller {
	setup() {
		// setup formatters for fieldtype
		criscostack.meta.docfield_map[
	...
	show_db_utilization() {
		...
		criscostack
			.xcall("criscostack.core.doctype.doctype.doctype.get_row_size_utilization", {
				doctype,
			})
			.then((r) => {
				if (r < 50.0) return;
	...
	setup_fetch_from_fields(doc, doctype, docname) {
		...
		let row = criscostack.get_doc(doctype, docname);
		...
		function update_fieldname_options() {
			...
			criscostack.model.with_doctype(link_doctype, () => {
				let fields = criscostack.meta
