criscostack.provide("criscostack.model.user_settings");

$.extend(criscostack.model.user_settings, {
	get: function (doctype) {
		return criscostack
			.call("criscostack.model.utils.user_settings.get", { doctype })
			.then((r) => JSON.parse(r.message || "{}"));
	},
	save: function (doctype, key, value) {
		if (criscostack.session.user === "Guest") return Promise.resolve();

		const old_user_settings = criscostack.model.user_settings[doctype] || {};
	...
	remove: function (doctype, key) {
		var user_settings = criscostack.model.user_settings[doctype] || {};
	...
	update: function (doctype, user_settings) {
		...
		return criscostack.call({
			method: "criscostack.model.utils.user_settings.save",
			args: {
				doctype: doctype,
				user_settings: user_settings,
			},
			callback: function (r) {
				criscostack.model.user_settings[doctype] = r.message;
			},
		});
	},
});

criscostack.get_user_settings = function (doctype, key) {
	var settings = criscostack.model.user_settings[doctype] || {};
	if (key) {
