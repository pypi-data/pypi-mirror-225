				...
				<div class="mt-2 text-center">
					...
					<button v-if="google_drive_settings.enabled" class="btn btn-file-upload" @click="show_google_drive_picker">
						<svg width="30" height="30">
							<image href="/assets/criscostack/icons/social/google_drive.svg" width="30" height="30"/>
						</svg>
...
if (criscostack.user_id !== "Guest") {
	criscostack.call({
		// method only available after login
		method: "criscostack.integrations.doctype.google_settings.google_settings.get_file_picker_settings",
		callback: (resp) => {
			if (!resp.exc) {
...
if (props.restrictions.max_file_size == null) {
	criscostack.call('criscostack.core.api.file.get_max_file_size')
		.then(res => {
			props.restrictions.max_file_size = Number(res.message);
		});
}
if (props.restrictions.max_number_of_files == null && props.doctype) {
	props.restrictions.max_number_of_files = criscostack.get_meta(props.doctype)?.max_attachments;
}
...
function upload_via_file_browser() {
	...
	if (!selected_file.value) {
		criscostack.msgprint(__('Click on a file to select it.'));
...
function upload_via_web_link() {
	...
	if (!file_url) {
		criscostack.msgprint(__('Invalid URL'));
		close_dialog.value = true;
		return Promise.reject();
	}
...
function return_as_dataurl() {
	let promises = files.value.map(file =>
		criscostack.dom.file_to_base64(file.file_obj)
			.then(dataurl => {
		...
		xhr.onreadystatechange = () => {
			if (xhr.readyState == XMLHttpRequest.DONE) {
				if (xhr.status === 200) {
					...
					criscostack.request.cleanup({}, error);
				}
			}
		}
...
function capture_image() {
	const capture = new criscostack.ui.Capture({
		animate: false,
		error: true
	});
	capture.show();
	capture.submit(data_urls => {
		data_urls.forEach(data_url => {
			let filename = `capture_${criscostack.datetime.now_datetime().replaceAll(/[: -]/g, '_')}.png`;
