class WidgetDialog {
		...
		this.dialog = new criscostack.ui.Dialog({
			...
			primary_action: (data) => {
				...
				if (!this.editing && !data.name) {
					data.name = `${this.type}-${this.label}-${criscostack.utils.get_random(20)}`;
				}
	...
	get_title() {
		...
		let label = (action = action + " " + criscostack.model.unscrub(this.type));
		return __(label);
	}
...
class QuickListDialog extends WidgetDialog {
	...
	generate_filter_from_json() {
		if (this.values && this.values.quick_list_filter) {
			this.filters = criscostack.utils.get_filter_from_json(
	...
	process_data(data) {
		if (this.filter_group) {
			...
			data.quick_list_filter = criscostack.utils.get_filter_as_json(filters);
		}
...
class ShortcutDialog extends WidgetDialog {
			...
			},
			{
				...
				onchange: () => {
					...
					if (doctype && this.dialog.get_value("type") == "DocType") {
						criscostack.model.with_doctype(doctype, async () => {
							let meta = criscostack.get_meta(doctype);

							if (doctype && criscostack.boot.single_types.includes(doctype)) {
								this.hide_filters();
							} else if (doctype) {
							...
							if (criscostack.boot.calendars.includes(doctype)) views.push("Calendar");

							const response = await criscostack.db.get_value(
								"Kanban Board",
			...
			},
			{
				...
				depends_on: (state) => {
					if (this.dialog) {
						...
						let is_single = criscostack.boot.single_types.includes(doctype);
	...
	process_data(data) {
		if (this.dialog.get_value("type") == "DocType" && this.filter_group) {
			let filters = this.filter_group.get_filters();
			data.stats_filter = criscostack.utils.get_filter_as_json(filters);
		}

		data.label = data.label ? data.label : criscostack.model.unscrub(data.link_to);
...
class NumberCardDialog extends WidgetDialog {
			...
			return [
				{
					...
					get_query: () => {
						return {
							query: "criscostack.desk.doctype.number_card.number_card.get_cards_for_user",
							filters: {
	...
	set_aggregate_function_fields() {
		...
		if (this.document_type && criscostack.get_meta(this.document_type)) {
			criscostack.get_meta(this.document_type).fields.map((df) => {
		...
		return [
			{
				...
				get_query: () => {
					return {
						query: "criscostack.desk.doctype.custom_html_block.custom_html_block.get_custom_blocks_for_user",
					};
				},
			},
		];
