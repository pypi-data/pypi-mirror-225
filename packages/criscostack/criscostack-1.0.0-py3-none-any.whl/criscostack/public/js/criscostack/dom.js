...
criscostack.provide("criscostack.dom");
...
criscostack.dom = {
	...
	get_unique_id: function () {
		const id = "unique-" + criscostack.dom.id_count;
		criscostack.dom.id_count++;
		return id;
	},
	set_unique_id: function (ele) {
		...
		var id = "unique-" + criscostack.dom.id_count;
		$ele.attr("id", id);
		criscostack.dom.id_count++;
		return id;
	},
	...
	add: function (parent, newtag, className, cs, innerHTML, onclick) {
		if (parent && parent.substr) parent = criscostack.dom.by_id(parent);
		...
		if (cs) criscostack.dom.css(c, cs);
	...
	freeze: function (msg, css_class) {
		...
		criscostack.dom.freeze_count++;
	},
	unfreeze: function () {
		if (!criscostack.dom.freeze_count) return; // anything open?
		criscostack.dom.freeze_count--;
		if (!criscostack.dom.freeze_count) {
	...
	scroll_to_section(section_name) {
		setTimeout(() => {
			...
			if (section.length) {
				...
				criscostack.ui.scroll(section.parent().parent());
			}
		}, 200);
	},
...
criscostack.ellipsis = function (text, max) {
	if (!max) max = 20;
	text = cstr(text);
	if (text.length > max) {
...
criscostack.run_serially = function (tasks) {
...
criscostack.load_image = (src, onload, onerror, preprocess = () => {}) => {
...
criscostack.timeout = (seconds) => {
	return new Promise((resolve) => {
		setTimeout(() => resolve(), seconds * 1000);
	});
};

criscostack.scrub = function (text, spacer = "_") {
	return text.replace(/ /g, spacer).toLowerCase();
};

criscostack.unscrub = function (txt) {
	return criscostack.model.unscrub(txt);
};

criscostack.get_data_pill = (label, target_id = null, remove_action = null, image = null) => {
	...
	if (remove_action) {
		let remove_btn = $(`
			<span class="remove-btn cursor-pointer">
				${criscostack.utils.icon("close", "sm")}
			</span>
		`).click(() => {
...
criscostack.get_modal = function (title, content) {
				...
				<div class="modal-header">
					...
					<div class="modal-actions">
						<button class="btn btn-modal-minimize btn-link hide">
							${criscostack.utils.icon("collapse")}
						</button>
						<button class="btn btn-modal-close btn-link" data-dismiss="modal">
							${criscostack.utils.icon("close-alt", "sm", "close-alt")}
						</button>
					</div>
				</div>
...
criscostack.is_online = function () {
	if (criscostack.boot.developer_mode == 1) {
...
criscostack.create_shadow_element = function (wrapper, html, css, js) {
	let random_id = "custom-block-" + criscostack.utils.get_random(5).toLowerCase();
	...
	class CustomBlock extends HTMLElement {
		constructor() {
			...
			div.innerHTML = criscostack.dom.remove_script_and_style(html);

			// link global desk css
			let link = document.createElement("link");
			link.rel = "stylesheet";
			link.href = criscostack.assets.bundled_asset("desk.bundle.css");
