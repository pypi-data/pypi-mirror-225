...
criscostack.ui.color = {
	get: function (color_name, shade) {
		...
		return criscostack.ui.color_map;
	},
	get_color: function (color_name) {
		const color_names = Object.keys(criscostack.ui.color_map);
		if (color_names.includes(color_name)) {
			return criscostack.ui.color_map[color_name];
		} else {
	...
	all: function () {
		return Object.values(criscostack.ui.color_map).reduce((acc, curr) => acc.concat(curr), []);
	},
	names: function () {
		return Object.keys(criscostack.ui.color_map);
	},
	...
	get_color_name: function (hex) {
		for (const key in criscostack.ui.color_map) {
			const colors = criscostack.ui.color_map[key];
...
criscostack.ui.color_map = criscostack.ui.color.get_color_map();
