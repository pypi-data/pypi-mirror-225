...
criscostack.ui.Tags = class {
	...
	bind() {
		...
		const select_tag = function () {
			const tagValue = criscostack.utils.xss_sanitise(me.$input.val());
			me.addTag(tagValue);
			me.$input.val("");
		};
	...
	removeTag(label) {
		label = criscostack.utils.xss_sanitise(label);
	...
	get_tag(label) {
		let $tag = criscostack.get_data_pill(label, label, (target, pill_wrapper) => {
