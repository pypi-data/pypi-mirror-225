...
criscostack.tags.utils = {
	get_tags: function (txt) {
		txt = txt.slice(1);
		let out = [];

		if (!criscostack.tags.tags) {
			criscostack.tags.utils.fetch_tags();
			return [];
		}

		for (let i in criscostack.tags.tags) {
			let tag = criscostack.tags.tags[i];
			let level = criscostack.search.utils.fuzzy_search(txt, tag);
			if (level) {
				out.push({
					type: "Tag",
					label: __("#{0}", [criscostack.search.utils.bolden_match_part(__(tag), txt)]),
					value: __("#{0}", [__(tag)]),
					index: 1 + level,
					match: tag,
					onclick() {
						// Use Global Search Dialog for tag search too.
						criscostack.searchdialog.search.init_search("#".concat(tag), "tags");
					},
				});
			}
		}
	...
	fetch_tags() {
		criscostack.call({
			method: "criscostack.desk.doctype.tag.tag.get_tags_list_for_awesomebar",
			callback: function (r) {
				if (r && r.message) {
					criscostack.tags.tags = $.extend([], r.message);
				}
			},
		});
	},
	...
	get_tag_results: function (tag) {
		...
		return new Promise(function (resolve) {
			criscostack.call({
				method: "criscostack.desk.doctype.tag.tag.get_documents_for_tag",
				args: {
					tag: tag,
				},
				callback: function (r) {
