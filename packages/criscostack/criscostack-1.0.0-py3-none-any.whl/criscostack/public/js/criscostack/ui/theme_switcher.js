...
criscostack.ui.ThemeSwitcher = class ThemeSwitcher {
	constructor() {
		this.setup_dialog();
		this.refresh();
	}
	...
	bind_events() {
		this.dialog.$wrapper.on("keydown", (e) => {
			...
			const key = criscostack.ui.keys.get_key(e);
			let increment_by;
				...
				<div class="background">
					<div>
						<div class="preview-check" data-theme=${is_auto_theme ? "dark" : theme.name}>
							${criscostack.utils.icon("tick", "xs")}
						</div>
					</div>
	...
	toggle_theme(theme) {
		...
		criscostack.show_alert(__("Theme Changed"), 3);

		criscostack.xcall("criscostack.core.doctype.user.user.switch_theme", {
			theme: toTitle(theme),
		});
	}
...
criscostack.ui.add_system_theme_switch_listener = () => {
	criscostack.ui.dark_theme_media_query.addEventListener("change", () => {
		criscostack.ui.set_theme();
	});
};

criscostack.ui.dark_theme_media_query = window.matchMedia("(prefers-color-scheme: dark)");

criscostack.ui.set_theme = (theme) => {
	const root = document.documentElement;
	let theme_mode = root.getAttribute("data-theme-mode");
	if (!theme) {
		if (theme_mode === "automatic") {
			theme = criscostack.ui.dark_theme_media_query.matches ? "dark" : "light";
		}
	}
