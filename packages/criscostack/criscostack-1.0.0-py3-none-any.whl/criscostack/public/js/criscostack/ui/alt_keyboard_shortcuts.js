criscostack.provide("criscostack.ui.keys");

let shortcut_groups = new WeakMap();
let shortcut_group_list = [];
criscostack.ui.keys.shortcut_groups = shortcut_groups;

criscostack.ui.keys.get_shortcut_group = (parent) => {
	// parent must be an object
	if (!shortcut_groups.has(parent)) {
		shortcut_groups.set(parent, new criscostack.ui.keys.AltShortcutGroup());
	}
...
criscostack.ui.keys.bind_shortcut_group_event = () => {
	...
	$(document).on("keydown", (e) => {
		let key = (criscostack.ui.keys.key_map[e.which] || "").toLowerCase();
...
criscostack.ui.keys.AltShortcutGroup = class AltShortcutGroup {
	constructor() {
		this.shortcuts_dict = {};
	...
	is_taken(letter) {
		let is_in_global_shortcut = criscostack.ui.keys.standard_shortcuts
			.filter((s) => !s.page)
