...
criscostack.provide("criscostack.tags");

criscostack.search.AwesomeBar = class AwesomeBar {
	setup(element) {
		var me = this;
			...
			criscostack.utils.debounce(function (e) {
				...
				if (txt && txt.length > 1) {
					...
					me.options = me.options.concat(
						me.deduplicate(criscostack.search.utils.get_recent_pages(txt || ""))
					);
					me.options = me.options.concat(criscostack.search.utils.get_frequent_links());
				}
				me.add_help();
		...
		$input.on("awesomplete-select", function (e) {
			...
			if (item.route_options) {
				criscostack.route_options = item.route_options;
			}
			...
			if (item.onclick) {
				...
				criscostack.set_route(item.route);
			}
			$input.val("");
			$input.trigger("blur");
		});
		...
		criscostack.search.utils.setup_recent();
	}
	...
	add_help() {
		this.options.push({
			...
			onclick: function () {
				...
				criscostack.msgprint(txt, __("Search Help"));
			},
		});
	}
	...
	build_options(txt) {
		var options = criscostack.search.utils
			.get_creatables(txt)
			.concat(
				criscostack.search.utils.get_search_in_list(txt),
				criscostack.search.utils.get_doctypes(txt),
				criscostack.search.utils.get_reports(txt),
				criscostack.search.utils.get_pages(txt),
				criscostack.search.utils.get_workspaces(txt),
				criscostack.search.utils.get_dashboards(txt),
				criscostack.search.utils.get_recent_pages(txt || ""),
				criscostack.search.utils.get_executables(txt)
			);
		if (txt.charAt(0) === "#") {
			options = criscostack.tags.utils.get_tags(txt);
		}
	...
	make_global_search(txt) {
		...
		// 		<span class="ellipsis">Search for ${criscostack.utils.xss_sanitise(txt).bold()}</span>
		// 		<kbd>↵</kbd>
		// 	</span>
		// `);

		// search_text.click(() => {
		// 	criscostack.searchdialog.search.init_search(txt, "global_search");
		// });
		...
		this.options.push({
			label: `
				<span class="flex justify-between text-medium">
					<span class="ellipsis">${__("Search for {0}", [criscostack.utils.xss_sanitise(txt).bold()])}</span>
					<kbd>↵</kbd>
				</span>
			`,
	...
	make_search_in_current(txt) {
		...
		if (route[0] === "List" && txt.indexOf(" in") === -1) {
			...
			var meta = criscostack.get_meta(criscostack.container.page.list_view.doctype);
		...
		if (first == parseInt(first) || first === "(" || first === "=") {
			...
			try {
				...
				this.options.push({
					...
					default: "Calculator",
					onclick: function () {
						criscostack.msgprint(formatted_value, __("Result"));
					},
				});
			} catch (e) {
				// pass
	...
	make_random(txt) {
		if (txt.toLowerCase().includes("random")) {
			this.options.push({
				...
				value: criscostack.utils.get_random(16),
				onclick: function () {
					criscostack.msgprint(criscostack.utils.get_random(16), __("Result"));
				},
			});
		}
	}
