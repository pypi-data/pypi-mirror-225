...
criscostack.ui.Tree = class {
	constructor({
		...
		if (!icon_set) {
			this.icon_set = {
				open: criscostack.utils.icon("folder-open", "md"),
				closed: criscostack.utils.icon("folder-normal", "md"),
				leaf: criscostack.utils.icon("primitive-dot", "xs"),
			};
		}
	...
	get_all_nodes(value, is_root, label) {
		...
		return new Promise((resolve) => {
			criscostack.call({
				method: "criscostack.desk.treeview.get_all_nodes",
				args: args,
				callback: (r) => {
	...
	on_node_click(node) {
		this.expand_node(node);
		criscostack.dom.activate(this.wrapper, node.$tree_link, "tree-link");
