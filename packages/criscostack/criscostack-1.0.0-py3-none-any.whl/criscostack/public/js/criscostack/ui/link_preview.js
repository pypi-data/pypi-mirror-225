criscostack.ui.LinkPreview = class {
	constructor() {
		this.popovers_list = [];
	...
	setup_popover_control(e) {
		if (!(criscostack.boot.link_preview_doctypes || []).includes(this.doctype)) {
			return;
		}
	...
	get_preview_data() {
		return criscostack.xcall("criscostack.desk.link_preview.get_preview_data", {
	...
	get_image_html(preview_data) {
		let avatar_html = criscostack.get_avatar(
			"avatar-medium",
	...
	get_content_html(preview_data) {
		...
		Object.keys(preview_data).forEach((key) => {
			if (!["preview_image", "preview_title", "name"].includes(key)) {
				let value = criscostack.ellipsis(preview_data[key], 280);
				let label = key;
				content_html += `
