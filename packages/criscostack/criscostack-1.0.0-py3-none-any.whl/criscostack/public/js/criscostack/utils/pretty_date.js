function prettyDate(date, mini) {
	...
	if (typeof date == "string") {
		date = criscostack.datetime.convert_to_user_tz(date);
		date = new Date(
			(date || "")
				.replace(/-/g, "/")
				.replace(/[TZ]/g, " ")
	...
	let diff =
		(new Date(criscostack.datetime.now_datetime().replace(/-/g, "/")).getTime() - date.getTime()) /
		1000;
...
window.comment_when = function (datetime, mini) {
	var timestamp = criscostack.datetime.str_to_user ? criscostack.datetime.str_to_user(datetime) : datetime;
	return (
		'<span class="criscostack-timestamp ' +
		(mini ? " mini" : "") +
		'" data-timestamp="' +
		datetime +
		'" title="' +
...
criscostack.datetime.comment_when = comment_when;
criscostack.datetime.prettyDate = prettyDate;

criscostack.datetime.refresh_when = function () {
	if (jQuery) {
		$(".criscostack-timestamp").each(function () {
