...
criscostack.provide("criscostack.datetime");

criscostack.defaultDateFormat = "YYYY-MM-DD";
criscostack.defaultTimeFormat = "HH:mm:ss";
criscostack.defaultDatetimeFormat = criscostack.defaultDateFormat + " " + criscostack.defaultTimeFormat;
moment.defaultFormat = criscostack.defaultDateFormat;

criscostack.provide("criscostack.datetime");

$.extend(criscostack.datetime, {
	convert_to_user_tz: function (date, format) {
		...
		let date_obj = null;
		if (criscostack.boot.time_zone && criscostack.boot.time_zone.system && criscostack.boot.time_zone.user) {
			date_obj = moment
				.tz(date, criscostack.boot.time_zone.system)
				.clone()
				.tz(criscostack.boot.time_zone.user);
		} else {
			date_obj = moment(date);
		}

		return format === false ? date_obj : date_obj.format(criscostack.defaultDatetimeFormat);
	},
	...
	convert_to_system_tz: function (date, format) {
		...
		if (criscostack.boot.time_zone && criscostack.boot.time_zone.system && criscostack.boot.time_zone.user) {
			date_obj = moment
				.tz(date, criscostack.boot.time_zone.user)
				.clone()
				.tz(criscostack.boot.time_zone.system);
		} else {
			date_obj = moment(date);
		}
	...
	is_system_time_zone: function () {
		if (criscostack.boot.time_zone && criscostack.boot.time_zone.system && criscostack.boot.time_zone.user) {
			return (
				moment().tz(criscostack.boot.time_zone.system).utcOffset() ===
				moment().tz(criscostack.boot.time_zone.user).utcOffset()
			);
		}
	...
	is_timezone_same: function () {
		return criscostack.datetime.is_system_time_zone();
	},

	str_to_obj: function (d) {
		return moment(d, criscostack.defaultDatetimeFormat)._d;
	},
	...
	obj_to_user: function (d) {
		return moment(d).format(criscostack.datetime.get_user_date_fmt().toUpperCase());
	},
	...
	get_user_time_fmt: function () {
		return (criscostack.sys_defaults && criscostack.sys_defaults.time_format) || "HH:mm:ss";
	},

	get_user_date_fmt: function () {
		return (criscostack.sys_defaults && criscostack.sys_defaults.date_format) || "yyyy-mm-dd";
	},
	...
	str_to_user: function (val, only_time = false, only_date = false) {
		if (!val) return "";
		const user_date_fmt = criscostack.datetime.get_user_date_fmt().toUpperCase();
		const user_time_fmt = criscostack.datetime.get_user_time_fmt();
		let user_format = user_time_fmt;

		if (only_time) {
			let date_obj = moment(val, criscostack.defaultTimeFormat);
			return date_obj.format(user_format);
		} else if (only_date) {
			let date_obj = moment(val, criscostack.defaultDateFormat);
			return date_obj.format(user_date_fmt);
		} else {
			let date_obj = moment.tz(val, criscostack.boot.time_zone.system);
			if (typeof val !== "string" || val.indexOf(" ") === -1) {
				user_format = user_date_fmt;
			} else {
				user_format = user_date_fmt + " " + user_time_fmt;
			}
			return date_obj.clone().tz(criscostack.boot.time_zone.user).format(user_format);
		}
	},
	...
	user_to_str: function (val, only_time = false) {
		var user_time_fmt = criscostack.datetime.get_user_time_fmt();
		if (only_time) {
			return moment(val, user_time_fmt).format(criscostack.defaultTimeFormat);
		}

		var user_fmt = criscostack.datetime.get_user_date_fmt().toUpperCase();
	...
	user_to_obj: function (d) {
		return criscostack.datetime.str_to_obj(criscostack.datetime.user_to_str(d));
	},
	...
	now_date: function (as_obj = false) {
		return criscostack.datetime._date(criscostack.defaultDateFormat, as_obj);
	},

	now_time: function (as_obj = false) {
		return criscostack.datetime._date(criscostack.defaultTimeFormat, as_obj);
	},

	now_datetime: function (as_obj = false) {
		return criscostack.datetime._date(criscostack.defaultDatetimeFormat, as_obj);
	},

	system_datetime: function (as_obj = false) {
		return criscostack.datetime._date(criscostack.defaultDatetimeFormat, as_obj, true);
	},

	_date: function (format, as_obj = false, system_time = false) {
		let time_zone = criscostack.boot.time_zone?.system || criscostack.sys_defaults.time_zone;
		...
		if (!system_time) {
			time_zone = criscostack.boot.time_zone?.user || time_zone;
		}
		let date = moment.tz(time_zone);

		return as_obj ? criscostack.datetime.moment_to_date_obj(date) : date.format(format);
	},
	...
	nowdate: function () {
		return criscostack.datetime.now_date();
	},
	...
	validate: function (d) {
		return moment(
			d,
			[criscostack.defaultDateFormat, criscostack.defaultDatetimeFormat, criscostack.defaultTimeFormat],
			true
		).isValid();
	},

	get_first_day_of_the_week_index() {
		const first_day_of_the_week = criscostack.sys_defaults.first_day_of_the_week || "Sunday";
