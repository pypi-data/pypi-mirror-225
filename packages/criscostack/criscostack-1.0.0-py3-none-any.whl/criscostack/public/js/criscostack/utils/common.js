...
criscostack.avatar = function (
	...
	if (user) {
		// desk
		user_info = criscostack.user_info(user);
	} else {
		// website
		let full_name = title || criscostack.get_cookie("full_name");
		user_info = {
			image: image_url === null ? criscostack.get_cookie("user_image") : image_url,
			fullname: full_name,
			abbr: criscostack.get_abbr(full_name),
			color: criscostack.get_palette(full_name),
		};
	}
...
criscostack.get_avatar = function (css_class, title, image_url = null, remove_color, data_attributes) {
	if (!css_class) {
	...
	if (image_url) {
		const image =
			window.cordova && image_url.indexOf("http") === -1
				? criscostack.base_url + image_url
				: image_url;
		...
		let abbr = criscostack.get_abbr(title);
		let style = "";
		if (!remove_color) {
			let color = criscostack.get_palette(title);
...
criscostack.avatar_group = function (users, limit = 4, options = {}) {
	...
	let html = display_users
		.map((user) =>
			criscostack.avatar(user, "avatar-small " + css_class, null, null, false, options.filterable)
		)
		.join("");
	if (extra_users.length === 1) {
		html += criscostack.avatar(
			extra_users[0],
		...
		html = `
			...
			<span class="avatar avatar-small ${css_class}">
				<div class="avatar-frame standard-image avatar-extra-count"
					title="${extra_users.map((u) => criscostack.user_info(u).fullname).join(", ")}">
					+${extra_users.length}
				</div>
			</span>
		`;
		...
		avatar_action_html = `
			<span class="avatar avatar-small">
				<div class="avatar-frame avatar-action">
					${criscostack.utils.icon(options.action_icon, "sm")}
				</div>
			</span>
		`;
	}
...
criscostack.ui.scroll = function (element, animate, additional_offset) {
...
criscostack.get_palette = function (txt) {
	if (!txt) return criscostack.palette[8]; // breaks when undefined
	var idx = cint((parseInt(md5(txt).substr(4, 2), 16) + 1) / 5.33);
	return criscostack.palette[idx % 8];
};

criscostack.get_abbr = function (txt, max_length) {
	if (!txt) return "";
	var abbr = "";
...
criscostack.gravatars = {};
criscostack.get_gravatar = function (email_id, size = 0) {
	var param = size ? "s=" + size : "d=retro";
	if (!criscostack.gravatars[email_id]) {
		// TODO: check if gravatar exists
		criscostack.gravatars[email_id] =
			"https://secure.gravatar.com/avatar/" + md5(email_id) + "?" + param;
	}
	return criscostack.gravatars[email_id];
};
...
criscostack.get_cookie = function getCookie(name) {
	return criscostack.get_cookies()[name];
};

criscostack.get_cookies = function getCookies() {
...
criscostack.is_mobile = function () {
	return $(document).width() < 768;
};

criscostack.utils.xss_sanitise = function (string, options) {
...
criscostack.utils.sanitise_redirect = (url) => {
	const is_external = (() => {
		return (url) => {
			function domain(url) {
	...
	url = criscostack.utils.strip_url(url);

	return is_external(url)
		? ""
		: sanitise_javascript(criscostack.utils.xss_sanitise(url, { strategies: ["js"] }));
};

criscostack.utils.strip_url = (url) => {
...
criscostack.utils.new_auto_repeat_prompt = function (frm) {
	const fields = [
		{
			fieldname: "frequency",
			fieldtype: "Select",
		...
		},
		{
			...
			label: __("Start Date"),
			reqd: 1,
			default: criscostack.datetime.nowdate(),
		},
		{
			fieldname: "end_date",
			fieldtype: "Date",
	...
	criscostack.prompt(
		...
		function (values) {
			criscostack.call({
				method: "criscostack.automation.doctype.auto_repeat.auto_repeat.make_auto_repeat",
				args: {
...
criscostack.utils.get_page_view_count = function (route) {
	return criscostack.call("criscostack.website.doctype.web_page_view.web_page_view.get_page_view_count", {
		path: route,
	});
};
