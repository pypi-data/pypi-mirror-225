...
criscostack.provide("criscostack.tools");

criscostack.tools.downloadify = function (data, roles, title) {
	if (roles && roles.length && !has_common(roles, roles)) {
		criscostack.msgprint(
			__("Export not allowed. You need {0} role to export.", [criscostack.utils.comma_or(roles)])
		);
		return;
	}

	var filename = title + ".csv";
	var csv_data = criscostack.tools.to_csv(data);
...
criscostack.markdown = function (txt) {
	if (!criscostack.md2html) {
		criscostack.md2html = new showdown.Converter();
	}
	...
	return criscostack.md2html.makeHtml(txt);
};

criscostack.tools.to_csv = function (data) {
	var res = [];
