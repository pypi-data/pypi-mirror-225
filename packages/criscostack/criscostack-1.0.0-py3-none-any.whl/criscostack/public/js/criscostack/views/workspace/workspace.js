...
criscostack.standard_pages["Workspaces"] = function () {
	var wrapper = criscostack.container.add_page("Workspaces");
	...
	criscostack.workspace = new criscostack.views.Workspace(wrapper);
	$(wrapper).bind("show", function () {
		criscostack.workspace.show();
	});
};

criscostack.views.Workspace = class Workspace {
	constructor(wrapper) {
		this.wrapper = $(wrapper);
		this.page = wrapper.page;
		this.blocks = criscostack.workspace_block.blocks;
	...
	async setup_pages(reload) {
		...
		if (this.all_pages) {
			...
			for (let page of this.all_pages) {
				criscostack.workspaces[criscostack.router.slug(page.name)] = {
					title: page.title,
					public: page.public,
				};
			}
	...
	get_pages() {
		return criscostack.xcall("criscostack.desk.desktop.get_workspace_sidebar_items");
	}
			...
			>
				<div class="desk-sidebar-item standard-sidebar-item ${item.selected ? "selected" : ""}">
					<a
						href="/app/${
							item.public
								? criscostack.router.slug(item.title)
								: "private/" + criscostack.router.slug(item.title)
						}"
						class="item-anchor ${item.is_editable ? "" : "block-click"}" title="${__(item.title)}"
					>
						<span class="sidebar-item-icon" item-icon=${item.icon || "folder-normal"}>${criscostack.utils.icon(
	...
	make_sidebar() {
		...
		this.sidebar.find(".selected").length &&
			!criscostack.dom.is_element_in_viewport(this.sidebar.find(".selected")) &&
	...
	build_sidebar_section(title, root_pages) {
		...
		let $title = $(`<div class="standard-sidebar-label">
			<span>${criscostack.utils.icon("small-down", "xs")}</span>
	...
	append_item(item, container) {
		let is_current_page =
			criscostack.router.slug(item.title) == criscostack.router.slug(this.get_page_to_show().name) &&
	...
	add_drop_icon(item, sidebar_control, item_container) {
		...
		let $drop_icon = $(
			`<span class="drop-icon hidden">${criscostack.utils.icon(drop_icon, "sm")}</span>`
	...
	show() {
		...
		if (!criscostack.router.current_route[0]) {
			criscostack.route_flags.replace_route = true;
			criscostack.set_route(criscostack.router.slug(page.public ? page.name : "private/" + page.name));
			return;
		}
	...
	get_data(page) {
		return criscostack
			.call("criscostack.desk.desktop.get_desktop_page", {
				page: page,
			})
			.then((data) => {
				...
				return criscostack.dashboard_utils.get_dashboard_settings().then((settings) => {
					if (settings) {
	...
	async show_page(page) {
		...
		if (this.all_pages.length) {
			...
			if (this.pages && this.pages[current_page.name]) {
				...
				await criscostack.after_ajax(() => this.get_data(current_page));
			}
	...
	setup_customization_buttons(page) {
		...
		this.page.set_secondary_action(__("Discard"), async () => {
			...
			this.reload();
			criscostack.show_alert({ message: __("Customizations Discarded"), indicator: "info" });
		});

		if (page.name && this.has_access) {
			this.page.add_inner_button(__("Settings"), () => {
				criscostack.set_route(`workspace/${page.name}`);
			});
		}
	}
	...
	add_sidebar_actions(item, sidebar_control, is_new) {
		if (!item.is_editable) {
			...
			criscostack.utils.add_custom_button(
				criscostack.utils.icon("duplicate", "sm"),
			...
			criscostack.utils.add_custom_button(
				criscostack.utils.icon("unhide", "sm"),
			...
			criscostack.utils.add_custom_button(
				criscostack.utils.icon("drag", "xs"),
				null,
				"drag-handle",
				__("Drag"),
				null,
	...
	edit_page(item) {
		...
		const d = new criscostack.ui.Dialog({
			...
			primary_action: (values) => {
				values.title = criscostack.utils.escape_html(values.title);
				...
				criscostack.call({
					method: "criscostack.desk.doctype.workspace.workspace.update_page",
					args: {
						name: old_item.name,
						title: values.title,
						icon: values.icon || "",
					...
					callback: function (res) {
						if (res.message) {
							...
							criscostack.show_alert({ message: message, indicator: "green" });
						}
					},
				});
				...
				if (this.make_page_selected) {
					...
					let route = pre_url + criscostack.router.slug(values.title);
	...
	update_sidebar(old_item, new_item) {
		...
		if (is_title_changed || is_section_changed) {
			if (new_item.is_public) {
				...
				let user = criscostack.session.user;
	...
	update_cached_values(old_item, new_item, duplicate, new_page) {
		...
		// update criscostack.workspaces
		if (criscostack.workspaces[criscostack.router.slug(old_item.name)] || new_page) {
			!duplicate && delete criscostack.workspaces[criscostack.router.slug(old_item.name)];
			if (new_item) {
				criscostack.workspaces[criscostack.router.slug(new_item.name)] = { title: new_item.title };
			}
		}
	...
	add_settings_button(item, sidebar_control) {
		this.dropdown_list = [
			{
				...
				icon: criscostack.utils.icon("edit", "sm"),
				action: () => this.edit_page(item),
			},
			{
				label: __("Duplicate"),
				title: __("Duplicate Workspace"),
				icon: criscostack.utils.icon("duplicate", "sm"),
				action: () => this.duplicate_page(item),
			},
			{
				label: __("Hide"),
				title: __("Hide Workspace"),
				icon: criscostack.utils.icon("hide", "sm"),
		...
		if (this.is_item_deletable(item)) {
			this.dropdown_list.push({
				...
				icon: criscostack.utils.icon("delete-active", "sm"),
		...
		let $button = $(`
			<div class="btn btn-secondary btn-xs setting-btn dropdown-btn" title="${__("Setting")}">
				${criscostack.utils.icon("dot-horizontal", "xs")}
			</div>
	...
	is_item_deletable(item) {
		...
		if (
			!item.public ||
			(item.public && (!item.module || (item.module && criscostack.boot.developer_mode)))
		)
			return true;
		return false;
	}
	...
	delete_page(page) {
		criscostack.confirm(
			...
			() => {
				criscostack.call({
					method: "criscostack.desk.doctype.workspace.workspace.delete_page",
					args: { page: page },
					callback: function (res) {
						if (res.message) {
	...
	duplicate_page(page) {
		...
		const d = new criscostack.ui.Dialog({
			...
			primary_action: (values) => {
				...
				d.hide();
				criscostack.call({
					method: "criscostack.desk.doctype.workspace.workspace.duplicate_page",
					args: {
						page_name: page.name,
						new_page: values,
					},
				...
				new_page.name = values.title + (new_page.public ? "" : "-" + criscostack.session.user);
				new_page.label = new_page.name;
				new_page.icon = values.icon;
				new_page.parent_page = values.parent || "";
				new_page.for_user = new_page.public ? "" : criscostack.session.user;
	...
	hide_unhide_workspace(page, event, hide) {
		...
		criscostack.call({
			method: "criscostack.desk.doctype.workspace.workspace." + method,
			args: {
				page_name: page.name,
			},
			callback: (r) => {
	...
	update_sorted_sidebar() {
		if (this.sorted_public_items || this.sorted_private_items) {
			criscostack.call({
				method: "criscostack.desk.doctype.workspace.workspace.sort_pages",
				args: {
				...
				callback: function (res) {
					if (res.message) {
						...
						criscostack.show_alert({ message: __(message), indicator: "green" });
					}
				},
			});
		}
	...
	initialize_new_page() {
		...
		const d = new criscostack.ui.Dialog({
			...
			primary_action: (values) => {
				...
				let name = values.title + (values.is_public ? "" : "-" + criscostack.session.user);
				let blocks = [
					{
						type: "header",
				...
				let new_page = {
					...
					title: values.title,
					public: values.is_public || 0,
					for_user: values.is_public ? "" : criscostack.session.user,
					icon: values.icon,
				...
				this.editor
					...
					.then(async () => {
						...
						criscostack.call({
							method: "criscostack.desk.doctype.workspace.workspace.new_page",
							args: {
								new_page: new_page,
							},
							callback: function (res) {
						...
						let route = pre_url + criscostack.router.slug(new_page.title);
						criscostack.set_route(route);
	...
	validate_page(new_page, old_page) {
		...
		if (criscostack.router.doctype_route_exist(criscostack.router.slug(new_page.title))) {
	...
	initialize_editorjs(blocks) {
		this.tools = {
			...
			HeaderSize: criscostack.workspace_block.tunes["header_size"],
		};
	...
	save_page(page) {
		...
		return this.editor
			...
			.then((outputData) => {
				...
				page.content = JSON.stringify(blocks);
				criscostack.call({
					method: "criscostack.desk.doctype.workspace.workspace.save_page",
					args: {
						title: page.title,
	...
	create_page_skeleton() {
		...
		this.body.prepend(criscostack.render_template("workspace_loading_skeleton"));
	...
	create_sidebar_skeleton() {
		...
		$(criscostack.render_template("workspace_sidebar_loading_skeleton")).insertBefore(this.sidebar);
	...
	register_awesomebar_shortcut() {
		"abcdefghijklmnopqrstuvwxyz".split("").forEach((letter) => {
			...
			criscostack.ui.keys.add_shortcut({ shortcut: letter, ...default_shortcut });
			criscostack.ui.keys.add_shortcut({ shortcut: `shift+${letter}`, ...default_shortcut });
		});
	}
};
