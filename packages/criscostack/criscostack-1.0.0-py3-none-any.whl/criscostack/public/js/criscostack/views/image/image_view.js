/**
 * criscostack.views.ImageView
 */
criscostack.provide("criscostack.views");

criscostack.views.ImageView = class ImageView extends criscostack.views.ListView {
	get view_name() {
		return "Image";
	}
	...
	item_html(item) {
		...
		const escaped_title = criscostack.utils.escape_html(title);
		...
		const expand_button_html = item._image_url
			? `<div class="zoom-view" data-name="${encoded_name}">
				${criscostack.utils.icon("expand", "xs")}
			</div>`
			: "";
	...
	get_attached_images() {
		return criscostack
			.call({
				method: "criscostack.core.api.file.get_attached_images",
				args: {
	...
	setup_gallery() {
		var me = this;
		this.gallery = new criscostack.views.GalleryView({
			doctype: this.doctype,
			items: this.items,
			wrapper: this.$result,
...
criscostack.views.GalleryView = class GalleryView {
	constructor(opts) {
		$.extend(this, opts);
		var me = this;
	...
	prepare() {
		...
		if (this.pswp_root.length === 0) {
			var pswp = criscostack.render_template("photoswipe_dom");
	...
	load_lib() {
		return new Promise((resolve) => {
			var asset_dir = "assets/criscostack/js/lib/photoswipe/";
			criscostack.require(
				[
