...
criscostack.views.FileView = class FileView extends criscostack.views.ListView {
	static load_last_view() {
		const route = criscostack.get_route();
		if (route.length === 2) {
			const view_user_settings = criscostack.get_user_settings("File", "File");
			criscostack.set_route(
				"List",
				"File",
				view_user_settings.last_folder || criscostack.boot.home_folder
			);
			return true;
		}
	...
	setup_view() {
		...
		this.page.add_button(__("Toggle Grid View"), () => {
			criscostack.views.FileView.grid_view = !criscostack.views.FileView.grid_view;
			this.refresh();
		});
	}
	...
	get_args() {
		...
		if (criscostack.views.FileView.grid_view) {
			Object.assign(args, {
		...
		const items = [
			{
				label: __("Home"),
				action: () => {
					criscostack.set_route("List", "File", "Home");
				},
			},
			{
				label: __("New Folder"),
					...
					criscostack.prompt(
						...
						(values) => {
							if (values.value.indexOf("/") > -1) {
								criscostack.throw(__("Folder name should not include '/' (slash)"));
							}
							const data = {
							...
							criscostack.call({
								method: "criscostack.core.api.file.create_new_folder",
								args: data,
							});
						},
			...
			},
			{
				...
				action: () => {
					new criscostack.ui.FileUploader({
						...
						on_success: (file) => {
							criscostack.show_alert(__("Unzipping files..."));
							criscostack
								.call("criscostack.core.api.file.unzip_file", {
									name: file.name,
								})
								.then((r) => {
									if (r.message) {
										criscostack.show_alert(__("Unzipped {0} files", [r.message]));
									}
								});
						},
					});
	...
	add_file_action_buttons() {
		this.$cut_button = this.page
			.add_button(__("Cut"), () => {
				criscostack.file_manager.cut(this.get_checked_items(), this.current_folder);
				this.$checks.parents(".file-wrapper").addClass("cut");
			})
			.hide();

		this.$paste_btn = this.page
			.add_button(__("Paste"), () => criscostack.file_manager.paste(this.current_folder))
			.hide();

		this.page.add_actions_menu_item(__("Export as zip"), () => {
			let docnames = this.get_checked_items(true);
			if (docnames.length) {
				open_url_post("/api/method/criscostack.core.api.file.zip_files", {
	...
	set_fields() {
		this.fields = this.meta.fields
			.filter((df) => criscostack.model.is_value_type(df.fieldtype) && !df.hidden)
			.map((df) => df.fieldname)
	...
	prepare_datum(d) {
		...
		if (d.is_folder) {
			...
			type = "folder";
		} else if (criscostack.utils.is_image_file(d.file_name)) {
			icon_class = "image";
			type = "image";
		} else {
			icon_class = "file";
		...
		d.subject_html = `
			${criscostack.utils.icon(icon_class)}
			<span>${title}</span>
	...
	before_render() {
		super.before_render();
		criscostack.model.user_settings.save("File", "grid_view", criscostack.views.FileView.grid_view);
	...
	get_header_html() {
		...
		let header_selector_html = !criscostack.views.FileView.grid_view
			? `<input class="level-item list-check-all hidden-xs" type="checkbox" title="${__(
					"Select All"
			  )}">`
			: "";

		let header_columns_html = !criscostack.views.FileView.grid_view
	...
	get_creation_date(file) {
		...
		let created_on;
		if (date === criscostack.datetime.now_date()) {
			created_on = comment_when(file.creation);
		} else {
			created_on = criscostack.datetime.str_to_user(date);
		}
		return created_on;
	}
	...
	get_left_html(file) {
		...
		const file_size = file.file_size ? criscostack.form.formatters.FileSize(file.file_size) : "";
		...
		this.$result.on("drop", (e) => {
			...
			if (dataTransfer.files && dataTransfer.files.length > 0) {
				...
				if ($el.parents(".folders").length !== 0) {
					...
					criscostack.file_manager.paste(folder_name);
					criscostack.show_alert(`File ${file_name} moved to ${folder_name}`);
				}
			}
		});
	}
	...
	toggle_cut_paste_buttons() {
		const hide_paste_btn =
			!criscostack.file_manager.can_paste ||
			criscostack.file_manager.old_folder === this.current_folder;
...
criscostack.views.FileView.grid_view = criscostack.get_user_settings("File").grid_view || false;
