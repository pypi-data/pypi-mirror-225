...
import DataTable from "criscostack-datatable";

// Expose DataTable globally to allow customizations.
window.DataTable = DataTable;

criscostack.provide("criscostack.widget.utils");
criscostack.provide("criscostack.views");
criscostack.provide("criscostack.query_reports");

criscostack.standard_pages["query-report"] = function () {
	var wrapper = criscostack.container.add_page("query-report");
	...
	criscostack.query_report = new criscostack.views.QueryReport({
		parent: wrapper,
	});

	$(wrapper).bind("show", function () {
		criscostack.query_report.show();
	});
};

criscostack.views.QueryReport = class QueryReport extends criscostack.views.BaseList {
	show() {
	...
	init() {
		...
		this.init_promise = criscostack.run_serially(tasks);
		return this.init_promise;
	}
	...
	setup_defaults() {
		...
		this.refresh = criscostack.utils.throttle(this.refresh, 300);

		this.menu_items = [];
	}

	update_url_with_filters() {
		if (criscostack.get_route_str() == this.page_name) {
	...
	get_no_result_message() {
		return `<div class="msg-box no-border">
			<div>
				<img src="/assets/criscostack/images/ui-states/list-empty-state.svg" alt="Generic Empty State" class="null-state">
			</div>
	...
	setup_events() {
		criscostack.realtime.on("report_generated", (data) => {
			...
			if (data.report_name) {
				...
				if (data.name == this.prepared_report_doc_name) {
					...
					criscostack.show_alert({ message: alert_message, indicator: "orange" });
				}
			}
		});
	...
	load() {
		if (criscostack.get_route().length < 2) {
			this.toggle_nothing_to_show(true);
			return;
		}

		let route_options = {};
		route_options = Object.assign(route_options, criscostack.route_options);

		if (this.report_name !== criscostack.get_route()[1]) {
			// different report
			this.load_report(route_options);
		} else if (criscostack.has_route_options()) {
	...
	add_card_to_dashboard() {
		let field_options = criscostack.report_utils.get_field_options_from_report(
			this.columns,
			this.raw_data
		);
		const dashboard_field = criscostack.dashboard_utils.get_dashboard_link_field();
		const set_standard = criscostack.boot.developer_mode;

		const dialog = new criscostack.ui.Dialog({
	...
	add_chart_to_dashboard() {
		if (this.chart_fields || this.chart_options) {
			...
			criscostack.msgprint(__("Please Set Chart"));
		}
	}
	...
	create_number_card(values, dashboard_name, card_name, set_standard) {
		...
		this.add_to_dashboard(
			"criscostack.desk.doctype.number_card.number_card.create_report_number_card",
			args,
			dashboard_name,
			card_name,
			"Number Card"
	...
	create_dashboard_chart(chart_args, dashboard_name, chart_name, set_standard) {
		let args = {
			...
			chart_type: "Report",
			report_name: this.report_name,
			type: chart_args.chart_type || criscostack.model.unscrub(chart_args.type),
		...
		this.add_to_dashboard(
			"criscostack.desk.doctype.dashboard_chart.dashboard_chart.create_report_chart",
			args,
			dashboard_name,
			chart_name,
			"Dashboard Chart"
	...
	add_to_dashboard(method, args, dashboard_name, name, doctype) {
		criscostack.xcall(method, { args: args }).then(() => {
			let message;
			if (dashboard_name) {
			...
			criscostack.msgprint(message, __("New {0} Created", [__(doctype)]));
		});
	}
	...
	refresh_report(route_options) {
		...
		return criscostack.run_serially([
			() => this.setup_filters(),
	...
	get_report_doc() {
		return criscostack.model
			.with_doc("Report", this.report_name)
			.then((doc) => {
				this.report_doc = doc;
			})
			.then(() => criscostack.model.with_doctype(this.report_doc.ref_doctype));
	}

	get_report_settings() {
		return new Promise((resolve, reject) => {
			if (criscostack.query_reports[this.report_name]) {
				this.report_settings = criscostack.query_reports[this.report_name];
				resolve();
			} else {
				criscostack
					.xcall("criscostack.desk.query_report.get_script", {
						report_name: this.report_name,
					})
					.then((settings) => {
						criscostack.dom.eval(settings.script || "");
						criscostack.after_ajax(() => {
							...
							criscostack.query_reports[this.report_name] = this.report_settings;
	...
	get_local_report_settings() {
		let report_script_name =
			this.report_doc.report_type === "Custom Report"
				...
				: this.report_name;
		return criscostack.query_reports[report_script_name] || {};
	}
	...
	setup_progress_bar() {
		...
		this.interval = setInterval(function () {
			seconds_elapsed += 1;
			criscostack.show_progress(__("Preparing Report"), seconds_elapsed, execution_time);
		}, 1000);
	}
	...
	evaluate_depends_on_value(expression, filter_label) {
		...
		if (doc) {
			if (typeof expression === "boolean") {
				...
				try {
					out = criscostack.utils.eval(expression.substr(5), { doc });
				} catch (e) {
					criscostack.throw(
	...
	set_route_filters(route_options) {
		if (!route_options) route_options = criscostack.route_options;
		...
		if (route_options) {
			...
			promises.push(() => {
				criscostack.route_options = null;
			});

			return criscostack.run_serially(promises);
		}
	}
	...
	refresh(have_filters_changed) {
		...
		return new Promise((resolve) => {
			this.last_ajax = criscostack.call({
				method: "criscostack.desk.query_report.run",
				type: "GET",
				args: {
					report_name: this.report_name,
					filters: filters,
			...
			.then((r) => {
				...
				if (data.result && data.result.length) {
					...
					if (this.chart_options) {
						...
						this.$chart.empty();
						if (this.chart_fields) {
							this.chart_options = criscostack.report_utils.make_chart_options(
								this.columns,
								this.raw_data,
								this.chart_fields
							);
	...
	render_summary(data) {
		data.forEach((summary) => {
			criscostack.utils.build_summary_item(summary).appendTo(this.$summary);
		});
	...
	get_query_params() {
		const query_string = criscostack.utils.get_query_string(criscostack.get_route_str());
		const query_params = criscostack.utils.get_query_params(query_string);
		return query_params;
	}
	...
	add_prepared_report_buttons(doc) {
		if (doc) {
			this.page.add_inner_button(__("Download Report"), function () {
				window.open(
					criscostack.urllib.get_full_url(
						"/api/method/criscostack.core.doctype.prepared_report.prepared_report.download_attachment?" +
							"dn=" +
			...
			let pretty_diff = criscostack.datetime.comment_when(doc.report_end_time);
			const days_old = criscostack.datetime.get_day_diff(
				criscostack.datetime.now_datetime(),
				doc.report_end_time
			);
			if (days_old > 1) {
		...
		this.primary_action_map = {
			...
			Edit: {
				label: __("Edit"),
				click: () => {
					criscostack.set_route(criscostack.get_route());
				},
			},
			Rebuild: {
				label: __("Rebuild"),
	...
	show_warning_or_generate_report() {
		criscostack
			.xcall(
				"criscostack.core.doctype.prepared_report.prepared_report.get_reports_in_queued_state",
				{
			...
			.then((reports) => {
				...
				if (reports.length) {
					...
					this.prepared_report_dialog = criscostack.warn(
					...
					criscostack.utils.bind_actions_with_object(
	...
	delete_old_queued_reports() {
		...
		criscostack
			.xcall("criscostack.core.doctype.prepared_report.prepared_report.delete_prepared_reports", {
	...
	generate_background_report() {
		...
		if (!missing_mandatory.length) {
			...
			return new Promise((resolve) =>
				criscostack.call({
					method: "criscostack.core.doctype.prepared_report.prepared_report.make_prepared_report",
					args: {
						report_name: this.report_name,
						filters: filters,
					},
				...
				let alert_message =
					...
					`</a>`;
				criscostack.show_alert({ message: alert_message, indicator: "orange" }, 10);
				this.toggle_nothing_to_show(true);
			});
		}
	}
	...
	render_datatable() {
		...
		if (
			...
			let datatable_options = {
				...
				data: data,
				inlineFilters: true,
				language: criscostack.boot.lang,
				translations: criscostack.utils.datatable.get_translations(),
				treeView: this.tree_report,
				layout: "fixed",
				cellHeight: 33,
				showTotalRow: this.raw_data.add_total_row && !this.report_settings.tree,
				direction: criscostack.utils.is_rtl() ? "rtl" : "ltr",
				hooks: {
					columnTotal: criscostack.utils.report_column_total,
				},
			};
	...
	get_chart_options(data) {
		...
		options.axisOptions = {
			...
			numberFormatter: criscostack.utils.format_chart_axis_number,
		};
		options.height = 280;
		return options;
	}
	...
	render_chart(options) {
		this.$chart.empty();
		this.$chart.show();
		this.chart = new criscostack.Chart(this.$chart[0], options);
	}
	...
	open_create_chart_dialog() {
		...
		function preview_chart() {
			...
			if (values.x_field && values.y_fields.length) {
				let options = criscostack.report_utils.make_chart_options(
					me.columns,
					me.raw_data,
					values
				);
				...
				wrapper.empty();
				new criscostack.Chart(wrapper[0], options);
	...
	prepare_columns(columns) {
		return columns.map((column) => {
			column = criscostack.report_utils.prepare_field_from_column(column);
			...
			if (column.fieldtype === "Date") {
				compareFn = (cell, keyword) => {
					...
					const keywordValue = criscostack.datetime.user_to_obj(keyword);
					const cellValue = criscostack.datetime.str_to_obj(cell.content);
	...
	set_breadcrumbs() {
		...
		const ref_doctype = criscostack.get_meta(this.report_doc.ref_doctype);
		criscostack.breadcrumbs.add(ref_doctype.module);
	}

	make_access_log(method, file_format) {
		criscostack.call("criscostack.core.doctype.access_log.access_log.make_access_log", {
	...
	pdf_report(print_settings) {
		const base_url = criscostack.urllib.get_base_url();
		const print_css = criscostack.boot.print_css;
		const landscape = print_settings.orientation == "Landscape";
		...
		const content = criscostack.render_template(template, {
			title: __(this.report_name),
			subtitle: filters_html,
		...
		const html = criscostack.render_template("print_template", {
			title: __(this.report_name),
			content: content,
			base_url: base_url,
			print_css: print_css,
			...
			landscape: landscape,
			columns: columns,
			lang: criscostack.boot.lang,
			layout_direction: criscostack.utils.is_rtl() ? "rtl" : "ltr",
			can_use_smaller_font: this.report_doc.is_standard === "Yes" && custom_format ? 0 : 1,
		});
		...
		criscostack.render_pdf(html, print_settings);
	}
	...
	get_filters_html_for_print() {
		...
		return Object.keys(applied_filters)
			.map((fieldname) => {
				const docfield = criscostack.query_report.get_filter(fieldname).df;
				const value = applied_filters[fieldname];
				return `<h6>${__(docfield.label)}: ${criscostack.format(value, docfield)}</h6>`;
			})
			.join("");
	}
	...
	export_report() {
		...
		this.export_dialog = criscostack.report_utils.get_export_dialog(
			__(this.report_name),
			extra_fields,
			({ file_format, include_indentation, csv_delimiter, csv_quoting }) => {
				...
				let filters = this.get_filter_values(true);
				if (criscostack.urllib.get_dict("prepared_report_name")) {
					filters = Object.assign(
						criscostack.urllib.get_dict("prepared_report_name"),
						filters
					);
				}
				...
				const args = {
					cmd: "criscostack.desk.query_report.export_query",
					report_name: this.report_name,
				...
				open_url_post(criscostack.request.url, args);
		...
		return rows.map((row) => {
			...
			return row.slice(standard_column_count).map((cell, i) => {
				if (cell.column.fieldtype === "Duration") {
					cell.content = criscostack.utils.get_formatted_duration(cell.content);
				}
	...
	get_menu_items() {
		let items = [
			...
			{
				label: __("Edit"),
				action: () => criscostack.set_route("Form", "Report", this.report_name),
				condition: () => criscostack.user.is_report_manager(),
				standard: true,
			},
			{
				label: __("Print"),
				action: () => {
					let dialog = criscostack.ui.get_print_settings(
						false,
				...
				condition: () => criscostack.model.can_print(this.report_doc.ref_doctype),
				standard: true,
			},
			{
				label: __("PDF"),
			...
			},
			{
				...
				condition: () => criscostack.model.can_export(this.report_doc.ref_doctype),
				standard: true,
			},
			{
				label: __("Setup Auto Email"),
				action: () =>
					criscostack.set_route("List", "Auto Email Report", { report: this.report_name }),
				standard: true,
			},
			{
				label: __("Add Column"),
				action: () => {
					let d = new criscostack.ui.Dialog({
						...
						fields: [
							{
								...
								change: () => {
									...
									criscostack.model.with_doctype(doctype, () => {
										let options = criscostack.meta
											.get_docfields(doctype)
											.filter(criscostack.model.is_value_type)
											.map((df) => ({
												label: df.label,
												value: df.fieldname,
											}));
						...
						primary_action: (values) => {
							...
							let df = criscostack.meta.get_docfield(values.doctype, values.field);
							...
							criscostack.call({
								method: "criscostack.desk.query_report.get_data_for_custom_field",
								args: {
									field: values.field,
									doctype: values.doctype,
								},
			...
			},
			{
				...
				condition: () => criscostack.user.has_role("System Manager"),
				standard: true,
			},
		];

		if (criscostack.user.is_report_manager()) {
			items.push({
				label: __("Save"),
				action: () => {
					let d = new criscostack.ui.Dialog({
						...
						primary_action: (values) => {
							criscostack.call({
								method: "criscostack.desk.query_report.save_report",
								args: {
								...
								callback: function (r) {
									this.show_save = false;
									d.hide();
									criscostack.set_route("query-report", r.message);
								},
							});
						},
					});
