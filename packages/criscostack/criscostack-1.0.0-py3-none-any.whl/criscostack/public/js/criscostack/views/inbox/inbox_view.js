/**
 * criscostack.views.InboxView
 */

criscostack.provide("criscostack.views");

criscostack.views.InboxView = class InboxView extends criscostack.views.ListView {
	static load_last_view() {
		const route = criscostack.get_route();
		if (!route[3] && criscostack.boot.email_accounts.length) {
			let email_account;
			if (criscostack.boot.email_accounts[0].email_id == "All Accounts") {
				email_account = "All Accounts";
			} else {
				email_account = criscostack.boot.email_accounts[0].email_account;
			}
			criscostack.set_route("List", "Communication", "Inbox", email_account);
			return true;
		...
		function is_valid(email_account) {
			return criscostack.boot.email_accounts.find((d) => d.email_account === email_account);
		}
	}
	...
	setup_defaults() {
		...
		this.email_account = criscostack.get_route()[3];
	...
	get_seen_class(doc) {
		const seen =
			Boolean(doc.seen) || JSON.parse(doc._seen || "[]").includes(criscostack.session.user)
				? ""
				: "bold";
		return seen;
	}
	...
	get_meta_html(email) {
		...
		if (email.reference_doctype && email.reference_doctype !== this.doctype) {
			link = `<a class="text-muted grey"
				href="${criscostack.utils.get_form_link(email.reference_doctype, email.reference_name)}"
	...
	get_inbox_filters() {
		...
		if (email_account === "Sent") {
			...
			filters = default_filters.concat([
				...
				["Communication", "email_account", "in", criscostack.boot.all_accounts, true],
			]);
		} else {
			var op = "=";
			if (email_account == "All Accounts") {
				op = "in";
				email_account = criscostack.boot.all_accounts;
			}
	...
	get_no_result_message() {
		...
		} else if (!email_account && !criscostack.boot.email_accounts.length) {
		...
		const html = criscostack.model.can_create(args.doctype)
			? `<p>${args.msg}</p>
			<p>
	...
	make_new_doc() {
		if (!this.email_account && !criscostack.boot.email_accounts.length) {
			criscostack.route_options = {
				email_id: criscostack.session.user_email,
			};
			criscostack.new_doc("Email Account");
		} else {
