...
criscostack.provide("criscostack.treeview_settings");
criscostack.provide("criscostack.views.trees");
window.cur_tree = null;

criscostack.views.TreeFactory = class TreeFactory extends criscostack.views.Factory {
	make(route) {
		criscostack.model.with_doctype(route[1], function () {
			var options = {
				doctype: route[1],
				meta: criscostack.get_meta(route[1]),
			};

			if (
				!criscostack.treeview_settings[route[1]] &&
				!criscostack.meta.get_docfield(route[1], "is_group")
			) {
				criscostack.msgprint(__("Tree view is not available for {0}", [route[1]]));
				return false;
			}
			$.extend(options, criscostack.treeview_settings[route[1]] || {});
			criscostack.views.trees[options.doctype] = new criscostack.views.TreeView(options);
		});
	}
	...
	on_show() {
		 ...
		 * treeview is visible.
		 */
		let route = criscostack.get_route();
		let treeview = criscostack.views.trees[route[1]];
...
criscostack.views.TreeView = class TreeView {
	constructor(opts) {
		var me = this;
		...
		this.get_tree_nodes = me.opts.get_tree_nodes || "criscostack.desk.treeview.get_children";
	...
	get_permissions() {
		this.can_read = criscostack.model.can_read(this.doctype);
		this.can_create =
			criscostack.boot.user.can_create.indexOf(this.doctype) !== -1 ||
			criscostack.boot.user.in_create.indexOf(this.doctype) !== -1;
		this.can_write = criscostack.model.can_write(this.doctype);
		this.can_delete = criscostack.model.can_delete(this.doctype);
	}
	make_page() {
		var me = this;
		this.parent = criscostack.container.add_page(this.page_name);
		criscostack.ui.make_app_page({ parent: this.parent, single_column: true });
		...
		this.page.main.addClass("criscostack-card");
	...
	make_filters() {
		var me = this;
		criscostack.treeview_settings.filters = [];
		$.each(this.opts.filters || [], function (i, filter) {
			if (criscostack.route_options && criscostack.route_options[filter.fieldname]) {
				filter.default = criscostack.route_options[filter.fieldname];
			}
	...
	make_tree() {
		...
		this.tree = new criscostack.ui.Tree({
			parent: this.body,
			label: use_label,
	...
	rebuild_tree() {
		...
		criscostack.call({
			method: "criscostack.utils.nestedset.rebuild_tree",
			args: {
				doctype: me.doctype,
	...
	select_node(node) {
		...
		if (this.opts.view_template) {
			...
			$(
				criscostack.render_template(me.opts.view_template, {
					data: node.data,
					doctype: me.doctype,
				})
		...
		var toolbar = [
			{
				...
				click: function (node) {
					criscostack.set_route("Form", me.doctype, node.label);
				},
			},
			{
				label: __("Add Child"),
			...
			},
			{
				...
				condition: function (node) {
					...
					if (me.doctype && criscostack.get_meta(me.doctype)) {
						if (!criscostack.get_meta(me.doctype).allow_rename) allow_rename = false;
					}
					return !node.is_root && me.can_write && allow_rename;
				},
				click: function (node) {
					criscostack.model.rename_doc(me.doctype, node.label, function (new_name) {
			...
			},
			{
				...
				click: function (node) {
					criscostack.model.delete_doc(me.doctype, node.label, function () {
						node.parent.remove();
					});
				},
				btnClass: "hidden-xs",
	...
	new_node() {
		...
		if (!(node && node.expandable)) {
			criscostack.msgprint(__("Select a group node first."));
			return;
		}
		...
		d.set_primary_action(__("Create New"), function () {
			...
			d.hide();
			criscostack.dom.freeze(__("Creating {0}", [me.doctype]));

			$.extend(args, v);
			return criscostack.call({
				method: me.opts.add_tree_node || "criscostack.desk.treeview.add_node",
				args: args,
				callback: function (r) {
					if (!r.exc) {
	...
	print_tree() {
		if (!criscostack.model.can_print(this.doctype)) {
			criscostack.msgprint(__("You are not allowed to print this report"));
			return false;
		}
		var tree = $(".tree:visible").html();
		var me = this;
		criscostack.ui.get_print_settings(false, function (print_settings) {
			var title = __(me.docname || me.doctype);
			criscostack.render_tree({ title: title, tree: tree, print_settings: print_settings });
			criscostack.call({
				method: "criscostack.core.doctype.access_log.access_log.make_access_log",
				args: {
					doctype: me.doctype,
	...
	set_menu_item() {
		...
		this.menu_items = [
			{
				label: __("View List"),
				action: function () {
					criscostack.set_route("List", me.doctype);
				},
			},
			{
				label: __("Print"),
		...
		if (
			...
			criscostack.meta.has_field(me.doctype, "lft") &&
			criscostack.meta.has_field(me.doctype, "rgt")
		) {
