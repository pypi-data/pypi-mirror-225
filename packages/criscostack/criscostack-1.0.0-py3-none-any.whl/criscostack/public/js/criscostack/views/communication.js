...
criscostack.last_edited_communication = {};
const separator_element = "<div>---</div>";

criscostack.views.CommunicationComposer = class {
	constructor(opts) {
		$.extend(this, opts);
		if (!this.doc) {
	...
	get_fields() {
		const fields = [
			...
			{
				fieldtype: "Button",
				label: criscostack.utils.icon("down", "xs"),
			...
			},
			{
				...
				fieldname: "content",
				onchange: criscostack.utils.debounce(this.save_as_draft.bind(this), 300),
			},
			{
				fieldtype: "Button",
			...
			{ fieldtype: "Section Break" },
			{
				...
				default: criscostack.boot.user.send_me_a_copy,
			},
			{
		...
		const email_accounts = criscostack.boot.email_accounts.filter((account) => {
			return (
	...
	toggle_more_options(show_options) {
		...
		const label = criscostack.utils.icon(show_options ? "up-line" : "down", "xs");
	...
	setup_multiselect_queries() {
		["recipients", "cc", "bcc"].forEach((field) => {
			this.dialog.fields_dict[field].get_data = () => {
				...
				criscostack.call({
					method: "criscostack.email.get_contact_list",
					args: { txt },
					callback: (r) => {
	...
	setup_email_template() {
		...
		function add_template() {
			...
			criscostack.call({
				method: "criscostack.email.doctype.email_template.email_template.get_email_template",
				args: {
		...
		criscostack.utils.add_select_group_button(clear_and_add_template, email_template_actions);
	}
	...
	get_last_edited_communication(clear) {
		if (!criscostack.last_edited_communication[this.doctype]) {
			criscostack.last_edited_communication[this.doctype] = {};
		}

		if (clear || !criscostack.last_edited_communication[this.doctype][this.key]) {
			criscostack.last_edited_communication[this.doctype][this.key] = {};
		}

		return criscostack.last_edited_communication[this.doctype][this.key];
	}
	...
	async set_values() {
		...
		const subject = this.subject ? criscostack.utils.html2text(this.subject) : "";
	...
	setup_print() {
		...
		if (this.frm) {
			const print_formats = criscostack.meta.get_print_formats(this.frm.meta.name);
	...
	setup_attach() {
		...
		$(`
			...
			<p class='add-more-attachments'>
				<button class='btn btn-xs btn-default'>
					${criscostack.utils.icon("small-add", "xs")}&nbsp;
					${__("Add Attachment")}
				</button>
			</p>
		...
		attach
			...
			.on("click", () => new criscostack.ui.FileUploader(args));
			...
			if (files.length) {
				$.each(files, (i, f) => {
					...
					if (!attachment_rows.find(`[data-file-name="${f.name}"]`).length) {
						f.file_url = criscostack.urllib.get_full_url(f.file_url);
	...
	get_attachment_row(attachment, checked) {
		return $(`<p class="checkbox flex">
			...
			<a href="${attachment.file_url}" target="_blank" class="btn-linkF">
				${criscostack.utils.icon("link-url")}
			</a>
		</p>`);
	}
	...
	setup_email() {
		...
		$(fields.send_me_a_copy.input).on("click", () => {
			...
			criscostack.db.set_value("User", criscostack.session.user, "send_me_a_copy", val);
			criscostack.boot.user.send_me_a_copy = val;
		});
	}
	...
	send_email(btn, form_values, selected_attachments, print_html, print_format) {
		...
		if (!form_values.recipients) {
			criscostack.msgprint(__("Enter Email Recipient(s)"));
			return;
		}
		...
		if (this.frm && !criscostack.model.can_email(this.doc.doctype, this.frm)) {
			criscostack.msgprint(__("You are not allowed to send emails related to this document"));
			return;
		}

		return criscostack.call({
			method: "criscostack.core.doctype.communication.email.make",
			args: {
				...
				sender_full_name: form_values.sender ? criscostack.user.full_name() : undefined,
			...
			callback(r) {
				if (!r.exc) {
					criscostack.utils.play_sound("email");

					if (r.message["emails_not_sent_to"]) {
						criscostack.msgprint(
							__("Email not sent to {0} (unsubscribed / disabled)", [
								criscostack.utils.escape_html(r.message["emails_not_sent_to"]),
							])
						);
					}
	...
	is_print_letterhead_checked() {
		if (this.frm && $(this.frm.wrapper).find(".form-print-wrapper").is(":visible")) {
			...
			return (
				criscostack.model.get_doc(":Print Settings", "Print Settings") || { with_letterhead: 1 }
			).with_letterhead
				? 1
				: 0;
		}
	...
	async get_signature(sender_email) {
		let signature = criscostack.boot.user.email_signature;
		...
		if (!signature) {
			...
			const email_accounts = await criscostack.db.get_list("Email Account", {
				filters: filters,
		...
		if (!criscostack.utils.is_html(signature)) {
	...
	get_earlier_reply() {
		...
		const communication_date = criscostack.datetime.global_date_format(
	...
	html2text(html) {
		...
		const text = criscostack.utils.html2text(html);
		return text.replace(/\n{3,}/g, "\n\n");
	}
};
