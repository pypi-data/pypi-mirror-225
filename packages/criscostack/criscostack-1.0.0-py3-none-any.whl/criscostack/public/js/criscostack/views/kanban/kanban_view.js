...
criscostack.views.KanbanView = class KanbanView extends criscostack.views.ListView {
	static load_last_view() {
		const route = criscostack.get_route();
		if (route.length === 3) {
			const doctype = route[1];
			const user_settings = criscostack.get_user_settings(doctype)["Kanban"] || {};
			if (!user_settings.last_kanban_board) {
				return new criscostack.views.KanbanView({ doctype: doctype });
			}
	...
	show() {
		criscostack.views.KanbanView.get_kanbans(this.doctype).then((kanbans) => {
			if (!kanbans.length) {
				return criscostack.views.KanbanView.show_kanban_dialog(this.doctype, true);
			} else if (kanbans.length && criscostack.get_route().length !== 4) {
				return criscostack.views.KanbanView.show_kanban_dialog(this.doctype, true);
			} else {
	...
	setup_defaults() {
		return super.setup_defaults().then(() => {
			...
			this.board_name = criscostack.get_route()[3] || get_board_name() || null;
	...
	set_board_perms_and_push_menu_items() {
		...
		return criscostack.call({
			method: "criscostack.client.get_doc_permissions",
			args: {
	...
	push_menu_items() {
		...
		if (this.board_perms.delete) {
			this.menu_items.push({
				...
				action: () => {
					criscostack.confirm(__("Are you sure you want to proceed?"), () => {
						criscostack.db.delete_doc("Kanban Board", this.board_name).then(() => {
							criscostack.show_alert(`Kanban Board ${this.board_name} deleted.`);
							criscostack.set_route("List", this.doctype, "List");
						});
					});
				},
			});
	...
	get_board() {
		return criscostack.db.get_doc("Kanban Board", this.board_name).then((board) => {
			this.board = board;
	...
	save_kanban_board_filters() {
		...
		criscostack.db.set_value("Kanban Board", this.board_name, "filters", filters).then((r) => {
			if (r.exc) {
	...
	render() {
		...
		if (!this.kanban) {
			this.kanban = new criscostack.views.KanbanBoard({
	...
	get_card_meta() {
		var meta = criscostack.get_meta(this.doctype);
		// preserve route options erased by new doc
		let route_options = { ...criscostack.route_options };
		var doc = criscostack.model.get_new_doc(this.doctype);
		criscostack.route_options = route_options;
		var title_field = null;
		var quick_entry = false;

		if (this.meta.title_field) {
			title_field = criscostack.meta.get_field(this.doctype, this.meta.title_field);
		}
		...
		if (
			mandatory.some((df) => criscostack.model.table_fields.includes(df.fieldtype)) ||
			mandatory.length > 1
		) {
			quick_entry = true;
		}
		...
		if (!title_field) {
			title_field = criscostack.meta.get_field(this.doctype, "name");
		}
...
criscostack.views.KanbanView.get_kanbans = function (doctype) {
	let kanbans = [];

	return get_kanban_boards().then((kanban_boards) => {
		if (kanban_boards) {
			kanban_boards.forEach((board) => {
				let route = `/app/${criscostack.router.slug(board.reference_doctype)}/view/kanban/${
					board.name
				}`;
	...
	function get_kanban_boards() {
		return criscostack
			.call("criscostack.desk.doctype.kanban_board.kanban_board.get_kanban_boards", { doctype })
			.then((r) => r.message);
	}
};

criscostack.views.KanbanView.show_kanban_dialog = function (doctype) {
	let dialog = new_kanban_dialog();
	dialog.show();

	function make_kanban_board(board_name, field_name, project) {
		return criscostack.call({
			method: "criscostack.desk.doctype.kanban_board.kanban_board.quick_kanban_board",
			args: {
				doctype,
				board_name,
				field_name,
			...
			callback: function (r) {
				var kb = r.message;
				if (kb.filters) {
					criscostack.provide("criscostack.kanban_filters");
					criscostack.kanban_filters[kb.kanban_board_name] = kb.filters;
				}
				criscostack.set_route("List", doctype, "Kanban", kb.kanban_board_name);
			},
		});
	}
	...
	function new_kanban_dialog() {
		...
		const select_fields = criscostack.get_meta(doctype).fields.filter((df) => {
		...
		let primary_action = () => {
			if (to_save) {
				...
				make_kanban_board(values.board_name, values.field_name, values.project).then(
					() => dialog.hide(),
					(err) => criscostack.msgprint(err)
				);
			} else {
				criscostack.set_route("Form", "Customize Form", { doc_type: doctype });
			}
		};

		return new criscostack.ui.Dialog({
			title: dialog_title,
