...
criscostack.views.ListView = class ListView extends criscostack.views.BaseList {
	static load_last_view() {
		const route = criscostack.get_route();
		const doctype = route[1];

		if (route.length === 2) {
			const user_settings = criscostack.get_user_settings(doctype);
			const last_view = user_settings.last_view;
			criscostack.set_route(
				"list",
				criscostack.router.doctype_layout || doctype,
				criscostack.views.is_valid(last_view) ? last_view.toLowerCase() : "list"
			);
			return true;
		}
		return false;
	...
	constructor(opts) {
		super(opts);
		this.show();
		this.debounced_refresh = criscostack.utils.debounce(
	...
	has_permissions() {
		const can_read = criscostack.perm.has_perm(this.doctype, 0, "read");
		return can_read;
	}
	...
	check_permissions() {
		if (!this.has_permissions()) {
			criscostack.set_route("");
			criscostack.throw(__("Not permitted to view {0}", [this.doctype]));
		}
	}
	...
	validate_filters(filters) {
		...
		valid_fields = valid_fields.concat(criscostack.model.std_fields_list);
	...
	show_restricted_list_indicator_if_applicable() {
		const match_rules_list = criscostack.perm.get_match_rules(this.doctype);
		if (match_rules_list.length) {
			this.restricted_list = $(
				`<button class="btn btn-xs restricted-button flex align-center">
					${criscostack.utils.icon("restriction", "xs")}
				</button>`
			)
	...
	show_restrictions(match_rules_list = []) {
		criscostack.msgprint(
			criscostack.render_template("list_view_permission_restrictions", {
	...
	async set_fields() {
		...
		await Promise.all(
			fields.map((f) => {
				return new Promise((resolve) => {
					const df =
						typeof f === "string" ? criscostack.meta.get_docfield(this.doctype, f) : f;
					if (
						df &&
						df.fieldtype == "Link" &&
						criscostack.boot.link_title_doctypes.includes(df.options)
					) {
						criscostack.model.with_doctype(df.options, () => {
							const meta = criscostack.get_meta(df.options);
		...
		this.fields.forEach((f) => {
			const df = criscostack.meta.get_docfield(f[1], f[0]);
	...
	patch_refresh_and_load_lib() {
		...
		this.refresh = criscostack.utils.throttle(this.refresh, 1000);
		this.load_lib = new Promise((resolve) => {
			if (this.required_libs) {
				criscostack.require(this.required_libs, resolve);
			} else {
				resolve();
			}
		});
	...
	set_primary_action() {
		if (this.can_create && !criscostack.boot.read_only) {
			const doctype_name = __(criscostack.router.doctype_layout) || __(this.doctype);
	...
	make_new_doc() {
		...
		this.filter_area.get().forEach((f) => {
			if (f[2] === "=" && criscostack.model.is_non_std_field(f[1])) {
				options[f[1]] = f[3];
			}
		});
		criscostack.new_doc(doctype, options);
	}
	...
	setup_columns() {
		...
		const get_df = criscostack.meta.get_docfield.bind(null, this.doctype);
		...
		if (criscostack.has_indicator(this.doctype)) {
			// indicator
			this.columns.push({
				type: "Status",
			});
		...
		this.columns = this.columns.concat(
			fields_in_list_view
				.filter((df) => {
					if (criscostack.has_indicator(this.doctype) && df.fieldname === "status") {
						return false;
					}
					if (!df.in_list_view) {
						return false;
	...
	get_no_result_message() {
		...
		let empty_state_image =
			...
			"/assets/criscostack/images/ui-states/list-empty-state.svg";
	...
	before_refresh() {
		if (criscostack.route_options && this.filter_area) {
	...
	before_render() {
		...
		criscostack.model.user_settings.save(this.doctype, "last_view", this.view_name);
	...
	get_header_html() {
		...
		let subject_html = `
			...
			<span class="level-item list-liked-by-me hidden-xs">
				<span title="${__("Likes")}">${criscostack.utils.icon("heart", "sm", "like-icon")}</span>
			</span>
		...
		const $columns = this.columns
			.map((col) => {
				let classes = [
					...
					criscostack.model.is_numeric_field(col.df) ? "text-right" : "",
				].join(" ");
	...
	get_column_html(col, doc) {
		...
		let translated_doctypes = criscostack.boot?.translated_doctypes || [];
		...
		const format = () => {
			if (df.fieldtype === "Code") {
				...
				return criscostack.format(value, df, null, doc);
			}
		};
			...
			if (strip_html_required) {
				...
				_value =
					typeof value_display === "string"
						? criscostack.utils.escape_html(value_display)
						: value_display;
			}
			...
			if (df.fieldtype === "Image") {
				html = df.options
					...
					: `<div class="missing-image small">
						${criscostack.utils.icon("restriction")}
					</div>`;
			} else if (df.fieldtype === "Select") {
				html = `<span class="filterable indicator-pill ${criscostack.utils.guess_colour(
					_value
				)} ellipsis"
				...
				html = `<a class="filterable ellipsis"
					data-filter="${fieldname},=,${criscostack.utils.escape_html(value)}">
					${format()}
				</a>`;
			}

			return `<span class="ellipsis"
				title="${__(label)}: ${criscostack.utils.escape_html(_value)}">
				${html}
			</span>`;
		};
		...
		const css_class = [
			"list-row-col ellipsis",
			class_map[col.type],
			criscostack.model.is_numeric_field(df) ? "text-right" : "",
		].join(" ");
	...
	get_tags_html(user_tags, limit, colored = false) {
		let get_tag_html = (tag) => {
			...
			if (tag) {
				if (colored) {
					color = criscostack.get_palette(tag);
	...
	get_meta_html(doc) {
		...
		if (assigned_users.length) {
			assigned_to = `<div class="list-assignments">
					${criscostack.avatar_group(assigned_users, 3, { filterable: true })[0].outerHTML}
				</div>`;
		}
		...
		if (this.list_view_settings && !this.list_view_settings.disable_comment_count) {
			...
			$(comment_count).append(`
				${criscostack.utils.icon("small-message")}
	...
	get_form_link(doc) {
		...
		return `/app/${criscostack.router.slug(
			criscostack.router.doctype_layout || this.doctype
		)}/${encodeURIComponent(cstr(doc.name))}`;
	}

	get_seen_class(doc) {
		return JSON.parse(doc._seen || "[]").includes(criscostack.session.user) ? "" : "bold";
	}

	get_like_html(doc) {
		const liked_by = JSON.parse(doc._liked_by || "[]");
		let heart_class = liked_by.includes(criscostack.session.user) ? "liked-by liked" : "not-liked";

		return `<span
			class="like-action ${heart_class}"
			data-name="${doc.name}" data-doctype="${this.doctype}"
			data-liked-by="${encodeURI(doc._liked_by) || "[]"}"
			title="${liked_by.map((u) => criscostack.user_info(u).fullname).join(", ")}">
			${criscostack.utils.icon("heart", "sm", "like-icon")}
		</span>
	...
	get_subject_html(doc) {
		...
		let escaped_subject = criscostack.utils.escape_html(subject);
	...
	get_indicator_html(doc, show_workflow_state) {
		const indicator = criscostack.get_indicator(doc, this.doctype, show_workflow_state);
	...
	get_indicator_dot(doc) {
		const indicator = criscostack.get_indicator(doc, this.doctype);
		if (!indicator) return "";
	...
	get_image_url(doc) {
		...
		if (window.cordova && !criscostack.utils.is_url(url)) {
			url = criscostack.base_url + url;
		}
		return url || null;
	}
	...
	setup_filterable() {
		...
		this.$result.on("click", ".filterable", (e) => {
			...
			const filters_to_apply = filters.map((f) => {
				f = f.split(",");
				if (f[2] === "Today") {
					f[2] = criscostack.datetime.get_today();
				} else if (f[2] == "User") {
					f[2] = criscostack.session.user;
				}
	...
	setup_list_click() {
		this.$result.on("click", ".list-row, .image-view-header, .file-header", (e) => {
			...
			if (link) {
				criscostack.set_route(link.pathname);
				return false;
			}
		});
	}
	...
	setup_like() {
		this.$result.on("click", ".like-action", criscostack.ui.click_toggle_like);
	...
	setup_realtime_updates() {
		...
		criscostack.realtime.doctype_subscribe(this.doctype);
		criscostack.realtime.off("list_update");
		criscostack.realtime.on("list_update", (data) => {
	...
	disable_realtime_updates() {
		criscostack.realtime.doctype_unsubscribe(this.doctype);
	...
	process_document_refreshes() {
		...
		criscostack.call(call_args).then(({ message }) => {
			if (!message) return;
			const data = criscostack.utils.dict(message.keys, message.values);
	...
	save_view_user_settings(obj) {
		return criscostack.model.user_settings.save(this.doctype, this.view_name, obj);
	}

	on_update() {}

	update_url_with_filters() {
		if (criscostack.get_route_str() == this.page_name && !this.report_name) {
	...
	get_menu_items() {
		...
		if (criscostack.model.can_import(doctype, null, this.meta)) {
			items.push({
				label: __("Import", null, "Button in list view menu"),
				action: () =>
					criscostack.set_route("list", "data-import", {
		...
		if (criscostack.user_roles.includes("System Manager")) {
			items.push({
				...
				action: () =>
					criscostack.set_route("permission-manager", {
						doctype,
					}),
				standard: true,
			});
		...
		if (
			criscostack.model.can_create("Custom Field") &&
			criscostack.model.can_create("Property Setter")
		) {
			items.push({
				...
				action: () => {
					if (!this.meta) return;
					if (this.meta.custom) {
						criscostack.set_route("form", "doctype", doctype);
					} else if (!this.meta.custom) {
						criscostack.set_route("form", "customize-form", {
							doc_type: doctype,
						});
					}
				},
		...
		if (criscostack.user.has_role("System Manager") && criscostack.boot.developer_mode === 1) {
			// edit doctype
			items.push({
				label: __("Edit DocType", null, "Button in list view menu"),
				action: () => criscostack.set_route("form", "doctype", doctype),
				standard: true,
			});
		}
	...
	show_list_settings() {
		criscostack.model.with_doctype(this.doctype, () => {
			new ListSettings({
				listview: this,
				doctype: this.doctype,
				settings: this.list_view_settings,
				meta: criscostack.get_meta(this.doctype),
			});
		});
	}
	...
	get_workflow_action_menu_items() {
		...
		if (criscostack.model.has_workflow(this.doctype)) {
			const actions = criscostack.workflow.get_all_transition_actions(this.doctype);
			actions.forEach((action) => {
				workflow_actions.push({
					...
					action: () => {
						...
						criscostack
							.xcall("criscostack.model.workflow.bulk_workflow_approval", {
	...
	toggle_workflow_actions() {
		if (!criscostack.model.has_workflow(this.doctype)) return;
		...
		criscostack
			.xcall("criscostack.model.workflow.get_common_transition_actions", {
				docs: checked_items,
				doctype: this.doctype,
			})
			.then((actions) => {
	...
	get_actions_menu_items() {
		...
		const is_field_editable = (field_doc) => {
			return (
				field_doc.fieldname &&
				criscostack.model.is_value_type(field_doc) &&
		...
		const bulk_edit = () => {
			return {
				...
				action: () => {
					...
					criscostack.meta.get_docfields(doctype).forEach((field_doc) => {
		...
		// bulk printing
		if (criscostack.model.can_print(doctype)) {
			actions_menu_items.push(bulk_printing());
		}

		// bulk submit
		if (
			criscostack.model.is_submittable(doctype) &&
			has_submit_permission(doctype) &&
			!criscostack.model.has_workflow(doctype)
		) {
			actions_menu_items.push(bulk_submit());
		}

		// bulk cancel
		if (criscostack.model.can_cancel(doctype) && !criscostack.model.has_workflow(doctype)) {
			actions_menu_items.push(bulk_cancel());
		}

		// bulk delete
		if (criscostack.model.can_delete(doctype)) {
	...
	parse_filters_from_route_options() {
		...
		for (let field in criscostack.route_options) {
			let doctype = null;
			let value = criscostack.route_options[field];
			...
			if (!doctype) {
				doctype = criscostack.meta.get_doctype_for_field(this.doctype, field);
			}
...
criscostack.get_list_view = (doctype) => {
	let route = `List/${doctype}/List`;
	return criscostack.views.list_view[route];
};
