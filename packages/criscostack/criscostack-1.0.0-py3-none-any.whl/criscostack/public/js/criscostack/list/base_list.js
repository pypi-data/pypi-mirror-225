...
criscostack.views.BaseList = class BaseList {
	constructor(opts) {
	...
	setup_defaults() {
		...
		this.page_title = this.page_title || criscostack.router.doctype_layout || __(this.doctype);
		this.meta = criscostack.get_meta(this.doctype);
		this.settings = criscostack.listview_settings[this.doctype] || {};
		this.user_settings = criscostack.get_user_settings(this.doctype);

		this.start = 0;
		this.page_length = 20;
		this.data = [];
		this.method = "criscostack.desk.reportview.get";

		this.can_create = criscostack.model.can_create(this.doctype);
	...
	get_list_view_settings() {
		return criscostack
			.call("criscostack.desk.listview.get_list_settings", {
	...
	async set_fields() {
		let fields = [].concat(criscostack.model.std_fields_list, this.meta.title_field);
	...
	_add_field(fieldname, doctype) {
		...
		const is_valid_field =
			criscostack.model.std_fields_list.includes(fieldname) ||
			criscostack.meta.has_field(doctype, fieldname) ||
			fieldname === "_seen";
	...
	set_stats() {
		...
		this.workflow_state_fieldname = criscostack.workflow.get_state_fieldname(this.doctype);
		if (this.workflow_state_fieldname) {
			if (!criscostack.workflow.workflows[this.doctype]["override_status"]) {
	...
	fetch_meta() {
		return criscostack.model.with_doctype(this.doctype);
	}
	...
	setup_page() {
		...
		!this.hide_card_layout && this.page.main.addClass("criscostack-card");
	...
	setup_view_menu() {
		...
		if (criscostack.boot.desk_settings.view_switcher && !this.meta.force_re_route_to_default_view) {
			/* @preserve
			...
			this.views_list = new criscostack.views.ListViewSelect({
				doctype: this.doctype,
	...
	set_breadcrumbs() {
		criscostack.breadcrumbs.add(this.meta.module, this.doctype);
	}

	setup_side_bar() {
		if (this.hide_sidebar || !criscostack.boot.desk_settings.list_sidebar) return;
		this.list_sidebar = new criscostack.views.ListSidebar({
			doctype: this.doctype,
			stats: this.stats,
	...
	setup_list_wrapper() {
		this.$criscostack_list = $('<div class="criscostack-list">').appendTo(this.page.main);
	}
	...
	setup_result_area() {
		...
		this.$criscostack_list.append(this.$result);
	}
	...
	setup_no_result_area() {
		...
		`).hide();
		this.$criscostack_list.append(this.$no_result);
	}

	setup_freeze_area() {
		this.$freeze = $('<div class="freeze"></div>').hide();
		this.$criscostack_list.append(this.$freeze);
	}
	...
	setup_paging_area() {
		...
		this.$criscostack_list.append(this.$paging_area);
	...
	get_fields() {
		...
		return this.fields.map((f) => criscostack.model.get_full_column_name(f[0], f[1]));
	}

	get_group_by() {
		let name_field = this.fields && this.fields.find((f) => f[0] == "name");
		if (name_field) {
			return criscostack.model.get_full_column_name(name_field[0], name_field[1]);
		}
		return null;
	}
	...
	refresh() {
		...
		return criscostack.call(args).then((r) => {
			// render
			this.prepare_data(r);
	...
	prepare_data(r) {
		...
		Object.assign(criscostack.boot.user_info, data.user_info);
...
class FilterArea {
	...
	clear(refresh = true) {
		...
		return criscostack.run_serially(promises).then(() => {
			this.trigger_refresh = true;
		});
	}
			...
			doctype_fields
				.filter(
					(df) =>
						...
						(df.in_standard_filter && criscostack.model.is_value_type(df.fieldtype))
				)
				.map((df) => {
					...
					if (
						df.fieldtype == "Link" &&
						df.options &&
						criscostack.boot.treeviews.includes(df.options)
					) {
						condition = "descendants of (inclusive)";
					}
	...
	make_filter_list() {
		$(`<div class="filter-selector">
			<div class="btn-group">
				...
				<button class="btn btn-default btn-sm filter-x-button" title="${__("Clear all filters")}">
					<span class="filter-icon">
						${criscostack.utils.icon("filter-x")}
					</span>
				</button>
			</div>
		...
		this.filter_list = new criscostack.ui.FilterGroup({
...
criscostack.views.is_valid = (view_mode) => criscostack.views.view_modes.includes(view_mode);
