...
from criscostack.utils.data import add_to_date, get_datetime, now_datetime
...
class Reminder(Document):
	...
	def clear_old_logs(days=30):
		...
		table = criscostack.qb.DocType("Reminder")
		criscostack.db.delete(table, filters=(table.remind_at < (Now() - Interval(days=days))))

	def validate(self):
		self.user = criscostack.session.user
		if get_datetime(self.remind_at) < now_datetime():
			criscostack.throw(_("Reminder cannot be created in past."))
...
def create_new_reminder(
	...
	reminder = criscostack.new_doc("Reminder")
...
def send_reminders():
	...
	job_freq = cint(criscostack.get_conf().scheduler_interval) or 240
	...
	pending_reminders = criscostack.get_all(
		"Reminder",
		filters=[
	...
	for reminder in pending_reminders:
		criscostack.get_doc("Reminder", reminder).send_reminder()
