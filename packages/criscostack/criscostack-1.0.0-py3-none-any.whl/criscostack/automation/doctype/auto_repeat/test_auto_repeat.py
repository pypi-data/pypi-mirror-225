...
import criscostack
from criscostack.automation.doctype.auto_repeat.auto_repeat import (
...
from criscostack.utils import add_days, add_months, getdate, today

if TYPE_CHECKING:
	from criscostack.custom.doctype.custom_field.custom_field import CustomField
...
def add_custom_fields() -> "CustomField":
	...
	return create_custom_field("ToDo", df) or criscostack.get_doc(
		"Custom Field", dict(fieldname=df["fieldname"], dt="ToDo")
	)


class TestAutoRepeat(CriscoTestCase):
	@classmethod
	def setUpClass(cls):
	...
	def test_daily_auto_repeat(self):
		...
		todo = criscostack.get_doc(doc.reference_doctype, doc.reference_document)
		self.assertEqual(todo.auto_repeat, doc.name)

		new_todo = criscostack.db.get_value(
			"ToDo", {"auto_repeat": doc.name, "name": ("!=", todo.name)}, "name"
		)

		new_todo = criscostack.get_doc("ToDo", new_todo)
	...
	def monthly_auto_repeat(self, doctype, docname, start_date, end_date=None):
		...
		data = get_auto_repeat_entries(getdate(today()))
		create_repeated_entries(data)
		docnames = criscostack.get_all(doc.reference_doctype, {"auto_repeat": doc.name})
		self.assertEqual(len(docnames), 1)

		doc = criscostack.get_doc("Auto Repeat", doc.name)
	...
	def test_email_notification(self):
		...
		email_queue = criscostack.db.exists(
	...
	def test_submit_on_creation(self):
		...
		submittable_doc = criscostack.get_doc(dict(doctype=doctype, test="test submit on creation")).insert()
	...
	doc = criscostack.get_doc(
		{
			...
			"reference_document": args.reference_document or criscostack.db.get_value("ToDo", "name"),
...
def create_submittable_doctype(doctype, submit_perms=1):
	if criscostack.db.exists("DocType", doctype):
		return
	else:
		doc = criscostack.get_doc(
			{
