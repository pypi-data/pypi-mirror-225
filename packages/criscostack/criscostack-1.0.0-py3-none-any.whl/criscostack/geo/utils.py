...
def merge_location_features_in_one(coords):
	...
	for element in coords:
		geojson_loc = criscostack.parse_json(element["location"])
		if not geojson_loc:
			continue
...
def return_location(doctype, filters_sql):
	...
	if filters_sql:
		try:
			coords = criscostack.db.sql(
				f"""SELECT name, location FROM `tab{doctype}`  WHERE {filters_sql}""", as_dict=True
			)
		except criscostack.db.InternalError:
			criscostack.msgprint(criscostack._("This Doctype does not contain location fields"), raise_exception=True)
			return
	else:
		coords = criscostack.get_all(doctype, fields=["name", "location"])
	return coords
	...
	if filters_sql:
		try:
			...
			criscostack.msgprint(
				criscostack._("This Doctype does not contain latitude and longitude fields"), raise_exception=True
			)
			return
	else:
		coords = criscostack.get_all(doctype, fields=["name", "latitude", "longitude"])
	return coords
...
def get_coords_conditions(doctype, filters=None):
	...
	if not criscostack.has_permission(doctype):
		criscostack.throw(criscostack._("Not Permitted"), criscostack.PermissionError)
