...
EXCLUDE_EXCEPTIONS = (
	...
	criscostack.CSRFTokenError,  # CSRF covers OAuth too
	criscostack.SecurityException,
	criscostack.InReadOnlyMode,
)
...
def log_error(
	...
	traceback = criscostack.as_unicode(traceback or criscostack.get_traceback(with_context=True))

	if not criscostack.db:
		print(f"Failed to log error in db: {title}")
		return

	error_log = criscostack.get_doc(
		doctype="Error Log",
		error=traceback,
		method=title,
	...
	if criscostack.flags.read_only or defer_insert:
...
def log_error_snapshot(exception: Exception):
	...
	logger = criscostack.logger(with_more_info=True)
	...
	def decorator_raise_error_on_no_output(func):
		...
		def wrapper_raise_error_on_no_output(*args, **kwargs):
			...
			if (not response) and raise_error:
				criscostack.throw(error_message, error_type or Exception)
			return response
...
def guess_exception_source(exception: str) -> str | None:
	...
	- For criscostack.throws the exception source is possibly present after skipping criscostack.throw frames
