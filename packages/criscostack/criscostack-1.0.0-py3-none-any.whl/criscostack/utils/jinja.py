...
def get_jenv():
	...
	if not getattr(criscostack.local, "jenv", None):
		from jinja2 import DebugUndefined
		from jinja2.sandbox import SandboxedEnvironment

		# criscostack will be loaded last, so app templates will get precedence
		...
		criscostack.local.jenv = jenv

	return criscostack.local.jenv
...
def validate_template(html):
	...
	try:
		...
		criscostack.msgprint(f"Line {e.lineno}: {e.message}")
		criscostack.throw(criscostack._("Syntax error in template"))
...
def render_template(template, context=None, is_path=None, safe_render=True):
	...
	from criscostack import _, get_traceback, throw
...
def get_jloader():
	...
	if not getattr(criscostack.local, "jloader", None):
		from jinja2 import ChoiceLoader, PackageLoader, PrefixLoader

		apps = criscostack.get_hooks("template_apps")
		if not apps:
			apps = list(
				reversed(criscostack.local.flags.web_pages_apps or criscostack.get_installed_apps(_ensure_on_bench=True))
			)

		if "criscostack" not in apps:
			apps.append("criscostack")

		criscostack.local.jloader = ChoiceLoader(
	...
	return criscostack.local.jloader
...
def get_jinja_hooks():
	...
	if not getattr(criscostack.local, "site", None):
		return (None, None)
	...
	def get_obj_dict_from_paths(object_paths):
		...
		for obj_path in object_paths:
			try:
				obj = criscostack.get_module(obj_path)
			except ModuleNotFoundError:
				obj = criscostack.get_attr(obj_path)
	...
	values = criscostack.get_hooks("jinja")
