...
def get_secret():
	from criscostack.utils.password import get_encryption_key

	return criscostack.local.conf.get("secret") or get_encryption_key()


def verify_request():
	"""Verify if the incoming signed request if it is correct."""
	query_string = criscostack.safe_decode(
		criscostack.local.flags.signed_query_string or getattr(criscostack.request, "query_string", None)
	)
	...
	if signature_string in query_string:
		...
		valid_method = criscostack.request.method == "GET"
		valid_request_data = not (criscostack.request.form or criscostack.request.data)
...
def get_url(cmd, params, nonce=None, secret=None):
	...
	return criscostack.utils.get_url("".join(["api/method/", cmd, "?", urlencode(params)]))


def get_signature(params, nonce, secret=None):
	params = "".join(criscostack.utils.cstr(p) for p in params.values())
	if not secret:
		secret = criscostack.local.conf.get("secret") or "secret"
