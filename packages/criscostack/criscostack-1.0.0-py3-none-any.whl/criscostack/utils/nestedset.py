...
from criscostack.query_builder.functions import Coalesce, Max
from criscostack.query_builder.terms import SubQuery
from criscostack.query_builder.utils import DocType


class NestedSetRecursionError(criscostack.ValidationError):
	pass


class NestedSetMultipleRootsError(criscostack.ValidationError):
	pass


class NestedSetChildExistsError(criscostack.ValidationError):
	pass


class NestedSetInvalidMergeError(criscostack.ValidationError):
	pass
...
def update_nsm(doc):
	...
	parent_field = "parent_" + criscostack.scrub(doc.doctype)
	...
	criscostack.db.set_value(doc.doctype, doc.name, old_parent_field, parent or "", update_modified=False)
...
def update_add_node(doc, parent, parent_field):
	...
	if parent:
		left, right = criscostack.db.get_value(doctype, {"name": parent}, ["lft", "rgt"], for_update=True)
	...
	criscostack.qb.update(Table).set(Table.rgt, Table.rgt + 2).where(Table.rgt >= right).run()
	criscostack.qb.update(Table).set(Table.lft, Table.lft + 2).where(Table.lft >= right).run()

	if (
		criscostack.qb.from_(Table).select("*").where((Table.lft == right) | (Table.rgt == right + 1)).run()
	):
		criscostack.throw(_("Nested set error. Please contact the Administrator."))

	# update index of new node
	criscostack.qb.update(Table).set(Table.lft, right).set(Table.rgt, right + 1).where(
		Table.name == name
	).run()
	return right
...
def update_move_node(doc: Document, parent_field: str):
	...
	# move to dark side
	criscostack.qb.update(Table).set(Table.lft, -Table.lft).set(Table.rgt, -Table.rgt).where(
		(Table.lft >= doc.lft) & (Table.rgt <= doc.rgt)
	).run()

	# shift left
	diff = doc.rgt - doc.lft + 1
	criscostack.qb.update(Table).set(Table.lft, Table.lft - diff).set(Table.rgt, Table.rgt - diff).where(
		Table.lft > doc.rgt
	).run()

	# shift left rgts of ancestors whose only rgts must shift
	criscostack.qb.update(Table).set(Table.rgt, Table.rgt - diff).where(
		(Table.lft < doc.lft) & (Table.rgt > doc.rgt)
	).run()
	...
	if parent:
		...
		# set parent lft, rgt
		criscostack.qb.update(Table).set(Table.rgt, Table.rgt + diff).where(Table.name == parent).run()
		...
		# new root
		max_rgt = criscostack.qb.from_(Table).select(Max(Table.rgt)).run(pluck=True)[0]
		new_diff = max_rgt + 1 - doc.lft

	# bring back from dark side
	criscostack.qb.update(Table).set(Table.lft, -Table.lft + new_diff).set(
...
def rebuild_tree(doctype, parent_field):
	...
	if criscostack.request and criscostack.local.form_dict.cmd == "rebuild_tree":
		criscostack.only_for("System Manager")

	meta = criscostack.get_meta(doctype)
	if not meta.has_field("lft") or not meta.has_field("rgt"):
		criscostack.throw(
			_("Rebuilding of tree is not supported for {}").format(criscostack.bold(doctype)),
	...
	criscostack.db.auto_commit_on_many_writes = 0
...
def rebuild_node(doctype, parent, left, parent_field):
	...
	result = (criscostack.qb.from_(table).where(column == parent).select(table.name)).run()
	...
	criscostack.db.set_value(doctype, parent, {"lft": left, "rgt": right}, update_modified=False)
...
def validate_loop(doctype, name, lft, rgt):
	...
	if name in criscostack.get_all(
		doctype, filters={"lft": ["<=", lft], "rgt": [">=", rgt]}, pluck="name"
	):
		criscostack.throw(_("Item cannot be added to its own descendants"), NestedSetRecursionError)
...
def remove_subtree(doctype: str, name: str, throw=True):
	...
	criscostack.qb.update(table).set(table.lft, table.lft - width).where(table.lft > rgt).run()
	criscostack.qb.update(table).set(table.rgt, table.rgt - width).where(table.rgt > rgt).run()
...
class NestedSet(Document):
	...
	def on_trash(self, allow_root_deletion=False):
		...
		if not getattr(self, "nsm_parent_field", None):
			self.nsm_parent_field = criscostack.scrub(self.doctype) + "_parent"

		parent = self.get(self.nsm_parent_field)
		if not parent and not getattr(self, "allow_root_deletion", True):
			criscostack.throw(_("Root {0} cannot be deleted").format(_(self.doctype)))
	...
	def validate_if_child_exists(self):
		has_children = criscostack.db.count(self.doctype, filters={self.nsm_parent_field: self.name})
		if has_children:
			criscostack.throw(
	...
	def before_rename(self, olddn, newdn, merge=False, group_fname="is_group"):
		if merge and hasattr(self, group_fname):
			is_group = criscostack.db.get_value(self.doctype, newdn, group_fname)
	...
	def validate_one_root(self):
		if not self.get(self.nsm_parent_field):
			if self.get_root_node_count() > 1:
				criscostack.throw(_("""Multiple root nodes not allowed."""), NestedSetMultipleRootsError)

	def get_root_node_count(self):
		return criscostack.db.count(self.doctype, {self.nsm_parent_field: ""})
	...
	def validate_ledger(self, group_identifier="is_group"):
		if hasattr(self, group_identifier) and not bool(self.get(group_identifier)):
			if criscostack.get_all(self.doctype, {self.nsm_parent_field: self.name, "docstatus": ("!=", 2)}):
				criscostack.throw(
...
def get_root_of(doctype):
	...
	node_query = SubQuery(
		criscostack.qb.from_(t2).select(Count("*")).where((t2.lft < t1.lft) & (t2.rgt > t1.rgt))
	)
	result = criscostack.qb.from_(t1).select(t1.name).where((node_query == 0) & (t1.rgt > t1.lft)).run()
