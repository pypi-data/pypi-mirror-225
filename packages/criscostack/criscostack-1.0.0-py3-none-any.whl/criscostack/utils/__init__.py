...
def get_fullname(user=None):
	...
	if not hasattr(criscostack.local, "fullnames"):
		criscostack.local.fullnames = {}

	if not criscostack.local.fullnames.get(user):
		p = criscostack.db.get_value("User", user, ["first_name", "last_name"], as_dict=True)
		if p:
			criscostack.local.fullnames[user] = (
				" ".join(filter(None, [p.get("first_name"), p.get("last_name")])) or user
			)
		else:
			criscostack.local.fullnames[user] = user

	return criscostack.local.fullnames.get(user)
...
def get_email_address(user=None):
	...
	return criscostack.db.get_value("User", user, "email")
...
def validate_phone_number_with_country_code(phone_number: str, fieldname: str) -> None:
	...
	try:
		...
		if not valid_number:
			criscostack.throw(
				error_message.format(criscostack.bold(phone_number), criscostack.bold(fieldname)),
				title=error_title,
				exc=criscostack.InvalidPhoneNumberError,
			)
...
def validate_phone_number(phone_number, throw=False):
	...
	if not match and throw:
		criscostack.throw(
			criscostack._("{0} is not a valid Phone Number").format(phone_number), criscostack.InvalidPhoneNumberError
		)
...
def validate_name(name, throw=False):
	...
	Note: "Name" here is name of a person, not the primary key in Crisco doctypes.
	"""
	if not name:
		return False
	...
	if not match and throw:
		criscostack.throw(criscostack._("{0} is not a valid Name").format(name), criscostack.InvalidNameError)
...
def validate_email_address(email_str, throw=False):
	...
	def _check(e):
		...
		if not _valid:
			if throw:
				invalid_email = criscostack.utils.escape_html(e)
				criscostack.throw(
					criscostack._("{0} is not a valid Email Address").format(invalid_email),
					criscostack.InvalidEmailAddressError,
				)
			return None
		else:
			return matched
...
def validate_url(
	...
	if not is_valid and throw:
		criscostack.throw(criscostack._("'{0}' is not a valid URL").format(criscostack.bold(txt)))
...
def has_gravatar(email: str) -> str:
	...
	if criscostack.flags.in_import or criscostack.flags.in_install or criscostack.flags.in_test:
...
def get_gravatar_url(email: str, default: Literal["mm", "404"] = "mm") -> str:
	hexdigest = hashlib.md5(criscostack.as_unicode(email).encode("utf-8")).hexdigest()
...
def log(event, details):
	criscostack.logger(event).info(details)
...
def get_defaults(key=None):
	...
	"""
	return criscostack.db.get_defaults(key)
...
def set_default(key, val):
	...
	"""
	return criscostack.db.set_default(key, val)
...
def get_path(*path, **kwargs):
	...
	if not base:
		base = criscostack.local.site_path
	return os.path.join(base, *path)


def get_site_base_path():
	return criscostack.local.site_path
...
def get_bench_path():
	return os.environ.get("CRISCOSTACK_BENCH_ROOT") or os.path.realpath(
		os.path.join(os.path.dirname(criscostack.__file__), "..", "..", "..")
	)


def get_bench_id():
	return criscostack.get_conf().get("bench_id", get_bench_path().strip("/").replace("/", "-"))


def get_site_id(site=None):
	return f"{site or criscostack.local.site}@{get_bench_id()}"
...
def get_site_url(site):
	return f"http://{site}:{criscostack.get_conf(site).webserver_port}"
...
def get_test_client(use_cookies=True) -> Client:
	"""Returns an test instance of the Crisco WSGI"""
	from criscostack.app import application

	return Client(application, use_cookies=use_cookies)


def get_hook_method(hook_name, fallback=None):
	method = criscostack.get_hooks().get(hook_name)
	if method:
		method = criscostack.get_attr(method[0])
		return method
	if fallback:
		return fallback
...
def call_hook_method(hook, *args, **kwargs):
	out = None
	for method_name in criscostack.get_hooks(hook):
		out = out or criscostack.get_attr(method_name)(*args, **kwargs)
...
def update_progress_bar(txt, i, l, absolute=False):
	...
	if not getattr(criscostack.local, "request", None) or is_cli():  # pragma: no cover
		lt = len(txt)
		try:
...
def get_html_format(print_path):
	...
	if os.path.exists(print_path):
		...
		for include_directive, path in INCLUDE_DIRECTIVE_PATTERN.findall(html_format):
			for app_name in criscostack.get_installed_apps():
				include_path = criscostack.get_app_path(app_name, *path.split(os.path.sep))
...
def get_sites(sites_path=None):
	if not sites_path:
		sites_path = getattr(criscostack.local, "sites_path", None) or "."
...
def markdown(text, sanitize=True, linkify=True):
	html = text if is_html(text) else criscostack.utils.md_to_html(text)
...
def get_site_info():
	from criscostack.email.queue import get_emails_sent_this_month
	from criscostack.utils.user import get_system_managers

	# only get system users
	users = criscostack.get_all(
		"User",
		filters={"user_type": "System User", "name": ("not in", criscostack.STANDARD_USERS)},
	...
	system_settings = criscostack.db.get_singles_dict("System Settings")
	space_usage = criscostack._dict((criscostack.local.conf.limits or {}).get("space_usage", {}))
	...
	site_info = {
		...
		"last_logins": criscostack.get_all("Activity Log", **kwargs),
	}

	# from other apps
	for method_name in criscostack.get_hooks("get_site_info"):
		site_info.update(criscostack.get_attr(method_name)(site_info) or {})

	# dumps -> loads to prevent datatype conflicts
	return json.loads(criscostack.as_json(site_info))
...
def get_db_count(*args):
	...
	Example:
	        via terminal:
	                bench --site criscoerp.local execute criscostack.utils.get_db_count --args "['DocType', 'Communication']"
	"""
	db_count = {}
	for doctype in args:
		db_count[doctype] = criscostack.db.count(doctype)

	return json.loads(criscostack.as_json(db_count))
...
def call(fn, *args, **kwargs):
	...
	Pass a doctype or a series of doctypes to get the count of docs in them
	Parameters:
	        fn: criscostack function to be called
	...
	Example:
	        via terminal:
	                bench --site criscoerp.local execute criscostack.utils.call --args '''["criscostack.get_all", "Activity Log"]''' --kwargs '''{"fields": ["user", "creation", "full_name"], "filters":{"Operation": "Login", "Status": "Success"}, "limit": "10"}'''
	"""
	return json.loads(criscostack.as_json(criscostack.call(fn, *args, **kwargs)))
...
def get_safe_filters(filters):
	try:
		...
		if isinstance(filters, (int, float)):
			filters = criscostack.as_unicode(filters)
...
def set_request(**kwargs):
	...
	criscostack.local.request = Request(builder.get_environ())
...
def get_html_for_route(route):
	...
	html = criscostack.safe_decode(response.get_data())
	return html
...
def get_build_version():
	try:
		return str(os.path.getmtime(os.path.join(criscostack.local.sites_path, "assets/assets.json")))
	except OSError:
		# .build can sometimes not exist
		# this is not a major problem so send fallback
		return criscostack.utils.random_string(8)


def get_assets_json():
	def _get_assets():
		# get merged assets.json and assets-rtl.json
		assets = criscostack.parse_json(criscostack.read_file("assets/assets.json"))

		if assets_rtl := criscostack.read_file("assets/assets-rtl.json"):
			assets.update(criscostack.parse_json(assets_rtl))

		return assets

	if not hasattr(criscostack.local, "assets_json"):
		if not criscostack.conf.developer_mode:
			criscostack.local.assets_json = criscostack.cache.get_value(
				"assets_json",
				_get_assets,
				shared=True,
			)
			...
			criscostack.local.assets_json = _get_assets()

	return criscostack.local.assets_json
...
def add_user_info(user, user_info):
	if user not in user_info:
		info = (
			...
			or criscostack._dict()
		)
		user_info[user] = criscostack._dict(
...
class CallbackManager:
	...
	Example usage: criscostack.db.after_commit
	"""
