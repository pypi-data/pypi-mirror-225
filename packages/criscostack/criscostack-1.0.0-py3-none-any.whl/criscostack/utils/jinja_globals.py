...
def inspect(var, render=True):
	from criscostack.utils.jinja import get_jenv
...
def web_blocks(blocks):
	import criscostack
	from criscostack import _, _dict, throw
	from criscostack.website.doctype.web_page.web_page import get_web_blocks_html
	...
	if not criscostack.flags.web_block_scripts:
	...
	for template, scripts in out.scripts.items():
		...
		if template not in criscostack.flags.web_block_scripts:
			for script in scripts:
				html += f"<script data-web-template='{template}'>{script}</script>"
			criscostack.flags.web_block_scripts[template] = True

	for template, styles in out.styles.items():
		# deduplication of styles when web_blocks methods are used in web pages
		# see render_dynamic method web_page.py
		if template not in criscostack.flags.web_block_styles:
			for style in styles:
				html += f"<style data-web-template='{template}'>{style}</style>"
			criscostack.flags.web_block_styles[template] = True

	return html


def get_dom_id(seed=None):
	from criscostack import generate_hash
...
def include_script(path, preload=True):
	...
	if preload:
		...
		criscostack.local.preload_assets["script"].append(path)
...
def include_style(path, rtl=None, preload=True):
	...
	if preload:
		...
		criscostack.local.preload_assets["style"].append(path)

	return f'<link type="text/css" rel="stylesheet" href="{path}">'


def bundled_asset(path, rtl=None):
	from criscostack.utils import get_assets_json
	from criscostack.website.utils import abs_url
