...
from criscostack.utils import cint, strip_html_tags
from criscostack.utils.data import cstr
...
def reset():
	...
	:return:
	"""
	criscostack.db.delete("__global_search")
...
def get_doctypes_with_global_search(with_child_tables=True):
	...
	def _get():
		...
		for d in criscostack.get_all("DocType", fields=["name", "module"], filters=filters):
			meta = criscostack.get_meta(d.name)
			if len(meta.get_global_search_fields()) > 0:
				global_search_doctypes.append(d)

		installed_apps = criscostack.get_installed_apps()
		module_app = criscostack.local.module_app

		doctypes = [
			d.name
			for d in global_search_doctypes
			if module_app.get(criscostack.scrub(d.module))
			and module_app[criscostack.scrub(d.module)] in installed_apps
		]

		return doctypes

	return criscostack.cache.get_value("doctypes_with_global_search", _get)
...
def rebuild_for_doctype(doctype):
	...
	"""
	if criscostack.local.conf.get("disable_global_search"):
		return

	def _get_filters():
		filters = criscostack._dict({"docstatus": ["!=", 2]})
	...
	if cint(meta.istable) == 1:
		parent_doctypes = criscostack.get_all(
			"DocField",
			fields="parent",
			filters={"fieldtype": ["in", criscostack.model.table_fields], "options": doctype},
		)
	...
	all_records = criscostack.get_all(doctype, fields=fieldnames, filters=_get_filters())
		...
		if content:
			...
			try:
				if hasattr(get_controller(doctype), "is_website_published") and meta.allow_guest_to_view:
					d = criscostack.get_doc(doctype, doc.name)
			...
			all_contents.append(
				{
					"doctype": criscostack.db.escape(doctype),
					"name": criscostack.db.escape(doc.name),
					"content": criscostack.db.escape(" ||| ".join(content or "")),
					"published": published,
					"title": criscostack.db.escape((title or "")[: int(criscostack.db.VARCHAR_LEN)]),
					"route": criscostack.db.escape((route or "")[: int(criscostack.db.VARCHAR_LEN)]),
				}
			)
	if all_contents:
...
def delete_global_search_records_for_doctype(doctype):
	criscostack.db.delete("__global_search", {"doctype": doctype})
...
def get_children_data(doctype, meta):
	...
	"""
	all_children = criscostack._dict()
	child_search_fields = criscostack._dict()

	for child in meta.get_table_fields():
		child_meta = criscostack.get_meta(child.options)
		...
		if search_fields:
			...
			child_records = criscostack.get_all(
				child.options, fields=child_fieldnames, filters={"docstatus": ["!=", 1], "parenttype": doctype}
			)

			for record in child_records:
				all_children.setdefault(record.parent, criscostack._dict()).setdefault(child.options, []).append(
					record
				)
...
def update_global_search(doc):
	...
	for field in doc.meta.get_global_search_fields():
		if doc.get(field.fieldname) and field.fieldtype not in criscostack.model.table_fields:
	...
	if content:
		...
		title = (cstr(doc.get_title()) or "")[: int(criscostack.db.VARCHAR_LEN)]
...
def update_global_search_for_all_web_pages():
	if criscostack.conf.get("disable_global_search"):
		return
...
def get_routes_to_index():
	apps = criscostack.get_installed_apps()

	routes_to_index = []
	for app in apps:
		base = criscostack.get_app_path(app, "www")
		path_to_index = criscostack.get_app_path(app, "www")
...
def add_route_to_global_search(route):
	...
	criscostack.local.no_cache = True
...
def get_formatted_value(value, field):
	...
	if getattr(field, "fieldtype", None) in ["Text", "Text Editor"]:
		value = unescape_html(criscostack.safe_decode(value))
...
def sync_global_search():
	...
	:return:
	"""
	while criscostack.cache.llen("global_search_queue") > 0:
		# rpop to follow FIFO
		# Last one should override all previous contents of same document
		value = json.loads(criscostack.cache.rpop("global_search_queue").decode("utf-8"))
		sync_value(value)
...
def sync_value_in_queue(value):
	try:
		...
		criscostack.cache.lpush("global_search_queue", json.dumps(value))
...
def delete_for_document(doc):
	...
	"""
	criscostack.db.delete("__global_search", {"doctype": doc.doctype, "name": doc.name})
...
def search(text, start=0, limit=20, doctype=""):
	...
	"""
	from criscostack.desk.doctype.global_search_settings.global_search_settings import (
		get_doctypes_for_global_search,
	)
	from criscostack.query_builder.functions import Match
	...
	for doctype in allowed_doctypes:
		for index, r in enumerate(results):
			if r.doctype == doctype and r.rank > 0.0:
				try:
					meta = criscostack.get_meta(r.doctype)
					if meta.image_field:
						r.image = criscostack.db.get_value(r.doctype, r.name, meta.image_field)
				except Exception:
...
def web_search(text: str, scope: str | None = None, start: int = 0, limit: int = 20):
	...
	for text in texts:
		...
		result = criscostack.db.multisql(
			{
