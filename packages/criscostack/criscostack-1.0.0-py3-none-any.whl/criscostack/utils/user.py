...
from criscostack.boot import get_allowed_reports
from criscostack.core.doctype.domain_settings.domain_settings import get_active_modules
from criscostack.permissions import get_roles, get_valid_perms
from criscostack.query_builder import DocType, Order
...
class UserPermissions:
	"""
	A user permission object can be accessed as `criscostack.get_user()`
	"""

	def __init__(self, name=""):
		self.defaults = None
		self.name = name or criscostack.session.get("user")
		self.roles = []
	...
	def setup_user(self):
		def get_user_doc():
			user = None
			try:
				user = criscostack.get_doc("User", self.name).as_dict()
			except criscostack.DoesNotExistError:
				pass
			except Exception as e:
				# install boo-boo
				if not criscostack.db.is_table_missing(e):
					raise

			return user

		if not criscostack.flags.in_install_db and not criscostack.flags.in_test:
			user_doc = criscostack.cache.hget("user_doc", self.name, get_user_doc)
			if user_doc:
				self.doc = criscostack.get_doc(user_doc)
	...
	def build_doctype_map(self):
		...
		all_doctypes = criscostack.get_all(
			"DocType",
			fields=[
				"name",
				"in_create",
	...
	def build_perm_map(self):
		...
		for r in get_valid_perms():
			...
			for k in criscostack.permissions.rights:
	...
	def build_permissions(self):
		...
		self.build_perm_map()
		user_shared = criscostack.share.get_shared_doctypes()
		no_list_view_link = []
		...
		self.shared = criscostack.get_all(
		...
		if "System Manager" in self.get_roles():
			self.can_import += criscostack.get_all("DocType", {"allow_import": 1}, pluck="name")
			self.can_import += criscostack.get_all(
				"Property Setter",
				pluck="doc_type",
				filters={"property": "allow_import", "value": "1"},
			)

		criscostack.cache.hset("can_import", criscostack.session.user, self.can_import)

	def get_defaults(self):
		import criscostack.defaults

		self.defaults = criscostack.defaults.get_defaults(self.name)
	...
	def load_user(self):
		...
		d.name = self.name
		d.onboarding_status = criscostack.parse_json(d.onboarding_status)
...
def get_fullname_and_avatar(user: str) -> _dict:
	first_name, last_name, avatar, name = criscostack.db.get_value(
...
def get_system_managers(only_name: bool = False) -> list[str]:
	...
	system_managers = (
		...
		.where(
			...
			& (User.docstatus < 2)
			& (User.name.notin(criscostack.STANDARD_USERS))
		)
		.select(*fields)
...
def add_role(user: str, role: str) -> None:
	criscostack.get_doc("User", user).add_roles(role)
...
def is_website_user(username: str | None = None) -> str | None:
	return criscostack.db.get_value("User", username or criscostack.session.user, "user_type") == "Website User"
...
def is_system_user(username: str | None = None) -> str | None:
	return criscostack.db.get_value(
		"User",
		{
			"email": username or criscostack.session.user,
			"enabled": 1,
			"user_type": "System User",
		},
	)
