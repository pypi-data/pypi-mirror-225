...
def request_cache(func: Callable) -> Callable:
	...
	criscostack.local.request_cache. The cache only persists for the current request
	...
	def wrapper(*args, **kwargs):
		if not getattr(criscostack.local, "initialised", None):
			return func(*args, **kwargs)
		if not hasattr(criscostack.local, "request_cache"):
			criscostack.local.request_cache = defaultdict(dict)
		...
		try:
			return criscostack.local.request_cache[func][args_key]
		except KeyError:
			return_val = func(*args, **kwargs)
			criscostack.local.request_cache[func][args_key] = return_val
			return return_val
...
def site_cache(ttl: int | None = None, maxsize: int | None = None) -> Callable:
	...
	criscostack.utils.caching._SITE_CACHE. The cache persists on the parent process.
	...
	workers, use redis (criscostack.cache API) instead.
	...
	def time_cache_wrapper(func: Callable = None) -> Callable:
		...
		def site_cache_wrapper(*args, **kwargs):
			if getattr(criscostack.local, "initialised", None):
				...
				if hasattr(func, "maxsize") and len(_SITE_CACHE[func_key][criscostack.local.site]) >= func.maxsize:
					_SITE_CACHE[func_key][criscostack.local.site].pop(
						next(iter(_SITE_CACHE[func_key][criscostack.local.site])), None
					)

				if func_call_key not in _SITE_CACHE[func_key][criscostack.local.site]:
					_SITE_CACHE[func_key][criscostack.local.site][func_call_key] = func(*args, **kwargs)

				return _SITE_CACHE[func_key][criscostack.local.site][func_call_key]
...
def redis_cache(ttl: int | None = 3600, user: str | bool | None = None) -> Callable:
	...
	def wrapper(func: Callable = None) -> Callable:
		...
		def clear_cache():
			criscostack.cache.delete_keys(func_key)
		...
		def redis_cache_wrapper(*args, **kwargs):
			...
			if criscostack.cache.exists(func_call_key):
				return criscostack.cache.get_value(func_call_key, user=user)
			else:
				val = func(*args, **kwargs)
				ttl = getattr(func, "ttl", 3600)
				criscostack.cache.set_value(func_call_key, val, expires_in_sec=ttl, user=user)
				return val
