...
def main():
	...
	commands.update({"get-criscostack-commands": get_criscostack_commands, "get-criscostack-help": get_criscostack_help})
	click.Group(commands=commands)(prog_name="bench")


def get_app_groups() -> dict[str, click.Group]:
	"""Get all app groups, put them in main group "criscostack" since bench is
	designed to only handle that"""
	commands = {}
	for app in get_apps():
		if app_commands := get_app_commands(app):
			commands |= app_commands
	return dict(criscostack=click.group(name="criscostack", commands=commands)(app_group))
...
def app_group(ctx, site=False, force=False, verbose=False, profile=False):
	...
	if ctx.info_name == "criscostack":
		ctx.info_name = ""


def get_sites(site_arg: str) -> list[str]:
	if site_arg == "all":
		return criscostack.utils.get_sites()
	elif site_arg:
		return [site_arg]
	elif os.environ.get("CRISCOSTACK_SITE"):
		return [os.environ.get("CRISCOSTACK_SITE")]
	elif default_site := criscostack.get_conf().default_site:
		return [default_site]
	# This is not supported, just added here for warning.
	elif (site := criscostack.read_file("currentsite.txt")) and site.strip():
		click.secho(
			dedent(
				f"""
...
@click.command("get-criscostack-commands")
def get_criscostack_commands():
	commands = list(get_app_commands("criscostack"))
...
@click.command("get-criscostack-help")
def get_criscostack_help():
	print(click.Context(get_app_groups()["criscostack"]).get_help())


def get_apps():
	return criscostack.get_all_apps(with_internal_apps=False, sites_path=".")


if __name__ == "__main__":
	if not criscostack._dev_server:
