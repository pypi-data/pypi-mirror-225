...
from criscostack.translate import print_language
...
from criscostack.www.printview import validate_print_permission
...
def download_multi_pdf(
	...
	if not isinstance(doctype, dict):
		...
		criscostack.local.response.filename = "{doctype}.pdf".format(
		...
		criscostack.local.response.filename = f"{name}.pdf"

	criscostack.local.response.filecontent = read_multi_pdf(output)
	criscostack.local.response.type = "pdf"


def read_multi_pdf(output):
	# Get the content of the merged pdf files
	fname = os.path.join("/tmp", f"criscostack-pdf-{criscostack.generate_hash()}.pdf")
...
def download_pdf(
	...
	doc = doc or criscostack.get_doc(doctype, name)
	validate_print_permission(doc)

	with print_language(language):
		pdf_file = criscostack.get_print(
	...
	criscostack.local.response.filecontent = pdf_file
	criscostack.local.response.type = "pdf"
...
def report_to_pdf(html, orientation="Landscape"):
	...
	criscostack.local.response.filename = "report.pdf"
	criscostack.local.response.filecontent = get_pdf(html, {"orientation": orientation})
	criscostack.local.response.type = "pdf"
...
def print_by_server(
	...
	print_settings = criscostack.get_doc("Network Printer Settings", printer_setting)
	try:
		import cups
	except ImportError:
		criscostack.throw(_("You need to install pycups to use this feature!"))
	...
	try:
		...
		if not file_path:
			file_path = os.path.join("/", "tmp", f"criscostack-pdf-{criscostack.generate_hash()}.pdf")
		...
		if (
			...
			criscostack.throw(_("PDF generation failed"))
	except cups.IPPError:
		criscostack.throw(_("Printing failed"))
