function setup_list_filters() {
	...
	criscostack.call('criscostack.website.doctype.web_form.web_form.get_form_data', {
	...
	.then((r) => {
		...
		web_form_filters = web_form.web_form_fields.filter(df => {
			...
			const value = criscostack.utils.get_query_params()[df.fieldname];
			const f = criscostack.ui.form.make_control({
				df: {
					...
					is_filter: true,
					change: (e) => {
						const query_params = Object.assign(criscostack.utils.get_query_params(), {
							[df.fieldname]: f.get_value()
						})
						const query_string = criscostack.utils.make_query_string(query_params);
...
criscostack.ready(setup_list_filters);
