...
import criscostack
from criscostack.defaults import _clear_cache
from criscostack.utils import cint, is_git_url
...
def _is_scheduler_enabled() -> bool:
	...
	try:
		criscostack.connect()
		enable_scheduler = cint(criscostack.db.get_single_value("System Settings", "enable_scheduler"))
	except Exception:
		pass
	finally:
...
def _new_site(
	...
	"""Install a new Crisco site"""

	from criscostack.utils import get_site_path, scheduler, touch_file
	...
	if not db_name:
		...
		db_name = "_" + hashlib.sha1(os.path.realpath(criscostack.get_site_path()).encode()).hexdigest()[:16]
	...
	with filelock("bench_new_site", timeout=1):
		...
		apps_to_install = (
			["criscostack"] + (criscostack.conf.get("install_apps") or []) + (list(install_apps) or [])
		)
	...
	scheduler_status = "disabled" if criscostack.utils.scheduler.is_scheduler_disabled() else "enabled"
...
def install_db(
	...
	from criscostack.database import setup_database

	if not db_type:
		db_type = criscostack.conf.db_type
	...
	criscostack.flags.in_install_db = True

	criscostack.flags.root_login = root_login
	criscostack.flags.root_password = root_password
	setup_database(force, source_sql, verbose, no_mariadb_socket)

	criscostack.conf.admin_password = criscostack.conf.admin_password or admin_password

	remove_missing_apps()

	criscostack.db.create_auth_table()
	criscostack.db.create_global_search_table()
	criscostack.db.create_user_settings_table()

	criscostack.flags.in_install_db = False
...
def find_org(org_repo: str) -> tuple[str, str]:
	...
	find_org()
	ref -> https://github.com/criscostack/bench/blob/develop/bench/utils/__init__.py#L390
	...
	from criscostack.exceptions import InvalidRemoteException

	for org in ["criscostack", "criscoerp"]:
...
def fetch_details_from_tag(_tag: str) -> tuple[str, str, str]:
	...
	ref -> https://github.com/criscostack/bench/blob/develop/bench/utils/__init__.py#L403
...
def parse_app_name(name: str) -> str:
	...
	ref -> https://github.com/criscostack/bench/blob/develop/bench/app.py#L114
...
def install_app(name, verbose=False, set_as_patched=True, force=False):
	...
	from criscostack.model.sync import sync_for
	from criscostack.modules.utils import sync_customizations
	from criscostack.utils.fixtures import sync_fixtures

	criscostack.flags.in_install = name
	criscostack.flags.ignore_in_install = False

	criscostack.clear_cache()
	app_hooks = criscostack.get_hooks(app_name=name)
	installed_apps = criscostack.get_installed_apps()
	...
	if name not in criscostack.get_all_apps():
	...
	for before_install in app_hooks.before_install or []:
		out = criscostack.get_attr(before_install)()
		if out is False:
			return

	for fn in criscostack.get_hooks("before_app_install"):
		criscostack.get_attr(fn)(name)
	...
	criscostack.get_doc("Portal Settings", "Portal Settings").sync_menu()

	if set_as_patched:
		set_all_patches_as_completed(name)

	for after_install in app_hooks.after_install or []:
		criscostack.get_attr(after_install)()

	for fn in criscostack.get_hooks("after_app_install"):
		criscostack.get_attr(fn)(name)
	...
	for after_sync in app_hooks.after_sync or []:
		criscostack.get_attr(after_sync)()  #

	criscostack.flags.in_install = False
...
def add_to_installed_apps(app_name, rebuild_website=True):
	...
	if app_name not in installed_apps:
		...
		criscostack.db.set_global("installed_apps", json.dumps(installed_apps))
		criscostack.db.commit()
		if criscostack.flags.in_install:
...
def remove_app(app_name, dry_run=False, yes=False, no_backup=False, force=False):
	...
	app_hooks = criscostack.get_hooks(app_name=app_name)

	# dont allow uninstall app if not installed unless forced
	if not force:
		if app_name not in criscostack.get_installed_apps():
	...
	if not (dry_run or no_backup):
		from criscostack.utils.backups import scheduled_backup

		print("Backing up...")
		scheduled_backup(ignore_files=True)

	criscostack.flags.in_uninstall = True

	for before_uninstall in app_hooks.before_uninstall or []:
		criscostack.get_attr(before_uninstall)()

	for fn in criscostack.get_hooks("before_app_uninstall"):
		criscostack.get_attr(fn)(app_name)

	modules = criscostack.get_all("Module Def", filters={"app_name": app_name}, pluck="name")
	...
	for after_uninstall in app_hooks.after_uninstall or []:
		criscostack.get_attr(after_uninstall)()

	for fn in criscostack.get_hooks("after_app_uninstall"):
		criscostack.get_attr(fn)(app_name)

	click.secho(f"Uninstalled App {app_name} from Site {site}", fg="green")
	criscostack.flags.in_uninstall = False
...
def _delete_modules(modules: list[str], dry_run: bool) -> list[str]:
	...
	for module_name in modules:
		...
		for doctype in criscostack.get_all(
			...
			if not dry_run:
				if doctype.issingle:
					criscostack.delete_doc("DocType", doctype.name, ignore_on_trash=True, force=True)
				else:
		...
		if not dry_run:
			criscostack.delete_doc("Module Def", module_name, ignore_on_trash=True, force=True)
...
def _delete_linked_documents(
	...
	for doctype, fieldname in doctype_linkfield_map.items():
		for record in criscostack.get_all(doctype, filters={fieldname: module_name}, pluck="name"):
			print(f"* removing {doctype} '{record}'...")
			if not dry_run:
				criscostack.delete_doc(doctype, record, ignore_on_trash=True, force=True)
...
def _get_module_linked_doctype_field_map() -> dict[str, str]:
	...
	linked_doctypes = criscostack.get_all(
		"DocField",
		filters={"fieldtype": "Link", "options": "Module Def"},
		fields=["parent", "fieldname"],
	)
	existing_linked_doctypes = [d for d in linked_doctypes if criscostack.db.exists("DocType", d.parent)]
...
def _delete_doctypes(doctypes: list[str], dry_run: bool) -> None:
	for doctype in set(doctypes):
		...
		if not dry_run:
			criscostack.delete_doc("DocType", doctype, ignore_on_trash=True, force=True)
			criscostack.db.sql_ddl(f"DROP TABLE IF EXISTS `tab{doctype}`")
...
def set_all_patches_as_completed(app):
	from criscostack.modules.patch_handler import get_patches_from_app

	patches = get_patches_from_app(app)
	for patch in patches:
		criscostack.get_doc({"doctype": "Patch Log", "patch": patch}).insert(ignore_permissions=True)
	criscostack.db.commit()


def init_singles():
	singles = criscostack.get_all("DocType", filters={"issingle": True}, pluck="name")
	for single in singles:
		if criscostack.db.get_singles_dict(single):
			continue

		try:
			doc = criscostack.new_doc(single)
			doc.flags.ignore_mandatory = True
			doc.flags.ignore_validate = True
			doc.save()
		except (ImportError, criscostack.DoesNotExistError):
...
def make_conf(
	...
	sites_path = criscostack.local.sites_path
	criscostack.destroy()
	criscostack.init(site, sites_path=sites_path)


def make_site_config(
	db_name=None, db_password=None, site_config=None, db_type=None, db_host=None, db_port=None
):
	criscostack.create_folder(os.path.join(criscostack.local.site_path))
...
def _update_config_file(key: str, value, config_file: str):
	...
	if hasattr(criscostack.local, "conf"):
		criscostack.local.conf[key] = value


def get_site_config_path():
	return os.path.join(criscostack.local.site_path, "site_config.json")
...
def make_site_dirs():
	for dir_path in [
		...
		path = criscostack.get_site_path(dir_path)
		os.makedirs(path, exist_ok=True)


def add_module_defs(app, ignore_if_duplicate=False):
	modules = criscostack.get_module_list(app)
	for module in modules:
		d = criscostack.new_doc("Module Def")
		d.app_name = app
...
def remove_missing_apps():
	...
	apps = ("criscostack_subscription", "shopping_cart")
	installed_apps = json.loads(criscostack.db.get_global("installed_apps") or "[]")
	for app in apps:
...
def extract_sql_from_archive(sql_file_path):
	...
	"""
	from criscostack.utils import get_bench_relative_path
...
def extract_files(site_name, file_path):
	...
	# Need to do criscostack.init to maintain the site locals
	criscostack.init(site=site_name)
	abs_site_path = os.path.abspath(criscostack.get_site_path())
...
def is_downgrade(sql_file_path, verbose=False):
	...
	with open(sql_file_path) as f:
		for line in f:
			if head in line:
				# 'line' (str) format: ('2056588823','2020-05-11 18:21:31.488367','2020-06-12 11:49:31.079506','Administrator','Administrator',0,'Installed Applications','installed_applications','Installed Applications',1,'criscostack','v10.1.71-74 (3c50d5e) (v10.x.x)','v10.x.x'),('855c640b8e','2020-05-11 18:21:31.488367','2020-06-12 11:49:31.079506','Administrator','Administrator',0,'Installed Applications','installed_applications','Installed Applications',2,'your_custom_app','0.0.1','master')
				line = line.strip().lstrip(head).rstrip(";").strip()
				app_rows = criscostack.safe_eval(line)
				...
				# 'all_apps' (list) format: [('criscostack', '12.x.x-develop ()', 'develop'), ('your_custom_app', '0.0.1', 'master')]
				...
				for app in all_apps:
					...
					if app_name == "criscostack":
						try:
							current_version = Version(criscostack.__version__)
						...
						if verbose and downgrade:
							print(f"Your site will be downgraded from Crisco {backup_version} to {current_version}")
...
def partial_restore(sql_file_path, verbose=False):
	...
	if criscostack.conf.db_type == "mariadb":
		from criscostack.database.mariadb.setup_db import import_db_from_sql
	elif criscostack.conf.db_type == "postgres":
		import warnings

		from criscostack.database.postgres.setup_db import import_db_from_sql
...
def validate_database_sql(path, _raise=True):
	...
	if _raise and (missing_table or empty_file):
		raise criscostack.InvalidDatabaseFile
