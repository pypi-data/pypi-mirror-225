criscostack.pages["workflow-builder"].on_page_load = function (wrapper) {
	...
	if (criscostack.boot.developer_mode) {
		...
		criscostack.hot_update.push(() => load_workflow_builder(wrapper));
	}
};

criscostack.pages["workflow-builder"].on_page_show = function (wrapper) {
...
function load_workflow_builder(wrapper) {
	...
	if (route.length > 1) {
		criscostack.require("workflow_builder.bundle.js").then(() => {
			criscostack.workflow_builder = new criscostack.ui.WorkflowBuilder({
				wrapper: $parent,
				page: wrapper.page,
				workflow: route[1],
			});
		...
		let d = new criscostack.ui.Dialog({
			...
			primary_action({ action, doctype, workflow, workflow_name }) {
				if (action === "Edit") {
					criscostack.set_route("workflow-builder", workflow);
					...
					criscostack.db
						...
						.then((doc) => {
							criscostack.set_route("workflow-builder", doc.name);
						})
						.finally(() => {
