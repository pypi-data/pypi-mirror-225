...
class TestWorkflow(CriscoTestCase):
	@classmethod
	def setUpClass(cls):
		super().setUpClass()
	...
	def setUp(self):
		...
		if self._testMethodName == "test_if_workflow_actions_were_processed_using_user":
			if not criscostack.db.has_column("Workflow Action", "user"):
				...
				criscostack.cache.delete_value("table_columns")

	def tearDown(self):
		criscostack.delete_doc("Workflow", "Test ToDo")
		if self._testMethodName == "test_if_workflow_actions_were_processed_using_user":
	...
	def test_if_workflow_actions_were_processed_using_role(self):
		criscostack.db.delete("Workflow Action")
		...
		workflow_actions = criscostack.get_all("Workflow Action", fields=["*"])
		...
		workflow_actions = criscostack.get_all("Workflow Action", fields=["status"])
		self.assertEqual(len(workflow_actions), 1)
	...
	def test_if_workflow_actions_were_processed_using_user(self):
		...
		criscostack.qb.update(WorkflowAction).set(WorkflowAction.user, "test2@example.com").run()
		criscostack.qb.update(WorkflowActionPermittedRole).set(WorkflowActionPermittedRole.role, "").run()
...
def create_todo_workflow():
	from criscostack.tests.ui_test_helpers import UI_TEST_USER

	if criscostack.db.exists("Workflow", "Test ToDo"):
		criscostack.delete_doc("Workflow", "Test ToDo")

	TEST_ROLE = "Test Approver"

	if not criscostack.db.exists("Role", TEST_ROLE):
		criscostack.get_doc(dict(doctype="Role", role_name=TEST_ROLE)).insert(ignore_if_duplicate=True)
		if criscostack.db.exists("User", UI_TEST_USER):
			criscostack.get_doc("User", UI_TEST_USER).add_roles(TEST_ROLE)

	workflow = criscostack.new_doc("Workflow")
...
def create_new_todo():
	return criscostack.get_doc(dict(doctype="ToDo", description="workflow " + random_string(10))).insert()
