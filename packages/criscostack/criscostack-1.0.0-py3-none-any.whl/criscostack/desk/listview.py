# Copyright (c) 2022, Crisco Technologies Pvt. Ltd. and Contributors
# License: MIT. See LICENSE

import criscostack
from criscostack.query_builder import Order
from criscostack.query_builder.functions import Count
from criscostack.query_builder.terms import SubQuery
from criscostack.query_builder.utils import DocType


@criscostack.whitelist()
def get_list_settings(doctype):
	try:
		return criscostack.get_cached_doc("List View Settings", doctype)
	except criscostack.DoesNotExistError:
		criscostack.clear_messages()


@criscostack.whitelist()
def set_list_settings(doctype, values):
	try:
		doc = criscostack.get_doc("List View Settings", doctype)
	except criscostack.DoesNotExistError:
		doc = criscostack.new_doc("List View Settings")
		doc.name = doctype
		criscostack.clear_messages()
	doc.update(criscostack.parse_json(values))
	doc.save()


@criscostack.whitelist()
def get_group_by_count(doctype: str, current_filters: str, field: str) -> list[dict]:
	current_filters = criscostack.parse_json(current_filters)

	if field == "assigned_to":
		ToDo = DocType("ToDo")
		User = DocType("User")
		count = Count("*").as_("count")
		filtered_records = criscostack.qb.get_query(
			doctype,
			filters=current_filters,
			fields=["name"],
			validate_filters=True,
		)

		return (
			criscostack.qb.from_(ToDo)
			.from_(User)
			.select(ToDo.allocated_to.as_("name"), count)
			.where(
				(ToDo.status != "Cancelled")
				& (ToDo.allocated_to == User.name)
				& (User.user_type == "System User")
				& (ToDo.reference_name.isin(SubQuery(filtered_records)))
			)
			.groupby(ToDo.allocated_to)
			.orderby(count, order=Order.desc)
			.limit(50)
			.run(as_dict=True)
		)

	return criscostack.get_list(
		doctype,
		filters=current_filters,
		group_by=f"`tab{doctype}`.{field}",
		fields=["count(*) as count", f"`{field}` as name"],
		order_by="count desc",
		limit=50,
	)
