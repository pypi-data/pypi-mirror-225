...
def after_doctype_insert():
	criscostack.db.add_unique("Desktop Icon", ("module_name", "owner", "standard"))
...
def get_desktop_icons(user=None):
	...
	user_icons = criscostack.cache.hget("desktop_icons", user)
	...
	if not user_icons:
		...
		blocked_doctypes = criscostack.get_all(
			"DocType",
		...
		standard_icons = criscostack.get_all("Desktop Icon", fields=fields, filters={"standard": 1})
		...
		user_icons = criscostack.get_all(
		...
		user_blocked_modules = criscostack.get_doc("User", user).get_blocked_modules()
		for icon in user_icons:
		...
		criscostack.cache.hset("desktop_icons", user, user_icons)
...
def add_user_icon(_doctype, _report=None, label=None, link=None, type="link", standard=0):
	...
	icon_name = criscostack.db.exists(
		"Desktop Icon", {"standard": standard, "link": link, "owner": criscostack.session.user}
	)

	if icon_name:
		if criscostack.db.get_value("Desktop Icon", icon_name, "hidden"):
			# if it is hidden, unhide it
			criscostack.db.set_value("Desktop Icon", icon_name, "hidden", 0)
			clear_desktop_icons_cache()

	else:
		idx = (
			criscostack.db.sql("select max(idx) from `tabDesktop Icon` where owner=%s", criscostack.session.user)[0][
				0
			]
			or criscostack.db.sql("select count(*) from `tabDesktop Icon` where standard=1")[0][0]
		)

		if not criscostack.db.get_value("Report", _report):
			_report = None
			userdefined_icon = criscostack.db.get_value(
		...
		module_icon = criscostack.get_value(
			"Desktop Icon",
		...
		except criscostack.UniqueValidationError as e:
			criscostack.throw(_("Desktop Icon already exists"))
	...
	for i, module_name in enumerate(new_order):
		if module_name not in ("Explore",):
			if user:
				...
				name = criscostack.db.get_value("Desktop Icon", {"standard": 1, "module_name": module_name})
				if name:
					icon = criscostack.get_doc("Desktop Icon", name)
				else:
...
def set_desktop_icons(visible_list, ignore_duplicate=True):
	...
	if not int(criscostack.defaults.get_defaults().setup_complete or 0):
		criscostack.db.delete("Desktop Icon", {"standard": 0})

	# set standard as blocked and hidden if setting first active domain
	if not criscostack.flags.keep_desktop_icons:
		criscostack.db.sql("update `tabDesktop Icon` set blocked=0, hidden=1 where standard=1")

	# set as visible if present, or add icon
	for module_name in visible_list:
		name = criscostack.db.get_value("Desktop Icon", {"module_name": module_name})
		if name:
			criscostack.db.set_value("Desktop Icon", name, "hidden", 0)
		else:
			if criscostack.db.exists("DocType", module_name):
				try:
...
def set_hidden(module_name, user=None, hidden=1):
	...
	if user:
		...
		if hidden and icon.custom:
			criscostack.delete_doc(icon.doctype, icon.name, ignore_permissions=True)
			return

		# hidden by user
		icon.db_set("hidden", hidden)
	else:
		icon = criscostack.get_doc("Desktop Icon", {"standard": 1, "module_name": module_name})
...
def get_all_icons():
	return [
		d.module_name
		for d in criscostack.get_all("Desktop Icon", filters={"standard": 1}, fields=["module_name"])
	]


def clear_desktop_icons_cache(user=None):
	criscostack.cache.hdel("desktop_icons", user or criscostack.session.user)
	criscostack.cache.hdel("bootinfo", user or criscostack.session.user)
...
def get_user_copy(module_name, user=None):
	...
	desktop_icon_name = criscostack.db.get_value(
		"Desktop Icon", {"module_name": module_name, "owner": user, "standard": 0}
	)

	if desktop_icon_name:
		return criscostack.get_doc("Desktop Icon", desktop_icon_name)
	else:
...
def make_user_copy(module_name, user):
	...
	standard_name = criscostack.db.get_value("Desktop Icon", {"module_name": module_name, "standard": 1})

	if not standard_name:
		criscostack.throw(_("{0} not found").format(module_name), criscostack.DoesNotExistError)

	original = criscostack.get_doc("Desktop Icon", standard_name)

	desktop_icon = criscostack.get_doc(
...
def sync_from_app(app):
	...
	try:
		modules = criscostack.get_attr(app + ".config.desktop.get_data")() or {}
	except ImportError:
		return []
	...
	for i, m in enumerate(modules_list):
		...
		if desktop_icon_name:
			desktop_icon = criscostack.get_doc("Desktop Icon", desktop_icon_name)
		else:
			# new icon
			desktop_icon = criscostack.get_doc(
		...
		try:
			desktop_icon.save()
		except criscostack.exceptions.UniqueValidationError:
			pass
...
def update_icons(hidden_list, user=None):
	...
	criscostack.msgprint(criscostack._("Updated"), indicator="green", title=_("Success"), alert=True)


def get_context(context):
	context.icons = get_user_icons(criscostack.session.user)
	context.user = criscostack.session.user

	if "System Manager" in criscostack.get_roles():
		context.users = criscostack.get_all(
			"User",
...
def get_module_icons(user=None):
	...
	if not user:
		icons = criscostack.get_all("Desktop Icon", fields="*", filters={"standard": 1}, order_by="idx")
	else:
		criscostack.cache.hdel("desktop_icons", user)
		icons = get_user_icons(user)

	for icon in icons:
		icon.value = criscostack.db.escape(_(icon.label or icon.module_name))
...
def get_user_icons(user):
	...
	from criscostack.boot import get_allowed_pages
