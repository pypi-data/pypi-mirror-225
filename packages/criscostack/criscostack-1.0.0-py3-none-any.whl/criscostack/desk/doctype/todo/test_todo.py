...
import criscostack
from criscostack.core.doctype.doctype.doctype import clear_permissions_cache
from criscostack.model.db_query import DatabaseQuery
from criscostack.permissions import add_permission, reset_perms
from criscostack.tests.utils import CriscoTestCase

test_dependencies = ["User"]


class TestToDo(CriscoTestCase):
	def test_delete(self):
		todo = criscostack.get_doc(
			dict(doctype="ToDo", description="test todo", assigned_by="Administrator")
		).insert()

		criscostack.db.delete("Deleted Document")
		todo.delete()

		deleted = criscostack.get_doc(
	...
	def test_fetch(self):
		...
		self.assertEqual(
			todo.assigned_by_full_name, criscostack.db.get_value("User", todo.assigned_by, "full_name")
		)

	def test_fetch_setup(self):
		criscostack.db.delete("ToDo")

		todo_meta = criscostack.get_doc("DocType", "ToDo")
	...
	def test_doc_read_access(self):
		...
		test_user = criscostack.get_doc("User", "test4@example.com")
...
def create_new_todo(description, assigned_by):
	...
	return criscostack.get_doc(todo).insert()
