...
from criscostack.desk.desktop import save_new_widget
from criscostack.desk.utils import validate_route_conflict
from criscostack.model.document import Document
from criscostack.model.rename_doc import rename_doc
from criscostack.modules.export_file import delete_folder, export_to_files
...
class Workspace(Document):
	...
	if TYPE_CHECKING:
		...
		from criscostack.desk.doctype.workspace_chart.workspace_chart import WorkspaceChart
		from criscostack.desk.doctype.workspace_custom_block.workspace_custom_block import (
			WorkspaceCustomBlock,
		)
		from criscostack.desk.doctype.workspace_link.workspace_link import WorkspaceLink
		from criscostack.desk.doctype.workspace_number_card.workspace_number_card import WorkspaceNumberCard
		from criscostack.desk.doctype.workspace_quick_list.workspace_quick_list import WorkspaceQuickList
		from criscostack.desk.doctype.workspace_shortcut.workspace_shortcut import WorkspaceShortcut
	...
	def validate(self):
		if self.public and not is_workspace_manager() and not disable_saving_as_public():
			criscostack.throw(_("You need to be Workspace Manager to edit this document"))
		...
		try:
			...
			criscostack.throw(_("Content data shoud be a list"))

	def on_update(self):
		if disable_saving_as_public():
			return

		if criscostack.conf.developer_mode and self.public:
			if self.module:
	...
	def after_delete(self):
		...
		if self.module and criscostack.conf.developer_mode:
			delete_folder(self.module, "Workspace", self.title)

	@staticmethod
	def get_module_wise_workspaces():
		workspaces = criscostack.get_all(
			"Workspace",
			fields=["name", "module"],
	...
	def get_link_groups(self):
		cards = []
		current_card = criscostack._dict(
			{
				"label": "Link",
				"type": "Card Break",
				"icon": None,
		...
		for link in self.links:
			...
			if link.type == "Card Break":
				if card_links and (
					...
					or current_card.get("only_for") == criscostack.get_system_settings("country")
				):
...
def disable_saving_as_public():
	return (
		...
		or criscostack.flags.in_uninstall
...
def get_report_type(report):
	report_type = criscostack.get_value("Report", report, "report_type")
...
def new_page(new_page):
	...
	doc = criscostack.new_doc("Workspace")
...
def save_page(title, public, new_widgets, blocks):
	public = criscostack.parse_json(public)

	filters = {"public": public, "label": title}

	if not public:
		filters = {"for_user": criscostack.session.user, "label": title + "-" + criscostack.session.user}
	pages = criscostack.get_all("Workspace", filters=filters)
	if pages:
		doc = criscostack.get_doc("Workspace", pages[0])
...
def update_page(name, title, icon, parent, public):
	public = criscostack.parse_json(public)
	doc = criscostack.get_doc("Workspace", name)
	...
	if doc:
		...
		if doc.public != public:
			doc.sequence_id = criscostack.db.count("Workspace", {"public": public}, cache=True)
			doc.public = public
		doc.for_user = "" if public else doc.for_user or criscostack.session.user
		...
		child_docs = criscostack.get_all(
			"Workspace", filters={"parent_page": doc.title, "public": doc.public}
		)
		if child_docs:
			for child in child_docs:
				child_doc = criscostack.get_doc("Workspace", child.name)
				child_doc.parent_page = doc.title
				if child_doc.public != public:
					child_doc.public = public
				child_doc.for_user = "" if public else child_doc.for_user or criscostack.session.user
...
def hide_unhide_page(page_name: str, is_hidden: bool):
	page = criscostack.get_doc("Workspace", page_name)

	if page.get("public") and not is_workspace_manager():
		criscostack.throw(
			_("Need Workspace Manager role to hide/unhide public workspaces"), criscostack.PermissionError
		)

	if not page.get("public") and page.get("for_user") != criscostack.session.user:
		criscostack.throw(_("Cannot update private workspace of other users"), criscostack.PermissionError)
...
def duplicate_page(page_name, new_page):
	...
	old_doc = criscostack.get_doc("Workspace", page_name)
	doc = criscostack.copy_doc(old_doc)
	...
	doc.module = ""
	if not doc.public:
		doc.for_user = doc.for_user or criscostack.session.user
...
def delete_page(page):
	...
	if criscostack.db.exists("Workspace", page.get("name")):
		criscostack.get_doc("Workspace", page.get("name")).delete(ignore_permissions=True)
...
def sort_pages(sb_public_items, sb_private_items):
	...
	workspace_private_pages = get_page_list(["name", "title"], {"for_user": criscostack.session.user})
	...
	for seq, d in enumerate(pages):
		for page in workspace_pages:
			if page.title == d.get("title"):
				doc = criscostack.get_doc("Workspace", page.name)
...
def last_sequence_id(doc):
	doc_exists = criscostack.db.exists(
...
def get_page_list(fields, filters):
	return criscostack.get_all("Workspace", fields=fields, filters=filters, order_by="sequence_id asc")


def is_workspace_manager():
	return "Workspace Manager" in criscostack.get_roles()
