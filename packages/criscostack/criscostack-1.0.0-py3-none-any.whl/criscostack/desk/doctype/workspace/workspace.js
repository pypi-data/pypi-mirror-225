...
criscostack.ui.form.on("Workspace", {
	setup: function () {
		criscostack.meta.get_field("Workspace Link", "only_for").no_default = true;
	},
	...
	refresh: function (frm) {
		...
		let url = `/app/${
			frm.doc.public
				? criscostack.router.slug(frm.doc.title)
				: "private/" + criscostack.router.slug(frm.doc.title)
		}`;
		frm.sidebar
		...
		if (
			...
			(frm.doc.public &&
				!frm.has_perm("write") &&
				!criscostack.user.has_role("Workspace Manager"))
		) {
