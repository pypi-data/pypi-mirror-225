...
from criscostack.desk.doctype.event.event import get_events
from criscostack.test_runner import make_test_objects
from criscostack.tests.utils import CriscoTestCase

test_records = criscostack.get_test_records("Event")


class TestEvent(CriscoTestCase):
	def setUp(self):
		criscostack.db.delete("Event")
		make_test_objects("Event", reset=True)

		self.test_records = criscostack.get_test_records("Event")
	...
	def test_allowed_public(self):
		criscostack.set_user(self.test_user)
		doc = criscostack.get_doc("Event", criscostack.db.get_value("Event", {"subject": "_Test Event 1"}))
		self.assertTrue(criscostack.has_permission("Event", doc=doc))

	def test_not_allowed_private(self):
		criscostack.set_user(self.test_user)
		doc = criscostack.get_doc("Event", criscostack.db.get_value("Event", {"subject": "_Test Event 2"}))
		self.assertFalse(criscostack.has_permission("Event", doc=doc))

	def test_allowed_private_if_in_event_user(self):
		name = criscostack.db.get_value("Event", {"subject": "_Test Event 3"})
		criscostack.share.add("Event", name, self.test_user, "read")
		criscostack.set_user(self.test_user)
		doc = criscostack.get_doc("Event", name)
		self.assertTrue(criscostack.has_permission("Event", doc=doc))
		criscostack.set_user("Administrator")
		criscostack.share.remove("Event", name, self.test_user)
	...
	def test_revert_logic(self):
		ev = criscostack.get_doc(self.test_records[0]).insert()
		name = ev.name

		criscostack.delete_doc("Event", ev.name)
	...
	def test_assign(self):
		...
		ev = criscostack.get_doc("Event", ev.name)
