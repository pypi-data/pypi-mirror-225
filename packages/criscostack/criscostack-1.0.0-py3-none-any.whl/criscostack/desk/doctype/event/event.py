...
from criscostack.contacts.doctype.contact.contact import get_default_contact
...
from criscostack.utils.user import get_enabled_system_users
...
class Event(Document):
	...
	if TYPE_CHECKING:
		from criscostack.desk.doctype.event_participants.event_participants import EventParticipants
	...
	def validate(self):
		...
		if (
			...
			criscostack.throw(_("Daily Events should finish on the Same Day."))

		if self.sync_with_google_calendar and not self.google_calendar:
			criscostack.throw(_("Select Google Calendar to which event should be synced."))
	...
	def on_trash(self):
		...
		if communications:
			for communication in communications:
				criscostack.delete_doc_if_exists("Communication", communication.name)
	...
	def sync_communication(self):
		if self.event_participants:
			for participant in self.event_participants:
				...
				comms = criscostack.get_all("Communication", filters=filters, fields=["name"])

				if comms:
					for comm in comms:
						communication = criscostack.get_doc("Communication", comm.name)
						self.update_communication(participant, communication)
				else:
					meta = criscostack.get_meta(participant.reference_doctype)
	...
	def update_communication(self, participant, communication):
		...
		communication.sender_full_name = criscostack.utils.get_fullname(self.owner)
		communication.reference_doctype = self.doctype
		communication.reference_name = self.name
	...
	def set_participants_email(self):
		for participant in self.event_participants:
			...
			participant.email = (
				criscostack.get_value("Contact", participant_contact, "email_id") if participant_contact else None
			)


@criscostack.whitelist()
def delete_communication(event, reference_doctype, reference_docname):
	deleted_participant = criscostack.get_doc(reference_doctype, reference_docname)
	...
	comms = criscostack.get_list("Communication", filters=filters, fields=["name"])

	if comms:
		deletion = []
		for comm in comms:
			delete = criscostack.get_doc("Communication", comm.name).delete()
...
def send_event_digest():
	...
	for user in users:
		...
		if events:
			criscostack.set_user_lang(user.name, user.language)
			...
			criscostack.sendmail(
				...
				subject=criscostack._("Upcoming Events for Today"),
				template="upcoming_events",
				args={
					"events": events,
				},
				header=[criscostack._("Events in Today's Calendar"), "blue"],
			)


@criscostack.whitelist()
def get_events(start, end, user=None, for_reminder=False, filters=None) -> list[criscostack._dict]:
	if not user:
		user = criscostack.session.user
...
def delete_events(ref_type, ref_name, delete_event=False):
	participations = criscostack.get_all(
		"Event Participants",
	...
	if participations:
		for participation in participations:
			if delete_event:
				criscostack.delete_doc("Event", participation.parent, for_reload=True)
			else:
				total_participants = criscostack.get_all(
					"Event Participants", filters={"parenttype": "Event", "parent": participation.parent}
				)

				if len(total_participants) <= 1:
					criscostack.db.delete("Event", {"name": participation.parent})
					criscostack.db.delete("Event Participants", {"name": participation.name})


# Close events if ends_on or repeat_till is less than now_datetime
def set_status_of_events():
	events = criscostack.get_list(
	...
	for event in events:
		if (event.ends_on and getdate(event.ends_on) < getdate(nowdate())) or (
			...
			criscostack.db.set_value("Event", event.name, "status", "Closed")
