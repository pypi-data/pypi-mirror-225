...
test_records = criscostack.get_test_records("Print Format")


class TestPrintFormat(CriscoTestCase):
	def test_print_user(self, style=None):
		print_html = criscostack.get_print("User", "Administrator", style=style)
	...
	def test_export_doc(self):
		doc: "PrintFormat" = criscostack.get_doc("Print Format", test_records[0]["name"])

		# this is only to make export_doc happy
		doc.standard = "Yes"
		_before = criscostack.conf.developer_mode
		criscostack.conf.developer_mode = True
		export_path = doc.export_doc()
		criscostack.conf.developer_mode = _before
		...
		with open(exported_doc_path) as f:
			exported_doc = criscostack.parse_json(f.read())
