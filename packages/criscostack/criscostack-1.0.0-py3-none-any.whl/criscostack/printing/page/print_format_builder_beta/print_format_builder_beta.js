criscostack.pages["print-format-builder-beta"].on_page_load = function (wrapper) {
	...
	if (criscostack.boot.developer_mode) {
		criscostack.hot_update = criscostack.hot_update || [];
		criscostack.hot_update.push(() => load_print_format_builder_beta(wrapper));
	}
};

criscostack.pages["print-format-builder-beta"].on_page_show = function (wrapper) {
...
function load_print_format_builder_beta(wrapper) {
	...
	if (route.length > 1) {
		criscostack.require("print_format_builder.bundle.js").then(() => {
			criscostack.print_format_builder = new criscostack.ui.PrintFormatBuilder({
				wrapper: $parent,
				page: wrapper.page,
		...
		let d = new criscostack.ui.Dialog({
			...
			fields: [
				...
				},
				{
					...
					reqd: 1,
					default: criscostack.route_options ? criscostack.route_options.doctype : null,
				},
				{
			...
			primary_action({ action, doctype, print_format, print_format_name }) {
				if (action === "Edit") {
					criscostack.set_route("print-format-builder-beta", print_format);
					...
					criscostack.db
						...
						.then((doc) => {
							criscostack.set_route("print-format-builder-beta", doc.name);
						})
						.finally(() => {
