criscostack.pages["print-format-builder"].on_page_load = function (wrapper) {
	criscostack.print_format_builder = new criscostack.PrintFormatBuilder(wrapper);
	criscostack.breadcrumbs.add("Setup", "Print Format");
};

criscostack.pages["print-format-builder"].on_page_show = function (wrapper) {
	var route = criscostack.get_route();
	if (route.length > 1) {
		criscostack.model.with_doc("Print Format", route[1], function () {
			criscostack.print_format_builder.print_format = criscostack.get_doc("Print Format", route[1]);
			criscostack.print_format_builder.refresh();
		});
	} else if (criscostack.route_options) {
		if (criscostack.route_options.make_new) {
			let { doctype, name, based_on, beta } = criscostack.route_options;
			criscostack.route_options = null;
			criscostack.print_format_builder.setup_new_print_format(doctype, name, based_on, beta);
		} else {
			criscostack.print_format_builder.print_format = criscostack.route_options.doc;
			criscostack.route_options = null;
			criscostack.print_format_builder.refresh();
		}
	}
...
criscostack.PrintFormatBuilder = class PrintFormatBuilder {
	constructor(parent) {
		this.parent = parent;
		this.make();
		this.refresh();
	...
	make() {
		this.page = criscostack.ui.make_app_page({
			parent: this.parent,
		...
		this.page.main = $(
			'<div class="col-md-12 border print-format-builder-main criscostack-card"></div>'
	...
	show_start() {
		this.page.main.html(criscostack.render_template("print_format_builder_start", {}));
	...
	start_edit_print_format() {
		...
		var me = this;
		this.print_format_input = criscostack.ui.form.make_control({
		...
		this.page.main.find(".btn-edit-print-format").on("click", function () {
			...
			if (!name) return;
			criscostack.model.with_doc("Print Format", name, function (doc) {
				criscostack.set_route("print-format-builder", name);
			});
		});
	}
	start_new_print_format() {
		var me = this;
		this.doctype_input = criscostack.ui.form.make_control({
		...
		this.name_input = criscostack.ui.form.make_control({
		...
		this.page.main.find(".btn-new-print-format").on("click", function () {
			...
			if (!(doctype && name)) {
				criscostack.msgprint(__("Both DocType and Name required"));
				return;
			}
	...
	setup_new_print_format(doctype, name, based_on, beta) {
		criscostack.call({
			method: "criscostack.printing.page.print_format_builder.print_format_builder.create_custom_format",
			args: {
				doctype: doctype,
				name: name,
				based_on: based_on,
	...
	setup_print_format() {
		var me = this;
		criscostack.model.with_doctype(this.print_format.doc_type, function (doctype) {
			me.meta = criscostack.get_meta(me.print_format.doc_type);
			me.setup_sidebar();
			me.render_layout();
			...
			me.page.add_inner_button(__("Edit Properties"), function () {
				criscostack.set_route("Form", "Print Format", me.print_format.name);
			});
		});
	}
	setup_sidebar() {
		...
		this.page.sidebar.html(
			$(criscostack.render_template("print_format_builder_sidebar", { fields: fields }))
		);
	...
	render_layout() {
		...
		this.prepare_data();
		$(
			criscostack.render_template("print_format_builder_layout", {
		...
		$.each(this.data, function (i, f) {
			...
			if (!f.name && f.fieldname) {
				...
				if (f.fieldname === "_custom_html") {
					...
					f = $.extend(
						criscostack.meta.get_docfield(me.print_format.doc_type, f.fieldname) || {},
						f
					);
				}
			}
			...
			if (f.fieldtype === "Section Break") {
				...
				set_column();
			} else if (!in_list(criscostack.model.layout_fields, f.fieldtype)) {
	...
	init_visible_columns(f) {
		f.visible_columns = [];
		$.each(criscostack.get_meta(f.options).fields, function (i, _f) {
			if (
			...
			onAdd: function (evt) {
				...
				if (!$item.hasClass("print-format-builder-field")) {
					...
					if (fieldname === "_custom_html") {
						...
						field = criscostack.meta.get_docfield(me.print_format.doc_type, fieldname);
					}

					var html = criscostack.render_template("print_format_builder_field", {
						field: field,
						me: me,
					});
	...
	setup_column_selector() {
		...
		this.page.main.on("click", ".select-columns", function () {
			...
			var doc_fields = criscostack.get_meta(doctype).fields;
			...
			// render checkboxes
			$(
				criscostack.render_template("print_format_builder_column_selector", {
					fields: fields,
	...
	save_print_format() {
		...
		// save format_data
		criscostack.call({
			method: "criscostack.client.set_value",
			args: {
			...
			callback: function (r) {
				...
				criscostack.show_alert({ message: __("Saved"), indicator: "green" });
			},
		});
	}
};
