criscostack.pages["print"].on_page_load = function (wrapper) {
	criscostack.ui.make_app_page({
		parent: wrapper,
	});

	let print_view = new criscostack.ui.form.PrintView(wrapper);

	$(wrapper).bind("show", () => {
		const route = criscostack.get_route();
		const doctype = route[1];
		const docname = route.slice(2).join("/");
		if (!criscostack.route_options || !criscostack.route_options.frm) {
			criscostack.model.with_doc(doctype, docname, () => {
				let frm = { doctype: doctype, docname: docname };
				frm.doc = criscostack.get_doc(doctype, docname);
				criscostack.model.with_doctype(doctype, () => {
					frm.meta = criscostack.get_meta(route[1]);
					print_view.show(frm);
				});
			});
		} else {
			print_view.frm = criscostack.route_options.frm.doctype
				? criscostack.route_options.frm
				: criscostack.route_options.frm.frm;
			criscostack.route_options.frm = null;
			print_view.show(print_view.frm);
		}
	});
};

criscostack.ui.form.PrintView = class {
	constructor(wrapper) {
	...
	make() {
		this.print_wrapper = this.page.main.empty().html(
			`<div class="print-preview-wrapper"><div class="print-preview">
				${criscostack.render_template("print_skeleton_loading")}
		...
		this.print_settings = criscostack.model.get_doc(":Print Settings", "Print Settings");
		this.setup_menu();
		this.setup_toolbar();
		this.setup_sidebar();
	...
	add_sidebar_item(df, is_dynamic) {
		...
		let field = criscostack.ui.form.make_control({
			df: df,
	...
	setup_menu() {
		...
		if (criscostack.model.can_create("Print Format")) {
	...
	set_breadcrumbs() {
		criscostack.breadcrumbs.add(this.frm.meta.module, this.frm.doctype);
	}
	...
	setup_additional_settings() {
		...
		criscostack
			.xcall("criscostack.printing.page.print.print.get_print_settings_to_show", {
	...
	edit_print_format() {
		...
		if (is_standard_but_editable) {
			criscostack.set_route("Form", "Print Format", print_format.name);
			return;
		}
		if (is_custom_format) {
			if (print_format.print_format_builder_beta) {
				criscostack.set_route("print-format-builder-beta", print_format.name);
			} else {
				criscostack.set_route("print-format-builder", print_format.name);
			}
			return;
		}
		...
		criscostack.prompt(
			...
			(data) => {
				...
				criscostack.set_route("print-format-builder");
	...
	setup_keyboard_shortcuts() {
		this.wrapper.find(".print-toolbar a.btn-default").each((i, el) => {
			criscostack.ui.keys.get_shortcut_group(this.frm.page).add($(el));
		});
	}
	...
	set_default_print_language() {
		let print_format = this.get_print_format();
		this.lang_code =
			this.frm.doc.language || print_format.default_print_language || criscostack.boot.lang;
	...
	preview() {
		...
		this.get_print_html((out) => {
			...
			const print_height_inches = criscostack.dom.pixel_to_inches(print_height);
	...
	setup_print_format_dom(out, $print_format) {
		...
		let base_url = criscostack.urllib.get_base_url();
		let print_css = criscostack.assets.bundled_asset(
			"print.bundle.css",
			criscostack.utils.is_rtl(this.lang_code)
		);
		this.$print_format_body
			.find("html")
			.attr("dir", criscostack.utils.is_rtl(this.lang_code) ? "rtl" : "ltr");
	...
	go_to_form_view() {
		...
		criscostack.set_route("Form", this.frm.doctype, this.frm.docname);
	}
			...
			if (me.is_raw_printing()) {
				me.get_raw_commands(function (out) {
					criscostack.ui.form
						...
						.then(criscostack.ui.form.qz_success)
						.catch((err) => {
							criscostack.ui.form.qz_fail(err);
						});
				});
			} else {
	...
	print_by_server() {
		...
		if (localStorage.getItem("network_printer")) {
			criscostack.call({
				method: "criscostack.utils.print_format.print_by_server",
				args: {
	...
	network_printer_setting_dialog(callback) {
		criscostack.call({
			method: "criscostack.printing.doctype.network_printer_settings.network_printer_settings.get_network_printer_settings",
			callback: function (r) {
				if (r.message) {
	...
	render_pdf() {
		...
		if (print_format.print_format_builder_beta) {
			...
			let w = window.open(`/api/method/criscostack.utils.weasyprint.download_pdf?${params}`);
			if (!w) {
				criscostack.msgprint(__("Please enable pop-ups"));
				return;
			}
		} else {
			this.render_page("/api/method/criscostack.utils.print_format.download_pdf?");
		}
	}
	...
	get_print_html(callback) {
		...
		this._req = criscostack.call({
			method: "criscostack.www.printview.get_html_and_style",
			args: {
				doc: this.frm.doc,
	...
	get_raw_commands(callback) {
		...
		criscostack.call({
			method: "criscostack.www.printview.get_rendered_raw_commands",
			args: {
				doc: this.frm.doc,
				print_format: this.selected_format(),
				_lang: this.lang_code,
	...
	set_style(style) {
		criscostack.dom.set_style(style || criscostack.boot.print_css, "print-style");
	}
	...
	printer_setting_dialog() {
		...
		this.printer_list = [];
		criscostack.ui.form.qz_get_printer_list().then((data) => {
						...
						fields: [
							{
								...
								fieldname: "print_format",
								default: 0,
								options: criscostack.meta.get_print_formats(this.frm.doctype),
								read_only: 0,
								in_list_view: 1,
								label: __("Print Format"),
							},
			...
			if (!(this.printer_list && this.printer_list.length)) {
				criscostack.throw(__("No Printer is Available."));
			}
		});
	}
};
