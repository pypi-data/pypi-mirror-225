...
import criscostack
from criscostack import _, safe_encode, task
from criscostack.core.utils import html2text
from criscostack.email.doctype.email_account.email_account import EmailAccount
from criscostack.email.email_body import add_attachment, get_email, get_formatted_html
from criscostack.email.queue import get_unsubcribed_url, get_unsubscribe_message
from criscostack.email.smtp import SMTPServer
from criscostack.model.document import Document
from criscostack.query_builder import DocType, Interval
...
class EmailQueue(Document):
	...
	if TYPE_CHECKING:
		from criscostack.email.doctype.email_queue_recipient.email_queue_recipient import EmailQueueRecipient
	...
	def prevent_email_queue_delete(self):
		if criscostack.session.user != "Administrator":
			criscostack.throw(_("Only Administrator can delete Email Queue"))

	def get_duplicate(self, recipients):
		values = self.as_dict()
		del values["name"]
		duplicate = criscostack.get_doc(values)
	...
	def new(cls, doc_data, ignore_permissions=False):
		...
		doc = criscostack.new_doc(cls.DOCTYPE)
		doc.update(data)
	...
	def update_status(self, status, commit=False, **kwargs):
		...
		if self.communication:
			communication_doc = criscostack.get_doc("Communication", self.communication)
	...
	def get_email_account(self):
		if self.email_account:
			return criscostack.get_cached_doc("Email Account", self.email_account)
	...
	def can_send_now(self):
		if (
			criscostack.are_emails_muted()
			or not self.is_to_be_sent()
			or cint(criscostack.db.get_default("suspend_email_queue")) == 1
		):
			return False
	...
	def send(self, smtp_server_instance: SMTPServer = None):
		...
		with SendMailContext(self, smtp_server_instance) as ctx:
			...
			if criscostack.flags.in_test:
				criscostack.flags.sent_mail = message
				return
	...
	def clear_old_logs(days=30):
		...
		"""
		days = days or 31
		email_queue = criscostack.qb.DocType("Email Queue")
		email_recipient = criscostack.qb.DocType("Email Queue Recipient")

		# Delete queue table
		(
			criscostack.qb.from_(email_queue)
			.delete()
		...
		(
			criscostack.qb.from_(email_recipient)
			.delete()
...
class SendMailContext:
	...
	def get_tracker_str(self, recipient_email) -> str:
		...
		elif criscostack.conf.use_ssl and self.email_account_doc.track_email_status:
			tracker_url = f"{get_url()}/api/method/criscostack.core.doctype.communication.email.mark_email_as_seen?name={self.queue_doc.communication}"
	...
	def include_attachments(self, message):
		...
		for attachment in attachments:
			...
			if file_filters:
				_file = criscostack.get_doc("File", file_filters)
				...
				print_format_file = criscostack.attach_print(**attachment)
...
def bulk_retry(queues):
	...
	email_queue = criscostack.qb.DocType("Email Queue")
	criscostack.qb.update(email_queue).set(email_queue.status, "Not Sent").set(
		email_queue.modified, now()
	).set(email_queue.modified_by, criscostack.session.user).where(
...
def toggle_sending(enable):
	criscostack.only_for("System Manager")
	criscostack.db.set_default("suspend_email_queue", 0 if sbool(enable) else 1)


def on_doctype_update():
	"""Add index in `tabCommunication` for `(reference_doctype, reference_name)`"""
	criscostack.db.add_index(
		"Email Queue", ("status", "send_after", "priority", "creation"), "index_bulk_flush"
	)

	criscostack.db.add_index("Email Queue", ["message_id(140)"])


def get_email_retry_limit():
	return cint(criscostack.db.get_system_setting("email_retry_limit")) or 3
...
class QueueBuilder:
	...
	def __init__(
		...
		:param unsubscribe_method: URL method for unsubscribe. Default is `/api/method/criscostack.email.queue.unsubscribe`.
	...
	def unsubscribe_method(self):
		return self._unsubscribe_method or "/api/method/criscostack.email.queue.unsubscribe"
	...
	def get_unsubscribed_user_emails(self):
		...
		if len(all_ids) > 0:
			unsubscribed = (
				criscostack.qb.from_(EmailUnsubscribe)
			...
			for att in self._attachments:
				if att.get("fid") or att.get("file_url"):
					...
					if not att.get("lang", None):
						att["lang"] = criscostack.local.lang
	...
	def process(self, send_now=False):
		...
		if not queue_separately:
			...
			if send_now and len(final_recipients) >= 1000:
				...
				send_now = False
			for recipients in criscostack.utils.create_batch(final_recipients, 1000):
				criscostack.enqueue(
					self.send_emails,
					queue_data=queue_data,
					final_recipients=recipients,
					job_name=criscostack.utils.get_job_name(
						"send_bulk_emails_for", self.reference_doctype, self.reference_name
					),
					now=criscostack.flags.in_test or send_now,
					queue="long",
				)
	...
	def as_dict(self, include_recipients=True):
		...
		except criscostack.InvalidEmailAddressError:
			# bad Email Address - don't add to queue
			criscostack.log_error(
