...
def create_email_flag_queue(names, action):
	...
	def mark_as_seen_unseen(name, action):
		doc = criscostack.get_doc("Communication", name)
		if action == "Read":
			doc.add_seen()
		else:
			_seen = json.loads(doc._seen or "[]")
			_seen = [user for user in _seen if criscostack.session.user != user]
	...
	for name in json.loads(names or []):
		uid, seen_status, email_account = criscostack.db.get_value(
		...
		if (action == "Read" and seen_status == 0) or (action == "Unread" and seen_status == 1):
			create_new = True
			email_flag_queue = criscostack.db.sql(
			...
			for queue in email_flag_queue:
				if queue.action != action:
					criscostack.delete_doc("Email Flag Queue", queue.name, ignore_permissions=True)
				elif queue.action == action:
					# Read or Unread request for email is already available
					create_new = False

			if create_new:
				flag_queue = criscostack.get_doc(
					{
						"uid": uid,
						"action": action,
						"communication": name,
				...
				criscostack.db.set_value("Communication", name, "seen", seen, update_modified=False)
...
def mark_as_closed_open(communication, status):
	...
	criscostack.db.set_value("Communication", communication, "status", status)


@criscostack.whitelist()
def move_email(communication, email_account):
	"""Move email to another email account."""
	criscostack.db.set_value("Communication", communication, "email_account", email_account)


@criscostack.whitelist()
def mark_as_trash(communication):
	"""Set email status to trash."""
	criscostack.db.set_value("Communication", communication, "email_status", "Trash")


@criscostack.whitelist()
def mark_as_spam(communication, sender):
	"""Set email status to spam."""
	email_rule = criscostack.db.get_value("Email Rule", {"email_id": sender})
	if not email_rule:
		criscostack.get_doc({"doctype": "Email Rule", "email_id": sender, "is_spam": 1}).insert(
			ignore_permissions=True
		)
	criscostack.db.set_value("Communication", communication, "email_status", "Spam")
