...
import criscostack
from criscostack.query_builder import Interval, Order
from criscostack.query_builder.functions import Date, Sum, UnixTimestamp
...
def get_energy_points_heatmap_data(user, date):
	...
	eps_log = criscostack.qb.DocType("Energy Point Log")

	return dict(
		criscostack.qb.from_(eps_log)
...
def get_user_rank(user):
	...
	monthly_rank = criscostack.get_all(
		"Energy Point Log",
	...
	all_time_rank = criscostack.get_all(
		"Energy Point Log",
		group_by="`tabEnergy Point Log`.`user`",
		filters={"type": ["!=", "Review"]},
		fields=["user", "sum(points)"],
...
def update_profile_info(profile_info):
	profile_info = criscostack.parse_json(profile_info)
