...
from criscostack.build import scrub_html_template
from criscostack.model.meta import Meta
from criscostack.model.utils import render_include
from criscostack.modules import get_module_path, load_doctype_module, scrub
from criscostack.utils import get_html_format
...
def get_meta(doctype, cached=True) -> "FormMeta":
	...
	cached = cached and not criscostack.conf.developer_mode
	if cached:
		meta = criscostack.cache.hget("doctype_form_meta", doctype)
		if not meta:
			# Cache miss - explicitly get meta from DB to avoid
			meta = FormMeta(doctype, cached=False)
			criscostack.cache.hset("doctype_form_meta", doctype, meta)
	else:
		meta = FormMeta(doctype)

	if criscostack.local.lang != "en":
		meta.set_translations(criscostack.local.lang)
...
class FormMeta(Meta):
	def __init__(self, doctype, *, cached=True):
		self.__dict__.update(criscostack.get_meta(doctype, cached=cached).__dict__)
		self.load_assets()
	...
	def add_code(self):
		...
		system_country = criscostack.get_system_settings("country")
...
"""
	...
	def add_search_fields(self):
		...
		for df in self.get("fields", {"fieldtype": "Link", "options": ["!=", "[Select]"]}):
			if df.options:
				try:
					search_fields = criscostack.get_meta(df.options).search_fields
	...
	def _show_missing_doctype_msg(self, df):
		...
		msg = _("Field {0} is referring to non-existing doctype {1}.").format(
			criscostack.bold(df.fieldname), criscostack.bold(df.options)
		)
		...
		criscostack.throw(msg, title=_("Missing DocType"))

	def add_linked_document_type(self):
		for df in self.get("fields", {"fieldtype": "Link"}):
			if df.options:
				try:
					df.linked_document_type = criscostack.get_meta(df.options).document_type
				except criscostack.DoesNotExistError:
					self._show_missing_doctype_msg(df)

	def load_print_formats(self):
		print_formats = criscostack.db.sql(
	...
	def load_workflows(self):
		...
		if workflow_name and criscostack.db.exists("Workflow", workflow_name):
			workflow = criscostack.get_doc("Workflow", workflow_name)
			workflow_docs.append(workflow)

			for d in workflow.get("states"):
				workflow_docs.append(criscostack.get_doc("Workflow State", d.state))
		...
		if not self.custom:
			...
			if hasattr(module, "form_grid_templates"):
				for key, path in module.form_grid_templates.items():
					templates[key] = get_html_format(criscostack.get_app_path(app, path))

				self.set("__form_grid_templates", templates)

	def set_translations(self, lang):
		from criscostack.translate import extract_messages_from_code, make_dict_from_messages

		self.set("__messages", criscostack.get_lang_dict("doctype", self.name))
...
def get_code_files_via_hooks(hook, name):
	...
	for app_name in criscostack.get_installed_apps():
		code_hook = criscostack.get_hooks(hook, default={}, app_name=app_name)
		if not code_hook:
			continue
		...
		for file in files:
			path = criscostack.get_app_path(app_name, *file.strip("/").split("/"))
