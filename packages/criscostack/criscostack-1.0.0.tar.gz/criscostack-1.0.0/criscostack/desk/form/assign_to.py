...
class DuplicateToDoError(criscostack.ValidationError):
	pass
...
def add(args=None):
	...
	for assign_to in criscostack.parse_json(args.get("assign_to")):
		filters = {
		...
		if criscostack.get_all("ToDo", filters=filters):
			users_with_duplicate_todo.append(assign_to)
		else:
			from criscostack.utils import nowdate
			...
			d = criscostack.get_doc(
				{
					...
					"assigned_by": args.get("assigned_by", criscostack.session.user),
					"assignment_rule": args.get("assignment_rule"),
				}
			).insert(ignore_permissions=True)

			# set assigned_to if field exists
			if criscostack.get_meta(args["doctype"]).get_field("assigned_to"):
				criscostack.db.set_value(args["doctype"], args["name"], "assigned_to", assign_to)

			doc = criscostack.get_doc(args["doctype"], args["name"])

			# if assignee does not have permissions, share or inform
			if not criscostack.has_permission(doc=doc, user=assign_to):
				if criscostack.get_system_settings("disable_document_sharing"):
					msg = _("User {0} is not permitted to access this document.").format(criscostack.bold(assign_to))
					msg += "<br>" + _(
						"As document sharing is disabled, please give them the required permissions before assigning."
					)
					criscostack.throw(msg, title=_("Missing Permission"))
				else:
					criscostack.share.add(doc.doctype, doc.name, assign_to)
					shared_with_users.append(assign_to)

			# make this document followed by assigned user
			if criscostack.get_cached_value("User", assign_to, "follow_assigned_documents"):
	...
	if users_with_duplicate_todo:
		...
		criscostack.msgprint(_("Already in the following Users ToDo list:{0}").format(user_list, alert=True))
...
def set_status(doctype, name, todo=None, assign_to=None, status="Cancelled"):
	...
	try:
		...
		if todo:
			todo = criscostack.get_doc("ToDo", todo)
			todo.status = status
	...
	if criscostack.get_meta(doctype).get_field("assigned_to") and status == "Cancelled":
		criscostack.db.set_value(doctype, name, "assigned_to", None)
...
def notify_assignment(
	...
	if assigned_by == allocated_to or not criscostack.db.get_value("User", allocated_to, "enabled"):
		return

	# Search for email address in description -- i.e. assignee
	user_name = criscostack.get_cached_value("User", criscostack.session.user, "full_name")
	...
	if action == "CLOSE":
		subject = _("Your assignment on {0} {1} has been removed by {2}").format(
			criscostack.bold(doc_type), get_title_html(title), criscostack.bold(user_name)
		)
	else:
		user_name = criscostack.bold(user_name)
		document_type = criscostack.bold(doc_type)
	...
	notification_doc = {
		...
		"from_user": criscostack.session.user,
