...
def get_notifications():
	...
	if criscostack.flags.in_install or not criscostack.db.get_single_value("System Settings", "setup_complete"):
		return out
	...
	for name in groups:
		count = criscostack.cache.hget("notification_count:" + name, criscostack.session.user)
...
def get_notifications_for_doctypes(config, notification_count):
	...
	can_read = criscostack.get_user().get_can_read()
	open_count_doctype = {}
			...
			if d in notification_count:
				...
				try:
					if isinstance(condition, dict):
						...
						result = criscostack.get_attr(condition)()

				except criscostack.PermissionError:
					criscostack.clear_messages()
					pass
					# criscostack.msgprint("Permission Error in notifications for {0}".format(d))
					...
					criscostack.cache.hset("notification_count:" + d, criscostack.session.user, result)
...
def clear_notifications(user=None):
	...
	for name in groups:
		if user:
			criscostack.cache.hdel("notification_count:" + name, user)
		else:
			criscostack.cache.delete_key("notification_count:" + name)


def clear_notification_config(user):
	criscostack.cache.hdel("notification_config", user)


def delete_notification_count_for(doctype):
	criscostack.cache.delete_key("notification_count:" + doctype)
...
def get_notification_info():
	...
	conditions = {}
	module_doctypes = {}
	doctype_info = dict(criscostack.db.sql("""select name, module from tabDocType"""))
...
def get_notification_config():
	...
	return criscostack.cache.hget("notification_config", user, _get)
...
def get_open_count(doctype, name, items=None):
	...
	if criscostack.flags.in_migrate or criscostack.flags.in_install:
		return {"count": []}
	...
	for d in items:
		...
		if filters:
			...
			total = len(
				criscostack.get_all(d, fields="name", filters=filters, limit=100, distinct=True, ignore_ifnull=True)
			)
	...
	if not meta.custom:
		module = criscostack.get_meta_module(doctype)
...
def notify_mentions(ref_doctype, ref_name, content):
	if ref_doctype and ref_name and content:
		...
		sender_fullname = get_fullname(criscostack.session.user)
		...
		notification_message = _("""{0} mentioned you in a comment in {1} {2}""").format(
			criscostack.bold(sender_fullname), criscostack.bold(ref_doctype), get_title_html(title)
		)
...
def extract_mentions(txt):
	...
	for mention in soup.find_all(class_="mention"):
		if mention.get("data-is-group") == "true":
			try:
				user_group = criscostack.get_cached_doc("User Group", mention["data-id"])
