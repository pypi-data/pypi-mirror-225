...
from criscostack.boot import get_allowed_report_names
...
from criscostack.utils import cint, get_datetime, getdate, has_common, now_datetime, nowdate
from criscostack.utils.dashboard import cache_source
from criscostack.utils.data import format_date
from criscostack.utils.dateutils import (
...
def get_permission_query_conditions(user):
	...
	allowed_doctypes = [
		criscostack.db.escape(doctype) for doctype in criscostack.permissions.get_doctypes_with_read()
	]
	allowed_reports = [criscostack.db.escape(report) for report in get_allowed_report_names()]
	allowed_modules = [
		criscostack.db.escape(module.get("module_name")) for module in get_modules_from_all_apps_for_user()
	]
...
def has_permission(doc, ptype, user):
	...
	if doc.chart_type == "Report":
		...
		allowed_doctypes = criscostack.permissions.get_doctypes_with_read()
...
def get(
	...
	if chart_name:
		chart = criscostack.get_doc("Dashboard Chart", chart_name)
	else:
		chart = criscostack._dict(criscostack.parse_json(chart))
	...
	filters = criscostack.parse_json(filters) or criscostack.parse_json(chart.filters_json)
	if not filters:
		filters = []
...
def create_dashboard_chart(args):
	...
	doc = criscostack.new_doc("Dashboard Chart")
	...
	if criscostack.db.exists("Dashboard Chart", args.chart_name):
...
def add_chart_to_dashboard(args):
	...
	dashboard = criscostack.get_doc("Dashboard", args.dashboard)
	dashboard_link = criscostack.new_doc("Dashboard Chart Link")
	dashboard_link.chart = args.chart_name or args.name

	if args.set_standard and dashboard.is_standard:
		chart = criscostack.get_doc("Dashboard Chart", dashboard_link.chart)
		chart.is_standard = 1
...
def get_charts_for_user(doctype, txt, searchfield, start, page_len, filters):
	or_filters = {"owner": criscostack.session.user, "is_public": 1}
...
class DashboardChart(Document):
	...
	if TYPE_CHECKING:
		...
		from criscostack.desk.doctype.dashboard_chart_field.dashboard_chart_field import DashboardChartField
	...
	def on_update(self):
		criscostack.cache.delete_key(f"chart-data:{self.name}")
	...
	def validate(self):
		if not criscostack.conf.developer_mode and self.is_standard:
			criscostack.throw(_("Cannot edit Standard charts"))
	...
	def check_required_field(self):
		if not self.document_type:
			criscostack.throw(_("Document type is required to create a dashboard chart"))

		if (
			self.document_type
			and criscostack.get_meta(self.document_type).istable
			and not self.parent_document_type
		):
			criscostack.throw(_("Parent document type is required to create a dashboard chart"))

		if self.chart_type == "Group By":
			if not self.group_by_based_on:
				criscostack.throw(_("Group By field is required to create a dashboard chart"))
			if self.group_by_type in ["Sum", "Average"] and not self.aggregate_function_based_on:
				criscostack.throw(_("Aggregate Function field is required to create a dashboard chart"))
		else:
			if not self.based_on:
				criscostack.throw(_("Time series based on is required to create a dashboard chart"))

	def check_document_type(self):
		if criscostack.get_meta(self.document_type).issingle:
			criscostack.throw(_("You cannot create a dashboard chart from single DocTypes"))
	...
	def validate_custom_options(self):
		if self.custom_options:
			try:
				...
				criscostack.throw(_("Invalid json added in the custom options: {0}").format(error))
