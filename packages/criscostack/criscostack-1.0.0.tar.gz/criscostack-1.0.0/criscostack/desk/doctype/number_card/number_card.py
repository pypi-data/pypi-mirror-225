...
from criscostack.query_builder import Criterion
...
class NumberCard(Document):
	...
	def autoname(self):
		...
		if criscostack.db.exists("Number Card", self.name):
	...
	def validate(self):
		if self.type == "Document Type":
			if not (self.document_type and self.function):
				criscostack.throw(_("Document Type and Function are required to create a number card"))

			if self.function != "Count" and not self.aggregate_function_based_on:
				criscostack.throw(_("Aggregate Field is required to create a number card"))

			if criscostack.get_meta(self.document_type).istable and not self.parent_document_type:
				criscostack.throw(_("Parent Document Type is required to create a number card"))

		elif self.type == "Report":
			if not (self.report_name and self.report_field and self.function):
				criscostack.throw(_("Report Name, Report Field and Fucntion are required to create a number card"))

		elif self.type == "Custom":
			if not self.method:
				criscostack.throw(_("Method is required to create a number card"))
...
def has_permission(doc, ptype, user):
	...
	if doc.type == "Report":
		...
		allowed_doctypes = tuple(criscostack.permissions.get_doctypes_with_read())
...
def get_result(doc, filters, to_date=None):
	...
	if not filters:
		...
		filters = criscostack.parse_json(filters)

	if to_date:
		filters.append([doc.document_type, "creation", "<", to_date])

	res = criscostack.get_list(
...
def get_percentage_difference(doc, filters, result):
	doc = criscostack.parse_json(doc)
	result = criscostack.parse_json(result)

	doc = criscostack.get_doc("Number Card", doc.name)
...
def calculate_previous_result(doc, filters):
	from criscostack.utils import add_to_date

	current_date = criscostack.utils.now()
...
def create_number_card(args):
	...
	doc = criscostack.new_doc("Number Card")
...
def get_cards_for_user(doctype, txt, searchfield, start, page_len, filters):
	...
	condition_query = criscostack.qb.get_query(
		doctype,
		filters=filters,
	...
	return (
		...
		.where((numberCard.owner == criscostack.session.user) | (numberCard.is_public == 1))
...
def create_report_number_card(args):
	...
	args.name = card.name
	if args.dashboard:
		add_card_to_dashboard(criscostack.as_json(args))
...
def add_card_to_dashboard(args):
	...
	dashboard_link = criscostack.new_doc("Number Card Link")
	dashboard_link.card = args.name

	if args.set_standard and dashboard.is_standard:
		card = criscostack.get_doc("Number Card", dashboard_link.card)
		card.is_standard = 1
