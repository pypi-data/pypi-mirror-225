...
from criscostack.model import child_table_fields, default_fields, get_permitted_fields, optional_fields
...
from criscostack.utils import add_user_info, format_duration
...
def get_list():
	...
	if is_virtual_doctype(args.doctype):
		...
		# uncompressed (refactored from criscostack.model.db_query.get_list)
		data = execute(**args)
...
def get_form_params():
	...
	data = criscostack._dict(criscostack.local.form_dict)
	clean_params(data)
	validate_args(data)
	return data
	...
	if isinstance(filters, list):
		...
		for condition in filters:
			if len(condition) == 3:
				...
				if is_standard(fieldname):
					continue
				meta = criscostack.get_meta(condition[0])
...
def setup_group_by(data):
	...
	if data.group_by and data.aggregate_function:
		if data.aggregate_function.lower() not in ("count", "sum", "avg"):
			criscostack.throw(_("Invalid aggregate function"))

		if criscostack.db.has_column(data.aggregate_on_doctype, data.aggregate_on_field):
			data.fields.append(
...
def raise_invalid_field(fieldname):
	criscostack.throw(_("Field not permitted in query") + f": {fieldname}", criscostack.DataError)
...
def get_meta_and_docfield(fieldname, data):
	...
	meta = criscostack.get_meta(parenttype)
...
def get_parenttype_and_fieldname(field, data):
	if "." in field:
		...
		if parenttype.startswith("`tab"):
			...
			parenttype = criscostack.get_meta(data.doctype).get_field(parenttype).options
	else:
...
def compress(data, args=None):
	...
	from criscostack.desk.query_report import add_total_row
	...
	if args.get("add_total_row"):
		meta = criscostack.get_meta(args.doctype)
...
def save_report(name, doctype, report_settings):
	...
	if criscostack.db.exists("Report", name):
		report = criscostack.get_doc("Report", name)
		if report.is_standard == "Yes":
			criscostack.throw(_("Standard Reports cannot be edited"))

		if report.report_type != "Report Builder":
			criscostack.throw(_("Only reports of type Report Builder can be edited"))

		if report.owner != criscostack.session.user and not report.has_permission("write"):
			criscostack.throw(_("Insufficient Permissions for editing Report"), criscostack.PermissionError)
	else:
		report = criscostack.new_doc("Report")
	...
	criscostack.msgprint(
		_("Report {0} saved").format(criscostack.bold(report.name)),
		indicator="green",
		alert=True,
	)
	return report.name
...
def delete_report(name):
	...
	report = criscostack.get_doc("Report", name)
	if report.is_standard == "Yes":
		criscostack.throw(_("Standard Reports cannot be deleted"))

	if report.report_type != "Report Builder":
		criscostack.throw(_("Only reports of type Report Builder can be deleted"))

	if report.owner != criscostack.session.user and not report.has_permission("delete"):
		criscostack.throw(_("Insufficient Permissions for deleting Report"), criscostack.PermissionError)

	report.delete(ignore_permissions=True)
	criscostack.msgprint(
		_("Report {0} deleted").format(criscostack.bold(report.name)),
		indicator="green",
		alert=True,
	)
...
def export_query():
	...
	from criscostack.desk.utils import get_csv_bytes, pop_csv_params, provide_binary_file
	...
	criscostack.permissions.can_export(doctype, raise_exception=True)
	...
	if file_format_type == "CSV":
		...
		content = get_csv_bytes(
			[[handle_html(criscostack.as_unicode(v)) if isinstance(v, str) else v for v in r] for r in data],
			csv_params,
		)
	elif file_format_type == "Excel":
		from criscostack.utils.xlsxutils import make_xlsx
...
def get_labels(fields, doctype):
	...
	for key in fields:
		...
		if parenttype == doctype and fieldname == "name":
			...
			df = criscostack.get_meta(parenttype).get_field(fieldname)
...
def delete_items():
	...
	items = sorted(json.loads(criscostack.form_dict.get("items")), reverse=True)
	doctype = criscostack.form_dict.get("doctype")

	if len(items) > 10:
		criscostack.enqueue("criscostack.desk.reportview.delete_bulk", doctype=doctype, items=items)
	else:
...
def delete_bulk(doctype, items):
	...
	for i, d in enumerate(items):
		try:
			criscostack.delete_doc(doctype, d)
			if len(items) >= 5:
...
def get_stats(stats, doctype, filters=None):
	...
	try:
		db_columns = criscostack.db.get_table_columns(doctype)
	except (criscostack.db.InternalError, criscostack.db.ProgrammingError):
	...
	for column in columns:
		if column not in db_columns:
			continue
		try:
			tag_count = criscostack.get_list(
				doctype,
				fields=[column, "count(*)"],
			...
			if column == "_user_tags":
				...
				no_tag_count = criscostack.get_list(
					doctype,
					fields=[column, "count(*)"],
		...
		except criscostack.db.SQLError:
			pass
		except criscostack.db.InternalError as e:
			# raised when _user_tags column is added on the fly
			pass
