...
from criscostack.core.utils import ljust_list
from criscostack.desk.reportview import clean_params, parse_json
from criscostack.model.utils import render_include
...
from criscostack.utils import cint, cstr, flt, format_duration, get_html_format, sbool


def get_report_doc(report_name):
	doc = criscostack.get_doc("Report", report_name)
	doc.custom_columns = []
	doc.custom_filters = []
	...
	if doc.report_type == "Custom Report":
		...
		doc = criscostack.get_doc("Report", reference_report)
	...
	if not criscostack.has_permission(doc.ref_doctype, "report"):
		criscostack.throw(
	...
	if doc.disabled:
		criscostack.throw(_("Report {0} is disabled").format(report_name))
...
def generate_report_result(
	...
	return {
		...
		"status": None,
		"execution_time": criscostack.cache.hget("report_execution_time", report.name) or 0,
	}
...
def get_script(report_name):
	...
	module = report.module or criscostack.db.get_value("DocType", report.ref_doctype, "module")

	is_custom_module = criscostack.get_cached_value("Module Def", module, "custom")
	...
	if not script:
		script = "criscostack.query_reports['%s']={}" % report_name

	return {
		"script": render_include(script),
		"html_format": html_format,
		"execution_time": criscostack.cache.hget("report_execution_time", report_name) or 0,
...
def run(
	...
	if not criscostack.has_permission(report.ref_doctype, "report"):
...
def get_prepared_report_result(report, filters, dn="", user=None):
	from criscostack.core.doctype.prepared_report.prepared_report import get_completed_prepared_report
	...
	doc = criscostack.get_doc("Prepared Report", dn) if dn else None
	if doc:
		try:
			...
			criscostack.msgprint(_("Prepared report render failed"))
			doc = None
...
def export_query():
	...
	form_params = criscostack._dict(criscostack.local.form_dict)
	csv_params = pop_csv_params(form_params)
	clean_params(form_params)
	parse_json(form_params)

	report_name = form_params.report_name
	criscostack.permissions.can_export(
		criscostack.get_cached_value("Report", report_name, "ref_doctype"),
		raise_exception=True,
	)

	file_format_type = form_params.file_format_type
	custom_columns = criscostack.parse_json(form_params.custom_columns or "[]")
...
def format_duration_fields(data: criscostack._dict) -> None:
...
def get_data_for_custom_field(doctype, field):
	...
	if not criscostack.has_permission(doctype, "read"):
		criscostack.throw(_("Not Permitted to read {0}").format(doctype), criscostack.PermissionError)

	value_map = criscostack._dict(criscostack.get_all(doctype, fields=["name", field], as_list=1))
...
def save_report(reference_report, report_name, columns, filters):
	...
	if docname:
		report = criscostack.get_doc("Report", docname)
		...
		report.save()
		criscostack.msgprint(_("Report updated successfully"))

		return docname
	else:
		new_report = criscostack.get_doc(
			{
				"doctype": "Report",
				"report_name": report_name,
		...
		criscostack.msgprint(_("{0} saved successfully").format(new_report.name))
...
def get_filtered_data(ref_doctype, columns, data, user):
	...
	shared = criscostack.share.get_shared(ref_doctype, user)
	columns_dict = get_columns_dict(columns)

	role_permissions = get_role_permissions(criscostack.get_meta(ref_doctype), user)
			...
			for match_filters in filter_list:
				...
				for dt, idx in linked_doctypes.items():
					...
					if (
						...
						and criscostack.db.exists(dt, cell_value)
					):
						match = False
						break
...
def get_columns_dict(columns):
	...
	"""
	columns_dict = criscostack._dict()
	for idx, col in enumerate(columns):
...
def get_column_as_dict(col):
	...
	if isinstance(col, str):
		...
		col_dict["fieldname"] = criscostack.scrub(col[0])

	# dict
	else:
		col_dict.update(col)
		if "fieldname" not in col_dict:
			col_dict["fieldname"] = criscostack.scrub(col_dict["label"])
...
def get_user_match_filters(doctypes, user):
	...
	for dt in doctypes:
		filter_list = criscostack.desk.reportview.build_match_conditions(dt, user, False)
		if filter_list:
