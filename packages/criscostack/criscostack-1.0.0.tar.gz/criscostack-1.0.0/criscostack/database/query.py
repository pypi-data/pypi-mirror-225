...
from criscostack.database.operator_map import OPERATOR_MAP
from criscostack.database.schema import SPECIAL_CHAR_PATTERN
from criscostack.database.utils import DefaultOrderBy, get_doctype_name
from criscostack.query_builder import Criterion, Field, Order, functions
from criscostack.query_builder.functions import Function, SqlFunctions
from criscostack.query_builder.utils import PseudoColumnMapper
from criscostack.utils.data import MARIADB_SPECIFIC_COMMENT
...
# less restrictive version of criscostack.core.doctype.doctype.doctype.START_WITH_LETTERS_PATTERN
...
class Engine:
	def get_query(
		...
		*,
		...
		self.is_mariadb = criscostack.db.db_type == "mariadb"
		self.is_postgres = criscostack.db.db_type == "postgres"
		...
		if isinstance(table, Table):
			...
			self.table = criscostack.qb.DocType(table)

		if update:
			self.query = criscostack.qb.update(self.table)
		elif into:
			self.query = criscostack.qb.into(self.table)
		elif delete:
			self.query = criscostack.qb.from_(self.table).delete()
		else:
			self.query = criscostack.qb.from_(self.table)
	...
	def validate_doctype(self):
		if not TABLE_NAME_PATTERN.match(self.doctype):
			criscostack.throw(_("Invalid DocType: {0}").format(self.doctype))
	...
	def _apply_filter(
		...
		if not isinstance(_field, str):
			...
			criscostack.throw(_("Invalid filter: {0}").format(_field))
		elif not doctype or doctype == self.doctype:
			_field = self.table[field]
		elif doctype:
			_field = criscostack.qb.DocType(doctype)[field]
		...
		if _operator in OPERATOR_MAP["nested_set"]:
			...
			_df = criscostack.get_meta(self.doctype).get_field(field)
...
class Permission:
	...
	def check_permissions(cls, query, **kwargs):
		...
		for dt in doctype:
			...
			) and not criscostack.has_permission(
				dt,
				"read",
				user=kwargs.get("user"),
				parent_doctype=kwargs.get("parent_doctype"),
			):
				criscostack.throw(_("Insufficient Permission for {0}").format(criscostack.bold(dt)))
	...
	def parse(field: str, doctype: str):
		if "." in field:
			...
			if field.startswith("`tab") or field.startswith('"tab'):
				...
				linked_field = criscostack.get_meta(doctype).get_field(linked_fieldname)
				linked_doctype = linked_field.options
				if linked_field.fieldtype == "Link":
					return LinkTableField(linked_doctype, fieldname, doctype, linked_fieldname, alias=alias)
				elif linked_field.fieldtype in criscostack.model.table_fields:
...
class ChildTableField(DynamicTableField):
	def __init__(
		...
		self.alias = alias
		self.parent_doctype = parent_doctype
		self.table = criscostack.qb.DocType(self.doctype)
		self.field = self.table[self.fieldname]

	def apply_select(self, query: QueryBuilder) -> QueryBuilder:
		table = criscostack.qb.DocType(self.doctype)
	...
	def apply_join(self, query: QueryBuilder) -> QueryBuilder:
		table = criscostack.qb.DocType(self.doctype)
		main_table = criscostack.qb.DocType(self.parent_doctype)
...
class ChildQuery:
	def __init__(
		...
		field = criscostack.get_meta(parent_doctype).get_field(fieldname)
		if field.fieldtype not in criscostack.model.table_fields:
			return
		self.fieldname = fieldname
		self.fields = fields
		self.parent_doctype = parent_doctype
...
def get_nested_set_hierarchy_result(doctype: str, name: str, hierarchy: str) -> list[str]:
	...
	table = criscostack.qb.DocType(doctype)
	try:
		lft, rgt = criscostack.qb.from_(table).select("lft", "rgt").where(table.name == name).run()[0]
	except IndexError:
		lft, rgt = None, None
