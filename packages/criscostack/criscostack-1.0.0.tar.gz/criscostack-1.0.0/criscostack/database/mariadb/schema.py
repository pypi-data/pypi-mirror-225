...
from criscostack.database.schema import DBTable
...
class MariaDBTable(DBTable):
	...
	def alter(self):
		...
		for col in self.add_index:
			...
			if not criscostack.db.get_column_index(self.table_name, col.fieldname, unique=False):
		...
		for col in {*self.drop_index, *self.drop_unique}:
			...
			if unique_constraint_changed and not col.unique:
				if unique_index := criscostack.db.get_column_index(self.table_name, col.fieldname, unique=True):
			...
			if index_constraint_changed and not col.set_index:
				if index_record := criscostack.db.get_column_index(self.table_name, col.fieldname, unique=False):
