...
from criscostack.database.mariadb.schema import MariaDBTable
from criscostack.utils import UnicodeWithAttrs, cstr, get_datetime, get_table_name
...
class MariaDBConnectionUtil:
	...
	def get_connection_settings(self) -> dict:
		...
		if self.user not in (criscostack.flags.root_login, "root"):
			conn_settings["database"] = self.user

		if self.port:
			conn_settings["port"] = int(self.port)

		if criscostack.conf.local_infile:
			conn_settings["local_infile"] = criscostack.conf.local_infile

		if criscostack.conf.db_ssl_ca and criscostack.conf.db_ssl_cert and criscostack.conf.db_ssl_key:
			conn_settings["ssl"] = {
				"ca": criscostack.conf.db_ssl_ca,
				"cert": criscostack.conf.db_ssl_cert,
				"key": criscostack.conf.db_ssl_key,
			}
...
class MariaDBDatabase(MariaDBConnectionUtil, MariaDBExceptionUtil, Database):
	...
	def escape(s, percent=True):
		...
		# Also, given we're promoting use of the PyPika builder via criscostack.qb, the use
		# of this method should be limited.

		# pymysql expects unicode argument to escape_string with Python 3
		s = criscostack.as_unicode(escape_string(criscostack.as_unicode(s)), "utf-8").replace("`", "\\`")
	...
	def get_tables(self, cached=True):
		...
		if cached:
			tables = criscostack.cache.get_value("db_tables")
			to_query = not tables
		...
		if to_query:
			...
			criscostack.cache.set_value("db_tables", tables)
	...
	def get_row_size(self, doctype: str) -> int:
		...
		est_row_size = criscostack.db.sql(
			"""
