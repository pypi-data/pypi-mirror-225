...
from criscostack.database.schema import DBTable, get_definition
from criscostack.model import log_types
from criscostack.utils import cint, flt


class PostgresTable(DBTable):
	def create(self):
		varchar_len = criscostack.db.VARCHAR_LEN
		...
		if (
			...
			criscostack.db.create_sequence(self.doctype, check_not_exists=True, cache=criscostack.db.SEQUENCE_CACHE)
	...
	def create_indexes(self):
		...
		if create_index_query:
			# nosemgrep
			criscostack.db.sql(create_index_query)
	...
	def alter(self):
		...
		for col in self.set_default:
			...
			if col.fieldtype in ("Check", "Int"):
				...
				col_default = f"{criscostack.db.escape(col.default)}"
		...
		try:
			if query:
				...
				# nosemgrep
				criscostack.db.sql(final_alter_query)
			if create_contraint_query:
				# nosemgrep
				criscostack.db.sql(create_contraint_query)
			if drop_contraint_query:
				# nosemgrep
				criscostack.db.sql(drop_contraint_query)
		except Exception as e:
			# sanitize
			if criscostack.db.is_duplicate_fieldname(e):
				criscostack.throw(str(e))
			elif criscostack.db.is_duplicate_entry(e):
