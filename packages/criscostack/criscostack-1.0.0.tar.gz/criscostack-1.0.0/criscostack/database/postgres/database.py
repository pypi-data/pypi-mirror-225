...
from criscostack.database.postgres.schema import PostgresTable
from criscostack.database.utils import EmptyQueryValues, LazyDecode
from criscostack.utils import cstr, get_table_name
...
class PostgresExceptionUtil:
	...
	def is_statement_timeout(e):
		return PostgresDatabase.is_timedout(e) or isinstance(e, criscostack.QueryTimeoutError)
		...
		return [
			...
			where table_catalog='{}'
				...
				and table_schema='{}'""".format(
					criscostack.conf.db_name, criscostack.conf.get("db_schema", "public")
				)
			)
		]
