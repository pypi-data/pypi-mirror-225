...
import criscostack
from criscostack.website.utils import extract_title, get_frontmatter
...
def get_page_info_from_web_page_with_dynamic_routes(path):
	...
	"""
	from criscostack.website.doctype.web_page.web_page import get_dynamic_web_pages
...
def get_page_info_from_web_form(path):
	...
	from criscostack.website.doctype.web_form.web_form import get_published_web_forms
	...
	if end_point:
		if path.endswith("/list"):
			criscostack.form_dict.is_list = True
		elif path.endswith("/new"):
			criscostack.form_dict.is_new = True
		elif path.endswith("/edit"):
			criscostack.form_dict.is_edit = True
		else:
			criscostack.form_dict.is_read = True
		return page_info[end_point]
...
def evaluate_dynamic_routes(rules, path):
	...
	if hasattr(criscostack.local, "request") and criscostack.local.request.environ:
		urls = route_map.bind_to_environ(criscostack.local.request.environ)
		try:
			...
			if args:
				...
				criscostack.local.form_dict.update(args)
...
def get_pages(app=None):
	...
	def _build(app):
		...
		if app:
			...
			apps = criscostack.local.flags.web_pages_apps or criscostack.get_installed_apps()

		for app in apps:
			app_path = criscostack.get_app_path(app)
	...
	return criscostack.cache.get_value("website_pages", lambda: _build(app))
...
def get_pages_from_path(start, app, app_path):
	...
	if os.path.exists(start_path):
		for basepath, folders, files in os.walk(start_path):
			...
			if not "__init__.py" in files and criscostack.conf.get("developer_mode"):
				open(os.path.join(basepath, "__init__.py"), "a").close()

			for fname in files:
				fname = criscostack.utils.cstr(fname)
				if not "." in fname:
					continue
				...
				if extn in ("html", "xml", "js", "css", "md"):
					...
					# print criscostack.as_json(pages[-1])
...
def setup_source(page_info):
	...
	if page_info.template.endswith((".md", ".html")):
		...
		if page_info.template.endswith(".md"):
			source = criscostack.utils.md_to_html(source)
...
def get_base_template(path=None):
	...
	"""
	if not path:
		path = criscostack.local.request.path

	base_template_map = criscostack.get_hooks("base_template_map") or {}
...
def load_properties_from_controller(page_info):
	...
	module = criscostack.get_module(page_info.controller)
	if not module:
		return
...
def get_doctypes_with_web_view():
	...
	def _get():
		...
		doctypes = criscostack.get_hooks("website_generators")
		doctypes_with_web_view = criscostack.get_all(
			"DocType", fields=["name", "module"], filters=dict(has_web_view=1)
		)
		module_app_map = criscostack.local.module_app
		doctypes += [
			d.name
			for d in doctypes_with_web_view
			if module_app_map.get(criscostack.scrub(d.module)) in installed_apps
		]
		return doctypes

	return criscostack.cache.get_value("doctypes_with_web_view", _get)


def get_start_folders():
	return criscostack.local.flags.web_pages_folders or ("www", "templates/pages")


def clear_routing_cache():
	from criscostack.website.doctype.web_form.web_form import get_published_web_forms
	from criscostack.website.doctype.web_page.web_page import get_dynamic_web_pages
	from criscostack.website.page_renderers.document_page import _find_matching_document_webview
