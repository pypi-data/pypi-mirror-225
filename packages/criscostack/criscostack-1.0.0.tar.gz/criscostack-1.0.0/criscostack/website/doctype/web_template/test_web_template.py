...
class TestWebTemplate(CriscoTestCase):
	def test_render_web_template_with_values(self):
		doc = criscostack.get_doc("Web Template", "Hero with Right Image")
		values = {
			"title": "Test Hero",
	...
	def test_custom_stylesheet(self):
		...
		criscostack.get_doc("Website Theme", "Standard").set_as_default()

	def create_web_page(self):
		if not criscostack.db.exists("Web Page", "test-web-template"):
			criscostack.get_doc(
				{
					"doctype": "Web Page",
	...
	def create_website_theme(self):
		if not criscostack.db.exists("Website Theme", "Custom"):
			theme = criscostack.get_doc({"doctype": "Website Theme", "theme": "Custom"}).insert()
		else:
			theme = criscostack.get_doc("Website Theme", "Custom")
		return theme
