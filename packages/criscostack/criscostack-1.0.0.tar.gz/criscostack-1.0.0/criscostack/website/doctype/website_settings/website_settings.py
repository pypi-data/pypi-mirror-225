...
from criscostack.utils import encode, get_request_site_address
from criscostack.website.utils import get_boot_data
...
class WebsiteSettings(Document):
	...
	if TYPE_CHECKING:
		...
		from criscostack.website.doctype.top_bar_item.top_bar_item import TopBarItem
		from criscostack.website.doctype.website_route_redirect.website_route_redirect import (
	...
	def validate_google_settings(self):
		if self.enable_google_indexing and not criscostack.db.get_single_value("Google Settings", "enable"):
		...
		for idx, row in enumerate(self.route_redirects):
			try:
				...
				if not criscostack.flags.in_migrate:
					criscostack.throw(_("Invalid redirect regex in row #{}: {}").format(idx, str(e)))
	...
	def get_access_token(self):
		...
		if not self.indexing_refresh_token:
			button_label = criscostack.bold(_("Allow API Indexing Access"))
...
def get_website_settings(context=None):
	...
	context = criscostack._dict(context or {})
	settings: "WebsiteSettings" = criscostack.get_cached_doc("Website Settings")
	...
	if context.disable_website_theme:
		context.theme = criscostack._dict()

	else:
		from criscostack.website.doctype.website_theme.website_theme import get_active_theme

		context.theme = get_active_theme() or criscostack._dict()

	if not context.get("favicon"):
		context["favicon"] = "/assets/criscostack/images/criscostack-favicon.svg"
	...
	context.read_only_mode = criscostack.flags.read_only
...
def get_auto_account_deletion():
	return criscostack.db.get_single_value("Website Settings", "auto_account_deletion")
