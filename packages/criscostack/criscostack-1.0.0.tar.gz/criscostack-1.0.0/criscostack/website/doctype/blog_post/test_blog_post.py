...
from criscostack.utils import random_string, set_request
from criscostack.website.doctype.blog_post.blog_post import get_blog_list
...
class TestBlogPost(CriscoTestCase):
	def setUp(self):
	...
	def test_generator_not_found(self):
		pages = criscostack.get_all("Blog Post", fields=["name", "route"], filters={"published": 0}, limit=1)

		route = f"test-route-{criscostack.generate_hash(length=5)}"

		criscostack.db.set_value("Blog Post", pages[0].name, "route", route)
	...
	def test_category_link(self):
		...
		blog_page_html = criscostack.safe_decode(blog_page_response.get_data())
		...
		cached_value = criscostack.db.value_cache.get(("DocType", "Blog Post", "name"))
		criscostack.db.value_cache[("DocType", "Blog Post", "name")] = (("Blog Post",),)
		...
		category_page_html = criscostack.safe_decode(category_page_response.get_data())
		# Category page should contain the blog post title
		self.assertIn(blog.title, category_page_html)

		# Cleanup
		criscostack.db.value_cache[("DocType", "Blog Post", "name")] = cached_value
		criscostack.delete_doc("Blog Post", blog.name)
		criscostack.delete_doc("Blog Category", blog.blog_category)
	...
	def test_blog_pagination(self):
		...
		filters = criscostack._dict({"blog_category": scrub(category_title)})
		...
		for blog in blogs:
			criscostack.delete_doc(blog.doctype, blog.name)
		criscostack.delete_doc("Blog Category", blogs[0].blog_category)

	def test_caching(self):
		# to enable caching
		criscostack.flags.force_website_cache = True
		print(criscostack.session.user)
	...
	def test_like_dislike(self):
		...
		criscostack.db.delete("Comment", {"comment_type": "Like", "reference_doctype": "Blog Post"})

		from criscostack.templates.includes.likes.likes import like

		criscostack.form_dict.reference_doctype = "Blog Post"
		criscostack.form_dict.reference_name = test_blog.name
		criscostack.form_dict.like = True
		criscostack.local.request_ip = "127.0.0.1"

		liked = like()
		self.assertEqual(liked, True)

		criscostack.form_dict.like = False
...
def make_test_blog(category_title="Test Blog Category"):
	...
	if not criscostack.db.exists("Blog Category", category_name):
		criscostack.get_doc(dict(doctype="Blog Category", title=category_title)).insert()
	if not criscostack.db.exists("Blogger", "test-blogger"):
		criscostack.get_doc(
			dict(doctype="Blogger", short_name="test-blogger", full_name="Test Blogger")
		).insert()

	test_blog = criscostack.get_doc(
		dict(
			doctype="Blog Post",
