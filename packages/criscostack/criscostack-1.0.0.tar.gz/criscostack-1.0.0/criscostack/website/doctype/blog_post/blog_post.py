...
from criscostack.website.utils import (
	clear_cache,
	find_first_image,
	get_comment_list,
...
class BlogPost(WebsiteGenerator):
	...
	def make_route(self):
		if not self.route:
			return (
				criscostack.db.get_value("Blog Category", self.blog_category, "route")
				+ "/"
	...
	def validate(self):
		...
		if self.featured:
			if not self.meta_image:
				criscostack.throw(_("A featured post must have a cover image"))
			self.reset_featured_for_other_blogs()

		self.set_read_time()

	def reset_featured_for_other_blogs(self):
		all_posts = criscostack.get_all("Blog Post", {"featured": 1})
		for post in all_posts:
			criscostack.db.set_value("Blog Post", post.name, "featured", 0)
	...
	def get_context(self, context):
		...
		context.enable_cta = not self.hide_cta and criscostack.db.get_single_value(
			"Blog Settings", "show_cta_in_blog", cache=True
		)

		if self.blogger:
			context.blogger_info = criscostack.get_doc("Blogger", self.blogger).as_dict()
		...
		context.category = criscostack.db.get_value(
		...
		context.guest_allowed = criscostack.db.get_single_value("Blog Settings", "allow_guest_to_comment")

	def fetch_cta(self):
		if criscostack.db.get_single_value("Blog Settings", "show_cta_in_blog", cache=True):
			blog_settings = criscostack.get_cached_doc("Blog Settings")
	...
	def fetch_social_links_info(self):
		if not criscostack.db.get_single_value("Blog Settings", "enable_social_sharing", cache=True):
			return []

		url = criscostack.local.site + "/" + self.route
	...
	def load_likes(self, context):
		...
		context.like_count = criscostack.db.count("Comment", filters) or 0
		...
		context.like = criscostack.db.count("Comment", filters) or 0
...
def get_list_context(context=None):
	list_context = criscostack._dict(
	...
	blog_settings = criscostack.get_doc("Blog Settings").as_dict(no_default_fields=True)
	list_context.update(blog_settings)

	category_name = criscostack.utils.escape_html(
		criscostack.local.form_dict.blog_category or criscostack.local.form_dict.category
	)
	if category_name:
		category = criscostack.get_doc("Blog Category", category_name)
	...
	elif criscostack.local.form_dict.blogger:
		blogger = criscostack.db.get_value("Blogger", {"name": criscostack.local.form_dict.blogger}, "full_name")
		list_context.sub_title = _("Posts by {0}").format(blogger)
		list_context.title = blogger

	elif criscostack.local.form_dict.txt:
		list_context.sub_title = _('Filtered by "{0}"').format(sanitize_html(criscostack.local.form_dict.txt))
...
def get_blog_categories():
	...
	post, category = criscostack.qb.DocType("Blog Post"), criscostack.qb.DocType("Blog Category")
	return (
		criscostack.qb.from_(category)
		.select(category.name, category.route, category.title)
		.where(
			(category.published == 1)
			& ExistsCriterion(
				criscostack.qb.from_(post)
				.select("name")
...
def get_blog_list(
	...
	if filters and filters.get("blog_category"):
		...
		category = criscostack.utils.escape_html(
			criscostack.local.form_dict.blog_category or criscostack.local.form_dict.category
		)

	if filters and filters.get("blogger"):
		conditions.append("t1.blogger=%s" % criscostack.db.escape(filters.get("blogger")))

	if category:
		conditions.append("t1.blog_category=%s" % criscostack.db.escape(category))

	if txt:
		conditions.append(
			'(t1.content like {0} or t1.title like {0}")'.format(criscostack.db.escape("%" + txt + "%"))
		)

	if conditions:
		criscostack.local.no_cache = 1
	...
	posts = criscostack.db.sql(query, as_dict=1)
	...
	for post in posts:
		...
		post.category = criscostack.db.get_value(
