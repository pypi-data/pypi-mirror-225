...
from criscostack.utils import cint, cstr, get_assets_json, get_system_timezone, md_to_html
...
def delete_page_cache(path):
	criscostack.cache.delete_value("full_index")
	groups = ("website_page", "page_context")
	if path:
		for name in groups:
			criscostack.cache.hdel(name, path)
	else:
		for name in groups:
			criscostack.cache.delete_key(name)
...
def can_cache(no_cache=False):
	if criscostack.flags.force_website_cache:
		return True
	if criscostack.conf.disable_website_cache or criscostack.conf.developer_mode:
		return False
	if getattr(criscostack.local, "no_cache", False):
		return False
	return not no_cache


def get_comment_list(doctype, name):
	comments = criscostack.get_all(
		"Comment",
		...
		or_filters=[["owner", "=", criscostack.session.user], ["published", "=", 1]],
	)

	communications = criscostack.get_all(
		"Communication",
		fields=[
			"name",
			"creation",
		...
		or_filters=[
			["recipients", "like", f"%{criscostack.session.user}%"],
			["cc", "like", f"%{criscostack.session.user}%"],
			["bcc", "like", f"%{criscostack.session.user}%"],
		],
	)
...
def get_home_page():
	if criscostack.local.flags.home_page and not criscostack.flags.in_test:
		return criscostack.local.flags.home_page
	...
	def _get_home_page():
		...
		if criscostack.session.user != "Guest":
			# by role
			for role in criscostack.get_roles():
				home_page = criscostack.db.get_value("Role", role, "home_page")
				if home_page:
					break

			# portal default
			if not home_page:
				home_page = criscostack.db.get_single_value("Portal Settings", "default_portal_home")
		...
		# global
		if not home_page:
			home_page = criscostack.db.get_single_value("Website Settings", "home_page")

		if not home_page:
			home_page = "login" if criscostack.session.user == "Guest" else "me"
	...
	return criscostack.cache.hget("home_page", criscostack.session.user, _get_home_page)


def get_home_page_via_hooks():
	home_page = None

	home_page_method = criscostack.get_hooks("get_website_user_home_page")
	if home_page_method:
		home_page = criscostack.get_attr(home_page_method[-1])(criscostack.session.user)
	elif criscostack.get_hooks("website_user_home_page"):
		home_page = criscostack.get_hooks("website_user_home_page")[-1]

	if not home_page:
		role_home_page = criscostack.get_hooks("role_home_page")
		if role_home_page:
			for role in criscostack.get_roles():
	...
	if not home_page:
		home_page = criscostack.get_hooks("home_page")
		if home_page:
...
def get_boot_data():
	return {
		"lang": "en",
		"sysdefaults": {
			"float_precision": cint(criscostack.get_system_settings("float_precision")) or 3,
			"date_format": criscostack.get_system_settings("date_format") or "yyyy-mm-dd",
			"time_format": criscostack.get_system_settings("time_format") or "HH:mm:ss",
		},
		"time_zone": {
			"system": get_system_timezone(),
			"user": criscostack.db.get_value("User", criscostack.session.user, "time_zone") or get_system_timezone(),
		},
		"assets_json": get_assets_json(),
		"sitename": criscostack.local.site,
	}


def is_signup_disabled():
	return criscostack.db.get_single_value("Website Settings", "disable_signup", True)
...
def get_toc(route, url_prefix=None, app=None):
	...
	return criscostack.get_template("templates/includes/full_index.html").render(
...
def get_full_index(route=None, app=None):
	...
	from criscostack.website.router import get_pages

	if not criscostack.local.flags.children_map:
		...
		children_map = criscostack.cache.get_value("website_full_index", _build)

		criscostack.local.flags.children_map = children_map

	return criscostack.local.flags.children_map
...
def clear_cache(path=None):
	...
	for key in (
		...
		criscostack.cache.delete_value(key)

	clear_routing_cache()

	criscostack.cache.delete_value("website_404")
	if path:
		criscostack.cache.hdel("website_redirects", path)
		delete_page_cache(path)
	else:
		clear_sitemap()
		criscostack.clear_cache("Guest")
		for key in (
			"portal_menu_items",
			"home_page",
			"website_route_rules",
	...
	for method in criscostack.get_hooks("website_clear_cache"):
		criscostack.get_attr(method)(path)
...
def get_sidebar_items(parent_sidebar, basepath=None):
	...
	hooks = criscostack.get_hooks("look_for_sidebar_json")
	look_for_sidebar_json = hooks[0] if hooks else criscostack.flags.look_for_sidebar
	...
	if not sidebar_items and parent_sidebar:
		sidebar_items = criscostack.get_all(
...
def get_portal_sidebar_items():
	sidebar_items = criscostack.cache.hget("portal_menu_items", criscostack.session.user)
	if sidebar_items is None:
		sidebar_items = []
		roles = criscostack.get_roles()
		portal_settings = criscostack.get_doc("Portal Settings", "Portal Settings")
		...
		items_via_hooks = criscostack.get_hooks("portal_menu_items")
		if items_via_hooks:
			for i in items_via_hooks:
				i["enabled"] = 1
			add_items(sidebar_items, items_via_hooks)

		criscostack.cache.hset("portal_menu_items", criscostack.session.user, sidebar_items)
...
def get_sidebar_items_from_sidebar_file(basepath, look_for_sidebar_json):
	...
	with open(sidebar_json_path) as sidebarfile:
		try:
			...
			criscostack.throw("Invalid Sidebar JSON at " + sidebar_json_path)
...
def cache_html(func):
	...
	def cache_html_decorator(*args, **kwargs):
		if can_cache():
			html = None
			page_cache = criscostack.cache.hget("website_page", args[0].path)
			if page_cache and criscostack.local.lang in page_cache:
				html = page_cache[criscostack.local.lang]
			if html:
				criscostack.local.response.from_cache = True
				return html
		html = func(*args, **kwargs)
		context = args[0].context
		if can_cache(context.no_cache):
			page_cache = criscostack.cache.hget("website_page", args[0].path) or {}
			page_cache[criscostack.local.lang] = html
			criscostack.cache.hset("website_page", args[0].path, page_cache)
...
def build_response(path, data, http_status_code, headers: dict | None = None):
	...
	response.headers["X-From-Cache"] = criscostack.local.response.from_cache or False
...
def add_preload_for_bundled_assets(response):
	...
	for css in criscostack.local.preload_assets["style"]:
		links.append(f"<{css}>; rel=preload; as=style")

	for js in criscostack.local.preload_assets["script"]:
