# Copyright (c) 2013, Crisco Technologies and contributors
# License: MIT. See LICENSE

from datetime import datetime

import criscostack
from criscostack.query_builder.functions import Coalesce, Count
...
class WebsiteAnalytics:
	def __init__(self, filters=None):
		self.filters = criscostack._dict(filters or {})

		if not self.filters.to_date:
			self.filters.to_date = datetime.now()

		if not self.filters.from_date:
			self.filters.from_date = criscostack.utils.add_days(self.filters.to_date, -7)

		if not self.filters.range:
			self.filters.range = "Daily"

		self.filters.to_date = criscostack.utils.add_days(self.filters.to_date, 1)
	...
	def get_columns(self):
		meta = criscostack.get_meta("Web Page View")
	...
	def get_data(self):
		WebPageView = criscostack.qb.DocType("Web Page View")
		count_all = Count("*").as_("count")
		case = criscostack.qb.terms.Case().when(WebPageView.is_unique == "1", "1")
		count_is_unique = Count(case).as_("unique_count")

		return (
			criscostack.qb.from_(WebPageView)
			...
			.orderby("count", order=criscostack.qb.desc)
		).run()
	...
	def get_chart_data(self):
		current_dialect = criscostack.db.db_type or "mariadb"
		...
		self.chart_data = criscostack.db.sql(query, values=values, as_dict=1)
	...
	def prepare_chart_data(self, data):
		...
		if self.filters.range == "Monthly":
			date_range = [criscostack.utils.add_days(dd, 1) for dd in date_range]
