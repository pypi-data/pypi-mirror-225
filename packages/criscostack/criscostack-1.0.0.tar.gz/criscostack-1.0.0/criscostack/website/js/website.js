...
criscostack.provide("website");
criscostack.provide("criscostack.awesome_bar_path");
window.cur_frm = null;
...
$.extend(criscostack, {
	...
	require: async function (links, callback) {
		...
		links = links.map((link) => criscostack.bundled_asset(link));
	...
	bundled_asset(path, is_rtl = null) {
		if (!path.startsWith("/assets") && path.includes(".bundle.")) {
			...
			path = criscostack.boot.assets_json[path] || path;
			return path;
		}
		return path;
	},
	add_asset_to_head(link) {
		return new Promise((resolve) => {
			if (criscostack._assets_loaded.includes(link)) return resolve();
			let el;
			...
			el.onload = () => {
				criscostack._assets_loaded.push(link);
				resolve();
			};
		});
	},
	...
	call: function (opts) {
		...
		return $.ajax({
			...
			statusCode: opts.statusCode || {
				404: function () {
					criscostack.msgprint(__("Not found"));
				},
				403: function () {
					criscostack.msgprint(__("Not permitted"));
				},
				200: function (data) {
			...
			criscostack.process_response(opts, data);
		});
	},
	...
	process_response: function (opts, data) {
		...
		if (data._server_messages) {
			...
			if (opts.error_msg) {
				...
				criscostack.msgprint(server_messages);
			}
		}
	...
	has_permission: function (doctype, docname, perm_type, callback) {
		return criscostack.call({
			type: "GET",
			method: "criscostack.client.has_permission",
			no_spinner: true,
	...
	render_user: function () {
		if (criscostack.is_user_logged_in()) {
			$(".btn-login-area").toggle(false);
			$(".logged-in").toggle(true);
			$(".user-image").attr("src", criscostack.get_cookie("user_image"));

			$(".user-image-wrapper").html(
				criscostack.avatar(null, "avatar-medium", null, null, null, true)
			);
			$(".user-image-sidebar").html(
				criscostack.avatar(null, "avatar-medium", null, null, null, true)
			);
			$(".user-image-myaccount").html(
				criscostack.avatar(null, "avatar-large", null, null, null, true)
			);
		}
	},
	freeze_count: 0,
	freeze: function (msg) {
		...
		criscostack.freeze_count++;
	},
	unfreeze: function () {
		if (!criscostack.freeze_count) return; // anything open?
		criscostack.freeze_count--;
		if (!criscostack.freeze_count) {
	...
	trigger_ready: function () {
		criscostack.ready_events.forEach(function (fn) {
			fn();
		});
	},
	...
	bind_filters: function () {
		...
		$(".filter").each(function () {
			...
			var val = criscostack.utils.get_url_arg(key).replace(/\+/g, " ");
	...
	bind_navbar_search: function () {
		criscostack.get_navbar_search().on("keypress", function (e) {
			var val = $(this).val();
			if (e.which === 13 && val) {
				$(this).val("").blur();
				criscostack.do_search(val);
				return false;
			}
		});
	},
	do_search: function (val) {
		var path =
			(criscostack.awesome_bar_path && criscostack.awesome_bar_path[location.pathname]) ||
			window.search_path ||
			location.pathname;
	...
	set_search_path: function (path) {
		criscostack.awesome_bar_path[location.pathname] = path;
	},
	...
	is_user_logged_in: function () {
		return criscostack.get_cookie("user_id") !== "Guest" && criscostack.session.user !== "Guest";
	},
	...
	show_language_picker() {
		if (criscostack.session.user === "Guest" && window.show_language_picker) {
			criscostack
				.call("criscostack.translate.get_all_languages", {
					with_language_name: true,
				})
				.then((res) => {
					let language_list = res.message;
					let language = criscostack.get_cookie("preferred_language");
					let language_codes = [];
...
criscostack.setup_search = function (target, search_scope) {
	...
	$input.on(
		...
		criscostack.utils.debounce(() => {
			...
			criscostack
				.call({
					method: "criscostack.search.web_search",
					args: {
...
window.valid_email = function (id) {
	// copied regex from criscostack/utils.js validate_type
...
$(document).ready(function () {
	window.full_name = criscostack.get_cookie("full_name");
	var logged_in = criscostack.is_user_logged_in();
	$("#website-login").toggleClass("hide", logged_in ? true : false);
	$("#website-post-login").toggleClass("hide", logged_in ? false : true);
	$(".logged-in").toggleClass("hide", logged_in ? false : true);

	criscostack.bind_navbar_search();

	// switch to app link
	if (criscostack.get_cookie("system_user") === "yes" && logged_in) {
		criscostack.add_switch_to_desk();
	}

	criscostack.render_user();
...
$(document).on("page-change", function () {
	...
	$.extend(criscostack, criscostack.get_cookies());
	criscostack.session = { user: criscostack.user_id };

	criscostack.datetime.refresh_when();
	criscostack.trigger_ready();
	criscostack.bind_filters();
	criscostack.highlight_code_blocks();
	criscostack.add_link_to_headings();
	criscostack.make_navbar_active();
	// scroll to hash
...
criscostack.ready(function () {
	criscostack.show_language_picker();
	criscostack.setup_videos();
	criscostack.realtime.init(window.socketio_port, true); // lazy connection
});
