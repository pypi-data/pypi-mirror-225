...
def execute():
	...
	reports = criscostack.get_all("Report", {"report_type": "Report Builder"})

	for report_name in reports:
		settings = criscostack.db.get_value("Report", report_name, "json")
		if not settings:
			continue

		settings = criscostack._dict(json.loads(settings))
		...
		criscostack.db.set_value("Report", report_name, "json", json.dumps(settings))
