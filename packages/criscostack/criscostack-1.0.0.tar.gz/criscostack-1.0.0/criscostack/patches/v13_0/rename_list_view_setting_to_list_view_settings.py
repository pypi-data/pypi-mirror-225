...
def execute():
	if not criscostack.db.table_exists("List View Setting"):
		return
	if not criscostack.db.exists("DocType", "List View Setting"):
		return

	criscostack.reload_doc("desk", "doctype", "List View Settings")

	existing_list_view_settings = criscostack.get_all(
		"List View Settings", as_list=True, order_by="modified"
	)
	for list_view_setting in criscostack.get_all(
		"List View Setting",
