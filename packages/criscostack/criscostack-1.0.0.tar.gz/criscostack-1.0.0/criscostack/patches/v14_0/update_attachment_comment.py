...
def execute():
	...
	for name, content in criscostack.get_all(
		...
		criscostack.db.set_value("Comment", name, "content", content, update_modified=False)
	...
	for name, content in criscostack.get_all(
		...
		if content and content.startswith("Removed "):
			criscostack.db.set_value("Comment", name, "content", content[8:], update_modified=False)
