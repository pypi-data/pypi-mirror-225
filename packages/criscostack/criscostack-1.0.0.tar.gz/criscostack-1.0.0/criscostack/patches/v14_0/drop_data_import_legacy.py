...
def execute():
	...
	criscostack.db.delete("DocType", {"name": doctype})
	...
	click.secho(
		...
		f" using\n\n\tbench --site {criscostack.local.site} backup -i '{doctype}'\n\nAfter"
		...
		f" --site {criscostack.local.site} execute criscostack.db.sql --args \"('DROP TABLE IF"
		f" EXISTS `{table}`', )\"\n",
		fg="yellow",
	)
