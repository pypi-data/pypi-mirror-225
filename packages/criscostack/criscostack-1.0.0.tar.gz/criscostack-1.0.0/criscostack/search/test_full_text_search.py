...
class TestFullTextSearch(CriscoTestCase):
	def setUp(self):
		index = get_index()
		index.build()
		self.index = index
	...
	def test_search_term(self):
		...
		self.assertEqual(res[0], "sw/criscoerp")
	...
	def test_update_index(self):
		...
		self.index.update_index({"name": "sw/criscoerp", "content": """AwesomeCriscoerp"""})

		res = self.index.search("CommonSearchTerm")
		self.assertTrue("sw/criscoerp" not in res)

		res = self.index.search("AwesomeCriscoerp")
		self.assertEqual(res[0], "sw/criscoerp")

		# Update new doc
		self.index.update_index({"name": "sw/criscostackbooks", "content": """DesktopAccounting"""})

		res = self.index.search("DesktopAccounting")
		self.assertEqual(res[0], "sw/criscostackbooks")
...
def get_index():
	return TestWrapper("test_criscostack_index")
...
def get_documents():
	...
	docs.append(
		{
			"name": "sw/criscoerp",
			"content": """Criscoerp is a free and open-source integrated Enterprise Resource Planning software developed by
			Crisco Technologies Pvt. Ltd. and is built on MariaDB database system using a Python based server-side framework.
			Criscoerp is a generic ERP software used by manufacturers, distributors and services companies. CommonSearchTerm""",
		}
	)
	...
	docs.append(
		{
			"name": "sw/criscostack",
			"content": """Crisco Framework is a full-stack web framework, that includes everything you need to build and
