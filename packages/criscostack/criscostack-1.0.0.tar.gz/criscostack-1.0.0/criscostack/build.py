...
def build_missing_files():
	...
	for type in ["css", "js"]:
		folder = os.path.join(sites_path, "assets", "criscostack", "dist", type)
	...
	assets_json = criscostack.read_file("assets/assets.json")
	if assets_json:
		assets_json = criscostack.parse_json(assets_json)

		for bundle_file, output_file in assets_json.items():
			if not output_file.startswith("/assets/criscostack"):
				continue
		...
		if missing_assets:
			...
			files_to_build = ["criscostack/" + name for name in missing_assets]
			bundle(build_mode, files=files_to_build)
	else:
		# no assets.json, run full build
		bundle(build_mode, apps="criscostack")


def get_assets_link(criscostack_head) -> str:
	import requests

	tag = getoutput(
		r"cd ../apps/criscostack && git show-ref --tags -d | grep %s | sed -e 's,.*"
		r" refs/tags/,,' -e 's/\^{}//'" % criscostack_head
	)

	if tag:
		# if tag exists, download assets from github release
		url = f"https://github.com/criscostack/criscostack/releases/download/{tag}/assets.tar.gz"
	else:
		url = f"http://assets.criscostackframework.com/{criscostack_head}.tar.gz"

	if not requests.head(url):
		reference = f"Release {tag}" if tag else f"Commit {criscostack_head}"
		raise AssetsDontExistError(f"Assets for {reference} don't exist")

	return url


def fetch_assets(url, criscostack_head):
	click.secho("Retrieving assets...", fg="yellow")

	prefix = mkdtemp(prefix="criscostack-assets-", suffix=criscostack_head)
	assets_archive = download_file(url, prefix)

	if not assets_archive:
		raise AssetsNotDownloadedError(f"Assets could not be retrived from {url}")

	click.echo(click.style("✔", fg="green") + f" Downloaded Crisco assets from {url}")
...
def setup_assets(assets_archive):
	...
	with tarfile.open(assets_archive) as tar:
		for file in tar:
			if not file.isdir():
				dest = "." + file.name.replace("./criscostack-bench/sites", "")
...
def download_criscostack_assets(verbose=True):
	"""Downloads and sets up Crisco assets if they exist based on the current
	commit HEAD.
	Returns True if correctly setup else returns False.
	"""
	criscostack_head = getoutput("cd ../apps/criscostack && git rev-parse HEAD")

	if not criscostack_head:
		return False

	try:
		url = get_assets_link(criscostack_head)
		assets_archive = fetch_assets(url, criscostack_head)
...
def setup():
	...
	for app in criscostack.get_all_apps(True):
		try:
			pymodules.append(criscostack.get_module(app))
		except ImportError:
			pass
	app_paths = [os.path.dirname(pymodule.__file__) for pymodule in pymodules]
	assets_path = os.path.join(criscostack.local.sites_path, "assets")
...
def bundle(
	...
	criscostack_app_path = criscostack.get_app_path("criscostack", "..")
	criscostack.commands.popen(command, cwd=criscostack_app_path, env=get_node_env(), raise_err=True)
...
def watch(apps=None):
	...
	live_reload = criscostack.utils.cint(os.environ.get("LIVE_RELOAD", criscostack.conf.live_reload))
	...
	check_node_executable()
	criscostack_app_path = criscostack.get_app_path("criscostack", "..")
	criscostack.commands.popen(command, cwd=criscostack_app_path, env=get_node_env())
...
def generate_assets_map():
	...
	for app_name in criscostack.get_all_apps():
		app_doc_path = None

		pymodule = criscostack.get_module(app_name)
...
def html_to_js_template(path, content):
	"""returns HTML template content as Javascript code, adding it to `criscostack.templates`"""
	return """criscostack.templates["{key}"] = '{content}';\n""".format(
