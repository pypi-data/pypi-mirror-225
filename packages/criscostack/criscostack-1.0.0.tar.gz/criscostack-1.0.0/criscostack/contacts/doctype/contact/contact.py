...
from criscostack.contacts.address_and_contact import set_link_title
from criscostack.core.doctype.dynamic_link.dynamic_link import deduplicate_dynamic_links
from criscostack.model.document import Document
from criscostack.model.naming import append_number_if_name_exists
from criscostack.utils import cstr, has_gravatar
...
class Contact(Document):
	...
	if TYPE_CHECKING:
		from criscostack.contacts.doctype.contact_email.contact_email import ContactEmail
		from criscostack.contacts.doctype.contact_phone.contact_phone import ContactPhone
		from criscostack.core.doctype.dynamic_link.dynamic_link import DynamicLink
	...
	def autoname(self):
		...
		if criscostack.db.exists("Contact", self.name):
	...
	def validate(self):
		...
		if self.get("sync_with_google_contacts") and not self.get("google_contacts"):
			criscostack.throw(_("Select Google Contacts to which contact should be synced."))

		deduplicate_dynamic_links(self)

	def set_user(self):
		if not self.user and self.email_id:
			self.user = criscostack.db.get_value("User", {"email": self.email_id})
	...
	def add_email(self, email_id, is_primary=0, autosave=False):
		if not criscostack.db.exists("Contact Email", {"email_id": email_id, "parent": self.name}):
	...
	def add_phone(self, phone, is_primary_phone=0, is_primary_mobile_no=0, autosave=False):
		if not criscostack.db.exists("Contact Phone", {"phone": phone, "parent": self.name}):
			self.append(
				"phone_nos",
				{
					"phone": phone,
	...
	def set_primary_email(self):
		...
		if len([email.email_id for email in self.email_ids if email.is_primary]) > 1:
			criscostack.throw(_("Only one {0} can be set as primary.").format(criscostack.bold("Email ID")))
	...
	def set_primary(self, fieldname):
		...
		if len(is_primary) > 1:
			criscostack.throw(
				_("Only one {0} can be set as primary.").format(criscostack.bold(criscostack.unscrub(fieldname)))
			)
...
def invite_user(contact):
	contact = criscostack.get_doc("Contact", contact)

	if not contact.email_id:
		criscostack.throw(_("Please set Email Address"))
...
def update_contact(doc, method):
	...
	contact_name = criscostack.db.get_value("Contact", {"email_id": doc.name})
	if contact_name:
...
def contact_query(doctype, txt, searchfield, start, page_len, filters):
	from criscostack.desk.reportview import get_match_cond

	doctype = "Contact"
	if (
		not criscostack.get_meta(doctype).get_field(searchfield)
		and searchfield not in criscostack.db.DEFAULT_COLUMNS
	):
		return []
...
def get_contacts_linked_from(doctype, docname, fields=None):
	...
	link_fields = criscostack.get_meta(doctype).get("fields", {"fieldtype": "Link", "options": "Contact"})
	if not link_fields:
		return []

	contact_names = criscostack.get_value(doctype, docname, fieldname=[f.fieldname for f in link_fields])
	if not contact_names:
		return []

	return criscostack.get_list("Contact", fields=fields, filters={"name": ("in", contact_names)})
...
def get_contact_display_list(doctype: str, name: str) -> list[dict]:
	from criscostack.contacts.doctype.address.address import get_condensed_address

	if not criscostack.has_permission("Contact", "read"):
		return []

	contact_list = criscostack.get_list(
		"Contact",
		filters=[
			["Dynamic Link", "link_doctype", "=", doctype],
	...
	for contact in contact_list:
		contact["email_ids"] = criscostack.get_all(
			"Contact Email",
			filters={"parenttype": "Contact", "parent": contact.name, "is_primary": 0},
			fields=["email_id"],
		)

		contact["phone_nos"] = criscostack.get_all(
			"Contact Phone",
			filters={
				"parenttype": "Contact",
				"parent": contact.name,
		...
		if contact.address and criscostack.has_permission("Address", "read"):
			address = criscostack.get_doc("Address", contact.address)
