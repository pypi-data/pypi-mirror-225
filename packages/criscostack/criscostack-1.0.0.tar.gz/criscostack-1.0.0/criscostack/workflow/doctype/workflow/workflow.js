criscostack.provide("criscostack.core");

criscostack.ui.form.on("Workflow", {
	onload: function (frm) {
	...
	refresh: function (frm) {
		...
		let message = `
		<div class="flex">
			<div class="mr-3"><img style="border-radius: var(--border-radius-md)" width="510" src="/assets/criscostack/images/workflow-builder.gif"></div>
			<div>
				...
				<div>
					<a class="btn btn-primary btn-sm" href="${workflow_builder_url}">
						${__("Workflow Builder")} ${criscostack.utils.icon("right", "xs")}
					</a>
				</div>
			</div>
		</div>
		...
		if (frm.doc.document_type) {
			frm.add_custom_button(__("Go to {0} List", [frm.doc.document_type]), () => {
				criscostack.set_route("List", frm.doc.document_type);
			});
		}
	...
	update_field_options: function (frm) {
		...
		criscostack.model.with_doctype(doc.document_type, () => {
			const fieldnames = criscostack
				.get_meta(doc.document_type)
				.fields.filter((field) => !criscostack.model.no_value_type.includes(field.fieldtype))
	...
	set_table_html: function (frm) {
		const promises = frm.states.map((r) => {
			...
			return criscostack.utils.get_indicator_color(state).then((color) => {
				return `<tr>
				<td>
	...
	get_orphaned_states_and_count: function (frm) {
		...
		return criscostack
			.xcall("criscostack.workflow.doctype.workflow.workflow.get_workflow_state_count", {
		...
		if (frm.states && frm.states.length) {
			...
			$(frm.state_table)
				...
				.on("click", (e) => {
					...
					criscostack.set_route("List", frm.doc.document_type, filters);
				});
		}
	},
});

criscostack.ui.form.on("Workflow Document State", {
