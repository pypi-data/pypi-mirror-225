...
from criscostack.desk.form.utils import get_pdf_link
from criscostack.desk.notifications import clear_doctype_notifications
from criscostack.model.document import Document
from criscostack.model.workflow import (
	apply_workflow,
	get_workflow_name,
...
from criscostack.utils import get_datetime, get_url
from criscostack.utils.background_jobs import enqueue
from criscostack.utils.data import get_link_to_form
from criscostack.utils.user import get_users_with_role
...
class WorkflowAction(Document):
	...
	if TYPE_CHECKING:
		...
		from criscostack.workflow.doctype.workflow_action_permitted_role.workflow_action_permitted_role import (
...
def on_doctype_update():
	...
	criscostack.db.add_index("Workflow Action", ["reference_name", "reference_doctype", "status"])
...
def get_permission_query_conditions(user):
	...
	permitted_workflow_actions = (
		criscostack.qb.from_(WorkflowAction)
	...
	""".format(
		permitted_workflow_actions=permitted_workflow_actions, user=criscostack.db.escape(user)
	)
...
def has_permission(doc, user):
	...
	return not permitted_roles.isdisjoint(criscostack.get_roles(user))
...
def confirm_action(doctype, docname, user, action):
	...
	logged_in_user = criscostack.session.user
	...
	if logged_in_user == "Guest":
		criscostack.set_user(logged_in_user)
...
def return_success_page(doc):
	criscostack.respond_as_web_page(
		...
		_("{0}: {1} is set to state {2}").format(
			doc.get("doctype"), criscostack.bold(doc.get("name")), criscostack.bold(get_doc_workflow_state(doc))
		),
...
def return_link_expired_page(doc, doc_workflow_state):
	criscostack.respond_as_web_page(
		...
		_("Document {0} has been set to state {1} by {2}").format(
			criscostack.bold(doc.get("name")),
			criscostack.bold(doc_workflow_state),
			criscostack.bold(criscostack.get_value("User", doc.get("modified_by"), "full_name")),
		),
		indicator_color="blue",
	)
...
def get_allowed_roles(user, workflow, workflow_state):
	user = user if user else criscostack.session.user

	allowed_roles = criscostack.get_all(
		"Workflow Transition",
		fields="allowed",
	...
	user_roles = set(criscostack.get_roles(user))
...
def update_completed_workflow_actions_using_role(user=None, workflow_action=None):
	...
	(
		criscostack.qb.update(WorkflowAction)
...
def clear_old_workflow_actions_using_user(doc, user=None):
	...
	if criscostack.db.has_column("Workflow Action", "user"):
...
def get_next_possible_transitions(workflow_name, state, doc=None):
	transitions = criscostack.get_all(
		"Workflow Transition",
	...
	for transition in transitions:
		...
		for user in filtered_users:
			if not user_data_map.get(user):
				user_data_map[user] = criscostack._dict(
					{
						"possible_actions": [],
						"email": criscostack.db.get_value("User", user, "email"),
					}
				)
...
def create_workflow_actions_for_roles(roles, doc):
	workflow_action = criscostack.get_doc(
		{
...
def send_workflow_action_email(users_data, doc):
	...
	for d in users_data:
		...
		enqueue(method=criscostack.sendmail, queue="short", **email_args)
...
def get_workflow_action_url(action, doc, user):
	apply_action_method = (
		"/api/method/criscostack.workflow.doctype.workflow_action.workflow_action.apply_action"
	)
...
def get_confirm_workflow_action_url(doc, action, user):
	confirm_action_method = (
		"/api/method/criscostack.workflow.doctype.workflow_action.workflow_action.confirm_action"
	)
...
def get_common_email_args(doc):
	...
	if email_template:
		subject = criscostack.render_template(email_template.subject, vars(doc))
		response = criscostack.render_template(email_template.response, vars(doc))
	else:
	...
	common_args = {
		...
		"attachments": [criscostack.attach_print(doctype, docname, file_name=docname, doc=doc)],
		"subject": subject,
		"message": response,
	}
	return common_args
...
def get_email_template(doc):
	...
	template_name = criscostack.db.get_value(
	...
	if not template_name:
		return
	return criscostack.get_doc("Email Template", template_name)


def get_state_optional_field_value(workflow_name, state):
	return criscostack.get_cached_value(
