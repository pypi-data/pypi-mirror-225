...
if TYPE_CHECKING:
	...
	from criscostack.workflow.doctype.workflow.workflow import Workflow


class WorkflowStateError(criscostack.ValidationError):
	pass


class WorkflowTransitionError(criscostack.ValidationError):
	pass


class WorkflowPermissionError(criscostack.ValidationError):
	pass


def get_workflow_name(doctype):
	workflow_name = criscostack.cache.hget("workflow", doctype)
	if workflow_name is None:
		workflow_name = criscostack.db.get_value(
			"Workflow", {"document_type": doctype, "is_active": 1}, "name"
		)
		criscostack.cache.hset("workflow", doctype, workflow_name or "")
...
def get_transitions(
	...
	if not isinstance(doc, Document):
		doc = criscostack.get_doc(criscostack.parse_json(doc))
		doc.load_from_db()
	...
	if not current_state:
		if raise_exception:
			...
			criscostack.throw(_("Workflow State not set"), WorkflowStateError)
...
def get_workflow_safe_globals():
	# access to criscostack.db.get_value, criscostack.db.get_list, and date time utils.
	return dict(
		criscostack=criscostack._dict(
			db=criscostack._dict(get_value=criscostack.db.get_value, get_list=criscostack.db.get_list),
			session=criscostack.session,
			utils=criscostack._dict(
				now_datetime=criscostack.utils.now_datetime,
				add_to_date=criscostack.utils.add_to_date,
				get_datetime=criscostack.utils.get_datetime,
...
def apply_workflow(doc, action):
	...
	if not transition:
		criscostack.throw(_("Not a valid Workflow Action"), WorkflowTransitionError)

	if not has_approval_access(user, doc, transition):
		criscostack.throw(_("Self approval is not allowed"))
	...
	if doc.docstatus.is_draft() and new_docstatus == DocStatus.draft():
		...
		criscostack.throw(_("Illegal Document Status for {0}").format(next_state.state))
...
def validate_workflow(doc):
	...
	if not state_row:
		criscostack.throw(
			_("{0} is not a valid Workflow State. Please update your Workflow and try again.").format(
				criscostack.bold(current_state)
			)
		)
	...
	if current_state != next_state:
		bold_current = criscostack.bold(current_state)
		bold_next = criscostack.bold(next_state)
...
def get_workflow(doctype) -> "Workflow":
	return criscostack.get_cached_doc("Workflow", get_workflow_name(doctype))
...
def get_workflow_field_value(workflow_name, field):
	return criscostack.get_cached_value("Workflow", workflow_name, field)
...
def bulk_workflow_approval(docnames, doctype, action):
	...
	print("Clearing criscostack.message_log...")
	...
	for (idx, docname) in enumerate(docnames, 1):
		...
		try:
			...
			apply_workflow(criscostack.get_doc(doctype, docname), action)
			criscostack.db.commit()
		except Exception as e:
			if not criscostack.message_log:
			...
			if not message_dict:
				if criscostack.message_log:
					messages = criscostack.get_message_log()
...
def print_workflow_log(messages, title, doctype, indicator):
	if messages.keys():
		...
		for doc in messages.keys():
			if len(messages[doc]):
				html = f"<details><summary>{criscostack.utils.get_link_to_form(doctype, doc)}</summary>"
		...
		criscostack.msgprint(msg, title=_("Workflow Status"), indicator=indicator, is_minimizable=True)
	...
	try:
		for (i, doc) in enumerate(docs, 1):
			...
			actions = [
				...
				if has_approval_access(criscostack.session.user, doc, t)
			]
			if not actions:
				return []
...
def set_workflow_state_on_action(doc, workflow_name, action):
	workflow = criscostack.get_doc("Workflow", workflow_name)
