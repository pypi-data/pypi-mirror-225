...
import criscostack
from criscostack import _, is_whitelisted, msgprint
from criscostack.core.doctype.server_script.server_script_utils import run_server_script_for_doc_event
from criscostack.desk.form.document_follow import follow_document
from criscostack.integrations.doctype.webhook import run_webhooks
from criscostack.model import optional_fields, table_fields
from criscostack.model.base_document import BaseDocument, get_controller
from criscostack.model.docstatus import DocStatus
from criscostack.model.naming import set_new_name, validate_name
from criscostack.model.utils import is_virtual_doctype
from criscostack.model.workflow import set_workflow_state_on_action, validate_workflow
from criscostack.types import DF
from criscostack.utils import compare, cstr, date_diff, file_lock, flt, get_datetime_str, now
from criscostack.utils.data import get_absolute_url
from criscostack.utils.global_search import update_global_search


def get_doc(*args, **kwargs):
	"""returns a criscostack.model.Document object.
...
class Document(BaseDocument):
	...
	doctype: DF.Data
	name: DF.Data | None
	flags: criscostack._dict[str, Any]
	owner: DF.Link
	...
	def load_from_db(self):
		...
		if not getattr(self, "_metaclass", False) and self.meta.issingle:
			single_doc = criscostack.db.get_singles_dict(self.doctype, for_update=self.flags.for_update)
			if not single_doc:
				single_doc = criscostack.new_doc(self.doctype, as_dict=True)
			...
			d = criscostack.db.get_value(
				doctype=self.doctype, filters=self.name, fieldname="*", **get_value_kwargs
			)

			if not d:
				criscostack.throw(
					_("{0} {1} not found").format(_(self.doctype), self.name), criscostack.DoesNotExistError
				)
		...
		for df in self._get_table_fields():
			...
			# During criscostack is installed, the property "is_virtual" is not available in tabDocType, so
			# we need to filter those cases for the access to criscostack.db.get_value() as it would crash otherwise.
	...
	def check_permission(self, permtype="read", permlevel=None):
		"""Raise `criscostack.PermissionError` if not permitted"""
		if not self.has_permission(permtype):
			self.raise_no_permission_to(permlevel or permtype)

	def has_permission(self, permtype="read") -> bool:
		"""
		Call `criscostack.permissions.has_permission` if `ignore_permissions` flag isn't truthy
		...
		return criscostack.permissions.has_permission(self.doctype, permtype, self)

	def raise_no_permission_to(self, perm_type):
		"""Raise `criscostack.PermissionError`."""
		criscostack.flags.error_message = _("Insufficient Permission for {0}").format(self.doctype)
		raise criscostack.PermissionError
	...
	def insert(
		...
		if not (
			criscostack.flags.in_migrate or criscostack.local.flags.in_install or criscostack.flags.in_setup_wizard
		):
			if criscostack.get_cached_value("User", criscostack.session.user, "follow_created_documents"):
				follow_document(self.doctype, self.name, criscostack.session.user)
		return self

	def check_if_locked(self):
		if self.creation and self.is_locked:
			raise criscostack.DocumentLockedError
	...
	def _save(self, ignore_permissions=None, ignore_version=None) -> "Document":
		...
		self.flags.ignore_version = criscostack.flags.in_test if ignore_version is None else ignore_version
	...
	def copy_attachments_from_amended_from(self):
		...
		from criscostack.desk.form.load import get_attachments
	...
	def update_child_table(self, fieldname, df=None):
		...
		if (
			...
			or criscostack.get_meta(df.options).is_virtual == 1
		):
		...
		if rows:
			...
			deleted_rows = criscostack.db.sql(
			...
			if len(deleted_rows) > 0:
				...
				criscostack.db.delete(df.options, {"name": ("in", tuple(row[0] for row in deleted_rows))})
	...
	def set_new_name(self, force=False, set_name=None, set_child_names=True):
		"""Calls `criscostack.naming.set_new_name` for parent and child docs."""
	...
	def update_single(self, d):
		...
		criscostack.db.delete("Singles", {"doctype": self.doctype})
		...
		if self.doctype in criscostack.db.value_cache:
			del criscostack.db.value_cache[self.doctype]

	def set_user_and_timestamp(self):
		self._original_modified = self.modified
		self.modified = now()
		self.modified_by = criscostack.session.user
		...
		if self.is_new() and not (
			criscostack.flags.in_install or criscostack.flags.in_patch or criscostack.flags.in_migrate
		):
		...
		criscostack.flags.currently_saving.append((self.doctype, self.name))
	...
	def _validate_non_negative(self):
		def get_msg(df):
			if self.get("parentfield"):
				return "{} {} #{}: {} {}".format(
					criscostack.bold(_(self.doctype)),
					_("Row"),
					self.idx,
					_("Value cannot be negative for"),
					criscostack.bold(_(df.label)),
				)
			else:
				return _("Value cannot be negative for {0}: {1}").format(
					_(df.parent), criscostack.bold(_(df.label))
				)
		...
		for df in self.meta.get(
			...
			if flt(self.get(df.fieldname)) < 0:
				msg = get_msg(df)
				criscostack.throw(msg, criscostack.NonNegativeError, title=_("Negative Value"))
				...
				if fail:
					criscostack.throw(
						_("Value cannot be changed for {0}").format(
							criscostack.bold(self.meta.get_label(field.fieldname))
						),
						exc=criscostack.CannotChangeConstantError,
					)
	...
	def apply_fieldlevel_read_permissions(self):
		...
		for table_field in self.meta.get_table_fields():
			all_fields += criscostack.get_meta(table_field.options).fields or []
		...
		for table_field in self.meta.get_table_fields():
			for df in criscostack.get_meta(table_field.options).fields or []:
	...
	def validate_higher_perm_levels(self):
		...
		if self.flags.ignore_permissions or criscostack.flags.in_install:
			return
		...
		for df in self.meta.get_table_fields():
			high_permlevel_fields = criscostack.get_meta(df.options).get_high_permlevel_fields()
			if high_permlevel_fields:
				for d in self.get(df.fieldname):
	...
	def get_permissions(self):
		if self.meta.istable:
			...
			permissions = criscostack.get_meta(self.parenttype).permissions
		else:
	...
	def _set_defaults(self):
		...
		new_doc = criscostack.new_doc(self.doctype, as_dict=True)
		self.update_if_missing(new_doc)

		# children
		for df in self.meta.get_table_fields():
			new_doc = criscostack.new_doc(df.options, as_dict=True)
	...
	def check_if_latest(self):
		...
		if cstr(previous.modified) != cstr(self._original_modified):
			criscostack.msgprint(
				...
				raise_exception=criscostack.TimestampMismatchError,
			)
	...
	def check_docstatus_transition(self, to_docstatus):
		...
		if to_docstatus == DocStatus.draft():
			if self.docstatus.is_draft():
				...
				raise criscostack.DocstatusTransitionError(
					_("Cannot change docstatus from 0 (Draft) to 2 (Cancelled)")
				)
			else:
				raise criscostack.ValidationError(_("Invalid docstatus"), self.docstatus)
			...
			raise criscostack.ValidationError(_("Cannot edit cancelled document"))
	...
	def _validate_mandatory(self):
		...
		if criscostack.flags.print_messages:
			print(self.as_json().encode("utf-8"))

		raise criscostack.MandatoryError(
	...
	def _validate_links(self):
		...
		if invalid_links:
			...
			criscostack.throw(_("Could not find {0}").format(msg), criscostack.LinkValidationError)

		if cancelled_links:
			msg = ", ".join(each[2] for each in cancelled_links)
			criscostack.throw(_("Cannot link cancelled document: {0}").format(msg), criscostack.CancelledLinkError)
	...
	def run_notifications(self, method):
		...
		if (
			(criscostack.flags.in_import and criscostack.flags.mute_emails)
		...
		from criscostack.email.doctype.notification.notification import evaluate_alert
		...
		if self.flags.notifications is None:
			...
			self.flags.notifications = criscostack.cache.hget("notifications", self.doctype, _get_notifications)
	...
	def _rename(
		...
		"""Rename the document. Triggers criscostack.rename_doc, then reloads."""
	...
	def load_doc_before_save(self, *, raise_exception: bool = False):
		...
		try:
			self._doc_before_save = criscostack.get_doc(self.doctype, self.name, for_update=True)
	...
	def run_post_save_methods(self):
		...
		if (self.doctype, self.name) in criscostack.flags.currently_saving:
			criscostack.flags.currently_saving.remove((self.doctype, self.name))

	def clear_cache(self):
		criscostack.clear_document_cache(self.doctype, self.name)
	...
	def reset_seen(self):
		...
		if (
			...
			criscostack.db.set_value(
				self.doctype, self.name, "_seen", json.dumps([criscostack.session.user]), update_modified=False
			)

	def notify_update(self):
		"""Publish realtime that the current document is modified"""
		if criscostack.flags.in_patch:
			return
		...
		if (
			...
			data = {"doctype": self.doctype, "name": self.name, "user": criscostack.session.user}
			criscostack.publish_realtime("list_update", data, after_commit=True)
	...
	def db_set(self, fieldname, value=None, update_modified=True, notify=False, commit=False):
		...
		:param commit: default False. run criscostack.db.commit()
		"""
		...
		if update_modified and (self.doctype, self.name) not in criscostack.flags.currently_saving:
			# don't update modified timestamp if called from post save methods
			# like on_update or on_submit
			self.set("modified", now())
			self.set("modified_by", criscostack.session.user)
	...
	def db_get(self, fieldname):
		...
		return criscostack.db.get_value(self.doctype, self.name, fieldname)

	def check_no_back_links_exist(self):
		"""Check if document links to any active document before Cancel."""
		from criscostack.model.delete_doc import check_if_doc_is_dynamically_linked, check_if_doc_is_linked
	...
	def save_version(self):
		...
		if (
			...
			or criscostack.flags.in_install
			or (not self._doc_before_save and criscostack.flags.in_patch)
		):
			return

		doc_to_compare = self._doc_before_save
		if not doc_to_compare and (amended_from := self.get("amended_from")):
			doc_to_compare = criscostack.get_doc(self.doctype, amended_from)
	...
	def hook(f):
		...
		def composer(self, *args, **kwargs):
			hooks = []
			method = f.__name__
			doc_events = criscostack.get_doc_hooks()
			for handler in doc_events.get(self.doctype, {}).get(method, []) + doc_events.get("*", {}).get(
				method, []
			):
				hooks.append(criscostack.get_attr(handler))
	...
	def validate_table_has_rows(self, parentfield, raise_exception=None):
		...
		if not (isinstance(self.get(parentfield), list) and len(self.get(parentfield)) > 0):
			...
			criscostack.throw(
				_("Table {0} cannot be empty").format(label), raise_exception or criscostack.EmptyTableError
			)
	...
	def add_comment(
		...
		out = criscostack.get_doc(
			{
				"doctype": "Comment",
				"comment_type": comment_type,
				"comment_email": comment_email or criscostack.session.user,
	...
	def add_seen(self, user=None):
		...
		if self.meta.track_seen and not criscostack.flags.read_only and not self.meta.issingle:
			_seen = self.get("_seen") or []
			_seen = criscostack.parse_json(_seen)

			if user not in _seen:
				_seen.append(user)
				criscostack.db.set_value(self.doctype, self.name, "_seen", json.dumps(_seen), update_modified=False)
				criscostack.local.flags.commit = True
	...
	def add_viewed(self, user=None, force=False, unique_views=False):
		...
		if unique_views and criscostack.db.exists(
	...
	def get_document_share_key(self, expires_on=None, no_expiry=False):
		...
		existing_key = criscostack.db.exists(
			"Document Share Key",
			{
				"reference_doctype": self.doctype,
		...
		if existing_key:
			doc = criscostack.get_doc("Document Share Key", existing_key)
		else:
			doc = criscostack.new_doc("Document Share Key")
	...
	def set_onload(self, key, value):
		if not self.get("__onload"):
			self.set("__onload", criscostack._dict())
		self.get("__onload")[key] = value

	def get_onload(self, key=None):
		if not key:
			return self.get("__onload", criscostack._dict())
	...
	def queue_action(self, action, **kwargs):
		...
		from criscostack.utils.background_jobs import enqueue
		...
		try:
			self.lock()
		except criscostack.DocumentLockedError:
			criscostack.throw(
		...
		return enqueue(
			"criscostack.model.document.execute_action",
	...
	def lock(self, timeout=None):
		...
		criscostack.local.locked_documents.append(self)

	def unlock(self):
		"""Delete the lock file for this document"""
		file_lock.delete_lock(self.get_signature())
		if self in criscostack.local.locked_documents:
			criscostack.local.locked_documents.remove(self)
		...
		if date_diff(to_date, from_date) < 0:
			criscostack.throw(
				_("{0} must be after {1}").format(
					criscostack.bold(_(self.meta.get_label(to_date_field))),
					criscostack.bold(_(self.meta.get_label(from_date_field))),
				),
				criscostack.exceptions.InvalidDates,
			)

	def get_assigned_users(self):
		assigned_users = criscostack.get_all(
			"ToDo",
			fields=["allocated_to"],
			filters={
	...
	def add_tag(self, tag):
		...
		from criscostack.desk.doctype.tag.tag import DocTags
	...
	def deferred_insert(self) -> None:
		...
		from criscostack.deferred_insert import deferred_insert
...
def execute_action(__doctype, __name, __action, **kwargs):
	...
	doc = criscostack.get_doc(__doctype, __name)
	doc.unlock()
	try:
		...
		# add a comment (?)
		if criscostack.local.message_log:
			msg = json.loads(criscostack.local.message_log[-1]).get("message")
		else:
			msg = "<pre><code>" + criscostack.get_traceback() + "</pre></code>"
...
def bulk_insert(
	...
	columns = criscostack.get_meta(doctype).get_valid_columns()
...
def _document_values_generator(
	...
	for doc in documents:
		...
		doc.created_by = doc.modified_by = criscostack.session.user
