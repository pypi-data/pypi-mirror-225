...
def delete_fields(args_dict, delete=0):
	...
	* If single, deletes record from `tabSingles`
	...
	for dt in args_dict:
		...
		if criscostack.db.get_value("DocType", dt, "issingle"):
			criscostack.db.delete(
				"Singles",
				{
					"doctype": dt,
			...
			existing_fields = criscostack.db.describe(dt)
...
def get_permitted_fields(
	...
	if permission_type is None:
		permission_type = "select" if criscostack.only_has_select_perm(doctype, user=user) else "read"
