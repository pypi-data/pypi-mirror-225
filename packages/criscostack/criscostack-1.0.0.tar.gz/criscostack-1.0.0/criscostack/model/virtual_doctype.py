...
class VirtualDoctype(Protocol):
	...
	- DocType controller has to inherit from `criscostack.model.document.Document` class
	...
	@staticmethod
	def get_list(args) -> list[criscostack._dict]:
		...
		if not isinstance(method, staticmethod):
			criscostack.msgprint(
				_("Virtual DocType {} requires a static method called {} found {}").format(
					criscostack.bold(doctype), criscostack.bold(m), criscostack.bold(_as_str(method))
				),
