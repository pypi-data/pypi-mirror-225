...
import criscostack
from criscostack import _, _dict
...
from criscostack.model.naming import set_new_name
from criscostack.model.utils.link_count import notify_link_count
from criscostack.modules import load_doctype_module
from criscostack.utils import cast_fieldtype, cint, compare, cstr, flt, now, sanitize_html, strip_html
from criscostack.utils.html_utils import unescape_html
...
def get_controller(doctype):
	...
	For `custom` type, returns `criscostack.model.document.Document`.

	:param doctype: DocType name as string.
	"""

	if criscostack.local.dev_server or criscostack.flags.in_migrate:
		return import_controller(doctype)

	site_controllers = criscostack.controllers.setdefault(criscostack.local.site, {})
...
def import_controller(doctype):
	...
	from criscostack.utils.nestedset import NestedSet

	module_name = "Core"
	if doctype not in DOCTYPES_FOR_DOCTYPE:
		doctype_info = criscostack.db.get_value("DocType", doctype, fieldname="*")
		if doctype_info:
	...
	module_path = None
	class_overrides = criscostack.get_hooks("override_doctype_class")
	if class_overrides and class_overrides.get(doctype):
		import_path = class_overrides[doctype][-1]
		module_path, classname = import_path.rsplit(".", 1)
		module = criscostack.get_module(module_path)
...
class BaseDocument:
	...
	def meta(self):
		if not (meta := getattr(self, "_meta", None)):
			self._meta = meta = criscostack.get_meta(self.doctype)
	...
	def get_db_value(self, key):
		return criscostack.db.get_value(self.doctype, self.name, key)
			...
			if df:
				if is_virtual_field:
					...
					if d[fieldname] is None and (options := getattr(df, "options", None)):
						from criscostack.utils.safe_exec import get_safe_globals

						d[fieldname] = criscostack.safe_eval(
							code=options,
				...
				if isinstance(d[fieldname], list) and df.fieldtype not in table_fields:
					criscostack.throw(_("Value for {0} cannot be a list").format(_(df.label)))
	...
	def get_valid_columns(self) -> list[str]:
		if self.doctype not in criscostack.local.valid_columns:
			if self.doctype in DOCTYPES_FOR_DOCTYPE:
				from criscostack.model.meta import get_table_columns

				valid = get_table_columns(self.doctype)
			else:
				valid = self.meta.get_valid_columns()

			criscostack.local.valid_columns[self.doctype] = valid

		return criscostack.local.valid_columns[self.doctype]
	...
	def as_json(self):
		return criscostack.as_json(self.as_dict())
	...
	def db_insert(self, ignore_if_duplicate=False):
		...
		if ignore_if_duplicate and criscostack.db.db_type == "postgres":
			conflict_handler = "on conflict (name) do nothing"

		if not self.creation:
			self.creation = self.modified = now()
			self.created_by = self.modified_by = criscostack.session.user
		...
		try:
			...
			if criscostack.db.is_primary_key_violation(e):
				if self.meta.autoname == "hash":
					# hash collision? try again
					criscostack.flags.retry_count = (criscostack.flags.retry_count or 0) + 1
					if criscostack.flags.retry_count > 5 and not criscostack.flags.in_test:
						raise
					self.name = None
					self.db_insert()
					return
				...
				if not ignore_if_duplicate:
					criscostack.msgprint(
						_("{0} {1} already exists").format(_(self.doctype), criscostack.bold(self.name)),
						title=_("Duplicate Name"),
						indicator="red",
					)
					raise criscostack.DuplicateEntryError(self.doctype, self.name, e)

			elif criscostack.db.is_unique_key_violation(e):
				# unique constraint
	...
	def db_update(self):
		...
		try:
			...
			if criscostack.db.is_unique_key_violation(e):
				self.show_unique_validation_message(e)
			else:
				raise
	...
	def show_unique_validation_message(self, e):
		if criscostack.db.db_type != "postgres":
			...
			criscostack.msgprint(_("{0} must be unique").format(label or fieldname))

		# this is used to preserve traceback
		raise criscostack.UniqueValidationError(self.doctype, self.name, e)
	...
	def update_modified(self):
		...
		if getattr(self.meta, "issingle", False):
			criscostack.db.set_single_value(self.doctype, "modified", self.modified, update_modified=False)
		else:
			criscostack.db.set_value(self.doctype, self.name, "modified", self.modified, update_modified=False)
		...
		for df in self.meta.get_link_fields() + self.meta.get(
			...
			if docname:
				if df.fieldtype == "Link":
					doctype = df.options
					if not doctype:
						criscostack.throw(_("Options not set for link field {0}").format(df.fieldname))
				else:
					doctype = self.get(df.options)
					if not doctype:
						criscostack.throw(_("{0} must be set first").format(self.meta.get_label(df.options)))
				...
				if not criscostack.get_meta(doctype).get("is_virtual"):
					if not fields_to_fetch:
						# cache a single value type
						values = _dict(name=criscostack.db.get_value(doctype, docname, "name", cache=True))
					else:
						values_to_fetch = ["name"] + [_df.fetch_from.split(".")[-1] for _df in fields_to_fetch]

						# don't cache if fetching other values too
						values = criscostack.db.get_value(doctype, docname, values_to_fetch, as_dict=True)

				if getattr(criscostack.get_meta(doctype), "issingle", 0):
					values.name = doctype

				if criscostack.get_meta(doctype).get("is_virtual"):
					values = criscostack.get_doc(doctype, docname).as_dict()
				...
				if values:
					...
					if not values.name:
						...
						and criscostack.get_meta(doctype).is_submittable
						and cint(criscostack.db.get_value(doctype, docname, "docstatus")) == DocStatus.cancelled()
					):
	...
	def set_fetch_from_value(self, doctype, df, values):
		...
		if df.fieldtype in ["Small Text", "Text", "Data"]:
			from criscostack.model.meta import get_default_df

			fetch_from_df = get_default_df(fetch_from_fieldname) or criscostack.get_meta(doctype).get_field(
				fetch_from_fieldname
			)

			if not fetch_from_df:
				criscostack.throw(
					_('Please check the value of "Fetch From" set for field {0}').format(criscostack.bold(df.label)),
	...
	def _validate_selects(self):
		...
		for df in self.meta.get_select_fields():
			...
			if value not in options and not (criscostack.flags.in_test and value.startswith("_T-")):
	...
	def _validate_data_fields(self):
		# data_field options defined in criscostack.model.data_field_options
		for phone_field in self.meta.get_phone_fields():
			phone = self.get(phone_field.fieldname)
			criscostack.utils.validate_phone_number_with_country_code(phone, phone_field.fieldname)
		...
		for data_field in self.meta.get_data_fields():
			...
			if data_field_options == "Email":
				if (self.owner in criscostack.STANDARD_USERS) and (data in criscostack.STANDARD_USERS):
					continue
				for email_address in criscostack.utils.split_emails(data):
					criscostack.utils.validate_email_address(email_address, throw=True)

			if data_field_options == "Name":
				criscostack.utils.validate_name(data, throw=True)

			if data_field_options == "Phone":
				criscostack.utils.validate_phone_number(data, throw=True)

			if data_field_options == "URL":
				if not data:
					continue

				criscostack.utils.validate_url(data, throw=True)

	def _validate_constants(self):
		if criscostack.flags.in_import or self.is_new() or self.flags.ignore_validate_constants:
			return

		constants = [d.fieldname for d in self.meta.get("fields", {"set_only_once": ("=", 1)})]
		if constants:
			values = criscostack.db.get_value(self.doctype, self.name, constants, as_dict=True)
		...
		for fieldname in constants:
			...
			if self.get(fieldname) != value:
				criscostack.throw(
					...
					criscostack.CannotChangeConstantError,
				)
	...
	def _validate_length(self):
		...
		type_map = criscostack.db.type_map
	...
	def _validate_code_fields(self):
		for field in self.meta.get_code_fields():
			...
			if language == "Python":
				criscostack.utils.validate_python_code(code_string, fieldname=field.label, is_expression=False)

			elif language == "PythonExpression":
				criscostack.utils.validate_python_code(code_string, fieldname=field.label)
	...
	def throw_length_exceeded_error(self, df, max_length, value):
		...
		criscostack.throw(
			...
			criscostack.CharacterLengthExceededError,
			title=_("Value too big"),
		)

	def _validate_update_after_submit(self):
		# get the full doc with children
		db_values = criscostack.get_doc(self.doctype, self.name).as_dict()
				...
				if self_value != db_value:
					criscostack.throw(
						_("{0} Not allowed to change {1} after submission from {2} to {3}").format(
							...
							criscostack.bold(db_value),
							criscostack.bold(self_value),
						),
						criscostack.UpdateAfterSubmitError,
	...
	def _sanitize_content(self):
		...
		for fieldname, value in self.get_valid_dict(ignore_virtual=True).items():
			...
			value = criscostack.as_unicode(value)
	...
	def _save_passwords(self):
		...
		from criscostack.utils.password import remove_encrypted_password, set_encrypted_password
	...
	def get_formatted(
		...
		from criscostack.utils.formatters import format_value
		...
		if (
			...
			currency = criscostack.db.get_value("Currency", currency_value, cache=True)
	...
	def reset_values_if_no_permlevel_access(self, has_access_to, high_permlevel_fields):
		...
		if to_reset:
			if self.is_new():
				...
				ref_doc = criscostack.new_doc(self.doctype)
			else:
	...
	def _extract_images_from_text_editor(self):
		from criscostack.core.doctype.file.utils import extract_images_from_doc
