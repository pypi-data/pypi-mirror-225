...
from criscostack.custom.doctype.property_setter.property_setter import delete_property_setter
from criscostack.model import core_doctypes_list, no_value_fields
...
class CustomizeForm(Document):
	...
	if TYPE_CHECKING:
		...
		from criscostack.custom.doctype.customize_form_field.customize_form_field import CustomizeFormField
	...
	def on_update(self):
		criscostack.db.delete("Singles", {"doctype": "Customize Form"})
		criscostack.db.delete("Customize Form Field")
	...
	def fetch_to_customize(self):
		...
		meta = criscostack.get_meta(self.doc_type, cached=False)
	...
	def validate_doctype(self, meta):
		...
		if self.doc_type in core_doctypes_list:
			criscostack.throw(_("Core DocTypes cannot be customized."))

		if meta.issingle:
			criscostack.throw(_("Single DocTypes cannot be customized."))

		if meta.custom:
			criscostack.throw(_("Only standard DocTypes are allowed to be customized from Customize Form."))
	...
	def get_name_translation(self):
		...
		return criscostack.get_value(
			"Translation",
			{"source_text": self.doc_type, "language": criscostack.local.lang or "en"},
	...
	def set_name_translation(self):
		...
		if not self.label:
			if current:
				# clear translation
				criscostack.delete_doc("Translation", current.name)
			return
			...
			criscostack.get_doc(
				{
					...
					"language_code": criscostack.local.lang or "en",
				}
			).insert()
			return

		if self.label != current.translated_text:
			criscostack.db.set_value("Translation", current.name, "translated_text", self.label)
	...
	def save_customization(self):
		...
		if self.flags.update_db:
			try:
				criscostack.db.updatedb(self.doc_type)
			except Exception as e:
				if criscostack.db.is_db_table_size_limit(e):
					criscostack.throw(
						_("You have hit the row size limit on database table: {0}").format(
							"<a href='https://docs.criscoerp.com/docs/v14/user/manual/en/customize-criscoerp/articles/maximum-number-of-fields-in-a-form'>"
		...
		if not hasattr(self, "hide_success") or not self.hide_success:
			criscostack.msgprint(_("{0} updated").format(_(self.doc_type)), alert=True)
		criscostack.clear_cache(doctype=self.doc_type)
		self.fetch_to_customize()

		if self.flags.rebuild_doctype_for_global_search:
			criscostack.enqueue(
				"criscostack.utils.global_search.rebuild_for_doctype",
	...
	def allow_property_change(self, prop, meta_df, df):
		if prop == "fieldtype":
			...
			if not criscostack.db.get_value(
			...
			(
				criscostack.db.get_value("DocField", {"parent": self.doc_type, "fieldname": df.fieldname}, "reqd")
				== 1
			)
			and (df.get(prop) == 0)
		):
			...
			criscostack.msgprint(_("You cannot unset 'Read Only' for field {0}").format(df.label))
			return False

		elif prop == "options" and df.get("fieldtype") not in ALLOWED_OPTIONS_CHANGE:
			criscostack.msgprint(_("You can't set 'Options' for field {0}").format(df.label))
			return False

		elif prop == "translatable" and not supports_translation(df.get("fieldtype")):
			criscostack.msgprint(_("You can't set 'Translatable' for field {0}").format(df.label))
			return False
	...
	def set_property_setters_for_actions_and_links(self, meta):
		...
		for doctype, fieldname, field_map in (
			...
			for i, d in enumerate(self.get(fieldname) or []):
				d.idx = i
				if criscostack.db.exists(doctype, d.name) and not d.custom:
					# check property and apply property setter
					original = criscostack.get_doc(doctype, d.name)
	...
	def update_order_property_setter(self, has_custom, fieldname):
		...
		if has_custom:
			...
			criscostack.db.delete("Property Setter", dict(property=property_name, doc_type=self.doc_type))
	...
	def clear_removed_items(self, doctype, items):
		...
		"""
		if items:
			criscostack.db.delete(doctype, dict(parent=self.doc_type, custom=1, name=("not in", items)))
		else:
			criscostack.db.delete(doctype, dict(parent=self.doc_type, custom=1))
	...
	def update_custom_fields(self):
		for i, df in enumerate(self.get("fields")):
			...
			if not criscostack.db.exists("Custom Field", {"dt": self.doc_type, "fieldname": df.fieldname}):
	...
	def add_custom_field(self, df, i):
		d = criscostack.new_doc("Custom Field")
	...
	def update_in_custom_field(self, df, i):
		...
		custom_field = criscostack.get_doc("Custom Field", meta_df[0].name)
		changed = False
		for prop in docfield_properties:
	...
	def delete_custom_fields(self):
		...
		for fieldname in fields_to_remove:
			...
			if not is_standard_or_system_generated_field(df):
				criscostack.delete_doc("Custom Field", df.name)
	...
	def get_existing_property_value(self, property_name, fieldname=None):
		...
		if fieldname:
			...
			if criscostack.db.has_column("DocType", property_name):
				property_value = criscostack.db.get_value("DocType", self.doc_type, property_name)
			else:
				property_value = None
	...
	def validate_fieldtype_change(self, df, old_value, new_value):
		...
		if allowed:
			old_value_length = cint(criscostack.db.type_map.get(old_value)[1])
			new_value_length = cint(criscostack.db.type_map.get(new_value)[1])
	...
	def validate_fieldtype_length(self):
		for field in self.check_length_for_fieldtypes:
			df = field.get("df")
			max_length = cint(criscostack.db.type_map.get(df.fieldtype)[1])
			fieldname = df.fieldname
			docs = criscostack.db.sql(
				"""
			...
			label = df.label
			for doc in docs:
				links.append(criscostack.utils.get_link_to_form(self.doc_type, doc.name))
			...
			if docs:
				criscostack.throw(
					...
					).format(criscostack.bold(label), links_str, criscostack.bold(max_length)),
	...
	def reset_layout(self):
		...
		property_setters = criscostack.get_all(
			"Property Setter",
		...
		criscostack.db.delete("Property Setter", {"name": ("in", property_setters)})
		criscostack.clear_cache(doctype=self.doc_type)
		self.fetch_to_customize()
...
def reset_customization(doctype):
	...
	for setter in setters:
		criscostack.delete_doc("Property Setter", setter)
	...
	for field in custom_fields:
		criscostack.delete_doc("Custom Field", field)
