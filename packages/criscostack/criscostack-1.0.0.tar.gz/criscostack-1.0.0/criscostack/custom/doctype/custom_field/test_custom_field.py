...
test_records = criscostack.get_test_records("Custom Field")


class TestCustomField(CriscoTestCase):
	def test_create_custom_fields(self):
		...
		self.assertTrue(criscostack.db.exists("Custom Field", "Address-_test_custom_field_1"))
		self.assertTrue(criscostack.db.exists("Custom Field", "Address-_test_custom_field_2"))
		self.assertTrue(criscostack.db.exists("Custom Field", "Contact-_test_custom_field_2"))
	...
	def test_custom_field_sorting(self):
		try:
			...
			fields = criscostack.get_meta("ToDo", cached=False).fields
