...
import criscostack
from criscostack import _, safe_decode
...
def get_change_log(user=None):
	...
	last_known_versions = criscostack._dict(
		json.loads(criscostack.db.get_value("User", user, "last_known_versions") or "{}")
	)
	...
	if "criscostack" in current_versions:
		set_in_change_log("criscostack", current_versions["criscostack"], change_log)

	return change_log


def get_change_log_for_app(app, from_version, to_version):
	change_log_folder = os.path.join(criscostack.get_app_path(app), "change_log")
		...
		if folder in major_version_folders:
			for file in os.listdir(os.path.join(change_log_folder, folder)):
				...
				if from_version < version <= to_version:
					...
					content = criscostack.read_file(file_path)
...
def get_versions():
	...
	Example:
	        ...
	        {
	                "criscostack": {
	                        "title": "Crisco Framework",
	                        "version": "5.0.0"
	                }
	        }"""
	versions = {}
	for app in criscostack.get_installed_apps(_ensure_on_bench=True):
		app_hooks = criscostack.get_hooks(app_name=app)
		versions[app] = {
		...
		try:
			versions[app]["version"] = criscostack.get_attr(app + ".__version__")
...
def check_for_update():
	updates = criscostack._dict(major=[], minor=[], patch=[])
	apps = get_versions()
...
def check_release_on_github(app: str):
	"""
	Check the latest release for a given Crisco application hosted on Github.

	Args:
	        app (str): The name of the Crisco application.
...
def add_message_to_redis(update_json):
	...
	criscostack.cache.set_value("update-info", json.dumps(update_json))
	user_list = [x.name for x in criscostack.get_all("User", filters={"enabled": True})]
	system_managers = [user for user in user_list if "System Manager" in criscostack.get_roles(user)]
	criscostack.cache.sadd("update-user-set", *system_managers)
...
def show_update_popup():
	...
	update_info = criscostack.cache.get_value("update-info")
	if not update_info:
		return
	...
	update_message = ""
	if criscostack.cache.sismember("update-user-set", user):
		for update_type in updates:
			release_links = ""
			for app in updates[update_type]:
				app = criscostack._dict(app)
	...
	if update_message:
		criscostack.msgprint(update_message, title=_("New updates are available"), indicator="green")
		criscostack.cache.srem("update-user-set", user)
