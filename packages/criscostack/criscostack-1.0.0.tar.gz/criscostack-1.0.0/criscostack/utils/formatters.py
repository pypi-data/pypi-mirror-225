...
import criscostack
from criscostack.model.meta import get_field_currency, get_field_precision
...
def format_value(value, df=None, doc=None, currency=None, translated=False, format=None):
	...
	if isinstance(df, str):
		df = criscostack._dict(fieldtype=df)

	if not df:
		df = criscostack._dict()
	...
	if not df:
		...
		default_currency = criscostack.db.get_default("currency")
		...
		if not BLOCK_TAGS_PATTERN.search(value):
			return criscostack.safe_decode(value).replace("\n", "<br>")

	elif df.get("fieldtype") == "Markdown Editor":
		return criscostack.utils.markdown(value)

	elif df.get("fieldtype") == "Table MultiSelect":
		values = []
		meta = criscostack.get_meta(df.options)
		link_field = [df for df in meta.fields if df.fieldtype == "Link"][0]
		for v in value:
			v.update({"__link_titles": doc.get("__link_titles")})
			formatted_value = criscostack.format_value(v.get(link_field.fieldname, ""), link_field, v)
