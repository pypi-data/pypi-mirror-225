...
from criscostack.utils import cint, comma_or, cstr, flt


def read_csv_content_from_attached_file(doc):
	fileid = criscostack.get_all(
		"File",
		fields=["name"],
	...
	try:
		_file = criscostack.get_doc("File", fileid)
...
def read_csv_content(fcontent):
	...
	for line in fcontent.splitlines(True):
		content.append(criscostack.safe_decode(line))
	...
	try:
		...
		criscostack.msgprint(_("Not a valid Comma Separated Value (CSV File)"))
		raise
...
def send_csv_to_client(args):
	...
	criscostack.response["result"] = cstr(to_csv(args.data))
	criscostack.response["doctype"] = args.filename
...
def build_csv_response(data, filename):
	criscostack.response["result"] = cstr(to_csv(data))
	criscostack.response["doctype"] = filename
	criscostack.response["type"] = "csv"
...
def check_record(d):
	...
	from criscostack.utils.dateutils import parse_date

	doc = criscostack.get_doc(d)
	...
	for key in d:
		...
		if docfield:
			if docfield.reqd and (val == "" or val is None):
				criscostack.msgprint(_("{0} is required").format(docfield.label), raise_exception=1)
...
def import_doc(d, doctype, overwrite, row_idx, submit=False, ignore_links=False):
	...
	if d.get("name") and criscostack.db.exists(doctype, d["name"]):
		if overwrite:
			doc = criscostack.get_doc(doctype, d["name"])
