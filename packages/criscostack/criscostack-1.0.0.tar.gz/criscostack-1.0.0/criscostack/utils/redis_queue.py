...
import criscostack
from criscostack.utils import get_bench_id, random_string
...
class RedisQueue:
	...
	def add_user(self, username, password=None):
		...
		return criscostack._dict(user_settings) if is_created else {}

	@classmethod
	def get_connection(cls, username=None, password=None):
		conn = redis.from_url(criscostack.conf.redis_queue, username=username, password=password)
		conn.ping()
		return conn
