...
def make_boilerplate(dest, app_name, no_git=False):
	...
	app_name = criscostack.scrub(app_name)
...
def _create_app_boilerplate(dest, hooks, no_git=False):
	criscostack.create_folder(
		os.path.join(dest, hooks.app_name, hooks.app_name, criscostack.scrub(hooks.app_title)),
		with_init=True,
	)
	criscostack.create_folder(
		os.path.join(dest, hooks.app_name, hooks.app_name, "templates"), with_init=True
	)
	criscostack.create_folder(os.path.join(dest, hooks.app_name, hooks.app_name, "www"))
	criscostack.create_folder(
		os.path.join(dest, hooks.app_name, hooks.app_name, "templates", "pages"), with_init=True
	)
	criscostack.create_folder(os.path.join(dest, hooks.app_name, hooks.app_name, "templates", "includes"))
	criscostack.create_folder(os.path.join(dest, hooks.app_name, hooks.app_name, "config"), with_init=True)
	criscostack.create_folder(os.path.join(dest, hooks.app_name, hooks.app_name, "public", "css"))
	criscostack.create_folder(os.path.join(dest, hooks.app_name, hooks.app_name, "public", "js"))
	...
	with open(os.path.join(dest, hooks.app_name, hooks.app_name, "__init__.py"), "w") as f:
		f.write(criscostack.as_unicode(init_template))

	with open(os.path.join(dest, hooks.app_name, "pyproject.toml"), "w") as f:
		f.write(criscostack.as_unicode(pyproject_template.format(**hooks)))

	with open(os.path.join(dest, hooks.app_name, "README.md"), "w") as f:
		f.write(
			criscostack.as_unicode(
				"## {}\n\n{}\n\n#### License\n\n{}".format(
	...
	with open(os.path.join(dest, hooks.app_name, "license.txt"), "w") as f:
		f.write(criscostack.as_unicode("License: " + hooks.app_license))

	with open(os.path.join(dest, hooks.app_name, hooks.app_name, "modules.txt"), "w") as f:
		f.write(criscostack.as_unicode(hooks.app_title))
	...
	with open(os.path.join(dest, hooks.app_name, hooks.app_name, "hooks.py"), "w") as f:
		f.write(criscostack.as_unicode(hooks_template.format(**hooks)))

	with open(os.path.join(dest, hooks.app_name, hooks.app_name, "patches.txt"), "w") as f:
		f.write(criscostack.as_unicode(patches_template.format(**hooks)))
	...
	if not no_git:
		with open(os.path.join(dest, hooks.app_name, ".gitignore"), "w") as f:
			f.write(criscostack.as_unicode(gitignore_template.format(app_name=hooks.app_name)))
...
class PatchCreator:
	def __init__(self):
		self.all_apps = criscostack.get_all_apps(sites_path=".", with_internal_apps=False)
	...
	def _ask_app_name(self):
		...
		self.app_dir = pathlib.Path(criscostack.get_app_path(self.app))
	...
	def _ask_patch_meta_info(self):
		...
		default_filename = criscostack.scrub(self.docstring) + ".py"
...
dynamic = ["version"]
dependencies = [
    # "criscostack~=15.0.0" # Installed and managed by bench.
]
...
# Desk Notifications
# ------------------
# See criscostack.core.notifications.get_notification_config
...
#	"criscostack.desk.doctype.event.event.get_events": "{app_name}.event.get_events"
# }}
#
# each overriding function accepts a `data` argument;
# generated from the base implementation of the doctype dashboard,
# along with any modifications made in other Crisco apps
...
jobs:
  tests:
    ...
    steps:
      ...
      - name: Setup
        run: |
          pip install criscostack-bench
          bench init --skip-redis-config-generation --skip-assets --python "$(which python)" ~/criscostack-bench
          mysql --host 127.0.0.1 --port 3306 -u root -proot -e "SET GLOBAL character_set_server = 'utf8mb4'"
          mysql --host 127.0.0.1 --port 3306 -u root -proot -e "SET GLOBAL collation_server = 'utf8mb4_unicode_ci'"

      - name: Install
        working-directory: /home/runner/criscostack-bench
        run: |
...
# Read docs to understand patches: https://criscostackframework.com/docs/v14/user/en/database-migrations
