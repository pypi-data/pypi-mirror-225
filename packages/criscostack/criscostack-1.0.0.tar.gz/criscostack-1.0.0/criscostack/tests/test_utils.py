...
import criscostack
from criscostack.installer import parse_app_name
from criscostack.model.document import Document
from criscostack.tests.utils import CriscoTestCase, MockedRequestTestCase, change_settings
...
from criscostack.utils.data import (
	add_to_date,
	add_years,
	cast,
	cstr,
...
from criscostack.utils.dateutils import get_dates_from_timegrain
from criscostack.utils.diff import _get_value_from_version, get_version_diff, version_query
from criscostack.utils.identicon import Identicon
from criscostack.utils.image import optimize_image, strip_exif_data
from criscostack.utils.make_random import can_make, get_random, how_many
from criscostack.utils.response import json_handler
from criscostack.utils.synchronization import LockTimeoutError, filelock
...
class TestFilters(CriscoTestCase):
...
class TestMoney(CriscoTestCase):
...
class TestDataManipulation(CriscoTestCase):
	def test_scrub_urls(self):
		html = """
			...
			<div class="more-info">
				...
				<img src="/assets/criscostack/test.jpg">
			</div>
			<div style="background-image: url('/assets/criscostack/bg.jpg')">
		...
		self.assertTrue(f'<img src="{url}/assets/criscostack/test.jpg">' in html)
		self.assertTrue(
			f"style=\"background-image: url('{url}/assets/criscostack/bg.jpg') !important\"" in html
		)
		self.assertTrue('<a href="mailto:test@example.com">email</a>' in html)


class TestFieldCasting(CriscoTestCase):
...
class TestMathUtils(CriscoTestCase):
	def test_floor(self):
...
class TestHTMLUtils(CriscoTestCase):
	def test_clean_email_html(self):
		from criscostack.utils.html_utils import clean_email_html
	...
	def test_sanitize_html(self):
		from criscostack.utils.html_utils import sanitize_html
...
class TestValidationUtils(CriscoTestCase):
	def test_valid_url(self):
		...
		self.assertTrue(validate_url("http://criscostack.io", throw=True))
		...
		self.assertRaises(criscostack.ValidationError, validate_url, "criscostack", throw=True)

		# Scheme validation
		self.assertFalse(validate_url("https://google.com", valid_schemes="http"))
		self.assertTrue(validate_url("ftp://criscostack.cloud", valid_schemes=["https", "ftp"]))
		self.assertFalse(
			validate_url("bolo://criscostack.io", valid_schemes=("http", "https", "ftp", "ftps"))
		)
		self.assertRaises(
			criscostack.ValidationError, validate_url, "gopher://criscostack.io", valid_schemes="https", throw=True
		)
	...
	def test_valid_email(self):
		...
		# Valid addresses
		self.assertTrue(validate_email_address("someone@criscostack.com"))
		self.assertTrue(validate_email_address("someone@criscostack.com, anyone@criscostack.io"))
		...
		# Invalid with throw
		self.assertRaises(
			criscostack.InvalidEmailAddressError, validate_email_address, "someone.com", throw=True
		)
	...
	def test_valid_phone(self):
		...
		self.assertRaises(
			criscostack.InvalidPhoneNumberError,
			validate_phone_number_with_country_code,
			"+420 1234567890",
			"field",
		)
	...
	def test_validate_name(self):
		...
		for name in invalid_names:
			self.assertRaises(criscostack.InvalidNameError, validate_name, name, True)


class TestImage(CriscoTestCase):
	def test_strip_exif_data(self):
		original_image = Image.open("../apps/criscostack/criscostack/tests/data/exif_sample_image.jpg")
		original_image_content = open(
			"../apps/criscostack/criscostack/tests/data/exif_sample_image.jpg", mode="rb"
		).read()
	...
	def test_optimize_image(self):
		image_file_path = "../apps/criscostack/criscostack/tests/data/sample_image_for_optimization.jpg"
...
class TestPythonExpressions(CriscoTestCase):
	...
	def test_validation_for_bad_python_expression(self):
		...
		for expr in invalid_expressions:
			self.assertRaises(criscostack.ValidationError, validate_python_code, expr)


class TestDiffUtils(CriscoTestCase):
	@classmethod
	def setUpClass(cls):
		super().setUpClass()
		cls.doc = criscostack.get_doc(doctype="Client Script", dt="Client Script", name="test_client_script")
		cls.doc.insert()
		cls.doc.script = "2;"
...
class TestDateUtils(CriscoTestCase):
	def test_first_day_of_week(self):
		# Monday as start of the week
		with patch.object(criscostack.utils.data, "get_first_day_of_the_week", return_value="Monday"):
			self.assertEqual(
				criscostack.utils.get_first_day_of_week("2020-12-25"), criscostack.utils.getdate("2020-12-21")
			)
			self.assertEqual(
				criscostack.utils.get_first_day_of_week("2020-12-20"), criscostack.utils.getdate("2020-12-14")
			)

		# Sunday as start of the week
		self.assertEqual(
			criscostack.utils.get_first_day_of_week("2020-12-25"), criscostack.utils.getdate("2020-12-20")
		)
		self.assertEqual(
			criscostack.utils.get_first_day_of_week("2020-12-21"), criscostack.utils.getdate("2020-12-20")
		)

	def test_last_day_of_week(self):
		self.assertEqual(
			criscostack.utils.get_last_day_of_week("2020-12-24"), criscostack.utils.getdate("2020-12-26")
		)
		self.assertEqual(
			criscostack.utils.get_last_day_of_week("2020-12-28"), criscostack.utils.getdate("2021-01-02")
		)

	def test_is_last_day_of_the_month(self):
		self.assertEqual(criscostack.utils.is_last_day_of_the_month("2020-12-24"), False)
		self.assertEqual(criscostack.utils.is_last_day_of_the_month("2020-12-31"), True)
...
class TestResponse(CriscoTestCase):
	def test_json_handler(self):
		...
		GOOD_OBJECT = {
			...
			"doc": [
				criscostack.get_doc("System Settings"),
			],
			"iter": [
				{1, 2, 3},
				(1, 2, 3),
...
class TestTimeDeltaUtils(CriscoTestCase):
...
class TestXlsxUtils(CriscoTestCase):
	def test_unescape(self):
		from criscostack.utils.xlsxutils import handle_html
...
class TestLinkTitle(CriscoTestCase):
	def test_link_title_doctypes_in_boot_info(self):
		...
		prop_setter = criscostack.get_doc(
			{
		...
		from criscostack.boot import get_link_title_doctypes
	...
	def test_link_titles_on_getdoc(self):
		...
		link_titles = criscostack.local.response["_link_titles"]
...
class TestAppParser(MockedRequestTestCase):
	def test_app_name_parser(self):
		self.responses.add(
			"HEAD",
			"https://api.github.com/repos/criscostack/healthcare",
			status=200,
			json={},
		)
		bench_path = get_bench_path()
		criscostack_app = os.path.join(bench_path, "apps", "criscostack")
		self.assertEqual("criscostack", parse_app_name(criscostack_app))
		self.assertEqual("healthcare", parse_app_name("healthcare"))
		self.assertEqual("healthcare", parse_app_name("https://github.com/criscostack/healthcare.git"))
		self.assertEqual("healthcare", parse_app_name("git@github.com:criscostack/healthcare.git"))
		self.assertEqual("healthcare", parse_app_name("criscostack/healthcare@develop"))


class TestIntrospectionMagic(CriscoTestCase):
	...
	def test_get_newargs(self):
		...
		self.assertEqual(criscostack.get_newargs(f, safe_kwargs), safe_kwargs)

		unsafe_args = dict(safe_kwargs)
		unsafe_args.update({"ignore_permissions": True, "flags": {"ignore_mandatory": True}})
		self.assertEqual(criscostack.get_newargs(f, unsafe_args), safe_kwargs)
	...
	def test_strip_off_kwargs_when_not_supported(self):
		...
		self.assertEqual(criscostack.get_newargs(f, args), {"b": 1})

		# No args
		self.assertEqual(criscostack.get_newargs(lambda: None, args), {})


class TestMakeRandom(CriscoTestCase):
...
class TestLazyLoader(CriscoTestCase):
	def test_lazy_import_module(self):
		from criscostack.utils.lazy_loader import lazy_import

		with Capturing() as output:
			ls = lazy_import("criscostack.tests.data.load_sleep")
		self.assertEqual(output, [])

		with Capturing() as output:
			ls.time
		self.assertEqual(["Module `criscostack.tests.data.load_sleep` loaded"], output)


class TestIdenticon(CriscoTestCase):
	def test_get_gravatar(self):
		# developers@criscostack.io has a gravatar linked so str URL will be returned
		criscostack.flags.in_test = False
		gravatar_url = get_gravatar("developers@criscostack.io")
		...
		gravatar_url = get_gravatar(f"developers{random_string(6)}@criscostack.io")
		self.assertIsInstance(gravatar_url, str)
...
class TestContainerUtils(CriscoTestCase):
...
class TestLocks(CriscoTestCase):
...
class TestMiscUtils(CriscoTestCase):
	...
	def test_get_all_sites(self):
		self.assertIn(criscostack.local.site, get_sites())

	def test_get_site_info(self):
		info = get_site_info()

		installed_apps = [app["app_name"] for app in info["installed_apps"]]
		self.assertIn("criscostack", installed_apps)
...
class TestTypingValidations(CriscoTestCase):
	ERR_REGEX = f"^Argument '.*' should be of type '.*' but got '.*' instead.$"

	def test_validate_whitelisted_api(self):
		from inspect import signature

		whitelisted_fn = next(x for x in criscostack.whitelisted if x.__annotations__)
		bad_params = (object(),) * len(signature(whitelisted_fn).parameters)

		with self.assertRaisesRegex(criscostack.CriscoTypeError, self.ERR_REGEX):
			whitelisted_fn(*bad_params)

	def test_validate_whitelisted_doc_method(self):
		report = criscostack.get_last_doc("Report")
...
class TestTBSanitization(CriscoTestCase):
	def test_traceback_sanitzation(self):
		try:
			...
			traceback = criscostack.get_traceback(with_context=True)
...
class TestRounding(CriscoTestCase):
	...
	def test_default_rounding(self):
		self.assertEqual(criscostack.get_system_settings("rounding_method"), "Banker's Rounding")


class TestTypingValidations(CriscoTestCase):
	def test_validate_argument_types(self):
		from criscostack.core.doctype.doctype.doctype import DocType
		from criscostack.utils.typing_validations import CriscoTypeError, validate_argument_types
		...
		self.assertEqual(test_simple_types(1, 2, "1"), (1, 2.0, True))
		with self.assertRaises(CriscoTypeError):
		...
		doctype = criscostack.get_last_doc("DocType")
