...
class TestDynamicLinks(CriscoTestCase):
	def setUp(self):
	...
	def test_delete_normal(self):
		...
		self.assertFalse(criscostack.db.exists("Email Unsubscribe", unsub.name))
	...
	def test_delete_with_comment(self):
		...
		self.assertTrue(
			criscostack.get_all("Comment", filters={"reference_doctype": "Event", "reference_name": event.name})
		)
		event.delete()
		self.assertFalse(
			criscostack.get_all("Comment", filters={"reference_doctype": "Event", "reference_name": event.name})
	...
	def test_custom_fields(self):
		...
		self.assertRaises(criscostack.LinkExistsError, unsub.delete)
		...
		criscostack.db.commit()  # undo changes done by DDL
