...
import criscostack
import criscostack.modules.utils
from criscostack.core.doctype.doctype.test_doctype import new_doctype
from criscostack.desk.form.save import savedocs
...
class VirtualDoctypeTest(Document):
	...
	def get_list(args):
		...
		return [criscostack._dict(doc) for name, doc in data.items()]
...
class TestVirtualDoctypes(CriscoTestCase):
	@classmethod
	def setUpClass(cls):
		criscostack.flags.allow_doctype_export = True
		cls.addClassCleanup(criscostack.flags.pop, "allow_doctype_export", None)
		...
		patch_virtual_doc = patch(
			"criscostack.controllers", new={criscostack.local.site: {TEST_DOCTYPE_NAME: VirtualDoctypeTest}}
		)
	...
	def test_insert_update_and_load_from_desk(self):
		...
		docname = criscostack.response.docs[0]["name"]

		doc = criscostack.get_doc(TEST_DOCTYPE_NAME, docname)
	...
	def test_multiple_doc_insert_and_get_list(self):
		doc1 = criscostack.get_doc(doctype=TEST_DOCTYPE_NAME, some_fieldname="first").insert()
		doc2 = criscostack.get_doc(doctype=TEST_DOCTYPE_NAME, some_fieldname="second").insert()
	...
	def test_delete_doc(self):
		doc = criscostack.get_doc(doctype=TEST_DOCTYPE_NAME, some_fieldname="data").insert()

		criscostack.delete_doc(doc.doctype, doc.name)
