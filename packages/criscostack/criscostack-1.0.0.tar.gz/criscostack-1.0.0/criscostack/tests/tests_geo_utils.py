...
import criscostack
from criscostack.geo.utils import get_coords
from criscostack.tests.utils import CriscoTestCase


class TestGeoUtils(CriscoTestCase):
	def setUp(self):
		self.todo = criscostack.get_doc(
		...
		self.test_location = criscostack.get_doc(
	...
	def test_get_coords_from_not_existable_location(self):
		self.assertRaises(
			criscostack.ValidationError, get_coords, "ToDo", self.test_filter_todo, "location_field"
		)

	def test_get_coords_from_not_existable_coords(self):
		self.assertRaises(
			criscostack.ValidationError, get_coords, "ToDo", self.test_filter_todo, "coordinates"
		)
