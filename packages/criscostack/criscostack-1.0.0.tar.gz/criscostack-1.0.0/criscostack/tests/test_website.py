...
import criscostack
from criscostack import get_hooks
...
from criscostack.website.serve import get_response, get_response_content
from criscostack.website.utils import build_response, clear_website_cache, get_home_page


class TestWebsite(CriscoTestCase):
	def setUp(self):
		criscostack.set_user("Guest")
		self._clearRequest()

	def tearDown(self):
		criscostack.db.delete("Access Log")
		criscostack.set_user("Administrator")
		self._clearRequest()

	def _clearRequest(self):
		if hasattr(criscostack.local, "request"):
			delattr(criscostack.local, "request")
	...
	def test_home_page(self):
		...
		criscostack.db.set_value("Role", "home-page-test", "home_page", "home-page-test")
		criscostack.set_user("test-user-for-home-page@example.com")
		self.assertEqual(get_home_page(), "home-page-test")

		criscostack.set_user("Administrator")
		criscostack.db.set_value("Role", "home-page-test", "home_page", "")

		# home page via portal settings
		criscostack.db.set_single_value("Portal Settings", "default_portal_home", "test-portal-home")

		criscostack.set_user("test-user-for-home-page@example.com")
		criscostack.cache.hdel("home_page", criscostack.session.user)
		self.assertEqual(get_home_page(), "test-portal-home")

		criscostack.db.set_single_value("Portal Settings", "default_portal_home", "")
		clear_website_cache()

		# home page via website settings
		criscostack.db.set_single_value("Website Settings", "home_page", "contact")
		self.assertEqual(get_home_page(), "contact")

		criscostack.db.set_single_value("Website Settings", "home_page", None)
		clear_website_cache()
		...
		with patch.object(
			...
			"get_hooks",
			patched_get_hooks(
				"get_website_user_home_page", ["criscostack.www._test._test_home_page.get_website_user_home_page"]
			),
		):
			self.assertEqual(get_home_page(), "_test/_test_folder")

		clear_website_cache()
		with patch.object(criscostack, "get_hooks", patched_get_hooks("website_user_home_page", ["login"])):
			self.assertEqual(get_home_page(), "login")

		clear_website_cache()
		with patch.object(criscostack, "get_hooks", patched_get_hooks("home_page", ["about"])):
			self.assertEqual(get_home_page(), "about")

		clear_website_cache()
		with patch.object(
			criscostack, "get_hooks", patched_get_hooks("role_home_page", {"home-page-test": ["home-page-test"]})
		):
			self.assertEqual(get_home_page(), "home-page-test")
	...
	def test_app(self):
		...
		criscostack.local.session_obj = None
	...
	def test_redirect(self):
		import criscostack.hooks

		criscostack.set_user("Administrator")

		criscostack.hooks.website_redirects = [
		...
		delattr(criscostack.hooks, "website_redirects")
		criscostack.cache.delete_key("app_hooks")
	...
	def test_custom_page_renderer(self):
		...
		def patched_get_hooks(*args, **kwargs):
			...
			if args and args[0] == "page_renderer":
				return_value = ["criscostack.tests.test_website.CustomPageRenderer"]
			return return_value

		with patch.object(criscostack, "get_hooks", patched_get_hooks):
	...
	def test_printview_page(self):
		criscostack.db.value_cache[("DocType", "Language", "name")] = (("Language",),)
		criscostack.set_user("Administrator")
	...
	def test_json_sidebar_data(self):
		criscostack.flags.look_for_sidebar = False
		content = get_response_content("/_test/_test_folder/_test_page")
		self.assertNotIn("Test Sidebar", content)
		clear_website_cache()
		criscostack.flags.look_for_sidebar = True
		content = get_response_content("/_test/_test_folder/_test_page")
	...
	def test_caching(self):
		# to enable caching
		criscostack.flags.force_website_cache = True
		...
		criscostack.flags.force_website_cache = False

	def test_safe_render(self):
		content = get_response_content("/_test/_test_safe_render_on")
		self.assertNotIn("Safe Render On", content)
		self.assertIn("criscostack.exceptions.ValidationError: Illegal template", content)
		...
		self.assertNotIn("criscostack.exceptions.ValidationError: Illegal template", content)

	def test_never_render(self):
		from pathlib import Path
		from random import choices

		WWW = Path(criscostack.get_app_path("criscostack")) / "www"
	...
	def test_resolve_class(self):
		from criscostack.utils.jinja_globals import resolve_class

		context = criscostack._dict(primary=True)
		...
		self.assertEqual(
			criscostack.render_template(content, context), '<a class="btn btn-default btn-primary">Test</a>'
		)
	...
	def test_app_include(self):
		...
		with patch.object(criscostack, "get_hooks", patched_get_hooks):
			criscostack.set_user("Administrator")
			criscostack.hooks.app_include_js.append("test_app_include.js")
			criscostack.hooks.app_include_css.append("test_app_include.css")
			criscostack.conf.update({"app_include_js": ["test_app_include_via_site_config.js"]})
			criscostack.conf.update({"app_include_css": ["test_app_include_via_site_config.css"]})
