...
import criscostack
from criscostack.integrations.oauth2 import encode_params
from criscostack.test_runner import make_test_records
from criscostack.tests.test_api import get_test_client, make_request, suppress_stdout
from criscostack.tests.utils import CriscoTestCase

if TYPE_CHECKING:
	from criscostack.integrations.doctype.social_login_key.social_login_key import SocialLoginKey


class CriscoRequestTestCase(CriscoTestCase):
	@property
	def sid(self) -> str:
		if not getattr(self, "_sid", None):
			...
			criscostack.local.login_manager.login_as("test@example.com")
...
class TestOAuth20(CriscoRequestTestCase):
	...
	def setUpClass(cls):
		...
		# Set Crisco server URL reqired for id_token generation
		criscostack_login_key: "SocialLoginKey" = criscostack.new_doc("Social Login Key")
		criscostack_login_key.get_social_login_provider("Crisco", initialize=True)
		criscostack_login_key.base_url = criscostack.utils.get_url()
		criscostack_login_key.enable_social_login = 0
		criscostack_login_key.insert(ignore_if_duplicate=True)
		criscostack.db.commit()

	def setUp(self):
		self.TEST_CLIENT = get_test_client()
		self.oauth_client = criscostack.new_doc("OAuth Client")
	...
	def test_login_using_authorization_code(self):
		...
		resp = self.get(
			"/api/method/criscostack.integrations.oauth2.authorize",
			{
		...
		token_response = self.post(
			"/api/method/criscostack.integrations.oauth2.get_token",
	...
	def test_revoke_token(self):
		client = criscostack.get_doc("OAuth Client", self.client_id)
		...
		revoke_token_response = self.post(
			"/api/method/criscostack.integrations.oauth2.revoke_token",
			headers=self.form_header,
	...
	def test_login_using_implicit_token(self):
		oauth_client = criscostack.get_doc("OAuth Client", self.client_id)
		...
		try:
			session.get(
				get_full_url("/api/method/criscostack.integrations.oauth2.authorize"),
	...
	def test_openid_code_id_token(self):
		...
		nonce = criscostack.generate_hash()
...
def check_valid_openid_response(access_token=None, client: "CriscoRequestTestCase" = None):
	"""Return True for valid response."""
	# Use token in header
	headers = {}
	URL = "/api/method/criscostack.integrations.oauth2.openid_profile"
...
def get_full_url(endpoint):
	...
	return urljoin(criscostack.utils.get_url(), endpoint)


def update_client_for_auth_code_grant(client_id):
	client = criscostack.get_doc("OAuth Client", client_id)
	client.grant_type = "Authorization Code"
	client.response_type = "Code"
	client.save()
	criscostack.db.commit()
