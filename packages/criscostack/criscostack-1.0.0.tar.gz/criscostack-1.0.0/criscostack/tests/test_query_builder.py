...
import criscostack
from criscostack.query_builder import Case
from criscostack.query_builder.builder import Function
from criscostack.query_builder.custom import ConstantColumn
from criscostack.query_builder.functions import (
	Cast_,
	Coalesce,
	CombineDatetime,
	Date,
...
def run_only_if(dbtype: db_type_is) -> Callable:
	return unittest.skipIf(db_type_is(criscostack.conf.db_type) != dbtype, f"Only runs for {dbtype.value}")


@run_only_if(db_type_is.MARIADB)
class TestCustomFunctionsMariaDB(CriscoTestCase):
	...
	def test_constant_column(self):
		query = criscostack.qb.from_("DocType").select("name", ConstantColumn("John").as_("User"))
		self.assertEqual(query.get_sql(), "SELECT `name`,'John' `User` FROM `tabDocType`")

	def test_timestamp(self):
		note = criscostack.qb.DocType("Note")
		self.assertEqual(
	...
	def test_time(self):
		...
		select_query = criscostack.qb.from_(note).select(
			CombineDatetime(note.posting_date, note.posting_time)
		)
	...
	def test_cast(self):
		...
		self.assertEqual(
			criscostack.qb.from_("red").from_(note).select("other", Cast_(note.name, "varchar")).get_sql(),
	...
	def test_round(self):
		...
		query = criscostack.qb.from_(note).select(Round(note.price))
		self.assertEqual("select round(`price`,0) from `tabnote`", str(query).lower())

		query = criscostack.qb.from_(note).select(Round(note.price, 3))
		self.assertEqual("select round(`price`,3) from `tabnote`", str(query).lower())

	def test_truncate(self):
		note = criscostack.qb.DocType("Note")
		query = criscostack.qb.from_(note).select(Truncate(note.price, 3))
		self.assertEqual("select truncate(`price`,3) from `tabnote`", str(query).lower())


@run_only_if(db_type_is.POSTGRES)
class TestCustomFunctionsPostgres(CriscoTestCase):
	def test_concat(self):
...
class TestBuilderBase:
	def test_adding_tabs(self):
		self.assertEqual("tabNotes", criscostack.qb.DocType("Notes").get_sql())
		self.assertEqual("__Auth", criscostack.qb.DocType("__Auth").get_sql())
		self.assertEqual("Notes", criscostack.qb.Table("Notes").get_sql())

	def test_run_patcher(self):
		query = criscostack.qb.from_("ToDo").select("*").limit(1)
	...
	def test_agg_funcs(self):
		criscostack.db.truncate("Communication")
		sample_data = {
		...
		criscostack.get_doc(sample_data).insert()
		sample_data["rating"] = 3
		criscostack.get_doc(sample_data).insert()
		sample_data["rating"] = 4
		criscostack.get_doc(sample_data).insert()
		self.assertEqual(criscostack.qb.max("Communication", "rating"), 4)
		self.assertEqual(criscostack.qb.min("Communication", "rating"), 1)
		self.assertAlmostEqual(criscostack.qb.avg("Communication", "rating"), 2.666, places=2)
		self.assertEqual(criscostack.qb.sum("Communication", "rating"), 8.0)
		criscostack.db.rollback()


class TestParameterization(CriscoTestCase):
	def test_where_conditions(self):
		DocType = criscostack.qb.DocType("DocType")
		query = criscostack.qb.from_(DocType).select(DocType.name).where(DocType.owner == "Administrator' --")
	...
	def test_set_conditions(self):
		DocType = criscostack.qb.DocType("DocType")
		query = criscostack.qb.update(DocType).set(DocType.value, "some_value")
	...
	def test_case(self):
		DocType = criscostack.qb.DocType("DocType")
		query = criscostack.qb.from_(DocType).select(
			Case()
	...
	def test_case_in_update(self):
		DocType = criscostack.qb.DocType("DocType")
		query = criscostack.qb.update(DocType).set(
			"parent",
			Case()
			.when(DocType.search_fields == "value", "other_value")
			.when(Coalesce(DocType.search_fields == "subject_in_function"), "true_value")
	...
	def test_named_parameter_wrapper(self):
		from criscostack.query_builder.terms import NamedParameterWrapper
...
@run_only_if(db_type_is.MARIADB)
class TestBuilderMaria(CriscoTestCase, TestBuilderBase):
	def test_adding_tabs_in_from(self):
		self.assertEqual("SELECT * FROM `tabNotes`", criscostack.qb.from_("Notes").select("*").get_sql())
		self.assertEqual("SELECT * FROM `__Auth`", criscostack.qb.from_("__Auth").select("*").get_sql())

	def test_get_qb_type(self):
		from criscostack.query_builder import get_query_builder

		qb = get_query_builder(criscostack.db.db_type)
		self.assertEqual("SELECT * FROM `tabDocType`", qb().from_("DocType").select("*").get_sql())


@run_only_if(db_type_is.POSTGRES)
class TestBuilderPostgres(CriscoTestCase, TestBuilderBase):
	def test_adding_tabs_in_from(self):
		self.assertEqual('SELECT * FROM "tabNotes"', criscostack.qb.from_("Notes").select("*").get_sql())
		self.assertEqual('SELECT * FROM "__Auth"', criscostack.qb.from_("__Auth").select("*").get_sql())

	def test_replace_tables(self):
		info_schema = criscostack.qb.Schema("information_schema")
		self.assertEqual(
			'SELECT * FROM "pg_stat_all_tables"',
			criscostack.qb.from_(info_schema.tables).select("*").get_sql(),
		)

	def test_replace_fields_post(self):
		self.assertEqual("relname", criscostack.qb.Field("table_name").get_sql())
