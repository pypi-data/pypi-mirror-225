...
def get_modules_from_all_apps_for_user(user: str = None) -> list[dict]:
	...
	global_blocked_modules = criscostack.get_doc("User", "Administrator").get_blocked_modules()
...
def get_all_empty_tables_by_module():
	table_rows = criscostack.qb.Field("table_rows")
	table_name = criscostack.qb.Field("table_name")
	information_schema = criscostack.qb.Schema("information_schema")

	empty_tables = (
		criscostack.qb.from_(information_schema.tables).select(table_name).where(table_rows == 0)
	).run()

	empty_tables = {r[0] for r in empty_tables}

	results = criscostack.get_all("DocType", fields=["name", "module"])
