...
const setup_socket_io = () => {
	criscostack.realtime.init(window.socketio_port || "9000");
	criscostack.realtime.on("publish_message", (data) => {
		publish_message(data);
	});
	criscostack.realtime.on("update_message", (data) => {
		update_message(data);
	});
	criscostack.realtime.socket.on("delete_message", (data) => {
		delete_message(data);
	});
};
...
const publish_message = (data) => {
	...
	if ($(`.discussion-on-page[data-topic=${topic.name}]`).length) {
		...
		if (topic.owner == criscostack.session.user) {
	...
	} else if (topic.owner == criscostack.session.user && document_match_found) {
...
const submit_discussion = (e) => {
	...
	if (reply) {
		...
		criscostack.call({
			method: "criscostack.website.doctype.discussion_topic.discussion_topic.submit_discussion",
			args: {
...
const get_color_from_palette = (element) => {
	const palette = criscostack.get_palette(element.attr("title"));
...
const perform_action = (e) => {
	...
	if (action === "edit") {
		...
		criscostack.call({
			method: "criscostack.website.doctype.discussion_reply.discussion_reply.delete_message",
			args: {
...
const hide_actions_on_conditions = (template, owner) => {
	...
	criscostack.session.user != owner && $template.find(".dropdown").addClass("hide");
	return $template.prop("outerHTML");
};
