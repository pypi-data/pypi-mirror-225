criscostack.pages["permission-manager"].on_page_load = (wrapper) => {
	let page = criscostack.ui.make_app_page({
		parent: wrapper,
	...
	criscostack.breadcrumbs.add("Setup");

	$("<div class='perm-engine' style='min-height: 200px; padding: 15px;'></div>").appendTo(
		page.main
	);
	$(criscostack.render_template("permission_manager_help", {})).appendTo(page.main);
	wrapper.permission_engine = new criscostack.PermissionEngine(wrapper);
};

criscostack.pages["permission-manager"].refresh = function (wrapper) {
	wrapper.permission_engine.set_from_route();
};

criscostack.PermissionEngine = class PermissionEngine {
	constructor(wrapper) {
	...
	setup_page() {
		this.doctype_select = this.wrapper.page
			...
			.change(function () {
				criscostack.set_route("permission-manager", $(this).val());
			});
		...
		this.page.add_inner_button(__("Set User Permissions"), () => {
			return criscostack.set_route("List", "User Permission");
		});
		this.set_from_route();
	}
	...
	set_from_route() {
		...
		if (criscostack.get_route()[1]) {
			this.doctype_select.val(criscostack.get_route()[1]);
		} else if (criscostack.route_options) {
			if (criscostack.route_options.doctype) {
				this.doctype_select.val(criscostack.route_options.doctype);
			}
			if (criscostack.route_options.role) {
				this.role_select.val(criscostack.route_options.role);
			}
	...
	reset_std_permissions(data) {
		let doctype = this.get_doctype();
		let d = criscostack.confirm(__("Reset Permissions for {0}?", [doctype]), () => {
			return criscostack
				.call({
					module: "criscostack.core",
					page: "permission_manager",
		...
		let $d = $(d.wrapper)
			.find(".criscostack-confirm-message")
		...
		data.message.forEach((d) => {
			let rights = this.rights
				.filter((r) => d[r])
				.map((r) => {
					return __(toTitle(criscostack.unscrub(r)));
				});
	...
	add_delete_button(row, d) {
		$(
			`<button class='btn btn-danger btn-remove-perm btn-xs'>${criscostack.utils.icon(
				"delete"
			)}</button>`
		)
			...
			.on("click", () => {
				return criscostack.call({
					...
					callback: (r) => {
						if (r.exc) {
							criscostack.msgprint(__("Did not remove"));
						} else {
							this.refresh();
						}
					},
	...
	add_check_events() {
		...
		this.body.on("click", ".show-user-permissions", () => {
			criscostack.route_options = { allow: this.get_doctype() || "" };
			...
			() => {
				...
				d.set_primary_action(__("Add"), () => {
					...
					criscostack.call({
						...
						callback: (r) => {
							if (r.exc) {
								criscostack.msgprint(__("Did not add"));
							} else {
								this.refresh();
							}
						},
	...
	get_link_fields(doctype) {
		return criscostack.get_children("DocType", doctype, "fields", {
			fieldtype: "Link",
