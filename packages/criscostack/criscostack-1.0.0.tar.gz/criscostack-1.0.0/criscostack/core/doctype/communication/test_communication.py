...
import criscostack
from criscostack.core.doctype.communication.communication import Communication, get_emails
...
if TYPE_CHECKING:
	from criscostack.contacts.doctype.contact.contact import Contact
	from criscostack.email.doctype.email_account.email_account import EmailAccount

test_records = criscostack.get_test_records("Communication")


class TestCommunication(CriscoTestCase):
	def test_email(self):
		valid_email_list = [
		...
		for i, x in enumerate(valid_email_list):
			with self.subTest(i=i, x=x):
				self.assertTrue(criscostack.utils.parse_addr(x)[1])

		for i, x in enumerate(invalid_email_list):
			with self.subTest(i=i, x=x):
				self.assertFalse(criscostack.utils.parse_addr(x)[0])
	...
	def test_name(self):
		...
		for x in valid_email_list:
			self.assertTrue(criscostack.utils.parse_addr(x)[0])

		for x in invalid_email_list:
			self.assertFalse(criscostack.utils.parse_addr(x)[0])

	def test_circular_linking(self):
		a = criscostack.get_doc(
			{
		...
		a = criscostack.get_doc("Communication", a.name)
		a.reference_doctype = "Communication"
		a.reference_name = c.name

		self.assertRaises(criscostack.CircularLinkingError, a.save)

	def test_deduplication_timeline_links(self):
		criscostack.delete_doc_if_exists("Note", "deduplication timeline links")
	...
	def test_contacts_attached(self):
		contact_sender: "Contact" = criscostack.get_doc(
			{
				"doctype": "Contact",
		...
		contact_recipient: "Contact" = criscostack.get_doc(
			{
				"doctype": "Contact",
		...
		contact_cc: "Contact" = criscostack.get_doc(
			{
				"doctype": "Contact",
		...
		comm: Communication = criscostack.get_doc(
			{
				"doctype": "Communication",
				"communication_medium": "Email",
	...
	def test_get_communication_data(self):
		from criscostack.desk.form.load import get_communication_data

		criscostack.delete_doc_if_exists("Note", "get communication data")

		note = criscostack.get_doc(
			{"doctype": "Note", "title": "get communication data", "content": "get communication data"}
		).insert(ignore_permissions=True)

		comm_note_1 = criscostack.get_doc(
			{
				"doctype": "Communication",
				"communication_type": "Communication",
		...
		comm_note_2 = criscostack.get_doc(
			{
				"doctype": "Communication",
				"communication_type": "Communication",
	...
	def test_link_in_email(self):
		...
		notes = {}
		for i in range(2):
			criscostack.delete_doc_if_exists("Note", f"test document link in email {i}")
			notes[i] = criscostack.get_doc(
				{
					"doctype": "Note",
	...
	def test_signature_in_email_content(self):
		...
		comm_with_signature = criscostack.get_doc(
			base_communication
			| {
		...
		comm_without_signature = criscostack.get_doc(
			base_communication
			| {
...
class TestCommunicationEmailMixin(CriscoTestCase):
	...
	def test_cc(self):
		def test(assertion, cc_list=None, set_user_as=None, include_sender=False, thread_notify=False):
			if set_user_as:
				criscostack.set_user(set_user_as)
...
def create_email_account() -> "EmailAccount":
	criscostack.delete_doc_if_exists("Email Account", "_Test Comm Account 1")
