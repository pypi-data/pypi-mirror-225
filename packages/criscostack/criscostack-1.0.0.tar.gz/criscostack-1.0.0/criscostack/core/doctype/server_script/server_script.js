...
criscostack.ui.form.on("Server Script", {
	setup: function (frm) {
	...
	refresh: function (frm) {
		...
		if (!frm.is_new()) {
			frm.add_custom_button(__("Compare Versions"), () => {
				new criscostack.ui.DiffView("Server Script", "script", frm.doc.name);
			});
		}
...
if criscostack.form_dict.message == "ping":
	criscostack.response['message'] = "pong"
else:
	criscostack.response['message'] = "ok"
</code></pre>
...
tenant_id = criscostack.db.get_value(...)
