...
# this is a separate file since it is imported in criscostack.model.document
...
def run_server_script_for_doc_event(doc, event):
	...
	if scripts:
		...
		for script_name in scripts:
			criscostack.get_doc("Server Script", script_name).execute_doc(doc)
...
def get_server_script_map():
	...
	# 	}
	# }
	if criscostack.flags.in_patch and not criscostack.db.table_exists("Server Script"):
		return {}

	script_map = criscostack.cache.get_value("server_script_map")
	if script_map is None:
		script_map = {"permission_query": {}}
		enabled_server_scripts = criscostack.get_all(
			"Server Script",
		...
		criscostack.cache.set_value("server_script_map", script_map)
