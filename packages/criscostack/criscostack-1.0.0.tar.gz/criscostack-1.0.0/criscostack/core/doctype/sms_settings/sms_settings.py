...
import criscostack
from criscostack import _, msgprint, throw
...
class SMSSettings(Document):
	...
	if TYPE_CHECKING:
		from criscostack.core.doctype.sms_parameter.sms_parameter import SMSParameter
...
def send_sms(receiver_list, msg, sender_name="", success_msg=True):
	...
	arg = {
		"receiver_list": receiver_list,
		"message": criscostack.safe_decode(msg).encode("utf-8"),
		"success_msg": success_msg,
	}

	if criscostack.db.get_single_value("SMS Settings", "sms_gateway_url"):
...
def send_via_gateway(arg):
	...
	message = criscostack.safe_decode(arg.get("message"))
	...
	if len(success_list) > 0:
		...
		if arg.get("success_msg"):
			criscostack.msgprint(_("SMS sent to following numbers: {0}").format("\n" + "\n".join(success_list)))


def get_headers(sms_settings=None):
	if not sms_settings:
		sms_settings = criscostack.get_doc("SMS Settings", "SMS Settings")
...
def create_sms_log(args, sent_to):
	sl = criscostack.new_doc("SMS Log")
	sl.sent_on = nowdate()
