...
class TestVersion(CriscoTestCase):
	...
	def test_no_version_on_new_doc(self):
		from criscostack.desk.form.load import get_versions

		t = criscostack.get_doc(doctype="ToDo", description="something")
		t.save(ignore_version=False)

		self.assertFalse(get_versions(t))

		t = criscostack.get_doc(t.doctype, t.name)
