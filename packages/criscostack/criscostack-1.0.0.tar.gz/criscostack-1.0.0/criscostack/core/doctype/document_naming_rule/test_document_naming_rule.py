...
class TestDocumentNamingRule(CriscoTestCase):
	def test_naming_rule_by_series(self):
		naming_rule = criscostack.get_doc(
			dict(doctype="Document Naming Rule", document_type="ToDo", prefix="test-todo-", prefix_digits=5)
		).insert()

		todo = criscostack.get_doc(
			dict(doctype="ToDo", description="Is this my name " + criscostack.generate_hash())
		).insert()
	...
	def test_naming_rule_by_condition(self):
		...
		# another rule
		naming_rule_1 = criscostack.copy_doc(naming_rule)
		...
		naming_rule_2 = criscostack.copy_doc(naming_rule)
		...
		todo = criscostack.get_doc(
			dict(doctype="ToDo", priority="High", description="Is this my name " + criscostack.generate_hash())
		).insert()

		todo_1 = criscostack.get_doc(
			dict(doctype="ToDo", priority="Medium", description="Is this my name " + criscostack.generate_hash())
		).insert()

		todo_2 = criscostack.get_doc(
			dict(doctype="ToDo", priority="Low", description="Is this my name " + criscostack.generate_hash())
		).insert()
