...
import criscostack
import criscostack.desk.query_report
from criscostack import _, scrub
from criscostack.core.doctype.custom_role.custom_role import get_custom_allowed_roles
from criscostack.core.doctype.page.page import delete_custom_role
from criscostack.desk.reportview import append_totals_row
from criscostack.model.document import Document
from criscostack.modules import make_boilerplate
from criscostack.modules.export_file import export_to_files
from criscostack.utils import cint, cstr
from criscostack.utils.safe_exec import check_safe_sql_query, safe_exec
...
class Report(Document):
	...
	if TYPE_CHECKING:
		...
		from criscostack.core.doctype.report_column.report_column import ReportColumn
		from criscostack.core.doctype.report_filter.report_filter import ReportFilter
	...
	def validate(self):
		...
		if not self.module:
			self.module = criscostack.db.get_value("DocType", self.ref_doctype, "module")

		if not self.is_standard:
			self.is_standard = "No"
			if (
				criscostack.session.user == "Administrator" and getattr(criscostack.local.conf, "developer_mode", 0) == 1
			):
		...
		if self.is_standard == "No":
			...
			if self.report_type != "Report Builder":
				criscostack.only_for("Script Manager", True)

			if criscostack.db.get_value("Report", self.name, "is_standard") == "Yes":
				criscostack.throw(_("Cannot edit a standard report. Please duplicate and create a new report"))

		if self.is_standard == "Yes" and criscostack.session.user != "Administrator":
			criscostack.throw(_("Only Administrator can save a standard report. Please rename and save."))
	...
	def on_trash(self):
		if (
			self.is_standard == "Yes"
			and not cint(getattr(criscostack.local.conf, "developer_mode", 0))
			and not criscostack.flags.in_patch
		):
			criscostack.throw(_("You are not allowed to delete Standard Report"))
	...
	def set_doctype_roles(self):
		if not self.get("roles") and self.is_standard == "No":
			meta = criscostack.get_meta(self.ref_doctype)
			if not meta.istable:
	...
	def is_permitted(self):
		...
		from criscostack.utils import has_common

		allowed = [
			d.role for d in criscostack.get_all("Has Role", fields=["role"], filters={"parent": self.name})
		]
		...
		if has_common(criscostack.get_roles(), allowed):
			return True
	...
	def export_doc(self):
		...
		if self.is_standard == "Yes" and (criscostack.local.conf.get("developer_mode") or 0) == 1:
			export_to_files(
	...
	def execute_query_report(self, filters):
		if not self.query:
			criscostack.throw(_("Must specify a Query to run"), title=_("Report Document Error"))

		check_safe_sql_query(self.query)

		result = [list(t) for t in criscostack.db.sql(self.query, filters)]
		columns = self.get_columns() or [cstr(c[0]) for c in criscostack.db.get_description()]
	...
	def execute_script_report(self, filters):
		...
		if execution_time > threshold and not self.prepared_report:
			criscostack.enqueue(enable_prepared_report, report=self.name)

		criscostack.cache.hset("report_execution_time", self.name, execution_time)

		return res

	def execute_module(self, filters):
		# report in python module
		module = self.module or criscostack.db.get_value("DocType", self.ref_doctype, "module")
		method_name = get_report_module_dotted_path(module, self.name) + ".execute"
		return criscostack.get_attr(method_name)(criscostack._dict(filters))

	def execute_script(self, filters):
		# server script
		loc = {"filters": criscostack._dict(filters), "data": None, "result": None}
	...
	def run_query_report(
		...
		columns, result = [], []
		data = criscostack.desk.query_report.run(
			self.name,
			filters=filters,
			user=user,
		...
		for d in data.get("columns"):
			if isinstance(d, dict):
				...
				columns.append(
					criscostack._dict(label=parts[0], fieldtype=fieldtype, fieldname=parts[0], options=options)
				)
	...
	def get_standard_report_columns(self, params):
		if params.get("fields"):
			...
			for df in criscostack.get_meta(self.ref_doctype).fields:
				if df.in_list_view:
	...
	def get_standard_report_order_by(self, params):
		...
		if params.get("group_by"):
			group_by_args = criscostack._dict(params["group_by"])
	...
	def build_standard_report_columns(self, columns, group_by_args):
		...
		for (fieldname, doctype) in columns:
			...
			if meta.get_field(fieldname):
				...
				field = criscostack._dict(fieldname=fieldname, label=label)
	...
	def toggle_disable(self, disable: bool):
		if not self.has_permission("write"):
			criscostack.throw(_("You are not allowed to edit the report."))

		self.db_set("disabled", cint(disable))


def is_prepared_report_enabled(report):
	return cint(criscostack.db.get_value("Report", report, "prepared_report")) or 0


def get_report_module_dotted_path(module, report_name):
	return (
		criscostack.local.module_app[scrub(module)]
		+ "."
		+ scrub(module)
		+ ".report."
		+ scrub(report_name)
...
def enable_prepared_report(report: str):
	criscostack.db.set_value("Report", report, "prepared_report", 1)
