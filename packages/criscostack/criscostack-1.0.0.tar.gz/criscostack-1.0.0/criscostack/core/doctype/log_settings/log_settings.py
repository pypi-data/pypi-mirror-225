...
class LogSettings(Document):
	...
	if TYPE_CHECKING:
		from criscostack.core.doctype.logs_to_clear.logs_to_clear import LogsToClear
	...
	def remove_unsupported_doctypes(self):
		for entry in list(self.logs_to_clear):
			...
			msg = _("{} does not support automated log clearing.").format(criscostack.bold(entry.ref_doctype))
			if criscostack.conf.developer_mode:
				msg += "<br>" + _("Implement `clear_old_logs` method to enable auto error clearing.")
			criscostack.msgprint(msg, title=_("DocType not supported by Log Settings."))
			self.remove(entry)
	...
	def add_default_logtypes(self):
		...
		for logtype, retention in DEFAULT_LOGTYPES_RETENTION.items():
			if logtype not in existing_logtypes and _supports_log_clearing(logtype):
				if not criscostack.db.exists("DocType", logtype):
					continue
	...
	def clear_logs(self):
		...
		for entry in self.logs_to_clear:
			...
			kwargs = criscostack.get_newargs(func, {"days": entry.days})
			func(**kwargs)
...
def run_log_clean_up():
	doc = criscostack.get_doc("Log Settings")
...
def has_unseen_error_log():
	if criscostack.get_all("Error Log", filters={"seen": 0}, limit=1):
		return {
			"show_alert": True,
...
def get_log_doctypes(doctype, txt, searchfield, start, page_len, filters):
	...
	doctypes = criscostack.get_list("DocType", filters=filters, pluck="name")
...
def clear_log_table(doctype, days=90):
	...
	"""
	from criscostack.utils import get_table_name
	...
	try:
		criscostack.db.sql_ddl(f"CREATE TABLE `{temporary}` LIKE `{original}`")
		...
		criscostack.db.sql_ddl(f"RENAME TABLE `{original}` TO `{backup}`, `{temporary}` TO `{original}`")
	except Exception:
		criscostack.db.rollback()
		criscostack.db.sql_ddl(f"DROP TABLE IF EXISTS `{temporary}`")
		raise
	else:
		criscostack.db.sql_ddl(f"DROP TABLE `{backup}`")
