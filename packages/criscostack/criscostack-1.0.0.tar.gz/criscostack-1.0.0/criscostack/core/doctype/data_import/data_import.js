...
criscostack.ui.form.on("Data Import", {
	setup(frm) {
		criscostack.realtime.on("data_import_refresh", ({ data_import }) => {
			frm.import_in_progress = false;
			if (data_import !== frm.doc.name) return;
			criscostack.model.clear_doc("Data Import", frm.doc.name);
			criscostack.model.with_doc("Data Import", frm.doc.name).then(() => {
				frm.refresh();
			});
		});
		criscostack.realtime.on("data_import_progress", (data) => {
		...
		frm.set_query("reference_doctype", () => {
			return {
				filters: {
					name: ["in", criscostack.boot.user.can_import],
				},
			};
		});
	...
	refresh(frm) {
		...
		if (frm.doc.status.includes("Success")) {
			frm.add_custom_button(__("Go to {0} List", [__(frm.doc.reference_doctype)]), () =>
				criscostack.set_route("List", frm.doc.reference_doctype)
			);
		}
	},
	...
	update_indicators(frm) {
		const indicator = criscostack.get_indicator(frm.doc);
		if (indicator) {
	...
	show_import_status(frm) {
		criscostack.call({
			method: "criscostack.core.doctype.data_import.data_import.get_import_status",
			args: {
				...
				.then((result) => {
					if (result.length > 0) {
						frm.add_custom_button("Report Error", () => {
							...
							criscostack.request.report_error(fake_xhr, {});
						});
					}
				});
		}
	...
	download_template(frm) {
		criscostack.require("data_import_tools.bundle.js", () => {
			frm.data_exporter = new criscostack.data_import.DataExporter(
	...
	show_import_preview(frm, preview_data) {
		...
		criscostack.require("data_import_tools.bundle.js", () => {
			frm.import_preview = new criscostack.data_import.ImportPreview({
	...
	export_errored_rows(frm) {
		open_url_post(
			"/api/method/criscostack.core.doctype.data_import.data_import.download_errored_template",
			{
				data_import_name: frm.doc.name,
			}
		);
	...
	export_import_log(frm) {
		open_url_post(
			"/api/method/criscostack.core.doctype.data_import.data_import.download_import_log",
			{
				data_import_name: frm.doc.name,
			}
		);
						...
						if (log.success) {
							if (frm.doc.import_type === "Insert New Records") {
								html = __("Successfully imported {0}", [
									`<span class="underline">${criscostack.utils.get_form_link(
										frm.doc.reference_doctype,
										log.docname,
										true
									)}<span>`,
							...
							let messages = JSON.parse(log.messages || "[]")
								...
								.join("");
							let id = criscostack.dom.get_unique_id();
							html = `${messages}
	...
	show_import_log(frm) {
		...
		criscostack.call({
			method: "criscostack.client.get_count",
			args: {
				doctype: "Data Import Log",
				filters: {
					data_import: frm.doc.name,
