criscostack.ui.form.on("File", {
	...
	preview_file: function (frm) {
		...
		if (criscostack.utils.is_image_file(frm.doc.file_url)) {
		...
		} else if (criscostack.utils.is_video_file(frm.doc.file_url)) {
			$preview = $(`<div class="img_preview">
	...
	optimize: function (frm) {
		criscostack.show_alert(__("Optimizing image..."));
		frm.call("optimize_file").then(() => {
			criscostack.show_alert(__("Image optimized"));
		});
	},

	unzip: function (frm) {
		criscostack.call({
			method: "criscostack.core.api.file.unzip_file",
			args: {
				name: frm.doc.name,
			},
			callback: function () {
				criscostack.set_route("List", "File");
			},
		});
	},
});
