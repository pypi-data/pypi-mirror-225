...
import criscostack
from criscostack.core.doctype.file.file import File, setup_folder_path
...
def unzip_file(name: str):
	...
	file: File = criscostack.get_doc("File", name)
	return file.unzip()


@criscostack.whitelist()
def get_attached_images(doctype: str, names: list[str] | str) -> criscostack._dict:
	...
	img_urls = criscostack.db.get_list(
		"File",
		filters={
...
def get_files_in_folder(folder: str, start: int = 0, page_length: int = 20) -> dict:
	attachment_folder = criscostack.db.get_value(
		"File",
		"Home/Attachments",
...
def get_max_file_size() -> int:
	return cint(criscostack.conf.get("max_file_size")) or 10485760


@criscostack.whitelist()
def create_new_folder(file_name: str, folder: str) -> File:
	"""create new folder under current parent folder"""
	file = criscostack.new_doc("File")
...
def move_file(file_list: list[File | dict] | str, new_parent: str, old_parent: str) -> None:
	...
	# recalculate sizes
	criscostack.get_doc("File", old_parent).save()
	criscostack.get_doc("File", new_parent).save()


@criscostack.whitelist()
def zip_files(files: str):
	files = criscostack.parse_json(files)
	criscostack.response["filename"] = "files.zip"
	criscostack.response["filecontent"] = File.zip_files(files)
	criscostack.response["type"] = "download"
