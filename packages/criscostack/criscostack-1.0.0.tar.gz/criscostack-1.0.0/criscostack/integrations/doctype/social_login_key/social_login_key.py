...
class BaseUrlNotSetError(criscostack.ValidationError):
	pass


class AuthorizeUrlNotSetError(criscostack.ValidationError):
	pass


class AccessTokenUrlNotSetError(criscostack.ValidationError):
	pass


class RedirectUrlNotSetError(criscostack.ValidationError):
	pass


class ClientIDNotSetError(criscostack.ValidationError):
	pass


class ClientSecretNotSetError(criscostack.ValidationError):
	pass
...
class SocialLoginKey(Document):
	...
	if TYPE_CHECKING:
		...
		social_login_provider: DF.Literal[
			"Custom", "Facebook", "Crisco", "GitHub", "Google", "Office 365", "Salesforce", "fairlogin"
		]
		user_id_property: DF.Data | None
	# end: auto-generated types
	def autoname(self):
		self.name = criscostack.scrub(self.provider_name)

	def validate(self):
		self.set_icon()
		if self.custom_base_url and not self.base_url:
			criscostack.throw(_("Please enter Base URL"), exc=BaseUrlNotSetError)
		if not self.authorize_url:
			criscostack.throw(_("Please enter Authorize URL"), exc=AuthorizeUrlNotSetError)
		if not self.access_token_url:
			criscostack.throw(_("Please enter Access Token URL"), exc=AccessTokenUrlNotSetError)
		if not self.redirect_url:
			criscostack.throw(_("Please enter Redirect URL"), exc=RedirectUrlNotSetError)
	...
	def set_icon(self):
		...
		if self.provider_name in icon_map:
			...
			self.icon = f"/assets/criscostack/icons/social/{icon_file}"
	...
	def get_social_login_provider(self, provider, initialize=False):
		...
		providers["Office 365"] = {
			...
			"redirect_url": "/api/method/criscostack.integrations.oauth2_logins.login_via_office365",
			"api_endpoint": None,
		...
		providers["GitHub"] = {
			...
			"redirect_url": "/api/method/criscostack.integrations.oauth2_logins.login_via_github",
			"api_endpoint": "user",
			"api_endpoint_args": None,
		...
		providers["Google"] = {
			...
			"redirect_url": "/api/method/criscostack.integrations.oauth2_logins.login_via_google",
		...
		providers["Facebook"] = {
			...
			"redirect_url": "/api/method/criscostack.integrations.oauth2_logins.login_via_facebook",
		...
		providers["Crisco"] = {
			"provider_name": "Crisco",
			"enable_social_login": 1,
			"custom_base_url": 1,
			"icon": "/assets/criscostack/images/criscostack-favicon.svg",
			"redirect_url": "/api/method/criscostack.integrations.oauth2_logins.login_via_criscostack",
			"api_endpoint": "/api/method/criscostack.integrations.oauth2.openid_profile",
			"api_endpoint_args": None,
			"authorize_url": "/api/method/criscostack.integrations.oauth2.authorize",
			"access_token_url": "/api/method/criscostack.integrations.oauth2.get_token",
			"auth_url_data": json.dumps({"response_type": "code", "scope": "openid"}),
		}
		...
		providers["Salesforce"] = {
			...
			"redirect_url": "/api/method/criscostack.integrations.oauth2_logins.login_via_salesforce",
		...
		providers["fairlogin"] = {
			...
			"custom_base_url": 0,
			"icon": "fa fa-key",
			"redirect_url": "/api/method/criscostack.integrations.oauth2_logins.login_via_fairlogin",
