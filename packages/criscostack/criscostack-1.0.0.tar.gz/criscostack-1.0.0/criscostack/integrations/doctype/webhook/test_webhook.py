...
import criscostack
from criscostack.integrations.doctype.webhook.webhook import (
	enqueue_webhook,
	get_webhook_data,
	get_webhook_headers,
)
...
def get_test_webhook(config):
	wh = criscostack.get_doc(config)
	if not wh.name:
		wh.name = criscostack.generate_hash()
	wh.insert()
	wh.reload()
	try:
		yield wh
...
class TestWebhook(CriscoTestCase):
	@classmethod
	def setUpClass(cls):
		# delete any existing webhooks
		criscostack.db.delete("Webhook")
		# Delete existing logs if any
		criscostack.db.delete("Webhook Request Log")
		super().setUpClass()
	...
	def create_sample_webhooks(cls):
		samples_webhooks_data = [
			{
				"name": criscostack.generate_hash(),
		...
		for wh_fields in samples_webhooks_data:
			wh = criscostack.new_doc("Webhook")
			wh.update(wh_fields)
			wh.insert()
	...
	def setUp(self):
		...
		if criscostack.db.exists("Webhook", webhook_fields):
			self.webhook = criscostack.get_doc("Webhook", webhook_fields)
		else:
			self.webhook = criscostack.new_doc("Webhook")
			self.webhook.update(webhook_fields)

		# create a User document
		self.user = criscostack.new_doc("User")
		self.user.first_name = criscostack.mock("name")
		self.user.email = criscostack.mock("email")
		self.user.save()

		# Create another test user specific to this test
		self.test_user = criscostack.new_doc("User")
	...
	def test_webhook_trigger_with_enabled_webhooks(self):
		...
		criscostack.flags.webhooks = None

		# Insert the user to db
		self.test_user.insert()

		self.assertTrue("User" in criscostack.flags.webhooks)
		# only 1 hook (enabled) must be queued
		self.assertEqual(len(criscostack.flags.webhooks.get("User")), 1)
		self.assertTrue(self.test_user.email in criscostack.flags.webhooks_executed)
		self.assertEqual(
			criscostack.flags.webhooks_executed.get(self.test_user.email)[0], self.sample_webhooks[0].name
		)
	...
	def test_validate_doc_events(self):
		...
		self.assertRaises(criscostack.ValidationError, self.webhook.save)
	...
	def test_webhook_req_log_creation(self):
		...
		if not criscostack.db.get_value("User", "user2@integration.webhooks.test.com"):
			user = criscostack.get_doc(
				{"doctype": "User", "email": "user2@integration.webhooks.test.com", "first_name": "user2"}
			).insert()
		else:
			user = criscostack.get_doc("User", "user2@integration.webhooks.test.com")

		webhook = criscostack.get_doc("Webhook", {"webhook_doctype": "User"})
		enqueue_webhook(user, webhook)

		self.assertTrue(criscostack.get_all("Webhook Request Log", pluck="name"))
	...
	def test_webhook_with_array_body(self):
		...
		with get_test_webhook(wh_config) as wh:
			...
			log = criscostack.get_last_doc("Webhook Request Log")
