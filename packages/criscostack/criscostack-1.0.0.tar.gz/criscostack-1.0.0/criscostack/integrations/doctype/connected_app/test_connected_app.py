...
import criscostack
from criscostack.integrations.doctype.social_login_key.test_social_login_key import (
...
def get_connected_app():
	...
	connected_app = criscostack.new_doc(doctype)
	connected_app.provider_name = "criscostack"
...
def get_oauth_client():
	oauth_client = criscostack.new_doc("OAuth Client")
...
class TestConnectedApp(CriscoTestCase):
	def setUp(self):
		"""Set up a Connected App that connects to our own oAuth provider.

		Crisco comes with it's own oAuth2 provider that we can test against. The
		...
		Crisco's "Authorization URL" and "Access Token URL" (actually they're
	...
	def test_web_application_flow(self):
		...
		resp = oauth2_session.get(urljoin(self.base_url, "/api/method/criscostack.auth.get_logged_user"))
	...
	def tearDown(self):
		...
		if getattr(self, "oauth_client", None):
			tokens = criscostack.get_all("OAuth Bearer Token", filters={"client": self.oauth_client.name})
			for token in tokens:
				doc = criscostack.get_doc("OAuth Bearer Token", token.name)
				doc.delete()

			codes = criscostack.get_all("OAuth Authorization Code", filters={"client": self.oauth_client.name})
			for code in codes:
				doc = criscostack.get_doc("OAuth Authorization Code", code.name)
				doc.delete()
