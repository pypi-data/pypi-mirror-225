...
class S3BackupSettings(Document):
	...
	def validate(self):
		...
		try:
			...
			bucket_name = criscostack.bold(self.bucket)
			if error_code == "403":
...
def take_backup():
	...
	enqueue(
		"criscostack.integrations.doctype.s3_backup_settings.s3_backup_settings.take_backups_s3",
		queue="long",
		timeout=1500,
	)
...
def take_backups_if(freq):
	if cint(criscostack.db.get_single_value("S3 Backup Settings", "enabled")):
		if criscostack.db.get_single_value("S3 Backup Settings", "frequency") == freq:
			take_backups_s3()
...
def notify():
	error_message = criscostack.get_traceback()
	send_email(False, "Amazon S3", "S3 Backup Settings", "notify_email", error_message)


def backup_to_s3():
	from criscostack.utils import get_backups_path
	from criscostack.utils.backups import new_backup

	doc = criscostack.get_single("S3 Backup Settings")
	bucket = doc.bucket
