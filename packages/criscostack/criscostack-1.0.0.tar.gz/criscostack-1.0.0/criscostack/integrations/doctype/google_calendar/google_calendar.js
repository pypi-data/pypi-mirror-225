...
criscostack.ui.form.on("Google Calendar", {
	refresh: function (frm) {
		...
		criscostack.realtime.on("import_google_calendar", (data) => {
			if (data.progress) {
		...
		if (frm.doc.refresh_token) {
			frm.add_custom_button(__("Sync Calendar"), function () {
				...
				criscostack
					.call({
						method: "criscostack.integrations.doctype.google_calendar.google_calendar.sync",
						args: {
					...
					.then((r) => {
						...
						criscostack.msgprint(r.message);
					});
			});
		}
	},
	authorize_google_calendar_access: function (frm) {
		...
		criscostack.call({
			method: "criscostack.integrations.doctype.google_calendar.google_calendar.authorize_access",
			args: {
				g_calendar: frm.doc.name,
