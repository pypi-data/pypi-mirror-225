...
from criscostack.utils import cint, cstr
...
def print_has_permission_check_logs(func):
	def inner(*args, **kwargs):
		criscostack.flags["has_permission_check_logs"] = []
		result = func(*args, **kwargs)
		self_perm_check = True if not kwargs.get("user") else kwargs.get("user") == criscostack.session.user
		raise_exception = kwargs.get("raise_exception", True)

		# print only if access denied
		# and if user is checking his own permission
		if not result and self_perm_check and raise_exception:
			msgprint(("<br>").join(criscostack.flags.get("has_permission_check_logs", [])))
		criscostack.flags.pop("has_permission_check_logs", None)
		return result
...
def has_permission(
	...
	*,
	...
	:param raise_exception:
	        ...
	        If not False, will display a message using criscostack.msgprint
	...
	if ptype == "share" and criscostack.get_system_settings("disable_document_sharing"):
		return False
	...
	if doc:
		if isinstance(doc, (str, int)):
			doc = criscostack.get_doc(meta.name, doc)
		perm = get_doc_permissions(doc, user=user, ptype=ptype).get(ptype)
		if not perm:
			msg = _("User {0} does not have access to this document").format(criscostack.bold(user))
			if criscostack.has_permission(doc.doctype):
		...
		if not perm:
			push_perm_check_log(
				_("User {0} does not have doctype access via role permission for document {1}").format(
					criscostack.bold(user), criscostack.bold(doctype)
				)
			)
	...
	def false_if_not_shared():
		if ptype in ("read", "write", "share", "submit", "email", "print"):
			...
			if doc:
				...
				shared = criscostack.share.get_shared(
					doctype,
					user,
					rights=rights,
			...
			elif criscostack.share.get_shared(doctype, user, rights=rights, limit=1):
...
def get_doc_permissions(doc, user=None, ptype=None):
	...
	meta = criscostack.get_meta(doc.doctype)
...
def get_role_permissions(doctype_meta, user=None, is_owner=None):
	...
	if isinstance(doctype_meta, str):
		doctype_meta = criscostack.get_meta(doctype_meta)  # assuming doctype name was passed
	...
	if not criscostack.local.role_permissions.get(cache_key):
		perms = criscostack._dict(if_owner={})

		roles = criscostack.get_roles(user)
		...
		criscostack.local.role_permissions[cache_key] = perms

	return criscostack.local.role_permissions[cache_key]


def get_user_permissions(user):
	from criscostack.core.doctype.user_permission.user_permission import get_user_permissions
...
def has_user_permission(doc, user=None):
	...
	apply_strict_user_permissions = criscostack.get_system_settings("apply_strict_user_permissions")
	...
	def check_user_permission_on_link_fields(d):
		...
		meta = criscostack.get_meta(d.get("doctype"))
...
def has_controller_permissions(doc, ptype, user=None):
	...
	methods = criscostack.get_hooks("has_permission").get(doc.doctype, [])

	if not methods:
		return None

	for method in reversed(methods):
		controller_permission = criscostack.call(criscostack.get_attr(method), doc=doc, ptype=ptype, user=user)
...
def get_all_perms(role):
	...
	perms = criscostack.get_all("DocPerm", fields="*", filters=dict(role=role))
	custom_perms = criscostack.get_all("Custom DocPerm", fields="*", filters=dict(role=role))
	doctypes_with_custom_perms = criscostack.get_all("Custom DocPerm", pluck="parent", distinct=True)
...
def get_roles(user=None, with_standard=True):
	...
	def get():
		if user == "Administrator":
			return criscostack.get_all("Role", pluck="name")  # return all available roles
		else:
	...
	roles = criscostack.cache.hget("roles", user, get)
...
def get_perms_for(roles, perm_doctype="DocPerm"):
	...
	return criscostack.get_all(perm_doctype, fields=["*"], filters=filters)


def get_doctypes_with_custom_docperms():
	"""Returns all the doctypes with Custom Docperms"""

	doctypes = criscostack.get_all("Custom DocPerm", fields=["parent"], distinct=1)
...
def add_user_permission(
	...
	"""Add user permission"""
	from criscostack.core.doctype.user_permission.user_permission import user_permission_exists

	if not user_permission_exists(user, doctype, name, applicable_for):
		if not criscostack.db.exists(doctype, name):
			criscostack.throw(_("{0} {1} not found").format(_(doctype), name), criscostack.DoesNotExistError)
...
def remove_user_permission(doctype, name, user):
	user_permission_name = criscostack.db.get_value(
		"User Permission", dict(user=user, allow=doctype, for_value=name)
	)
	criscostack.delete_doc("User Permission", user_permission_name)
...
def clear_user_permissions_for_doctype(doctype, user=None):
	...
	if user:
		filters["user"] = user
	user_permissions_for_doctype = criscostack.get_all("User Permission", filters=filters)
	for d in user_permissions_for_doctype:
		criscostack.delete_doc("User Permission", d.name)


def can_import(doctype, raise_exception=False):
	if not ("System Manager" in criscostack.get_roles() or has_permission(doctype, "import")):
		if raise_exception:
			raise criscostack.PermissionError(f"You are not allowed to import: {doctype}")
		else:
			return False
	return True
...
def can_export(doctype, raise_exception=False):
	if "System Manager" in criscostack.get_roles():
		return True
	else:
		role_permissions = criscostack.permissions.get_role_permissions(doctype)
		has_access = role_permissions.get("export") or role_permissions.get("if_owner").get("export")
		if not has_access and raise_exception:
			raise criscostack.PermissionError(_("You are not allowed to export {} doctype").format(doctype))
		return has_access


def update_permission_property(doctype, role, permlevel, ptype, value=None, validate=True):
	"""Update a property in Custom Perm"""
	from criscostack.core.doctype.doctype.doctype import validate_permissions_for_doctype

	out = setup_custom_perms(doctype)

	name = criscostack.db.get_value("Custom DocPerm", dict(parent=doctype, role=role, permlevel=permlevel))
	table = DocType("Custom DocPerm")
	criscostack.qb.update(table).set(ptype, value).where(table.name == name).run()
...
def setup_custom_perms(parent):
	...
	if not criscostack.db.exists("Custom DocPerm", dict(parent=parent)):
		copy_perms(parent)
		return True
...
def add_permission(doctype, role, permlevel=0, ptype=None):
	...
	custom_docperm = criscostack.get_doc(
		{
...
def copy_perms(parent):
	...
	for d in criscostack.get_all("DocPerm", fields="*", filters=dict(parent=parent)):
		custom_perm = criscostack.new_doc("Custom DocPerm")
...
def reset_perms(doctype):
	...
	from criscostack.desk.notifications import delete_notification_count_for

	delete_notification_count_for(doctype)
	criscostack.db.delete("Custom DocPerm", {"parent": doctype})


def get_linked_doctypes(dt: str) -> list:
	meta = criscostack.get_meta(dt)
	linked_doctypes = [dt] + [
		d.options
		for d in meta.get(
			"fields",
...
def push_perm_check_log(log):
	if criscostack.flags.get("has_permission_check_logs") is None:
		return

	criscostack.flags.get("has_permission_check_logs").append(_(log))
...
def has_child_permission(
	...
	if isinstance(child_doc, str):
		child_doc = criscostack.db.get_value(
			child_doctype,
			child_doc,
	...
	if not parent_doctype:
		push_perm_check_log(
			_("Please specify a valid parent DocType for {0}").format(criscostack.bold(child_doctype))
		)
		return False

	parent_meta = criscostack.get_meta(parent_doctype)
	...
	if parent_meta.istable or not (
		...
		push_perm_check_log(
			_("{0} is not a valid parent DocType for {1}").format(
				criscostack.bold(parent_doctype), criscostack.bold(child_doctype)
			)
		)
		return False
	...
	if child_doc:
		...
		if not parentfield:
			push_perm_check_log(
				_("Parentfield not specified in {0}: {1}").format(
					criscostack.bold(child_doctype), criscostack.bold(child_doc.name)
				)
			)
			return False
		...
		if parentfield not in valid_parentfields:
			push_perm_check_log(
				_("{0} is not a valid parentfield for {1}").format(
					criscostack.bold(parentfield), criscostack.bold(child_doctype)
				)
			)
			return False
		...
		if permlevel > 0 and permlevel not in parent_meta.get_permlevel_access(ptype, user=user):
			push_perm_check_log(
				_("Insufficient Permission Level for {0}").format(criscostack.bold(parent_doctype))
			)
			return False
