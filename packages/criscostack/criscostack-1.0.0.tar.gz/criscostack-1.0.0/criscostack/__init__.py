...
"""
Crisco - Low Code Open Source Framework in Python and JS

Crisco, pronounced fra-pay, is a full stack, batteries-included, web
framework written in Python and Javascript with MariaDB as the database.
It is the framework which powers Criscoerp. It is pretty generic and can
be used to build database driven apps.

Read the documentation: https://criscostackframework.com/docs
"""
import functools
import gc
import importlib
...
from criscostack.query_builder import (
	get_query,
	get_query_builder,
	patch_query_aggregation,
	patch_query_execute,
)
from criscostack.utils.caching import request_cache
from criscostack.utils.data import cstr, sbool
...
_tune_gc = bool(sbool(os.environ.get("CRISCOSTACK_TUNE_GC", True)))
...
def _(msg: str, lang: str | None = None, context: str | None = None) -> str:
	...
	"""
	from criscostack.translate import get_all_translations
	from criscostack.utils import is_html, strip_html_tags
...
def get_lang_dict(fortype: str, name: str | None = None) -> dict[str, str]:
	...
	from criscostack.translate import get_dict

	return get_dict(fortype, name)


def set_user_lang(user: str, user_language: str | None = None) -> None:
	"""Guess and set user language for the session. `criscostack.local.lang`"""
	from criscostack.translate import get_user_lang
...
if TYPE_CHECKING:
	from criscostack.database.mariadb.database import MariaDBDatabase
	from criscostack.database.postgres.database import PostgresDatabase
	from criscostack.model.document import Document
	from criscostack.query_builder.builder import MariaDB, Postgres
	from criscostack.utils.redis_wrapper import RedisWrapper
...
def init(site: str, sites_path: str = ".", new_site: bool = False, force=False) -> None:
	"""Initialize criscostack for the current site. Reset thread locals `criscostack.local`"""
...
def connect(
	...
	:param site: If site is given, calls `criscostack.init`.
	:param db_name: Optional. Will use from `site_config.json`.
	:param set_admin_as_user: Set Administrator as current user.
	"""
	from criscostack.database import get_db
...
def get_site_config(sites_path: str | None = None, site_path: str | None = None) -> dict[str, Any]:
	...
	config["redis_queue"] = (
		os.environ.get("CRISCOSTACK_REDIS_QUEUE") or config.get("redis_queue") or "redis://127.0.0.1:11311"
	)
	config["redis_cache"] = (
		os.environ.get("CRISCOSTACK_REDIS_CACHE") or config.get("redis_cache") or "redis://127.0.0.1:13311"
	)
	config["db_type"] = os.environ.get("CRISCOSTACK_DB_TYPE") or config.get("db_type") or "mariadb"
	config["db_host"] = os.environ.get("CRISCOSTACK_DB_HOST") or config.get("db_host") or "127.0.0.1"
	config["db_port"] = (
		os.environ.get("CRISCOSTACK_DB_PORT") or config.get("db_port") or db_default_ports(config["db_type"])
	)
...
def setup_redis_cache_connection():
	"""Defines `criscostack.cache` as `RedisWrapper` instance"""
	global cache
...
def get_traceback(with_context: bool = False) -> str:
	...
	from criscostack.utils import get_traceback
...
def throw(
	...
	:param msg: Message.
	:param exc: Exception class. Default `criscostack.ValidationError`"""
	msgprint(
		msg,
		raise_exception=exc,
		title=title,
...
def create_folder(path, with_init=False):
	...
	from criscostack.utils import touch_file
...
def get_user():
	from criscostack.utils.user import UserPermissions
...
def get_roles(username=None) -> list[str]:
	...
	return criscostack.permissions.get_roles(username or local.session.user)
...
def sendmail(
	...
	if as_markdown:
		from criscostack.utils import md_to_html
...
def whitelist(allow_guest=False, xss_safe=False, methods=None):
	...
	def innerfn(fn):
		from criscostack.utils.typing_validations import validate_argument_types
...
def read_only():
	def innfn(fn):
		def wrapper_fn(*args, **kwargs):
			...
			# criscostack.read_only could be called from nested functions, in such cases don't swap the
			# connection again.
...
def only_for(roles: list[str] | tuple[str] | str, message=False):
	"""
	Raises `criscostack.PermissionError` if the user does not have any of the permitted roles.
...
def clear_cache(user: str | None = None, doctype: str | None = None):
	...
	import criscostack.utils.caching
	from criscostack.website.router import clear_routing_cache

	if doctype:
		criscostack.cache_manager.clear_doctype_cache(doctype)
		reset_metadata_version()
	elif user:
		criscostack.cache_manager.clear_user_cache(user)
	else:  # everything
		from criscostack import translate

		criscostack.cache_manager.clear_user_cache()
		criscostack.cache_manager.clear_domain_cache()
	...
	criscostack.utils.caching._SITE_CACHE.clear()
	local.role_permissions = {}
...
def only_has_select_perm(doctype, user=None, ignore_permissions=False):
	...
	from criscostack.permissions import get_role_permissions
...
def has_permission(
	...
	*,
	...
	Raises `criscostack.PermissionError` if user isn't permitted and `throw` is truthy
	...
	out = criscostack.permissions.has_permission(
		doctype,
		ptype,
		doc=doc,
		user=user,
...
def has_website_permission(doc=None, ptype="read", user=None, verbose=False, doctype=None):
	"""Raises `criscostack.PermissionError` if not permitted.
...
def get_precision(
	...
	from criscostack.model.meta import get_field_precision
...
def new_doc(
	...
	**kwargs,
	...
	from criscostack.model.create_new import get_new_doc
...
def set_value(doctype, docname, fieldname, value=None):
	"""Set document value. Calls `criscostack.client.set_value`"""
	import criscostack.client

	return criscostack.client.set_value(doctype, docname, fieldname, value)
...
def get_doc(**kwargs: dict) -> "_NewDocument":
	...
	Not recommended. Use `criscostack.new_doc` instead."""
	pass
...
def get_doc(*args, **kwargs):
	"""Return a `criscostack.model.document.Document` object of the given type and name.
	...
	Examples:
	        ...
	        todo = criscostack.get_doc({"doctype":"ToDo", "description": "test"})
	        todo.insert()

	        # open an existing document
	        todo = criscostack.get_doc("ToDo", "TD0001")

	"""
	import criscostack.model.document

	doc = criscostack.model.document.get_doc(*args, **kwargs)
...
def get_single(doctype):
	"""Return a `criscostack.model.document.Document` object of the given Single doctype."""
	return get_doc(doctype, doctype)


def get_meta(doctype, cached=True):
	"""Get `criscostack.model.meta.Meta` instance of given doctype name."""
	import criscostack.model.meta

	return criscostack.model.meta.get_meta(doctype, cached=cached)


def get_meta_module(doctype):
	import criscostack.modules

	return criscostack.modules.load_doctype_module(doctype)
...
def delete_doc(
	...
	"""Delete a document. Calls `criscostack.model.delete_doc.delete_doc`.
	...
	import criscostack.model.delete_doc

	return criscostack.model.delete_doc.delete_doc(
		doctype,
		name,
		force,
		ignore_doctypes,
...
def reload_doc(
	...
	return criscostack.modules.reload_doc(module, dt, dn, force=force, reset_permissions=reset_permissions)
...
def rename_doc(
	...
	*,
	...
	Calls `criscostack.model.rename_doc.rename_doc`
	"""
...
def get_all_apps(with_internal_apps=True, sites_path=None):
	...
	if "criscostack" in apps:
		apps.remove("criscostack")
	apps.insert(0, "criscostack")
...
def get_file_items(path, raise_not_found=False, ignore_empty_lines=True):
	...
	if content:
		content = criscostack.utils.strip(content)
...
def import_doc(path):
	...
	from criscostack.core.doctype.data_import.data_import import import_doc
...
def redirect(url):
	...
	from criscostack.exceptions import Redirect
...
def redirect_to_message(title, html, http_status_code=None, context=None, indicator_color=None):
	...
	Example Usage:
	        criscostack.redirect_to_message(_('Thank you'), "<div><p>You will receive an email at test@example.com</p></div>")
...
def build_match_conditions(doctype, as_condition=True):
	...
	import criscostack.desk.reportview

	return criscostack.desk.reportview.build_match_conditions(doctype, as_condition=as_condition)


def get_list(doctype, *args, **kwargs):
	"""List database query via `criscostack.model.db_query`. Will also check for permissions.
	...
	Example usage:
	        ...
	        # simple dict filter
	        criscostack.get_list("ToDo", fields=["name", "description"], filters = {"owner":"test@example.com"})

	        # filter as a list of lists
	        criscostack.get_list("ToDo", fields="*", filters = [["modified", ">", "2014-01-01"]])
	"""
	import criscostack.model.db_query

	return criscostack.model.db_query.DatabaseQuery(doctype).execute(*args, **kwargs)


def get_all(doctype, *args, **kwargs):
	"""List database query via `criscostack.model.db_query`. Will **not** check for permissions.
	Parameters are same as `criscostack.get_list`
	...
	Example usage:
	        ...
	        # simple dict filter
	        criscostack.get_all("ToDo", fields=["name", "description"], filters = {"owner":"test@example.com"})

	        # filter as a list of lists
	        criscostack.get_all("ToDo", fields=["*"], filters = [["modified", ">", "2014-01-01"]])
	"""
...
def get_value(*args, **kwargs):
	...
	Alias for `criscostack.db.get_value`
...
def as_json(obj: dict | list, indent=1, separators=None, ensure_ascii=True) -> str:
	from criscostack.utils.response import json_handler
...
def get_test_records(doctype):
	...
	from criscostack.modules import get_doctype_module, get_module_path
...
def format_value(*args, **kwargs):
	...
	import criscostack.utils.formatters

	return criscostack.utils.formatters.format_value(*args, **kwargs)
...
def get_print(
	...
	from criscostack.utils.pdf import get_pdf
...
def attach_print(
	...
	from criscostack.utils import scrub_urls
	from criscostack.utils.pdf import get_pdf
...
def publish_progress(*args, **kwargs):
	...
	"""
	import criscostack.realtime

	return criscostack.realtime.publish_progress(*args, **kwargs)
...
def publish_realtime(*args, **kwargs):
	...
	return criscostack.realtime.publish_realtime(*args, **kwargs)


def local_cache(namespace, key, generator, regenerate_if_none=False):
	"""A key value store for caching within a request

	:param namespace: criscostack.local.cache[namespace]
	:param key: criscostack.local.cache[namespace][key] used to retrieve value
...
def enqueue(*args, **kwargs):
	...
	"""
	import criscostack.utils.background_jobs

	return criscostack.utils.background_jobs.enqueue(*args, **kwargs)
...
def enqueue_doc(*args, **kwargs):
	...
	return criscostack.utils.background_jobs.enqueue_doc(*args, **kwargs)
...
def logger(
	...
	from criscostack.utils.logger import get_logger
...
def get_active_domains():
	from criscostack.core.doctype.domain_settings.domain_settings import get_active_domains
...
def get_version(doctype, name, limit=None, head=False, raise_err=True):
	...
	Example
	>>> criscostack.get_version('User', 'foobar@gmail.com')
	>>>
	[
	        {
	...
	if meta.track_changes:
		...
		from criscostack.utils import dictify, safe_json_loads, squashify
...
def parse_json(val):
	from criscostack.utils import parse_json
...
def mock(type, size=1, locale="en"):
	...
	from criscostack.utils import squashify
...
def validate_and_sanitize_search_inputs(fn):
	...
	def wrapper(*args, **kwargs):
		from criscostack.desk.search import sanitize_searchfield
		from criscostack.utils import cint
...
from criscostack.utils.error import log_error  # noqa: backward compatibility

if _tune_gc:
	# generational GC gets triggered after certain allocs (g0) which is 700 by default.
	# This number is quite small for criscostack where a single query can potentially create 700+
	# objects easily.
