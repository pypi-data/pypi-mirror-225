...
def pass_context(f):
	...
	def _func(ctx, *args, **kwargs):
		...
		try:
			ret = f(criscostack._dict(ctx.obj), *args, **kwargs)
		except criscostack.exceptions.SiteNotSpecifiedError as e:
			click.secho(str(e), fg="yellow")
			sys.exit(1)
		except criscostack.exceptions.IncorrectSitePath:
...
def get_site(context, raise_err=True):
	try:
		...
		if raise_err:
			raise criscostack.SiteNotSpecifiedError
		return None
...
def get_commands():
	...
	clickable_link = "https://criscostackframework.com/docs"
	all_commands = (
