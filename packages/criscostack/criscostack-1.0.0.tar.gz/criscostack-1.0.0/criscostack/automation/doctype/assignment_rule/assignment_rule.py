...
from criscostack.cache_manager import clear_doctype_map, get_doctype_map
...
class AssignmentRule(Document):
	...
	if TYPE_CHECKING:
		from criscostack.automation.doctype.assignment_rule_day.assignment_rule_day import AssignmentRuleDay
		from criscostack.automation.doctype.assignment_rule_user.assignment_rule_user import (
			AssignmentRuleUser,
		)
	...
	def validate_document_types(self):
		if self.document_type == "ToDo":
			criscostack.throw(
				_("Assignment Rule is not allowed on {0} document type").format(criscostack.bold("ToDo"))
			)
		...
		if user:
			assign_to.add(
				dict(
					...
					name=doc.get("name"),
					description=criscostack.render_template(self.description, doc),
	...
	def get_user_based_on_field(self, doc):
		...
		if criscostack.db.exists("User", val):
			return val

	def safe_eval(self, fieldname, doc):
		try:
			if self.get(fieldname):
				return criscostack.safe_eval(self.get(fieldname), None, doc)
		except Exception as e:
			# when assignment fails, don't block the document as it may be
			# a part of the email pulling
			criscostack.msgprint(criscostack._("Auto assignment failed: {0}").format(str(e)), indicator="orange")
	...
	def is_rule_not_applicable_today(self):
		today = criscostack.flags.assignment_day or criscostack.utils.get_weekday()
		assignment_days = self.get_assignment_days()
...
def reopen_closed_assignment(doc):
	...
	for todo in todo_list:
		todo_doc = criscostack.get_doc("ToDo", todo)
...
def apply(doc=None, method=None, doctype=None, name=None):
	...
	assignment_rule_docs: list[AssignmentRule] = [
		criscostack.get_cached_doc("Assignment Rule", d.get("name")) for d in assignment_rules
	]
	...
	if assignments:
		for assignment_rule in assignment_rule_docs:
			...
			if not new_apply:
				...
				if to_close_todos:
					# close todo status
					todos_to_close = criscostack.get_all(
						"ToDo",
						filters={
							"reference_type": doc.doctype,
							"reference_name": doc.name,
					...
					for todo in todos_to_close:
						_todo = criscostack.get_doc("ToDo", todo)
						_todo.status = "Closed"
...
def update_due_date(doc, state=None):
	...
	for rule in assignment_rules:
		rule_doc = criscostack.get_cached_doc("Assignment Rule", rule.get("name"))
		...
		if field_updated:
			assignment_todos = criscostack.get_all(
				"ToDo",
				filters={
...
def get_assignment_rules() -> list[str]:
	return criscostack.get_all("Assignment Rule", filters={"disabled": 0}, pluck="document_type")
