...
import criscostack
from criscostack import _, get_module_path, scrub
...
def export_module_json(doc: "Document", is_standard: bool, module: str) -> str | None:
	...
	`/home/gavin/criscostack-bench/apps/criscostack/criscostack/core/print_format/_test_print_format_1/_test_print_format_1`
	"""
	if not criscostack.flags.in_import and is_standard and criscostack.conf.developer_mode:
		...
		return os.path.join(
			criscostack.get_module_path(module), scrub(doc.doctype), scrub(doc.name), scrub(doc.name)
		)
...
def get_doc_module(module: str, doctype: str, name: str) -> "ModuleType":
	...
	module_name = "{app}.{module}.{doctype}.{name}.{name}".format(
		app=criscostack.local.module_app[scrub(module)],
		doctype=scrub(doctype),
		module=scrub(module),
		name=scrub(name),
	)
	return criscostack.get_module(module_name)
...
def export_customizations(
	...
	if not criscostack.conf.developer_mode:
		criscostack.throw(_("Only allowed to export customizations in developer mode"))

	custom = {
		"custom_fields": criscostack.get_all("Custom Field", fields="*", filters={"dt": doctype}),
		"property_setters": criscostack.get_all("Property Setter", fields="*", filters={"doc_type": doctype}),
		"custom_perms": [],
		"links": criscostack.get_all("DocType Link", fields="*", filters={"parent": doctype}),
		"doctype": doctype,
		"sync_on_migrate": sync_on_migrate,
	}

	if with_permissions:
		custom["custom_perms"] = criscostack.get_all(
			"Custom DocPerm", fields="*", filters={"parent": doctype}
		)

	# also update the custom fields and property setters for all child tables
	for d in criscostack.get_meta(doctype).get_table_fields():
	...
	if custom["custom_fields"] or custom["property_setters"] or custom["custom_perms"]:
		...
		with open(path, "w") as f:
			f.write(criscostack.as_json(custom))

		criscostack.msgprint(_("Customizations for <b>{0}</b> exported to:<br>{1}").format(doctype, path))
		return path
...
def sync_customizations(app=None):
	...
	for app_name in apps:
		for module_name in criscostack.local.app_modules.get(app_name) or []:
			folder = criscostack.get_app_path(app_name, module_name, "custom")
...
def sync_customizations_for_doctype(data: dict, folder: str, filename: str = ""):
		...
		def sync_single_doctype(doc_type):
			def _insert(data):
				if data.get(doctype_fieldname) == doc_type:
					...
					doc = criscostack.get_doc(data)
					doc.db_insert()

			if custom_doctype != "Custom Field":
				criscostack.db.delete(custom_doctype, {doctype_fieldname: doc_type})
				...
				for d in data[key]:
					field = criscostack.db.get_value("Custom Field", {"dt": doc_type, "fieldname": d["fieldname"]})
					if not field:
	...
	if not criscostack.db.exists("DocType", doctype):
	...
	if update_schema and not criscostack.db.get_value("DocType", doctype, "issingle"):
...
def reload_doc(
	...
	from criscostack.modules.import_file import import_files
...
def export_doc(doctype, name, module=None):
	...
	from criscostack.modules.export_file import write_document_file

	print(f"Exporting Document {doctype} {name}")
	module = module or criscostack.db.get_value("DocType", name, "module")
	write_document_file(criscostack.get_doc(doctype, name), module)


def get_doctype_module(doctype: str) -> str:
	"""Returns **Module Def** name of given doctype."""
	doctype_module_map = criscostack.cache.get_value(
		"doctype_modules",
		generator=lambda: dict(criscostack.qb.from_("DocType").select("name", "module").run()),
	)

	if module_name := doctype_module_map.get(doctype):
		return module_name
	else:
		criscostack.throw(_("DocType {} not found").format(doctype), exc=criscostack.DoesNotExistError)
...
def load_doctype_module(doctype, module=None, prefix="", suffix=""):
	...
	if key not in doctype_python_modules:
		try:
			doctype_python_modules[key] = criscostack.get_module(module_name)
...
def get_module_app(module: str) -> str:
	app = criscostack.local.module_app.get(scrub(module))
	if app is None:
		criscostack.throw(_("Module {} not found").format(module), exc=criscostack.DoesNotExistError)
	return app
...
def get_app_publisher(module: str) -> str:
	app = get_module_app(module)
	if not app:
		criscostack.throw(_("App not found for module: {0}").format(module))
	return criscostack.get_hooks(hook="app_publisher", app_name=app)[0]


def make_boilerplate(
	template: str, doc: Union["Document", "criscostack._dict"], opts: Union[dict, "criscostack._dict"] = None
):
	...
	doc = doc or criscostack._dict()
	opts = opts or criscostack._dict()
	app_publisher = get_app_publisher(doc.module)
	base_class = "Document"
	base_class_import = "from criscostack.model.document import Document"
	controller_body = "pass"

	if doc.get("is_tree"):
		base_class = "NestedSet"
		base_class_import = "from criscostack.utils.nestedset import NestedSet"
	...
	with open(target_file_path, "w") as target, open(template_file_path) as source:
		...
		target.write(criscostack.as_unicode(controller_file_content))
