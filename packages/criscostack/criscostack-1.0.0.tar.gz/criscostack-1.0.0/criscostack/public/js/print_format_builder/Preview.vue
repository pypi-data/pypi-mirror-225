...
function get_default_docname() {
	return criscostack.db.get_list(doctype.value, { limit: 1 }).then(doc => {
...
let url = computed(() => {
	...
	let _url =
		type.value == "PDF"
			? `/api/method/criscostack.utils.weasyprint.download_pdf`
			: "/printpreview";
...
// mounted
onMounted(() => {
	doc_select.value = criscostack.ui.form.make_control({
	...
	preview_type.value = criscostack.ui.form.make_control({
