...
function toggle_edit_letterhead() {
	...
	if (!control.value) {
		control.value = criscostack.ui.form.make_control({
			parent: editor.value,
			df: {
...
// mounted
onMounted(() => {
	if (!letterhead.value && criscostack.boot.sysdefaults.letter_head) {
		set_letterhead(criscostack.boot.sysdefaults.letter_head);
	}
