...
function remove_field() {
	if (store.is_customize_form && props.field.df.is_custom_field == 0) {
		criscostack.msgprint(__("Cannot delete standard field. You can hide it if you want"));
...
function duplicate_field() {
	...
	duplicate_field.df.owner = criscostack.session.user;
		...
		>
			<template #label>
				<div class="field-label">
					...
					<div class="help-icon" v-if="field.df.documentation_url" v-html="criscostack.utils.icon('help', 'sm')"></div>
				</div>
			</template>
			<template #actions>
				<div class="field-actions" :hidden="store.read_only">
					<button
						...
						@click="edit_html"
					>
						<div v-html="criscostack.utils.icon('edit', 'sm')"></div>
					</button>
					<button
					...
					>
						<div v-html="criscostack.utils.icon('move', 'sm')"></div>
					</button>
					<button class="btn btn-xs btn-icon" @click.stop="duplicate_field">
						<div v-html="criscostack.utils.icon('duplicate', 'sm')"></div>
					</button>
					<button class="btn btn-xs btn-icon" @click.stop="remove_field">
						<div v-html="criscostack.utils.icon('remove', 'sm')"></div>
					</button>
				</div>
			</template>
		</component>
