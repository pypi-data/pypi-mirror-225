...
criscostack.provide("criscostack.ui.keys.handlers");

criscostack.ui.keys.setup = function () {
	$(window).on("keydown", function (e) {
		var key = criscostack.ui.keys.get_key(e);
		if (criscostack.ui.keys.handlers[key]) {
			var out = null;
			for (var i = 0, l = criscostack.ui.keys.handlers[key].length; i < l; i++) {
				var handler = criscostack.ui.keys.handlers[key][i];
...
criscostack.ui.keys.standard_shortcuts = standard_shortcuts;
criscostack.ui.keys.add_shortcut = ({
	...
	criscostack.ui.keys.off(shortcut, page);
	// attach new handler
	criscostack.ui.keys.on(shortcut, handler);
...
criscostack.ui.keys.show_keyboard_shortcut_dialog = () => {
	if (criscostack.ui.keys.is_dialog_shown) return;
		...
		let html = shortcuts
			...
			.map((shortcut) => {
				let shortcut_label = shortcut.shortcut
					.split("+")
					.map(criscostack.utils.to_title_case)
					.join("+");
				if (criscostack.utils.is_mac()) {
	...
	let dialog = new criscostack.ui.Dialog({
		...
		on_hide() {
			criscostack.ui.keys.is_dialog_shown = false;
		},
	});
	...
	dialog.show();
	criscostack.ui.keys.is_dialog_shown = true;
};

criscostack.ui.keys.get_key = function (e) {
	var keycode = e.keyCode || e.which;
	var key = criscostack.ui.keys.key_map[keycode] || String.fromCharCode(keycode);
...
criscostack.ui.keys.on = function (key, handler) {
	if (!criscostack.ui.keys.handlers[key]) {
		criscostack.ui.keys.handlers[key] = [];
	}
	criscostack.ui.keys.handlers[key].push(handler);
};

criscostack.ui.keys.off = function (key, page) {
	let handlers = criscostack.ui.keys.handlers[key];
	if (!handlers || handlers.length === 0) return;
	criscostack.ui.keys.handlers[key] = handlers.filter((h) => {
...
criscostack.ui.keys.add_shortcut({
	shortcut: "ctrl+s",
	action: function (e) {
		criscostack.app.trigger_primary_action();
		e.preventDefault();
		return false;
	},
...
criscostack.ui.keys.on("escape", function (e) {
	handle_escape_key();
});

criscostack.ui.keys.on("esc", function (e) {
	handle_escape_key();
});

criscostack.ui.keys.on("enter", function (e) {
	if (window.cur_dialog && cur_dialog.confirm_dialog) {
		cur_dialog.get_primary_btn().trigger("click");
	}
});

criscostack.ui.keys.on("ctrl+down", function (e) {
	var grid_row = criscostack.ui.form.get_open_grid_form();
	grid_row &&
...
criscostack.ui.keys.on("ctrl+up", function (e) {
	var grid_row = criscostack.ui.form.get_open_grid_form();
	grid_row &&
		grid_row.toggle_view(false, function () {
			grid_row.open_prev();
...
"abcdefghijklmnopqrstuvwxyz".split("").forEach((letter, i) => {
	criscostack.ui.keys.key_map[65 + i] = letter;
});

// keyCode map
criscostack.ui.keyCode = {
	ESCAPE: 27,
	LEFT: 37,
	RIGHT: 39,
	UP: 38,
