criscostack.ui.form.set_user_image = function (frm) {
	...
	if (image) {
		image = window.cordova && image.indexOf("http") === -1 ? criscostack.base_url + image : image;
		...
		image_section
			...
			.removeClass("hide")
			.find(".standard-image")
			.html(criscostack.get_abbr(title));

		title_image.css("background-image", "").html(criscostack.get_abbr(title));
...
criscostack.ui.form.setup_user_image_event = function (frm) {
	// re-draw image on change of user image
	if (frm.meta.image_field) {
		criscostack.ui.form.on(frm.doctype, frm.meta.image_field, function (frm) {
			criscostack.ui.form.set_user_image(frm);
		});
	}
