...
criscostack.views.ListViewSelect = class ListViewSelect {
	constructor(opts) {
		$.extend(this, opts);
	...
	set_current_view() {
		...
		const view_name = criscostack.utils.to_title_case(route[2] || "");
		if (route.length > 2 && criscostack.views.view_modes.includes(view_name)) {
	...
	setup_views() {
		const views = {
			...
			Report: {
				...
				current_view_handler: () => {
					...
					if (criscostack.get_route().length > 3) {
						default_action = {
			...
			Calendar: {
				condition: criscostack.views.calendar[this.doctype],
			...
			Inbox: {
				condition: this.doctype === "Communication" && criscostack.boot.email_accounts.length,
				action: () => this.set_route("inbox"),
				current_view_handler: () => {
					const accounts = this.get_email_accounts();
					let default_action;
					if (has_common(criscostack.user_roles, ["System Manager", "Administrator"])) {
						default_action = {
							label: __("New Email Account"),
							action: () => criscostack.new_doc("Email Account"),
						};
					}
			...
			Tree: {
				condition:
					criscostack.treeview_settings[this.doctype] ||
					criscostack.get_meta(this.doctype).is_tree,
		...
		criscostack.views.view_modes.forEach((view) => {
	...
	setup_kanban_switcher(kanbans) {
		...
		if (can_create) {
			this.page.add_custom_menu_item(
				...
				() => criscostack.views.KanbanView.show_kanban_dialog(this.doctype),
				true
			);
		}
	}
	...
	get_page_name() {
		return criscostack.utils.to_title_case(criscostack.get_route().slice(-1)[0] || "");
	}
	...
	get_reports() {
		...
		var reports =
			Object.values(criscostack.boot.user.all_reports).sort((a, b) =>
	...
	setup_kanban_boards() {
			...
			criscostack.db.get_value(
				...
				(board) => {
					if (!$.isEmptyObject(board)) {
						criscostack.set_route("list", doctype, "kanban", board.name);
					} else {
						criscostack.views.KanbanView.show_kanban_dialog(doctype);
					}
				}
			);
		}
		...
		const last_opened_kanban =
			criscostack.model.user_settings[this.doctype]["Kanban"]?.last_kanban_board;
		if (!last_opened_kanban) {
			fetch_kanban_board(this.doctype);
		} else {
			criscostack.db.exists("Kanban Board", last_opened_kanban).then((exists) => {
				if (exists) {
					criscostack.set_route("list", this.doctype, "kanban", last_opened_kanban);
				} else {
					fetch_kanban_board(this.doctype);
				}
			});
	...
	get_calendars() {
		...
		return criscostack.db
			...
			.then((result) => {
				...
				if (criscostack.views.calendar[this.doctype]) {
	...
	get_email_accounts() {
		let accounts_to_add = [];
		let accounts = criscostack.boot.email_accounts;
	...
	slug() {
		return criscostack.router.slug(criscostack.router.doctype_layout || this.doctype);
	}
};
