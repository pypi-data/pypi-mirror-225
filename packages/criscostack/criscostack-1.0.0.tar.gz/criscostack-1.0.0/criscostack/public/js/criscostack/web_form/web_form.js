...
criscostack.provide("criscostack.web_form");
import EventEmitterMixin from "../../criscostack/event_emitter";

export default class WebForm extends criscostack.ui.FieldGroup {
	constructor(opts) {
		super();
		Object.assign(this, opts);
		criscostack.web_form = this;
		criscostack.web_form.events = {};
		Object.assign(criscostack.web_form.events, EventEmitterMixin);
	...
	make() {
		...
		criscostack.init_client_script && criscostack.init_client_script();
		this.setup_listeners();
		criscostack.web_form.events.trigger("after_load");
	...
	set_default_values() {
		...
		let values = criscostack.utils.get_query_params();
		delete values.new;
	...
	render_progress_dots() {
		...
		for (let i = 0; i <= this.page_breaks.length; i++) {
			let $dot = $(`<div class="slide-step">
				...
				<div class="slide-step-complete">${criscostack.utils.icon("tick", "xs")}</div>
	...
	is_next_section_empty(section) {
		...
		let visible_controls = _section.find(".criscostack-control:not(.hide-control)");
	...
	save() {
		...
		// Save
		window.saving = true;
		criscostack.form_dirty = false;

		criscostack.call({
			type: "POST",
			method: "criscostack.website.doctype.web_form.web_form.accept",
			args: {
				data: this.doc,
				web_form: this.name,
				for_payment,
			...
			callback: (response) => {
				...
				if (!response.exc) {
					...
					criscostack.web_form.events.trigger("after_save");
					...
					if (is_new && (response.message.attachment || response.message.file)) {
						criscostack.call({
							type: "POST",
							method: "criscostack.handler.upload_file",
							args: {
	...
	handle_success(data) {
		...
		if (this.success_url) {
			criscostack.utils.setup_timer(5, 0, $(".time"));
			setTimeout(() => {
