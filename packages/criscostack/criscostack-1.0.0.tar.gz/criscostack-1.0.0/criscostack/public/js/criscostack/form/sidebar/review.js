...
criscostack.ui.form.Review = class Review {
	constructor({ parent, frm }) {
		this.parent = parent;
		this.frm = frm;
		this.points = criscostack.boot.points;
	...
	update_points() {
		return criscostack
			.xcall("criscostack.social.doctype.energy_point_log.energy_point_log.get_energy_points", {
				user: criscostack.session.user,
			})
			.then((data) => {
				criscostack.boot.points = data;
				this.points = data;
			});
	}
	...
	show_review_dialog() {
		...
		const review_dialog = new criscostack.ui.Dialog({
			title: __("Add Review"),
			fields: [
				{
					fieldname: "to_user",
			...
			primary_action: (values) => {
				...
				if (values.points > this.points.review_points) {
					return criscostack.msgprint(__("You do not have enough points"));
				}
				criscostack
					.xcall("criscostack.social.doctype.energy_point_log.energy_point_log.review", {
						doc: {
	...
	setup_detail_popover(el, data) {
		let subject = "";
		let fullname = criscostack.user.full_name(data.user);
		let timestamp = `<span class="text-muted">${criscostack.datetime.comment_when(
			data.creation
		)}</span>`;
			...
			content: () => {
				return `
					<div class="text-medium">
						...
						<div class="subject">
							${criscostack.utils.icon("review")}
							${subject}

							<p class="mt-1">
								<!-- ${criscostack.avatar("shivam@example.com", "avatar-xs")} -->
								<span>${criscostack.user.full_name(data.owner)}</span> - ${timestamp}
							</p>
						</div>
					</div>
				`;
