...
criscostack.ui.form.AssignTo = class AssignTo {
	constructor(opts) {
		$.extend(this, opts);
	...
	render(assignments) {
		...
		let avatar_group = criscostack.avatar_group(assigned_users, 5, {
			align: "left",
			overlap: true,
		});
		...
		avatar_group.click(() => {
			new criscostack.ui.form.AssignmentDialog({
	...
	add() {
		...
		if (this.frm.is_new()) {
			criscostack.throw(__("Please save the document before assignment"));
			return;
		}

		if (!me.assign_to) {
			me.assign_to = new criscostack.ui.form.AssignToDialog({
	...
	remove(owner) {
		if (this.frm.is_new()) {
			criscostack.throw(__("Please save the document before removing assignment"));
			return;
		}

		return criscostack
			.xcall("criscostack.desk.form.assign_to.remove", {
...
criscostack.ui.form.AssignToDialog = class AssignToDialog {
	constructor(opts) {
		$.extend(this, opts);
	...
	make() {
		...
		me.dialog = new criscostack.ui.Dialog({
	...
	assign_to_me() {
		...
		if (me.dialog.get_value("assign_to_me")) {
			assign_to.push(criscostack.session.user);
		}
		...
		return [
			...
			},
			{
				...
				get_data: function (txt) {
					return criscostack.db.get_link_options("User", txt, {
...
criscostack.ui.form.AssignmentDialog = class {
	constructor(opts) {
		this.frm = opts.frm;
	...
	make() {
		...
		this.assignment_list.removeClass("criscostack-control");
	...
	add_assignment(assignment) {
		return criscostack
			.xcall("criscostack.desk.form.assign_to.add", {
				doctype: this.frm.doctype,
				name: this.frm.docname,
	...
	remove_assignment(assignment) {
		return criscostack.xcall("criscostack.desk.form.assign_to.remove", {
			doctype: this.frm.doctype,
			name: this.frm.docname,
			assign_to: assignment,
		});
	...
	get_assignment_row(assignment) {
		let row = $(`
			<div class="dialog-assignment-row" data-user="${assignment}">
				<span>
					${criscostack.avatar(assignment)}
					${criscostack.user.full_name(assignment)}
				</span>
			</div>
		`);

		if (assignment === criscostack.session.user || this.frm.perm[0].write) {
			row.append(`
				<span class="remove-btn cursor-pointer">
					${criscostack.utils.icon("close")}
				</span>
			`);
