criscostack.ui.form.ControlImage = class ControlImage extends criscostack.ui.form.Control {
	make() {
		super.make();
	...
	refresh_input() {
		...
		if (doc && this.df.options && doc[this.df.options]) {
			...
			this.$buffer = $(
				`<div class='missing-image'>${criscostack.utils.icon("restriction", "md")}</div>`
			).appendTo(this.$body);
		}
		return false;
	}
