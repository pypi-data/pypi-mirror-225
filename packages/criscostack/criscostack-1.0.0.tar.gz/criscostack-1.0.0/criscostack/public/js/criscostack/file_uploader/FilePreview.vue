	...
	<div class="file-preview">
		<div class="file-icon">
			<img
				...
				:alt="file.name"
			>
			<div class="fallback" v-else v-html="criscostack.utils.icon('file', 'md')">
			</div>
		</div>
		<div>
			<div>
			...
			<div class="flex config-area">
				<label v-if="is_optimizable" class="criscostack-checkbox"><input type="checkbox" :checked="optimize" @change="emit('toggle_optimize')">{{ __('Optimize') }}</label>
				<label class="criscostack-checkbox"><input type="checkbox" :checked="file.private" @change="emit('toggle_private')">{{ __('Private') }}</label>
			</div>
			<div>
		...
		<div class="file-actions">
			<ProgressRing
				...
				:stroke="3"
			/>
			<div v-if="uploaded" v-html="criscostack.utils.icon('solid-success', 'lg')"></div>
			<div v-if="file.failed" v-html="criscostack.utils.icon('solid-error', 'lg')"></div>
			<div class="file-action-buttons">
				<button v-if="is_cropable" class="btn btn-crop muted" @click="emit('toggle_image_cropper')" v-html="criscostack.utils.icon('crop', 'md')"></button>
				<button v-if="!uploaded && !file.uploading && !file.failed" class="btn muted" @click="emit('remove')" v-html="criscostack.utils.icon('delete', 'md')"></button>
			</div>
		</div>
	</div>
</template>
...
let file_size = computed(() => {
	return criscostack.form.formatters.FileSize(props.file.file_obj.size);
});
