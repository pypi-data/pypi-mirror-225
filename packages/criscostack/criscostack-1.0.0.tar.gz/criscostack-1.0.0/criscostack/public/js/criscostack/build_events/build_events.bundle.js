...
criscostack.realtime.on("build_event", (data) => {
	if (data.success) {
			...
			for (let file of changed_files) {
				if (file.includes(".bundle.")) {
					...
					if (parts.length === 2) {
						...
						criscostack.assets.executed_ = criscostack.assets.executed_.filter(
		...
		// update assets json
		criscostack.call("criscostack.sessions.get_boot_assets_json").then((r) => {
			if (r.message) {
				criscostack.boot.assets_json = r.message;

				if (criscostack.hot_update) {
					criscostack.hot_update.forEach((callback) => {
						callback();
					});
				}
			}
