...
criscostack.ui.form.ControlPhone = class ControlPhone extends criscostack.ui.form.ControlData {
	async make_input() {
	...
	async setup_country_codes() {
		...
		if (data) {
			...
			const data = await criscostack.xcall("criscostack.geo.country_info.get_country_timezone_info");
	...
	input_events() {
		this.$input.keydown((e) => {
			...
			if ([criscostack.ui.keyCode.BACKSPACE].includes(key_code)) {
	...
	setup_country_code_picker() {
		...
		if (!this.selected_icon.length) {
			this.selected_icon = $(
				`<div class="selected-phone">${criscostack.utils.icon("down", "sm")}</div>`
			);
	...
	get_country_flag(country) {
		...
		return criscostack.utils.flag(code);
	}
