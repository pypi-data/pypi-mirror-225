...
criscostack.ui.open_dialogs = [];

criscostack.ui.Dialog = class Dialog extends criscostack.ui.FieldGroup {
	constructor(opts) {
		super();
		this.display = false;
		this.is_dialog = true;
	...
	make() {
		this.$wrapper = criscostack.get_modal("", "");
		...
		this.$wrapper
			.on("hide.bs.modal", function () {
				...
				if (criscostack.ui.open_dialogs[criscostack.ui.open_dialogs.length - 1] === me) {
					criscostack.ui.open_dialogs.pop();
					if (criscostack.ui.open_dialogs.length) {
						window.cur_dialog =
							criscostack.ui.open_dialogs[criscostack.ui.open_dialogs.length - 1];
					} else {
						window.cur_dialog = null;
					}
				}
			...
			.on("shown.bs.modal", function () {
				...
				criscostack.ui.open_dialogs.push(me);
				me.focus_on_first_input();
				me.hide_scrollbar(true);
				me.on_page_show && me.on_page_show();
				$(document).trigger("criscostack.ui.Dialog:shown");
	...
	toggle_minimize() {
		...
		this.get_minimize_btn().html(criscostack.utils.icon(icon));
