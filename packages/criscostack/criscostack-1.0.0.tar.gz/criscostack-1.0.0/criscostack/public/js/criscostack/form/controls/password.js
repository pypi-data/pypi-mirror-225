criscostack.ui.form.ControlPassword = class ControlPassword extends criscostack.ui.form.ControlData {
	...
	make_input() {
		...
		this.toggle_password.on("click", () => {
			if (this.$input.attr("type") === "password") {
				...
				this.toggle_password.html(criscostack.utils.icon("hide", "sm"));
			} else {
				this.$input.attr("type", "password");
				this.toggle_password.html(criscostack.utils.icon("unhide", "sm"));
			}
		});
	...
	get_password_strength(value) {
		...
		criscostack.call({
			type: "POST",
			method: "criscostack.core.doctype.user.user.test_password_strength",
			args: {
				new_password: value || "",
			},
			callback: function (r) {
