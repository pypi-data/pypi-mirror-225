...
criscostack.quick_edit = function (doctype, name) {
	criscostack.db.get_doc(doctype, name).then((doc) => {
		criscostack.ui.form.make_quick_entry(doctype, null, null, doc);
	});
};

criscostack.ui.form.make_quick_entry = (doctype, after_insert, init_callback, doc, force) => {
	var trimmed_doctype = doctype.replace(/ /g, "");
	var controller_name = "QuickEntryForm";

	if (criscostack.ui.form[trimmed_doctype + "QuickEntryForm"]) {
		controller_name = trimmed_doctype + "QuickEntryForm";
	}

	criscostack.quick_entry = new criscostack.ui.form[controller_name](
		doctype,
		after_insert,
		init_callback,
		doc,
	...
	return criscostack.quick_entry.setup();
};

criscostack.ui.form.QuickEntryForm = class QuickEntryForm {
	...
	check_quick_entry_doc() {
		if (!this.doc) {
			this.doc = criscostack.model.get_new_doc(this.doctype, null, null, true);
		}
	}
	...
	render_dialog() {
		...
		this.dialog.onhide = () => (criscostack.quick_entry = null);
		this.dialog.show();
	...
	insert() {
		...
		return new Promise((resolve) => {
			me.update_doc();
			criscostack.call({
				method: "criscostack.client.save",
				args: {
					doc: me.dialog.doc,
				},
				callback: function (r) {
					if (criscostack.model.is_submittable(me.doctype)) {
						...
						me.dialog.hide();
						// delete the old doc
						criscostack.model.clear_doc(me.dialog.doc.doctype, me.dialog.doc.name);
						me.dialog.doc = r.message;
						if (criscostack._from_link) {
							criscostack.ui.form.update_calling_link(me.dialog.doc);
						} else {
							if (me.after_insert) {
	...
	submit(doc) {
		var me = this;
		criscostack.call({
			method: "criscostack.client.submit",
			args: {
				doc: doc,
			},
			callback: function (r) {
	...
	open_form_if_not_list() {
		...
		if (route && !(route[0] === "List" && route[1] === doc.doctype)) {
			criscostack.run_serially([() => criscostack.set_route("Form", doc.doctype, doc.name)]);
		}
	}
	...
	open_doc(set_hooks) {
		...
		if (set_hooks && this.after_insert) {
			criscostack.route_options = criscostack.route_options || {};
			criscostack.route_options.after_save = (frm) => {
				this.after_insert(frm);
			};
		}
		criscostack.set_route("Form", this.doctype, this.doc.name);
	}
