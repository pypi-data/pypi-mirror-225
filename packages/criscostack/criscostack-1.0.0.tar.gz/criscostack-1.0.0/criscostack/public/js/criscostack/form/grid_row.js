...
export default class GridRow {
	...
	set_docfields(update = false) {
		if (this.doc && this.parent_df.options) {
			criscostack.meta.make_docfield_copy_for(
				this.parent_df.options,
				this.doc.name,
				this.docfields
			);
			const docfields = criscostack.meta.get_docfields(this.parent_df.options, this.doc.name);
			if (update) {
			...
			if (this.frm) {
				...
				criscostack
					.run_serially([
						...
						() => {
							criscostack.model.clear_doc(this.doc.doctype, this.doc.name);
		...
		criscostack.prompt(
			...
			function (values) {
				if (me.doc._sortable === false) {
					criscostack.msgprint(__("Cannot move row"));
					return;
				}
	...
	render_template() {
		...
		this.row_display = $('<div class="row-data sortable-handle template-row"></div>')
			.appendTo(this.row)
			.html(
				criscostack.render(this.grid.template, {
					doc: this.doc ? criscostack.get_format_helper(this.doc) : null,
					frm: this.frm,
					row: this,
				})
			);
	...
	render_row(refresh) {
		...
		if (!this.row_index && !this.show_search) {
			...
			criscostack.utils.only_allow_num_decimal(this.row_index.find("input"));
		} else {
				...
				if (!this.configure_columns) {
					this.open_form_button = $(`
						<div class="btn-open-row">
							<a>${criscostack.utils.icon("edit", "xs")}</a>
		...
		if (this.configure_columns && this.frm) {
			this.configure_columns_button = $(`
				<div class="col grid-static-col d-flex justify-content-center" style="cursor: pointer;">
					<a>${criscostack.utils.icon("setting-gear", "sm", "", "filter: opacity(0.5)")}</a>
				</div>
			`)
				.appendTo(this.row)
				.on("click", () => {
	...
	configure_dialog_for_columns_selector() {
		this.grid_settings_dialog = new criscostack.ui.Dialog({
		...
		d.set_primary_action(__("Add"), () => {
			...
			if (selected_fields) {
				selected_fields.forEach((selected_column) => {
					let docfield = criscostack.meta.get_docfield(this.grid.doctype, selected_column);
	...
	prepare_columns_for_dialog(selected_fields) {
		...
		const blocked_fields = criscostack.model.no_value_type;
	...
	render_selected_columns() {
		...
		if (this.selected_columns_for_grid) {
			this.selected_columns_for_grid.forEach((d) => {
				let docfield = criscostack.meta.get_docfield(this.grid.doctype, d.fieldname);
					...
					<div class='control-input flex align-center form-control fields_order sortable-handle sortable'
						...
						<div class='row'>
							<div class='col-md-1' style='padding-top: 2px'>
								<a style='cursor: grabbing;'>${criscostack.utils.icon("drag", "xs")}</a>
							</div>
		...
		$(this.fields_html_wrapper)
			...
			.change((event) => {
				if (cint(event.target.value) === 0) {
					...
					criscostack.throw(__("Column width cannot be zero."));
				}
	...
	validate_columns_width() {
		...
		if (total_column_width && total_column_width > 10) {
			criscostack.throw(__("The total column width cannot be more than 10."));
		}
	}
		...
		$(this.fields_html_wrapper)
			...
			.click((event) => {
				...
				if (selected_columns_for_grid && selected_columns_for_grid.length === 0) {
					criscostack.throw(__("At least one column is required to show in the grid."));
				}
	...
	update_user_settings_for_grid() {
		...
		criscostack.model.user_settings.save(this.frm.doctype, "GridView", value).then((r) => {
			criscostack.model.user_settings[this.frm.doctype] = r.message || r;
			this.grid.reset_grid();
		});
	}

	reset_user_settings_for_grid() {
		criscostack.model.user_settings.save(this.frm.doctype, "GridView", null).then((r) => {
			criscostack.model.user_settings[this.frm.doctype] = r.message || r;
			this.grid.reset_grid();
		});
	}
	...
	setup_columns() {
		...
		this.grid.visible_columns.forEach((col, ci) => {
			...
			let txt = this.doc
				? criscostack.format(this.doc[df.fieldname], df, null, this.doc)
				: __(df.label);
	...
	evaluate_depends_on_value(expression) {
		...
		if (typeof expression === "boolean") {
			...
			try {
				out = criscostack.utils.eval(expression.substr(5), { doc, parent });
				if (parent && parent.istable && expression.includes("is_submittable")) {
					out = true;
				}
			} catch (e) {
				criscostack.throw(__('Invalid "depends_on" expression'));
			}
	...
	make_search_column(df, colsize) {
		...
		["Currency", "Float", "Int", "Percent", "Rating"].includes(df.fieldtype) &&
			criscostack.utils.only_allow_num_decimal($search_input);
		...
		var $col = $(
			...
			.on("touchmove", function (event) {
				...
				if (horizontal) {
					...
					grid.style.left = `${criscostack.utils.is_rtl() ? "" : "-"}${grid_start}px`;
				}
			})
			...
			.on("click", function (event) {
				if (criscostack.ui.form.editable_row !== me) {
	...
	toggle_editable_row(show) {
		...
		if (
			...
			if (criscostack.ui.form.editable_row && criscostack.ui.form.editable_row !== this) {
				criscostack.ui.form.editable_row.toggle_editable_row(false);
			}
			...
			criscostack.ui.form.editable_row = this;
			return false;
		} else {
			...
			criscostack.ui.form.editable_row = null;
		}
	}
	...
	make_control(column) {
		...
		var field = criscostack.ui.form.make_control({
			df: df,
			parent: parent,
			only_input: true,
			with_link_btn: true,
	...
	set_arrow_keys(field) {
		...
		if (field.$input) {
			field.$input.on("keydown", function (e) {
				var { TAB, UP: UP_ARROW, DOWN: DOWN_ARROW } = criscostack.ui.keyCode;
	...
	show_form() {
		...
		criscostack.dom.freeze("", "dark grid-form");
		if (cur_frm) cur_frm.cur_grid = this;
		this.wrapper.addClass("grid-row-open");
		if (
			!criscostack.dom.is_element_in_viewport(this.wrapper) &&
			!criscostack.dom.is_element_in_modal(this.wrapper)
		) {
			// -15 offset to make form look visually centered
			criscostack.utils.scroll_to(this.wrapper, true, -15);
		}
	...
	hide_form() {
		...
		if (!criscostack.dom.is_element_in_modal(this.row)) {
			criscostack.utils.scroll_to(this.row, true, 15);
		}
		this.refresh();
	...
	refresh_field(fieldname, txt) {
		...
		if (df && this.doc) {
			txt = criscostack.format(this.doc[fieldname], df, null, this.doc);
		}

		if (!txt && this.frm) {
			txt = criscostack.format(this.doc[fieldname], df, null, this.frm.doc);
		}
	...
	get_visible_columns(blacklist = []) {
		...
		var visible_columns = $.map(this.docfields, function (df) {
			var visible =
				...
				!in_list(criscostack.model.layout_fields, df.fieldtype) &&
