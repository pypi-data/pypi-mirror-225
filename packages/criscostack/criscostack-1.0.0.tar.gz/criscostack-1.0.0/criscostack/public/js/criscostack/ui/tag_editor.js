criscostack.ui.TagEditor = class TagEditor {
	constructor(opts) {
		/* docs:
		Arguments
	...
	setup_tags() {
		...
		this.tags = new criscostack.ui.Tags({
			parent: this.wrapper,
			placeholder: __("Add a tag ..."),
			onTagAdd: (tag) => {
				if (me.initialized && !me.refreshing) {
					return criscostack.call({
						method: "criscostack.desk.doctype.tag.tag.add_tag",
			...
			onTagRemove: (tag) => {
				if (!me.refreshing) {
					return criscostack.call({
						method: "criscostack.desk.doctype.tag.tag.remove_tag",
						args: me.get_args(tag),
						callback: function (r) {
	...
	setup_awesomplete() {
		...
		$input.on("input", function (e) {
			...
			criscostack.call({
				method: "criscostack.desk.doctype.tag.tag.get_tags",
				args: {
