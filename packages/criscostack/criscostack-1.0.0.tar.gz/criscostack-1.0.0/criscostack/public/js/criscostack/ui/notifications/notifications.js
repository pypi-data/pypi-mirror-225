...
criscostack.ui.Notifications = class Notifications {
	constructor() {
		this.tabs = {};
		this.notification_settings = criscostack.boot.notification_settings;
		this.make();
	}
	...
	make() {
		...
		this.user = criscostack.session.user;
	...
	setup_headers() {
		...
		$(`<span class="notification-settings pull-right" data-action="go_to_settings">
			${criscostack.utils.icon("setting-gear")}
		</span>`)
			.on("click", (e) => {
				e.stopImmediatePropagation();
				this.dropdown.dropdown("hide");
				criscostack.set_route("Form", "Notification Settings", criscostack.session.user);
			})
		...
		$(`<span class="mark-all-read pull-right" data-action="mark_all_as_read">
			${criscostack.utils.icon("mark-as-read")}
		</span>`)
	...
	mark_all_as_read(e) {
		...
		criscostack.call("criscostack.desk.doctype.notification_log.notification_log.mark_all_as_read");
	}
...
criscostack.ui.notifications = {
	get_notification_config() {
		return criscostack.xcall("criscostack.desk.notifications.get_notification_info").then((r) => {
			criscostack.ui.notifications.config = r;
			return r;
		});
	},

	show_open_count_list(doctype) {
		if (!criscostack.ui.notifications.config) {
	...
	route_to_list_with_filters(doctype) {
		let filters = criscostack.ui.notifications.config["conditions"][doctype];
		if (filters && $.isPlainObject(filters)) {
			if (!criscostack.route_options) {
				criscostack.route_options = {};
			}
			$.extend(criscostack.route_options, filters);
		}
		criscostack.set_route("List", doctype);
	},
};
...
class NotificationsView extends BaseNotificationsView {
	make() {
		...
		this.get_notifications_list(this.max_length).then((r) => {
			...
			criscostack.update_user_info(r.message.user_info);
	...
	mark_as_read(docname, $el) {
		criscostack
			.call("criscostack.desk.doctype.notification_log.notification_log.mark_as_read", {
				docname: docname,
			})
			.then(() => {
	...
	get_dropdown_item_html(notification_log) {
		...
		message = title
			? message.replace(title[1], criscostack.ellipsis(strip_html(title[1]), 100))
			: message;

		let timestamp = criscostack.datetime.comment_when(notification_log.creation);
		...
		let user_avatar = criscostack.avatar(user, "avatar-medium user-avatar");
			...
			if (this.dropdown_items.length) {
				...
				this.container.append(
					...
					<div class="text-center">
						<img src="/assets/criscostack/images/ui-states/notification-empty-state.svg" alt="Generic Empty State" class="null-state">
	...
	get_notifications_list(limit) {
		return criscostack.call(
			"criscostack.desk.doctype.notification_log.notification_log.get_notification_logs",
			{ limit: limit }
		);
	}
	...
	get_item_link(notification_doc) {
		...
		return criscostack.utils.get_form_link(link_doctype, link_docname);
	}
	...
	toggle_seen(flag) {
		criscostack.call(
			"criscostack.desk.doctype.notification_settings.notification_settings.set_seen_value",
			{
				value: cint(flag),
	...
	setup_notification_listeners() {
		criscostack.realtime.on("notification", () => {
			this.toggle_notification_icon(false);
			this.update_dropdown();
		});

		criscostack.realtime.on("indicator_hide", () => {
		...
		this.parent.on("show.bs.dropdown", () => {
			...
			if (this.notifications_icon.find(".notifications-unseen").is(":visible")) {
				this.toggle_notification_icon(true);
				criscostack.call(
					"criscostack.desk.doctype.notification_log.notification_log.trigger_indicator_hide"
				);
			}
		});
	}
...
class EventsView extends BaseNotificationsView {
	make() {
		let today = criscostack.datetime.get_today();
		criscostack
			.xcall("criscostack.desk.doctype.event.event.get_events", {
				start: today,
				end: today,
			})
			.then((event_list) => {
	...
	render_events_html(event_list) {
		...
		if (event_list.length) {
			let get_event_html = (event) => {
				let time = __("All Day");
				if (!event.all_day) {
					let start_time = criscostack.datetime.get_time(event.starts_on);
					let days_diff = criscostack.datetime.get_day_diff(event.ends_on, event.starts_on);
					let end_time = criscostack.datetime.get_time(event.ends_on);
					if (days_diff > 1) {
				...
				if (event.particpants) {
					particpants = criscostack.avatar_group(event.particpants, 3);
				}
			...
			html = `
				<div class="notification-null-state">
					<div class="text-center">
					<img src="/assets/criscostack/images/ui-states/event-empty-state.svg" alt="Generic Empty State" class="null-state">
