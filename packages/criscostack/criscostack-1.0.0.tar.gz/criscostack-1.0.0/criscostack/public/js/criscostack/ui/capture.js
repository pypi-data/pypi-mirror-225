// criscostack.ui.Capture
// Author - Achilles Rasquinha <achilles@criscostack.io>
 ...
 *
 * @example
 * criscostack._.get_data_uri(video)
 * // returns "data:image/pngbase64,..."
 */
criscostack._.get_data_uri = (element) => {
...
/**
 * @description Crisco's Capture object.
 *
 * @example
 * const capture = criscostack.ui.Capture()
 * capture.show()
 *
 ...
 */
criscostack.ui.Capture = class {
	constructor(options = {}) {
		this.options = criscostack.ui.Capture.OPTIONS;
	...
	set_options(options) {
		this.options = { ...criscostack.ui.Capture.OPTIONS, ...options };
	...
	show() {
		...
		if (cint(criscostack.boot.sysdefaults.force_web_capture_mode_for_uploads)) {
			this.show_for_desktop();
		} else if (criscostack.is_mobile()) {
	...
	build_dialog() {
		...
		me.$template = $(criscostack.ui.Capture.TEMPLATE);
	...
	show_for_desktop() {
		...
		this.render_stream()
			...
			.catch((err) => {
				if (me.options.error) {
					criscostack.show_alert(criscostack.ui.Capture.ERR_MESSAGE, 3);
				}
			...
			images += `
				<div class="mt-1 p-1 rounded col-md-3 col-sm-4 col-xs-4" data-idx="${idx}">
					<span class="capture-remove-btn" data-idx="${idx}">
						${criscostack.utils.icon("close", "lg")}
					</span>
	...
	setup_take_photo_action() {
		...
		this.dialog.set_primary_action(__("Take Photo"), () => {
			const data_url = criscostack._.get_data_uri(me.video);
...
criscostack.ui.Capture.OPTIONS = {
	title: __("Camera"),
	animate: false,
	error: false,
};
criscostack.ui.Capture.ERR_MESSAGE = __("Unable to load camera.");
criscostack.ui.Capture.TEMPLATE = `
<div class="criscostack-capture">
	<div class="embed-responsive embed-responsive-16by9 fc-stream-container">
		<video class="fc-stream embed-responsive-item">${criscostack.ui.Capture.ERR_MESSAGE}</video>
	</div>
