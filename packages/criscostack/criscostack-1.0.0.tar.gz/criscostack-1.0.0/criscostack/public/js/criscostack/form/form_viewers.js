criscostack.ui.form.FormViewers = class FormViewers {
	...
	refresh() {
		...
		let avatar_group = criscostack.avatar_group(this.active_users, 5, {
			align: "left",
			overlap: true,
		});
	...
	setup_events() {
		let me = this;
		criscostack.realtime.off("doc_viewers");
		criscostack.realtime.on("doc_viewers", function (data) {
			me.update_users(data);
		});
	}

	async update_users({ doctype, docname, users = [] }) {
		users = users.filter((u) => u != criscostack.session.user);
	...
	async fetch_user_info(users) {
		...
		for (let user of users) {
			if (!criscostack.boot.user_info[user]) unknown_users.push(user);
		}
		if (!unknown_users.length) return;

		const data = await criscostack.xcall("criscostack.desk.form.load.get_user_info_for_viewers", {
			users: unknown_users,
		});
		Object.assign(criscostack.boot.user_info, data);
	}
};
