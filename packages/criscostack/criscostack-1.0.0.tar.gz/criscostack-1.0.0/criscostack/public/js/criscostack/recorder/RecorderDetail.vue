...
let requests = ref([]);
let page = criscostack.pages["recorder"].page;
...
// Started
criscostack.recorder.router = router.value;
...
function refresh() {
	criscostack.call("criscostack.recorder.get").then( r => requests.value = r.message);
}
function update(message) {
	requests.value.push(JSON.parse(message));
}
function clear() {
	criscostack.call("criscostack.recorder.delete").then(r => refresh());
}
function start() {
	criscostack.call("criscostack.recorder.start").then(r => fetch_status());
}
function stop() {
	criscostack.call("criscostack.recorder.stop").then(r => fetch_status());
}
function fetch_status() {
	criscostack.call("criscostack.recorder.status").then(r => update_status(r.message));
}
function update_status(result) {
	...
	if(status.value.status == "Active") {
		criscostack.realtime.on("recorder-dump-event", update);
	} else {
		criscostack.realtime.off("recorder-dump-event", update);
	}
...
function export_data() {
	...
	criscostack.call("criscostack.recorder.export_data")
		.then((r) => {
...
onMounted(() => {
	...
	page.set_secondary_action(__("Clear"), () => {
		criscostack.set_route("recorder");
		clear();
	});
