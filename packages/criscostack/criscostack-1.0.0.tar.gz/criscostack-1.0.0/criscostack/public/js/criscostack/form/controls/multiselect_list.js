criscostack.ui.form.ControlMultiSelectList = class ControlMultiSelectList extends (
	criscostack.ui.form.ControlData
) {
	...
	set_selectable_items(options) {
		let html = options
			.map((option) => {
				...
				<div class="multiselect-check">${criscostack.utils.icon("tick", "xs")}</div>
			</li>`;
			})
			.join("");
		if (!html) {
