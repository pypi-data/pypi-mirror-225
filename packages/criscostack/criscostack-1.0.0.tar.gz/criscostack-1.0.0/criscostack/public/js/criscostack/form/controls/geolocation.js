...
criscostack.ui.form.ControlGeolocation = class ControlGeolocation extends criscostack.ui.form.ControlData {
	...
	set_disp_area(value) {
		...
		if (this.frm) {
			...
			$(document).on("criscostack.ui.Dialog:shown", () => {
				this.make_map();
			});
		}
	}
	...
	bind_leaflet_map() {
		...
		this.map.setView(criscostack.utils.map_defaults.center, criscostack.utils.map_defaults.zoom);
	...
	bind_leaflet_draw_control() {
		if (
			!criscostack.perm.has_perm(this.doctype, this.df.permlevel, "write", this.doc) ||
			this.df.read_only
		) {
			return;
		}
	...
	get_leaflet_controls() {
		return new L.Control.Draw({
			...
			draw: {
				polyline: {
					shapeOptions: {
						color: criscostack.ui.color.get("blue"),
						weight: 10,
					},
				},
				polygon: {
					...
					drawError: {
						color: criscostack.ui.color.get("orange"), // Color the shape will turn when intersects
