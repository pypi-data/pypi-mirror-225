criscostack.ui.form.Control = class BaseControl {
	constructor(opts) {
		$.extend(this, opts);
		this.make();
	...
	make_wrapper() {
		this.$wrapper = $("<div class='criscostack-control'></div>").appendTo(this.parent);
	...
	get_status(explain) {
		...
		var status = criscostack.perm.get_field_display_status(
			this.df,
			criscostack.model.get_doc(this.doctype, this.docname),
		...
		let value = criscostack.model.get_value(this.doctype, this.docname, this.df.fieldname);
	...
	show_translatable_button(value) {
		...
		if (
			...
			!criscostack.model.can_write("Translation") ||
			!value
		)
			return;

		// Disable translation in website
		if (!criscostack.views || !criscostack.views.TranslationManager) return;
		...
		$(translation_btn)
			...
			.on("click", () => {
				if (!this.doc.__islocal) {
					new criscostack.views.TranslationManager({
						df: this.df,
