...
criscostack.provide("criscostack.realtime");
...
class RealTimeClient {
	...
	init(port = 9000, lazy_connect = false) {
		if (criscostack.boot.disable_async) {
			return;
		}
	...
	get_host(port = 3000) {
		...
		if (window.dev_server) {
			...
			port = criscostack.boot.socketio_port || port.toString() || "3000";
			if (parts.length > 2) {
				host = parts[0] + ":" + parts[1];
			}
			host = host + ":" + port;
		}
		return host + `/${criscostack.boot.sitename}`;
	}
	...
	doc_subscribe(doctype, docname) {
		if (criscostack.flags.doc_subscribe) {
		...
		criscostack.flags.doc_subscribe = true;

		// throttle to 1 per sec
		setTimeout(function () {
			criscostack.flags.doc_subscribe = false;
		}, 1000);
	...
	process_response(data, method) {
		...
		// "callback" is std criscostack term
		if (method === "success") {
...
criscostack.realtime = new RealTimeClient();

// backward compatbility
criscostack.socketio = criscostack.realtime;
