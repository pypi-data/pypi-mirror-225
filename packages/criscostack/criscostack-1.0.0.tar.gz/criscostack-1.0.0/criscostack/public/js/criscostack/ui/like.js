...
criscostack.ui.is_liked = function (doc) {
	var liked = criscostack.ui.get_liked_by(doc);
	return liked.indexOf(criscostack.session.user) === -1 ? false : true;
};

criscostack.ui.get_liked_by = function (doc) {
...
criscostack.ui.toggle_like = function ($btn, doctype, name, callback) {
	var add = $btn.hasClass("not-liked") ? "Yes" : "No";
	// disable click
	$btn.css("pointer-events", "none");

	criscostack.call({
		method: "criscostack.desk.like.toggle_like",
		quiet: true,
		args: {
			doctype: doctype,
			name: name,
			...
			if (!r.exc) {
				...
				if (doc) {
					var liked_by = JSON.parse(doc._liked_by || "[]"),
						idx = liked_by.indexOf(criscostack.session.user);
					if (add === "Yes") {
						if (idx === -1) liked_by.push(criscostack.session.user);
					} else {
						if (idx !== -1) {
...
criscostack.ui.click_toggle_like = function () {
	var $btn = $(this);
	...
	criscostack.ui.toggle_like($btn, doctype, name, function () {
		if (not_liked) {
...
criscostack.ui.setup_like_popover = ($parent, selector) => {
	if (criscostack.dom.is_touchscreen()) {
		return;
	}
			...
			content: () => {
				...
				liked_by.forEach((user) => {
					...
					liked_by_list.append(`
						<li data-user=${user}>${criscostack.avatar(user, "avatar-xs")}
							<span>${criscostack.user.full_name(user)}</span>
						</li>
					`);
				});
				...
				liked_by_list.children("li").click((ev) => {
					...
					criscostack.set_route(link_base + user);
				});
