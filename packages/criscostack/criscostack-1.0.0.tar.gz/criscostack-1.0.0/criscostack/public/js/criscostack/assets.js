...
// depends on criscostack.versions to manage versioning

criscostack.require = function (items, callback) {
	if (typeof items === "string") {
		items = [items];
	}
	items = items.map((item) => criscostack.assets.bundled_asset(item));

	return new Promise((resolve) => {
		criscostack.assets.execute(items, () => {
			resolve();
			callback && callback();
		});
	});
...
criscostack.assets = {
	...
	init_local_storage: function () {
		...
		if (criscostack.boot) localStorage.metadata_version = criscostack.boot.metadata_version;
	},
	...
	execute: function (items, callback) {
		...
		for (var i = 0, l = items.length; i < l; i++) {
			if (!criscostack.assets.exists(items[i])) {
				to_fetch.push(items[i]);
			}
		}
		if (to_fetch.length) {
			criscostack.assets.fetch(to_fetch, function () {
				criscostack.assets.eval_assets(items, callback);
			});
		} else {
			criscostack.assets.eval_assets(items, callback);
		}
	...
	eval_assets: function (items, callback) {
		for (var i = 0, l = items.length; i < l; i++) {
			...
			var path = items[i];
			if (criscostack.assets.executed_.indexOf(path) === -1) {
				// execute
				criscostack.assets.handler[criscostack.assets.extn(path)](criscostack.assets.get(path), path);
				criscostack.assets.executed_.push(path);
			}
		}
		callback && callback();
	},
	...
	// localstorage
	exists: function (src) {
		if (criscostack.assets.executed_.indexOf(src) !== -1) {
			return true;
		}
		if (criscostack.boot.developer_mode) {
			return false;
		}
		if (criscostack.assets.get(src)) {
			return true;
		} else {
			return false;
		}
	...
	fetch: function (items, callback) {
		...
		criscostack.call({
			type: "GET",
			method: "criscostack.client.get_js",
			args: {
				items: items,
			},
			callback: function (r) {
				$.each(items, function (i, src) {
					criscostack.assets.add(src, r.message[i]);
				});
				callback();
			},
			freeze: true,
	...
	add: function (src, txt) {
		if ("localStorage" in window) {
			try {
				criscostack.assets.set(src, txt);
			} catch (e) {
	...
	handler: {
		js: function (txt, src) {
			criscostack.dom.eval(txt);
		},
		css: function (txt, src) {
			criscostack.dom.set_style(txt);
		},
	},
