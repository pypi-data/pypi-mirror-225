...
criscostack.provide("criscostack.interaction_settings");

criscostack.views.InteractionComposer = class InteractionComposer {
	constructor(opts) {
		$.extend(this, opts);
		this.make();
	}
	...
	get_event_categories() {
		let me = this;
		criscostack.model.with_doctype("Event", () => {
			let categories = criscostack.meta
	...
	setup_attach() {
		...
		attach.find(".add-more-attachments a").on("click", () => new criscostack.ui.FileUploader(args));
		this.render_attach();
	}
	...
	create_interaction(btn, form_values, selected_attachments) {
		...
		if (!("owner" in interaction_values)) {
			interaction_values["owner"] = criscostack.session.user;
		}
		if (!("assigned_by" in interaction_values) && interaction_values["doctype"] == "ToDo") {
			interaction_values["assigned_by"] = criscostack.session.user;
		}
	...
	add_attachments(doc, attachments) {
		criscostack.call({
			method: "criscostack.utils.file_manager.add_attachments",
			args: {
				doctype: doc.doctype,
				name: doc.name,
