export default class BulkOperations {
	constructor({ doctype }) {
		if (!doctype) criscostack.throw(__("Doctype required"));
	...
	print(docs) {
		...
		const is_submittable = criscostack.model.is_submittable(this.doctype);
		...
		if (invalid_docs.length > 0) {
			criscostack.msgprint(__("You selected Draft or Cancelled documents"));
			return;
		}

		if (valid_docs.length === 0) {
			criscostack.msgprint(__("Select atleast 1 record for printing"));
			return;
		}
		...
		const dialog = new criscostack.ui.Dialog({
			...
			fields: [
				...
				},
				{
					...
					fieldname: "print_sel",
					options: criscostack.meta.get_print_formats(this.doctype),
					default: criscostack.get_meta(this.doctype).default_print_format,
				},
				{
					fieldtype: "Select",
					label: __("Page Size"),
					fieldname: "page_size",
					options: criscostack.meta.get_print_sizes(),
		...
		dialog.set_primary_action(__("Print"), (args) => {
			if (!args) return;
			const default_print_format = criscostack.get_meta(this.doctype).default_print_format;
			...
			if (args.page_size === "Custom") {
				if (args.page_height === 0 || args.page_width === 0) {
					criscostack.throw(__("Page height and width cannot be zero"));
				}
			...
			const w = window.open(
				"/api/method/criscostack.utils.print_format.download_multi_pdf?" +
					"doctype=" +
	...
	get_letterhead_options() {
		...
		criscostack.call({
			method: "criscostack.client.get_list",
			args: {
				doctype: "Letter Head",
	...
	delete(docnames, done = null) {
		criscostack
			.call({
				method: "criscostack.desk.reportview.delete_items",
				freeze: true,
				freeze_message:
					docnames.length <= 10
	...
	assign(docnames, done) {
		if (docnames.length > 0) {
			const assign_to = new criscostack.ui.form.AssignToDialog({
				obj: this,
				method: "criscostack.desk.form.assign_to.add_multiple",
				doctype: this.doctype,
				docname: docnames,
				bulk_assign: true,
				re_assign: true,
			...
			criscostack.msgprint(__("Select records for assignment"));
		}
	}
	...
	apply_assignment_rule(docnames, done) {
		if (docnames.length > 0) {
			criscostack
				.call("criscostack.automation.doctype.assignment_rule.assignment_rule.bulk_apply", {
					doctype: this.doctype,
					docnames: docnames,
				})
				.then(() => done());
	...
	submit_or_cancel(docnames, action = "submit", done = null) {
		...
		criscostack
			.call({
				method: "criscostack.desk.doctype.bulk_update.bulk_update.submit_cancel_or_update_docs",
				args: {
					doctype: this.doctype,
					action: action,
					docnames: docnames,
			...
			.then((r) => {
				...
				if (failed.length < docnames.length) {
					criscostack.utils.play_sound(action);
					if (done) done();
				}
			});
	}
			...
			primary_action: ({ value }) => {
				...
				criscostack
					...
					.then((r) => {
						...
						dialog.hide();
						criscostack.show_alert(__("Updated successfully"));
					});
			},
		...
		const dialog = new criscostack.ui.Dialog({
			...
			fields: [
				{
					...
					get_data: function (txt) {
						return criscostack.db.get_link_options("Tag", txt);
					},
				},
			],
			...
			primary_action: () => {
				...
				if (args && args.tags) {
					...
					criscostack.call({
						method: "criscostack.desk.doctype.tag.tag.add_tags",
						args: {
							tags: args.tags,
							dt: this.doctype,
							docs: docnames,
	...
	export(doctype, docnames) {
		criscostack.require("data_import_tools.bundle.js", () => {
			const data_exporter = new criscostack.data_import.DataExporter(
				doctype,
				"Insert New Records"
			);
