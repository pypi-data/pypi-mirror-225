...
from .mixins import Commentable, CommentField
...
class Comment(CreatedModified, UserAuthored, Controllable, ElasticSearchable,
              ChangeNotifier, Previewable, Publishable, DateRangeObservable, MemoReferrable):
    ...
    def as_story_item(self, ar):
        header = [ar.obj2html(self, self.created_natural)]
        header.append(str(_(" by ")))
        header.append(ar.obj2html(self.user))
        header.append(E.span(":"))
        elems = [E.h3(*header)]
        try:
        ...
        if self.replies_to_this.count():
            childs.append(E.h4("Replies:"))
            for child in self.replies_to_this.all():
                childs.append(fromstring(child.as_story_item(ar)))

        elems.append(E.div(full_preview_elem, *childs, style="margin-left: 40px;"))
