...
class ExtRenderer(JsCacheRenderer):
    ...
    def action_call(self, ar, bound_action, status):
        ...
        # roger or lydia).
        return "{}()".format(
            self.get_panel_btn_handler(bound_action, status))
    ...
    def js_render_ParamsPanelSubclass(self, dh):
        ...
        if lc == 0:
            ...
            # raise Exception(msg)
            print((20150717, msg))
        fields = [e for e in dh.main.walk()
            if isinstance(e, ext_elems.FieldElement)]
    ...
    def js_render_detail_action_FormPanel(self, rh, ba):
        ...
        def doit(dtl):
            ...
            yield "Lino.%sPanel = Ext.extend(Lino.%s,{" % (
                ba.full_name(), dtl._formpanel_name)
    ...
    def js_render_GridPanel_class(self, rh):
        ...
        kw.update(ls_bbar_actions=self.toolbar(
            rh.actor.get_toolbar_actions(rh.actor.default_action.action, get_user_profile())))
...
'</td>')""" % (rh.actor.row_height * 11)
    ...
    def goto_instance(self, ar, obj, detail_action=None, **kw):
        ...
        <https://docs.djangoproject.com/en/4.1/topics/http/file-uploads/>`_
