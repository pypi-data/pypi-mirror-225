...
class Kernel(object):
    ...
    def kernel_startup(self, site):
        ...
        for a in actors.actors_list:
            ...
            a._actions_list = sorted(a._actions_list, key=lambda ba: ba.action.sort_index)
    ...
    def run_action(self, ar):
        ...
        try:
            a.run_from_ui(ar)
            if ar.request.method != 'GET' and ar.master_instance is not None and len(ar.selected_rows):
    ...
    def check_virgin(self):
        ...
        if not fn.exists():
            raise Exception("No `dbhash.json` in {} "
                "(did you run `django-admin prep`?)".format(self.site.site_dir))
...
def site_startup(self):
    ...
    with startup_rlock:
        ...
        # this issue bu causes other problems:
        if False:
            for res in actors.actors_list:
