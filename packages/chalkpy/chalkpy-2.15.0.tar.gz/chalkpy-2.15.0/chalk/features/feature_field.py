...
"""
    ...
    def _validate_filter(self, filter: Filter):
        ...
        conditions = (filter.lhs.namespace == self.namespace, filter.rhs.namespace == self.namespace)
        if sum(conditions) != 1:
            raise TypeError(
                f"Join function for {self.root_fqn} is incorrectly configured. "
                f"The feature '{self.fqn}' joins '{filter.lhs.fqn}' and '{filter.rhs.fqn}'. "
                f"Exactly one of the features in the join must be of the same namespace as the has-one feature. "
                f"Ensure that the join is a lambda function that sets features from two feature classes as equal."
            )
