...
from the command line from '{os.getcwd()}'. If you are still having trouble, please contact Chalk support.""",
    ...
    def test_streaming_resolver(
        ...
        request = ResolverTestRequest(resolver_fqn=resolver_fqn, num_messages=num_messages, test_messages=payloads)
        result = self._request(
            method="POST",
    ...
    def _validate_test_stream_resolver_inputs(
        ...
        message_bodies: Optional[List[Union[str, bytes]]] = None,
        message_timestamps: Optional[List[Union[str, datetime]]] = None,
    ) -> List[ResolverTestMessagePayload]:
            ...
            with open(message_filepath) as file:
                for i, line in enumerate(file):
                    try:
                        ...
                        message_keys.append(json_message["key"])
        ...
        payloads: List[ResolverTestMessagePayload] = [
            ResolverTestMessagePayload(
                key=message_keys[i],
                ...
                timestamp=timestamp_datetimes[i] if message_timestamps is not None else None,
            )
