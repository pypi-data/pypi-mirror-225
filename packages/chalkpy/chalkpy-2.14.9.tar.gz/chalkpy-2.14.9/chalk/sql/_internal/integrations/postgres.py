    ...
    def _get_parsed_table_via_polars(
        ...
        for col_name, converter in converters.items():
            ...
            if dtype == pl.Boolean:
                ...
                # Parse using the most lenient option,
                # which means we omit specifying a dtype
                # here since most of the time it's too
                # strict. Instead, we depend on
                #   `pl.read_csv(..., try_parse_dates=True)`
                # for parsing date and datetime columns.
                #
                # Later, we convert the column to the
                # correct strict dtype in pyarrow.
                continue
            else:
        ...
        pl_table = pl.read_csv(buffer, dtypes=parse_dtypes, try_parse_dates=True)
        if boolean_mappings:
