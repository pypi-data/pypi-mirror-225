...
def parse_function(
    ...
    if ret_val is None and not ignore_return:
        raise ValueError(
            f"Resolver {fqn} does not specify a return type. Please add an annotation (like `-> User.first_name`) to the resolver."
        )
...
def parse_and_register_stream_resolver(
    *,
    ...
    for resolver in StreamResolver.registry:
        if resolver.source == source and resolver.windowed_pseudofeatures:
