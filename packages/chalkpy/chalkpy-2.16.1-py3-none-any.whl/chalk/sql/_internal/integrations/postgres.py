...
class PostgreSQLSourceImpl(BaseSQLSource, TableIngestMixIn, SQLSourceWithTableIngestProtocol):
    ...
    def _get_parsed_table_via_polars(
        ...
        _logger.info(f"SEGFAULT DEBUG: get result csv buffer")
        buffer = self._get_result_csv_buffer(finalized_query, connection=connection, escape_char='"')
        _logger.info(f"SEGFAULT DEBUG: get result csv buffer done")

        _logger.info(f"SEGFAULT DEBUG: readline")
        first_line = buffer.readline()

        _logger.info(f"SEGFAULT DEBUG: python csv reader")
        column_names = list(csv.reader([first_line.decode("utf8")], doublequote=True, quoting=csv.QUOTE_MINIMAL))[0]

        _logger.info(f"SEGFAULT DEBUG: seek")
        buffer.seek(0)

        _logger.info(f"SEGFAULT DEBUG: columns_to_converters")
        converters = columns_to_converters(column_names)

        _logger.info(f"SEGFAULT DEBUG[{','.join(column_names)}]: init")
        parse_dtypes = {}
        boolean_columns = []
        ...
        _logger.info(f"SEGFAULT DEBUG: pl.read_csv")
        pl_table = pl.read_csv(
            buffer,
            dtypes=parse_dtypes,
            use_pyarrow=env_var_bool("USE_POLARS_READ_CSV_PYARROW_UNDERNEATH", default=False),
        )
        _logger.info(f"SEGFAULT DEBUG: pl.read_csv done")
        if boolean_columns:
            enum = os.environ.get("POLARS_BOOLEAN_MAPPER_TYPE_ENUM") or "1"
            if enum == "1":
                boolean_when_mappings = [
                    pl.when(pl.col(bool_col).is_null()).then(None).otherwise(pl.col(bool_col) == "t")
                    for bool_col in boolean_columns
                ]
                _logger.info(f"SEGFAULT DEBUG: boolean when map")
                pl_table = pl_table.with_columns(*boolean_when_mappings)
                _logger.info(f"SEGFAULT DEBUG: boolean when done")
            elif enum == "2":
                _logger.info(f"SEGFAULT DEBUG: boolean one by one mapping")
                for i, bool_col in enumerate(boolean_columns):
                    _logger.info(f"SEGFAULT DEBUG: boolean one by one mapping iter {i}")
                    pl_table = pl_table.with_columns(
                        pl.col(bool_col).map_dict({"f": False, "t": True}, return_dtype=pl.Boolean)
                    )
                    _logger.info(f"SEGFAULT DEBUG: boolean one by one mapping iter {i} done")
                _logger.info(f"SEGFAULT DEBUG: boolean one by one done")
            else:
                boolean_mappings = [
                    pl.col(bool_col).map_dict({"f": False, "t": True}, return_dtype=pl.Boolean)
                    for bool_col in boolean_columns
                ]
                _logger.info(f"SEGFAULT DEBUG: boolean mapping")
                pl_table = pl_table.with_columns(*boolean_mappings)
                _logger.info(f"SEGFAULT DEBUG: boolean done")
        if datetime_col_to_dtype:
            for dt_col, dt_type in datetime_col_to_dtype.items():
                _logger.info(f"SEGFAULT DEBUG[{dt_col}]: null_count")
                num_nulls_original = pl_table[dt_col].null_count()
                _logger.info(f"SEGFAULT DEBUG[{dt_col}]: null_count done")
                parsing_complete = lambda table: table[dt_col].null_count() == num_nulls_original
                _logger.info(f"SEGFAULT DEBUG[{dt_col}]: clone")
                original_dt_clone = pl_table[dt_col].clone()
                _logger.info(f"SEGFAULT DEBUG[{dt_col}]: clone done")
                unit_to_precision = {
                    "ms": "%.3f",
                    "us": "%.6f",
                    "ns": "%.9f",
                ...
                _logger.info(f"SEGFAULT DEBUG[{dt_col}]: strptime 1")
                formatted_col = original_dt_clone.str.strptime(
                    pl.Datetime, fmt=datetime_parser_formats.popleft(), strict=False
                ).alias(dt_col)
                _logger.info(f"SEGFAULT DEBUG[{dt_col}]: strptime 1 done")

                _logger.info(f"SEGFAULT DEBUG[{dt_col}]: with_columns 1")
                pl_table = pl_table.with_columns(formatted_col)
                _logger.info(f"SEGFAULT DEBUG[{dt_col}]: with_columns 1 done")
                count = 2
                while not parsing_complete(pl_table) and len(datetime_parser_formats) > 0:
                    fmt = datetime_parser_formats.popleft()
                    _logger.info(f"SEGFAULT DEBUG[{dt_col}]: strptime {count}")
                    formatted_col = original_dt_clone.str.strptime(pl.Datetime, fmt=fmt, strict=False).alias(dt_col)
                    _logger.info(f"SEGFAULT DEBUG[{dt_col}]: strptime {count} done")

                    _logger.info(f"SEGFAULT DEBUG[{dt_col}]: coalesce {count}")
                    coalesced_col = pl.coalesce(pl.col(dt_col), formatted_col)
                    _logger.info(f"SEGFAULT DEBUG[{dt_col}]: coalesce {count} done")

                    _logger.info(f"SEGFAULT DEBUG[{dt_col}]: with_columns {count}")
                    pl_table = pl_table.with_columns(coalesced_col)
                    _logger.info(f"SEGFAULT DEBUG[{dt_col}]: with_columns {count} done")
                    count += 1

                _logger.info(f"SEGFAULT DEBUG[{dt_col}]: parse complete check")
                is_parsing_complete = parsing_complete(pl_table)
                _logger.info(f"SEGFAULT DEBUG[{dt_col}]: parse complete check done")
        ...
        _logger.info(f"SEGFAULT DEBUG: to_arrow")
        parsed_table = pl_table.to_arrow()
        _logger.info(f"SEGFAULT DEBUG: to_arrow done")
        ...
        _logger.info(f"SEGFAULT DEBUG: cast")
        cast_table = parsed_table.select([c for c in converters.keys()]).cast(restricted_schema)
        _logger.info(f"SEGFAULT DEBUG: cast done")
        return cast_table
