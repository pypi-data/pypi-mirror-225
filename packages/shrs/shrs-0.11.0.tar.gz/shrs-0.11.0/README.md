...
## Design probes
Specify the multi-FASTA file or the folder containing target sequences (FASTA or GenBank format) as input file(s) after the '-i' option. This algorithm `DP` designs probes that hybridize to all input sequences. Input sequence columns in result table indicate the annealing positions on the each input sequence.
