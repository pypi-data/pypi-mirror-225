...
def create_trigger_py(
    trigger_name: str, trigger_description: str, trigger_dir_name: str
):
    """
...
def create_triggers(spec: PluginSpecTypes.Spec, triggers_dir_name: str):
    ...
    # Delete any triggers which no longer exist in spec
    _delete_old(spec, triggers_dir_name, "Triggers")
...
def create_connection_py(connection_dir_name: str):
    """
    ...
    if not exists(output_file):
        create_file_from_template(
            ...
            inputs={"base_module": BASE_MODULE},
            output_filename=output_file,
        )
...
def create_connection(spec: PluginSpecTypes.Spec, connection_dir_name: str):
    ...
    create_connection_py(connection_dir_name)
...
def create_action_py(action_name: str, action_description: str, action_dir_name: str):
    """
...
def create_actions(spec: PluginSpecTypes.Spec, actions_dir_name: str):
    ...
    # Delete any actions which no longer exist in spec
    _delete_old(spec, actions_dir_name, "Actions")
...
def create_tasks(spec: PluginSpecTypes.Spec, tasks_dir_name: str):
    ...
    # Delete any tasks which no longer exist in spec
    _delete_old(spec, tasks_dir_name, "Tasks")
...
def create_files(
    spec: PluginSpecTypes.Spec, dir_name: str, prefix: str
):
    """
    ...
    :param prefix: The prefix, either icon or komand
    :param is_create: Boolean to indicate create or refresh
    :return:
    """
    ...
    # Create unit tests for each action under target subdirectory called unit_test
...
def handle_refresh_create(
    spec: PluginSpecTypes.Spec, target_dir_name: str, is_create: bool
):
    """
    ...
    create_files(spec=spec, dir_name=dir_name, prefix=prefix)


def _delete_old(spec: PluginSpecTypes.Spec, dir_name: str, a_t_t: str):
    """
    Scan directory and delete actions/triggers/tasks which no longer exist in spec.
    :param spec: Spec object
    :param dir_name: Action/trigger/task dir name
    :param a_t_t: Action, Trigger or Task
    :return:
    """
    if a_t_t == "Actions":
        a_t_t = Constants.ACTIONS
    elif a_t_t == "Triggers":
        a_t_t = Constants.TRIGGERS
    elif a_t_t == "Tasks":
        a_t_t = Constants.TASKS

    # Delete directories for actions which don't exist in the plugin spec yaml anymore
    all_names = os.listdir(dir_name)
    # Remove init.py from the search since we don't find it in the plugin.spec.yaml
    all_names.remove("__init__.py")

    for name in all_names:
        if name not in spec.get(a_t_t):
            if os.path.isdir(os.path.join(dir_name, name)):
                print(
                    f"\n{name} ({a_t_t}) not found in plugin.spec.yaml. {name} will be lost. Would you "
                    f"like to continue? Y to delete, N to preserve."
                )
                delete_files = str(input("[y/n] "))
