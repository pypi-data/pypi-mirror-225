...
class SchemaUtil:
    ...
    def build_inner(input_output_spec: dict, spec: dict, task: bool) -> dict:
        ...
        if not task:
            ...
            outer_object = SchemaUtil._build_task_schema(
                outer_object, properties, required
            )
    ...
    def _build_task_schema(
        ...
        :param required: The built out required list.
        :type: List[str]
        ...
        # Remove last entry in definitions
        type_key = None
        try:
            type_key = items.get("$ref").split("/")[-1]
        except AttributeError:
            pass
        try:
            outer_object[SchemaConstants.DEFINITIONS][type_key].get(PROPERTIES, {})[
                REQUIRED
            ] = required[1:]
        except KeyError:
            pass

        # Move requirements up a level within the dict
        try:
            outer_object[SchemaConstants.DEFINITIONS][type_key][REQUIRED] = (
                outer_object[SchemaConstants.DEFINITIONS][type_key]
                .get(PROPERTIES)
                .pop(REQUIRED)
            )
        except KeyError:
        ...
        for key, items in current_section.items():
            ...
            # This key may be a custom type or an array. Check that this type is defined in the original plugin spec.
                ...
                if current_type not in definitions:
                    ...
                    _, required = SchemaUtil.recurse_build(
                    ...
                    if len(required) > 0:
                        new_parent[SchemaConstants.REQUIRED] = required
