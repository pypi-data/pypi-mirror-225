...
class APIAnalysisVersion:
    ...
    async def create(self, analysis_id: str, analysis_manifest: AnalysisManifest) -> AnalysisVersion:
        """
        Given a top level analysis, create a new version, which must be semantically greater than the previously created
        version. Once an analysis version has been created, it is immutable. Any changes require a new version to be
    ...
    async def list(self, **kwargs) -> List[AnalysisConfig]:
        ...
        for key in kwargs.keys():
            ...
            if key == 'include_deactivated':
                request_fragments.append(AnalysisConfigListRequest(include_deactivated=kwargs[key]))
            else:
