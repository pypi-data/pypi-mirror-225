...
def python(client_type, network, tag, feature):
    ...
    try:
        ...
        load_feature_files()
        return_code = launch_behave(tag, feature)
...
def javascript(network, tag, feature, invalidate_cache):
    ...
    try:
        ...
        load_feature_files()
        return_code = launch_cucumber(tag, feature)
        unload_feature_files()
        set_to_project_root_wd()
        return return_code
    except Exception as e:
...
def launch_main_menu():
    ...
    if selection_text == "Rippled":
        clear_screen()
        launch_rippled_task_menu()
    elif selection_text == "Network":
        clear_screen()
        launch_network_task_menu()
    elif selection_text == "Sidechain":
        clear_screen()
        launch_sidechain_task_menu()
    elif selection_text == "Tests":
        clear_screen()
        launch_test_task_menu()
    elif selection_text == "Miscellaneous":
        clear_screen()
        launch_misc_task_menu()
...
def launch_network_task_menu():
    relaunch_wizard = True
    questions = [
        inquirer.List(
            "network_task_menu",
            message="Here you can find a list of network related tasks. Please use ↑ ↓ and ↵ keys to choose an option. Current Selection",
            choices=[
                "Start local-mainnet",
    ...
    selection_text = inquirer.prompt(questions, theme=GreenPassion())['network_task_menu']
    if selection_text == 'Start local-mainnet':
        print("\nIn order to '{}', run: 'claudia local-mainnet start'".format(
            selection_text))
        if get_confirmation("Are you sure you want to run this command?"):
            relaunch_wizard = False
            clear_screen()
        ...
        print("\nIn order to '{}', run: 'claudia local-mainnet status'".format(selection_text))
        if get_confirmation("Are you sure you want to run this command?"):
            relaunch_wizard = False
            clear_screen()
            print("{}:\n".format(selection_text))
        ...
        print("\nIn order to '{}', run: 'claudia local-mainnet stop'".format(selection_text))
        if get_confirmation("Are you sure you want to run this command?"):
            relaunch_wizard = False
            clear_screen()
            print("{}:\n".format(selection_text))
...
def launch_sidechain_task_menu():
    relaunch_wizard = True
    questions = [
        inquirer.List(
            "sidechain_task_menu",
            message="Here you can find a list of sidechain related tasks. Please use ↑ ↓ and ↵ keys to choose an option. Current Selection",
            choices=[
    ...
    selection_text = inquirer.prompt(questions, theme=GreenPassion())['sidechain_task_menu']
    if selection_text == 'Build witness server':
        print("\nIn order to '{}', run: 'claudia witness build --repo <absolute_path_to_local_repo>'".format(
            selection_text))
        if get_confirmation("Are you sure you want to run this command?"):
            relaunch_wizard = False
            repo_path = get_valid_repo_path()
        ...
        print("\nIn order to '{}', run: 'claudia witness status'".format(selection_text))
        if get_confirmation("Are you sure you want to run this command?"):
            relaunch_wizard = False
            clear_screen()
            print("{}:\n".format(selection_text))
        ...
        print("\nIn order to '{}', run: 'claudia local-sidechain start'".format(selection_text))
        if get_confirmation("Are you sure you want to run this command?"):
            relaunch_wizard = False
            clear_screen()
            print("{}:\n".format(selection_text))
        ...
        print("\nIn order to '{}', run: 'claudia local-sidechain stop'".format(selection_text))
        if get_confirmation("Are you sure you want to run this command?"):
            relaunch_wizard = False
            clear_screen()
            print("{}:\n".format(selection_text))
...
def launch_test_task_menu():
    ...
    if selection_text == 'Run e2e tests':
...
def launch_misc_task_menu():
    relaunch_wizard = True
    questions = [
        inquirer.List(
            "misc_task_menu",
            message="Here you can find a list of miscelleneous tasks. Please use ↑ ↓ and ↵ keys to choose an option. Current Selection",
            choices=[
                "List e2e features",
    ...
    if selection_text == 'List e2e features':
        ...
        print("\nIn order to '{}', run: 'claudia set-install-mode build'".format(selection_text))
        if get_confirmation("Are you sure you want to run this command?"):
            relaunch_wizard = False
            clear_screen()
            print("{}:\n".format(selection_text))
        ...
        print("\nIn order to '{}', run: 'claudia set-install-mode install'".format(selection_text))
        if get_confirmation("Are you sure you want to run this command?"):
            relaunch_wizard = False
            clear_screen()
            print("{}:\n".format(selection_text))
        ...
        print("\nIn order to '{}', run: 'claudia enable-feature --feature <feature_name>'".format(selection_text))
        if get_confirmation("Are you sure you want to run this command?"):
            relaunch_wizard = False
            clear_screen()
            print("{}:\n".format(selection_text))
        ...
        print("\nIn order to '{}', run: 'claudia disable-feature --feature <feature_name>'".format(selection_text))
        if get_confirmation("Are you sure you want to run this command?"):
            relaunch_wizard = False
            clear_screen()
            print("{}:\n".format(selection_text))
        ...
        print("\nIn order to '{}', run: 'claudia clean'".format(selection_text))
        if get_confirmation("Are you sure you want to run this command?"):
            relaunch_wizard = False
            clear_screen()
            print("{}:\n".format(selection_text))
...
def launch_command_clean(ask_confirmation=True):
    ...
    set_to_project_root_wd()
    command = "sh network_setup/setup.sh --clean"
    return subprocess.call(command, shell=True)


@main.command()
@click.option('--feature', required=True, help="Rippled feature name")
...
def launch_command_enable_feature(feature):
    set_to_project_root_wd()
    command = "sh network_setup/setup.sh --enableFeature {}".format(feature)
    return subprocess.call(command, shell=True)
...
def launch_command_disable_feature(feature):
    set_to_project_root_wd()
    command = "sh network_setup/setup.sh --disableFeature {}".format(feature)
    return subprocess.call(command, shell=True)
...
def launch_command_sidechain_start():
    set_to_project_root_wd()
    command = "sh network_setup/setup.sh --sidechainStart"
    return subprocess.call(command, shell=True)
...
def launch_command_sidechain_stop():
    set_to_project_root_wd()
    command = "sh network_setup/setup.sh --sidechainStop"
    return subprocess.call(command, shell=True)
...
def launch_command_witness_build(repo):
    ...
    set_to_project_root_wd()
    command = "sh network_setup/setup.sh --witnessBuild --repo {}".format(repo)
    return subprocess.call(command, shell=True)
...
def launch_command_witness_status():
    set_to_project_root_wd()
    command = "sh network_setup/setup.sh --witnessStatus"
    return subprocess.call(command, shell=True)
...
def launch_command_set_install_mode_to_build():
    set_to_project_root_wd()
    command = "sh network_setup/setup.sh --setInstallMode build"
    return subprocess.call(command, shell=True)
...
def launch_command_set_install_mode_to_install():
    set_to_project_root_wd()
    command = "sh network_setup/setup.sh --setInstallMode install"
    return subprocess.call(command, shell=True)
...
def launch_command_rippled_build(repo):
    ...
    if not os.path.isabs(repo) or not os.path.exists(repo):
        ...
        return
    set_to_project_root_wd()
    command = "sh network_setup/setup.sh --rippledBuild --repo {}".format(repo)
    return subprocess.call(command, shell=True)
...
def launch_command_rippled_install():
    set_to_project_root_wd()
    command = "sh network_setup/setup.sh --rippledInstall"
    return subprocess.call(command, shell=True)
...
def launch_command_network_start():
    set_to_project_root_wd()
    command = "sh ./network_setup/setup.sh --networkStart"
    return subprocess.call(command, shell=True)
