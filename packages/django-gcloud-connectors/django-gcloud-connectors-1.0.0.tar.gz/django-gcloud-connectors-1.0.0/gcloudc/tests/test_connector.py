...
def set_test_many_fields_model_field(model, field_name, field_value):
    setattr(model, field_name, field_value)
    model.save(update_fields=[field_name])
...
class BackendTests(TestCase):
    ...
    def test_update_atomic(self):
        """
        Test concurrent updates are atomic.
        """
        properties_to_set = {
            "field1": "f1_val",
            "field2": "f2_val",
        ...
        thread_property_map = {}
        original_fruit = ManyFields.objects.create()
...
class NamespaceTests(TestCase):
    multi_db = True
    databases = (
        "default", "nonamespace", "non_default_connection"
    )

    @skipIf("non_default_connection" not in settings.DATABASES,
            "This test is designed for the Djangae testapp settings")
    def test_database_specific_namespaces(self):
        TestFruit.objects.create(name="Apple", color="Red")
        TestFruit.objects.create(name="Orange", color="Orange")

        TestFruit.objects.using("non_default_connection").create(name="Apple", color="Red")

        self.assertEqual(1, TestFruit.objects.using("non_default_connection").count())
        self.assertEqual(2, TestFruit.objects.count())

        with self.assertRaises(TestFruit.DoesNotExist):
            TestFruit.objects.using("non_default_connection").get(name="Orange")
