...
class SelectCommand(object):
    ...
    def _build_query(self):
        ...
        for and_branch in self.query.where.children:
            ...
            for filter_node in filters:
                ...
                if isinstance(value, decimal.Decimal):
                    ...
                    value = connections[self.connection].ops.adapt_decimalfield_value(
                        value,
                        field.max_digits,
                ...
                if isinstance(value, (list, tuple, set)):
                    # Unlike in the old AppEngine Datastore API, we can't pass a list of values to
                    # `add_filter`, but instead have to pass each one separately.
                    for v in value:
                        query.add_filter(lookup[0], lookup[1], v)
                else:
        ...
        if len(queries) == 1:
            ...
            if identifier:
                # Yay for optimizations!
                return meta_queries.UniqueQuery(self.connection, identifier, queries[0],
                                                self.query.model, self.namespace)
    ...
    def execute(self):
        ...
        def insert_chunk(keys, entities):
            ...
            if has_active_unique_constraints(self.model):
                ...
                if was_in_atomic_block:
                    check_unique_markers_in_cache(self.model, entities, self.namespace)
        ...
        was_in_atomic_block = transaction.in_atomic_block(using=self.connection.alias)
    ...
    def _update_entity(self, result):
        def update_txt():
            ...
            def perform_insert():
                ...
                if has_active_unique_constraints(self.model):
                    perform_unique_checks(self.namespace, self.model, client, primary, test_fn)
            ...
            if has_active_unique_constraints(self.model):
                check_unique_markers_in_cache(self.model, self.results, self.namespace)
