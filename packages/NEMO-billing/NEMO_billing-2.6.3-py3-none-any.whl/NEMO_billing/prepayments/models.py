...
class Fund(BaseModel):
    ...
    def check_for_low_balance(self, balance_left: Decimal):
        try:
            if not self.balance_warning_sent:
                if self.balance_warning_percent:
                    ...
                    if balance_left <= warning_amount:
                        ...
                        recipients = [
                            user.get_emails(EmailNotificationType.BOTH_EMAILS) for user in User.objects.filter(is_active=True, is_accounting_officer=True)
                        ]
                        ...
                        message += f"Original amount: {self.amount}<br>\n"
                        message += f"Current balance: {balance_left}<br>\n"
                        send_mail(
                            subject=subject,
                            content=message,
