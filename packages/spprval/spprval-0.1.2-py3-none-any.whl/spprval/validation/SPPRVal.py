...
from .validator import ResourcesValidator, WorksValidator, TimeValidator
...
class PlanValidator:
    def __init__(self, plan, url):
        self.ksg_data = KSGDataset(ksg_data=plan).collect()
        self.plan_dataset = PlanDataset(ksg_data=self.ksg_data)
        self.data_source = DBWrapper(url=url)
    ...
    def validate(self):
        ...
        # act_dict_processed_swapped = object_name_processor.create_processed_dict(
        #     names=act, name_type="act", swap=True
        # )
        # act_dict_granulary_swapped = object_name_processor.create_granulary_dict(
        #     names=act, name_type="act", swap=True
        # )
        # res_dict_processed_swapped = object_name_processor.create_processed_dict(
        #     names=res, name_type="res", swap=True
        # )
        # res_dict_granulary_swapped = object_name_processor.create_granulary_dict(
        #     names=res, name_type="res", swap=True
        # )

        # # act_dict_processed_swapped = create_processed_dict_act_swapped(act)
        # # res_dict_processed_swapped = create_processed_dict_res_swapped(res)
        # # res_dict_granulary_swapped = create_granulary_dict_res_swapped(res)

        # processed_act_names = list(act_dict_processed_swapped.keys())
        # processed_res_names = list(res_dict_processed_swapped.keys())

        brave = pd.read_csv("../data/brave_new2.csv", index_col=0)
        ...
        seq_history = pd.read_csv("../data/links_history_messoyaha_with_granular_smr_v4.csv")
        (
            df_seq_stat,
            df_color_seq,
            norm_perc_seq,
            not_perc_seq,
        ) = self.aggregator.get_all_seq_statistic(
            seq_history, self.ksg_data
        )
    ...
    def window_zero(df: pd.DataFrame, min_window: int = 2, max_window: int = 10):
        ...
        date_list = [(base + datetime.timedelta(days=x)).strftime('%d.%m.%Y') for x in range(df.shape[0])]
        df.index = date_list
        ...
        for window_len in window_list:
            ...
            for start in range(0, len(df) - window_len + 1):
                ...
                if (end_day-start_day).days == window_len - 1:
                    ...
                    new_row_res = df[res_col].iloc[start:start + window_len].mean(axis=0, skipna=True)
...
class ProjectValidation:
    def __init__(self, project_ksg, url):
    ...
    def validate_plan(self):
        ...
        if regime:
            (
                ...
                work_res_common_perc,
                plan_statistic,
            ) = PlanValidator(plan=self.project_ksg, url=self.url).validate()
            ...
            for id_number, p in enumerate(json_files):
                (
                    ...
                    work_res_common_perc,
                    plan_statistic,
                ) = PlanValidator(plan=p, url=self.url).validate()
