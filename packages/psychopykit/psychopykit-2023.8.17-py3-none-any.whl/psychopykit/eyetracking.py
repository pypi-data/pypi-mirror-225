...
class EyeTracking:
    ...
    def to_bids(self, out_dir, bids_stub):
        ...
        self.mese_df.to_csv(tsv_fname, columns=req_cols, header=True, index=False, sep='\t')
    ...
    def trim_to_stimulus(self, stim_str=None):
        ...
        if stim_str:
            ...
            stim_start_times = self.log_df[stim_start]['Time (s)'].values

            if len(stim_start_times) > 0:

                t0 = stim_start_times[0]
                ...
                t0 = -1.0
                print(f'Stimulus start string \"{stim_str}\" not found - skipping')
