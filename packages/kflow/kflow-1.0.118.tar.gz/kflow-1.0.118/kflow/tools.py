    ...
    if source_object == 'table':
        ...
        for table in  name_tables: 
            try:
                ...
                warehouse_engine.execute(f"select * from staging.{table} limit 1")
                logging.info(f"Verifying...")
                sleep(1)
                warehouse_engine.execute(f"drop table if exists public.{table} cascade")
                logging.info(f"Deleting...")
                sleep(2)
                warehouse_engine.execute(f"create table public.{table} (LIKE staging.{table} INCLUDING DEFAULTS)")
                logging.info(f"Creating...")
                sleep(3)            
                warehouse_engine.execute(f"insert into public.{table} (select * from staging.{table})")
        ...
        for table in  name_tables: 
            ...
            try:
                ...
                sleep(2)
                logging.info(f"Creating...")
                warehouse_engine.execute(f"CREATE TABLE public.{table} as SELECT * FROM {source_shema}.{table}")
                sleep(6)
                warehouse_engine.execute(f"select * from public.{table} limit 1")
                logging.info(f"Donee baby!!")                            
            except:                
                raise ValueError(f"{table} not deploy")     
