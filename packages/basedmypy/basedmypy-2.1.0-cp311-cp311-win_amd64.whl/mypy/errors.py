...
class Errors:
    ...
    def format_messages(
        self, error_info: list[ErrorInfo], source_lines: list[str] | None, path: str
    ) -> list[str]:
        ...
        errors = self.render_messages(error_info, source_lines, self.simplify_path(path))
        errors = self.remove_duplicates(errors)
        self.filtered_baseline = self.filtered_baseline or bool(errors)
        for (
            file,
            line,
            column,
    ...
    def file_messages(self, path: str) -> list[str]:
        ...
        return self.format_messages(self.error_info_map[path], source_lines, path)
