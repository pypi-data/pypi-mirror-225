...
class PodFunctionTask(PythonFunctionTask[Pod]):
        ...
        for container in containers:
            ...
            if container.name == self.task_config.primary_container_name:
                ...
                container.command = [
                    "/bin/bash",
                    "-c",
                    f"exec 3>&1 4>&2 && ( {' '.join(sdk_default_container.args)} 1>&3 2>&4 )",
                ]
                container.args = []
    ...
    def local_execute(self, ctx: FlyteContext, **kwargs) -> Union[Tuple[Promise], Promise, None]:
        ...
        # patch kwargs to include default values if not explicitly provided
        signature = inspect.signature(self._task_function)
