...
class Task:
    ...
    block_pattern = re.compile(r"(bl:)(\w+)")
    """Regex pattern which defines a block key. format: bl:string"""

    blocked_by_pattern = re.compile(r"(by:)(\w+)")
    ...
    def __parse_priority(self, task_string):
        ...
        if self.priority_pattern.match(task_string):
            ...
            self.priority_pattern.sub("", self.stripped_string)
    ...
    def strip_t_date(self):
        ...
        stripped = self.t_date_pattern.sub("", stripped)
    ...
    def strip_due_date(self):
        ...
        """
        stripped = self.stripped_string if self.stripped_string else self.raw_string
        stripped = self.due_date_pattern.sub(" ", stripped)
        stripped = self.due_month_pattern.sub(" ", stripped)
        stripped = self.due_day_pattern.sub(" ", stripped)
        self.stripped_string = stripped
        return stripped
    ...
    def strip_context(self):
        ...
        """
        stripped = self.stripped_string if self.stripped_string else self.raw_string
        stripped = self.context_pattern.sub(" ", stripped)
        self.stripped_string = stripped
        return stripped
    ...
    def strip_project_string(self):
        ...
        """
        stripped = self.stripped_string if self.stripped_string else self.raw_string
        stripped = self.project_pattern.sub(" ", stripped)
        self.stripped_string = stripped
        return stripped
    ...
    def update_priority(self, new_priority: str) -> bool:
        ...
        if self.priority < 600:
            ...
            self.raw_string = self.priority_pattern.sub("(" + new_priority + ")", self.raw_string)
            return False
        else:
    ...
    def change_due(self, new_due: datetime) -> bool:
        ...
        if self.due_date < datetime(9999, 12, 31).date():
            ...
            self.raw_string = self.due_date_pattern.sub(
