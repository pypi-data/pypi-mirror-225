...
class Diagram_SolarSystem(diagram.Diagram):
    ...
    def setup_fg(self, *, cwidth, cheight):
        logger.debug(f'Render Diagram {self.__class__.__qualname__} @ {cwidth}x{cheight}')
...
class MiscFrame(FrameStateful):
    ...
    def onCStateChange(self, var, etype):
        ...
        if cstate:
            lwidget.set_label('↓ Drag Separator ↓')
            lwidget.gstate = GuiState(enabled=False)
        else:
            lwidget.set_label('Do Nothing')
...
class UpstreamBool(FrameLabelled):
    ...
    def onNoOp(self, event=None):
        ...
        self.bNoOp_Big.set_label(random.choice(['No Op', 'NoOp', 'NOOP']))
...
class NB_Complex_ListboxSet(FrameUnlabelled):
    ...
    def setup_widgets(self):
        self.__all = StaticList((f'String {idx:02}' for idx in range(1, 20 + 1)), defaultIndex=0)
...
class NB_Center_Radio(FrameUnlabelled):
    ...
    def setup_layout(self, layout):
        all_radio = set(w for w in self.widgets.values() if isinstance(w, Radio))
        for w in all_radio:
            w.grid(sticky=tk.NSEW)
        self.pgrid(*all_radio, uniform='radio')
...
class NB_Center_EventLoop(FrameUnlabelled):
    ...
    def setup_adefaults(self):
        ...
        self.onUpdateState(el)

    def onUpdateState(self, el):
        ...
        if isRunning:
            ...
            str_control = 'Start'
        w_control.set_label(f'[BG-Q]: {str_control}')
    ...
    def onELRes(self, eventloop, ridx, response, *, ustate=True):
        ...
        if txt is not None:
            if ridx is None:
                self.log(f'T[EL_Sleep] | {txt}')
            else:
                self.log(f'T[EL_Sleep<i>#{ridx}</i>] | {txt}')
        if ustate:
            self.onUpdateState(eventloop)
...
class NB_Center_SWin_Selection(SecondaryWindow):
    ...
    def setup_adefaults(self):
        for nidx in self.__class__.NAMES:
            ...
            wbtn.onClick = self.generateClick(nidx)
...
class RW(RootWindow):
    ...
    def setup_eventloops(self):
        widget_el = self.nb_center.wtab('el')
        return {
            'sleep': EL_Sleep(wcallback=bg.ELCallback(widget_el, widget_el.onELRes)),
        }
