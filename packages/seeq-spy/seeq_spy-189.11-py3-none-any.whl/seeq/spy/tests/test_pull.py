...
def test_pull_signal_with_grid():
    ...
    assert 400000 < df.spy.status.df.iloc[0]['Data Processed'].bytes_processed < 1000000
...
def test_pull_condition_as_capsules():
    ...
    }]), workbook='test_pull_condition_as_capsules', worksheet=None,
        session=session)
    ...
    assert 10000 < pull_df.spy.status.df.iloc[0]['Data Processed'].bytes_processed < 2000000
...
def test_pull_calculated_scalar():
    ...
    if pull_df.spy.status.df.iloc[0]['Data Processed'].bytes_processed != 0:
        assert 200000 < pull_df.spy.status.df.iloc[0]['Data Processed'].bytes_processed < 500000
