...
class AllureListener:
    ...
    def start_step(self, uuid, title, params):
        def get_last_item(allure_logger, item_type=None):
            for _uuid in reversed(allure_logger._items):
                if item_type is None:
                    return allure_logger._items.get(_uuid)
                if type(allure_logger._items[_uuid]) == item_type:
                    return allure_logger._items.get(_uuid)

        def generate_new_container(allure_logger):
            parent_uuid = uuid4()
            children = [get_last_item(allure_logger, TestResult).uuid]
            container = TestResultContainer(uuid=parent_uuid, children=children)
            allure_logger.start_group(parent_uuid, container)
            allure_logger.update_group(parent_uuid, start=now())
            return parent_uuid

        if '--- Set up' in title:
            parent_uuid = generate_new_container(self.allure_logger)
            ...
            self.allure_logger.saved_before_fixture_parent_uuid = parent_uuid
            ...
            self.allure_logger.saved_after_fixture_parent_uuid = parent_uuid
        else:
    ...
    def stop_step(self, uuid, exc_type, exc_val, exc_tb):
        ...
        if uuid == getattr(self.allure_logger, 'saved_before_fixture_uuid', None):
            ...
            self.allure_logger.stop_group(self.allure_logger.saved_before_fixture_parent_uuid, stop=now())
        # Closing '--- Tear down' step
            ...
            self.allure_logger.stop_after_fixture(self.allure_logger.saved_after_fixture_uuid,
                                                  ...
                                                  status=get_status(exc_val),
                                                  statusDetails=get_status_details(exc_type, exc_val, exc_tb))
            self.allure_logger.stop_group(self.allure_logger.saved_after_fixture_parent_uuid, stop=now())
        else:
