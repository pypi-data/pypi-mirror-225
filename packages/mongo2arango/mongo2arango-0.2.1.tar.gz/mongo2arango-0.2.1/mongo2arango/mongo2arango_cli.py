...
def migrate_core(db, col, input, key, threads):
    ...
    BULK_IMPORT_BATCH_SIZE = num_threads * 2500
        ...
        with ThreadPoolExecutor(max_workers=num_threads) as executor:
            ...
            with open(filename, "r") as f:
                ...
                for batch in split_data(items, THREAD_BATCH_SIZE):
                    # Convert batch to string and parse with simplejson
                    batch_str = "[" + ",".join(map(dumps, batch)) + "]"
                    batch_data = loads(batch_str)
                    futures.append(
                        executor.submit(process_batch, batch_data, progress_queue)
                    )
...
def migrate(db=None, col=None, input=None, key=None, threads=None):
    ...
    if not threads:
        threads = click.prompt(
            "Number of threads to use for migration", type=int, default=1
        )
...
def migrate_bulk(db=None, dir=None, key=None, threads=None):
    ...
    for json_file in json_files:
        ...
        processed_collections += 1  # Increment the counter for every collection
