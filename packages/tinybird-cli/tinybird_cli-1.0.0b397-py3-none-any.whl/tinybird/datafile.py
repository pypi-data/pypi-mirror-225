...
async def folder_pull(client: TinyB, folder: str, auto: bool, match: Optional[str], tag: Optional[str], force: bool, verbose: bool = True, include_tokens: Optional[bool] = False):  # noqa: C901
    ...
    try:
        ...
        tokens_versions = _get_latest_versions(remote_tokens, tag)
        ...
        await write_files(tokens_versions, resources, 'token', 'token_file')
...
def is_endpoint_with_no_dependencies(resource: Dict[str, Any], dep_map: Dict[str, Any], to_run: Dict[str, Any]):
    ...
    for node in resource.get('nodes', []):
        ...
        if node.get('params', {}).get('type', '').lower() in [PipeNodeTypes.MATERIALIZED, PipeNodeTypes.COPY, PipeNodeTypes.DATA_SINK]:
            return False
