...
class TinyB(object):
    ...
    def __init__(self, token: str, host: str = HOST, version: Optional[str] = None, disable_ssl_checks: bool = False, send_telemetry: bool = False):
    ...
    async def _req(self, endpoint: str, data=None, files=None, method: str = 'GET', retries: int = LIMIT_RETRIES, **kwargs):  # noqa: C901
        ...
        try:
            with requests.Session() as session:
                ...
                if method == 'POST':
                    if files:
                        response = await sync_to_async(session.post, thread_sensitive=False)(url, files=files, verify=verify_ssl, **kwargs)
                    else:
                        response = await sync_to_async(session.post, thread_sensitive=False)(url, data=data, verify=verify_ssl, **kwargs)
                elif method == 'PUT':
                    response = await sync_to_async(session.put, thread_sensitive=False)(url, data=data, verify=verify_ssl, **kwargs)
                elif method == 'DELETE':
                    response = await sync_to_async(session.delete, thread_sensitive=False)(url, data=data, verify=verify_ssl, **kwargs)
                else:
                    response = await sync_to_async(session.get, thread_sensitive=False)(url, verify=verify_ssl, **kwargs)
    ...
    async def connections(self, connector: Optional[str] = None):
        ...
        bigquery_connection = await self.bigquery_connection() if connector == 'bigquery' else None
    ...
    async def analyze_pipe_node(self, pipe_name: str, node: Dict[str, Any], dry_run: str = 'false', datasource_name: Optional[str] = None):
        params = {**{
    ...
    async def populate_node(self, pipe_name: str, node_name: str, populate_subset: bool = False, populate_condition: Optional[str] = None, truncate: bool = True, unlink_on_populate_error: bool = False, semver: Optional[str] = None):
        params: Dict[str, Any] = {
    ...
    async def pipe_data(self, pipe_name_or_uid: str, sql: Optional[str] = None, format: Optional[str] = 'json', params: Optional[Mapping[str, Any]] = None):
    ...
    async def pipe_create_copy(self, pipe_name_or_id: str, node_id: str, target_datasource: str, schedule_cron: Optional[str] = None):
        data = {'target_datasource': target_datasource}
        if schedule_cron:
            data['schedule_cron'] = schedule_cron

        return await self._req(f"/v0/pipes/{pipe_name_or_id}/nodes/{node_id}/copy",
                               method='POST',
                               data=data)

    async def pipe_update_copy(self, pipe_name_or_id: str, node_id: str, target_datasource: Optional[str] = None, schedule_cron: Optional[str] = None):
        data = {
    ...
    async def pipe_create_sink(self, pipe_name_or_id: str, node_id: str, params: Optional[Dict[str, str]] = None):
        params = {**params} if params else {}
        return await self._req(f"/v0/pipes/{pipe_name_or_id}/nodes/{node_id}/sink?{urlencode(params)}",
                               method='POST',
                               data='')
    ...
    async def create_workspace_branch(self, branch_name: str, last_partition: Optional[bool], all: Optional[bool], ignore_datasources: Optional[str]):
        params = {
            'name': branch_name,
        }
        if last_partition:
        ...
        return await self._req(f"/v0/environments?{urlencode(params)}", method='POST',
                               data=b'')

    async def deploy_environment(self, branch_id: str, semver: str, commit: str, dry_run: Optional[bool] = False, data_migrations: Optional[Any] = None):
        data = {
            'semver': semver,
            'dry_run': 'true' if dry_run else 'false',
    ...
    async def branch_workspace_data(self, workspace_id: str, last_partition: Optional[str], all: Optional[str], ignore_datasources: Optional[str] = None):
        params = {}
        if last_partition:
    ...
    async def branch_regression_tests(self, branch_id: str, pipe_name: Optional[str], test_type: str,
                                      failfast: Optional[bool] = False,
                                      limit: Optional[int] = None, sample_by_params: Optional[int] = None, match: Optional[List[str]] = None,
                                      params: Optional[List[Dict[str, Any]]] = None,
                                      assert_result: Optional[bool] = True, assert_result_no_error: Optional[bool] = True, assert_result_rows_count: Optional[bool] = True,
        ...
        test: Dict[str, Any] = {test_type: {'config': {'assert_result_ignore_order': assert_result_ignore_order,
        ...
        regression_commands: List[Dict[str, Any]] = [{'pipe': ".*" if pipe_name is None else pipe_name, 'tests': [test]}]
    ...
    async def wait_for_job(
        ...
        while not done:
            params = {
                'debug': 'blocks,block_log'
            }
            if semver:
                params['semver'] = semver
            res: Dict[str, Any] = await self._req(f"/v0/jobs/{job_id}?{urlencode(params)}")
    ...
    async def get_snowflake_integration_query(self, role: str, stage: Optional[str], integration: Optional[str]) -> Optional[Dict[str, Any]]:
        try:
            params = {
                'role': role,
            }
    ...
    async def validate_snowflake_connection(
        self,
        account_identifier: str,
        user: str,
        password: str
    ...
    async def _sql_get_format_remote(self, sql: str, with_clickhouse_format: bool = False) -> str:
        params = {
            'with_clickhouse_format': 'true' if with_clickhouse_format else 'false'
        }
    ...
    async def workspace_commit_update(self, workspace_id: str, commit: str):
        return await self._req(f'/v0/workspaces/{workspace_id}/releases/?commit={commit}&force=true', method='POST', data='')
    ...
    async def release_rm(self, workspace_id: str, semver: str, confirmation: Optional[str] = None, dry_run: bool = False):
        params = {
            'force': 'true' if confirmation else 'false',
            'dry_run': 'true' if dry_run else 'false'
        }
        if confirmation:
        ...
        if 'scopes' in token:
            for scope_dict in token['scopes']:
                ...
                for scope_type in scope_types:
                    scope = scope_type.strip() + f":{scope_dict['resource']}" if 'resource' in scope_dict else scope_dict['name']
