    ...
    if git:
        ...
        if current_ws.get('release'):
            ...
            try:
                ...
                click.echo(FeedbackManager.error_no_git_repo_for_init(repo_path=folder))
                return
            else:
                if not cli_git_release.is_dottinyb_ignored():
                    click.echo(FeedbackManager.error_dottinyb_not_ignored())
                    return
                ...
                try:
                    ...
                    raise CLIBranchException(FeedbackManager.error_exception(error=str(exc)))
        ...
        if click.confirm(FeedbackManager.prompt_init_cicd(), default=True):
            if not cli_git_release:
                try:
                    cli_git_release = CLIGitRelease(path=folder)
                except CLIGitReleaseException:
                    click.echo(FeedbackManager.error_no_git_repo_for_init(repo_path=folder))
                    return
            await init_cicd(client, current_ws, path=cli_git_release.working_dir())
