...
def renameall_ico(path):
    """ Renaming all ico file in subfolder as name of folder
    ...
    for folder in path.iterdir():
        for ico in folder.glob("*.ico"):
            ...
            try:
                ico.rename(folder / f"{name + ico.suffix}")
...
def set_all(path):
    ...
    for folder in path.iterdir():
        for ico in folder.glob("*.ico"):
            ...
            desktop_conf.write_text(syntax(ico.name))
            Attrib(folder).set_s()
            Attrib(ico).set_h()
