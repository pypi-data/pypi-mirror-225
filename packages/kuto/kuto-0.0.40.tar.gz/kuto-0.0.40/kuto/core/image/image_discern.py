...
class ImageDiscern:
    def __init__(self, target_image, grade=0.8, gauss_num=111) -> None:
        """__init__ [处理图片识别]
        Args:
        ...
        """
        self.source_image = os.path.join(os.path.abspath('image'), 'SourceImage.png')
        if not os.path.exists(self.source_image):
            raise FileNotFoundError(f"文件: {self.source_image} 不存在")
        self.target_image = os.path.join(os.path.abspath('static'), target_image)
        parent_dir = os.path.dirname(os.getcwd())
        patent_patent_dir = os.path.dirname(parent_dir)
        patent_patent_patent_dir = os.path.dirname(patent_patent_dir)
        if not os.path.exists(self.target_image):
            self.target_image = os.path.join(parent_dir, 'static', target_image)
            if not os.path.exists(self.target_image):
                self.target_image = os.path.join(patent_patent_dir, 'static', target_image)
                if not os.path.exists(self.target_image):
                    self.target_image = os.path.join(patent_patent_patent_dir, 'static', target_image)
                    if not os.path.exists(self.target_image):
