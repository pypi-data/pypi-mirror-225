...
class GPF:
    ...
    def _convert_and_transform_data(
        ...
        Args:
            data (Union[np.ndarray, torch.Tensor]): The input data.
            scaler (StandardScaler): The scaler used to fit and transform the data.
            fit (bool, optional): Whether to fit the scaler to the data. Defaults to False.

        Returns:
            torch.Tensor: The transformed data as a tensor on the specified device.
        """
    ...
    def _preprocess(
        self,
        groups: Dict[str, Dict[str, Union[np.ndarray, torch.Tensor]]],
        device: torch.device,
    ) -> Tuple[Dict[str, Dict[str, torch.Tensor]]]:
        """
        Preprocess the input data by fitting and transforming it using a StandardScaler.
        The transformed data is converted to tensors and stored on the specified device.

        Args:
            groups (Dict[str, Dict[str, Union[np.ndarray, torch.Tensor]]]): A dictionary containing the input data.
            device (torch.device): The device on which the output tensors should be stored.

        Returns:
            Tuple[Dict[str, Dict[str, torch.Tensor]], StandardScaler]: A tuple containing the preprocessed data as tensors and the fitted scaler.
        """
        if self.scale_x_values:
            groups["train"]["x_values"] = self._convert_and_transform_data(
                groups["train"]["x_values"], self.scaler_x_values, fit=True
            )

            groups["predict"]["x_values"] = self._convert_and_transform_data(
                groups["predict"]["x_values"], self.scaler_x_values, fit=False
            )
            ...
            groups["train"]["x_values"] = (
                torch.from_numpy(np.array(groups["train"]["x_values"]))
                .to(torch.float32)
                .to(device)
            )
            groups["predict"]["x_values"] = (
                torch.from_numpy(np.array(groups["predict"]["x_values"]))
                .to(torch.float32)
                .to(device)
            )

        print(groups["train"]["x_values"])
        print(self.scaled_seasonality)
    ...
    def _build_model(
        ...
        for i in range(self.groups["train"]["s"]):
            ...
            likelihood.noise_covar.initialize(noise=like_noise)
    ...
    def train(
        ...
        scale_rbf_kernel_outputscale: float = 0.5,
        periodic_kernel_lengthscale: float = 0.5,
        scale_periodic_kernel_outputscale: float = 1.5,
        k: float = 0.1,
        m: float = 0.1,
        b: float = 0.1,
        like_noise: float =0.1,
        n_splits: int = 3,
        ...
        Parameters:
            ...
            rbf_kernel_lengthscale: Lengthscale of RBF kernel
            scale_rbf_kernel_outputscale: Scale of output of RBF kernel
            periodic_kernel_lengthscale: Lengthscale of periodic kernel
            scale_periodic_kernel_outputscale: Scale of output of periodic kernel
            k, m, b: Parameters for the model
            n_splits: Number of splits for cross-validation

        Returns:
            tuple: Tuple containing the trained model and the likelihood.
        """
        rbf_kernel_lengthscale = torch.tensor([rbf_kernel_lengthscale])
        scale_rbf_kernel_outputscale = torch.tensor([scale_rbf_kernel_outputscale])
        periodic_kernel_lengthscale = torch.tensor([periodic_kernel_lengthscale])
        ...
        for fold_idx, (train_indices, val_indices) in enumerate(
            ...
            for epoch in range(epochs):
                ...
                es, non_decreasing = self.early_stopping(
                    fold_val_losses, patience=patience
                )

                if early_stopping and val_loss is not None and es:
                    print(f"EarlyStopping counter: {non_decreasing} out of {patience}")
                    break
    ...
    def predict(
        ...
        with torch.no_grad(), gpytorch.settings.fast_pred_var():
            combined_x = torch.squeeze(torch.cat((self.train_x, self.test_x), dim=0))
            test_x = (
                torch.arange(self.groups["predict"]["n"])
                .type(torch.float32)
                .to(self.device)
            )
            predictions = likelihood(
                *model(*[combined_x for i in range(self.groups["predict"]["s"])])
            )
            self._log_memory()
