...
class GPF:
    ...
    def train(
        ...
        if hyperparameters["mean_type"] == "constant":
            mean_func = ConstantMean(constant=hyperparameters.get("mean_constant", 0.0))
        ...
        for fold_idx, (train_indices, val_indices) in enumerate(
            ...
            likelihood_list, model_list = self._build_model(
                ...
                learn_like_noise=hyperparameters["learn_like_noise"],
                mean_func=mean_func,
            )
    ...
    def predict(
        ...
        with torch.no_grad(), gpytorch.settings.fast_pred_var():
            ...
            predictions = likelihood(
                *model(*[combined_x for _ in range(self.groups["predict"]["s"])])
            )
            self._log_memory()
