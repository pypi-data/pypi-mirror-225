...
def session(token, api_url='http://api.nijta.com/'):
    """
    Start a session.
...
def send_request(input_data, params, session_id, headers, api_url='http://api.nijta.com/'):
    """
    ...
    if audio_dataset:
        try:
            response = requests.post(f"{api_url}/tasks/{session_id}",
                                     json=json.dumps(audio_dataset.cast_column("audio", datasets.Audio(decode=True)),
...
def read_response(task_id, api_url='http://api.nijta.com/'):
    """
