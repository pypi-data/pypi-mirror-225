...
def builder_fastapi(deploy: Union[Dict, FastAPI] = None):
    if deploy is not None:
        if isinstance(deploy, FastAPI):
            return deploy
        _app = FastAPI(**deploy)
    else:
        _app = FastAPI(**BuilderSettings().settings.Fastapi.config)
    _app.add_middleware(
        CORSMiddleware,
        allow_origins=["*"],
...
class BuilderSettings:
    ...
    default_setting = 'settings.conf'

    def __init__(self, base_dir=None, default_setting=None):
        self.engine = None
        if base_dir is not None:
            self.base_dir = base_dir
        if default_setting is not None:
            self.default_setting = default_setting
        self.conf_path = os.path.join(self.base_dir, self.base_dir, self.default_setting)
