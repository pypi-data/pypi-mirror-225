...
from rest_fastapi.bases.model import BasicModel
from rest_fastapi.bases.schema import ListArgsSchema, ListFilterSchema, ListOrderSchema, RespListSchema, ListKeySchema
...
class BasicDao(object):
    ...
    async def count(self, db: Session, args: ListArgsSchema) -> int:
        ...
        if hasattr(self.Model, 'is_deleted'):
            filters.append(~self.Model.is_deleted)
        filters.extend(await self._handle_list_filters(args.filters))
    ...
    async def list(self, db: Session, args: ListArgsSchema) -> RespListSchema:
        ...
        resp.result = self._handle_list_keys(args.keys, obj_list)  # 处理list
        return resp

    def _handle_list_filters(self, args_filters: Optional[List[ListFilterSchema]]) -> List:
        """
        查询条件组装
        :param args_filters:
        :return:
    ...
    def _handle_list_orders(self, args_orders: Optional[List[ListOrderSchema]]) -> List:
        """
        处理list接口传入的排序条件
    ...
    def _handle_list_keys(self, args_keys: Optional[List[ListKeySchema]], obj_list: List[Model]) -> List[dict]:
        """
