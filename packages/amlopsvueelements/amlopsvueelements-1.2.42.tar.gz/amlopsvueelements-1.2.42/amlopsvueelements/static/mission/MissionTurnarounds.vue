  ...
  <div id="mission-turnarounds" :class="$style['mission-turnarounds']">
    <div :class="$style['mission-turnarounds__table-wrapper']">
      <div ref="tableWrapperRef">
        ...
        >
          <thead class="sticky top-0 left-0 z-50" :class="$style['service-table__header']">
            <tr>
              <th
            ...
            <tr :class="$style['service-table__header-row']">
              ...
              <th v-for="(item, index) in groundServicingLength" :key="index" class="!text-center">
                {{ item }}
              </th>
              <th></th>
            </tr>
              ...
              <tr v-if="!turnaround.is_deleted" :class="$style['service-table__body-row']">
                ...
                >
                  <div
                    ...
                    class="w-full h-full bg-grey-50/50 cursor-not-allowed absolute left-0 top-0 z-50"
                  >
                    <span
                      class="absolute z-50 opacity-100 text-[14px] text-[#97650e] w-[95%] text-center p-4 rounded-[0.5rem] top-[50%] left-1/2 -translate-y-[50%] -translate-x-[50%] border-[#fee5ba] bg-[#feeed1]"
                    >
                      Servicing is not requested
                    </span>
                  </div>
                  <div>
                    <div class="flex gap-x-3 justify-center">
                      ...
                      <div class="flex flex-col items-center">
                        ...
                        <UCheckboxWrapper
                          ...
                          :disabled="
                            isDisabledService(service.id, serviceOptions) ||
                            !findServiceById(service.id, serviceOptions).departure
                          "
                          class="!mb-0 d-flex shrink-0"
                          size="24px"
                        />
                ...
                <td>
                  <div :class="$style['delete-service-btn']" @click="onDeleteService(turnaround)">
                    <img
...
const groundServicingLength = computed(() => groundServicing.value?.length ?? 0)
...
const onDeleteService = async (turnaround: any) => {
...
const onClickAddServiceRow = () => {
  turnaroundsModel.value.push({
    id: null,
    is_deleted: false,
    turnarounds: Array.from({ length: groundServicingLength.value }).map((_, index) => {
      return {
