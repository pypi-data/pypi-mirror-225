...
class BaseExportLatestEntitiesData(BaseExportEntitiesData):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.order_number_entity_enum = self._get_order_number_entity_enum()

        self.async_task = self._get_async_task()
        self.task_id = kwargs.get('task_id')

    def _get_order_number_entity_enum(self) -> BaseEnumerate:
        """Возвращает перечисление с очередностью экспорта данных по сущностям."""
    ...
    def _find_exporting_entities_data_managers(self, *args, **kwargs) -> None:
        """
        Поиск менеджеров Функций экспорта данных по сущностям РВД.
        """
        logger.info('find exporting entities data manager..')
        ...
        export_entities = filter(
            lambda entity: entity.order_number not in (None, DEFAULT_ORDER_NUMBER), self.entities
        )

        # Сортируем сущности по очередности выгрузки
        sorted_entities = sorted(
            export_entities,
            key=lambda entity: self.order_number_entity_enum.get_constant_value_by_name(entity.key)
        )
