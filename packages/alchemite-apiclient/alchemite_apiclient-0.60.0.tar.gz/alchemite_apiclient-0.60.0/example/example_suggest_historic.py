...
SUGGEST_DATASET = "steels"  # 'steels' or 'vector'

if SUGGEST_DATASET == "steels":
    dataset_file = "steels.csv"
    dataset_name = "steels"
    model_name = "steels"
    suggest_args = "suggest_historic_args_steel.json"
    vector_pairs = None
elif SUGGEST_DATASET == "vector":
    dataset_file = "vector.csv"
    dataset_name = "vector"
    model_name = "vector"
    suggest_args = "suggest_historic_args_vector.json"
    vector_pairs = [["z", "t"]]
else:
    raise Exception("Invalid suggest dataset option")
...
print("suggest_historic job ID:", suggest_historic_job_id)
...
suggest_historic_job = await_job(get_suggest_historic_job_metadata)
print(suggest_historic_job)

############################################################################
### Delete model, dataset and suggest_historic job
############################################################################
# print("\n--- deleting suggest_historic job, model and dataset ---")
# response = api_models.models_id_suggest_historic_job_id_delete(
#     model_id, suggest_historic_job_id
# )
# print("deletion suggest_historic job successful") if response is None else print(
#     response
# )
