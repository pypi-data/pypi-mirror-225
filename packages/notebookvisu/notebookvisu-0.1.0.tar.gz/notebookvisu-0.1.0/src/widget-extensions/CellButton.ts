import { Cell, ICellModel, CellModel } from '@jupyterlab/cells';
...
const CELL_BUTTON_CLASS: string = 'cell-dashboard-button-container';
...
class CellButtonWidget extends Widget {
  constructor(
    ...
    // disable the button when there is no notebook_id
    button.disabled = notebook_id === undefined;
...
export class CellTracker implements IDisposable {
  constructor(panel: NotebookPanel, commands: CommandRegistry) {
    ...
    panel.context.ready.then(() => {
      let notebookId = panel.model?.metadata['notebook_id'];
      if (notebookId !== undefined) this._notebookId = notebookId as string;
      this._notebookName = panel.context.sessionContext.name;
    });
  ...
  private _notebookId: string | undefined = undefined;
