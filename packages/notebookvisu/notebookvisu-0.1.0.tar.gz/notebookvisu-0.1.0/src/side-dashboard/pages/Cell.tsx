...
const Cell = (props: { notebookId: string }): JSX.Element => {
  ...
  return (
    <>
      <div className="course-title-container">Cell ({content.cellId})</div>
      {isFetching ? (
        <Loader />
      ) : (
        <>
          ...
          {apiData.length === 0 ? (
            ...
            <>
              {/* Cell Executions */}
              {apiData.map((value: { [key: string]: any }, index) => {
                return (
                  <Row key={index}>
                    {(radioValue === 1 ||
