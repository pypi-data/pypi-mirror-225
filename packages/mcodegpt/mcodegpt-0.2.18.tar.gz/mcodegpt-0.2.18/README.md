...
[![DOI](https://zenodo.org/badge/13996/monarch-initiative/mcodegpt.svg)](https://zenodo.org/badge/latestdoi/13996/monarch-initiative/mcodegpt)
...
We can extract knowledge from the above text this into the [GO pathway datamodel](src/mcodegpt/templates/gocam.yaml) by running the following command:

#### Command

```bash
mcodegpt extract -t gocam.GoCamAnnotations -i ~/path/to/abstract.txt
```
...
```bash
mcodegpt enrichment -U tests/input/genesets/sensory-ataxia.yaml
```
...
There are a number of pre-defined LinkML data models already developed here - [src/mcodegpt/templates/](src/mcodegpt/templates/) which you can use as reference when creating your own data models.
...
Next step is to compile the schema. For that, you should place the schema YAML in the directory [src/mcodegpt/templates/](src/mcodegpt/templates/). Then, run the `make` command at the top level. This will compile the schema to Python (Pydantic classes).
...
```bash
mcodegpt extract -t mendelian_disease.MendelianDisease -i marfan-wikipedia.txt
```
...
Contributions on recipes to test welcome from anyone! Just make a PR [here](https://github.com/monarch-initiative/mcodegpt/blob/main/tests/input/recipe-urls.csv). See [this list](https://github.com/hhursev/recipe-scrapers) for accepted URLs
