...
class JobMonitor():
    ...
    def check_inprocess_jobs(self):
        ...
        log.debug('Checking running jobs ' + str(current_time), flush=True)
        running_jobs = job_tracker.get_jobs('running')
        self.attempt_to_resolve_jobs(running_jobs, current_time)

        log.debug("Checking for stalled queued parent jobs...", flush=True)
        ...
        log.debug("Checking for stalled queued child jobs...", flush=True)
    ...
    def attempt_to_resolve_jobs(self, inprocess_jobs, current_time):
        for job_doc in inprocess_jobs:
            job_id = job_doc["_id"]
            log.debug(f"inprocess job {str(job_id)}", flush=True)
            ...
            if (parent_ref is None and child_job_spawning_status and child_job_spawning_status != 'unfinished'):
                ...
                log.debug("Attempting to resolve parent job " + str(job_id), flush=True)
                ...
                if child_job_spawning_status == 'failure' or failed_job_count >= _max_child_errors or failed_job_count >= total_child_job_count:
                    log.debug("Found a failed parent job..." + str(job_id), flush=True)
                    ...
                    log.debug("Set the parent job's status to failed..." + str(job_id), flush=True)
                    celeryapp.execute.send_task("tasks.tasks.failed_job", args=[{'job_id': str(job_id), 'status': 'failed', 'space': space, 'manifest_urn': urn}], kwargs={}, queue=_failure_queue)
                # if job is finished (and not failed) we mark it as successful
                elif job_finished and total_child_job_count > 0:
                    log.debug("Found a complete parent job..." + str(job_id), flush=True)
                    job_tracker.set_job_status('success', job_id)
                    log.debug("Resolved the completed job..." + str(job_id), flush=True)
                    # We skip the time comparison if we resolve a job
                    space = job_doc.get('context', {}).get('globalSettings', {}).get('spaceDefault', None)
                    manifest_id = job_doc.get('context', {}).get('manifest', {}).get('@id', None)
                    urn = parse_urn(manifest_id) if manifest_id else None
                    celeryapp.execute.send_task("tasks.tasks.successful_job", args=[{'job_id': str(job_id), 'status': 'success', 'space': space, 'manifest_urn': urn}], kwargs={}, queue=_success_queue)
                else:
                    log.debug("Job is not finished...")
            elif (parent_ref is None and child_job_spawning_status == 'unfinished'):
                log.debug("Parent job has not finished spawning children " + str(job_id), flush=True)
            else:
                # Check for stalled child jobs
                log.debug("Checking status of child job " + str(job_id), flush=True)
                try:
    ...
    def handle_stalled_job(self, job_doc):
        ...
        if self.get_tries_left(number_of_tries, max_number_of_tries) > 0:
            #Requeue
            log.debug("Requeued child job id" + str(job_doc["_id"]))
