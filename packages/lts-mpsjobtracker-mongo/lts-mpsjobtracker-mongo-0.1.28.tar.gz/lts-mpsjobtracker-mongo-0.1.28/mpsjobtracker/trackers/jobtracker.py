...
class JobTracker():
    ...
    def filter_steplist_by_space(self, step_list, space):
        log.debug("filter_steplist_by_space")
        steps_to_skip = []
    ...
    def get_tracker_document(self, job_ticket_id):
        ...
        log.debug('*** MPSJOBTRACKER get_tracker_file ***')

        get_tracker_document_data = {
          'job_ticket_id': job_ticket_id
        }

        log.debug('get_tracker_document_data')
        log.debug(get_tracker_document_data)
        # Get tracker file
    ...
    def init_tracker_document(self, job_name, job_context = None, parent_job_ref = None):
        ...
        log.debug('*** MPSJOBTRACKER init_tracker_document ***')
        log.debug('job_name')
        log.debug(job_name)
        log.debug('job_context')
        log.debug(job_context)
        log.debug('parent_job_ticket_id')
        log.debug(str(parent_job_ref))
        ...
        log.debug('initial_tracker_doc')
        log.debug(initial_tracker_doc)

        # Get main step list
        main_step_list = self.__get_main_step_list()
        log.debug('main_step_list')
        log.debug(main_step_list)
        ...
        log.debug('Add list of job steps to initial_tracker_doc')
        log.debug(initial_tracker_doc)
    ...
    def update_timestamp(self, job_ticket_id, now=None, parent_also=False):
        ...
        log.debug('*** MPSJOBTRACKER update_timestamp ***')
        log.debug('job_ticket_id')
        log.debug(job_ticket_id)
    ...
    def append_error(self, job_ticket_id, error_msg, exception_msg = None, set_job_failed = True):
        ...
        log.error('append_error job_ticket_id {} error_msg {} exception_msg {} set_job_failed {}'.format(job_ticket_id, error_msg, exception_msg, set_job_failed))
