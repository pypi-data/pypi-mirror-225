...
class JobMonitor():
    ...
    def check_inprocess_jobs(self):
        ...
        log.info('Checking running jobs ' + str(current_time), flush=True)
        running_jobs = job_tracker.get_jobs('running')
        self.attempt_to_resolve_jobs(running_jobs, current_time)

        log.info("Checking for stalled queued parent jobs...", flush=True)
        ...
        log.info("Checking for stalled queued child jobs...", flush=True)
    ...
    def attempt_to_resolve_jobs(self, inprocess_jobs, current_time):
        for job_doc in inprocess_jobs:
            job_id = job_doc["_id"]
            log.info(f"inprocess job {str(job_id)}", flush=True)
            ...
            if (parent_ref is None and child_job_spawning_status and child_job_spawning_status != 'unfinished'):
                ...
                log.info("Attempting to resolve parent job " + str(job_id), flush=True)
                ...
                if child_job_spawning_status == 'failure' or failed_job_count >= _max_child_errors or failed_job_count >= total_child_job_count:
                    log.info("Found a failed parent job..." + str(job_id), flush=True)
                    ...
                    log.info("Set the parent job's status to failed..." + str(job_id), flush=True)
                ...
                # if job is finished (and not failed) we mark it as successful
                    ...
                    log.info("Found a complete parent job..." + str(job_id), flush=True)
                    job_tracker.set_job_status('success', job_id)
                    log.info("Resolved the completed job..." + str(job_id), flush=True)
                ...
                log.info("Parent job has not finished spawning children " + str(job_id), flush=True)
            else:
                # Check for stalled child jobs
                log.info("Checking status of child job " + str(job_id), flush=True)
                try:
