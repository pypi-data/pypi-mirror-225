...
class UserService:
    ...
    def check_image(self, image_info: str) -> bool:
        try:
            ...
            # seperate namespace and repository
            slash_index = image_name.rfind("/")
            if slash_index < 0:
                namespace = 'library'
            else:
                namespace = image_name[:slash_index]
            repository = image_name[slash_index + 1:]
            # get target route
            route = f'https://hub.docker.com/v2/repositories/{namespace}/{repository}/tags/{tag}'
    ...
    def update_image(self, provider_addr: str, secret: str, image_name: str):
