...
def feature_gen(file):
    ...
    def bond(file) :
        ...
        bonds_path = os.path.join(script_directory, '..', 'Data', 'bonds.csv')
    ...
    ###########################atom###############
    def atc(file) :
        # Construct the absolute path to the atom.csv file
        atom_path = os.path.join(script_directory, '..', 'Data', 'atom.csv')
        atom = pd.read_csv(atom_path, header=None)
        filename, file_extension = os.path.splitext(file)
        at=pd.DataFrame()
        i = 0
        C_atom = []
    ...
    def soc(file,gap):
        ...
        if min(ff) < gap:
            ...
            return 0
        schneider_wrede_path = os.path.join(script_directory, '..', 'Data', 'Schneider-Wrede.csv')
        mat1 = pd.read_csv(schneider_wrede_path, index_col='Name')
        grantham_path = os.path.join(script_directory, '..', 'Data', 'Grantham.csv')
        mat2 = pd.read_csv(grantham_path, index_col='Name')
        h1 = []
        h2 = []
    ...
    #######################################CeTD#######################
    def ctd(file):
        attr_path = os.path.join(script_directory, '..', 'Data', 'aa_attr_group.csv')
...
if Job == 1:
    ...
    # Construct the absolute path to the RF_model file
    model_path = os.path.join(script_directory, '..', 'Models', 'RF_model')
    print("\nprediction module loading...\n")
