...
async def edit_simple2(bot, chat_id, user_id, target_id, post_id, message_id, current, BASE_CHN):
    result = None
    try:
        ...
        len_ = 0
        POST_TEXT = POST_TEXT if POST_TEXT else str_empty
        reply_markup = await create_replymarkup3(target_id, post_id, POST_BUTTON) if POST_ISBUTTON else InlineKeyboardBuilder()
        ...
        if POST_ISGALLERY:
            ...
            if chat_id != user_id:
                middle = types.InlineKeyboardButton(text=f"{current}/{len_}", switch_inline_query_current_chat=f"{target_id} {post_id} ~")
            else:
                middle = types.InlineKeyboardButton(text=f"{current}/{len_}", callback_data=f'gal_current_{target_id}_{post_id}_{current}_{len_}')

            buttons = [
                types.InlineKeyboardButton(text="←", callback_data=f'gal_prev_{target_id}_{post_id}_{current}_{len_}'),
                middle,
                types.InlineKeyboardButton(text="→", callback_data=f'gal_next_{target_id}_{post_id}_{current}_{len_}'),
            ]
            reply_markup.row(*buttons)
...
async def format_text(txt):
    ...
    try:
        ...
        if not is_first_emoji and len(tmp_arr) > 1:
...
# region web
async def region_web(bot, ENT_TID, ENT_LC, POST_ID, POST_TYPE, POST_TEXT, POST_LNK, POST_BUTTON,
                     ENT_TYPE, MEDIA_D, BASE_D, bot_username, PROJECT_TYPE='bot'):
    result = None
    try:
        POST_TEXT = POST_TEXT.replace('<tg-spoiler>', '').replace('</tg-spoiler>', '')
        BASE_BOT = os.path.join(MEDIA_D, str(ENT_TID), f"{str(ENT_TID)}.db")
        ...
        if PROJECT_TYPE == 'bot':
            ...
            ENT_USERNAME = get_chat_.username if get_chat_.username else get_chat_.invite_link
            ENT_FIRSTNAME = get_chat_.title
        ...
        html_web = html_template.format(m_html, txt_html, btn_html, msg_views, ENT_USERNAME, ENT_TID, POST_ID,
                                        POST_LNKS)
        if POST_ID:
        ...
        while cnt >= 0:
            try:
                ...
                if PROJECT_TYPE == 'bot':
                    ...
                    data_bot = await db_select(sql, (int(ENT_TID),), BASE_D)
                    ENT_USERNAME, ENT_FIRSTNAME = data_bot[0]
                ...
                figure_html = ''
                telegraph_ = Telegraph()
                await telegraph_.create_account(short_name=short_name, author_name=ENT_USERNAME, author_url=author_url)
                ...
                if POST_LNK:
                    ...
                    for i in range(len(POST_LNKS)):
                        ...
                        if POST_TYPES[i] in ['video', 'video_note']:
                            figure_html = f'{figure_html}<figure><video src="{tgph_ph}" preload="auto" autoplay="autoplay" loop="loop" muted="muted"></video><figcaption>Video: {author_url}</figcaption></figure>'
                        else:
                            figure_html = f'{figure_html}<figure><img src="{tgph_ph}"/><figcaption>Photo: {author_url}</figcaption></figure>'
                ...
                page_blog = await telegraph_.create_page(title=f"📰 {ENT_FIRSTNAME}",
                                                   html_content=html_,
                                                   author_name=str(ENT_FIRSTNAME),
