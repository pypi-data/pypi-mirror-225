...
async def region_blog(bot, ENT_TID, POST_TYPE, POST_TEXT, POST_LNK, MEDIA_D, BASE_D, PROJECT_TYPE='bot', is_format=False):
    result = None
    try:
        cnt = 1
        while cnt >= 0:
            try:
                POST_TEXT = POST_TEXT.replace('<tg-spoiler>', '').replace('</tg-spoiler>', '').replace('<span>', '</span>', '')
                BASE_BOT = os.path.join(MEDIA_D, str(ENT_TID), f"{str(ENT_TID)}.db")
                os.makedirs(os.path.join(MEDIA_D, str(ENT_TID)), exist_ok=True, mode=0o777)
                await db_bot_create(BASE_BOT)
                ...
                figure_html = ''
                telegraph_ = Telegraph()
                telegraph_.create_account(short_name=short_name, author_name=ENT_USERNAME, author_url=author_url)
                ...
                page_blog = telegraph_.create_page(title=f"📰 {ENT_FIRSTNAME}",
                                                   html_content=html_,
