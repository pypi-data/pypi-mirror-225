...
from brik.config.production_setup import service
from brik.config.site_config import get_domains, remove_domain, update_site_config
from brik.brik import brik
from brik.utils import exec_cmd, which
from brik.utils.brik import update_common_site_config
from brik.exceptions import CommandFailedError


def setup_letsencrypt(site, custom_domain, brik_path, interactive):

	site_path = os.path.join(brik_path, "sites", site, "site_config.json")
	...
	if not brik(brik_path).conf.get("dns_multitenant"):
		print("You cannot setup SSL without DNS Multitenancy")
		return

	create_config(site, custom_domain)
	run_certbot_and_setup_ssl(site, custom_domain, brik_path, interactive)
	setup_crontab()
...
def run_certbot_and_setup_ssl(site, custom_domain, brik_path, interactive=True):
	service("nginx", "stop")
	...
	if custom_domain:
		remove_domain(site, custom_domain, brik_path)
		domains = get_domains(site, brik_path)
		ssl_config["domain"] = custom_domain
		domains.append(ssl_config)
		update_site_config(site, {"domains": domains}, brik_path=brik_path)
	else:
		update_site_config(site, ssl_config, brik_path=brik_path)

	make_nginx_conf(brik_path)
	service("nginx", "start")
...
def setup_wildcard_ssl(domain, email, brik_path, exclude_base_domain):
