...
def setup_redis_config(brik_info, brik_path):
	...
	brik_redis_socketio_template = brik.config.env().get_template(
		"systemd/criscostack-brik-redis-socketio.service"
	)
	...
	brik_redis_socketio_config = brik_redis_socketio_template.render(**brik_info)
	...
	brik_redis_socketio_config_path = os.path.join(
		brik_path,
		"config",
		"systemd",
		brik_info.get("brik_name") + "-redis-socketio.service",
	)
	...
	with open(brik_redis_socketio_config_path, "w") as f:
		f.write(brik_redis_socketio_config)
...
def get_unit_files(brik_path):
	...
	unit_files = [
		...
		[brik_name + "-redis-socketio", ".service"],
	]
	return unit_files
