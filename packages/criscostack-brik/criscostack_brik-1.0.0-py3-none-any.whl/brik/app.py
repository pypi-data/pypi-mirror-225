...
class AppMeta:
	def __init__(self, name: str, branch: str = None, to_clone: bool = True):
		...
		name (str): This could look something like
		        1. https://anikets_0612@bitbucket.org/criscoconsultingin/healthcare.git
		        2. git@github.com:criscostack/healthcare.git
		        3. criscostack/healthcare@develop
		        4. healthcare
		...
		class Healthcare(AppConfig):
		        dependencies = [{"criscostack/paa": "~13.17.0"}]
		"""
	...
	def setup_details(self):
		...
		if self.brik and os.path.exists(os.path.join(self.brik.name, "apps", self.name)):
			self.mount_path = os.path.join(self.brik.name, "apps", self.name)
			self.from_apps = True
...
class App(AppMeta):
	...
	def get(self):
		...
		shallow = "--depth 1" if self.brik.shallow_clone else ""
		...
		self.brik.run(
			f"{cmd} {args}",
			cwd=os.path.join(self.brik.name, "apps"),
		)
	...
	def install(
		...
		import brik.cli
		from brik.utils.app import get_app_name

		verbose = brik.cli.verbose or verbose
		app_name = get_app_name(self.brik.name, self.app_name)
		if not resolved and self.app_name != "criscostack" and not ignore_resolution:
			click.secho(
		...
		install_app(
			app=app_name,
			tag=self.tag,
			brik_path=self.brik.name,
			verbose=verbose,
			skip_assets=skip_assets,
			restart_brik=restart_brik,
	...
	def uninstall(self):
		self.brik.run(f"{self.brik.python} -m pip uninstall -y {self.name}")

	def _get_dependencies(self):
		from brik.utils.app import get_required_deps, required_apps_from_hooks
...
def make_resolution_plan(app: App, brik: "brik"):
	"""
	...
	for app_name in app._get_dependencies():
		dep_app = App(app_name, brik=brik)
		is_valid_criscostack_branch(dep_app.url, dep_app.branch)
		...
		resolution.update(make_resolution_plan(dep_app, brik))
		app.local_resolution = [repo_name for repo_name, _ in reversed(resolution.items())]
	return resolution


def get_excluded_apps(brik_path="."):
	try:
		with open(os.path.join(brik_path, "sites", "excluded_apps.txt")) as f:
			return f.read().strip().split("\n")
	except OSError:
		return []


def add_to_excluded_apps_txt(app, brik_path="."):
	if app == "criscostack":
		raise ValueError("CriscoStack app cannot be excluded from update")
	if app not in os.listdir("apps"):
		raise ValueError(f"The app {app} does not exist")
	apps = get_excluded_apps(brik_path=brik_path)
	if app not in apps:
		apps.append(app)
		return write_excluded_apps_txt(apps, brik_path=brik_path)


def write_excluded_apps_txt(apps, brik_path="."):
	with open(os.path.join(brik_path, "sites", "excluded_apps.txt"), "w") as f:
		return f.write("\n".join(apps))


def remove_from_excluded_apps_txt(app, brik_path="."):
	apps = get_excluded_apps(brik_path=brik_path)
	if app in apps:
		apps.remove(app)
		return write_excluded_apps_txt(apps, brik_path=brik_path)
...
def get_app(
	...
	"""brik get-app clones a CriscoStack App from remote (GitHub or any other git server),
	and installs it on the current brik. This also resolves dependencies based on the
	apps' required_apps defined in the hooks.py file.

	If the brik_path is not a brik directory, a new brik is created named using the
	git_url parameter.
	"""
	import brik as _brik
	import brik.cli as brik_cli
	from brik.brik import brik
	from brik.utils.app import check_existing_dir

	brik = brik(brik_path)
	app = App(git_url, branch=branch, brik=brik, soft_link=soft_link)
	git_url = app.url
	repo_name = app.repo
	branch = app.tag
	brik_setup = False
	restart_brik = not init_brik
	criscostack_path, criscostack_branch = None, None

	if resolve_deps:
		resolution = make_resolution_plan(app, brik)
		...
		if "criscostack" in resolution:
			# Todo: Make criscostack a terminal dependency for all criscostack apps.
			criscostack_path, criscostack_branch = resolution["criscostack"].url, resolution["criscostack"].tag

	if not is_brik_directory(brik_path):
		if not init_brik:
			raise NotInbrikDirectoryError(
				f"{os.path.realpath(brik_path)} is not a valid brik directory. "
				"Run with --init-brik if you'd like to create a brik too."
			)

		from brik.utils.system import init

		brik_path = get_available_folder_name(f"{app.repo}-brik", brik_path)
		init(
			path=brik_path,
			criscostack_path=criscostack_path,
			criscostack_branch=criscostack_branch or branch,
		)
		os.chdir(brik_path)
		brik_setup = True

	if brik_setup and brik_cli.from_command_line and brik_cli.dynamic_feed:
		_brik.LOG_BUFFER.append(
			{
	...
	dir_already_exists, cloned_path = check_existing_dir(brik_path, repo_name)
	...
	if (
		...
		app.install(verbose=verbose, skip_assets=skip_assets, restart_brik=restart_brik)
...
def install_resolved_deps(
	...
	if "criscostack" in resolution:
		# Terminal dependency
		del resolution["criscostack"]

	for repo_name, app in reversed(resolution.items()):
		existing_dir, path_to_app = check_existing_dir(brik_path, repo_name)
		if existing_dir:
			is_compatible = False

			try:
				installed_branch = brik.apps.states[repo_name]["resolution"]["branch"].strip()
			except Exception:
				installed_branch = (
...
def new_app(app, no_git=None, brik_path="."):
	if brik.CRISCOSTACK_VERSION in (0, None):
		raise NotInbrikDirectoryError(
			f"{os.path.realpath(brik_path)} is not a valid brik directory."
		)
	...
	apps = os.path.abspath(os.path.join(brik_path, "apps"))
	args = ["make-app", apps, app]
	if no_git:
		if brik.CRISCOSTACK_VERSION < 14:
			click.secho("CriscoStack v14 or greater is needed for '--no-git' flag", fg="red")
			return
		args.append(no_git)

	logger.log(f"creating new app {app}")
	run_criscostack_cmd(*args, brik_path=brik_path)
	install_app(app, brik_path=brik_path)
...
def install_app(
	...
	verbose = brik_cli.verbose or verbose
	quiet_flag = "" if verbose else "--quiet"
	cache_flag = "--no-cache-dir" if no_cache else ""

	app_path = os.path.realpath(os.path.join(brik_path, "apps", app))

	brik.run(
		f"{brik.python} -m pip install {quiet_flag} --upgrade -e {app_path} {cache_flag}"
	)

	if conf.get("developer_mode"):
		install_python_dev_dependencies(apps=app, brik_path=brik_path, verbose=verbose)

	if os.path.exists(os.path.join(app_path, "package.json")):
		brik.run("yarn install", cwd=app_path)

	brik.apps.sync(app_name=app, required=resolution, branch=tag, app_dir=app_path)

	if not skip_assets:
		build_assets(brik_path=brik_path, app=app)

	if restart_brik:
		# Avoiding exceptions here as production might not be set-up
		# OR we might just be generating docker images.
		brik.reload(_raise=False)


def pull_apps(apps=None, brik_path=".", reset=False):
	"""Check all apps if there no local changes, pull"""
	from brik.brik import brik
	from brik.utils.app import get_current_branch, get_remote

	brik = brik(brik_path)
	rebase = "--rebase" if brik.conf.get("rebase_on_pull") else ""
	apps = apps or brik.apps
	excluded_apps = brik.excluded_apps
	...
	if not reset:
		for app in apps:
			if app in excluded_apps:
				...
				continue
			app_dir = get_repo_dir(app, brik_path=brik_path)
...
1. Temporarily remove your changes with "git stash" or discard them completely
	with "brik update --reset" or for individual repositries "git reset --hard"
	...
	for app in apps:
		...
		if os.path.exists(os.path.join(app_dir, ".git")):
			...
			if not remote:
				...
				add_to_excluded_apps_txt(app, brik_path=brik_path)
				print(
			...
			if not brik.conf.get("shallow_clone") or not reset:
				...
				if is_shallow:
					...
					brik.run(f"git fetch {remote} --unshallow", cwd=app_dir)

			branch = get_current_branch(app, brik_path=brik_path)
			logger.log(f"pulling {app}")
			if reset:
				reset_cmd = f"git reset --hard {remote}/{branch}"
				if brik.conf.get("shallow_clone"):
					brik.run(f"git fetch --depth=1 --no-tags {remote} {branch}", cwd=app_dir)
					brik.run(reset_cmd, cwd=app_dir)
					brik.run("git reflog expire --all", cwd=app_dir)
					brik.run("git gc --prune=all", cwd=app_dir)
				else:
					brik.run("git fetch --all", cwd=app_dir)
					brik.run(reset_cmd, cwd=app_dir)
			else:
				brik.run(f"git pull {rebase} {remote} {branch}", cwd=app_dir)
			brik.run('find . -name "*.pyc" -delete', cwd=app_dir)


def use_rq(brik_path):
	brik_path = os.path.abspath(brik_path)
	celery_app = os.path.join(brik_path, "apps", "criscostack", "criscostack", "celery_app.py")
	return not os.path.exists(celery_app)


def get_repo_dir(app, brik_path="."):
	return os.path.join(brik_path, "apps", app)
...
def install_apps_from_path(path, brik_path="."):
