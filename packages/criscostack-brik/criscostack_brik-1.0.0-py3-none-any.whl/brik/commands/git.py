...
from brik.utils import set_git_remote_url
from brik.utils.app import get_remote
...
@click.command('remote-reset-url', help="Reset app remote url to criscostack official")
@click.argument('app')
def remote_reset_url(app):
	git_url = f"https://anikets_0612@bitbucket.org/criscoconsultingin/{app}.git"
	set_git_remote_url(git_url)


@click.command('remote-urls', help="Show apps remote url")
def remote_urls():
	for app in brik(".").apps:
