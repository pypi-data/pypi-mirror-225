...
from brik.config.common_site_config import update_config, put_config

# imports - third party imports
import click


@click.group(help="Change brik configuration")
def config():
	pass
...
@click.command(
	"dns_multitenant", help="Enable/Disable brik multitenancy on running brik update"
)
@click.argument("state", type=click.Choice(["on", "off"]))
...
def set_common_config(configs):
	...
	update_config(common_site_config, brik_path=".")


@click.command(
	"remove-common-config", help="Remove specific keys from current brik's common config"
)
@click.argument("keys", nargs=-1)
def remove_common_config(keys):
	from brik.brik import brik

	common_site_config = brik(".").conf
	for key in keys:
