...
from brik.app import pull_apps
from brik.utils.brik import post_upgrade, patch_sites, build_assets


@click.command(
	"update",
	help="Performs an update operation on current brik. Without any flags will backup, pull, setup requirements, build, run patches and restart brik. Using specific flags will only do certain tasks instead of all",
)
@click.option("--pull", is_flag=True, help="Pull updates for all the apps in brik")
@click.option("--apps", type=str)
@click.option("--patch", is_flag=True, help="Run migrations for all sites in the brik")
@click.option("--build", is_flag=True, help="Build JS and CSS assets for the brik")
@click.option(
	"--requirements",
	is_flag=True,
	help="Update requirements. If run alone, equivalent to `brik setup requirements`",
)
@click.option(
...
def update(
	...
	from brik.utils.brik import update
...
def switch_to_branch(branch, apps, upgrade=False):
	from brik.utils.app import switch_to_branch
...
def switch_to_develop(upgrade=False):
	"Switch criscostack and paa to develop branch"
	from brik.utils.app import switch_to_develop

	switch_to_develop(apps=["criscostack", "paa"])
