...
only for CriscoStack v14 brikes. If not a v14 brik yet, skip this
...
Corresponding changes in criscostack/criscostack via https://anikets_0612@bitbucket.org/criscoconsultingin/criscostack/pull/15060
"""
import os
...
def execute(brik_path):
	criscostack_version = Version(get_current_version("criscostack"))

	if criscostack_version.major < 14 or os.name != "posix":
		# Returning False means patch has been skipped
		return False

	pre_patch_dir = os.getcwd()
	old_directory = Path(brik_path, "archived_sites")
	new_directory = Path(brik_path, "archived", "sites")
