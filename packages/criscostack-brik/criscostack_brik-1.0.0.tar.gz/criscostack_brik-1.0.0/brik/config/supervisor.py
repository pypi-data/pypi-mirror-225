...
from brik.utils import get_brik_name, which
...
def generate_supervisor_config(brik_path, user=None, yes=False, skip_redis=False):
	"""Generate supervisor config for respective brik path"""
	if not user:
		user = getpass.getuser()

	config = brik(brik_path).conf
	template = brik.config.env().get_template("supervisor.conf")
	...
	config = template.render(
		**{
			...
			"redis_socketio_config": os.path.join(brik_dir, "config", "redis_socketio.conf"),
	...
	conf_path = os.path.join(brik_path, "config", "supervisor.conf")
	...
	update_config({"restart_supervisor_on_update": True}, brik_path=brik_path)
...
def check_supervisord_config(user=None):
	"""From brik v5.x, we're moving to supervisor running as user"""
	# i don't think brik should be responsible for this but we're way past this now...
	# removed updating supervisord conf & reload in Aug 2022 - gavin@criscostack.io
	import configparser
