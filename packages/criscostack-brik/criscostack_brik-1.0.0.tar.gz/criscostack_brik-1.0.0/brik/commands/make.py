...
@click.command("init", help="Initialize a new brik instance in the specified path")
@click.argument("path")
@click.option(
	"--version",
	"--criscostack-branch",
	"criscostack_branch",
	default=None,
	help="Clone a particular branch of criscostack",
)
@click.option(
	"--ignore-exist", is_flag=True, default=False, help="Ignore if brik instance exists."
)
@click.option(
...
@click.option("--criscostack-path", default=None, help="path to criscostack repo")
...
@click.option(
	"--no-backups",
	is_flag=True,
	help="Do not set up automatic periodic backups for all sites on this brik",
)
@click.option(
...
def init(
	...
	try:
		init(
			...
			criscostack_branch=criscostack_branch,
		...
		log(f"brik {path} initialized", level=1)
	except SystemExit:
		raise
	except Exception:
		import shutil
		...
		from brik.utils import get_traceback
...
def drop(path):
	...
	from brik.exceptions import brikNotFoundError, ValidationError

	brik = brik(path)

	if not brik.exists:
		raise brikNotFoundError(f"brik {brik.name} does not exist")

	if brik.sites:
		raise ValidationError("Cannot remove non-empty brik directory")
...
@click.command(
	["get", "get-app"],
	help="Clone an app from the internet or filesystem and set it up in your brik",
)
...
@click.option(
	"--init-brik", is_flag=True, default=False, help="Initialize brik if not in one"
)
@click.option(
	"--resolve-deps",
	is_flag=True,
...
def get_app(
	...
	"clone an app from the internet and set it up in your brik"
	from brik.app import get_app
...
@click.command("new-app", help="Create a new CriscoStack application under apps folder")
@click.option(
	"--no-git",
	is_flag=True,
	flag_value="--no-git",
	help="Do not initialize git repository for the app (available in CriscoStack v14+)",
)
@click.argument("app-name")
def new_app(app_name, no_git=None):
	from brik.app import new_app
...
@click.command(
	...
	help=(
		"Completely remove app from brik and re-build assets if not installed on any site"
	),
)
...
def remove_app(app_name, no_backup=False, force=False):
	...
	brik = brik(".")
	brik.uninstall(app_name, no_backup=no_backup, force=force)


@click.command("exclude-app", help="Exclude app from updating")
@click.argument("app_name")
def exclude_app_for_update(app_name):
	from brik.app import add_to_excluded_apps_txt
...
def include_app_for_update(app_name):
	...
	from brik.app import remove_from_excluded_apps_txt
...
@click.command(
	...
	help="For pip help use `brik pip help [COMMAND]` or `brik pip [COMMAND] -h`",
)
@click.argument("args", nargs=-1)
@click.pass_context
def pip(ctx, args):
	"Run pip commands in brik env"
	import os
