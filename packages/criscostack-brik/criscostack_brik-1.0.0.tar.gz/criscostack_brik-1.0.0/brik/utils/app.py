...
def is_version_upgrade(app="criscostack", brik_path=".", branch=None):
	upstream_version = get_upstream_version(app=app, branch=branch, brik_path=brik_path)
	...
	local_version = get_major_version(get_current_version(app, brik_path=brik_path))
...
def switch_branch(branch, apps=None, brik_path=".", upgrade=False, check_upgrade=True):
	import git
	from brik.brik import brik
	from brik.utils import log, exec_cmd
	...
	apps_dir = os.path.join(brik_path, "apps")
	...
	for app in apps:
		...
		if check_upgrade:
			version_upgrade = is_version_upgrade(app=app, brik_path=brik_path, branch=branch)
	...
	if switched_apps:
		...
		print(
			"Please run `brik update --patch` to be safe from any differences in"
			" database schema"
		)

	if version_upgrade[0] and upgrade:
		brik(brik_path).setup.requirements()
		backup_all_sites()
		patch_sites()
		build_assets()
...
def switch_to_branch(branch=None, apps=None, brik_path=".", upgrade=False):
	switch_branch(branch, apps=apps, brik_path=brik_path, upgrade=upgrade)


def switch_to_develop(apps=None, brik_path=".", upgrade=True):
	switch_branch("develop", apps=apps, brik_path=brik_path, upgrade=upgrade)
...
def get_develop_version(app, brik_path="."):
	repo_dir = get_repo_dir(app, brik_path=brik_path)
...
def get_upstream_version(app, branch=None, brik_path="."):
	repo_dir = get_repo_dir(app, brik_path=brik_path)
	if not branch:
...
def get_current_criscostack_version(brik_path="."):
	try:
		return get_major_version(get_current_version("criscostack", brik_path=brik_path))
	except OSError:
		return 0


def get_current_branch(app, brik_path="."):
...
def get_app_name(brik_path: str, folder_name: str) -> str:
	...
	"""
	app_name = None
	apps_path = os.path.join(os.path.abspath(brik_path), "apps")
...
def check_existing_dir(brik_path, repo_name):
	cloned_path = os.path.join(brik_path, "apps", repo_name)
	dir_already_exists = os.path.isdir(cloned_path)
	return dir_already_exists, cloned_path


def get_current_version(app, brik_path="."):
