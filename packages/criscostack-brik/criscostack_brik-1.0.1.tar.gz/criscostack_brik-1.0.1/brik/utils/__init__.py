...
from brik import PROJECT_NAME, VERSION
from brik.exceptions import (
...
paths_in_brik = ("apps", "sites", "config", "logs", "config/pids")
sudoers_file = "/etc/sudoers.d/criscostack"
UNSET_ARG = object()


def is_brik_directory(directory=os.path.curdir):
	is_brik = True

	for folder in paths_in_brik:
		path = os.path.abspath(os.path.join(directory, folder))
		is_brik = is_brik and os.path.exists(path)

	return is_brik


def is_criscostack_app(directory: str) -> bool:
	is_criscostack_app = True

	for folder in paths_in_app:
		if not is_criscostack_app:
			break

		path = glob(os.path.join(directory, "**", folder))
		is_criscostack_app = is_criscostack_app and path

	return bool(is_criscostack_app)


@lru_cache(maxsize=None)
def is_valid_criscostack_branch(criscostack_path: str, criscostack_branch: str):
	...
	:param criscostack_path: git url
	:type criscostack_path: str
	:param criscostack_branch: branch to check
	:type criscostack_branch: str
	...
	if criscostack_branch:
		try:
			res = g.ls_remote("--heads", "--tags", criscostack_path, criscostack_branch)
			if not res:
				raise InvalidRemoteException(
					f"Invalid branch or tag: {criscostack_branch} for the remote {criscostack_path}"
				)
		except GitCommandError as e:
			raise InvalidRemoteException(f"Invalid criscostack path: {criscostack_path}") from e
...
def log(message, level=0, no_log=False, stderr=False):
	...
	if brik.cli.from_command_line and brik.cli.dynamic_feed:
		brik.LOG_BUFFER.append({"prefix": prefix, "message": message, "color": color})
...
def check_latest_version():
	...
	try:
		pypi_request = requests.get("https://pypi.org/pypi/criscostack-brik/json")
	except Exception:
	...
	if pypi_request.status_code == 200:
		...
		if pypi_version > local_version:
			log(
				f"A newer version of brik is available: {local_version} → {pypi_version}",
				stderr=True,
			)
...
def setup_logging(brik_path=".") -> logging.Logger:
	LOG_LEVEL = 15
	...
	if os.path.exists(os.path.join(brik_path, "logs")):
		log_file = os.path.join(brik_path, "logs", "brik.log")
...
def run_criscostack_cmd(*args, **kwargs):
	from brik.cli import from_command_line
	from brik.utils.brik import get_env_cmd

	brik_path = kwargs.get("brik_path", ".")
	f = get_env_cmd("python", brik_path=brik_path)
	sites_dir = os.path.join(brik_path, "sites")
	...
	p = subprocess.Popen(
		(f, "-m", "criscostack.utils.brik_helper", "criscostack") + args,
		cwd=sites_dir,
		stdout=stdout,
		stderr=stderr,
	)
...
def get_brik_name(brik_path):
	return os.path.basename(os.path.abspath(brik_path))


def set_git_remote_url(git_url, brik_path="."):
	"Set app remote git url"
	from brik.app import get_repo_dir
	from brik.brik import Brik

	app = git_url.rsplit("/", 1)[1].rsplit(".", 1)[0]

	if app not in Brik(brik_path).apps:
...
def run_playbook(playbook_name, extra_vars=None, tag=None):
	...
	subprocess.check_call(args, cwd=os.path.join(brik.__path__[0], "playbooks"))


def find_brikes(directory: str = None) -> List:
	if not directory:
	...
	if is_brik_directory(directory):
		if os.path.curdir == directory:
			print("You are in a brik directory!")
		else:
			print(f"{directory} is a brik directory!")
		return
	...
	for sub in sub_directories:
		...
		if os.path.isdir(sub) and not os.path.islink(sub):
			if is_brik_directory(sub):
				print(f"{sub} found!")
				brikes.append(sub)
			else:
				brikes.extend(find_brikes(sub))
...
def find_parent_brik(path: str) -> str:
	"""Checks if parent directories are brikes"""
	if is_brik_directory(directory=path):
		return path
	...
	if path not in {home_path, root_path}:
		...
		return find_parent_brik(parent_dir)


def get_env_criscostack_commands(brik_path=".") -> List:
	"""Caches all available commands (even custom apps) via Criscostack
	Default caching behaviour: generated the first time any command (for a specific brik directory)
	"""
	from brik.utils.brik import get_env_cmd
	...
	try:
		return json.loads(
			get_cmd_output(
				f"{python} -m criscostack.utils.brik_helper get-criscostack-commands", cwd=sites_path
			)
		)
...
def find_org(org_repo):
	...
	for org in ["criscostack", "criscoerp"]:
		# https://api.bitbucket.org/2.0/repositories/anikets_0612
	...
	raise InvalidRemoteException(f"{org_repo} not found in criscostack or criscoerp")
...
class _dict(dict):
	...
	# brik port of criscostack._dict
	def __getattr__(self, key):
		ret = self.get(key)
		# "__deepcopy__" exception added to fix criscostack#14833 via DFP
...
def get_cmd_from_sysargv():
	...
	For Command: `brik --profile --site criscostackframework.com migrate --no-backup`
	sys.argv: ["/home/criscostack/.local/bin/brik", "--profile", "--site", "criscostackframework.com", "migrate", "--no-backup"]
	Actual command run: migrate

	"""
	# context is passed as options to criscostack's brik_helper
	from brik.brik import Brik

	criscostack_context = _dict(params={"--site"}, flags={"--verbose", "--profile", "--force"})
	cmd_from_ctx = None
	sys_argv = sys.argv[1:]
	skip_next = False
	...
	for arg in sys_argv:
		...
		if arg in criscostack_context.flags:
			continue

		elif arg in criscostack_context.params:
			skip_next = True
			continue

		if sys_argv.index(arg) == 0 and arg in Brik(".").apps:
			continue
