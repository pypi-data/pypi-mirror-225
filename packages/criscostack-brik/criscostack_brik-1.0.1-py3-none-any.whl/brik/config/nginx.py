...
from brik.utils import get_brik_name


def make_nginx_conf(brik_path, yes=False, logging=None, log_format=None):
	conf_path = os.path.join(brik_path, "config", "nginx.conf")
	...
	if allow_rate_limiting:
		template_vars.update(
			{
				"brik_name_hash": hashlib.sha256(brik_name).hexdigest()[:16],
...
def make_brik_manager_nginx_conf(brik_path, yes=False, port=23624, domain=None):
	from brik.config.site_config import get_site_config

	template = brik.config.env().get_template("brik_manager_nginx.conf")
	brik_path = os.path.abspath(brik_path)
	sites_path = os.path.join(brik_path, "sites")

	config = Brik(brik_path).conf
	site_config = get_site_config(domain, brik_path=brik_path)
	brik_name = get_brik_name(brik_path)

	template_vars = {
		"port": port,
		"domain": domain,
		"brik_manager_site_name": "brik-manager.local",
		"sites_path": sites_path,
		"http_timeout": config.get("http_timeout"),
		"webserver_port": config.get("webserver_port"),
		"socketio_port": config.get("socketio_port"),
	...
	brik_manager_nginx_conf = template.render(**template_vars)

	conf_path = os.path.join(brik_path, "config", "nginx.conf")

	if not yes and os.path.exists(conf_path):
		click.confirm(
			"nginx.conf already exists and brik-manager configuration will be appended to it. Do you want to continue?",
			abort=True,
		)

	with open(conf_path, "a") as myfile:
		myfile.write(brik_manager_nginx_conf)


def prepare_sites(config, brik_path):
	sites = {
		"that_use_port": [],
		"that_use_dns": [],
		"that_use_ssl": [],
	...
	sites_configs = get_sites_with_config(brik_path=brik_path)
...
def get_sites_with_config(brik_path):
	from brik.brik import Brik
	from brik.config.site_config import get_site_config
	...
	for site in sites:
		try:
			site_config = get_site_config(site, brik_path=brik_path)
	...
	use_wildcard_certificate(brik_path, ret)

	return ret


def use_wildcard_certificate(brik_path, ret):
	"""
	stored in common_site_config.json as:
	"wildcard": {
	        "domain": "*.criscoerp.com",
	        "ssl_certificate": "/path/to/criscoerp.com.cert",
	        "ssl_certificate_key": "/path/to/criscoerp.com.key"
	}
	"""
	from brik.brik import Brik
	...
	for site in ret:
		...
		if (site.get("domain") or site["name"]).endswith(domain):
			# example: ends with .criscoerp.com
...
def get_error_pages():
	brik_app_path = os.path.abspath(brik.__path__[0])
	templates = os.path.join(brik_app_path, "config", "templates")
