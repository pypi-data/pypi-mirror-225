...
@click.command("start", help="Start Criscostack development processes")
@click.option("--no-dev", is_flag=True, default=False)
@click.option(
	"--no-prefix",
	is_flag=True,
	default=False,
	help="Hide process name from brik start log",
)
...
def start(no_dev, concurrency, procfile, no_prefix, man):
	from brik.utils.system import start
...
def restart(web, supervisor, systemd):
	...
	Brik(".").reload(web, supervisor, systemd)
...
def set_nginx_port(site, port):
	from brik.config.site_config import set_nginx_port
...
def set_ssl_certificate(site, ssl_certificate_path):
	from brik.config.site_config import set_ssl_certificate
...
def set_ssl_certificate_key(site, ssl_certificate_key_path):
	from brik.config.site_config import set_ssl_certificate_key
...
def set_url_root(site, url_root):
	from brik.config.site_config import set_url_root

	set_url_root(site, url_root)


@click.command("set-mariadb-host", help="Set MariaDB host for brik")
@click.argument("host")
def set_mariadb_host(host):
	from brik.utils.brik import set_mariadb_host

	set_mariadb_host(host)


@click.command("set-redis-cache-host", help="Set Redis cache host for brik")
@click.argument("host")
def set_redis_cache_host(host):
	"""
	Usage: brik set-redis-cache-host localhost:6379/1
	"""
	from brik.utils.brik import set_redis_cache_host

	set_redis_cache_host(host)


@click.command("set-redis-queue-host", help="Set Redis queue host for brik")
@click.argument("host")
def set_redis_queue_host(host):
	"""
	Usage: brik set-redis-queue-host localhost:6379/2
	"""
	from brik.utils.brik import set_redis_queue_host

	set_redis_queue_host(host)


@click.command("set-redis-socketio-host", help="Set Redis socketio host for brik")
@click.argument("host")
def set_redis_socketio_host(host):
	"""
	Usage: brik set-redis-socketio-host localhost:6379/3
	"""
	from brik.utils.brik import set_redis_socketio_host
...
def download_translations():
	from brik.utils.translation import download_translations_p
...
def renew_lets_encrypt():
	from brik.config.lets_encrypt import renew_certs

	renew_certs()


@click.command("backup-all-sites", help="Backup all sites in current brik")
def backup_all_sites():
	from brik.utils.system import backup_all_sites

	backup_all_sites(brik_path=".")


@click.command(
	"disable-production", help="Disables production environment for the brik."
)
def disable_production():
	from brik.config.production_setup import disable_production

	disable_production(brik_path=".")


@click.command(
	"src", help="Prints brik source folder path, which can be used as: cd `brik src`"
)
def brik_src():
	from brik.cli import src

	print(os.path.dirname(src))


@click.command("find", help="Finds brikes recursively from location")
@click.argument("location", default="")
def find_brikes(location):
	from brik.utils import find_brikes

	find_brikes(directory=location)
...
def migrate_env(python, backup=True):
	from brik.utils.brik import migrate_env
