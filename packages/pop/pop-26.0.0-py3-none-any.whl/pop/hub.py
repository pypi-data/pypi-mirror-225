...
class Sub:
    ...
    def _prepare(self):
        ...
        if self._contract_dirs:
            self._contracts = Sub(
                hub=self._hub,
                subname=f"{self._subname}.contracts",
        ...
        if self._recursive_contract_dirs:
            ...
            self._recursive_contracts = Sub(
                hub=self._hub,
                subname=f"{self._subname}.recursive_contracts",
    ...
    def _prep_mod(self, mod: ModuleType, iface: str, bname: str):
        ...
        recursive_contracts = pop.contract.load_contract(
...
class ReverseSub(Sub):
    ...
    def __call__(self, *args, **kwargs):
        ...
        if isinstance(func, pop.contract.Contracted):
            discovered_contracted_function = func
        else:
            discovered_contracted_function = pop.contract.create_contracted(
                hub=self._hub,
                contracts=getattr(self._contracts, "_loaded", {}).values(),
                func=func,
                ref=".".join(self._refs),
                name=func.__name__,
                implicit_hub=False,
        ...
        if self._recursive_contracts and not all(
            rc in discovered_contracted_function.contracts
            for rc in self._recursive_contracts
        ):
            discovered_contracted_function.contracts.extend(
                self._recursive_contracts._loaded.values()
            )
            discovered_contracted_function._load_contracts()

        return discovered_contracted_function(*args, **kwargs)
