...
class ColumnSelector(YamlModelWithValidatorContext):
    ...
    def explode_to_cohort_patterns(self, is_exclude: bool = False) -> Union[Tuple[str, str, str, str], Tuple[None, None, None, None]]:
        """
        Returns: Tuple(source_pattern, schema_pattern, table_pattern, column_pattern)
        """
        if is_exclude:
            names = explode_fq_name(self.exclude) if self.exclude else ""
        elif self.is_raw_regex():
            names = explode_fq_name(self.regex, is_regex=True)
        else:
        ...
        def _map_column_selectors_to_metrics(simple_metric: SimpleMetricDefinition):
            ...
            for td in tag_deployments:
                for metric in td.metrics:
                    ...
                    # assume conditions are always equal as we arbitrarily update conditions for 1+ matching types
                    saved_metric.conditions = cleaned_metric.conditions
