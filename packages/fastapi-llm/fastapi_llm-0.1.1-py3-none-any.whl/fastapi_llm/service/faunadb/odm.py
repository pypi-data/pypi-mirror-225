...
from .lib.errors import FaunaException
...
class FaunaModelMetaclass(ModelMetaclass):
    def __new__(mcs, name, bases, attrs):
        ...
        if bases and issubclass(bases[0], BaseModel):
            new_cls.__doc__ = new_cls.schema_json()
        mcs.Metadata.__subclasses__.append(new_cls)
        return new_cls

    class Metadata:
        __subclasses__: List[T] = []
    ...
    async def provision(cls) -> bool:
        ...
        try:
            if not await _q(q.exists(q.collection(cls.__name__.lower()))):
                ...
                logger.info("Created collection %s", cls.__name__.lower())
            ...
            if not await _q(q.exists(q.index(cls.__name__.lower()))):
                ...
                logger.info("Created index %s", cls.__name__.lower())
