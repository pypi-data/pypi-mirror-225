...
async def pdf_loader(file: UploadFile = File(...)) -> AsyncGenerator[str, None]:
...
async def website_loader(url: str) -> AsyncGenerator[str, None]:
    """Reads a website and returns a list of strings"""

    async with ClientSession(headers=HEADERS) as session:
        urls = await sitemap(url, session)
        for url in urls:
            website = await fetch_website(url, session)
            logger.info(f"Loaded {url}")
            logger.debug(f"Loaded {website}")
            yield website
