...
class Client:
    ...
    def __init__(self, patch_context, as_tenant=None):
        ...
        self._ssl_context = ssl._create_unverified_context() if patch_context.allow_insecure  else ssl.create_default_context()
        self._urlopen = lambda url, **kwargs: urllib.request.urlopen(url, context=self._ssl_context, **kwargs)
