...
def create_emr_cluster(
    ...
    terminate_cluster_idle_seconds: Optional[int] = None,
) -> str:
    """
    ...
    :param terminate_cluster_idle_seconds : Idle seconds to wait before terminating the cluster
    ...
    try:
        ...
        if terminate_cluster_idle_seconds:
            payload["AutoTerminationPolicy"][
                "IdleTimeout"
            ] = terminate_cluster_idle_seconds
...
python3 -m pip install {Software.jsl_lib.pypi_name}=={settings.raw_version_jsl_lib}

__installation_script__
sudo bash -c "mkdir -p /usr/lib/spark/jars; cp $HOME/.johnsnowlabs/java_installs/*.jar /usr/lib/spark/jars/"
...
"""
    installation_script = ""
    if license:
        installation_script += f"python3 -c \"from johnsnowlabs import nlp;nlp.install(med_license='{secrets.HC_LICENSE or ''}',aws_access_key='{secrets.AWS_ACCESS_KEY_ID or ''}',aws_key_id='{secrets.AWS_SECRET_ACCESS_KEY or ''}', spark_nlp={spark_nlp}, nlp={nlp}, visual={visual}, hardware_platform='{hardware_platform}')\""
    else:
        installation_script += f"python3 -c \"from johnsnowlabs import nlp;nlp.install(browser_login=False, nlp=False, hardware_platform='{hardware_platform}')\""
    ...
    bootstrap_bucket = create_emr_bucket(
        sts_client=sts_client, s3_client=s3_client, bucket_name=bootstrap_bucket
    )
