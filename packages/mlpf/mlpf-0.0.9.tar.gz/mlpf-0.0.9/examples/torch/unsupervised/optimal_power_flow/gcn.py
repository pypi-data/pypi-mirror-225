...
def main():
    ...
    loss_component_lambdas: dict = {
        MeanActivePowerError.__name__: 1.0,
        MeanReactivePowerError.__name__: 1.0,
        MeanUpperVoltageError.__name__: 1.0,
        MeanLowerVoltageError.__name__: 1.0,
        MeanUpperActivePowerError.__name__: 1.0,
        MeanLowerActivePowerError.__name__: 1.0,
        MeanUpperReactivePowerError.__name__: 1.0,
        MeanLowerReactivePowerError.__name__: 1.0
    ...
    for epoch in range(num_epochs):
        ...
        for batch in train_loader:
            ...
            loss = batch_metrics[MeanActivePowerCost.__name__] + sum([torch.abs(batch_metrics[key]) * loss_component_lambdas[key] for key in loss_component_lambdas.keys()])
            loss.backward()
