...
from mlpf.data.conversion.numpy.power_flow import extract_power_arrays, extract_voltage_arrays
from mlpf.data.conversion.torch.optimal_power_flow import extract_voltage_limits_tensors, extract_active_power_limits_tensors, \
    extract_reactive_power_limits_tensors, extract_demand_tensors, extract_cost_coefficients_tensor
from mlpf.data.conversion.torch.power_flow import extract_power_tensors, extract_voltage_tensors
...
def get_bound_errors_numpy(ppc):
    ...
    voltage_magnitudes, _ = extract_voltage_arrays(ppc)
    ...
    voltage_upper_errors = bounds_numpy.upper_bound_errors(voltage_magnitudes, voltages_max)
    voltage_lower_errors = bounds_numpy.lower_bound_errors(voltage_magnitudes, voltages_min)
...
def get_bound_errors_torch(ppc):
    ...
    voltage_upper_errors = bounds_torch.upper_bound_errors(voltage_magnitudes, voltages_max)
    voltage_lower_errors = bounds_torch.lower_bound_errors(voltage_magnitudes, voltages_min)
...
def get_cost_difference_torch(ppc):
    ...
    active_power_demands, reactive_power_demands = extract_demand_tensors(ppc, dtype=torch.float64)
...
class TestOptimalPowerFlowLoss(unittest.TestCase):
    ...
    def test_case118_uniform(self):
        ...
        tolerance = 1e-8
        for ppc in tqdm(ppc_list, desc="Checking costs and bounds"):
