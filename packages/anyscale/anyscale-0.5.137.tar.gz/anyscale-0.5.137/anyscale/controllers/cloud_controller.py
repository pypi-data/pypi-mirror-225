...
class CloudController(BaseController):
    ...
    def edit_aws_cloud(  # noqa: PLR0912
        self,
        *,
        cloud_name: Optional[str],
        cloud_id: Optional[str],
        ...
        The editable fields are in EditableCloudResource.
        Steps:
        ...
        if cloud_resource is None:
            raise ClickException(
                f"[Error] Cloud {cloud_name}({cloud_id}) does not contain resource records."
            )
        # Log edit details.
        edit_details = {
            "aws_s3_id": aws_s3_id,
        ...
        self.log.info(
            "[Reminder] If the old resources are being used by any running workload, you may not want to delete the old resources."
        )
        try:
            boto3_session = boto3.Session(region_name=cloud.region)
            ...
            raise ClickException(
                f"[Error] Failed to get the mount target IP for new aws_efs_ip {aws_efs_id}, please make sure it has mount targets."
            ) from e

        # Static Verify.
        self.log.info(
            "[Reminder] Before moving on to edit cloud resources, please check the following cloud verification results to decide whether you want to proceed with the edit. Start cloud static verification..."
        )
        ...
        if not self.verify_aws_cloud_resources(
            ...
            raise ClickException(
                "[Error] Cloud verification failed. Please check the verification results above, fix them, and try again."
            )
        confirm(
            "[Confirm] Please make sure you checked the verification warnings above, are you sure you want to edit these cloud resource? ",
            yes,
        )
        ...
        try:
            ...
            self.log.error(str(e))
            raise ClickException(
                "[Error] Edit cloud resource failed! The backend server might be under maintainance right now, please reach out to support or your SA for assistance."
            ) from e
        ...
        if len(functions_to_verify) > 0:
            ...
            if not functional_verify_succeed:
                raise ClickException(
                    "[Error] Cloud functional verification failed. Please consider the following options:\n"
