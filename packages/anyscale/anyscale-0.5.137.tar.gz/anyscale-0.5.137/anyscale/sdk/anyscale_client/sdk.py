...
from anyscale.sdk.anyscale_client.models.job_status import JobStatus
...
from anyscale.cli_logger import BlockLogger, pad_string
...
class AnyscaleSDK(DefaultApi):  # type: ignore
    ...
    @deprecated("`get_production_job_logs()` is deprecated. "
                "Use `fetch_production_job_logs()` to retrieve the logs string instead.")
    ...
    def fetch_production_job_logs(self, job_id: str) -> str:
        ...
        """
        job: ProductionJob = self.get_production_job(job_id).result
        get_job_logs_fns: List[Callable[..., str]] = [
            *(
                [lambda: asyncio.run(_get_logs_from_running_production_job(self, job_id))] if job.state.current_state == HaJobStates.RUNNING else []
            ),
