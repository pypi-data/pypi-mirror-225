...
def print_factors(A, shape, keys=None, sym=False, latex=True):
    ...
    for i in range(len(A)):
        ...
        if latex:
            output = "\n".join((output, f"N_{{{keys[i]}}} &= {coef} {sp.latex(simplify(expr))} \\\\"))
...
def _make_shapes(A, terms, keys=None, sym=False):
    ...
    for i in range(len(A)):
        ...
        output.update({keys[i]: simplify(sp.factor(exp))})

    return output

def simplify(shape, variables=None):
    import sympy as sp
...
class Quadrilateral:
    ...
    def internal_nodes(self):
        return {k: coord for k,coord in self.nodes.items() if 1.0 not in coord and -1.0 not in coord}
    ...
    def print_shape(self, latex=True):
        A = self.vandermonde()
        f = lambda x,y: [eval(t,dict(x=x,y=y)) for t in self.terms]
        return print_factors(A, f, keys=list(self.nodes.keys()), latex=latex)
