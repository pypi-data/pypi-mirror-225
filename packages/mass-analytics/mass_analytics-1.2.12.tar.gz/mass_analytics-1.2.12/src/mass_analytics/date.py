...
def daily_to_monthly(df, date_col, categorical_cols=None, agg=None):
    ...
    df_copy[date_col] = df_copy[date_col].dt.to_period('M').dt.to_timestamp()
...
def weekly_to_daily(df, date_col, categorical_cols=None, input_starting=True, agg=None):
    ...
    if categorical_cols is not None:
        ...
        categorical_cols.append(date_col)
        return new_df.groupby(by=categorical_cols).agg(aggregation).reset_index()
    else:
        return new_df
...
def monthly_to_daily(df, date_col, categorical_cols=None, agg=None):
    ...
    for ind,date in enumerate(df[date_col]):
        df_temp = pd.DataFrame({date_col: pd.date_range(date, date + pd.offsets.MonthEnd())})
        for num in num_cols:
            df_temp[num] = pd.Series([df.iloc[ind][num] / df_temp.shape[0] ] * df_temp.shape[0])
        for cat in all_cat_cols:
            df_temp[cat] = pd.Series([df.iloc[ind][cat]] * df_temp.shape[0])
        new_df = new_df._append(df_temp, ignore_index=True)
