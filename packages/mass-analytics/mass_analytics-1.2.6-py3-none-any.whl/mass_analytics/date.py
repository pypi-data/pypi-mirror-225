...
def daily_to_weekly(df, date_col, weekday, categorical_cols=[], output_starting=True, agg=None):
    ...
    if output_starting:
        df[date_col] = df[date_col].apply(lambda x: x - pd.to_timedelta(str(x.weekday())+' D') + pd.to_timedelta(str(day)+' D') if - x.weekday() + day <= 0 else x - pd.to_timedelta(str(x.weekday() + 7)+' D') + pd.to_timedelta(str(day)+' D'))
    else:
        df[date_col] = df[date_col].apply(lambda x: x - pd.to_timedelta(str(x.weekday() - 7)+' D') + pd.to_timedelta(str(day)+' D') if - x.weekday() + day <= 0 else x - pd.to_timedelta(str(x.weekday())+' D') + pd.to_timedelta(str(day)+' D'))
    
    categorical_cols.append(date_col)
    
    return df.groupby(by=categorical_cols, as_index=False).agg(aggregation)
