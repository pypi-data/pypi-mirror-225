"""
应用JAVA程序来做空间矫正和血流灌注重建的处理西门子老数据4延迟数据版本
2022-05-08 模块建立
姚泽山
"""
...
# 西门子老数据4延迟处理
class AnImageDataSorterSiemens4:
    ...
    def generate_from_raw_folder(self, series_dic):
        ...
        self._make_json()
        seriesm0 = series_dic["M0"][0]
        m0keys = sorted(list(seriesm0.keys()))
        for i in range(1, len(seriesm0), 1):
            shutil.copyfile(seriesm0[m0keys[i]], os.path.join(self.intermediate_folder, "java_temp",
                                                              "reordered", "temp_m0_dcm_{:04d}.dcm".format(i)))
        save_idx = 0
        for j in range(len(series_dic["Control"])):
            seriesctrl = series_dic["Control"][j]
            ctrlkeys = sorted(list(seriesctrl.keys()))
            for i in range(1, len(seriesctrl), 1):
                shutil.copyfile(seriesctrl[ctrlkeys[i]], os.path.join(self.intermediate_folder, "java_temp",
                                                                      "reordered",
                                                                      "temp_ctrl_dcm_{:04d}.dcm".format(save_idx)))
                save_idx += 1

        save_idx = 0
        for j in range(len(series_dic["Label"])):
            serieslbl = series_dic["Label"][j]
            lblkeys = sorted(list(serieslbl.keys()))
            for i in range(1, len(serieslbl), 1):
                shutil.copyfile(serieslbl[lblkeys[i]], os.path.join(self.intermediate_folder, "java_temp",
                                                                    "reordered",
                                                                    "temp_lbl_dcm_{:04d}.dcm".format(save_idx)))
                save_idx += 1
        ...
        if self.delay_num == 4:  # TODO 增加CBF2-1的输出
    ...
    def _3d_dcm_to_2d_and_nii(self, input_name, series_description, series_id, scaling):
        ...
        if len(output) == 0:
            ...
            slice_loc = output[0]
        output = DCMUtil.iterative_seq_tag_search(self.ref_image_data, "00180050", [])  # thickness
    ...
    def siemens_data_check(self, seriesm0):
        ...
        if "MOSAIC" in self.ref_image_data[0x0008, 0x0008].value:
            ...
            self.data_type = "4delay_3d"
            self.slice_num = len(seriesm0[0])-1
            print("该pCASL序列是3D文件（非mosaic）")

    def Sorter(self, intermediate_folder, series_dic, delay_time, delay_rep, debugMode=False):
        # print(json.dumps(series_dic, sort_keys=True, indent=4))
        ...
        self.siemens_data_check(series_dic["M0"])

        # 对control与label序列进行排序；按照延迟时间排序
        series_dic['Control'] = sorted(series_dic['Control'], key=lambda x: x[0][1], reverse=False)
        series_dic['Label'] = sorted(series_dic['Label'], key=lambda x: x[0][1], reverse=False)
        self.generate_from_raw_folder(series_dic)
