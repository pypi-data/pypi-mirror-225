...
class GearExchangeDB:
    ...
    def connect(self):
        ...
        curs = conn.cursor()
        curs.execute(
            "CREATE TABLE IF NOT EXISTS properties(name text, value text, "
            "PRIMARY KEY(name))"
        )
        curs.execute(
            """CREATE TABLE IF NOT EXISTS manifests(
            filename text, folder text, name text, version text, geardoc text, "
    ...
    def update(self):
        ...
        archive_path = None
        resp = requests.get(EXCHANGE_URL, headers=request_headers, timeout=30)
                    ...
                    with manifest_fs.opendir(subdir_name) as subdir_fs:
                        for entry in subdir_fs.scandir("/", namespaces=["basic"]):
                            ...
                            if not curs.fetchone():
                                ...
                                curs.execute(
                                    "INSERT INTO manifests(filename, folder, name, "
                                    "version, geardoc) VALUES(?, ?, ?, ?, ?)",
                                    (
                                        filename,
                                        subdir_name,
                                        manifest["gear"]["name"],
