...
'''.format(self._settings.readSettingAsString("stationName"),
    ...
    def _reportHTML(self):
        ...
        srcAssetsPath = Path(self._parent.appPath) / "assets"
        copytree(srcAssetsPath, self._assetsPath)
...
'''.format(df.loc['4', 'B'], df.loc['4', 'D'], self._totalDays, df.loc['5', 'B'])
        ...
        origLogoName = df.loc['2', 'C']
        origLogo = QPixmap(origLogoName)
        destPath = self._imgPath / origLogoName
        origLogo.save(destPath.name)
        logoPath = Path("assets/img/logo") / origLogoName
