...
class BaseLabelme(metaclass=SingletonMeta):
    ...
    def bhattacharyya_filter_labelme(self, parameter):
        ...
        for index, dataset in tqdm(enumerate(self.datasets)):
            ...
            for j in range(len(output_idx)-1, len(output_idx)-101, -1):
                if j < 0:
                    break
                similarity1 = histogram_lists[index].dot(histogram_lists[output_idx[j]]) / (np.linalg.norm(histogram_lists[index]) * np.linalg.norm(histogram_lists[output_idx[j]]))  # 余弦相似度
    ...
    def mosaic_zy(self, index, s=640):
        min_offset_x = np.random.uniform(0.2, 0.8)
        min_offset_y = np.random.uniform(0.2, 0.8)

        indices = [index] + np.random.choice(len(self.datasets), 3, replace=False)
        ...
        for idx in indices:
            image = cv2.imread(self.datasets[idx].get('full_path'))
            shapes = self.datasets[idx].get('labelme_info').get('shapes')
            ...
            for shape in shapes:
                ...
                x1_new = max(x1 - image.shape[1] + dx, dx)
                y1_new = max(y1 - image.shape[0] + dy, dy)
                x2_new = min(x2 - image.shape[1] + dx, dx2)
                y2_new = min(y2 - image.shape[0] + dy, dy2)

                if (x1_new >= dx2) or (y1_new >= dy2) or (x2_new <= dx) or (y2_new <= dy):
                    continue

                bbox_new = [[x1_new, y1_new], [x2_new, y2_new]]
                shape['points'] = bbox_new
                shapes_mosaic.append(shape)

                img = image[y1:y2, x1:x2]
                new_dx2 = min(img.shape[1], dx2 - dx)
                new_dy2 = min(img.shape[0], dy2 - dy)

                if idx == 0:
                    new_image[split_point_y - new_dy2:split_point_y, split_point_x - new_dx2:split_point_x] = img[:new_dy2, :new_dx2]
                else:
                    new_image[split_point_y:split_point_y + new_dy2, split_point_x:split_point_x + new_dx2] = img[:new_dy2, :new_dx2]
