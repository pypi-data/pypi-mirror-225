    ...
    async def __await_impl__(self) -> None:
        match await self.receive():
            case {"type": "lifespan.startup"}:
                ...
                return await shield(startup_task)
            case {"type": "lifespan.shutdown"}:
                shutdown_task: Task[None] = create_task(self.on_shutdown(self.events.shutdown_events))
                return await shield(shutdown_task)

    async def on_startup(self, startup_events: MutableSet[AnyCallable]) -> None:
        try:
            if startup_events:
                await CancellableGather(map(ensure_async, startup_events))
        except BaseException:
            exc_info: str = format_exc()
            await self.send({"type": "lifespan.startup.failed", "message": exc_info})
            raise
        else:
            await self.send({"type": "lifespan.startup.complete"})

    async def on_shutdown(self, shutdown_events: MutableSet[AnyCallable]) -> None:
        try:
            if shutdown_events:
                await CancellableGather(map(ensure_async, shutdown_events))
        except BaseException:
            exc_info: str = format_exc()
            await self.send({"type": "lifespan.shutdown.failed", "message": exc_info})
            raise
        else:
