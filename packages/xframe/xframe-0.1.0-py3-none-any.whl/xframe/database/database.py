...
from xframe.database.interfaces import YAMLInterface
...
VTK_saver = VTKInterface
PDB_loader = PDBInterface
HDF5_access = HDF5Interface
YAML_access = YAMLInterface
OpenCV_access = OpenCVInterface
...
class default_DB(FileAccess):
    ...
    def __init__(self,**folders_files):
        ...
        self.access_methods={
            'file://':{
                ...
                'cv':OpenCV_access,
                self.default_data_type_str:HDF5_access
            },
            'pdb://':{
    ...
    def load(self,name,**kwargs):        
        try:
            ...
            if isinstance(loader,bool) or kwargs.get('skip_custom_methods',False):
                ...
                path = self.remove_access_method(path)
            #log.debug(loader)
            #log.debug(path)
    ...
    def save(self,name,data,**kwargs):
        try:            
            ...
            if isinstance(saver,bool) or kwargs.get('skip_custom_methods',False):
                ...
                if isinstance(path,bool):
                    path = name
                #log.info('saving path = {}'.format(path))
    ...
    def remove_access_method(self,path):
    ...
    def load_settings(self,name,plugin_path='',worker_name='',settings_file_name='',target='',direct_path = False):
        loader = self.access_methods['file://']['yaml'].load
        s_load = SettingsLoader(loader)
        settings,defaults,loader_sub_defaults = s_load.load_settings_files(plugin_path,worker_name,settings_file_name,target=target,direct_path = direct_path)
...
class SettingsLoader:
    ...
    def get_settings_path(self,folders,settings_file_name):
        ...
        if isinstance(settings_path,bool):
            log.warning('The non of the possible settings files {} exist. Try to start with unchanged default settings.'.format(settings_paths))
    ...
    def get_default_settings_path(self,folder,input_version=False,name = '',warn = False):
        ...
        if len(default_paths)>0:
            ...
            if warn:
                log.warning(f'No defaults found at {search_expression} . Trying to continue with empty default.')
            default_path = False
        return default_path

    def generate_load_sub_defaults(self,settings_version):
        folder = str(self.settings_folders[2])        
        def load(name):
    ...
    def load_settings_files(self,plugin_path,worker_name,settings_file_name,target = 'analysis',direct_path = False):
        self.settings_folders = [pjoin(plugin_path ,'settings/'+target+'/'+worker_name+'/'),
                                 pjoin(plugin_path ,'settings/'+target+'/'),
                                 pjoin(plugin_path ,'settings/'),
                            ]
        if not isinstance(direct_path,str):
            settings_folders = self.settings_folders
            settings_path = self.get_settings_path(settings_folders,settings_file_name)
