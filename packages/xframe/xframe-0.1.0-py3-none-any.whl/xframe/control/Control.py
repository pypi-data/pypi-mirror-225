...
class Controller:
    def __init__(self,analysis_worker=False,experiment_worker=False):       
        self._analysis_worker = analysis_worker
        self._experiment_worker = experiment_worker
        self.control_worker=ControlWorker(analysis_worker = analysis_worker,experiment_worker=experiment_worker)        
    ...
    @analysis_worker.setter
    def analysis_worker(self,worker):
        try:
            ...
            self.control_worker.analysis_worker = worker
    ...
    def experiment_worker(self, worker):
        try:
            assert isinstance(worker,ExperimentWorkerInterface),'experiment_worker has to inherit from {} but the given object {} does not.'.format(ExperimentWorkerInterface,worker)
    ...
    def startAnalysis(self,oneshot=False,restart_control_worker=False):
        if restart_control_worker:
            # doing this restart allows the control_workers to reload settings and change from the default
            # ones to the settings provided by the analysis worker. (e.g for profiling)
            self.control_worker.restart_working()
        try:
            assert isinstance(self._analysis_worker,AnalysisWorker),'analysis_worker has to inherit from {} but the given object {} does not.'.format(AnalysisWorker,self._analysis_worker)
            job=self.chooseJob()
            print('\n ------- Start {} ------- \n'.format(job))
            result=job.start_working()
            print('\n ------- Finished {} ------- \n'.format(job))
    ...
    def chooseJob(self):
        n_running_processes = 1 #get_global_number_of_processes()
        if n_running_processes==1:
            job=self.chooseJobSingleProcess()
        else:
            job=self.chooseJobMultiNode()
        return job
            
    def chooseJobSingleProcess(self):
        self.job=self.analysis_worker
        return self.job
    
    def chooseJobDualProcess(self):
        pass
    def chooseJobMultiProcess(self):
        pass
    def chooseJobMultiNode(self):
        pass

    
class ControlWorker:
    def __init__(self,analysis_worker=False,experiment_worker=False,start_working=True):
    ...
    def work(self,**kwargs):
        q = kwargs['queue']
        run_profiling=settings.analysis.get('profiling',DictNamespace(enable=False)).enable
        if run_profiling:
            process_id = settings.analysis.profiling.gpu_worker_id
