...
from xframe.Multiprocessing_interfaces import OpenClInterface,PsutilInterface
import traceback
import numpy as np
...
openCL_plugin = OpenClInterface
psutil_plugin = PsutilInterface
...
def _read_number_of_processes(n_processes):
    ...
    assert n_processes > 0,'n_processes is 0, abbort MP evaluation.' 
    return n_processes
...
def process_mp_request(function,
    ...
    log.info('Starting {} processes.'.format(n_processes))
...
def get_number_of_gpus():
    n_gpus = openCL_plugin.get_number_of_gpus()
    return n_gpus
def create_openCL_context():
    try:
        openCL_plugin.create_context()
    except Exception as e:
        if get_process_name()!=0:
            log.info('Could not create openCL context.')
            sys.exit()
        else:
            raise e
...
#            log.info('trying to delete shared memory object {} P {}'.format(self.name,get_process_name()))
            try:
                self.shm.close()
                #log.info('shared memory object {} is beeing closed P {}'.format(self.name,get_process_name()))
...
class GpuProcessHandle:
    ...
    def create_client_function(self,put_to_queue):
        ...
        input_arrays = tuple(_input.bind().array for _input in self.inputs)
        output_arrays = tuple(_output.bind().array for _output in self.outputs)
        copyto = np.copyto
        copy = np.array
    ...
    def __del__(self):
        #log.info('trying to delete gpu handle in P {}'.format(get_process_name()))
...
class GpuProcessManager:
    @staticmethod
    def request_gpu_handle_client(target_name,gpu_process):
        ...
        gpu_process_exists = client_queue.get(timeout=60.0) #tries for 5 seconds 
        ...
        if not gpu_process_exists:
            ...
            gpu_creation_done = client_queue.get(timeout=60.0) #tries for 5 seconds 
    ...
    def __init__(self):
        ...
        self.client_handles = [] # collects handles in individual client processes so they are not garbage collected.
    ...
    def process_create_gpu_function_request(self,python_process_id,gpu_process_data):         
    ...
    def add_gpu_process(self,gpu_process : openCL_plugin.ClProcess):
    ...
    def register_client_function(self,gpu_handle):
        ...
        self.client_handles.append(gpu_handle)
    ...
    def register_server_function(self,_hash,python_process_id):
        ...
        #log.info('added server handle for gpu_process {} on cpu_process{}.'.format(handle.gpu_process.name,get_process_id()))
    ...
    def __del__(self):
        #log.info('deleting gpu process manager')
