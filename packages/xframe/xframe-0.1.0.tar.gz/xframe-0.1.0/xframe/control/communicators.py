...
class SingleProcessCommunictionAnalysis(ComInterfaceAnalysis):
    ...
    def add_gpu_process(self,gpu_process):
        ...
        #log.info('selected control worker = {}'.format(control_worker.name))
        handle = Multiprocessing.GpuProcessManager.request_gpu_handle_client(control_worker.name,gpu_process)
        self.control_worker.gpu_manager.register_client_function(handle)
        return handle.run
