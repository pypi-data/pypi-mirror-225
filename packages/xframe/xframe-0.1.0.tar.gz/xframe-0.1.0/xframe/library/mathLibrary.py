...
gsl = GSLInterface
shtns = SphericalHarmonicTransformInterface
Soft = SoftInterface
leg_trf = DiscreteLegendreTransform_interface
nj = False
PeakDetector = PeakDetectorInterface

def __setattr__(module_self,name:str,value):
    try:
        if name == 'gsl':
            assert isinstance(value,GSLInterface),'provided gsl={} does not impplement {}'.format(value,GSLInterface)
        elif name == 'shtns':
            assert isinstance(value,SphericalHarmonicTransformInterface), 'provided shtns={} does not implement {}'.format(value,SphericalHarmonicTransformInterface)
        elif name == 'Soft':
            assert isinstance(value,SoftInterface), 'provided Soft={} does not implement {}'.format(value,SoftInterface)
...
#            log.info('point {} < radius {} = {}'.format(point,radius,point[0]<radius))
    ...
    @classmethod
    def get_polygon_function(cls,radius,n_corners,amplitude_function=lambda points: np.full(points.shape[:-1],1),coordSys='spherical',center = (0,0) ):
        corner_vectors=[]
        for n in range(n_corners):
            phi=2*np.pi/n_corners*n
...
#            log.info('Point {} is in polygon: {}'.format([r*np.cos(phi),r*np.sin(phi)],is_in_polygon))
        ...
        isInPolygon=cls.select_fct_by_coord_sys(coordSys,[isInPolygon_cartesian,isInPolygon_polar])
        polygon_function=cls.get_shape_function(isInPolygon,amplitude_function)
        return polygon_function
        
    @classmethod
    def get_tetrahedral_function(cls,radius,amplitude_function=lambda points: np.full(points.shape[:-1],1),coordSys='spherical',center=np.zeros(3), orientation=np.eye(3)):
