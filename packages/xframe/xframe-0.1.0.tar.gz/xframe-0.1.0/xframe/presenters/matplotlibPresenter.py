...
from xframe.presenters.interfaces import MatplotlibInterface,MPLToolkitInterface


matplotlib = MatplotlibInterface
mpl_toolkits= MPLToolkitInterface
plt = MatplotlibInterface
inset_locator = MPLToolkitInterface
...
class heat2D:
    ...
    def get_fig(cls,data,grid=False,layout={},scale='lin',vmin = False,vmax =False,cmap='inferno'):   
        ...
        if scale=='log':
            ...
            heatmap=ax.pcolormesh(x,y,data,norm = norm,cmap=cmap)
        else:
        ...
        if scale=='log':
            ...
            for ax_row,dataset_row in zip(axes,datasets):
                ...
                for ax,data in zip(ax_row,dataset_row):
                    ...
                    #    vmax = data.min()
                    heatmap_row.append(ax.pcolormesh(x,y,data,norm=matplotlib.colors.LogNorm(),cmap=cmap,vmin=vmin,vmax=vmax))
                    ax.set_aspect('equal')
            ...
            for ax_row,dataset_row in zip(axes,datasets):
                heatmap_row = []
                for ax,data in zip(ax_row,dataset_row):
                    heatmap_row.append(ax.pcolormesh(x,y,data,cmap=cmap,vmin=vmin,vmax=vmax))
                heatmaps.append(heatmap_row)
        #fig.colorbar(heatmap)
...
class imshow:
    ...
    def get_fig(cls,data,layout={},scale='lin',vmin = False,vmax =False,cmap = 'inferno',aspect = 1):     
        ...
        if scale=='log':
            cmap.set_bad(cmap(0))
            heatmap=ax.imshow(data,norm=matplotlib.colors.LogNorm(),cmap=cmap,vmin=vmin,vmax=vmax,aspect = aspect)
        else:
    ...
    def get_fig(cls,densities,grid,print_mask,shape = (1,1),layouts=[{'x_label':r'$q_x \quad [\AA^-1] $','y_label':r'$q_y \quad [\AA^-1]$'}],scale='log',vmin=None,vmax=None):
        ...
        for id,density in enumerate(densities):
            ...
            for m_id in range(len(grid)):
                ...
                heatmap = ax.pcolormesh(x, y, c, norm=norm ,cmap = cmap,vmin=vmin,vmax=vmax)
                ax.set_aspect(1)
