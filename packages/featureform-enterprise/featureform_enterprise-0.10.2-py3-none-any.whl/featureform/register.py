...
class StreamResource:
    ...
    def register(self):
        # TODO: determine if we can rely on having a registrar pointer from one of the
        # constructor args to avoid calling into the global registrar this way.
        register_stream(
            name=self.name,
            entity=self.entity,
            type=self.type,
...
def entity(cls):
    ...
    **Examples**
    ...
    for attr_name in cls.__dict__:
        if isinstance(cls.__dict__[attr_name], (ColumnResource, StreamResource)):
...
register_stream = global_registrar.register_stream
