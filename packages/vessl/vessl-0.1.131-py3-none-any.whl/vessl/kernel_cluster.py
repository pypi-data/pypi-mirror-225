...
class CreateClusterParam:
    ...
    - Optional
        ...
        agent_local_storage_name(str): Name of local storage class for agent.
    ...
    def __init__(
        ...
        self.agent_local_storage_name = agent_local_storage_name
...
def create_cluster(
    ...
    cluster_install_command = (
        ...
        .with_agent_local_storage_class_name(param.agent_local_storage_name)
        .set_local_path_provisioner_enabled(param.enable_local_path_provisioner)
        .with_helm_values(param.extra_helm_values)
        .set_pod_resource_path(param.pod_resource_path)
        .build()
    )

    stream = os.popen(" ".join(cluster_install_command))
    output = stream.read()
    if stream.close() != None:
        raise VesslRuntimeException(f"Failed to install VESSL agent: {e}")
