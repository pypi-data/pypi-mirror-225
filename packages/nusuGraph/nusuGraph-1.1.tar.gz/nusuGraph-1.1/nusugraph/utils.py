...
class HtmlToNodesParser(HTMLParser):
    ...
    def handle_starttag(self, tag, attrs_list):
        if tag not in ALLOWED_TAGS:
            raise Exception(f"{tag!r} tag is not allowed")
    ...
    def handle_endtag(self, tag):
        ...
        if not len(self.parent_nodes):
            raise Exception(f"{tag!r} missing start tag")
        ...
        if last_node["tag"] != tag:
            raise Exception(
                f"{tag!r} tag closed instead of {last_node['tag']!r}")
    ...
    def get_nodes(self):
        if self.parent_nodes:
            ...
            raise Exception(f"{not_closed_tag!r} tag is not closed")
