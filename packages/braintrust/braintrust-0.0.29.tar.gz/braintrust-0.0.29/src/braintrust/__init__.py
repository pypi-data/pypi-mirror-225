...
import autoevals  # This re-exports the package
import git
import requests
...
class Project(ModelWrapper):
    ...
    def __init__(self, name):
        ...
        existing = []
        if unique_key:
            existing = conn.get_json(self._ENDPOINT, unique_key)

        if not existing:
            existing = conn.post_json(self._ENDPOINT, unique_key)
...
def init(
    ...
    _state.current_experiment = Experiment(
        _state.current_project,
        name=experiment,
...
class Experiment(ModelWrapper):
    ...
    def __init__(
        ...
        if base_experiment is not None:
            resp = conn.get(
                "experiments",
                params={"project_id": project.id, "name": base_experiment},
            )
            resp.raise_for_status()
        ...
        if base_exp_id is None:
            resp = conn.post(
                "experiments-by-commits",
                json={
                    "project_id": project.id,
                    "commits": list(get_past_n_ancestors()),
                },
            )
            resp.raise_for_status()
            base_exp_id = resp.json()["experiment_id"]
        ...
        self.data = conn.post_json("register-experiment", args)[0]
    ...
    def log(
        ...
        :param allow_large: (Optional) Allow large objects (greater than 64KB) to be uploaded. Proceed with caution, as they may cause issues with the
        ...
        if not allow_large:
            for k, v in args.items():
                if len(json.dumps(v)) >= 65535:
                    raise ValueError(f"Events must be less than 64KB in size. Please reduce the size of your {k}.")
...
class Dataset(ModelWrapper):
    ...
    def log(self, input, output, metadata=None, id=None, allow_large=False):
        """
        ...
        if not allow_large:
            for k, v in args.items():
                if len(json.dumps(v)) >= 65535:
                    raise ValueError(f"Records must be less than 64KB in size. Please reduce the size of your {k}.")
    ...
    def summarize(self, summarize_data=True):
        ...
        project_url = f"{API_URL}/app/{urllib.parse.quote(ORG_NAME)}/p/{urllib.parse.quote(self.project.name)}"
        dataset_url = f"{project_url}/d/{urllib.parse.quote(self.name)}"
