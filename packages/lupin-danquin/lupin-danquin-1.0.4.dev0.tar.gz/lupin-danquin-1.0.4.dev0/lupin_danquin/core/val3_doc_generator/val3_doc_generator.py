...
class Val3Documentation:
    ...
    def _get_informations_from_applications(
        ...
        for application_name in application_names:
            program_path, application_name = self._split_path_and_app_name(base_program_path, application_name)
    ...
    def _split_path_and_app_name(self, base_program_path: str, application_name: str) -> str:
        application_name_path = application_name.split("/")
        if len(application_name_path) > 1:
            base_program_path = self.user_app_dir + os.sep + os.sep.join(
                application_name_path[:-1]
            ) + os.sep
            application_name = application_name_path[-1]
        program_path = base_program_path + application_name
        return program_path, application_name
