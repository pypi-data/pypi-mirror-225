...
def handle_simple_arguments(settings, args):
    ...
    stored_as_boolean = frozenset(('forwardbias', 'initial', 'omitreturn', 'reset', 'svg'))
    stored_as_list = frozenset(('debug', 'hold', 'label', 'samples', 'sense'))
...
def write_debug_information_to_log():
    ...
    --------------------------------------------------------------------------
    """
    # Ensure that the command line invokation could be copied and pasted back
...
def check_key(graceful_quit):
    ...
    'q' then 'enter' (both keypresses required to avoid an accidental quit),
...
def debug_hold_stability(settings, ivtdat):
    ...
    if hold is not None and hold >= 1:
        ...
        variation = 1 + random.random() * 0.05
...
def diode(voltage, variation):
    """
    A very rough approximation of the leakage current of a reverse biassed
    diode.
...
def noisy(value):
    ...
    --------------------------------------------------------------------------
    """
    return value + value * 0.01 * (random.random() - 0.5)
...
def get_iv_data_from_psu(dev, settings, pipeline, graceful_quit):
    ...
    --------------------------------------------------------------------------
    """
    ivtdat = common.Packet(dev.manufacturer, dev.model,
                           dev.serial_number, dev.channel, dev.ident)
...
def hold_stability(settings, pipeline, graceful_quit, ser, voltage, ivtdat, dev):
    ...
    hold_sta_raw = datetime.datetime.utcnow()
    hold_end_raw = hold_sta_raw + datetime.timedelta(minutes=hold)
    hold_end = hold_end_raw.isoformat().split('.')[0].replace('T', ' ')

    if hold == 0:
        tdesc = 'indefinitely'
    else:
        suffix = '' if hold == 1 else 's'
        tdesc = f'for {hold} minute{suffix} until {hold_end} UTC'

    common.log_with_colour(logging.INFO,
                           f'{dev.ident}, IT, holding at {voltage:.0f}V {tdesc}')

    ivtdat.hold_voltage = voltage
    ref_timestamp = timestamp = time.time()
    release_time = timestamp + hold * 60

    count = 0
    while time.time() < release_time or hold == 0:
        if graceful_quit.value:
            break
        count += 1

        measure_time = ref_timestamp + count * 5
        time.sleep(measure_time - time.time())
        ...
        ivtdat.hold_timestamp.append(recordts)
...
def rate_limit_delay(settings, previous_voltage, next_voltage):
    ...
    if settings['atlas']:
        ...
        if previous_voltage is None:
            # ASSUME that the maximum possible step size is 10V
            delta_v = 10
        else:
        ...
        # make sure there is a minimum settling time
...
def run_iv_test(settings, pipeline, ser, ivtdat, graceful_quit, dev,
    ...
    for voltage in sequence.test_run(start, stop, settings['stepsize'],
        ...
        # calculate the delay required to limit the rate of voltage change
        duration = rate_limit_delay(settings, last_successful_voltage, voltage)

        # obtain readings from power supply
        vpsu, ipsu = common.set_psu_voltage_and_read(settings, pipeline, voltage, ser, dev)
        if settings['settle']:
            vpsu, ipsu = settle(settings, pipeline, ser, vpsu, ipsu, dev)
...
def settle(settings, pipeline, ser, initial_voltage, initial_current, dev):
    ...
    if samples_received >= dev.window_size:
        final_value = stat.mean(dev.measured_voltages), stat.mean(dev.measured_currents)
    else:
...
def save_combined_iv_plot(settings, colors, data, directory, session):
    ...
    title = 'IV'
    title = f'{title}\n{data.label}' if data.label else title
    only_session = pathlib.Path(session).name
...
def save_combined_stability_plot(settings, colors, data, directory, session):
    ...
    # title of plot
    title = f'IT, {data.packets[0].hold_voltage:.0f}V'
    title = f'{title}\n{data.label}' if data.label else title
    only_session = pathlib.Path(session).name
    axis.set_title(f'{title}\n{only_session}')
...
def save_individual_stability_plots(settings, colors, data, directory, session):
    ...
    for packet, color in zip(data.packets, itertools.cycle(colors)):
        ...
        # title of plot
        title = f'IT, {packet.hold_voltage:.0f}V, {packet.ident}'
        title = f'{title}\n{data.label}' if data.label else title
        only_session = pathlib.Path(session).name
        axis.set_title(f'{title}\n{only_session}')
        plt.tight_layout()
...
def save_environmental_plots(settings, data, directory, session):
    ...
    for packet in data.packets:
        for test_iv, outbound, title in options:
            ...
            plot_title = f'{title} environment\n{data.label}' if data.label else title
            host.set_title(f'{plot_title}\n{session}')
