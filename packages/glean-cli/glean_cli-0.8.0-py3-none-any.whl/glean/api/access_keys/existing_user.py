...
from glean.utils.cli import parse_str_bool
...
def _get_existing_account_access_key_info(
    ...
    def qp_handler(query_params: dict) -> str:
        ...
        return f"{GLEAN_BASE_URI}/cliAuthConfirmation/success/existingUser?userEmail={access_key_info.user_email}"

    login_and_handle_query_params(qp_handler, False, intended_project_id)
    ...
    if has_multiple_projects:
        click.echo(
            f"You belong to multiple Glean projects. Please specify the ID of the project you'd like to create an access key for with the `--project-id` option."
        )
        direct_user_to_project_admin_settings()
        return
