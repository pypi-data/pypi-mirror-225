...
from glean.utils.input_validation import prompt_and_validate_value_length
...
from glean.api.access_keys.existing_user import create_access_key
from glean.api.access_keys.new_user import create_account_and_access_key
...
@click.option(
    ...
    show_default=True,
    help="Path to your Glean access key credentials. You can also control this by setting a GLEAN_CREDENTIALS_FILEPATH environment variable.",
...
def signup(ctx: click.Context):
    """Sign up for a Glean account and create an access key to get started.
    ...
    project_name = prompt_and_validate_value_length(
        "Enter a name for your Glean project", "Glean project name"
    )
...
def token(ctx: click.Context, project_id: Optional[str]):
    """Log into a Glean account and create a new access key.
...
def pull(
    ...
    result = pull_resource(
        s, project_id, resource_type, resource_id, resource_alias, dataops_only=(not all)
    )
    ...
    if len(remote) == 0 and not all:
        click.echo(
            "No DataOps-managed resources were found. To pull all resources in the project, use `glean pull --all`."
        )
        return
