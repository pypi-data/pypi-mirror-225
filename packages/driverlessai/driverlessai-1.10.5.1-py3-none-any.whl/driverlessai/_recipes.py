...
class DataRecipe(Recipe):
    """Interact with a data recipe on the Driverless AI server."""
...
class DataRecipes:
    """Interact with data recipes on the Driverless AI server.

    Examples::

        # Get list of all custom transformers
        [d for d in dai.recipes.data.list() if d.is_custom]

        # Get list of names of all transformers
        [d.name for d in dai.recipes.data.list()]
    """
    ...
    def list(self) -> Sequence["DataRecipe"]:
        """Return list of data recipe objects.

        Examples::

            dai.recipes.data.list()
        """
...
class IndividualRecipe(Recipe):
    """Interact with an individual recipe on the Driverless AI server."""
...
class IndividualRecipes:
    """Interact with individual recipes on the Driverless AI server.

    Examples::

        # Get list of all custom transformers
        [i for i in dai.recipes.individuals.list() if i.is_custom]

        # Get list of names of all transformers
        [i.name for i in dai.recipes.individuals.list()]
    """
    ...
    def list(self) -> Sequence["IndividualRecipe"]:
        """Return list of individual recipe objects.

        Examples::

            dai.recipes.individuals.list()
        """
        return _utils.ServerObjectList(
            data=[
...
class Recipes:
    ...
    def data(self) -> "DataRecipes":
        ...
        _utils.check_server_support(self._client, "1.10.0", "data")
        return self._data
    ...
    def individuals(self) -> "IndividualRecipes":
        ...
        _utils.check_server_support(self._client, "1.10.2", "individuals")
