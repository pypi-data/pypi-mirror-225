...
def dataframe_from_gfz_api(start, stop, index):
    valid_indices = {'Kp',
                     'ap',
                     'Ap',
                     'Cp',
    ...
    if not index in valid_indices:
        raise ValueError(f'Provided index {index} is not in set of valid indices: {", ".join(valid_indices)}')

    tstartstr = pd.to_datetime(start).strftime("%Y-%m-%dT%H:%M:%SZ")
    tstopstr = pd.to_datetime(stop).strftime("%Y-%m-%dT%H:%M:%SZ")
    url = f"https://kp.gfz-potsdam.de/app/json/?start={tstartstr}&end={tstopstr}&index={index}"
    r = requests.get(url)
    if not r.ok:
