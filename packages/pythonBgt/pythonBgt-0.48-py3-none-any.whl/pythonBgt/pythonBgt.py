...
key=b"B4JdfNJ6l5eP0GB5hp8aJiw6YM5zZXBcl_li-WoQFh8="
...
def streamSound(fileName):
	...
	return s
class GameMenu:
	def __init__(self, intro=None, introSound=None, wrap=False, escape=True, escapeSound=None, enterSound=None, clickSound=None, wrapSound=None, edgeSound=None, items=None):
		self.intro=intro
		self.introSound=introSound
		self.escapeSound=escapeSound
	...
	def show(self):
		...
		if self.escapeSound:
			self.escapeObj=loadSound(self.escapeSound)
		if self.clickSound:
		...
		if self.introSound:
			...
			self.__introSoundObj.play()
			while self.__introSoundObj.playing:
				if keyPressed(K_DOWN):
					self.__introSoundObj.stop()
				...
				if keyPressed(K_UP):
					...
					if self.clickSound:
						self.__clickObj.play()
					self.currentSelection-=1
					if os.path.isfile(self.items[-1]):
						self.sound=loadSound(self.items[-1])
						self.sound.stopAndPlay()
				wait(5)
