...
class KingstoneParser(BaseParser):
    ...
    def is_target_page(self, url):
        is_book = False
        ecpid = None
        match = re.search('www\.kingstone\.com\.tw\/new\/basic\/(\w+)?', url.lower())
        if match:
    ...
    def _extract_imgurl(self, soup):
        val = soup.select_one('div.mainpdarea div.swiperbox div.swiper-slide a img')['src']
        return val
    
    def _extract_imgurl_list(self, soup):
        elems = soup.select('div.mainpdarea div.swiperbox div.swiper-slide a img')
        imgs = [img['src'] for img in elems]
        val = ','.join(imgs)
        return val #已測試若沒有為''
    ...
    def _extract_eccatlog(self, soup):
        elems = soup.select('div.breadcrumb2 h1.breadcb2 a')
    ...
    def _extract_translator(self, soup):
        val = None
        elem = soup.find('li', text=re.compile(r'編／譯者'))
        if elem:
            val = elem.next_sibling.next_sibling.text.strip()
        return val
