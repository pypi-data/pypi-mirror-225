...
class YahooParser(BaseParser):
    ...
    def scrape_page_to_strprd(self, url):
        '''
        由main_parser呼叫此方法，若發生exception就塞回pubsub，若回傳None就先當作是下架商品刪除(可再規劃)
        '''
        # ---- 下載response回來 ----
    ...
    def _extract_url(self, soup):
        ...
        if 'gdbksale' in self._url:
            ...
            elem = soup.select_one('div#isoredux-data')
            info = json.loads(elem['data-state'])
            val = info['ecgql']['gqlItemPage']['url']
        return val
    ...
    def _extract_eccatlog(self, soup):
        val = None
        if 'gdbksale' in self._url:
            cats = soup.select('div.yui3-u-1 li.CategoryBreadCrumb__breadCrumbListItem___36uwc a')
            val = cats[-1].text
        else:
            elem = soup.select_one('div#isoredux-data')
            info = json.loads(elem['data-state'])
    ...
    def _extract_author(self, soup):
        ...
        if 'gdbksale' in self._url:
            ...
            if elem:
                val = elem.text.replace('作　者： ','').replace('\xa0',' ').strip()
        else:
            elem = soup.select_one('div#isoredux-data')
            info = json.loads(elem['data-state'])
            spec_tb_text = info['ecgql']['gqlItemPage']['detailDescription']['specifics']
    ...
    def _extract_isbn(self, soup):
        ...
        if 'gdbksale' in self._url:
            ...
            if elem:
                val = elem.text.replace('I S B N ： ','').strip()
        else:
            elem = soup.select_one('div#isoredux-data')
            info = json.loads(elem['data-state'])
            spec_tb_text = info['ecgql']['gqlItemPage']['detailDescription']['specifics']
    ...
    def _extract_desc(self, soup):
        val = None
        if 'gdbksale' in self._url:
            elem = soup.select_one('div#cl-gdintro div.content')
            ...
            elem = soup.select_one('div#isoredux-data')
            info = json.loads(elem['data-state'])
            val = info['ecgql']['gqlItemPage']['detailDescription']['longDescription']
            val = utility.remove_emojis_str(val)
            if val:
                val = html.escape(val)[:8000]
        return val
    ...
    def _extract_translator(self, soup):
        ...
        if 'gdbksale' in self._url:
            ...
            if elem:
                val = elem.text.replace('譯　者： ','').strip()
        else:
            elem = soup.select_one('div#isoredux-data')
            info = json.loads(elem['data-state'])
            spec_tb_text = info['ecgql']['gqlItemPage']['detailDescription']['specifics']
    ...
    def _extract_pub_date(self, soup):
        ...
        if 'gdbksale' in self._url:
            ...
            info = json.loads(elem['data-state'])
            spec_tb_text = info['ecgql']['gqlItemPage']['detailDescription']['specifics']
            match = re.search('<th>出版日期</th><td>(.+?)</td>', spec_tb_text)
            if match:
                val = match.group(1) if match.group(1) else None
            else:
                match = re.search('<li>出版日期：(.+?)</li>', spec_tb_text)
                if match:
                    val = match.group(1) if match.group(1) else None
        if val:
    ...
    def _extract_painter(self, soup):
        ...
        if 'gdbksale' in self._url:
            ...
            if elem:
                val = elem.text.replace('繪　者： ','').strip()
        else:
            elem = soup.select_one('div#isoredux-data')
            info = json.loads(elem['data-state'])
            spec_tb_text = info['ecgql']['gqlItemPage']['detailDescription']['specifics']
