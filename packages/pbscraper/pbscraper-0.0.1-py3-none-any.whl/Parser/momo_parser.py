...
class MomoParser(BaseParser):
    ...
    def _extract_name(self, soup):
        val = soup.select_one("article.productPage h2").text
        return val.strip()
    ...
    def _extract_imgurl(self, soup):
        val = None
        elem = soup.select_one('img[name="l_img"]')
        if elem:
            val = elem['src']
        if not val:
            elem = soup.select_one('div.mainBannerArea ul li img')
            if elem:
                val = elem['src']
        return val
    ...
    def _extract_imgurl_list(self, soup):
        val = ''
        img_list = []
        elems = soup.select('div.mainBannerArea ul li img')
        for elem in elems:
    ...
    def _extract_author(self, soup):
        val = None
        elem = soup.find('th', text=re.compile(r'作者'))
        if elem:
            val = elem.findNext('td').text.strip()
        # TODO:發現若是電子書作者等書籍資訊會放在商品特色裡 
        # https://m.momoshop.com.tw/goods.momo?i_code=6081811
        return val
