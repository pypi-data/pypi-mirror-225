...
class FabricCmd(cmd.Cmd):
    ...
    def __init__(self, slice_name=None):
    ...
    def _init_slices(self, slice_name=None):
        try:
            ...
            with Spinner():
                if slice_name:
                    slices = [self.fablib.get_slice(name=slice_name)]
                else:
    ...
    def complete_renew(self, text, line, b, e):
        parts = line.split(" ")
        return [x for x in self.config.keys() if x.startswith(parts[-1])]

    def do_delete(self, args):
        'Delete a slice: delete <slice_name>'
        parts = args.split(" ")
        if not len(parts) or len(parts) > 1:
            cout.error("Can only delete slices, must specify a single slice name.")
            return
        sname = parts[0]
        obj = self.config.get(sname, None)
        if not obj:
            cout.error(f"Slice \"{sname}\" not found")
            return
        else:
            try:
                obj = obj['_obj']
                obj.delete()
                del self.config[sname]
                cout.warn(f"Slice \"{sname}\" deleted.")
            except Exception as e:
                cout.error(f"{e}")
...
def main(args=None):
    ...
    if opt:
        ...
        cmd = FabricCmd(slice_name=sname)
        while True:
            try:
                cmd.cmdloop()
                break
