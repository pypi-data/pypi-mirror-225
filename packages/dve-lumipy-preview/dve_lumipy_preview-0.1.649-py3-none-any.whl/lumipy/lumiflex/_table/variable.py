...
from pydantic import StrictStr, Field, StrictInt, root_validator
...
class TableVar(BaseVarDef):
    label_: StrictStr = Field('table_var_def', const=True, alias='label')

    @root_validator
    def validate_variable(cls, values):
        name = values['name']
        subquery = values['parents_'][0]
        values['name'] = name if name is not None else record.make_name('tv', subquery)
        return values
    ...
    def build(self) -> [Table, Column]:
        return Table(meta=self.make_meta(), client=self.client, parents=(self,), parameters=tuple())
...
class DirectProviderVar(BaseVarDef):
    ...
    label_: StrictStr = Field('direct_provider_def', const=True, alias='label')
    meta: TableMeta
    ...
    def validate_variable(cls, values):
        name = values['name']
        meta = values['meta']
        params = values['use_params']
        params_str = ''.join(k + v for k, v in params.items())

        if name is None:
            name = record.make_name(meta.name.replace('.', '_').lower(), hash(meta) + hash(params_str) + hash(values['parents_']))
        values['name'] = name
...
class TableLiteralVar(BaseVarDef):
    ...
    def validate_variable(cls, values):
        name = values['name']
        df = values['df']
        values['name'] = record.make_name('pandas_df', df.to_csv()) if name is None else name
        return values
...
class DriveWriteVar(BaseVarDef):
    ...
    label_: StrictStr = Field('drive_write_def', const=True, alias='label')
    file_path: StrictStr

    @root_validator
    def validate_variable(cls, values):
        subquery = values['parents_'][0]
        values['name'] = record.make_name('drive_write', subquery)
        return values
...
class MakeViewVar(BaseVarDef):
    ...
    label_: StrictStr = Field('make_view_def', const=True, alias='label')
    view_name: StrictStr
    ...
    def validate_variable(cls, values):
        subquery = values['parents_'][0]
        values['name'] = record.make_name('make_view', subquery)
        return values
...
class ScalarVar(BaseVarDef):
    ...
    label_: StrictStr = Field('scalar_var_def', const=True, alias='label')
