...
def log_on(user=None, password=None, account=None, database=None, warehouse=None, role=None, login_timeout=10, authenticator=None, token=None):
    ...
    if (token is None):
        token = get_from_vars_or_args_or_environment(7, "SNOW_TOKEN", passed_variables, args)
...
class BeginLoading:
    ...
    def import_file_to_table(target_schema_table, define_columns, define_file, begin_loading_columns, begin_loading_values, field_delimiter = ",", skip_header=0, input_data_place_holder=None, con=None):
        ...
        if define_columns != "" and begin_loading_values != []:            
            ...
            exec(f"COPY INTO {target_schema_table} {begin_loading_columns} FROM (SELECT {select_values} FROM {stage_name})  FILE_FORMAT=(TYPE=CSV SKIP_HEADER={skip_header} {fileFormat}) ON_ERROR=CONTINUE", con=con)
        else:
            select_values = ', '.join(unordered_actual_values) if unordered_actual_values != [] else '*'
            exec(f"COPY INTO {target_schema_table} FROM (SELECT {select_values} FROM {stage_name})  FILE_FORMAT=(TYPE=CSV SKIP_HEADER={skip_header} {fileFormat}) ON_ERROR=CONTINUE", con=con)        
