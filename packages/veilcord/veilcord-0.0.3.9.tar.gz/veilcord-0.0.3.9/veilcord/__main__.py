...
class VeilCord:
    def __init__(
        ...
        if user_agent:
            ...
            self.bversion = device_version if device_version else "115.0"
    ...
    def getBuildNum(withStat: bool = False) -> Union[int, Tuple[int, str, str, Tuple[float, str]]]:
        def scrape():
            html_content = get('https://discord.com/channels/@me').content.decode('utf-8')
            script_tags = findall(r'<script[^>]+src="([^"]+)"[^>]*>', html_content)

            for script_url in script_tags:
                input_string = get(f"https://discord.com{script_url}").text

                if search(r'(?i)buildNumber', input_string):
