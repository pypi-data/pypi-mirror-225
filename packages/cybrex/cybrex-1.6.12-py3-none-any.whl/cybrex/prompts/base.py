...
class BasePrompter:
    ...
    def qa_prompt(self, question, chunks: List[dict]):
        if len(chunks) >= 1:
            return '''
SYSTEM: You are Cybrex AI created by People of Nexus. Given the following extracted parts of a long source_document and a question, create a final answer.
...
ASSISTANT:'''.format(question=question)
    ...
    def summarize_prompt(self, chunks: List[dict]):
        return '''
SYSTEM: You are Cybrex AI created by People of Nexus. Given the following extracted parts of a long source_document, summarize its content.
...
class Llama27bPrompter(BasePrompter):
    def qa_prompt(self, question: str, chunks: List[dict]):
        if len(chunks) >= 1:
            return '''
<<SYS>>
You are Cybrex AI created by People of Nexus. Given the following extracted parts of a long source_document and a question, create a final answer.
If you don't know the answer, just say that you don't know. Don't try to make up an answer.
<</SYS>>
[INST]
Extracted parts:
...
[/INST]'''.format(question=question)
    ...
    def summarize_prompt(self, chunks: List[dict]):
        return '''
<<SYS>>
You are Cybrex AI created by People of Nexus. Given the following extracted parts of a long source_document, summarize its content.
<</SYS>>
[INST]
Extracted parts:
{summary}
...
class OpenAIPrompter(BasePrompter):
    def qa_prompt(self, question: str, chunks: List[dict]):
        return """Given the following extracted parts of a long source_document and a question, create a final answer with references ("DOIs").
If you don't know the answer, just say that you don't know. Don't try to make up an answer.
...
FINAL ANSWER:""".format(question=question, summary=self.generate_summary(chunks))
    ...
    def summarize_prompt(self, chunks: List[dict]):
        return '''
Given the following extracted parts of a long source_document, summarize its content
