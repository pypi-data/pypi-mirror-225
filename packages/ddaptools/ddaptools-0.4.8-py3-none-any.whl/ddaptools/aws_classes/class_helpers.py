...
class PostgreSQLProvider(DatabaseProvider):
    ...
    def update_status_staging(self, staging_event_guid, status_update = "PROCESSED"):
        """Updates the status of the staging table
        """
        tablename = "staging_events"
        if status_update is not None:
            if ( False ): print("Updating status of staging table to:", status_update, "for:", staging_event_guid)
            self.cursor.execute(f"UPDATE {tablename} SET status = '{status_update}' WHERE guid = '{staging_event_guid}'")
            self.connection.commit()
            if ( False ): print(f"update status completed {status_update} for {staging_event_guid}")
    
    def getOne(self, key_value, key_column: str = "guid", status_update=None):
        """Gets one
        ...
        if row:
            ...
            # If is not None, then UPDATE  the status_update => ""

            if status_update is not None:
                self.update_status_staging(staging_event_guid=row_dict["guid"], status_update=status_update)
    ...
    def publish_to(self, events: List[dict], column_names: List[str], tablename: str, update_span_guid: bool = False):
        ...
        for row in events:
            ...
            if(update_span_guid):
                ...
                print("span_sequence:", span_sequence)
...
class MockOrganizationQuerier(OrganizationalQuerier):
    ...
    def initialization(self, organization_guid, platform_type = None):
        ...
        # TODO Here is just the mock, edit it to publish it.

        # 1. Create a dict of users. {id: user_profile}
        # 2. Create a dict of dict of users. | separated each identity by each platform. 
        
        # {platform_id: {identity: user_id}}

        # Then transform that into the format that we want.
