...
class RakutenParser(BaseParser):
    ...
    def _extract_name(self, soup):
        val = elem = soup.select_one('h1.b-ttl-main').text
        return val.strip()
    ...
    def _extract_imgurl(self, soup):
        val = None
        elem = soup.select_one('div.qa-product-mainImage img')
        if elem:
            val = elem['data-src'].replace('?_ex=330x330', '')
        return val
    
    def _extract_imgurl_list(self, soup):
        elems = soup.select('li[data-target="#productImageGallery"] img')
        val = ','.join([img['data-src'].replace('?_ex=60x60', '') for img in elems])
        return val #已測試若沒有為''
    ...
    def _extract_eccatlog(self, soup):
        val = None
        elems = soup.select('ul.b-list-horizontal-divided li.last-child a')
        if elems:
            eccats = [cat.text for cat in elems]
            val = '>'.join(eccats)
        return val
    ...
    def _extract_oriprice(self, soup):
        val = None
        elem = soup.select_one('del.qa-product-listPrice')
        if elem:
    ...
    def _extract_price(self, soup):
        val = None
        elem = soup.select_one('span.qa-product-actualPrice')
        if elem:
            val = elem.text.replace('元', '').replace(',', '').strip()
        return val
