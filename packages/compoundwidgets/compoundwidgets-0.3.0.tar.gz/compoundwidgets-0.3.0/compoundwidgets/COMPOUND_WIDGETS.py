...
class LabelSpinbox(LabelCompoundWidget):
    ...
    def _update_value(self, name, index, mode):
        current = self.variable.get().replace(',', '.')
        if isfloat(current):
            if self.entry_method:
...
class LabelEntryUnit(LabelCompoundWidget):
    ...
    def __init__(self, parent, label_text='label:', label_anchor='e', label_width=None,
        ...
        if True:
            self.precision = precision
            self.trace_variable = trace_variable
            self.entry_variable = tk.DoubleVar()
            if entry_value and isfloat(entry_value):
                if entry_value < 1 / (10 ** (self.precision - 1)):
                    self.entry_variable.set("%0.*e" % (self.precision, float(entry_value)))
                else:
                    self.entry_variable.set("%0.*f" % (self.precision, float(entry_value)))
        ...
        if isfloat(value):
            if self.trace_variable:
                ...
                if value < 1 / (10 ** (self.precision - 1)):
                    self.entry_variable.set("%0.*e" % (self.precision, float(value)))
                else:
