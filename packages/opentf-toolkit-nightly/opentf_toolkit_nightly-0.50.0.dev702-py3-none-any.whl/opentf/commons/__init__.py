...
def make_app(
    ...
    `app.config`: `CONFIG`, `CONTEXT`, and `MANIFEST`.
...
def get_context_parameter(
    ...
    - validator: a function of three arguments or None (None by default)
    ...
    def _maybe_validate(v) -> int:
        newv, reason = validator(app, name, v) if validator else (v, None)
