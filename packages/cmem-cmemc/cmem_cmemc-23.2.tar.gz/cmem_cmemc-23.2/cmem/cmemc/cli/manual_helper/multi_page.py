...
def enhance_help_text_markdown(
    ...
    for line in text.splitlines():
        ...
        if line == "":
            ...
            help_text_ = f"{help_text_} {line.rstrip().strip()}"
    help_text = ""
    for line in help_text_.splitlines():
        line = line.strip()
...
def get_markdown_for_index(ctx, commands) -> str:
...
"""
    ...
    for key in commands:
        ...
        if isinstance(item, Group):
            table_data = get_commands_for_table_recursive(
                ctx, item, table_data, "", item.name
            )
    table_data = sorted(table_data, key=lambda d: d['group_name'])  # type: ignore
...
def create_multi_page_documentation(ctx, directory):
...
def create_group_manual_dir_recursive(command_group, ctx, prefix, full_name):
    ...
    for key in commands:
        ...
        if isinstance(item, Group):
            create_group_manual_dir_recursive(
                ...
                ctx=ctx,
                prefix=directory,
                full_name=f"{full_name} {item.name}".strip()
            )


def create_group_index_md(ctx, command_group, directory, full_name):
...
""")
        for key in command_group.commands:
            item = command_group.commands[key]
            if isinstance(item, Group):
                continue
            if isinstance(item, Command):
...
def get_markdown_for_command(ctx, command, group_name) -> str:
    ...
    options_text_ = options_text_.replace(
        "Usage: cmemc",
        f"Usage: cmemc {group_name} {command.name}"
    )
...
"""
    ...
    usage_text = str(command.get_usage(ctx)).replace(
        "Usage: cmemc",
        f"$ cmemc {group_name} {command.name}"
    ).strip()
    if options_text == "":
...
"""
    ...
    markdown = f"""## {group_name} {command.name}
