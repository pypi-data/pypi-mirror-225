...
@click.pass_obj
def create_command(app: ApplicationContext, username):
    """
...
@click.option(
    ...
    type=click.STRING,
    required=False,
    help="Set a new first name."
)
@click.option(
    "--last-name",
    type=click.STRING,
    required=False,
    help="Set a new last name."
)
@click.option(
    "--email",
    type=click.STRING,
    required=False,
    help="Set a new email."
)
@click.option(
    "--assign-group",
    type=click.STRING,
...
def update_command(
    ...
    invalid_groups = [group for group in assign_group if group not in all_groups.keys()]
    ...
    if invalid_groups:
        raise ValueError(
            NO_GROUP_ERROR.format(
                ', '.join(invalid_groups),
                ', '.join(all_groups.keys() - set(existing_user_groups))
            )
        )
    ...
    if invalid_unassign_groups:
        raise ValueError(
            INVALID_UNASSIGN_GROUP_ERROR.format(
                ", ".join(invalid_unassign_groups),
                ", ".join(existing_user_groups)
            )
        )
...
@click.option(
    "--value",
    help="With this option, the new password can be set in a non-interactive way."
)
@click.option(
    "--temporary",
    is_flag=True,
...
def password_command(
    ...
    first (e.g. with `NEW_PASS=$(pwgen -1 40)`) and use it afterwards in the
...
def open_command(app: ApplicationContext, usernames: str):
    ...
    if not usernames:
        ...
        for _ in usernames:
            if _ not in user_name_id_map.keys():
