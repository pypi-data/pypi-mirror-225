...
def import_command(app, path, project_id, overwrite):
    ...
    """
    all_projects = get_projects()
    if project_id and not overwrite \
            and project_id in ([_["name"] for _ in all_projects]):
...
def reload_command(app, all_, project_ids):
    ...
    if project_ids == () and not all_:
        raise ValueError(
            "Either specify at least one project ID or use the --all option "
            "to reload all projects.")
    if all_:
        # in case --all is given, a list of project is fetched
        project_ids = (
            [currentProjects["name"] for currentProjects in all_projects]
        )
    # avoid double reloading
    project_ids = list(set(project_ids))
    ...
    for project_id in project_ids:
        app.echo_info(
            f"Reload project {current}/{count}: {project_id} ... ",
            nl=False
        )
