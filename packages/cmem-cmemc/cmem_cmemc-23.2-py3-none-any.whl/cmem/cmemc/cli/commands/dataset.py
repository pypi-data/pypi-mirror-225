...
def _get_dataset_tag_labels(dataset_) -> list:
    ...
    tag_labels = []
    for tag in dataset_["tags"]:
        tag_labels.append(tag["label"])
    return tag_labels


def _get_datasets_filtered(datasets, filter_name, filter_value):
    ...
    if filter_name not in DATASET_FILTER_TYPES:
        raise ValueError(
            ...
            f"Use one of {DATASET_FILTER_TYPES}."
        )
    # filter by project ID
    if filter_name == "project":
        return [_ for _ in datasets
    ...
    if filter_name == "type":
        return [
            _ for _ in datasets
            if re.search(filter_value, _["pluginId"])
        ]
    # filter by tag label
    if filter_name == "tag":
        return [
            _ for _ in datasets
            if filter_value in _get_dataset_tag_labels(_)
        ]
...
def delete_command(app, project_id, all_, filter_, dataset_ids):
    ...
    if dataset_ids and (all_ or filter_):
        raise ValueError(
            "Either specify a dataset ID OR"
            " use a --filter or the --all option."
        )
    ...
    if all_ or filter_:
        ...
        dataset_ids = []
        datasets = list_items(
            item_type="dataset", project=project_id
        )["results"]
        for _ in filter_:
            filter_type, filter_name = _
            datasets = _get_datasets_filtered(
                datasets, filter_type, filter_name
            )
        for _ in datasets:
    ...
    for _ in dataset_ids:
        app.echo_info(
            f"Delete dataset {current}/{count}: {_} ... ",
            nl=False
        )
        project_part, dataset_part = _validate_and_split_dataset_id(_)
        app.echo_debug(
            f"Project ID is {project_part}, dataset ID is {dataset_part}"
        )
...
def create_command(
    ...
    app.echo_info(
        f"Create a new dataset {project_id}:",
        nl=False
    )
