...
class BaseUndeletableUserLogModel(BaseUndeletableModel):
    ...
    user_modified_id = db.Column(PG_UUID(as_uuid=True), nullable=False, comment="Идентификатор пользователя, изменившего запись")
...
@db.event.listens_for(DbMapper, 'before_update')
def receive_before_update(mapper: DbMapper, connection: Connection, target: BaseUndeletableUserLogModel) -> None:
    if issubclass(target.__class__, BaseUndeletableUserLogModel):
        assert transaction_commit.transaction_context,\
...
def receive_before_create(mapper: DbMapper, connection: Connection, target: BaseUndeletableUserLogModel) -> None:
    if issubclass(target.__class__, BaseUndeletableUserLogModel):
        assert transaction_commit.transaction_context,\
            f'database insert must be in {transaction_commit} context manager'
