...
class BaseModel(DbModel, SerializerMixin):
    ...
    id = db.Column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4, comment="Первичный ключ")
    date_created = db.Column(db.DateTime(), default=datetime.utcnow(), nullable=False, comment="Дата и время создания записи")
    date_modified = db.Column(db.DateTime(), default=datetime.utcnow(), nullable=False, comment="Дата и время изменения записи")
    is_alive = db.Column(db.Boolean(), default=True, nullable=False, comment="Указатель жива ли запись")
...
def receive_before_update(mapper: DbMapper, connection: Connection, target: BaseModel) -> None:
...
def receive_before_create(mapper: DbMapper, connection: Connection, target: BaseModel) -> None:
    if issubclass(target.__class__, BaseModel):
        assert transaction_commit.transaction_context,\
            f'database updates must be in {transaction_commit} context manager'
