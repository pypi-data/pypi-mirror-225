...
def load_file_into_database(filename, 
                            table_name, 
                            column_mapping, 
                            before_statement,
                            after_statement):
    ...
    with open('db_config.yaml', 'r', encoding='UTF-8') as file:
...
def to_snake_case(title):
    title = title.replace(' ', '')
    title = re.sub('(.)([A-Z][a-z]+)', r'\1_\2', title)
    title = re.sub('([a-z0-9])([A-Z])', r'\1_\2', title)
