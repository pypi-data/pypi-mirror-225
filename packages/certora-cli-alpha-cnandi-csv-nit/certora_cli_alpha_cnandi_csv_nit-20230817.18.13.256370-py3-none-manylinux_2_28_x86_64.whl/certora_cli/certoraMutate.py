...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--auto_conf", type=Path,
                        help="Name of a mutation .conf file where certoraMutate will dump an"
                             "automatically generated mutation_conf, given a prover_conf.")
...
def validate_args(args: argparse.Namespace) -> None:
    ...
    if args.mutation_conf is not None:
        exit_unless(args.mutation_conf.exists() or args.auto_conf,
...
def submit(args: argparse.Namespace) -> None:
    ...
    if args.mutation_conf is not None:
        ...
        if Constants.MANUAL_MUTANTS in mutation_conf:
            manual_mutants = parse_manual_mutations(args.mutation_conf)
            if args.debug:
    ...
    with open(args.collect_file, 'w+') as collect_file:
...
def run_mutant(mutant: GambitMutant, src_dir: Path, trg_dir: Path, orig_conf: Path,
    ...
    orig_file_relative_to_conf_parent = Path(mutant.original_filename)
...
def run_gambit(args: argparse.Namespace) -> List[GambitMutant]:
    ...
    # BUT, if the gambit conf file has manual mutations,
    # remove them before passing it to gambit because manual mutations are a Certora specific feature.
    ...
    if Constants.MANUAL_MUTANTS in mutation_conf:
        ...
        with open(new_mutation_conf, 'w') as g_conf:
        ...
        for orig_file in prover_json[Constants.FILES]:
            mutation_conf_elt = {
                Constants.FILENAME: str(Path(os.path.relpath(os.getcwd(),
                                                             args.mutation_conf.parent)).joinpath(orig_file)),
...
def parse_manual_mutations(mutation_conf: Path) -> List[GambitMutant]:
    print(f"Parsing manual mutations from {mutation_conf} file.")
    ret_mutants = []
    manual_id = 0
    with open(mutation_conf, "r") as conf:
        gambit_cfg = json.load(conf)
        for orig_file in gambit_cfg[Constants.MANUAL_MUTANTS]:
            if not Path(orig_file).exists():
                print(f"File '{orig_file}' for manual mutations does not exist. Skipping verification for this file.")
                continue
            manual_mutant_dir = mutation_conf.resolve().parent.joinpath(gambit_cfg[Constants.MANUAL_MUTANTS][orig_file])
...
def json_to_csv(args: argparse.Namespace, json_obj: List[Any]) -> None:
    with open(args.dump_csv, 'w', newline='') as ui_out_csv:
        ...
        for elem in json_obj:
            name = elem["id"] + "_" + os.path.basename(elem["name"])
            row1.append(name)
        wr.writerow(row1)
        rnames = [rule["name"] for rule in json_obj[0]["rules"]]
        for rnm in rnames:
            statuses = []
            for elem in json_obj:
                status = [r["status"] for r in elem["rules"] if "name" in r and "status" in r and r["name"] == rnm]
                if status:
