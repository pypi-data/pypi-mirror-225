...
from radiens.grpc_radiens import (allegoserver_pb2, allegoserver_pb2_grpc,
                                  common_pb2, datasource_pb2, spikesorter_pb2)
...
def to_radiens_file_type(p: Path):
    ext = p.suffix.lower()
    if ext in ['.rhd']:
        return common_pb2.RHD
    if ext in ['.xdat', '']:
        return common_pb2.XDAT
    if ext in ['.csv']:
...
class BaseClient:
    ...
    def __init__(self):
        ...
        self.__dash_port = cfg.get_dash_port()
        self._base_hubs = {DEFAULT_HUB: {'ip_address': DEFAULT_IP,
                                         'port': self.__core_port, 'dash': self.__dash_port}}

    def _add_hub(self, new_hub: dict):
    ...
    def _server_address(self, hub_name, service):
        ...
        elif service == DASH_SERVICE:
            return '{}:{}'.format(self._hubs()[hub_name]['ip_address'], self._hubs()[hub_name]['dash'])
        else:
