...
class PrepID(CacheableBase):
    ...
    def reset(self):
        """
        The Workflow objects are stored in a cache. At times,
    ...
    def getWorkflows(self):
        ...
        data = getResponse(url=self.url,
                           ...
                           param='')
        if data is None:
            raise Exception("Can't contact couchdb/reqmgr_workload_cache/")
        rows = data['rows']
    ...
    def getLabels(self):
        ...
        if len(tickets) == 0:
            ...
            logging.warning("There is no JIRA ticket for %s" % self.prepID)
            labels = []
        else:
            ## pick up the last one
            logging.warning("There is at least one JIRA ticket for %s, taking the last one" % self.prepID)
    ...
    def getACDCs(self, valid=False):
        ...
        return [w for w in workflows if 'acdc' in w.workflowName.lower()] if workflows is not None else []
