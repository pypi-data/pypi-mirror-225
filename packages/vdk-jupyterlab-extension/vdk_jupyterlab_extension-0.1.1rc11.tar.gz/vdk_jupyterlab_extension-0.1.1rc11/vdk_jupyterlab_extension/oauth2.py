...
class OAuth2Handler(APIHandler, HasTraits):
    _authorization_url = Unicode(
    ...
    def get(self):
        ...
        if origin:
            redirect_uri = origin + "/vdk-jupyterlab-extension/login"
        else:
            redirect_uri = self._fix_localhost(self.request.full_url())
