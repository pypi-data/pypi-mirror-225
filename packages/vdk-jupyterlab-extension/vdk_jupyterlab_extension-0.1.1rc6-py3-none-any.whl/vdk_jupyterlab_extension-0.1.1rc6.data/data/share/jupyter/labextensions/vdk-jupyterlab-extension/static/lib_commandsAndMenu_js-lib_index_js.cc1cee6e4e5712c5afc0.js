...
async function checkToken() {
    ...
    console.debug("Checking token location");
    if (tokenLocation) {
        const token = await getTokenFromLocation(tokenLocation);
        if (token) {
            console.debug("Token found. Sending it to the server");
            await sendTokenToServer(token);
            console.debug("Sent token to the server");
        }
    }
...
//# sourceMappingURL=lib_commandsAndMenu_js-lib_index_js.cc1cee6e4e5712c5afc0.js.map