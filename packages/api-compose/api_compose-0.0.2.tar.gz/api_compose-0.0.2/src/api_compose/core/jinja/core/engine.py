...
class JinjaEngine:
    ...
    def __init__(self,
        ...
        self.undefined = StrictUndefined  # Keep variables in a template without a context intact
    ...
    def render_to_str(self, jinja_context: Optional[BaseJinjaContext] = None) -> Tuple[str, bool, Optional[Exception]]:
        ...
        if jinja_context is None:
            jinja_context = BaseJinjaContext.build()
        else:
        ...
        if self.current_template is not None:
            try:
                result, is_success, exec = self.current_template.render(jinja_context.dict()), True, None
            ...
            exec = FailedToFindTemplateByPathException(
