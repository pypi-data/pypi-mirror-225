...
class system:
    ...
    def import_data_ben(self):
      ...
      self.ben_emission = ben.iloc[:,ben_to_tru51_products['produto_BEN_num'].astype(float)] #ben products that generate emissions
    ...
    def estimation(self):
      ...
      for j in range(num_sectors):
        ...
        coef1_j = pd.DataFrame(coef1_j, columns=self.ben_emission.columns, index= tru_j.index)
        ...
        tep_j = self.ben_emission[self.ben_emission.index.str.contains(ben_[j].replace(' + ', '|'))].sum(axis=0).values#.T
        diag_tep_j = np.diag(tep_j.T.flatten())
        tep_j = coef1_j.values @ diag_tep_j

        tep_j_df = pd.DataFrame(tep_j, columns=self.ben_emission.columns, index= tru_j.index)
      ...
      tep_h = self.ben_emission[self.ben_emission.index.str.contains('RESIDENCIAL')]
      coef1_h = tep_h.copy()
      coef1_h = (coef1_h * 0) + 1
