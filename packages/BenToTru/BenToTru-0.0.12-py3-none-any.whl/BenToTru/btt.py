...
class system:
    ...
    def import_data_ben(self):
      ...
      self.ben = ben.drop(['CONSUMO FINAL NÃO-ENERGÉTICO', 'CONSUMO NÃO-IDENTIFICADO']) #Consider only the sectors that have been identified as generating emissions.
      self.ben_reduced = self.ben.iloc[:,ben_to_tru51_products['produto_BEN_num'].astype(float)] #ben products that generate emissions
    ...
    def estimation(self):
      ...
      for j in range(num_sectors):
        ...
        tep_j = self.ben_reduced[self.ben_reduced.index.str.contains(ben_[j].replace(' + ', '|'))].sum(axis=0).values#.T
