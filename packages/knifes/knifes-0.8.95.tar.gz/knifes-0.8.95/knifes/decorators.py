...
from .results import ApiResponse
    ...
    def actual_decorator(view_func):
        ...
        def wrapper(request: HttpRequest, *args, **kwargs):
            ...
            if not settings.DEBUG and required_signature:
                if request.headers.get(X_BODY) != md5((request.body or bytes()).decode('utf-8') + settings.BODY_SIGN_KEY):
                    return ApiResponse.failure(_('签名错误'))
                # # noinspection PyBroadException
                # try:
                #     encrypt_data = json.loads(aes.decrypt(request.data.get(X_BODY), settings.AES_BODY_KEY))
