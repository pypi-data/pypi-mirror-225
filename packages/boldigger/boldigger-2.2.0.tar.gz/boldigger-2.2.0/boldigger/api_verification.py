import datetime, contextlib, requests_html, joblib, json, openpyxl, psutil, lxml
...
def extract_data(xlsx_path, fasta_path):
    ...
    try:
        raw_data = pd.read_excel(
            xlsx_path, sheet_name="BOLDigger hit", engine="openpyxl"
        )
    except ValueError:
        return pd.DataFrame(), pd.DataFrame(), pd.DataFrame()
    data_to_check = raw_data.loc[raw_data["Similarity"].astype(str) != "No Match"]
    data_to_check = data_to_check.loc[
        (data_to_check["Similarity"] >= 98) & (data_to_check["Species"].isnull())
    ].copy()
    ...
    seq_dict = {
        ...
        if ">{}".format(k) in data_to_check["ID"].to_list()
    }
...
def request(item, session):
    ## request BOLD API
    r = session.get(
        "http://boldsystems.org/index.php/Ids_xml?db=COX1_SPECIES_PUBLIC&sequence={}".format(
            item[1]
        )
    )
    try:
        ...
        most_common = (
            r.loc[r["similarity"] >= 0.98]["taxonomicidentification"].mode().item()
        )
...
def refresh_data(result, raw_data, data_to_check, session):
    ...
    result["genus"], result["species"] = (
        result["tax"].str.split(" ", n=1).str[0],
        result["tax"].str.split(" ", n=1).str[1],
    )
    ...
    data_to_check["Genus"] = (
        data_to_check["ID"].map(genus).fillna(data_to_check["Genus"])
    )
    data_to_check["Species"] = (
        data_to_check["ID"].map(species).fillna(data_to_check["Species"])
    )
    data_to_check["Status"] = (
        data_to_check["ID"].map(status).fillna(data_to_check["Status"])
    )
    data_to_check["Flags"] = (
        data_to_check["ID"].map(flags).fillna(data_to_check["Flags"])
    )
    ...
    remaining_data = remaining_data.loc[
        ~((remaining_data["Genus"].isnull()) | (remaining_data["Species"].isnull()))
    ]

    ## collect the ids to request again via api
    bold_ids = {
        k: v for k, v in bold_ids.items() if k in remaining_data["ID"].to_list()
    }
    ...
    for id_pack in id_values:
        r = session.get(
            "http://www.boldsystems.org/index.php/API_Public/specimen?ids={}&format=json".format(
                "|".join(id_pack)
            )
        )
    ...
    remaining_data = pd.concat(
        [remaining_data.iloc[:, :1], additional_tax, remaining_data.iloc[:, 5:]], axis=1
    )
    ...
    for level in levels:
        raw_data[level] = np.where(
            raw_data[level].str.contains("[{}]".format(specials)),
            np.nan,
            raw_data[level],
        )
    ...
    while True:
        ...
        if not ran:
            ...
            if not raw_data.empty:
                ...
                window["out"].print(
                    "{}: Output will be routed to the terminal.".format(
                        datetime.datetime.now().strftime("%H:%M:%S")
                    )
                )
                window["out"].print(
                    "{}: This window will freeze in the meantime.".format(
                        datetime.datetime.now().strftime("%H:%M:%S")
                    )
                )
                window.Refresh()
                ...
                with tqdm_joblib(
                    ...
                    result = Parallel(n_jobs=psutil.cpu_count())(
                        delayed(request)(item, session)
                        for item in list(seq_dict.items())
                    )
