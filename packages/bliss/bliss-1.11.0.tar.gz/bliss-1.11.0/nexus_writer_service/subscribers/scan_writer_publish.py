...
from bliss.common.auto_filter.counters import FilterSetCounter, AutoFilterCalcCounter
...
def _device_info_add_ctr(devices, ctr):
    ...
    if isinstance(ctr, MythenCounter):
        ...
        device = {"device_info": device_info, "device_type": "lima"}
        devices[fullname] = device
    elif isinstance(ctr, (FilterSetCounter, AutoFilterCalcCounter)):
        ...
        logger.info(
            "Counter {} {} published as generic detector".format(
                fullname, type(ctr).__qualname__
            )
        )
        devices[fullname] = {}
    if alias:
