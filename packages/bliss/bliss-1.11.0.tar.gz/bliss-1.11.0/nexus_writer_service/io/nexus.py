...
from silx.io.dictdump import dicttojson, dicttoini, dicttonx
...
def _hdf5_string_encode(s):
    """Convert a scalar or array of bytes or unicode strings to a scalar
    or array of bytes. Needed for `asNxChar`
    """
    if isinstance(s, bytes):
        ...
        if numpy.issubdtype(s.dtype, numpy.bytes_):
            return s
        elif numpy.issubdtype(s.dtype, numpy.str_) and s.ndim == 0:
            return s.encode("utf-8", errors="surrogateescape")
    return [_hdf5_string_encode(si) for si in s]
...
def asNxChar(s, raiseExtended=True):
    ...
    try:
        ...
        if raiseExtended:
            raise
        return numpy.array(_hdf5_string_encode(s), dtype=nxcharBytes)
    else:
...
def dereference(uri, name=None, current=None):
    ...
    istart = 1
    try:
        with _open_when_needed(filename, current=current, mode="r") as f:
            while True:
                parts = path.split("/")[1:]
                for i in range(istart, len(parts) + 1):
                    ...
                    if uri != uri2:
                        if filename == filename2:
                            ...
                            return dereference(uri2, current=f)
                else:
                    break
    except OSError:
        pass
...
@contextmanager
def _open_when_needed(filename, current=None, **kwargs):
    is_current = False
    if current is not None:
        try:
            is_current = os.path.realpath(current.filename) == os.path.realpath(
                filename
            )
        except OSError:
            is_current = False
...
def uriContext(uri, **kwargs):
    ...
    filename, path = splitUri(uri)
    try:
        with _open_when_needed(filename, **kwargs) as f:
            try:
                item = f[path]
            except KeyError:
                item = None
...
def exists(uri, current=None):
    ...
    """
    try:
        with uriContext(uri, current=current, mode="r") as node:
...
def uriContains(uri, children=None, attributes=None, current=None):
    """
    :param str uri:
    :param list children:
    ...
    if not attributes:
        attributes = []
    try:
        with uriContext(uri, current=current, mode="r") as parent:
...
def nxComplete(uri):
    ...
    :returns bool:
    """
    return uriContains(uri, children=["end_time"])
...
def nxClassInit(
    ...
    if nxClassInstantiate(parent, name, nxclass, raise_on_exists=raise_on_exists):
        h5group = parent[name]
        nxdict = kwargs.get("nxdict")
        if nxdict is None:
            kwargs["nxdict"] = nxdict = dict()
        nxdict["@NX_class"] = nxclass
...
def _nxdict_without_removal(nxdict: dict) -> dict:
    result = dict()
    for k, v in nxdict.items():
        if isinstance(v, Mapping):
            result[k] = _nxdict_without_removal(v)
        elif v is not None:
            result[k] = v
    return result


def nxInit(h5group, nxdict=None):
    """
    Initialize NXclass instance

    :param h5py.Group h5group:
    :param dict nxdict:
    :param str update_mode:
    """
    if nxdict:
        nxdict = _nxdict_without_removal(nxdict)
        dicttonx(nxdict, h5group, update_mode="modify")
...
if File._LOCKING_MGR is not None:  # h5py < 3.5 or libhdf5 < 1.10.7 or libhdf5 == 1.12.0
    # silx >= 1.0.0 does not allow unsafe locking behaviour (old versions require a process-wide
    # environment variable to enable/disable locking)
    # assume the Nexus writer knows what it's doing and remove the check
    File._LOCKING_MGR = h5py_utils.Hdf5FileLockingManager()
    File._LOCKING_MGR._check_locking_env = types.MethodType(
        lambda self, enable: None, File._LOCKING_MGR
    )
...
def _datasetMergeInfo(
    ...
    if virtual_source_args:
        ...
        dtypes = set()
        for uri in uris:
            with uriContext(uri, mode="r", current=current) as dset:
...
def createConcatenatedDataset(
    ...
    **kwargs,
    ...
    for uri, index_generator in fill_generator():
        with uriContext(uri, mode="r", current=current) as dseti:
...
def vdsmapIsValid(dset, vdsmap):
    ...
    :returns bool:
    """
    return exists(vdsmapUri(dset, vdsmap), current=dset.file)
        ...
        if not merge:
            if isinstance(data, dsettypes):
                ...
                if isinstance(data, h5py.Dataset):
                    ...
                    with uriContext(data, mode="r", current=h5group.file) as dset:
                        if dset is not None:
