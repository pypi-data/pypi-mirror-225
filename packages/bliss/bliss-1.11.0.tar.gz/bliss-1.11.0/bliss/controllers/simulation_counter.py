...
class _Signal:
    ...
    def _missing_edge_of_gaussian_left(npoints, frac_missing):
        ...
        return numpy.concatenate(
            (
                signal.gaussian(p, 0.1 * npoints)[p2:],
                numpy.zeros(p2),
    ...
    SIGNALS = {
        "sawtooth": lambda npoints: signal.sawtooth(
            numpy.arange(0, 2 * numpy.pi * 1.1, 2 * numpy.pi * 1.1 / npoints), width=0.9
        ),
        "gaussian": lambda npoints: signal.gaussian(npoints, 0.2 * npoints),
        "flat": lambda npoints: numpy.ones(npoints),
        "off_center_gaussian": lambda npoints: numpy.concatenate(
            (
                numpy.zeros(npoints - npoints // 2),
                signal.gaussian(npoints // 2, 0.1 * npoints),
            )
        ),
        ...
        "bimodal": lambda npoints: numpy.concatenate(
            (
                signal.gaussian(npoints - npoints // 2, 0.15 * npoints) * 1.5,
                signal.gaussian(npoints // 2, 0.15 * npoints),
            )
        ),
        ...
        "inverted_gaussian": lambda npoints: 1
        - signal.gaussian(npoints, 0.2 * npoints),
        "expo_gaussian": lambda npoints: numpy.exp(
            signal.gaussian(npoints, 0.1 * npoints) * 30
        ),
