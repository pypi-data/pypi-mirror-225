...
import logging
import socket
from gevent.timeout import Timeout as GreenletTimeoutError
...
class BaseXIA(BaseMCA):
    ...
    def wait_hardware_reading(self):
        ...
        log_debug(self, "end of wait_hardware_reading")
...
class Mercury(BaseXIA):
    ...
    def _calc_max_block_size(self, mem_in_MB=2.0):
        mem = mem_in_MB * 1024.0 * 1024.0
        size = self.spectrum_size
        npts = ((mem / 2.0) - 256) / (256 + size)
        return int(npts)
