...
from bliss.controllers.mca.counter import (
    mca_counters,
    mca_counter_groups,
    AcquisitionMode,
)
...
Brand = enum.Enum(
    "Brand",
    "SIMULATED XIA OCEAN_OPTICS ISG HAMAMATSU AMPTEK VANTEC CANBERRA RONTEC XGLAB",
)

DetectorType = enum.Enum(
    "DetectorType",
    "SIMULATED FALCONX XMAP MERCURY MICRO_DXP DXP_2X "
    "MAYA2000 MUSST_MCA MCA8000D DSA1000 MULTIMAX DANTE",
)
...
# Base class
class BaseMCA(CounterController, HasMetadataForScanExclusive, HasMetadataForDataset):
            ...
            if params["npoints"] > 1:
                if params["trigger_mode"] in [
                    ...
                    block_size = self.estimate_block_size(
                        params["preset_time"], block_size
                    )
            params["block_size"] = 1 if block_size is None else block_size
    ...
    def run_software_acquisition(
        self, acquisition_number, acquisition_time=1.0, polling_time=0.1
    ):
