...
class PI_E712(pi_gcs.Communication, pi_gcs.Recorder, Controller):
    ...
    def state(self, axis):
        log_debug(
            self,
            f"axis {axis.name} closed loop is {self.get_closed_loop_state(axis).name}",
        )
        with self.sock.lock:
            ...
            if self.get_closed_loop_state(axis) is ClosedLoopState.ON:
    ...
    def start_all(self, *motions):
        ...
        for motion in motions:
            if self.get_closed_loop_state(motion.axis) is ClosedLoopState.ON:
                l_cmd = mov_cmd
            else:
                l_cmd = voltage_cmd
    ...
    def prepare_trajectory(self, *trajectories):
        ...
        for traj in trajectories:
            ...
            while True:
                ...
                if speed_up_down > seglength / 2.0:
                    speed_up_down = seglength / 2.0
    ...
    def activate_closed_loop(self, axis, onoff=True):
        ...
        if onoff:
            ...
            while (not _ont_state) and (time.perf_counter() - _t0) < cl_timeout:
                time.sleep(0.01)
                _ont_state = self._get_on_target_status(axis)
            if not _ont_state:
                ...
                log_info(
                    ...
                    "axis {0:s} ONT ok after {1:g} s".format(
                        axis.name, time.perf_counter() - _t0
                    ),
                )
...
class Switch(BaseSwitch):
    ...
    def _init(self):
        ...
        self.__output_axes = config.get("output-axes", None)
        if self.__output_axes is None:
            self.__axes = weakref.WeakValueDictionary(
                {name.upper(): axis for name, axis in self.__controller._axes.items()}
            )
        else:
            if type(self.__output_axes) == str:
                self.__output_axes = [self.__output_axes]
            self.__axes = weakref.WeakValueDictionary(
                {
                    name.upper(): axis
                    for name, axis in self.__controller._axes.items()
                    if name in self.__output_axes
                }
            )
    ...
    def _set(self, state):
        ...
        with self.__controller.sock.lock:
            ...
            if high_position < low_position:
                (low_position, high_position) = (high_position, low_position)
            ...
            position_scaling *= config.get("output-correction", 1.0)
    ...
    def scaling_and_offset(self):
        ...
        with self.__controller.sock.lock:
            ...
            scaling /= config.get("output-correction", 1.0)
