...
class Regul(object):
    ...
    def setPolynom(self, angle, a, b, c, d, e, f, g):
        ...
        if angle == "rx":
            ...
            var_name_b = (
                "plant/counters/counter_xtal_111_drx/6th-order-correction(unit)/b/Value"
            )
            var_name_c = (
                "plant/counters/counter_xtal_111_drx/6th-order-correction(unit)/c/Value"
            )
            var_name_d = (
                "plant/counters/counter_xtal_111_drx/6th-order-correction(unit)/d/Value"
            )
            var_name_e = (
                "plant/counters/counter_xtal_111_drx/6th-order-correction(unit)/e/Value"
            )
            var_name_f = (
                "plant/counters/counter_xtal_111_drx/6th-order-correction(unit)/f/Value"
            )
            var_name_g = (
                "plant/counters/counter_xtal_111_drx/6th-order-correction(unit)/g/Value"
            )
        elif angle == "ry":
            var_name_a = (
                "plant/counters/counter_xtal_111_dry/6th-order-correction(unit)/a/Value"
            ...
            var_name_b = (
                "plant/counters/counter_xtal_111_dry/6th-order-correction(unit)/b/Value"
            )
            var_name_c = (
                "plant/counters/counter_xtal_111_dry/6th-order-correction(unit)/c/Value"
            )
            var_name_d = (
                "plant/counters/counter_xtal_111_dry/6th-order-correction(unit)/d/Value"
            )
            var_name_e = (
                "plant/counters/counter_xtal_111_dry/6th-order-correction(unit)/e/Value"
            )
            var_name_f = (
                "plant/counters/counter_xtal_111_dry/6th-order-correction(unit)/f/Value"
            )
            var_name_g = (
        ...
        self.set_param(var_name_d, d)
        self.set_param(var_name_e, e)
        self.set_param(var_name_f, f)
        self.set_param(var_name_g, g)
    ...
    def getPolynom(self, angle):
        ...
        return (
            ...
            self.get_param(var_name_d),
            self.get_param(var_name_e),
            self.get_param(var_name_f),
            self.get_param(var_name_g),
        )
    ...
    def get_signal(self, signal):
        ...
        ##########################################################################
        ####          Save and Load (get and set) counter settings            ####
        ##########################################################################

    def save_counter_settings(self):

        counters_number = 41  #

        counter_settings_array_filename = "/users/blissadm/local/beamline_configuration/DCM/CALIB/PARAMS/counter_settings.dat"
        counter_settings_array = np.zeros((9 * counters_number))

        print("\nSave first 15 counters from FPGA1_SSIM1 to FPGA2_SSIM5\n")
        for i in range(
            counters_number - 26
        ):  # first 15 counters  from FPGA1_SSIM1 to FPGA2_SSIM5

            # breakpoint() # 9*i+1, 9*i+2, etc - debug expressions
            # # i, j, k - to go through all counters FPGA(j)_SSIM(i+1-k), where j is [1,2], for j=1: SSIM(1 to 10), j=2: SSIM(1 to 5)
            if i <= 9:
                j = 1
                k = 0
            else:
                j = 2
                k = 10
            counter_settings_array[9 * i] = self.get_param(
                "plant/counters/counter_FPGA%d_SSIM%d/offset(cnt)/Bias" % (j, i + 1 - k)
            )  # 9*i+1 #
            counter_settings_array[9 * i + 1] = self.get_param(
                "corr_val_a_FPGA%d_SSIM%d" % (j, i + 1 - k)
            )  # 9*i+2 #
            counter_settings_array[9 * i + 2] = self.get_param(
                "corr_val_b_FPGA%d_SSIM%d" % (j, i + 1 - k)
            )  # 9*i+3 #
            counter_settings_array[9 * i + 3] = self.get_param(
                "corr_val_c_FPGA%d_SSIM%d" % (j, i + 1 - k)
            )  # 9*i+4 #
            counter_settings_array[9 * i + 4] = self.get_param(
                "plant/counters/counter_FPGA%d_SSIM%d/6th-order-correction(unit)/d/Value"
                % (j, i + 1 - k)
            )  # 9*i+5 #
            counter_settings_array[9 * i + 5] = self.get_param(
                "plant/counters/counter_FPGA%d_SSIM%d/6th-order-correction(unit)/e/Value"
                % (j, i + 1 - k)
            )  # 9*i+6 #
            counter_settings_array[9 * i + 6] = self.get_param(
                "plant/counters/counter_FPGA%d_SSIM%d/6th-order-correction(unit)/f/Value"
                % (j, i + 1 - k)
            )  # 9*i+7 #
            counter_settings_array[9 * i + 7] = self.get_param(
                "plant/counters/counter_FPGA%d_SSIM%d/6th-order-correction(unit)/g/Value"
                % (j, i + 1 - k)
            )  # 9*i+8 #
            counter_settings_array[9 * i + 8] = self.get_param(
                "plant/counters/counter_FPGA%d_SSIM%d/correction_OnOff/Value"
                % (j, i + 1 - k)
            )  # 9*i+9 #

        print("\nSave 5 counters (15-20) counters FPGA2_SSIM6 to FPGA2_SSIM10\n")
        for i in range(
            counters_number - 36
        ):  #  5 counters (15-20) FPGA2_SSIM6 to FPGA2_SSIM10
            print(f"    Counter {i+1}/5", end="\r")
            # breakpoint()
            if i == 0:
                counter_settings_array[
                    9 * (counters_number - 26) + 9 * i
                ] = self.get_param(
                    "bragg_offset"
                )  # 9*(counters_number-26) + 9*i+1 #
                counter_settings_array[
                    9 * (counters_number - 26) + 9 * i + 1
                ] = self.get_param(
                    "plant/counters/counter_FPGA2_SSIM%d/6th-order-correction(unit)/a/Value"
                    % (i + 6)
                )  # 9*(counters_number-26) + 9*i+2 #
                counter_settings_array[
                    9 * (counters_number - 26) + 9 * i + 2
                ] = self.get_param(
                    "plant/counters/counter_FPGA2_SSIM%d/6th-order-correction(unit)/b/Value"
                    % (i + 6)
                )  # 9*(counters_number-26) + 9*i+3 #
                counter_settings_array[
                    9 * (counters_number - 26) + 9 * i + 3
                ] = self.get_param(
                    "plant/counters/counter_FPGA2_SSIM%d/6th-order-correction(unit)/c/Value"
                    % (i + 6)
                )  # 9*(counters_number-26) + 9*i+4 #
                counter_settings_array[
                    9 * (counters_number - 26) + 9 * i + 4
                ] = self.get_param(
                    "plant/counters/counter_FPGA2_SSIM%d/6th-order-correction(unit)/d/Value"
                    % (i + 6)
                )  # 9*(counters_number-26) + 9*i+5 #
                counter_settings_array[
                    9 * (counters_number - 26) + 9 * i + 5
                ] = self.get_param(
                    "plant/counters/counter_FPGA2_SSIM%d/6th-order-correction(unit)/e/Value"
                    % (i + 6)
                )  # 9*(counters_number-26) + 9*i+6 #
                counter_settings_array[
                    9 * (counters_number - 26) + 9 * i + 6
                ] = self.get_param(
                    "plant/counters/counter_FPGA2_SSIM%d/6th-order-correction(unit)/f/Value"
                    % (i + 6)
                )  #  9*(counters_number-26) + 9*i+7 #
                counter_settings_array[
                    9 * (counters_number - 26) + 9 * i + 7
                ] = self.get_param(
                    "plant/counters/counter_FPGA2_SSIM%d/6th-order-correction(unit)/g/Value"
                    % (i + 6)
                )  # 9*(counters_number-26) + 9*i+8 #
                counter_settings_array[
                    9 * (counters_number - 26) + 9 * i + 8
                ] = self.get_param(
                    "plant/counters/counter_FPGA2_SSIM%d/correction_OnOff/Value"
                    % (i + 6)
                )  # 9*(counters_number-26) + 9*i+9 #
            else:
                counter_settings_array[
                    9 * (counters_number - 26) + 9 * i
                ] = self.get_param(
                    "plant/counters/counter_FPGA2_SSIM%d/offset(cnt)/Bias" % (i + 6)
                )  # 9*(counters_number-26) + 9*i+1 #
                counter_settings_array[
                    9 * (counters_number - 26) + 9 * i + 1
                ] = self.get_param(
        ...
        print("\nSave last 21 counters  with counters metroFrame (33-35)\n")
        for i in range(
            counters_number - 20
        ):  # last 21 counters xtal(../1/2)_(111/311)_(z/dz, rx/drx, ry/dry) with counters metroFrame (33-35)
            print(f"    Counter {i+1}/21", end="\r")
            if i < 6:
                n_xtal = "xtal1_"
            elif i < 12:
                n_xtal = "xtal2_"
                ...
                n_xtal = "xtal_"
            if i in [0, 1, 2, 6, 7, 8, 15, 16, 17]:
                m_xtal = "311_"
            elif i in [3, 4, 5, 9, 10, 11, 18, 19, 20]:
                m_xtal = "111_"
            else:
                m_xtal = ""
            if i in range(0, 13, 3):
                k_xtal = "z"
            elif i in range(1, 14, 3):
                k_xtal = "ry"
            elif i in range(2, 15, 3):
                k_xtal = "rx"
            elif i in [15, 18]:
                k_xtal = "dz"
            elif i in [16, 19]:
            ...
            counter_settings_array[9 * (counters_number - 21) + 9 * i] = self.get_param(
                "plant/counters/counter_%s%s%s/offset(cnt)/Bias"
                % (n_xtal, m_xtal, k_xtal)
            )  # 9*(counters_number-21) + 9*i+1 #
            counter_settings_array[
                9 * (counters_number - 21) + 9 * i + 1
            ] = self.get_param(
                "plant/counters/counter_%s%s%s/6th-order-correction(unit)/a/Value"
                % (n_xtal, m_xtal, k_xtal)
            )  # 9*(counters_number-21) + 9*i+2 #
            counter_settings_array[
                9 * (counters_number - 21) + 9 * i + 2
            ] = self.get_param(
                "plant/counters/counter_%s%s%s/6th-order-correction(unit)/b/Value"
                % (n_xtal, m_xtal, k_xtal)
            )  # 9*(counters_number-21) + 9*i+3 #
            counter_settings_array[
                9 * (counters_number - 21) + 9 * i + 3
            ] = self.get_param(
                "plant/counters/counter_%s%s%s/6th-order-correction(unit)/c/Value"
                % (n_xtal, m_xtal, k_xtal)
            )  # 9*(counters_number-21) + 9*i+4 #
            counter_settings_array[
                9 * (counters_number - 21) + 9 * i + 4
            ] = self.get_param(
                "plant/counters/counter_%s%s%s/6th-order-correction(unit)/d/Value"
                % (n_xtal, m_xtal, k_xtal)
            )  # 9*(counters_number-21) + 9*i+5 #
            counter_settings_array[
                9 * (counters_number - 21) + 9 * i + 5
            ] = self.get_param(
                "plant/counters/counter_%s%s%s/6th-order-correction(unit)/e/Value"
                % (n_xtal, m_xtal, k_xtal)
            )  # 9*(counters_number-21) + 9*i+6 #
            counter_settings_array[
                9 * (counters_number - 21) + 9 * i + 6
            ] = self.get_param(
                "plant/counters/counter_%s%s%s/6th-order-correction(unit)/f/Value"
                % (n_xtal, m_xtal, k_xtal)
            )  # 9*(counters_number-21) + 9*i+7 #
            counter_settings_array[
                9 * (counters_number - 21) + 9 * i + 7
            ] = self.get_param(
                "plant/counters/counter_%s%s%s/6th-order-correction(unit)/g/Value"
                % (n_xtal, m_xtal, k_xtal)
            )  # 9*(counters_number-21) + 9*i+8 #
            counter_settings_array[
                9 * (counters_number - 21) + 9 * i + 8
            ] = self.get_param(
                "plant/counters/counter_%s%s%s/correction_OnOff/Value"
                % (n_xtal, m_xtal, k_xtal)
            )  # 9*(counters_number-21) + 9*i+9 #

        np.savetxt(counter_settings_array_filename, counter_settings_array)

        print("\n\nFINISHED")

    """ def test_save_counter_settings(self):
        #test indeces
        counters_number = 41
        test_counter_settings_array_filename = "/users/blissadm/local/beamline_configuration/DCM_ID21/CALIB/PARAMS/test_save_counter_settings.dat"
        test_counter_settings_array = np.zeros((9*counters_number))
        for j in range(9*counters_number):
            if j == 359 or j == 368:
                test_counter_settings_array[j] = (j+1-350)
            else:
                test_counter_settings_array[j] = (j+1)
        np.savetxt(test_counter_settings_array_filename, test_counter_settings_array)
    """

    def load_counter_settings(self):
        counter_settings_array_filename = "/users/blissadm/local/beamline_configuration/DCM/CALIB/PARAMS/counter_settings.dat"

        self.load_counter_settings_array = np.loadtxt(counter_settings_array_filename)
        counters_number = 41
        print("\nLoad first 15 counters from FPGA1_SSIM1 to FPGA2_SSIM5\n")
        for i in range(
            counters_number - 26
        ):  # first 15 counters from FPGA1_SSIM1 to FPGA2_SSIM5
            print(f"    Counter {i+1}/15", end="\r")
            # breakpoint() # 9*i+1, 9*i+2, etc - debug expressions
            # # i, j, k - to go through all counters FPGA(j)_SSIM(i+1-k), where j is [1,2], for j=1: SSIM(1 to 10), j=2: SSIM(1 to 5)
            if i <= 9:
                j = 1
                ...
                k = 10
            self.set_param(
                "plant/counters/counter_FPGA%d_SSIM%d/offset(cnt)/Bias"
                % (j, i + 1 - k),
                self.load_counter_settings_array[9 * i],
            )
            self.set_param(
                "corr_val_a_FPGA%d_SSIM%d" % (j, i + 1 - k),
                self.load_counter_settings_array[9 * i + 1],
            )
            self.set_param(
                "corr_val_b_FPGA%d_SSIM%d" % (j, i + 1 - k),
                self.load_counter_settings_array[9 * i + 2],
            )
            self.set_param(
                "corr_val_c_FPGA%d_SSIM%d" % (j, i + 1 - k),
                self.load_counter_settings_array[9 * i + 3],
            )
            self.set_param(
                "plant/counters/counter_FPGA%d_SSIM%d/6th-order-correction(unit)/d/Value"
                % (j, i + 1 - k),
                self.load_counter_settings_array[9 * i + 4],
            )
            self.set_param(
                "plant/counters/counter_FPGA%d_SSIM%d/6th-order-correction(unit)/e/Value"
                % (j, i + 1 - k),
                self.load_counter_settings_array[9 * i + 5],
            )
            self.set_param(
                "plant/counters/counter_FPGA%d_SSIM%d/6th-order-correction(unit)/f/Value"
                % (j, i + 1 - k),
                self.load_counter_settings_array[9 * i + 6],
            )
            self.set_param(
                "plant/counters/counter_FPGA%d_SSIM%d/6th-order-correction(unit)/g/Value"
                % (j, i + 1 - k),
                self.load_counter_settings_array[9 * i + 7],
            )
            self.set_param(
                "plant/counters/counter_FPGA%d_SSIM%d/correction_OnOff/Value"
                % (j, i + 1 - k),
                self.load_counter_settings_array[9 * i + 8],
            )

        print("\nLoad 5 counters (15-20) counters FPGA2_SSIM6 to FPGA2_SSIM105\n")
        for i in range(
            counters_number - 36
        ):  # 5 counters (15-20) counters FPGA2_SSIM6 to FPGA2_SSIM10
            print(f"    Counter {i+1}/5", end="\r")
            if i == 0:
                self.set_param(
                    "bragg_offset",
                    self.load_counter_settings_array[
                        9 * (counters_number - 26) + 9 * i
                    ],
                )
                self.set_param(
        ...
        print("\nLoad last 21 counters  with counters metroFrame (33-35)\n")
        for i in range(
            counters_number - 20
        ):  # last 21 counters xtal(../1/2)_(111/311)_(z/dz, rx/drx, ry/dry) with counters metroFrame (33-35)
            print(f"    Counter {i+1}/21", end="\r")
            ...
            if i in [0, 3, 6, 9, 12]:  # range(0,13,3):
                k_xtal = "z"
            elif i in [1, 4, 7, 10, 13]:  # range(1,14,3):
                k_xtal = "ry"
            elif i in [2, 5, 8, 11, 14]:  # range(2,15,3):
                k_xtal = "rx"
            elif i in [15, 18]:
                k_xtal = "dz"
            elif i in [16, 19]:
            ...
            self.set_param(
                "plant/counters/counter_%s%s%s/offset(cnt)/Bias"
                % (n_xtal, m_xtal, k_xtal),
                self.load_counter_settings_array[9 * (counters_number - 21) + 9 * i],
            )
            self.set_param(
                "plant/counters/counter_%s%s%s/6th-order-correction(unit)/a/Value"
                % (n_xtal, m_xtal, k_xtal),
...
class Motor(object):
    ...
    def _wait_start_move(self, timeout):
        ...
        while (time.perf_counter() - wait_start) < timeout:
            if self.is_moving == 1:
                return (True, time.perf_counter() - wait_start)
        return (False, 0)
    ...
    def _wait_stop_move(self, timeout):
        wait_stop = time.perf_counter()
        while (time.perf_counter() - wait_stop) < timeout:
            if self.is_moving == 0:
                return (True, time.perf_counter() - wait_stop)
        return (False, 0)
...
class Speedgoat(object):
    ...
    def set_param(self, param, value):
        ...
        if isinstance(value, np.ndarray):
            while not np.isclose(self.get_param(param), value).any():
                gevent.sleep(0.00001)  # 10us
        else:
            while self.get_param(param) != value:
                gevent.sleep(0.00001)  # 10us
