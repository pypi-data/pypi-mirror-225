...
def get_signal_infos(handle):
    return [get_signal_info(handle, i) for i in range(xpc.get_num_signals(handle))]
...
def get_signal_value_from_idxs(handle, idxs=None):
    if idxs is None:
        idxs = tuple(range(xpc.get_num_signals(handle)))
    n = len(idxs)
...
def get_scope_list(handle):
    n_scopes = xpc.get_num_scopes(handle)
...
def sc_get_signals(handle, scope_id):
    nb_signals = xpc.sc_get_num_signals(handle, scope_id)
...
def sc_get_data(
    ...
    if num_points is None:
        num_points = xpc.sc_get_num_samples(handle, scope_id) - first_point
