...
def setting_update_from_channel(value, setting_name=None, axis=None):
    ...
    if setting_name == "state":
        if axis._group_move._move_task is not None:
            # move started from this process: we have to
            # ignore updates from channel,
            # since state changes are reported directly
            # from '_set_moving_state' and '_set_move_done',
            # when "state" setting is set
            return
...
class AxisSettings:
    def __init__(self, axis):
        ...
        self._hash = settings.HashSetting(
            "axis.%s" % axis.name, default_values=axis.config.config_dict
        )
    ...
    def set(self, *settings_values):
        axis = self.__axis
        disabled_settings = self._disabled_settings
        updated_dict = {}
        events_dict = {}

        for setting_name, value in grouped(settings_values, 2):
            ...
            if setting_name in disabled_settings.names:
                if (
                    ...
                    events_dict[setting_name] = value
            else:
                events_dict[setting_name] = value
                if axis_settings.persistent_setting[setting_name]:
                    if value is None:
                        updated_dict[setting_name] = "None"
                    else:
                        updated_dict[setting_name] = value

        with self.lock:
            with KillMask():
                self._hash.update(updated_dict)

        for setting_name, value in events_dict.items():
            self._beacon_channels[setting_name].value = value

            event.send(axis, f"internal_{setting_name}", value)

            if setting_name == "state" or setting_name in disabled_settings.names:
                event.send_safe(axis, setting_name, value)
