...
from beartype.roar import BeartypeException, BeartypeDecorHintPep585DeprecationWarning
...
def add_autocomplete_property(inst, name, getter, setter=None, deleter=None):
    cls = type(inst)
    module = cls.__module__
    if not hasattr(cls, "__perinstance"):
        cls = type(cls.__name__, (cls,), {})
        ...
        cls.__module__ = module
        inst.__class__ = cls
    setattr(cls, name, autocomplete_property(getter, setter, deleter))
...
def chunk_list(lst, n):
    ...
    Each lists contain consecutive elements from the input list.
    """
...
def chunk_col(lst, n):
    ...
    Each lists contain elements of a distance of n from the input list.

    It is kind of transposition of the result from `chunk_list`.
    """
    n = int(numpy.ceil(len(lst) / n))
    size = len(lst)
    for i in range(0, n):
        yield lst[i:size:n]
...
Null = UndefinedType  # Note that Null was a class
"""Backward compatibility with BLISS <= 1.10"""
...
def _float_format(template_number):
    try:
        digits = int(numpy.ceil(numpy.log10(1 / template_number)))
...
def rounder(template_number, number):
    """Return rounded string representation of <number> according to <template_number>
    ...
    """
    fmt_string = f"{{:{_float_format(template_number)}}}"
    try:
...
def typecheck(function):
    """
    Typechecking decorator based on @beartype, but raises TypeError instead of
    beartype specific ones.
    """
    function = beartype(function)
    ...
    def wrapped_function(*args, **kwargs):
        ...
        return custom_error_msg(
            BeartypeException,
            f"Intended Usage: {function.__name__}({', '.join(mandatory_params)})  Hint:",
            new_exception_type=TypeError,
            ...
            for i, param in enumerate(params):
                if param.kind == inspect.Parameter.VAR_POSITIONAL:
                    ...
                    for j, arg in enumerate(var_args):
                        hint = args_pattern[j % len(args_pattern)]
                        if not is_bearable(arg, hint):
                            raise TypeError(f"Expected {hint}, found {arg.__class__}")
            return function(*args, **kwargs)
...
def rm_tree(pth: Path, ignore_not_empty: bool = True):
    for child in list(pth.iterdir()):
        if child.is_file():
            child.unlink()
        elif child.is_dir():
            rm_tree(child)
        # others can be: unix socket, for example
    try:
        pth.rmdir()
    except OSError as err:
        if err.errno == 39 and ignore_not_empty:
            return
        raise
...
def copy_tree(src: Path, dst: Path):
    for item in src.iterdir():
        ...
        dst_path = dst.joinpath(item.name)
        if src_path.is_dir():
