...
class GroupingMaster(AcquisitionMaster):
    ...
    def _reset_expiration_time(self, scan_node):
        proxy = get_redis_proxy(db=1)
        if proxy.ttl(scan_node.db_name) == -1:
            return
        data_db_names = set(scan_redis(scan_node.db_name + ":*", connection=proxy))
        parent_db_names = set(scan_node.get_db_names(include_parents=True))
        current_session.scan_saving.set_expiration_time(data_db_names, parent_db_names)
