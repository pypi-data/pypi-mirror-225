...
from blissdata.data.node import datanode_factory
...
class BasicScanSaving(EvalParametersWardrobe):
    ...
    def writer_options(self):
        return self.writer_object.writer_options()
    ...
    def set_expiration_time(self, data_db_names, parent_db_names):
        ...
        set_expiration_time(parent_db_names, data=False)
...
class ESRFScanSaving(BasicScanSaving):
    ...
    @property
    def template(self):
        version = self.scan_saving_config.get("directory_structure_version", 2)
        if version == 1:
            ...
            raise RuntimeError(
                "The scan saving key 'directory_structure_version' from the beamline configuration must be either 1 or 2."
            )
    ...
    def _validate_name(self, value: Union[str, int, None]):
        # "{" and "}" are forbidden because the name is used in `template`
        # os.sep is forbidding because the name is used in the data directory name
        if not isinstance(value, str):
            return
        for char in [os.sep, "{", "}"]:
            if char in value:
                raise ValueError(f"Character '{char}' should not be used")
    ...
    def _set_proposal_name(self, name, eval_dict=None, reset=False):
        ...
        self.proposal_session_name = None
        self._freeze_date()
        self._set_collection_name(None, eval_dict=eval_dict, reset=reset)
        if name:
            self.proposal.redo_expiration_time()
    ...
    @with_eval_dict
    def _set_collection_name(self, name: Union[str, None], eval_dict=None, reset=False):
        if name:
            self._validate_name(name)
        else:
            name = "sample"
        if not reset and name == self._collection:
            return
        self._close_dataset(eval_dict=eval_dict)
        self._close_collection()
        self._ensure_proposal()
    ...
    def icat_register_dataset(
        ...
        try:
            ...
            if raise_on_error:
                raise
            logtools.user_error(str(e))
            logtools.log_exception(self.proposal, str(e))
        else:
            logtools.user_print(f"Dataset '{dataset_name_or_id}' has been send to ICAT")
    ...
    def icat_save_dataset(
        ...
        try:
            ...
            logtools.user_print(f"Dataset '{dataset_name_or_id}' has been saved")
    ...
    def _get_dataset(self, dataset_name_or_id: Union[DatasetId, str]) -> Dataset:
        ...
        node = proposal.get_dataset_node(dataset_name_or_id)
        if node is None:
            raise RuntimeError(
    ...
    def icat_register_datasets(
        ...
        if not unconfirmed:
            logtools.user_print("All datasets are already registered in ICAT")
    ...
    def icat_dataset_info(self, timeout: Optional[bool] = None):
        ...
        if confirmed is None:
            logtools.user_print(
                f"Datasets: {len(unconfirmed)} unconfirmed, ??? confirmed"
            )
        else:
            logtools.user_print(
        ...
        if dataset_info:
            logtools.user_print(dataset_info)
    ...
    def icat_confirm_datasets(
        ...
        if confirmed is not None:
            for dataset_id in set(unconfirmed) & set(confirmed):
                node = proposal.get_dataset_node(dataset_id)
                if node is None:
                    continue
                dset = Dataset(node)
    ...
    def set_expiration_time(self, data_db_names, parent_db_names):
        """Set the expiration time of the Redis keys of the scan children only.
