...
class LimaAcquisitionMaster(AcquisitionMaster):
    ...
    def __iter__(self):
        if not self.prepare_once and not self.start_once:
            if not self.fast_synchro:
                if self.npoints == 0:
                    self.__current_wait_frame_id = self.npoints - 1
                else:
                    self.__current_wait_frame_id = (
                        self.npoints - 1 + self.__sequence_index
                    )
            self.__sequence_index += 1
            yield self
        else:
            if self.npoints == 0:
                ...
                total_frames = numpy.inf
                wait_frame_id_iter = count()
            else:
                total_frames = self.npoints
            ...
            while True:
                ...
                if last_image_acquired < total_frames - 1:
                    try:
    ...
    def set_image_saving(self, directory, prefix, force_no_saving=False):
        if self.save_flag and not force_no_saving:
            ...
            (
                ...
                validity,
            ) = self._lima_controller.get_mapped_path(directory, check_validity=True)
    ...
    def _get_saving_description(self):
        ...
        description = {
            ...
            "saving_frame_per_acquisition": self.npoints,
        ...
        try:
            while True:
                ...
                self.emit_progress_signal(
                    {
                        "last_image_ready": last_image_ready,
                        "last_image_saved": status["last_image_saved"],
                        "acq_state": acq_state,
                    }
                )
