...
from bliss.common.logtools import elog_print, user_print, elogbook
from bliss.common.interlocks import interlock_state  # noqa: F401
from blissdata.data import lima_image  # noqa: F401
...
__all__ = (
    ...
    + scans.__all__
    + ["lprint", "ladd", "elog_print", "elog_add", "elog_plot", "elogbook"]
    + [
        "cleanup",
        "error_cleanup",
        "plot",
...
def wm(*axes: _scannable_or_name, **kwargs):
    ...
    for wm_info in iter_axes_position(*axes, **kwargs):
        ...
        if wm_info.user_high_limit not in (None, err):
            user_high_limit = rounder(wm_info.axis.tolerance, wm_info.user_high_limit)
            dial_high_limit = rounder(wm_info.axis.tolerance, wm_info.dial_high_limit)
        else:
            user_high_limit = dial_high_limit = _MISSING_VAL

        if wm_info.user_low_limit not in (None, err):
            user_low_limit = rounder(wm_info.axis.tolerance, wm_info.user_low_limit)
            dial_low_limit = rounder(wm_info.axis.tolerance, wm_info.dial_low_limit)
        else:
        ...
        user_position = rounder(wm_info.axis.tolerance, user_position)
        dial_position = rounder(wm_info.axis.tolerance, dial_position)
        header.append(axis_label)
        User.append(None)
        ...
        offset = rounder(wm_info.axis.tolerance, wm_info.offset)
        ...
        for counter in counters:
            if isinstance(counter, str):
                if counter not in all_counters_names:
                    print("")
                    logtools.user_warning(f"'{counter}' is not a valid counter")
...
def edit_mg(mg: MeasurementGroup):
    ...
    Arguments:
        mg: a measurement group if left to None == default
            measurement group.
    """
    active_mg = current_session.active_mg if mg is None else mg
...
def silx_view(scan: typing.Union[Scan, int, None] = None):
    ...
    if scan is None:
        ...
        try:
            ...
            uris = scan_utils.scan_uris(scan_obj)
    else:
