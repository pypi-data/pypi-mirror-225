...
import serial
from serial.rfc2217 import *  # noqa: F403
...
class RFC2217(_BaseSerial):
    class TelnetCmd:
        ...
        def telnet_send_option(self, action, option):
            self.data += b"".join([IAC, action, option])  # noqa: F405
    ...
    class TelnetSubNego:
        ...
        def rfc2217_send_subnegotiation(self, option, value):
            value = value.replace(IAC, IAC_DOUBLED)  # noqa: F405
            self.data += (
                IAC + SB + COM_PORT_OPTION + option + value + IAC + SE  # noqa: F405
            )
    ...
    def __init__(
        ...
        self.telnet_options = [
            TelnetOption(
                telnet_cmd, "ECHO", ECHO, DO, DONT, WILL, WONT, REQUESTED  # noqa: F405
            ),
            TelnetOption(
                telnet_cmd, "we-SGA", SGA, WILL, WONT, DO, DONT, REQUESTED  # noqa: F405
            ),
            TelnetOption(
                telnet_cmd,
                "they-SGA",
        ...
        self.rfc2217_port_settings = {
            "baudrate": TelnetSubnegotiation(
                telnet_sub_cmd,
                "baudrate",
                SET_BAUDRATE,  # noqa: F405
                SERVER_SET_BAUDRATE,  # noqa: F405
            ),
            "datasize": TelnetSubnegotiation(
                telnet_sub_cmd,
                "datasize",
                SET_DATASIZE,  # noqa: F405
                SERVER_SET_DATASIZE,  # noqa: F405
            ),
            "parity": TelnetSubnegotiation(
                telnet_sub_cmd, "parity", SET_PARITY, SERVER_SET_PARITY  # noqa: F405
            ),
            "stopsize": TelnetSubnegotiation(
                telnet_sub_cmd,
                "stopsize",
                SET_STOPSIZE,  # noqa: F405
                SERVER_SET_STOPSIZE,  # noqa: F405
            ),
        }
        self.rfc2217_options = {
            "purge": TelnetSubnegotiation(
                telnet_sub_cmd, "purge", PURGE_DATA, SERVER_PURGE_DATA  # noqa: F405
            ),
            "control": TelnetSubnegotiation(
                telnet_sub_cmd, "control", SET_CONTROL, SERVER_SET_CONTROL  # noqa: F405
            ),
        }
        ...
        for option in self.telnet_options:
            if option.state is REQUESTED:  # noqa: F405
        ...
        with gevent.Timeout(
            ...
            if self.rtscts and self.xonxoff:
                ...
                self.rfc2217_options["control"].set(
                    SET_CONTROL_USE_HW_FLOW_CONTROL  # noqa: F405
                )
            elif self.xonxoff:
                self.rfc2217_options["control"].set(
                    SET_CONTROL_USE_SW_FLOW_CONTROL  # noqa: F405
                )
            else:
                self.rfc2217_options["control"].set(
                    SET_CONTROL_USE_NO_FLOW_CONTROL  # noqa: F405
                )
    ...
    def write(self, msg, timeout):
        msg = msg.replace(IAC, IAC_DOUBLED)  # noqa: F405
    ...
    def flushInput(self):
        ...
        purge.set(PURGE_RECEIVE_BUFFER)  # noqa: F405
        self._data = b""
        self._rx_filter = None
        self._socket.send(telnet_sub_cmd.data)
        telnet_sub_cmd.data = b""
    ...
    def _rfc2217_filter(self, data):
        if bytes([data[-1]]) == IAC and bytes([data[-2]]) != IAC:  # noqa: F405
        ...
        if self._pending_data:
            ...
            self._pending_data = None
        return data.replace(IAC_DOUBLED, IAC)  # noqa: F405
    ...
    def _parse_nego(self, telnet_cmd):
        ...
        while 1:
            while iac_pos == -1 or len(self._data[iac_pos:]) < 3:
                self._event.wait()
                self._event.clear()
                iac_pos = self._data.find(IAC)  # noqa: F405

            if (
                len(self._data[iac_pos:]) > 2
                and bytes([self._data[iac_pos + 1]]) == IAC  # noqa: F405
            ):  # ignore double IAC
                ...
                if command != SB:  # noqa: F405
                    # ignore other command than
                    if command in (DO, DONT, WILL, WONT):  # noqa: F405
                        known = False
                        ...
                        if not known:
                            if command == WILL:  # noqa: F405
                                telnet_cmd.telnet_send_option(
                                    DONT, option  # noqa: F405
                                )
                            elif command == DO:  # noqa: F405
                                telnet_cmd.telnet_send_option(
                                    WONT, option  # noqa: F405
                                )
                else:  # sub-negotiation
                    se_pos = self._data.find(IAC + SE)  # noqa: F405
                    while se_pos == -1:
                        self._event.wait()
                        self._event.clear()
                        se_pos = self._data.find(IAC + SE)  # noqa: F405
                    ...
                    if option == COM_PORT_OPTION:  # noqa: F405
                        if suboption == SERVER_NOTIFY_LINESTATE:  # noqa: F405
                            self._linestate = ord(value)
                        elif suboption == SERVER_NOTIFY_MODEMSTATE:  # noqa: F405
                            self._modemstate = ord(value)
                        elif suboption == FLOWCONTROL_SUSPEND:  # noqa: F405
                            self._remote_suspend_flow = True
                        elif suboption == FLOWCONTROL_RESUME:  # noqa: F405
                            self._remote_suspend_flow = False
                        else:
