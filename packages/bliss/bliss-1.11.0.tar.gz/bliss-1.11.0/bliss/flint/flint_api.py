...
class FlintApi:
    ...
    def set_icat_client_config(self, config: dict):
        manager = self.__flintModel.mainManager()
    ...
    def __get_plot_class_by_kind(self, plot_type: str):
        assert plot_type in ["scatter", "image", "curve", "mca", "onedim"]
        plot_classes = {
    ...
    def get_live_scan_plot_by_name(self, plot_type, plot_name: str):
        ...
        manager = self.__flintModel.mainManager()
        widget_class = manager.getWidgetClassFromPlotClass(plot_class)
        for iwidget, widget in enumerate(workspace.widgets()):
            if type(widget) is not widget_class:
                continue
    ...
    def get_default_live_scan_plot(self, plot_type):
        """Returns the identifier of the default plot according to it's type.
    ...
    def run_method(self, plot_id, method, args, kwargs):
        ...
        methodObject = self._get_method_object(plot_id, method + "()")
        return methodObject(*args, **kwargs)

    def _get_method_object(self, plot_id, method_path):
        """
        Returns a callable method from a plot.

        Arguments:
            plot_id: Identifier of the plot
            method_path: String description of the method. This can be in the
                         shape `foo()`, `prop.foo()`, or `meth().foo()`
        Returns:
            An object method.
        """
        holder = self._get_plot_widget(plot_id)
        plotWidget = holder.widget()

        methods = method_path.split(".")
        method = methods.pop(0)
        isMethod = method.endswith("()")
        method = method.replace("()", "")
        api = None
        if hasattr(holder, "plotApi"):
            api = holder.plotApi()
        if api and hasattr(api, method):
            obj = getattr(api, method)
        else:
            # TODO: It would be good to simplify that stuff now.
            if hasattr(plotWidget, method):
                # Method from the custom plot widget
                obj = getattr(plotWidget, method)
            else:
                # Method from PlotWidget class
                silxPlot = holder._silxPlot()
                if hasattr(silxPlot, method):
                    obj = getattr(silxPlot, method)
                else:
                    # Finally check the holder which is live plot
                    obj = getattr(holder, method)
        ...
        if not isMethod:
            raise ValueError(f"Requested method {method_path} is not callable")
        return obj
    ...
    def test_count_custom_plots(self):
        """Debug purpose function to count number of hold custom plots."""
        window = self.__flintModel.mainWindow()
        return window.countCustomPlots()
    ...
    def is_plot_exists(self, plot_id) -> bool:
        """Deprecated API in BLISS 1.10

        Use `plot_exists` instead.
        """
        self.plot_exists(plot_id)

    def plot_exists(self, plot_id) -> bool:
        """True if the plot id exists"""
    ...
    def add_plot(
        ...
        in_live_window: Optional[bool] = None,
    ) -> str:
        ...
        window.createCustomPlot(
            ...
            closeable=closeable,
            parentId=parent_id,
            parentLayoutParams=parent_layout_params,
            inLiveWindow=in_live_window,
        )
        return unique_name
        ...
        with model.transaction():
            ...
            if parentItem is not None:
                for userItem in list(model.items()):
                    if not isinstance(userItem, plot_item_model.UserValueItem):
                        continue
            ...
            if userItem is not None:
                ...
                if ydata is not None:
                    ...
                    userItem = plot_item_model.UserValueItem(model)
    ...
    def start_image_monitoring(self, channel_name, tango_address, kind="limavideo"):
        ...
        Arguments:
            ...
            kind: For now `limaimage` and `limavideo` are supported
        """
        from .manager import monitoring

        if kind == "limavideo":
            scan = monitoring.LimaVideoMonitoringScan(None, channel_name, tango_address)
        elif kind == "limaimage":
            scan = monitoring.LimaImageMonitoringScan(None, channel_name, tango_address)
        else:
            ValueError(f"Unknown monitoring kind {kind}")
        manager = self.__flintModel.mainManager()
    ...
    def set_displayed_channels(self, plot_id, channel_names, role: str = None):
        ...
        Arguments:
            plot_id: Id in the plot in Flint
            channel_names: Id in the plot in Flint
    ...
    def get_displayed_channels(self, plot_id):
    ...
    def request_select_shapes(
        ...
        if hasattr(plot, "deviceName"):
            deviceName = plot.deviceName()
            if deviceName is not None:
                selector.setLoadFromDevice(deviceName)
    ...
    def export_to_logbook(self, plot_id):
        ...
        widget = self._get_plot_widget(plot_id, custom_plot=True)
        if widget is None:
            raise ValueError("Widget %s not found" % plot_id)
        if isinstance(widget, CustomPlot):
            if hasattr(widget.widget(), "exportToLogbook"):
                # Handle custom plot
                icatClient = self.__flintModel.icatClient()
                if icatClient is None:
                    raise RuntimeError("Flint was not setup with Icat")
                widget.widget().exportToLogbook(icatClient)
                return
            raise RuntimeError("This widget do not allow to export to logbook")
