...
import gevent
import gevent.lock
from gevent import socket, event, queue
...
from blissdata.redis import RedisConnectionManager
...
class Connection:
    ...
    def __init__(self, host=None, port=None):
        ...
        port = port or os.environ.get("BEACON_PORT") or 25000
        try:
            port = int(port)
        except ValueError:
            if not os.access(port, os.R_OK):
                raise RuntimeError("Port should be a TCP port (int) or Unix socket")
        ...
        self._redis_connection_manager_create_lock = gevent.lock.Semaphore()
