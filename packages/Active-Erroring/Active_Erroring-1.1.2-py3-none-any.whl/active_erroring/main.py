import logging
import inspect


class ActiveErroring:
    def __init__(self):
        pass

    @staticmethod
    def drop_error(error_name=None, message=None, fake_code=None, error_line=None, relative_path=None) -> None:
        """
        Parameters:
            error_name (str): The name of the error you want to throw. By default, it's MeowError
            message (str):  The message of the error. By default, it's Fuck.
            fake_code (str): The fake code that caused an error. By default, it's not included.
            error_line (str): The line of the fake code that caused an error.
            relative_path (str): By default, it's set to abspath.

        Returns:
            None

        """

        if not error_name:
            error_name = "MeowError"

        if not message:
            message = "Fuck"

        if not error_line:
            error_line = "-1"

        if not relative_path:
            caller_frame = inspect.currentframe().f_back
            relative_path = caller_frame.f_globals['__file__']

        if type(fake_code) is str:
            fake_code_length = len(fake_code)
            the_code = f"\n    {fake_code}\n    {fake_code_length * '~'}"
        else:
            the_code = ""

        formatted_file_path = f"\033[34m{relative_path}\033[0m"

        logger = logging.getLogger(__name__)

        out = f"""Traceback (most recent call last):
  File "{formatted_file_path}", line {error_line}, in <module>{the_code}
{error_name}: {message}
"""

        logger.error(out)


if __name__ == '__main__':
    ActiveErroring.drop_error()
