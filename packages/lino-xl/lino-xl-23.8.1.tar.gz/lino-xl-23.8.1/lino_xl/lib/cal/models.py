...
from lino.mixins import Referrable, Draggable
...
from lino.modlib.system.choicelists import DurationUnits, Recurrencies
...
class Calendar(mixins.BabelNamed, GoogleCalendarSynchronized):
...
dd.update_field(
    RecurrentEvent, 'every_unit', default='yearly', blank=False, null=False)
...
class Event(
    Component, Ended, Assignable, TypedPrintable, Mailable, Postable,
    Publishable, GoogleCalendarEventSynchronized, Draggable, MemoReferrable):
    ...
    notify_before = models.IntegerField(verbose_name=_("Notify before"), null=True, blank=True)
    ...
    def on_dropped(self, ar, **kw):
        start_date = datetime.datetime.fromisoformat(ar.rqdata.get('start_date')).date()
    ...
    def get_display_color(self):
        ...
        return settings.SITE.site_config.default_color
...
if dd.is_installed('notify'):
    ...
    def get_notification_queryset():
        ...
        db_engine = settings.DATABASES[Event.objects.db_manager().db]['ENGINE'].lower()
        ...
        def get_duration():
            if postgres:
                return (models.When(
                        ...
                        then=Concat(
                            models.F('notify_before'), models.Value(' minutes'),
                            output_field=models.DurationField())),
                    models.When(
                        notify_unit=NotifyBeforeUnits.get_by_name('hours').value,
                        then=Concat(
                            models.F('notify_before'), models.Value(' hours'),
                            output_field=models.DurationField())),
                    models.When(
                        notify_unit=NotifyBeforeUnits.get_by_name('days').value,
                        then=Concat(
                            models.F('notify_before'), models.Value(' days'),
                            output_field=models.DurationField())),
                    models.When(
                        notify_unit=NotifyBeforeUnits.get_by_name('weeks').value,
                        then=Concat(
                            models.F('notify_before'), models.Value(' weeks'),
                            output_field=models.DurationField())))
            if oracle:
                raise NotImplementedError(
                    "Require implementation of int convertion to INTERVAL.\n" +
                    "See: https://docs.djangoproject.com/en/4.1/ref/models/fields/#durationfield for help.")

            return (models.When(notify_unit=NotifyBeforeUnits.get_by_name('minutes').value,
                    then=models.F('notify_before') * 60 * 1000000),
                models.When(notify_unit=NotifyBeforeUnits.get_by_name('hours').value,
                    then=models.F('notify_before') * 60 * 1000000 * 60),
                models.When(notify_unit=NotifyBeforeUnits.get_by_name('days').value,
                    then=models.F('notify_before') * 60 * 1000000 * 60 * 24),
                models.When(notify_unit=NotifyBeforeUnits.get_by_name('weeks').value,
                    then=models.F('notify_before') * 60 * 1000000 * 60 * 24 * 7))

        now = timezone.now()
        tolerance = datetime.timedelta(minutes=scheduler_interval/2)
        threshold = now + tolerance

        return Event.objects.filter(notify_before__isnull=False,
                notified=False, start_date__isnull=False
            ).annotate(start_timed=models.Case( # dynamic start_time, db value if exists else CSH:00:00 # CSH -> calendar_start_hour
                models.When(Q(start_time__isnull=True),
                    then=models.Value(datetime.time(settings.SITE.calendar_start_hour, 0))),
                default=models.F('start_time'),
                output_field=models.TimeField()
            )).annotate(start_datetime=Concat(
                models.F('start_date'), models.Value(' '), models.F('start_timed'),
                output_field=models.DateTimeField()
            )).annotate(notify_before_duration=models.Case(
                *get_duration(),
                output_field=models.DurationField()
            )).annotate(schedule=Cast(
                models.F('start_datetime') - models.F('notify_before_duration'),
                output_field=models.DateTimeField()
            )).filter(schedule__lte=threshold)
        ...
        for e in queryset:
            def event_message(user, mail_mode):
                if not hasattr(e, 'start_datetime'):
                    e.start_datetime = datetime.datetime.combine(e.start_date, e.start_time)
                subject = _("Upcoming Calendar Event @ {}").format(e.start_datetime.isoformat())
                context.update(user=e.user, obj=e)
                body = template.render(**context)
                return (subject, body)

            rt.models.notify.Message.emit_notification(
                None, None, message_type, event_message, [(e.user, None)])
