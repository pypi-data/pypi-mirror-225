...
from lino.modlib.system.choicelists import Recurrencies, Weekdays
...
class MoveEntryNext(dd.MultipleRowAction):
    ...
    def get_action_permission(self, ar, obj, state):
        ...
        if state.fixed:
            return False
        return super().get_action_permission(
            ar, obj, state)
    ...
    def disable_delete(self, ar=None):
        for ee in self.get_existing_auto_events():
            if ee.is_user_modified():
                return _("Cannot delete {} because it has user-modified calendar entries.").format(self)
    ...
    def get_wanted_auto_events(self, ar=None):
        ...
        for ee in qs:
            if date is None and ee.is_user_modified():
                next_date = rset.get_next_suggested_date(ar, ee.start_date)
                date = next_date
            ...
            if ee.auto_type in existing:
                ...
                if ee.is_user_modified():
                    ...
                    if oe.is_user_modified():
                        ar.info("Oops: duplicate auto_type {}".format(ee.auto_type))
                        return wanted, unwanted
                    unwanted.append(oe)
            ...
            while max_events is None or event_no < max_events:
                ...
                if ignore_before and date < ignore_before:
                    ...
                    we = Event(
                        ...
                        user=user,
                        start_date=date,
                        summary=self.update_cal_summary(event_type, event_no),
                    ...
                    if ee is None:
                        wanted.append(we)
                        # print("20221129 wanted event", ee)
                    elif ee.is_user_modified():
                        # print("20221129 is_user_modified", ee)
                        if last_date and ee.start_date < last_date:
                            msg = _("Automatic entry {} ({}) goes back in time.")
                            raise Warning(msg.format(ee.auto_type, ee.start_date))
                        # wanted.append(ee)
                        ar.debug(
                ...
                date = rset.find_start_date(date)
                if date is None:
                    ar.info("Could not find next date after %s (%s).", last_date, event_no)
                    break
