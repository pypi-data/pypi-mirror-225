from typing import Dict, Any, Iterable, Optional, TypeVar, Type, List
...
class BaseModel(MetaModel):
        ...
        if self._references:
            for ref in self._references:
                if ref.on_destroy == OnDestroy.RAISE:
                    ref_count = await ref.ref_class.find({ref.ref_field: self.id}).count()
                    if ref_count > 0:
                        raise ObjectHasReferences(f"{self.__class__.__name__} has dangling references")
    ...
    async def to_dict_ext(self,
        ...
        for field_name in fields:
            ...
            if descriptor is not None:
                ...
                computed = self._computed_fields.get(field_name)
                if not computed:
                    continue
                value = getattr(self, field_name)()
    ...
    def exposed_fields(cls) -> List[str]:
        ...
        fields.extend(cls._computed_fields.keys())
        return fields
