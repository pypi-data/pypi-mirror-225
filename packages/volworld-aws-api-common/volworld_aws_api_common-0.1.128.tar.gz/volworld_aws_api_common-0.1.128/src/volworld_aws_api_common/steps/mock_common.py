...
from volworld_common.test.behave.ACotA import ACotA
...
def when_system_reset_all_mock_scenario(c):
    Wiremock.rest_all_scenario()


@given('system reset all mock scenario')
def given_system_reset_all_mock_scenario(c):
    Wiremock.rest_all_scenario()
...
@when('System set mock scenario state as [{state}]')
def when__set_system_mock_state_by_name(c, state: str):
    set_system_mock_state_by_name(c, state)


@given('System set mock scenario state as [{state}]')
