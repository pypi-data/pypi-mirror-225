...
def cleaner(text):
    ...
    clean_text = re.sub("""(<img)+[a-zA-Z\d\s=":/.?\-;&,_{}!@#$%^*()_+=\[\]|\\'~`‘’]+(>)""", "<img_tags_retained>", text)
    ...
    #wsj
    clean_text = re.sub("Newsletter Sign-up","",clean_text)
    clean_text = re.sub("WSJ Pro","",clean_text)
    clean_text = re.sub(r'\*\*\* ONE-TIME USE \*\*\*',"",clean_text)
    clean_text = re.sub(r'<p>Write to [^<]*</p>', '', clean_text)
