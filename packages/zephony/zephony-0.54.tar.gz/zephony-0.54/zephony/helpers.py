...
class ApiFlask(Flask):
    ...
    def make_response(self, rv):
        if isinstance(rv, dict):
            try:
                # `Request` context is mandatory, if return type is dict.
                if request and request.response_object_details:
                    request.response_object_details['status'] = rv.get('http_status')
                    request.response_object_details['end_date_time'] = datetime.now(timezone.utc)

                if 'errors' in rv and rv['errors']:
                    request.response_object_details['errors'] = rv['errors']
                
                request.response_object_details['request-response cycle time'] = request.response_object_details.get('end_date_time') - request.response_object_details.get('start_date_time')
                pprint(request.response_object_details)
            except:
                logger.info(f'response : {rv}')
