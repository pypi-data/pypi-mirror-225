...
class Nlb(Construct):
    ...
    def __init__(
        ...
        if not is_production and create_dns.lower() == "true":
            ...
            route53_zone = _route53.PrivateHostedZone.from_hosted_zone_attributes(self,
            ...
            _route53.ARecord(self,
                             ...
                             zone=route53_zone,
                             target=_route53.RecordTarget.from_alias(_route53_targets.LoadBalancerTarget(nlb)),
                             record_name=f"{dns_record}.{domain_name}"
                             )
        self.nlb = nlb
        self.tg = self._create_tg(id=main_component_name, ec2_traffic_port=ec2_traffic_port,
                                  healthy_threshold_count=healthy_threshold_count,
                                  unhealthy_threshold_count=unhealthy_threshold_count, interval=interval,
                                  timeout=timeout)
