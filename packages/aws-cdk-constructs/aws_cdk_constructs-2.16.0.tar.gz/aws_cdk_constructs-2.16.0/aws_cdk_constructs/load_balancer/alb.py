...
class Alb(Construct):
        ...
        if use_cdn:
            ...
            if not internet_facing:
                ...
                if self.has_upstream:
                    security_group.sg.add_ingress_rule(
                        peer=self.upstream_security_group_as_peer,
                        connection=_ec2.Port.tcp(80) if self.is_https else _ec2.Port.tcp(int(traffic_port)),
                    ...
                    if self.is_https:
                        ...
                        if traffic_port != 80:
                            security_group.sg.add_ingress_rule(
                                peer=_ec2.Peer.any_ipv4(),
                                connection=_ec2.Port.tcp(443) if not traffic_port else _ec2.Port.tcp(int(traffic_port)),
                                description="Everyone to HTTPS"
                                )
                    else:
                        security_group.sg.add_ingress_rule(
                            peer=_ec2.Peer.any_ipv4(),
                            connection=_ec2.Port.tcp(80) if not traffic_port else _ec2.Port.tcp(int(traffic_port)),
    ...
    def add_https_listener(self) -> None:
        if self.tg:
            ...
            default_action = _alb.ListenerAction.fixed_response(200, message_body=f"{self.app_name} default landing page")
            self.listener = self.alb.add_listener(
                id="is_https",
                port=int(self.traffic_port.to_string()),
                open=False,
