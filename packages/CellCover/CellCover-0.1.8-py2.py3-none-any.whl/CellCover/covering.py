...
def SensList(mat, CellTypeLabels, CellTypeNames):
    ...
    for i in range(len(CellTypeNames)):
        ct = CellTypeNames[i]
        data = mat[CellTypeLabels == ct,:]
        sens = data.sum(axis = 0) / data.shape[0]
        sens_list.append(sens)
    sens_list = np.array(sens_list)
    return sens_list
    
def weight_component(sens,CellTypeNames,ct,w_type = "mean"):
    ct_index = np.where(CellTypeNames == ct)[0].item()
    not_ct_index = [i for i in range(len(CellTypeNames)) if i != ct_index]
...
def weight(mat,sens,CellTypeLabels,CellTypeNames,ct,gene,te = 0.1,top_num_gene = 6000,w_type = "mean",epsilon = 0.00001):
    #CT = np.unique(y)
    X = mat[CellTypeLabels == ct,:]
    numerator, denominator = weight_component(sens,CellTypeNames,ct,w_type)
