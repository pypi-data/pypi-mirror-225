...
from ..layers import (OutputQuantizer, OutputObserver, PaddedConv2D, DepthwiseConv2DTranspose,
                      Attention, BufferTempConv, DepthwiseBufferTempConv, reset_buffers)
...
def calibrate(model, qmodel, samples=None, num_samples=1024, batch_size=None, epochs=1):
    ...
    if len(buf_layers) > 0:
        if not isinstance(samples, np.ndarray):
            raise TypeError("TENN models calibration is only possible with np.array samples."
                            ...
                            " format before the calibration with the following shape:"
                            " [batch_size*Seq, dim_0,, ..., dim_n] where:"
                            " - batch_size is the same batch_size provided to the calibration."
                            " - Seq is a dataset parameter that defines the temporally coherent"
                            " data (eg number of frames per video clips).")
        # With TENN models, the num of samples is infered from the provided samples.
        num_samples = len(samples)
        if batch_size is None:
            warnings.warn("batch_size not specified. With spatiotemporal TENN models batch_size"
                          " must be given for the calibration, otherwise the result is"
                          " unpredictible. Refers to 'calibrate' function docstring for more"
                          " details.")
    ...
    # reset calibration_model buffers before the inference
    reset_buffers(calibration_model)
    for i in range(epochs):
        ...
        calibration_model.predict(x=calib_samples, steps=calibration_steps, batch_size=batch_size)
