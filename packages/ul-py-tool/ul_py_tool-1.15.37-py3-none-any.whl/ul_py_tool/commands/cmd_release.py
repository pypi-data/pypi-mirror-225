    ...
    def run(self) -> None:
        ...
        with stepper.step(f'{FG_GREEN}release{NC}'):
            ...
            for component_name, component_config in self.assembly_file.components.items():
                ...
                # pods, and then recreate them. But TODO: check if we need to scale all deployments to 0 and how ignore this
                if component_config.force_disable:
                    run_command(
                        [
                            f'kubectl scale deployment -n {release_name} --replicas 0 --all --kubeconfig={kubeconfig_file_path}',
                        ],
                        cwd=component_directory,
                        ignore_errors=True,
                    )
                    # Some time for kubernetes to destroy resources
