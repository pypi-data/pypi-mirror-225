...
from vpt_plugin_watershed.watershed.entity import Entity

os.environ["TF_CPP_MIN_LOG_LEVEL"] = "3"
from vpt_core.io.output_tools import suppress_output  # noqa

from vpt_plugin_watershed.stardist import StarPolygon  # noqa
...
class StardistSeedsExtractor:
    ...
    def extract_seeds(self, seed_images: np.ndarray, entity: Entity) -> Tuple[List[StardistResult], np.ndarray]:
        ...
        if not Entity.Seeds & entity:
            vector = [[StarPolygon(center=np.array(0), points=np.array(0))] * len(x) for x in vector]
