...
from pywa.types import Button, SectionList, Message, CallbackButton, CallbackSelection, MessageStatus, Contact, \
    MediaUrlResponse, ProductsSection, BusinessProfile, Industry, CommerceSettings, MediaUrlToCache
...
class WhatsApp:
    ...
    def _resolve_media_param(
            self,
            media: str | MediaUrlToCache | bytes | BinaryIO,
            mime_type: str,
            filename: str,
    ) -> tuple[bool, bool, str]:
        """
        ...
        Returns:
            is_url, is_cached_url, media_id_or_url
        """
        if isinstance(media, str):
            if media.startswith(("https://", "http://")):
                return True, False, media
            elif not os.path.isfile(media) and media.isdigit():
                return False, False, media  # media is a media ID
            else:
                return False, False, self.upload_media(
                    media=media,
                    mime_type=mimetypes.guess_type(media)[0] or mime_type,
                    filename=filename or os.path.basename(media)
                )
        elif isinstance(media, MediaUrlToCache):
            return True, True, media.url
        else:
            return False, False, self.api.upload_media(media=media, mime_type=mime_type, filename=filename)['id']

    def send_image(
            self,
            to: str | int,
            image: str | MediaUrlToCache | bytes | BinaryIO,
        ...
        Args:
            to: The phone ID of the WhatsApp user.
            image: The image to send (either a media ID, URL, ``MediaUrlToCache`` obj, file path, bytes, or an open() file).
        ...
        """
        is_url, is_cached_url, media = self._resolve_media_param(media=image, mime_type="image/jpeg", filename="image.jpg")
        if not buttons:
            return self.api.send_media(
                to=str(to),
                media_id_or_url=media,
                is_url=is_url,
                media_type="image",
                caption=caption,
                cache_headers=image.to_headers() if is_cached_url else None,
            )['messages'][0]['id']
    ...
    def send_video(
            self,
            to: str | int,
            video: str | MediaUrlToCache | bytes | BinaryIO,
            caption: str | None = None,
            body: str | None = None,
            footer: str | None = None,
            buttons: Iterable[Button] | None = None,
        ...
        Args:
            to: The phone ID of the WhatsApp user.
            video: The video to send (either a media ID, URL, ``MediaUrlToCache`` obj, file path, bytes, or an open() file).
        ...
        """
        is_url, is_cached_url, media = self._resolve_media_param(media=video, mime_type="video/mp4", filename="video.mp4")
        if not buttons:
            return self.api.send_media(
                to=str(to),
                media_id_or_url=media,
                ...
                media_type="video",
                caption=caption,
                cache_headers=video.to_headers() if is_cached_url else None,
            )['messages'][0]['id']
        if not body and not caption:
    ...
    def send_document(
            self,
            to: str | int,
            document: str | MediaUrlToCache | bytes | BinaryIO,
        ...
        Args:
            to: The phone ID of the WhatsApp user.
            document: The document to send (either a media ID, URL, ``MediaUrlToCache`` obj, file path, bytes, or an open() file).
        ...
        """
        (is_url, is_cached_url, media) = self._resolve_media_param(
            media=document,
        ...
        if not buttons:
            return self.api.send_media(
                ...
                caption=caption,
                filename=filename,
                cache_headers=document.to_headers() if is_cached_url else None,
            )['messages'][0]['id']
        if not body and not caption:
    ...
    def send_audio(
            self,
            to: str | int,
            audio: str | MediaUrlToCache | bytes | BinaryIO,
    ) -> str:
        """
        ...
        Args:
            to: The phone ID of the WhatsApp user.
            audio: The audio file to send (either a media ID, URL, ``MediaUrlToCache`` obj, file path, bytes, or an open() file).

        Returns:
            The message ID of the sent audio file.
        """
        is_url, is_cached_url, media = (
            self._resolve_media_param(media=audio, mime_type="audio/mpeg", filename="audio.mp3")
        )
        return self.api.send_media(
            to=str(to),
            media_id_or_url=media,
            is_url=is_url,
            media_type="audio",
            cache_headers=audio.to_headers() if is_cached_url else None,
        )['messages'][0]['id']

    def send_sticker(
            self,
            to: str | int,
            sticker: str | MediaUrlToCache | bytes | BinaryIO,
    ) -> str:
        """
        ...
        Args:
            to: The phone ID of the WhatsApp user.
            sticker: The sticker to send (either a media ID, URL, ``MediaUrlToCache`` obj, file path, bytes, or an open() file).

        Returns:
            The message ID of the sent message.
        """
        is_url, is_cached_url, media = (
            self._resolve_media_param(media=sticker, mime_type="image/webp", filename="sticker.webp")
        )
        return self.api.send_media(
            to=str(to),
            media_id_or_url=media,
            is_url=is_url,
            media_type="sticker",
            cache_headers=sticker.to_headers() if is_cached_url else None,
        )['messages'][0]['id']
