...
class YFinanceAPI(DataApi):
    ...
    def get_historical_bars(self,
        ...
        cache_filename = self.cache_filename(tickers, start, end, timeframe, "yfinance_bars")
        ...
        historical_bars = pd.concat(concatenated_data, axis=1).swaplevel(axis=1).sort_index(axis=1)
    ...
    def _query_historical_bars(self, tickers, timeframe, start: datetime.datetime, end: datetime.datetime):
        formatted_data = {}

        for ticker in tickers:
            url = (f'{self.base_url}{ticker}?period1={int(start.timestamp())}&period2={int(end.timestamp())}'
