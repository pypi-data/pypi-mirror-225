...
class DHPotluck:
    ...
    def _process_request(self):
        ...
        if isinstance(user, AuthenticatedUser):
            user_tags['dh_potluck.current_user'] = user.id
        ...
        if isinstance(impersonator, AuthenticatedUser):
            user_tags['dh_potluck.current_impersonator'] = impersonator.id
        if isinstance(impersonator, ApplicationUser):
            user_tags['dh_potluck.current_impersonator'] = type(user).__name__
        if user_tags:
            span = tracer.current_span()
            if span:
