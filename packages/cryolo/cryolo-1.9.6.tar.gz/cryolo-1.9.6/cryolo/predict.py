...
def main(args=None):
    ...
    if isinstance(args.gpu, list):
        args.gpu = [re.sub("[^0-9]", "", num) for num in args.gpu if re.sub("[^0-9]", "", num).isnumeric()]
        if len(args.gpu) == 1:
            num_gpus = 1
            if args.gpu[0] != "-1":
    ...
    config_proto_args = {}
    if args.num_cpu != -1:
        config_proto_args["intra_op_parallelism_threads"] =args.num_cpu
        config_proto_args["inter_op_parallelism_threads"] = args.num_cpu
    ...
    ###
    # WRITE CSPARC Files - DAMN STUPID CRYOLO SOFTWARE DESIGN :-/
    ###
    if predict_3D == False:
        all_coordinates = []
        all_micrographs = []
        if filament_mode:
            for picking_result_subset in tqdm(picking_result_with_boxes_subsets, desc="Write cryoSPARC coordinates"):
                for result in picking_result_subset:
                    if result["filaments"] or write_empty:
                        for frame in result["filaments"]:
                            all_coordinates.extend(result["filaments"][frame])
                            all_micrographs.extend([os.path.basename(result['img_path'])]*len(result["filaments"][frame]))

        else:
            all_coordinates = []
            all_micrographs = []
            for box_to_write in tqdm(picking_results,desc="Write cryoSPARC coordinates"):
                all_coordinates.extend(box_to_write["boxes"][0])
                all_micrographs.extend([os.path.basename(box_to_write['img_path'])]*len(box_to_write["boxes"][0]))
        write_cryosparc_spa_coords(outdir,
                                   coordinates=all_coordinates,
                                   micrograph_name=all_micrographs)
                ...
                if is_3d == False:
                    for frame in result["filaments"]:
                        ...
                        if len(result["filaments"]) > 1:
                            ...
                            root_ext = os.path.splitext(star_segmented)
                            star_segmented = root_ext[0] + "_slice" + str(frame).zfill(4) + \
                                             root_ext[1]
                       ...
                       # if not os.path.exists(os.path.dirname(star_cryopsarc)):
                        #    os.makedirs(os.path.dirname(star_cryopsarc))
                        ...
                        #CoordsIO.write_cryosparc_star_file(
                        #    path=star_cryopsarc,
                        #    coordinates=result["filaments"][frame],
                        #    micrograph_name=os.path.basename(result["img_path"])
                        #)
                else:
...
def write_tomo_coords(outdir: str, box_to_write, write_empty: bool):
...
def write_cryosparc_spa_coords(outdir: str, coordinates, micrograph_name):
    csparc_path = os.path.join(outdir, "CRYOSPARC", "cryosparc.star")
    os.makedirs(os.path.dirname(csparc_path), exist_ok=True)

    CoordsIO.write_cryosparc_star_file(path=csparc_path,
                                       coordinates=coordinates,
                                       micrograph_name=micrograph_name
                                       )
def write_spa_coords(outdir: str, box_to_write):
    original_path = box_to_write["pth"]
    eman_path = original_path
    box_set = box_to_write["boxes"]
    for boxes_index, boxes in enumerate(box_set):
        ...
        if outdir is not None:
            filename = os.path.basename(cbox_path)
            cbox_path = os.path.join(outdir, "CBOX", filename)

        # Create directory if it does not existes
        if not os.path.exists(os.path.dirname(cbox_path)):
            os.makedirs(os.path.dirname(cbox_path))
        CoordsIO.write_cbox_file(path=cbox_path, coordinates=box_to_write["boxes_unfiltered"][boxes_index])
...
def prediction_result_to_disk(outdir, picking_results, write_empty=False):
    for box_to_write in picking_results:
        if "is_3d" in box_to_write and box_to_write["is_3d"] == True:
            write_tomo_coords(outdir, box_to_write, write_empty)
        else:
            write_spa_coords(outdir, box_to_write)
...
def do_prediction(
    ...
    **kwargs
    ...
    for batch, tile_coord, slices_indicies, img_size, batch_img_paths in batch_generator:
        ...
        prnt_progress.append(f" Time: {measured_times[-1]:.2f}")
        print(*prnt_progress)
        batch_num += 1
