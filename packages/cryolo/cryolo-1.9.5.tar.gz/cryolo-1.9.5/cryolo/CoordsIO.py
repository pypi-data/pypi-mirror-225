...
def read_star_filament_file_(path, box_width, min_distance=0):
    sfile = star.StarFile(path)
...
def write_star_filemant_file_(path, filaments):
...
def read_eman1_boxfile(path, is_SPA=True, box_size_default=200) -> List[BoundBox]:
    """
    Read a box file in EMAN1 box format.
    :param path: Path to box file
    :param is_SPA: False if the file is referred to a tomo
    :param box_size_default: boxsize for tomo box files
    ...
    if is_SPA: #2D case is always with width and height values
        ...
        boxes = [BoundBox(x=box[0]-w/2, y=box[1]-h/2, z=box[2], w=w, h=h) for box in boxreader]
    return boxes
...
def read_star_filament_file(path, box_width, min_distance=0) -> typing.List[Filament]:
    sfile = star.StarFile(path)
    filaments = {}
    for i in range(len(sfile['']['_rlnCoordinateX'])):
        c = 1
        if '_rlnAutopickFigureOfMerit' in sfile['']:
            c= sfile['']['_rlnAutopickFigureOfMerit'][i]
        box = BoundBox(
            x=sfile['']['_rlnCoordinateX'][i]-box_width/2,
            y=sfile['']['_rlnCoordinateY'][i]-box_width/2,
            w=box_width,
            h=box_width,
            c=c
        )
        ...
        fid = int(sfile['']['_rlnHelicalTubeID'][i])
        if fid in filaments:
...
def write_cryosparc_star_file(path: str, coordinates: Union[List[BoundBox], List[Filament]], micrograph_name: str):
    ...
    is_filament_data = isinstance(coordinates[0], Filament)
    columns = ['_rlnMicrographName', '_rlnCoordinateX', '_rlnCoordinateY', '_rlnAutopickFigureOfMerit']
    if is_filament_data:
    ...
    if os.path.exists(path):
        starfile = star.StarFile(path)
        if '_rlnHelicalTubeID' in starfile[''] and len(starfile['']['_rlnHelicalTubeID'])>0:
            fid_offset=np.max(starfile['']['_rlnHelicalTubeID'])

    for clm in columns:
        data_dict[clm] = []
    if is_filament_data:
        for fil_id, fil in enumerate(coordinates):
            utils.set_filament_psi_priors(fil)
            m_name = micrograph_name[fil_id]
            for box_index, box in enumerate(fil.boxes):
                data_dict['_rlnMicrographName'].append(m_name)
                data_dict['_rlnCoordinateX'].append(box.x + box.w / 2)
                data_dict['_rlnCoordinateY'].append(box.y + box.h / 2)
                data_dict['_rlnAutopickFigureOfMerit'].append(box.c)
                data_dict['_rlnHelicalTubeID'].append(fid_offset+fil_id+1)
                data_dict['_rlnAnglePsiPrior'].append(fil.meta['psi'][box_index])
    else:
        for box_index, box in enumerate(coordinates):
            data_dict['_rlnMicrographName'].append(micrograph_name[box_index])
            data_dict['_rlnCoordinateX'].append(box.x + box.w / 2)
            data_dict['_rlnCoordinateY'].append(box.y + box.h / 2)
            data_dict['_rlnAutopickFigureOfMerit'].append(box.c)
...
def read_cbox_boxfile(path, min_dist_boxes_filament=0) -> Union[List[BoundBox], List[Filament]]:
    """
    ...
    args_meta_mapping = {
        '_EstWidth': 'est_box_size',
        '_EstHeight': 'est_box_size',
        '_NumBoxes': 'num_boxes',
        '_Angle': "angle"
    }
    try:
        ...
        if 'cryolo' in starfile:
            ...
            for i in range(len(starfile['cryolo']['_CoordinateX'])):
                ...
                kwargs = {}
                for arg in args_box_mapping:
                    if arg in starfile['cryolo']:
                        if starfile['cryolo'][arg][i] is None or np.isnan(starfile['cryolo'][arg][i]):
                            continue
                        kwargs[args_box_mapping[arg]] = starfile['cryolo'][arg][i]

                box = BoundBox(**kwargs)

                for arg in args_meta_mapping:
                    if arg in starfile['cryolo']:

                        if starfile['cryolo'][arg][i] is None or np.isnan(starfile['cryolo'][arg][i]):
                            continue
                        if args_meta_mapping[arg] in box.meta:
                            try:
                                box.meta[args_meta_mapping[arg]].extend(starfile['cryolo'][arg][i])
                            except Exception:
                                l = [box.meta[args_meta_mapping[arg]]]
                                l.append(starfile['cryolo'][arg][i])
                                box.meta[args_meta_mapping[arg]] = l
                        else:

                            box.meta[args_meta_mapping[arg]] = starfile['cryolo'][arg][i]
...
def is_star_filament_file(path: str) -> bool:
    if not path.endswith(".star"):
        return False

    sfile = star.StarFile(path)

    return '_rlnHelicalTubeID' in sfile['']
