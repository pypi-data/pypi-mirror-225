...
def run_code_l():
    ...
    for ndata in range(len(data_matrices)):
        ...
        for strategy, strategy_name in strategies:
            ...
            result = minimize(lambda paras: simulateLC_GUI(paras, nseeds, data, strategy=strategy, es_para = checkbox_vars, fixvalue = fixvalue_l)[0],
...
def fit_page_l():
    ...
    check_button_mll = tk.Checkbutton(fit_frame, text="mll", variable=var_mll)
    check_button_mr2 = tk.Checkbutton(fit_frame, text="null", variable=var_mr2)
    check_button_sse = tk.Checkbutton(fit_frame, text="null", variable=var_sse)
...
def open_welcome():
    ...
    version_label = tk.Label(window, text="Version 0.0.1.5", fg="gray")
    version_label.place(relx=1.0, rely=1.0, anchor="se")
    
    # Create Start button
    startFit_button = tk.Button(welcome_frame, text="Fitting for Numerosity Task", command=fit_page)
    startFit_button.place(relx=0.5, rely=0.4, anchor="center")

    Fit2_button = tk.Button(welcome_frame, text="Fitting for Localization Task", command=fit_page_l)
    Fit2_button.place(relx=0.5, rely=0.45, anchor="center")

    startSim_button = tk.Button(welcome_frame, text="Simulate for Continuous Condition", command=lambda: simu_page(1))
    startSim_button.place(relx=0.5, rely=0.5, anchor="center")

    startSimall_button = tk.Button(welcome_frame, text="Simulate for Numerosity Task", command=lambda: simu_page(2))
    startSimall_button.place(relx=0.5, rely=0.55, anchor="center")

    about_button = tk.Button(welcome_frame, text="About BCI Toolbox", command=about_bci)
    about_button.place(relx=0.5, rely=0.65, anchor="center")
