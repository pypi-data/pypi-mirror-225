...
def list(config):
    ...
    r = send_request(
        request=f"{agent_conf['context']['local']['url']}/packs/1/all",
        mode="get",
    )
...
def push(config, name):
    ...
    if os.path.exists(f"./{name}_pack/icon.png"):
        with open(f"./{name}_pack/icon.png", "rb") as file:
            pack_icon = base64.b64encode(file.read()).decode("utf-8")
    else:
        pack_icon = ""

    # check if file config.json exists
    if os.path.exists(f"./{name}_pack/config.json"):
        with open(f"./{name}_pack/config.json", "r") as file:
    ...
    r = send_request(
        request=f"{api_url}/packs/?name={name}&type={pack_type}&user_id={user_id}",
        mode="get",
    )
    ...
    if r.status_code == 200:
        ...
        logger.info(
            f"Pack [{pack_name}] already exists. Fetching Pack data"
        )
        r = send_request(
            request=f"{api_url}/packs/?name={name}&type={pack_type}&user_id={user_id}",
            mode="get",
        ...
        if r.status_code != 200:
            logger.error(f"Pack [{pack_name}] could not be fetched. Error code")
            sys.exit(1)

    else:
        logger.error(f"Pack [{pack_name}] could not be published. Error code")
...
- `config.json` : the configuration file for the runtime program
