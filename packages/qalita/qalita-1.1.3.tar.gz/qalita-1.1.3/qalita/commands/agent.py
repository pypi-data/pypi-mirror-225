...
def authenticate(config):
    ...
    r = send_api_request(request=f"/registries/1", mode="get")
...
def run(config, source_id, source_version, pack_id, pack_version):
    """Runs de agent"""
    # Pre-checks
    if config.mode == "job":
...
@pass_config
def job_run(config, source_id, source_version, pack_id, pack_version, job_id=None):
    logger.info("------------- Job Run -------------")
    ...
    if pack_version is None:
        ...
        logger.info(
            f"pack version not specified, Latest pack version is {pack_version}"
        )
    ...
    if r.status_code == 201:
        ...
        r = r.json()
        job_id = r["id"]
        logger.info(f"Job updated with id {job_id}")
    else:
        # The request failed
    ...
    if r.status_code != 200:
        logger.info(f"Failed updating job {job_id}")
        sys.exit(1)


@pass_config
def post_run(config, run_path, name, pack_id, pack_version, source_id, source_version):
    logger.info("------------- Job Post Run -------------")

    agent_conf = config.load_agent_config()

    logger.info(f"Uploading logs to Qalita Platform...")
    # check if the logs.txt file exits
