...
class NeuralNetworkNeurons(Neurons):
    ...
    def get_state(self, evaluate_at="last", save_torch=False, **kwargs):
        ...
        #TODO get it so that it only saves if called from .update() (otherwise plotting will reset firingrate_torch to a new state which could break training) 
...
class MultiLayerPerceptron(nn.Module):
    """A generic neural network class used for the core function in DeepNeuralNetworkNeurons. 
    This class can be used to create a deep neural network with any number of inputs, output and hidden layers (of any size). Each hidden layer is seperated by a ReLU and the output layer is linear. Biases are used by default.
    ...
    def __init__(self, n_in=20, n_out=1, n_hidden=[20,20]):
        ...
        # create an ordered dict of layers
