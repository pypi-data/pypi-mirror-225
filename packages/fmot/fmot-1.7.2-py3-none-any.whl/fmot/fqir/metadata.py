...
class IOMetaData:
    ...
    def run_graph(self, graph: GraphProto, inputs: List[np.ndarray], **kwargs):
        if len(inputs) != len(self.input_spec):
            raise ValueError(f"Not enough inputs. Expected {len(self.input_spec)}, recieved {len(inputs)}.")
        ...
        if len(outputs) != len(self.output_spec):
            raise ValueError(f"Not enough outputs. Expected {len(self.output_spec)}, recieved {len(outputs)}.")
