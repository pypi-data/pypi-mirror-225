...
class PatchIR:
    ...
    def call(self, module, *args):
        ...
        try:
            ...
            for name, x in zip(self.inputs, args):
                objs[name] = x
                if DEBUG:
                    print(f'Collecting input: {name}')
...
class AtenPatchNode(PatchIRNode):
    ...
    def from_torchscript(cls, node, patch_idx):
        ...
        inputs, outputs, sourceref = utils.get_inputs_outputs_sourceref(node,
            tensorial_inputs=True)
        inputs = ['self'] + inputs
        fname = node.kind()
        return cls(inputs, outputs, sourceref, patch_idx=patch_idx, fname=fname)

    def __repr__(self):
        rep = f'{", ".join(self.outputs)} = {self.inputs[0]}.patchlist[{self.patch_idx}]'
...
class PatchedModule(nn.Module):
    ...
    def __init__(self, orig_name, patchlist, quantdict, patchIR):
        ...
        self.patchlist = patchlist #: ``torch.nn.ModuleList`` containing the patch modules
        self.quantdict = quantdict #: ``torch.nn.ModuleDict`` containing the parameter quantizers
    ...
    def from_torch(cls, module, extra_patchings=None):
        ...
        patchlist = patchlist_from_torch(module, step=False, extra_patchings=extra_patchings)
...
class PatchedSequencer(Sequencer):
    ...
    wrapped by a parameter quantizer.
    """
    def __init__(self, orig_name, patchlist, quantdict, patchIR, state_shapes, seq_dim, return_hidden_state):
        super().__init__(state_shapes=state_shapes, seq_dim=seq_dim, return_hidden_state=return_hidden_state)
        self.orig_name = orig_name #: original name of the model
        self.patchlist = patchlist #: ``torch.nn.ModuleList`` containing the patch modules
        self.quantdict = quantdict #: ``torch.nn.ModuleDict`` containing the parameter quantizers
        self.patchIR = patchIR #: PatchIR graph, making the PatchedModule executable
    ...
    def from_torch(cls, module, extra_patchings=None):
        ...
        orig_name = type(module).__name__
        patchlist = patchlist_from_torch(module, step=True, extra_patchings=extra_patchings)
        quantdict = quantdict_from_torch(module)
        if len(patchlist) == 0:
            patchlist = None
        if len(quantdict) == 0:
