...
def add_to_reqtxt():
    ...
    try:
        ...
        result = subprocess.run(["pipdeptree", "--freeze", "--warn", "silence"], capture_output=True, text=True, check=True, stdout=subprocess.PIPE)
...
def install_requirements():
    ...
    try:
        subprocess.run([python_command, "-m", "pip", "install", "swiftly-sys", "--upgrade"], check=True, stdout=subprocess.PIPE)
    except subprocess.CalledProcessError:
        loader.end("Failed to upgrade swiftly-sys!", failed=True)
        return
