...
def check_layer_name(model, layer_name, base_model):
        if base_model == None:
            ...
            if layer_name in layer_names:
                if not isinstance(type(model.get_layer(layer_name)),
                                  keras.layers.convolutional.conv2d.Conv2D.__class__):
            ...
            if layer_name in layer_names:
                if not isinstance(type(model.get_layer(base_model).get_layer(layer_name)),
                                  keras.layers.convolutional.conv2d.Conv2D.__class__):
                    raise ValueError('The layer name you entered is not of type tensorflow.keras.layers.Conv2D. Please enter a Conv2D layer name.')
            else:
