...
def get_code_valuation_indices_baostock(
    code: str, start_date: str = "19800101"
) -> pd.DataFrame:
...
def get_valuation_indices_baostock(
    code: Iterable[str] | str, start_date: str = "19800101"
) -> pd.DataFrame:
    ...
    for c in code:
        df = get_code_valuation_indices_baostock(c, start_date)
        df["code"] = c
        dfs.append(df)
...
def get_code_outstading_period(code: str, start_date: str = "19800101") -> pd.DataFrame:
    df = ak.stock_share_change_cninfo(
        symbol=code, start_date=start_date, end_date="20301231"
    )
    return df[["变动日期", "已流通股份"]].drop_duplicates(subset=["已流通股份"], keep="first")

def get_outstanding_daily(code: Iterable[str] | str, max_date: str, start_date: str = "19800101") -> pd.DataFrame:
    if isinstance(code, str):
        code = [code]
    ...
    for c in code:
        df = get_code_outstading_period(c, start_date)
