        ...
        with site(self.site_name) as mysite:
            ...
            with user(user_name=self.user_name, site=mysite) as this_user:
                ...
                if len(trade_rec_list) > 0:
                   ...
                   msg = "[%s] %d transactions created in user account update job run" % (hostname, trade_rec_list.shape[0])
        ...
        with site(self.site_name) as mysite:            
            with user(user_name=self.user_name, site=mysite) as this_user:
                ...
                if len(trade_rec_list) > 0:
                    ...
                    import socket
                    hostname = socket.gethostname()                       
                    msg = "[%s] %d transactions created in user account trade job run" % (hostname, trade_rec_list.shape[0])
                    lineNotifyMessage( msg, this_user.notification_token)                      
        return
...
if __name__ == '__main__':
    ...
    #t =  update_user_account_position('mysite', 'stester')
    #t.execute()
    ...
    t =  trade_user_account('mysite', 'jihuang')
    t.execute()
