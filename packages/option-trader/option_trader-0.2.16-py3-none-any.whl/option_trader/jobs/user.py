...
class update_user_account_position():
        ...
        with site(self.site_name) as mysite:
            ...
            with user(user_name=self.user_name, site=mysite) as this_user:
                ...
                if len(trade_rec_list) > 0:
                   this_user.send_trade_report(trade_rec_list) 
                   import socket
                   hostname = socket.gethostname()               
                   msg = "[%s] %d transactions created in user account update job run" % (hostname, len(trade_rec_list))
                   lineNotifyMessage( msg, this_user.notification_token)                    
        return
    
class trade_user_account():
    ...
    def execute(self):
        ...
        with site(self.site_name) as mysite:            
            with user(user_name=self.user_name, site=mysite) as this_user:
                ...
                for account_name in account_list:
                    this_account = this_user.get_account(account_name)
                    trade_rec_list += this_account.try_open_new_strategy_positions()       
                if len(trade_rec_list) > 0:
                    this_user.send_trade_report(trade_rec_list)
                    import socket
                    hostname = socket.gethostname()                       
                    msg = "[%s] %d transactions created in user account trade job run" % (hostname, len(trade_rec_list))
                    lineNotifyMessage( msg, this_user.notification_token)                      
        return
    

class monitor_user_account():
        ...
        with site(self.site_name) as mysite:            
            with user(user_name=self.user_name, site=mysite) as this_user:
                ...
                if len(trade_rec_list) > 0:
                    ...
                    import socket
                    hostname = socket.gethostname()                       
                    msg = "[%s] %d transactions created in user account scheduled job run" % (hostname, len(trade_rec_list))
                    lineNotifyMessage( msg, this_user.notification_token)                      
        return
