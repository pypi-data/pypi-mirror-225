...
class optionLegDesc(object):
    def __init__(self, exp_date, strike, otype, open_action, quantity):
        self.STRIKE      = strike
        self.OTYPE       = otype
...
def save_to_db(account, total_cash, total_margin, summary_df, position_df):
    ...
    trade_date = str(datetime.now().astimezone(timezone(app_settings.TIMEZONE)).date())  
...
def fidelity_reader(account, summary_file, position_file):
    ...
    for pid in pair_list:            
        ...
        if fidelity_strategy == 'Unpaired':                     
            ...
            leg =  optionLegDesc("", np.nan, asset.STOCK, asset.BUY_TO_OPEN, quantity)
            ...
            leg  =  optionLegDesc(exp_date, strike, asset.PUT, asset.SELL_TO_OPEN, quantity)             
            sdf.at[sindex, option_summary.LEGS] = json.dumps([json.dumps(vars(leg))])  
            ...
            leg  =  optionLegDesc(exp_date, strike, asset.CALL, asset.BUY_TO_OPEN, quantity)             
            sdf.at[sindex, option_summary.LEGS] = json.dumps([json.dumps(vars(leg))])  
            ...
            stock_leg =  optionLegDesc("", np.nan, asset.STOCK, asset.BUY_TO_OPEN, share_quantity)            
            call_leg  =  optionLegDesc(exp_date, strike, asset.CALL, asset.SELL_TO_OPEN, call_quantity)            
            ...
            for i, rec in srow.iterrows():
                ...
                leg =  optionLegDesc(exp_date, strike, otype, open_action, abs(leg_quantity))                
