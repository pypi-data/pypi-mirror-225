...
def check_version_matched(os_list, pkg_list):
    ...
    for pkg_item in pkg_list:
        for item in ("DL", "MindSpore", "MindStudio", "Torch-npu", "CANN", "MEF", "TensorFlow"):
    ...
    for version in versions:
        ...
        for os_item in os_list:
            if os_item not in mef_support_list and item_counter.get('MEF', 0) > 0:
                return State.EXIT, "ascend-deployer only support download and install MEF on {}".format(mef_support_list)
    if warning_messages:
...
def get_name_version(pkg_item, std_out=True):
