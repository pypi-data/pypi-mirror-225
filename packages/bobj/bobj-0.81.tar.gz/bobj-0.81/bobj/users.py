...
class UserManagement:
    ...
    def modify_user_details(self, name, **kwargs):
        user_id = self.get_user_id_by_username(name)
        if user_id is None:
    ...
    def reset_user_password(self, name, new_password):
        user_id = self.get_user_id_by_username(name)
        if user_id is None:
            raise Exception("User not found")
...
class UserSupport:
    ...
    def _construct_reset_password_data(new_password):
        attrs = [
            '<attr name="password" type="string">{}</attr>'.format(new_password),
