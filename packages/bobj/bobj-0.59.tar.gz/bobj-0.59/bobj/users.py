...
class _usermanagement:
    ...
    def _create_user(self, username, **kwargs):
        ...
        response = requests.post(url, headers=self.header, data=user_data)

        return sup._handle_response(self, response)

    def _get_user_details_by_id(self, user_id):
        return self._make_request('get', f'users/{user_id}')

    def _get_user_details_by_username(self, username, pagesize=50, page=1):
