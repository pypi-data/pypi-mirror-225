...
class UserManagement:
    ...
    def reset_user_password(self, name, new_password):#6
        ...
        if user_id is None:
            raise Exception("User not found")
        user_data_xml = UserSupport._construct_reset_password_data(name, new_password) # Calling UserSupport method
        url = self.base_url + USER_DETAILS_ENDPOINT.format(user_id=user_id)
...
class UserSupport:
    ...
    @staticmethod
    def _construct_user_data(name = None, password = None,new_password = None, **kwargs):
