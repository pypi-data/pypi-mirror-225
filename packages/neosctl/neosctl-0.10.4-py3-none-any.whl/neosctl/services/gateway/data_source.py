...
def _data_source_url(ctx: typer.Context) -> str:
    return "{}/v2/data_source".format(ctx.obj.get_gateway_api_url().rstrip("/"))
...
def create_entity(
    ...
    entity.create_entity(
        ctx=ctx,
        url_prefix=_data_source_url(ctx=ctx),
        label=label,
        name=name,
...
def list_entities(ctx: typer.Context) -> None:
    """List data sources."""
    entity.list_entities(ctx=ctx, url_prefix=_data_source_url(ctx=ctx))
