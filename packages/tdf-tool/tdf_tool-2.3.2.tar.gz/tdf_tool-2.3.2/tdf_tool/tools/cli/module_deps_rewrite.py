        ...
        if os.path.isfile(packageConfigFilePath):
            with open(packageConfigFilePath, "r", encoding="utf-8") as readF:
                ...
                for item in packages:
                    if item['rootUri'] == '../../' + item['name']:
                        jsonPackages.append(item['name'])
        else:
            return False
        
        print("lockSpec  :" + pathPackages)
        print("jsonConfig:" + jsonPackages)

        # override的库一致，则不需要重新执行upgrade
        return sorted(depsKeys) == sorted(pathPackages) and sorted(depsKeys) == sorted(jsonPackages)
