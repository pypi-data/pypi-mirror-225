...
class ApplyConversionPatternsOp(_ods_ir.OpView):
  ...
  @builtins.property
  def partialConversion(self):
    return "partialConversion" in self.operation.attributes

  @partialConversion.setter
  def partialConversion(self, value):
    if bool(value):
      self.operation.attributes["partialConversion"] = _ods_ir.UnitAttr.get()
    elif "partialConversion" in self.operation.attributes:
      del self.operation.attributes["partialConversion"]

  @partialConversion.deleter
  def partialConversion(self):
    del self.operation.attributes["partialConversion"]
...
class NamedSequenceOp(_ods_ir.OpView):
  ...
  def __init__(self, sym_name, function_type, *, sym_visibility=None, arg_attrs=None, res_attrs=None, loc=None, ip=None):
    ...
    not _ods_ir.AttrBuilder.contains('anonymous_455')) else
      _ods_ir.AttrBuilder.get('anonymous_455')(function_type, context=_ods_context))
