...
class BufferizeToAllocationOp(_ods_ir.OpView):
  ...
  def __init__(self, allocated_buffer, new_ops, target, *, memory_space=None, memcpy_op=None, alloc_op=None, bufferize_destination_only=None, loc=None, ip=None):
    operands = []
    results = []
    attributes = {}
    regions = None
...
class PadOp(_ods_ir.OpView):
  ...
  def __init__(self, padded, pad, target, *, padding_values=None, padding_dimensions=None, pad_to_multiple_of=None, pack_paddings=None, transpose_paddings=None, copy_back=None, loc=None, ip=None):
    operands = []
    results = []
    attributes = {}
    regions = None
    ...
    if transpose_paddings is not None: attributes["transpose_paddings"] = (transpose_paddings if (
        ...
        not _ods_ir.AttrBuilder.contains('anonymous_454')) else
          _ods_ir.AttrBuilder.get('anonymous_454')(transpose_paddings, context=_ods_context))
    if copy_back is not None: attributes["copy_back"] = (copy_back if (
        issubclass(type(copy_back), _ods_ir.Attribute) or
        not _ods_ir.AttrBuilder.contains('BoolAttr')) else
          _ods_ir.AttrBuilder.get('BoolAttr')(copy_back, context=_ods_context))
  ...
  @builtins.property
  def copy_back(self):
    return self.operation.attributes["copy_back"]

  @copy_back.setter
  def copy_back(self, value):
    if value is None:
      raise ValueError("'None' not allowed as value for mandatory attributes")
    self.operation.attributes["copy_back"] = value
