...
def init_cortex(
    ...
    cpu: int = typer.Option(
        2,
        help="Number of CPUs to use for the Cortex deployment. Defaults to 2. Can be an integer between 1 and 32",
    ),
    ...
    if not os.path.exists(init_dir):
        ...
        if os.listdir(init_dir) and not overwrite:
            print("Directory is not empty. Please specify an empty directory.")
            sys.exit(1)
...
def deploy(
    ...
    if memory:
        ...
        assert memory <= 128.0, "Memory must be at most 128.0 GB."
...
def train(
    ...
    memory: Union[float, None] = typer.Option(
        None,
        help="Amount of memory (in GB) to use for training. Overrides the config file. Defaults to 16.",
    ),
