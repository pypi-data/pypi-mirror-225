        ...
        for i, r in enumerate(resolvers):
            if i == 0:
                ...
                if r.is_count and COUNT_POSTFIX in edge:
                    ...
                    resolver_s = f"{edge} := count((SELECT .{edge.split(COUNT_POSTFIX)[0]} {filters_str}){count_property_str})"
                else:
                ...
                if r.is_count and COUNT_POSTFIX in edge:
                    ...
                    resolver_s = f"{key_name} := count((SELECT .{edge.split(COUNT_POSTFIX)[0]} {filters_str}){count_property_str})"
                else:
                    filters_str, variables = r.full_query_str_and_vars(
                        include_select=False, prefix=new_prefix
                    )
