...
from pytailor.utils import dict_keys_str_to_int, dict_keys_int_to_str, get_or_create_node_id
...
class Workflow(APIBase):
    """
    Create a new workflow or fetch an existing workflow.

    **Instantiation patterns:**

    - To create a new workflow from a DAG use the default constructor
    - To create a new workflow from a workflow definition use *Workflow.from_definition_id()*
    - To retrieve a workflow from the backend use *Workflow.from_project_and_id()*

    **Parameters(default constructor):**

    - **project** (Project): The project to which the workflow belongs.
    - **dag** (DAG): The DAG for the workflow.
    - **name** (str, Optional): The name of the workflow.
    - **inputs** (dict, optional): Input data which can be queried from tasks during workflow execution.
    - **fileset** (FileSet, optional): Input files to the workflow.
    - **local_files** (LocalFiles, optional): Optionally use local files when running the workflow.
    """
    ...
    def __init__(
        ...
        self.__local_files = local_files
        self.__outputs = {}
        self.__id = None
        self.__model = None
    ...
    def reset_task(self, task_id: str):
        ...
        **Parameters:**
        ...
        with RestClient() as client:
            client.perform_task_operation(self.id, self.project.id, task_id, task_operation)

    def refresh(self) -> None:
        """Update an existing workflow with latest data from backend."""
        self.__assert_has_state()
    ...
    def __fetch_model(cls, project_id: str, wf_id: str) -> WorkflowModel:
        with RestClient() as client:
            return client.get_workflow(project_id, wf_id)

    @classmethod
    def from_project_and_id(cls, project: Project, wf_id: int) -> Workflow:
        """
        Retrieve an existing workflow from the backend.

        **Parameters:**

        - **project** (Project): The project to which the workflow belongs.
        - **wf_id** (int): The id of the workflow to retrieve.
        """
    ...
    def from_definition_id(
        ...
        **Parameters:**
        ...
        - **project** (Project): The project to which the workflow belongs.
        - **wf_def_id** (str): The id of the workflow definition to use.
        - **name** (str, Optional): The name of the workflow.
        - **inputs** (dict, optional): Input data which can be queried from tasks during workflow execution.
        - **fileset** (FileSet, optional): Input files to the workflow.
        """
    ...
    def run(self, distributed: bool = False, worker_name: Optional[str] = None) -> None:
        ...
        *distributed=False*
        ...
        # TODO: allow a "run_on_single_node" option to run the workflow on a single node?
        #   e.g. to avoid local files dependency issues
        ...
        if self.__local_files:
            local_files = self.__local_files.files
        else:
            local_files = None
        ...
        with RestClient() as client:
            wf_model = client.new_workflow(self.__project.id, create_data)
            self.__update_from_backend(wf_model)
        ...
        if not distributed:
            ...
            runner = SerialRunner(self.project.id, worker_name, wf_model.id, capabilities=requirements)
            runner.run()
    ...
    def __assert_has_state(self):
        if not self.__state:
            raise BackendResourceError("This workflow has not been run yet.")
    ...
    def get_task_info(self, task_id: str):
        """
        Get information about a task in the workflow.
        ...
        **Parameters:**
        ...
        self.__assert_has_state()
        with RestClient() as client:
            task_model = client.get_task(self.project.id, self.id, task_id)
        return task_model
