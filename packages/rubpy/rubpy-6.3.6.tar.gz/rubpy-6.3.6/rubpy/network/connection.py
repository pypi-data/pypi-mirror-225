...
class Connection:
    ...
    def __init__(self, client):
        ...
        self._connection.timeout = self._client._timeout
    ...
    def execute(self, request: dict):
        ...
        for _ in range(self._client._request_retries):
            for url in method_urls:
                try:
                    response = self._connection.post(url, json=request)
                    if response.status_code != 200:
                        continue

                    result = response.json()
                    if result.get('data_enc'):
                        result = Crypto.decrypt(result['data_enc'], key=self._client._key)
    ...
    def send_data_to_ws(self, ws, data='handSnake'):
        if data == 'handSnake':
            ws.send(json.dumps({
                'method': 'handShake',
                'data': '',
    ...
    def to_keep_alive(self):
        while True:
            time.sleep(5)
            try:
                self._client(methods.chats.GetChatsUpdates(
                    state=round(time()) - 200))
            except:
                continue

    def keep_socket(self, ws):
        while True:
            time.sleep(5)
            try:
                self.send_data_to_ws(ws, data='keep')
            except:
                continue
    ...
    def download(self, dc_id: int, file_id: int, access_hash: str, size: int, chunk=131072, callback=None):
        ...
        while True:
            ...
            response = self._connection.post(url, headers=headers)
            if response.status_code == 200:
