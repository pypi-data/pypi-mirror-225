...
import newgram
from newgram import __version__, __license__
from newgram import enums
from newgram import raw
from newgram import utils
from newgram.crypto import aes
from newgram.errors import CDNFileHashMismatch
from newgram.errors import (
    SessionPasswordNeeded,
    VolumeLocNotFound, ChannelPrivate,
    BadRequest
)
from newgram.handlers.handler import Handler
from newgram.methods import Methods
from newgram.session import Auth, Session
from newgram.storage import FileStorage, MemoryStorage
from newgram.types import User, TermsOfService
from newgram.utils import ainput
...
class Client(Methods):
    """Newgram Client, the main means for interacting with Telegram.
    ...
    Parameters:
        ...
        in_memory (``bool``, *optional*):
            ...
            :meth:`~newgram.Client.export_session_string` before stopping the client to get a session string you can
        ...
        workdir (``str``, *optional*):
            ...
            The working directory is the location in the filesystem where Newgram will store the session files.
    ...
    APP_VERSION = f"Newgram {__version__}"
    DEVICE_MODEL = f"Newgram"
    ...
    async def authorize(self) -> User:
        ...
        print(f"Welcome to Newgram (version {__version__})")
        print(f"Newgram is free software and comes with ABSOLUTELY NO WARRANTY. Licensed\n"
    ...
    def set_parse_mode(self, parse_mode: Optional["enums.ParseMode"]):
        ...
        Parameters:
            parse_mode (:obj:`~newgram.enums.ParseMode`):
        ...
        if session_empty:
            if not self.api_id or not self.api_hash:
                raise AttributeError("The API key is required for new authorizations. "
                                     "More info: https://docs.newgram.org/start/auth")
    ...
    async def get_file(
        ...
        async with self.get_file_semaphore:
            ...
            except newgram.StopTransmission:
                raise
            except Exception as e:
                log.exception(e)
            finally:
