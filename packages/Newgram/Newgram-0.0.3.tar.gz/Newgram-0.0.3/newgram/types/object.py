...
class Object:
    def __init__(self, client: "newgram.Client" = None):
        self._client = client

    def bind(self, client: "newgram.Client"):
        """Bind a Client instance to this and to all nested Newgram objects.

        Parameters:
            client (:obj:`~newgram.types.Client`):
                The Client instance to bind this object with. Useful to re-enable bound methods after serializing and
                deserializing Newgram objects with ``repr`` and ``eval``.
        """
        self._client = client
