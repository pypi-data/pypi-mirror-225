...
class OnDisconnect:
    def on_disconnect(self=None) -> Callable:
        ...
        :obj:`~newgram.handlers.DisconnectHandler`.
        """

        def decorator(func: Callable) -> Callable:
            if isinstance(self, newgram.Client):
                self.add_handler(newgram.handlers.DisconnectHandler(func))
            else:
                if not hasattr(func, "handlers"):
                    func.handlers = []

                func.handlers.append((newgram.handlers.DisconnectHandler(func), 0))
