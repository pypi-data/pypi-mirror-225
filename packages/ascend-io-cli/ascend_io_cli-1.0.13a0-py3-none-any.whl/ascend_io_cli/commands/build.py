...
def _build_transform(data_service_type: str, input_component: pb2.Transform, input_schema: pb2.ComponentSchemaResponse.Schema, join_columns: List[str]):
  ...
  """, ),
              additional_source=io.Code.Source(inline=f"CREATE TABLE IF NOT EXISTS {{{{target}}}} ({NL.join(type_factory(input_schema.schema.split(',')))})"),
          ), ),
...
def merge(
  ...
  data_service_config = client.get_data_service_data_plane_config(data_service_id).data
