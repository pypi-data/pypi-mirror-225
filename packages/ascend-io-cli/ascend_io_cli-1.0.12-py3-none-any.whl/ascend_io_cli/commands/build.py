...
from ascend_io_cli.support import get_client, print_response, to_snowflake_types
...
def _build_transform(input_component: pb2.Transform, input_schema: pb2.ComponentSchemaResponse.Schema, join_columns: List[str]):
  ...
  merge_transform = definitions.Transform(
      ...
      operator=operator.Operator(spark_function=operator.Spark.Function(
          executable=io.Executable(code=io.Code(
              language=function.Code.Language(snowflake_sql=function.Code.Language.SnowflakeSql(), ),
  ...
  """, ),
              additional_source=io.Code.Source(
                  inline=f"CREATE TABLE IF NOT EXISTS {{{{target}}}} ({NL.join(to_snowflake_types(input_schema.schema.split(',')))})"),
          ), ),
...
@app.command(help='Build a basic merge component from an existing transformation')
def merge(
    ctx: typer.Context,
  ...
  merge_transform, id_map = _build_transform(upstream_component[0], upstream_schema, join_columns)
