...
class RunBuildCapture(object):
    ...
    def __init__(self, my_args, log_path, blackduck_output_dir):
        ...
        self.cov_dir = util.get_absolute_path(self.my_args.args.cov_output_dir) if self.my_args.args.cov_output_dir else os.path.join(self.home,
                                                                                                              ".synopsys",
                                                                                                              "blackduck-c-cpp",
                                                                                                              "output",
        ...
        self.api_token = shlex.quote(self.my_args.args.api_token) if self.my_args.args.api_token else os.environ.get('BLACKDUCK_API_TOKEN') or os.environ.get('BD_HUB_TOKEN')
        ...
        self.scan_cli_dir = util.get_absolute_path(self.my_args.args.scan_cli_dir) if self.my_args.args.scan_cli_dir is not None else self.blackduck_output_dir
    ...
    def __enter__(self):
        if not self.offline_mode:
            self.hub_api.create_or_verify_project_version_exists(self.hub_project_name, self.hub_project_vers, self.project_group_name)
    ...
    def find_bazel_workspace_file(self):
        ...
        if len(bazel_workspace_files) != 1:
            if not bazel_workspace_files:
                logging.warning("The Bazel WORKSPACE file was not found so the required modification can't be verified.")
            else:
                logging.warning("Multiple possible WORKSPACE files were found.")
            logging.warning("If the Bazel WORKSPACE file has already been modified manually to work with Coverity, ignore this warning. Otherwise, see the Bazel Setup > Modify project files > Workspace file section in the PyPi documentation for instructions.")
            return None
        ...
        if bazel_workspace_file:
            with open(bazel_workspace_file, 'r') as f:
                ...
                if 'rules_coverity' in contents:
                    logging.debug("The bazel WORKSPACE file appears to have been already updated to work with Coverity and will not be modified")
                    return
            with open(bazel_workspace_file, 'a') as f:
                ...
                update_string = 'load("@bazel_tools//tools/build_defs/repo:http.bzl", "http_archive")' + os.linesep \
                                ...
                                + '    urls=["file:///{}"],'.format(os.path.join(self.cov_home, 'bazel', 'rules_coverity.tar.gz')) + os.linesep \
    ...
    def run_build(self, cov_emit_link_path, cov_build_path):
        ...
        EmitWrapper(self.cov_home, self.cov_dir, self.blackduck_output_dir, self.platform_name).archive_emit_dir()
