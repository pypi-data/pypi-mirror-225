...
class SigScanner:
    ...
    def __init__(self, cov_base_path, hub_api, files_notdetect_from_pkgmgr, project_name, project_version_name,
                 ...
                 disable_bdio_json_splitter, skip_includes, port):
    ...
    def find_current_sig_scanner(self):
        ...
        """
        self.scan_cli_directory = 'scan.cli-{}'.format(self.hub_api.get_hub_version())
        self.scan_cli_path = os.path.join(self.scan_cli_home, self.scan_cli_directory)
    ...
    @staticmethod
    def bdio_splitter(scan_path, sig_scan_output_directory, project_version_name, max_chunk_nodes=3000, max_file_entries=100000):
        ...
        with tempfile.TemporaryDirectory() as tmpdirname:
            ...
            while len(current_chunk) > 0:
                ...
                if entry_type == type_project:
                    pass
                if project_version_name:
                    project_entry_copy[type_name][0]['@value'] = project_version_name
    ...
    def build_sig_scanner_command(self):
        ...
        ## for versions of hub that are using signature scanner 8.x and when run in offline mode
        if self.version_grt and self.offline_mode:
    ...
    def upload_sig_bdio_results(self):
        ...
        if self.disable_bdio_json_splitter or (sig_path < self.json_splitter_limit):
            ...
            bdio_lst = self.bdio_splitter(self.bdio_file, self.sig_scan_output_directory, self.project_version_name, max_chunk_nodes=self.bdio_split_max_chunk_nodes, max_file_entries=self.bdio_split_max_file_entries)
            for bd in bdio_lst:
                self.bdio_file = bd
