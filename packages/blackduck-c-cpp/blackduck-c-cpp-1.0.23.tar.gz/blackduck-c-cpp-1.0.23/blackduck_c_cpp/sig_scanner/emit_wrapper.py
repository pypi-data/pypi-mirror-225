...
class EmitWrapper:
    ...
    def __init__(self, cov_base_path, cov_output_path, blackduck_output_dir, platform_name):
        ...
        self.emit_dir = os.path.join(self.blackduck_output_dir, 'emit')
    ...
    def archive_emit_dir(self):
        if os.path.exists(self.emit_dir):
            logging.info("Archiving emit directory")
            new_emit_dir = os.path.join(os.path.dirname(self.emit_dir), '.emit_{}'.format(datetime.now().strftime("%Y_%m_%d_%H_%M_%S")))
            move(self.emit_dir, new_emit_dir)
