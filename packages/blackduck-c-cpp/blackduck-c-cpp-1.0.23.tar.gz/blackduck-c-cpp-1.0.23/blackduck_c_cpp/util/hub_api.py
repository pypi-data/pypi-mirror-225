...
class HubAPI:
    ...
    def create_or_verify_project_version_exists(self, project_name, version_name, project_group_name, phase='DEVELOPMENT'):
        try:
            ...
            if project:
                ...
                if project_group_url:
                    response = self.create_project(project_name, version_name,parameters={'version_phase': phase, 'projectGroup': project_group_url})
                else:
                    response = self.create_project(project_name, version_name,parameters={'version_phase': phase})
        except requests.exceptions.SSLError:
            util.error_and_exit("SSLError occurred. Make sure you have the correct url and insecure flag is set correctly")
    ...
    def get_project_group_url_by_name(self, project_group_name):
        projects_group_list = self.hub.get_json("/api/project-groups")
