...
def create_models(arguments):
    ...
    # if tmds file matches input was not given, report file types found
    if arguments.tdms_file_matches is None:

        # look through metadata for tdms file types
        tdms_types = []
        for files in metadata:
            tdms_types += files['tdms_types']
        tdms_types = np.unique(tdms_types)
        tdms_types = [tdms_type.replace(' ', '_') for tdms_type in tdms_types]
        arguments.model_description = str(tdms_types)
    
    else:
        arguments.model_description = str(arguments.tdms_file_matches)

    # alter arguments for dac tdms
...
def parser ():
    ...
    parser.add_argument("--project-name", default="TDMS DAC Models", 
        help="New project name.  Default: Part No.")
