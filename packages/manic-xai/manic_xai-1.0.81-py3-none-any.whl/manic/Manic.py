...
class Manic:
    def __init__(self, data_instance, base_counterfactuals, categorical_features, immutable_features, feature_ranges, data, predict_fn, predict_proba_fn, class_labels, population_size=100, num_generations=50, alpha=0.25, beta=0.25, crossover_method="uniform", mutation_method="random_resetting", perturbation_fraction=0.1, num_parents=2, seed=42, verbose=1, early_stopping=None, max_time=None, disagreement_method="euclidean_distance", theta=0.25, gamma = 0.25, parallel=False, labels=[], weights = [1/3, 1/3, 1/3], wachter=False, feature_entropy=0.25, sparse=False):
        ...
        self.mutate = Mutation(mutation_method, perturbation_fraction, self.feature_ranges, self.categories, self.categorical_features).mutate
    ...
    def initialise_population(self):
        ...
        population = []
        random_method = self.generate_random_instance
        if(self.sparse):
            random_method = self.generate_sparse_random_instance

        options = [random_method, self.nearest_unlike_neighbors, self.randomly_sample_counterfactual]
