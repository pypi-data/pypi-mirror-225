...
parallelmagic
=============
Copied from ipyparallel, with the following changes:
    - add show for only showing the output of a slice of view
...
class ParallelMagics(Magics):
    ...
    def __init__(self, shell, view, suffix=""):
        ...
        self.display_targets = self.view.targets
    ...
    def parallel_execute(
        ...
        # result._targets = self.display_targets
        # result._children = [result._children[i] for i in result ._targets]

        childeren = []
        for i, target in enumerate(result._targets):
            if target in self.display_targets:
                childeren.append(result._children[i])
        result._targets = self.display_targets
        if save_name:
