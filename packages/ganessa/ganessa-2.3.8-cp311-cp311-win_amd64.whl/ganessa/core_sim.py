...
from numpy import array as nparray, ndarray, float64, zeros as npzeros
...
from ganessa.core import (M, DICT, SIMERR, LINK, NODE, TANK, ElemType, GanessaError, VectF,
        ...
        getindex, getall, _fn_undefined, _ret_errstat, _ret_errstat2)
...
#   * string attr: pump attribute (speed or number of units) or "N" or "NV" or "H" for reservoirs or ""
...
#****f* sFunctions/hstepcount, tslen, mslen, tsval, tsvals, msval, tsvalbymts, tsinterv, wqtslen, refdate
# PURPOSE
...
def tsvals(typelt: ElemType, id_seq: Sequence[str], sattr: str,
        fixed_interval: float = 0.0, valid_only: bool = False) -> Tuple[ndarray, int, List[Tuple[ndarray, Union[int, str]]]]:
    ...
    tvect, _vvec, nb = tsval(typelt, id_seq[first_existing_idx[0]], sattr, fixed_interval)
    # Piccolo / Ganessa_SIM index starts at 1
    idx_item -= 1
    # Prepare result matrix
    all_ts = npzeros((len(id_seq), nb))
    # Iterate over time
    for kt, instant in enumerate(tvect):
        browseH(instant)
        all_flows = getall(typelt, sattr)
        all_ts[:, kt] = all_flows[idx_item]
    idx_item += 1
    # get TS by element (the other dimension)
    # Return TS, length (None, 0 for nonexistent items) or (ID, TS) in valid_only mode
    if valid_only:
        ret_vals = [(sid, all_ts[k, :]) for k, (idx, sid) in enumerate(zip(idx_item, id_seq)) if idx]
    else:
        ret_vals = [((all_ts[k, :], nb) if idx else (None, 0)) for k, idx in enumerate(idx_item)]
    return tvect, nb, ret_vals
...
def getcalind(typelt : ElemType, id_elt : str, rank=1) -> Tuple[float, int]:
    ...
    if _dll_api.dll_version() < 20220908:
        if rank != 1:
            return (0.0, -1)
