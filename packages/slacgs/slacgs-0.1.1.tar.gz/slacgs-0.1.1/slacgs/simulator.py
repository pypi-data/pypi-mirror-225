...
def loss_theoretical(h_clf, cov):
  ...
  Parameters:
    ...
    cov(list[list] or np.ndarray): covariance matrix :math:`\Sigma` of the gaussian samples used to train the SVM model
...
def distance_from_origin_to_intersection_between_normalized_ellipsoid_and_main_diagonal(cov):
  ...
  Parameters:
    cov (list): covariance matrix :math:`\Sigma` of the ellipsoid

  Returns:
    float: distance from origin :math:`[0,0,...,0]` to the intersection point between the normalized ellipsoid and the main diagonal :math:`[1,1,...,1]`.
