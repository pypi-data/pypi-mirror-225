...
def _update_view(registry_object:RegistryObject,**kwargs) -> None:
    ...
    _template_name = kwargs.get("template_name",'views/update/update.html')
    _success_url = kwargs.get("success_url","/")
    _app = _model._meta.app_label
    _route = f'{_app}/{_model._meta.model_name.lower()}/update/<int:pk>/'
    _view_name = kwargs.get("view_name",_model._meta.model_name.lower() + '-update')
    
    class _View(UpdateView):
        queryset = _queryset
        model = _model
