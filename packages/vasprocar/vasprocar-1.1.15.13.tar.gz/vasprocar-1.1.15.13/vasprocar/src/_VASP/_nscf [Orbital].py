...
if (read_orb == 1):
   ...
   if (lorbit == 12):
      read_phase = 1
      file_fase = open(dir_files + '/output/Phase_factors.dat', "w")
"""
#-------------------------------------------------------------------------
if (read_phase == 1): 
   file_fase = open(dir_files + '/output/Phase_factors.dat', "w")
    ...
    for n_spin in range(1,(ispin+1)):
        ...
        for point_k in range(1, (nk+1)):                       
            ...
            for Band_n in range (1, (int(nb/ispin) +1)):
                ...
                #======================================================================
                # Reading the Phase Factors ===========================================
                #======================================================================

                t_rPx = 0.0; t_rPy = 0.0; t_rPz = 0.0
                t_iPx = 0.0; t_iPy = 0.0; t_iPz = 0.0

                rPx = 0.0; rPy = 0.0; rPz = 0.0
                iPx = 0.0; iPy = 0.0; iPz = 0.0

                if (read_phase == 1 and lorbit == 12 and bands_sn[Band_n] == "sim"):
                   #------------------------
                   VTemp = procar.readline()
                   #------------------------ 
                   for ijk in range(1,((ni*2)+1)):
                       #---------------------------
                       resto = (ijk/2) - int(ijk/2)
                       #---------------------------
                       VTemp = procar.readline().split()
                       if (esc_ions == 0 or (esc_ions == 1 and temp_sn == "sim")):
                          if (resto != 0):
                             if (VTemp[4] != '0.000' and VTemp[4] != '******'):
                                rPx = float(VTemp[4]); t_rPx = t_rPx + rPx
                             if (VTemp[2] != '0.000' and VTemp[2] != '******'):
                                rPy = float(VTemp[2]); t_rPy = t_rPy + rPy
                             if (VTemp[3] != '0.000' and VTemp[3] != '******'):
                                rPz = float(VTemp[3]); t_rPz = t_rPz + rPz
                          if (resto == 0):
                             if (VTemp[4] != '0.000' and VTemp[4] != '******'):
                                iPx = float(VTemp[4]); t_iPx = t_iPx + iPx
                             if (VTemp[2] != '0.000' and VTemp[2] != '******'):
                                iPy = float(VTemp[2]); t_iPy = t_iPy + iPy
                             if (VTemp[3] != '0.000' and VTemp[3] != '******'):
                                iPz = float(VTemp[3]); t_iPz = t_iPz + iPz    

                if (read_orb == 1 and lorbit == 12 and bands_sn[Band_n] == "sim"):
                   file_fase.write(f'{separacao[wp][point_k]} {energ} {t_rPx} {t_rPy} {t_rPz} {t_iPx} {t_iPy} {t_iPz} \n')
