  ...
  def reionize(self, mol):
    ...
    if not current_charge == 0:
      while charge_diff > 0:
        ...
        if ppos is None:
          break
        log.info(
          f'Ionizing {self.acid_base_pairs[ppos].name} to balance previous charge corrections')
    ...
    while True:
      ...
      if ioccur and poccur and ppos < ipos:
        ...
        log.info(
          f'Moved proton from {self.acid_base_pairs[ppos].name} to {self.acid_base_pairs[ipos].name}'
        )
        ...
        if (iatom.GetNoImplicit()
            or ((patom.GetAtomicNum() == 7 or patom.GetAtomicNum() == 15) and patom.GetIsAromatic())
            or iatom.GetTotalValence() not in list(Chem.GetPeriodicTable().GetValenceList(
              iatom.GetAtomicNum()))):
