...
class TautomerTransform(object):
  ...
  def __repr__(self):
    return 'TautomerTransform({!r}, {!r}, {!r}, {!r})'.format(self.name, self.tautomer_str,
                                                              self.bonds, self.charges)
...
class TautomerCanonicalizer(object):
  ...
  def __init__(self, transforms=TAUTOMER_TRANSFORMS, scores=TAUTOMER_SCORES,
  ...
  def canonicalize(self, mol):
    ...
    for t in tautomers:
      ...
      if not highest or highest['score'] < score or (highest['score'] == score
                                                     and smiles < highest['smiles']):
        ...
        if bond.GetBondType() == BondType.DOUBLE and bond.GetStereo() > BondStereo.STEREOANY:
          ...
          for othertautomer in tautomers.values():
            if not othertautomer.GetBondBetweenAtoms(begin, end).GetBondType() == BondType.DOUBLE:
              neighbours = tautomer.GetAtomWithIdx(begin).GetBonds() + tautomer.GetAtomWithIdx(
                end).GetBonds()
