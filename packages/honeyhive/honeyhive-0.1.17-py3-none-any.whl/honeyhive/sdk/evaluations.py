...
from honeyhive.sdk.projects import get_project
...
class HoneyHiveEvaluation:
    def __init__(self, project: str, name: str, dataset_name: str = "", description: str = "", metrics: List[str] = None):
    ...
    def log_run(
        ...
        # add metrics to self.logged_metrics at config_index, input_index
        if len(self.logged_metrics) <= config_index:
            self.logged_metrics.append([])
        if len(self.logged_metrics[config_index]) <= input_index:
            self.logged_metrics[config_index].append([])
        self.logged_metrics[config_index][input_index] = metrics
    ...
    def finish(self):
        ...
        for key in keys:
            ...
            for i in range(len(self.logged_metrics)):
                for j in range(len(self.logged_metrics[i])):
                    summary_entry[self.configs[j]["name"]] += self.logged_metrics[i][j][key]
        ...
        for i in range(len(self.inputs)):
            ...
            j = 0
            for config in self.configs:
                single_result[config["name"]] = self.completions[i][j]
