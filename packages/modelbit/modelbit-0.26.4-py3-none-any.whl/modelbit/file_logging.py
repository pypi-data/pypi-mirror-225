...
import os, tempfile, hashlib, zstd, base64, logging
...
def _getPltBytes(plt: ModuleType) -> Tuple[bytes, str]:
  ...
  with tempfile.NamedTemporaryFile(suffix=".png") as tf:
    ...
    pngBytes = _getImageBytes(tf.name, "image.png")
    if len(pngBytes[0]) < PngThresholdBytes:
      return pngBytes
...
def _uploadImage(data: bytes, remotePath: str) -> str:
  ...
  body = cipher.encrypt(pad(zstd.compress(data, 10), AES.block_size))
