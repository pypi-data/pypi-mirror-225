...
class Paperless:
    ...
    def _next(self, endpoint) -> List[object]:
        data = []
        response = {}
    ...
    def get_all(self, cls: PaperlessModel) -> List[PaperlessModel]:
        """
    ...
    def get_ids(self, cls: PaperlessModel) -> List[int]:
        """
    ...
    def get(self, cls: PaperlessModel, id: int) -> PaperlessModel:
        """
        Return exactly one model by id.
        """
        response = self._http_get(
            f"{self.basepath}/{cls._endpoint}/{id}/", format="json"
        )
        return cls(**response)

    def save(self, item: PaperlessModel) -> PaperlessModel:
        """
        Save model object by object.
        """
        if isinstance(item, PaperlessDocumentNote):
            raise TypeError(
                "Updating document notes is currently not supported due to paperless API implementation!"
            )

        response = self._http_put(
            f"{self.basepath}/{item.__class__._endpoint}/{item.id}/",
            payload=item.__dict__,
            format="json",
        )
        return item.__class__(**response)

    def create(self, item: PaperlessModel) -> PaperlessModel:
        """
        ...
        if isinstance(item, PaperlessDocumentNote):
            raise TypeError(
                "Creating document notes is currently not supported due to paperless API implementation!"
            )

        response = self._http_post(
            f"{self.basepath}/{item.__class__._endpoint}/",
            payload=item.__dict__,
            format="json",
        )
        return item.__class__(**response)
