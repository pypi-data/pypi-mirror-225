...
class Downloader:
                ...
                if self.method == 'GET':
                    ...
                    with s.post(targeturl, headers=self.headers, data=json.dumps(body).encode('utf-8'), stream=True) as r:
                        ...
                        with open(filepath, 'wb') as f:
                            ...
                            # tqdm 객체 생성
                            tqdm_dict[fname] = tqdm.tqdm(file_size, f"Receiving {fname}", unit="B",
    ...
    @dispatch(list, str, bool=True)  # for function overloading
    ...
    @dispatch(datetime, datetime, str, bool=True)#for function overloading
