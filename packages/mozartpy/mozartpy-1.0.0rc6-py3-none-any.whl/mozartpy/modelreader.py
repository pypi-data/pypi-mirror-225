...
class Model:
    ...
    def __getInputItemByCsvfile(self, key):
        ...
        if not exportCsv:
            ...
            exportCsv = datetime.fromtimestamp(pathlib.Path(vdatpath).stat().st_mtime) > datetime.fromtimestamp(pathlib.Path(csvpath).stat().st_mtime)
        ...
        df = pd.read_csv(csvpath, parse_dates=parse_dates, low_memory=False)
        return df
