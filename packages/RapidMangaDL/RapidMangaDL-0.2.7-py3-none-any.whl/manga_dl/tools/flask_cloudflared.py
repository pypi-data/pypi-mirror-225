    ...
    for _ in range(10):
        try:
            ...
            if tunnel_id or config_path:
                ...
                tunnel_url = (re.search(r"(?P<url>https?:\/\/[^\s]+.trycloudflare.com)", metrics).group("url"))  # type: ignore
                break
        except:
            time.sleep(3)
    else:
...
def start_cloudflared(port, metrics_port, tunnel_id=None, config_path=None):
    ...
    print(f" * Running on {cloudflared_address} (Public)")
    print(f" * Monitor trafic on http://127.0.0.1:{metrics_port}/metrics (Private)")
...
def run_with_cloudflared(app):
    ...
    def new_run(*args, **kwargs):
        ...
        thread = Thread(target=start_cloudflared, args=tunnel_args)
        thread.daemon = True
        thread.start()
