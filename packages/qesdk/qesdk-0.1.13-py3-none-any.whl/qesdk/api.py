...
def get_interest_rate(date):
    if date==None:
        date="none"
     # 如果日期是datetime.datetime类型，将其转换为'%Y-%m-%d'字符串
    elif isinstance(date, datetime):
        date = date.date().strftime('%Y-%m-%d')
    # 如果日期是datetime.date类型，将其转换为'%Y-%m-%d'字符串
    elif isinstance(date, str):
        if len(date) != 10 or not is_valid_date(date):
            print('Invalid start_date')
            return None

    return qedataClient.instance()('get_interest_rate', **locals())

@assert_auth
def get_shibor_rate(date):
    if date==None:
        date="none"
     # 如果日期是datetime.datetime类型，将其转换为'%Y-%m-%d'字符串
    elif isinstance(date, datetime):
    ...
    return qedataClient.instance()('get_shibor_rate', **locals())

@assert_auth
def update_public_ip(hostip, host='quantease.store'):
