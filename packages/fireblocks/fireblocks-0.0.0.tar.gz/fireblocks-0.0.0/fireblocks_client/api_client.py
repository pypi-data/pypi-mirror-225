...
from fireblocks_client.exceptions import ApiTypeError, ApiValueError
from fireblocks_client.schemas import (
    NoneClass,
    BoolClass,
    Schema,
    FileIO,
...
class OpenApiResponse(JSONDetector):
    ...
    def deserialize(
        ...
        return self.response_cls(
            response=response, headers=deserialized_headers, body=body_data
        )
...
class ApiClient:
    ...
    def sign_jwt(self, path, body_json=""):
        ...
        token = {
            ...
            "sub": self.configuration.api_key,
            "bodyHash": sha256(body_json).hexdigest(),
        }

        return jwt.encode(token, key=self.configuration.secret_key, algorithm="RS256")
    ...
    def get_user_agent(self):
        ...
        if self.configuration.options.user_agent:
            user_agent = f"{self.configuration.options.user_agent} {user_agent}"
        return user_agent

    def set_default_header(self, header_name, header_value):
        self.default_headers[header_name] = header_value

    def get_headers(self, resource_path, body, header_params) -> HTTPHeaderDict:
        token = self.sign_jwt(resource_path, body)
