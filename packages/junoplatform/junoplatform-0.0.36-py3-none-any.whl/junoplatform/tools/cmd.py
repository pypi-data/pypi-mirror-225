...
def create(base:CMDBase, name, plant, module):
    ...
    try:
        ...
        with open(f"{name}/project.yml", "w") as f:
        ...
        with open(f"{name}/input.json", "w") as f:
...
def new(base:CMDBase, conf_file, tag, message, input):
    ...
    try:
        ...
        logging.info(f"junocofig: {junoconfig.__dict__}")
...
def pub(base:CMDBase, package_id):
    ...
    if not package_id:
        try:
            ...
            logging.error("must under project dir to run this command when no package_id provided(using project default)")
            exit(1)
    ...
    with open(papath, 'rb') as f:
        r = requests.post(api, files = {'file': (f'{package_id}.zip', f, 'application/zip')}, headers={"Authorization": f"Bearer {base.juno_cfg['token']}"})
        if r.status_code != 200:
...
def deploy_package(base:CMDBase, package_id:str, kind:int):
    ...
    params["package_id"] = package_id
    params["kind"]= kind
    r = requests.post(api, params=params, headers={"Authorization": f"Bearer {base.juno_cfg['token']}"})
    if r.status_code != 200:
        msg = f"faild fetch packages "
        if "detail" in r.json():
            msg += r.json()["detail"]
...
def add_plant(base:CMDBase, name, location, description):
    ...
    r = requests.post(api, json=data, headers={"Authorization": f"Bearer {base.juno_cfg['token']}"})
    if r.status_code != 200:
