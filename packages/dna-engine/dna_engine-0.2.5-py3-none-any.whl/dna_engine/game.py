...
class Game():
    ...
    def __standard_event_handler(self, event : pygame.event.Event) -> None:
    ...
    def __text_entry_event_handler(event : pygame.event.Event, game_render : pygame.Surface) -> None:
    ...
    def ask(prompt : str) -> str:
        ...
        overlay = pygame.Surface(Game.__screen.get_rect())
        overlay.fill((32, 32, 32, 128))
        Game.__game_render.blit(overlay)
        

        while Game.__text_entry_toggle == True:
            Game.__text_entry_event_handler()
            
            fresh_render = pygame.Surface(Game.__screen.get_rect(), surface = Game.__game_render)
            text_surface = font.render(Game.__text_entry_field, pygame.Color(255, 255, 255))
            fresh_render.blit(text_surface, (Game.__screen.get_width() / 2, Game.__screen.get_height() - 64))
            Game.__screen.blit(pygame.transform.smoothscale(fresh_render, Game.__screen.get_size()).convert_alpha(), (0,0))
            # Flip the display
