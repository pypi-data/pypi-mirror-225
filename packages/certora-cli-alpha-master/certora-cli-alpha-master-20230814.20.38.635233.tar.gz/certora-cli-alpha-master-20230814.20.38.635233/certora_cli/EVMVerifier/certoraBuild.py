...
class CertoraBuildGenerator:
    ...
    def __init__(self, input_config: InputConfig, context: CertoraContext) -> None:
        ...
        self._compiled_artifacts_to_clean: Set[Tuple[str, CompilerLang]] = set()
    ...
    def collect_for_file(self,
        ...
        if not self.context.debug:
            self._compiled_artifacts_to_clean.add((sdc_name, smart_contract_lang))
        else:
    ...
    def cleanup(self) -> None:
        for f in self.generated_tmp_files:
            if f.is_file():
                Util.remove_file(f)
        for sdc_name, smart_contract_lang in self._compiled_artifacts_to_clean:
    ...
    @staticmethod
    def _find_closest_methods(method_name: str, methods: List[Func]) -> List[str]:
        """
    ...
    @staticmethod
    def _suggest_methods(wrong_sig: str, suggested_replacements: List[str]) -> None:
        """
    ...
    def _verify_method(self, method_input: str, sdc_pre_finder: SDC) -> None:
        ...
        if len(possible_methods) == 0:
            possible_methods = self._find_closest_methods(input_method_name, public_methods)

        if len(possible_methods) > 0:  # We have suggestions
            self._suggest_methods(method_input, possible_methods)
        ...
        for i, build_arg_contract_file in enumerate(sorted(self.input_config.files)):
            ...
            if context.method and (build_arg_contract_file in context.verified_contract_files):
                ...
                if verified_contract_name:
                    ...
                    self._verify_method(context.method, sdc_with_verified_contract_name)
    ...
    def _get_matching_sdc_names_for_SDCs_iterator(self, contract: str) -> Iterator[str]:
        return (k for k, v in self.SDCs.items() if k.startswith(f"{contract}_"))

    def get_one_sdc_name_from_SDCs(self, contract: str) -> str:
        return next(self._get_matching_sdc_names_for_SDCs_iterator(contract))

    def get_matching_sdc_names_from_SDCs(self, contract: str) -> List[str]:
        return list(self._get_matching_sdc_names_for_SDCs_iterator(contract))
...
class SpecWithImports:
    ...
    def __init__(self, _spec_file: str, _spec_idx: int, _abspath_imports_to_locs: Dict[str, Set[str]],
        ...
        self.eventual_path_to_spec = self._get_eventual_path_to_spec(Path(self.spec_file))
        ...
        self.eventual_path_to_orig_imports = {self._get_eventual_path_to_spec(Path(abspath)): list(orig_imports) for
        ...
        self.abspath_to_eventual_import_paths = {abspath: self._get_eventual_path_to_spec(Path(abspath)) for abspath in
        ...
        self.eventual_import_paths_to_relpaths = {
            self._get_eventual_path_to_spec(Path(abspath)): str(Path(abspath).relative_to(Path().resolve()))
            for abspath in self.abspath_imports_to_locs.keys()
        }

        self._assert_distinct_filenames()

    def _get_eventual_path_to_spec(self, path: Path) -> str:
    ...
    def _assert_distinct_filenames(self) -> None:
