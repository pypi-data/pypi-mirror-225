...
class CompilerCollectorFactory:
    ...
    def get_compiler_collector(self, path: Path) -> CompilerCollector:
        ...
        if str(path).endswith(".vy"):
            ...
            version = self._get_solc_version(path)
    ...
    def _get_solc_version(self, contract_file_path: Path) -> Tuple[int, int, int]:
        """
        @param contract_file_path: the contract that we are working on
        ...
        version = self._get_solc_exe_version(solc_path)
        return version

    @lru_cache(maxsize=32)
    def _get_solc_exe_version(self, solc_name: str) -> Tuple[int, int, int]:
        """
