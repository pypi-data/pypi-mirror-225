...
class StockParser(AbcParser, ABC):
    ...
    def contains_day_range(self, general_info_list: List) -> bool:
    ...
    def contains_ceo_row(self, general_info_list: List) -> bool:
        """
    ...
    def update_general_info(self, general_info_list: List):
        ...
        if self.contains_day_range(general_info_list):
        ...
        # expected_market_cap: str = general_info_list[(self.i+1)]
        # TODO: "jt" still only works for indonesian market. need to be adjusted properly!
        expected_market_cap: str = general_info_list[self.i]
        expected_avg_vol: str = general_info_list[(self.i + 1)]
        if "IDR" in expected_market_cap and ("jt" in expected_avg_vol or "rb" in expected_avg_vol):
        ...
        # only avg volume
        elif "IDR" not in expected_market_cap and ("jt" in expected_market_cap or "rb" in expected_market_cap):
            (self.info.general.avg_volume, self.info.general.avg_volume_unit) = get_price_with_unit(
                general_info_list[self.incr()], self.lang,
            )
        # only market cap
        elif "IDR" in expected_market_cap and ("jt" not in expected_avg_vol or "rb" not in expected_avg_vol):
            (self.info.general.avg_volume, self.info.general.avg_volume_unit) = get_price_with_unit(
                general_info_list[self.incr()], self.lang,
            )
        ...
        if len(general_info_list) > 7:
            if self.contains_ceo_row(general_info_list):
