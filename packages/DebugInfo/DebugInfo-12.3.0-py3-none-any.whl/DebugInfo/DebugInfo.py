...
class 调试模板:
    ...
    def 副本(self):
        ...
        # 复制表格对齐控制表
        if self.__表格列对齐:
            控制字串: str = ''.join(['r' if 方式 == 对齐方式.右对齐 else ('c' if 方式 == 对齐方式.居中对齐 else 'l') for 方式 in self.__表格列对齐])
            副本.设置列对齐(控制字串)
...
def 秒表(目标方法: callable):
    ...
    def 参数接收器(*args, **kwargs):
        ...
        # 准备打印内容
        白板.准备表格('lll').添加一行('项目', '值', '备注').修饰方法(青字)
        白板.添加一行('方法名称', 目标方法.__name__)
        白板.添加一行('秒表启动:', datetime.fromtimestamp(秒表启动时间), '来自 time.time')

        if 秒表计时 > 1:
            白板.添加一行('计时/s:', 绿字(秒表计时), '来自 time')
        else:
            白板.添加一行('计时/ms:', 绿字(秒表计时 * 1000), '来自 time')

        if 时钟计时 > 1:
            白板.添加一行('计时/s:', 绿字(时钟计时), '来自 perf_counter')
        elif 时钟计时 > 0.001:
            白板.添加一行('计时/ms:', 绿字(时钟计时 * 1000), '来自 perf_counter')
        elif 时钟计时 > 0.000001:
            白板.添加一行('计时/us:', 绿字(时钟计时_ns * 0.001), '来自 perf_counter_ns')
        else:
            白板.添加一行('计时/ns:', 绿字(时钟计时_ns), '来自 perf_counter_ns')

        if 程序计时 > 1:
            白板.添加一行('计时/s:', 绿字(程序计时), '来自 process_time')
        elif 程序计时 > 0.001:
            白板.添加一行('计时/ms:', 绿字(程序计时 * 1000), '来自 process_time')
        elif 程序计时 > 0.000001:
            白板.添加一行('计时/us:', 绿字(程序计时_ns * 0.001), '来自 process_time_ns')
        else:
            白板.添加一行('计时/ns:', 绿字(程序计时_ns), '来自 process_time_ns')

        白板.添加一行('秒表结束:', datetime.fromtimestamp(秒表结束时间), '来自 time.time')
