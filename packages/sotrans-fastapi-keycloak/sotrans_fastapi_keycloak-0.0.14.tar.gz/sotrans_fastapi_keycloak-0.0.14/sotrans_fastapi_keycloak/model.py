...
class KeycloakUserBaseModel(BaseModel):
...
def user_model_attributes_validator(cls: type[BaseModel], data: Any) -> Any:
    ...
    """
    if not isinstance(data, dict) or 'attributes' not in data or not isinstance(data['attributes'], dict):
        return data
    ...
    for attribute, value in attributes.copy().items():
        if attribute in cls.model_fields:
            ...
            if not _annotation_is_type(attribute, list) and isinstance(value, list) and len(value) <= 1:
...
def user_model_attributes_serializer(model: BaseModel) -> dict[str, Any]:
    ...
    def is_attribute(_field_name):
        return any([isinstance(meta, KeycloakUserAttribute) for meta in model.model_fields[_field_name].metadata])

    result = {}
    for field_name, field in model:
        if not field:
            continue
        elif is_attribute(field_name):
