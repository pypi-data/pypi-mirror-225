...
from celery.patches.kombu.utils import (
    retry_over_time as aretry_over_time,
    is_async_callable
)
...
class AbstractSignal:
    ...
    def connect(self, *args, **kwargs):
        ...
        if args and callable(args[0]):
            return self._handle_options(*args[1:], **kwargs)(args[0])
        return self._handle_options(*args, **kwargs)

    def _handle_options(self, sender=None, weak=True, dispatch_uid=None,
                        retry=False):
...
class SyncSignal(AbstractSignal):  # pragma: no cover
    """Create new signal.
    ...
    def __init__(self, providing_args=None, use_caching=False, name=None):
        super().__init__(providing_args, use_caching, name)
        self.receivers = []
...
class Signal(AbstractSignal):
    def __init__(self, providing_args=None, use_caching=False, name=None):
        super().__init__(providing_args, use_caching, name)
        self._signal = SyncSignal(providing_args, use_caching, name)
        self._asignal = AsyncSignal(providing_args, use_caching, name)

    def connect(self, *args, **kwargs):
        if args and callable(args[0]):
            if is_async_callable(args[0]):
                return self._asignal._handle_options(
                    *args[1:], **kwargs)(args[0])
            else:
                return self._signal._handle_options(
                    *args[1:], **kwargs)(args[0])
        return self._handle_options(*args, **kwargs)
    ...
    def _handle_options(self, sender=None, weak=True, dispatch_uid=None,
                        retry=False):
        def _connect_signal(fun):
            if is_async_callable(fun):
                handler = self._asignal._handle_options
            else:
                handler = self._signal._handle_options
            return handler(sender, weak, dispatch_uid, retry)(fun)

        return _connect_signal

    def send(self, sender, **named):
        return self._signal.send(sender, **named)

    async def asend(self, sender, **named):
        return await self._asignal.send(sender, **named)

    def disconnect(self, receiver=None, sender=None, weak=None,
                   dispatch_uid=None):
        disconnected = self._signal.disconnect(
            receiver, sender, weak, dispatch_uid)
        if not disconnected:
            disconnected = self._asignal.disconnect(
                receiver, sender, weak, dispatch_uid)
        return disconnected

    def has_listeners(self, sender=None):
        return (
            bool(self._signal._live_receivers(sender))
            or bool(self._asignal._live_receivers(sender))
        )
    ...
    @property
    def areceivers(self):
        return self._asignal.receivers
