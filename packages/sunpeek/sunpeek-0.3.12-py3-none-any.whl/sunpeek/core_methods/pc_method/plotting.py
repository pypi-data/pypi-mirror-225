...
class PCPlots:
    ...
    def plot_cover(self,
        ...
        txt_method = OrderedDict([
            ...
            ('Algorithm details', (
                f'Equation: {self.pc_output.equation}. '
    ...
    def plot_bars(self) -> Optional[List[matplotlib.figure.Figure]]:
        ...
        utc_str = lambda d: (f'{d.tz_convert(tz):{pu.Defaults.format_datetime.value}} '
                             f'(UTC{int(d.tz_convert(tz).utcoffset().total_seconds() / 3600):+0})')
        ...
        box_plant = TextArea((f'Plant name: "{plant_name}".\n'
                              ...
                              f'Data from {utc_str(self.pc_output.datetime_eval_start)} to {utc_str(self.pc_output.datetime_eval_end)}.'
                              ), textprops=textprops)
    ...
    def plot_shadow_and_intervals(self
        ...
        box_info = TextArea((
            ...
            f' long.\n'
            f'Total interval duration: '
            f'{pendulum.duration(seconds=self.pc_output.plant_output.total_interval_length.total_seconds()).in_words(locale="en")}.'
    ...
    def plot_square(self,
        ...
        fig_list.append(self._fig_square(data, box_component, axis_range, pu.Colors.red.value))
    ...
    def plot_time(self,
        ...
        fig_list.append(self._fig_time(data, time_display, box_component, y_ratio_limits, pu.Colors.red.value))
    ...
    def _fig_time(self,
        ...
        def _plot_ax(ax_, ratio, y_label, title):
            ...
            ax_.set_xlim(self.pc_output.datetime_eval_start.tz_convert(tz),
                         self.pc_output.datetime_eval_end.tz_convert(tz), auto=None)
    ...
    def plot_plant_overview(self) -> Optional[List[matplotlib.figure.Figure]]:
        ...
        txt_plant = OrderedDict([
            ...
            ('Plant altitude', f'{p.altitude:.0f~P}'),
        ])
        txt_plant = pu.anonymize(txt_plant, do_anonymize=self.settings.anonymize)
        ...
        for ao in self.pc_output.array_output:
            ...
            tbl_rows.append({
                'Array name': a.name,
                'Collector name': a.collector_type.product_name,
                'Manufacturer': a.collector_type.manufacturer_name,
    ...
    def plot_collector_overview(self) -> Optional[List[matplotlib.figure.Figure]]:
        ...
        # Data to plot: unqiue Collectors
        p_width = 0.8
        ...
        collectors = [ao.array.collector_type for ao in self.pc_output.array_output]
        ...
        for c in unique_collectors:
            tbl_rows.append({
                ...
                'Gross area': f'{c.area_gr.to("m**2").m:.1f}',
        ...
        for ao in self.pc_output.array_output:
            ...
            for i in range(self.pc_output.plant_output.n_intervals):
                tbl_rows.append({
                    ...
                    'rd_gti': f'{ao.data.rd_gti[i].to("W m**-2").m:0.0f}' if ao.data.rd_gti is not None else na_str,
                    'rd_bti': f'{ao.data.rd_bti[i].to("W m**-2").m:0.0f}' if ao.data.rd_bti is not None else na_str,
                    'rd_dti': f'{ao.data.rd_dti[i].to("W m**-2").m:0.0f}' if ao.data.rd_dti is not None else na_str,
                    've_wind': f'{ao.data.ve_wind[i].to("m s**-1").m:0.1f}' if ao.data.ve_wind is not None else na_str,
                    'tp_measured': f'{ao.tp_sp_measured[i].to("W m**-2").m:0.0f}' if has_tp else na_str,
                    'tp_estimated_safety': f'{ao.tp_sp_estimated_safety[i].to("W m**-2").m:0.0f}',
                    'tp_ratio': f'{(ao.tp_sp_measured[i] / ao.tp_sp_estimated_safety[i]).to("percent").m:0.1f}' if has_tp else na_str,
                })
    ...
    def plot_symbols_overview(self) -> Optional[List[matplotlib.figure.Figure]]:
        ...
        symbols = [
            ...
            [r'$F\prime$',
             'Collector efficiency factor (ratio of heat transfer resistances of absorber to ambient vs. fluid to ambient)',
             d],
            [r'$G_{hem}$', 'Hemispherical solar irradiance on the plane of collector', r'$\dfrac{W}{m^2}$'],
            [r'$G_{b}$', 'Direct solar irradiance (beam irradiance) on the plane of collector', r'$\dfrac{W}{m^2}$'],
            [r'$G_{d}$', 'Diffuse solar irradiance on the plane of collector', r'$\dfrac{W}{m^2}$'],
            [r'$K_b$', 'Incidence angle for direct solar radiation', d],
            [r'$K_d$', 'Incidence angle modifier for diffuse solar radiation', d],
            ...
            # Greek
            [r'$\eta_b$', r'Collector efficiency based on beam irradiance $G_b$', d],
            [r'$\eta_{hem}$', r'Collector efficiency based on hemispherical irradiance $G_{hem}$', d],
            [r'$\theta$', 'Angle of Incidence', r'°'],
            [r'$\vartheta_a$', 'Ambient air temperature', r'$°C$'],
            [r'$\vartheta_{in}$', 'Collector inlet temperature', r'$°C$'],
            [r'$\vartheta_{out}$', 'Collector inlet temperature', r'$°C$'],
            [r'$\vartheta_{op}$', (f'Collector operating temperature '
                                   r'$\vartheta_{op}=mean(\vartheta_{in}, \vartheta_{out})$'), r'$°C$'],
            [r'$\vartheta\backprime_{op}$', f'Derivative of collector operating temperature ', r'$K/h$'],
        ]
    ...
    def _plot_single_interval(self,
        ...
        if has_tp:
            ax_tp.axhline(y=tp_sp_meas, label='Average measured power',
                          linewidth=0.75, color=pu.Colors.almost_black, zorder=1.5, linestyle=(5, (10, 3)))
        ...
        vf_sp_max = axes_limits.get('vf_sp_max', max(20, np.ceil(vf_sp.astype(float).max())))
