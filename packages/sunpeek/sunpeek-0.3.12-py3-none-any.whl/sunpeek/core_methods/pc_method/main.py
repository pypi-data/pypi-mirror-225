...
from sunpeek.core_methods.pc_method import AvailablePCMethods, AvailablePCEquations, OutputUnits
...
class PCMethod(ABC):
    ...
    def _calc_output(self):
        ...
        for array in self.plant.arrays:
            ...
            df = self.equation.calc_estimated_power(array, aggregator)
    ...
    def _create_output_object(self) -> results.PCMethodOutput:
        ...
        out.evaluation_mode = 'ISO' if self.mode.value == AvailablePCMethods.iso else 'ISO Extended'
        ...
        for array in self.plant.arrays:
            ...
            if has_intervals:
                ...
                try:
                    ...
                    # Data columns that are always returned by Equation
