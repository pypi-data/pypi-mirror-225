...
class SyncAgent:
    ...
    def init(self, frequency=300, timeout=5, force_wait_time=30, finish_wait_time=30, decorate=True, verbose=False):
        self.frequency = frequency
        self.timeout = timeout
        self.verbose = verbose
        self.force_wait_time = force_wait_time
        self.finish_wait_time = finish_wait_time
    ...
    def finish_sync(self):
        if os.fork() != 0:
            return
        if self.verbose:
            print(f'WANDB SYNC FARM AGENT - Waiting for {self.finish_wait_time} seconds before requesting the final sync')
        time.sleep(self.finish_wait_time)
        self._request_sync()
