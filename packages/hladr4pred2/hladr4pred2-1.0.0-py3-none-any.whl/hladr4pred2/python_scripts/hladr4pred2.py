######################################################################################
# HLADR4Pred2.0 is developed for predicting, desigining and scanning HLA-DRB1*04:01  #
# binding peptides. It is developed by Prof G. P. S. Raghava's group.                #
# Please cite: https://webs.iiitd.edu.in/raghava/hladr4pred2/                        #
######################################################################################
import argparse
...
model_path = os.path.join(filepath, 'model', 'hladr4pred_model.onnx')

if not os.path.exists(model_path):
    print('***** Model is downloading... Please wait for a while *****')
    
    url = "https://webs.iiitd.edu.in/raghava/hladr4pred2/model.zip"
    file_name_dn = os.path.join(filepath, 'model.zip')
    
    with tqdm(unit="B", unit_scale=True, unit_divisor=1024, miniters=1, desc="Downloading") as t:
        urllib.request.urlretrieve(url, file_name_dn, reporthook=lambda blocknum, blocksize, totalsize: t.update(blocksize))
    
    with zipfile.ZipFile(file_name_dn, 'r') as zip_ref:
        zip_ref.extractall(filepath)        
    os.remove(file_name_dn)  # Remove the downloaded zip file
...
def main():  
    ...
    parser.add_argument("-t","--threshold", type=float, help="Threshold: Value between 0 to 1 by default 0.16")
    parser.add_argument("-w","--winleng", type=int, choices =range(9, 21), help="Window Length: 9 to 20 (scan mode only), by default 9")
    parser.add_argument("-d","--display", type=int, choices = [1,2], help="Display: 1:Binders only, 2: All peptides, by default 1")
    args = parser.parse_args()    
    ('############################################################################################')
    print('# This program HLADR4Pred2.0 is developed for predicting, desigining and scanning          #')
    print('# HLA-DRB1*04:01 binding peptides, developed by Prof G. P. S. Raghava group.               #')
    print('# Please cite: HLADR4Pred2.0; available at https://webs.iiitd.edu.in/raghava/hladr4pred2/  #')
    print('############################################################################################')
    ...
    if Job == 1:
        print('\n======= Thanks for using Predict module of HLADR4Pred2.0. Your results will be stored in file :',result_filename,' =====\n')
        df_2,dfseq = readseq(Sequence)
        df1 = lenchk(dfseq)
        X = feature_gen(df1)
        mlres = model_run(X,nf_path+'/model/hladr4pred_model.onnx')
        ...
        if dplay == 1:
            df44 = df44.loc[df44.Prediction=="Binder"]
        else:
            df44 = df44
        df44 = round(df44,3)
    ...
    #===================== Design Model Start from Here ======================
        ...
        print('\n======= Thanks for using Design module of HLADR4Pred2.0. Your results will be stored in file :',result_filename,' =====\n')
    ...
    #=============== Scan Model start from here ==================
        ...
        print('\n======= Thanks for using Scan module of HLADR4Pred2.0. Your results will be stored in file :',result_filename,' =====\n')
        ...
        os.remove(filename)
        print("\n=========Process Completed. Have an awesome day ahead.=============\n")
    print('\n======= Thanks for using HLADR4Pred2.0. Your results are stored in file :',result_filename,' =====\n\n')
