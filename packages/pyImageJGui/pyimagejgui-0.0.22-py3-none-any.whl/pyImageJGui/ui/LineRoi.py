...
class LineRoi(QGraphicsLineItem):
    ...
    def update_handles(self):
        ...
        self.start.setRect(
            QRectF(round(start_point.x() - handle_width / 2), round(start_point.y() - handle_width / 2), handle_width,
                   handle_width))

        end_point = self.line().p2()
        self.end.setRect(
            QRectF(round(end_point.x() - handle_width / 2), round(end_point.y() - handle_width / 2), handle_width,
                   handle_width))

        middle_point = self.line().center()
        self.middle.setRect(
            QRectF(round(middle_point.x() - handle_width / 2), round(middle_point.y() - handle_width / 2), handle_width,
                   handle_width))
