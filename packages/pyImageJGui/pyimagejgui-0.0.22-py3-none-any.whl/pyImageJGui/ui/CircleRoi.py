...
class CircleRoi(QGraphicsEllipseItem):
    ...
    def update_handles(self):
        ...
        self.left_top.setRect(
            QRectF(round(left_top_point[0] - handle_width / 2), round(left_top_point[1] - handle_width / 2),
                   handle_width, handle_width))

        left_middle_point = self.get_handle_point(180)
        self.left_middle.setRect(
            QRectF(round(left_middle_point[0] - handle_width / 2), round(left_middle_point[1] - handle_width / 2),
                   handle_width, handle_width))

        left_bottom_point = self.get_handle_point(225)
        self.left_bottom.setRect(
            QRectF(round(left_bottom_point[0] - handle_width / 2), round(left_bottom_point[1] - handle_width / 2),
                   handle_width, handle_width))

        middle_top_point = self.get_handle_point(90)
        self.middle_top.setRect(
            QRectF(round(middle_top_point[0] - handle_width / 2), round(middle_top_point[1] - handle_width / 2),
                   handle_width, handle_width))

        middle_bottom_point = self.get_handle_point(270)
        self.middle_bottom.setRect(
            QRectF(round(middle_bottom_point[0] - handle_width / 2), round(middle_bottom_point[1] - handle_width / 2),
                   handle_width, handle_width))

        right_top_point = self.get_handle_point(45)
        self.right_top.setRect(
            QRectF(round(right_top_point[0] - handle_width / 2), round(right_top_point[1] - handle_width / 2),
                   handle_width, handle_width))

        right_middle_point = self.get_handle_point(0)
        self.right_middle.setRect(
            QRectF(round(right_middle_point[0] - handle_width / 2), round(right_middle_point[1] - handle_width / 2),
                   handle_width, handle_width))

        right_bottom_point = self.get_handle_point(315)
        self.right_bottom.setRect(
            QRectF(round(right_bottom_point[0] - handle_width / 2), round(right_bottom_point[1] - handle_width / 2),
                   handle_width, handle_width))
