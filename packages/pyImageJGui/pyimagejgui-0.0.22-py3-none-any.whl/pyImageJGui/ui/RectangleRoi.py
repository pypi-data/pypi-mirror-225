...
class RectangleRoi(QGraphicsRectItem):
    ...
    def update_handles(self):
        ...
        self.left_top.setRect(
            QRectF(round(lt_x - handle_width / 2), round(lt_y - handle_width / 2), handle_width, handle_width))
        self.left_middle.setRect(
            QRectF(round(lt_x - handle_width / 2), round(lt_y + height / 2 - handle_width / 2), handle_width,
                   handle_width))
        self.left_bottom.setRect(
            QRectF(round(lt_x - handle_width / 2), round(lt_y + height - handle_width / 2), handle_width, handle_width))
        self.middle_top.setRect(
            QRectF(round(lt_x + width / 2 - handle_width / 2), round(lt_y - handle_width / 2), handle_width,
                   handle_width))
        self.middle_bottom.setRect(
            QRectF(round(lt_x + width / 2 - handle_width / 2), round(lt_y + height - handle_width / 2), handle_width,
                   handle_width))
        self.right_top.setRect(
            QRectF(round(lt_x + width - handle_width / 2), round(lt_y - handle_width / 2), handle_width, handle_width))
        self.right_middle.setRect(
            QRectF(round(lt_x + width - handle_width / 2), round(lt_y + height / 2 - handle_width / 2), handle_width,
                   handle_width))
        self.right_bottom.setRect(
            QRectF(round(lt_x + width - handle_width / 2), round(lt_y + height - handle_width / 2), handle_width,
                   handle_width))
