...
class BQClient:
    ...
    def run(
        ...
        if isinstance(circuits, list):
            ...
            if not asynchronous:
                ...
                job_results = self.wait(job_results)
                add_circuit_to_all(job_results, circuits)
                return job_results
    ...
    def wait(
        ...
        while True:
            job_results = self._get(job_ids, _need_qc_unprocessed=False)
            if self._check_all_in_terminal_states(job_results):
                if (
    ...
    def _get(
        ...
        job_results = jobs.search_jobs(
            ...
            _need_qc_unprocessed=_need_qc_unprocessed,
        )
