...
# line_chart(xAxisvalue, predicatedLine, actualLine, xlabel=xlabel, theme='white')
...
def horizontal_waterfall(categories, values, **optional_params):
    ...
    if ('ylabel' in optional_params):
        ax.set_ylabel(optional_params['ylabel'], fontsize=8, color='white' if theme == 'black' else 'black')
    if 'title' in optional_params:
...
# xlabel='xlabel'
# horizontal_waterfall(categories, values, xlabel=xlabel, title=title)
...
# vertical_waterfall(categories, values, title=title)
...
# title='Title'
# baseline_graph(baseline, x_labels, colors, series1, series2, series3, title=title)
...
# title='Title'
# colors = ["#FC2964", "#0095FF", "#5C33F6"]
# scatter_plot_graph(marker_shapes, x_values, series_data, colors,title=title)
...
def stack_bar(categories, below, above, **optional_params):
    ...
    if 'xlabel' in optional_params:
        ax.set_xlabel(optional_params['xlabel'], color='white')
    if 'ylabel' in optional_params:
        ax.set_ylabel(optional_params['ylabel'], color='white')
    if 'title' in optional_params:
        ax.set_title(optional_params['title'], color='white')
    if 'legend' in optional_params:
        legend = ax.legend(loc='lower center', title=optional_params['legend'], bbox_to_anchor=(0.50, -0.25), ncol=2)
...
# ylabel='Title'
# legend='legends'
# stack_bar(categories,below,above,title=title,ylabel=ylabel,legend=legend)
...
# heatmap_graph(y_axis_data, x_axis_data, colors,title=title)
...
# colors = ["#1D2026", "#393939", "#666666", "#BDBDBD", "#E0E0E0", "#F2F2F2", "#FD93B1",
#           "#FC2964", "#FEC9D8", "#FD5F8B", "#0095FF", "#E6F5FF", "#5C33F6", "#8566F8"]
# heatmap2_graph(y_axis_data, x_axis_data, colors,dataFrom,dataTo,title=title)
...
# xAxisTitle='Title'
# yAxisTitle='Title'
# boxPlot_graph(data, titles, colors,xlabel=xAxisTitle,ylabel=yAxisTitle)
...
# xAxisTitle = 'Title'
# yAxisTitle = 'Title'
# boxPlot2_graph(data, titles, colors, xlabel=xAxisTitle, ylabel=yAxisTitle)
...
#       [20, 21, 22, 23, 24]
# ] 
# boxPlot3_graph(data, colors, legend=legend,title=title, xlabel=xAxisTitle, ylabel=yAxisTitle)
...
# title = 'Scatter plot with dotted line'
# xlabel='Title'
# ylabel='Title'
# handles=['Group 1']
# scatter_plot_with_dotted_line(data,title=title,xlabel=xlabel,ylabel=ylabel)
...
# ylabel='Title'
# handles=['Group 1']
# scatter_plot_with_line(data,title=title,xlabel=xlabel,ylabel=ylabel)
