...
class ModelMixin:
    ...
    def get_model_repo(self, name: str) -> ModelRepo:
        resp = self.session.get_json(f"/models/repo/name/{name}")

        return build_model_repo(resp, self.session)
    ...
    def create_model_repo(
        ...
        if not self.session.is_plan_expired():
            ...
            return self.get_model_repo(name)
        else:
    ...
    def _create_or_retrieve_repository(
        ...
        try:
            ...
            if exists_ok and e.code == 400:
                ...
                resp = self.session.get_json(f"/models/repo/name/{repo_name}")
                repo_id = resp["id"]
                return repo_id
            else:
                raise e
