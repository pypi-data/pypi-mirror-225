...
class TorchForwardTransform:
    ...
    def execute_all(self, xt: torch.Tensor) -> (torch.Tensor, torch.Tensor):
        import numpy as np
        ...
        if has_channels:
            ...
            energy_t = torch.empty(frames, dtype=torch.single)
    ...
    def execute(self, xt: torch.Tensor) -> (torch.Tensor, torch.float32):
        from pyaaware.transform_types import Overlap
