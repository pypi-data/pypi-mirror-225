...
def map_formation_group_system(
    form_or_group_or_system: pd.Series, MissingValue: Union[float, str] = np.nan
    ...
    for item in form_or_group_or_system.unique():
        form, group, siistem = MissingValue, MissingValue, MissingValue
        try:
            dic = dic_names[item]
            name = dic["NAME"]
            parent = dic["PARENT"]
            ...
            pass
        form, group, siistem = pd.Series([form, group, siistem]).fillna(MissingValue)
        mapping[item] = (form, group, siistem)

    form, group, siistem = zip(*form_or_group_or_system.map(mapping))
...
def standardize_group_formation_name(name: Union[str, Any]) -> Union[str, Any]:
    ...
    known_prefixes = ["LOWER", "MIDDLE", "UPPER"]
    ...
    for mapping_name, mapping in map_similar_keys.items():
        ...
        if len(idx) == 0:  # No metadata matching this mapping
            ...
            values = cdf_meta.loc[idx].apply(
                lambda x: _apply_function(x, highest_rank_key), axis=0  # noqa: B023
            )
            ...
            if load_from_gis:
                try:
                    ...
                    rows["FORMATION"], rows["GROUP"], _ = map_formation_group_system(
                        form_or_group_or_system=rows.Lithostrat, MissingValue="UNKNOWN"
                    )
...
def get_trajectory_mapper_from_cdf(
    ...
    for well in well_names:
        ...
        if len(well_data_cdf) == 0:
            warnings.warn(f"No trajectory data was found for well {well}. Skipping it!")
            continue
        ...
        tvdss_query_ = well_df_discrete["TVDSS"].to_list()
        tvdss_query = [-x for x in tvdss_query_]
        tvdbml_query = well_df_discrete["TVDBML"].to_list()
        xloc_query = well_df_discrete["X"].to_list()
        yloc_query = well_df_discrete["Y"].to_list()

        trajectory_mapper_well = {
            "MD": md_query,
            "TVDKB": tvdkb_query,
            "TVDSS": tvdss_query,
            "TVDBML": tvdbml_query,
...
def interpolate_discrete_trajectory_data_along_wellbore(
    ...
    for well in trajectory_mapping:
        ...
        for trajectory_field in trajectory_data.keys():
            ...
            df_.loc[
                df_[id_column] == well, trajectory_field
...
def get_cognite_client(
    ...
    cognite_project_id = "akbp-subsurface"
...
def run_deployed_model(
    ...
    **kwargs,
    ...
    for well in predictions:
        ...
        df_.loc[df_[id_column] == well["well"], pred.columns] = pred.values
