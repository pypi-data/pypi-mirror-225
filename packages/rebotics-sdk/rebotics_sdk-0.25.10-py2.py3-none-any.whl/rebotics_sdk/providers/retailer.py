...
from .base import ReboticsBaseProvider, remote_service, PageResult, UniqueRequestProxy
...
class RetailerProvider(ReboticsBaseProvider):
    ...
    proxy_config = {
        'request_uid_header_name': 'REB-CLIENT-REQUEST-UID'
    }
    ...
    @remote_service('/api/v4/token-auth/', raw=True)
    def token_auth(self, username, password, **kwargs):
        payload = dict(
            username=username,
            password=password,
        )
        if 'verification_code' in kwargs.keys():
    ...
    @remote_service(
        '/api/v4/processing/actions/',
        request_uid_value=lambda **kwargs: kwargs['json'].pop('_request_uid', None))
    ...
    @remote_service('/api/v4/processing/actions/export-to-cvat/')
    def export_to_cvat(self, scan_ids, image_quality=80, segment_size=20,
                       workspace=CVAT_EXPORT_WORKSPACE, **kwargs):
        return self.session.post(json={
            'scans': sorted(list(set(scan_ids))),
    ...
    @remote_service('/api/v1/realograms/actions/{action_id}/tasks/{task_id}/core-result/')
    def get_core_result(self, action_id, task_id):
        return self.session.get(action_id=action_id, task_id=task_id)

    @remote_service('/api/v1/realograms/actions/{action_id}/tasks/')
    def get_task_list_for_action(self, action_id):
        return PageResult(
            self.session.get(action_id=action_id)
        )

    @remote_service('/api/v1/realograms/scans/{scan_id}/')
    def get_scan_info(self, scan_id):
        return self.session.get(scan_id=scan_id)
