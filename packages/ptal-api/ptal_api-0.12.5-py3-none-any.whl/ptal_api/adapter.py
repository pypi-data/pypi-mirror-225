...
class TalismanAPIAdapter:
        ...
        *,
        ...
        if with_text:
            ...
            if with_text_metadata:
                ...
                if with_text_translation:
                    dt.metadata().text_translations().text()
                    dt.metadata().text_translations().language().id()
        if with_updater:
            doc_object.last_updater().__fields__(*self.user_fields)
        if with_creator:
            doc_object.creator().__fields__(*self.user_fields)
        if with_facts:
    ...
    def get_all_documents(
        ...
        with_text_translation: bool = False,
