...
DEFUALT_IMAGE_CONTENT_TYPE = "image/jpg"  # type: Literal["image/jpg"]
...
DEFUALT_LOCALIZATION_STEAM_NAME = "localization"
...
class Client:
    ...
    * Handling teleop control datapoints
    ...
    def get_localization_manager(
        self,
        stream_name=DEFUALT_LOCALIZATION_STEAM_NAME,  # type: str
    ...
    def call_cloud(
        ...
        """
        call_cloud allows the user to make an arbitrary Formant API call with
        device authentication.
        @param endpoint (str): The endpoint to call
        @param method (str): The HTTP method to use
        @param body (str): The body of the request
        @param headers (Dict[str, str]): The headers to use
        @param require_formant_auth (bool): Whether or not to use device authentication.
                    if True, then device authentication is authmatically 
                    injected into the request
        @param buffer_call (bool): Whether or not to buffer the call.
                    If True, then the call is buffered and retried.
                    If a call is buffered, the response is not returned
        @param is_retryable (bool): (For buffer_call=True only) Whether or not to retry the call
        @param retryable_status_codes (List[int]): (For buffer_call=True only) The status codes to retry on
        """
