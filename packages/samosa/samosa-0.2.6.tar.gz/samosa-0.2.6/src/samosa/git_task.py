...
def initialise_repository(root, quiet_if_valid):
    pygit2.init_repository(root)
    return fix_and_validate_root(root, quiet_if_valid=quiet_if_valid)


def add_remote(root, remote_name, remote_url):
...
def checkout_branch(root, name):
    ...
    if local_branch := repo.branches.get(name):
        repo.checkout(local_branch)
    else:
        sh.git("checkout", "--track", "upstream/main", "-b", name)
