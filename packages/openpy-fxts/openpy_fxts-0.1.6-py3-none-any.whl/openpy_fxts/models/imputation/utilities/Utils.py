...
class Metrics:
    ...
    def add_dict_metrics(self, df_true, df_imp, dict_perf, method):
        df_aux = pd.DataFrame()
        for x in range(df_true.shape[1]):
            ...
            dict_metrics = Metrics()._metrics(y_true, y_imp)
    ...
    def _metrics(self, y_true, y_imp):
        dict_metrics = dict()
        dict_metrics['MAE'] = mean_absolute_error(y_true, y_imp)
        dict_metrics['MSE'] = mean_squared_error(y_true, y_imp)
        dict_metrics['MAPE'] = mean_absolute_percentage_error(y_true, y_imp)
        dict_metrics['R2'] = r2_score(y_true, y_imp)
        return dict_metrics
...
class Utils(init_class):
    ...
    def missing_values_table(self):
        ...
        update_logg_file(
            "Your selected dfframe has " + str(self.dataframe.shape[1]) + " columns.\n"
