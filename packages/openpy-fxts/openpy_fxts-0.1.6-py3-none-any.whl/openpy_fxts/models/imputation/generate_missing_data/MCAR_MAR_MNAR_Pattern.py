...
class generate_missing_data:
    ...
    def produce_NaN(
        ...
        if mecha == "MAR":
            mask = generate_missing_data(self.seed)._MAR_mask(X, p_miss, p_obs).double()
        elif mecha == "MNAR" and opt == "logistic":
            mask = generate_missing_data(self.seed)._MNAR_mask_logistic(X, p_miss, p_obs).double()
        elif mecha == "MNAR" and opt == "quantile":
            mask = generate_missing_data(self.seed)._MNAR_mask_quantiles(X, p_miss, q, 1 - p_obs).double()
        elif mecha == "MNAR" and opt == "selfmasked":
            mask = generate_missing_data(self.seed)._MNAR_self_mask_logistic(X, p_miss).double()
        else:
        ...
        df_data_miss = pd.DataFrame(X_nas, columns=df_true.columns)
        df_data_miss['datetime'] = df_true.index
        df_data_miss.set_index('datetime', inplace=True)

        return df_data_miss

    def produce_NaN_pattern(self, df_data: pd.DataFrame = None, columns: list = None, prob_missing: float = 0.3, Random: bool = False):

        if columns is None:
            columns = list(df_data.columns)
        df_data = df_data.copy()
        np.random.seed(self.seed)
        W_train = np.random.choice(2, df_data[columns].shape, p=[prob_missing, 1 - prob_missing])
        W_train = pd.DataFrame(W_train, columns=columns).replace(0, np.nan)
        W_train['datetime'] = df_data.index
        W_train.set_index('datetime', inplace=True)
        if not Random:
            aux = list(W_train.columns)
            aux2 = aux[1:]
            for i in aux2:
                W_train[i] = W_train[aux[0]]
        X_train = (df_data[columns] * W_train)
        df_data[columns] = X_train
        return df_data

    def _nanmean(self, v, *args, **kwargs):
        """
    ...
    #### Quantile ######
    def _quantile(self, X, q, dim=None):
        """
    ...
    def _pick_epsilon(self, X, quant=0.5, mult=0.05, max_points=2000):
        """
        ...
        """
        means = generate_missing_data(self.seed)._nanmean(X, 0)
        X_ = X.clone()
        mask = torch.isnan(X_)
        ...
        return generate_missing_data(self.seed)._quantile(dists, quant, 0).item() * mult

    #### Accuracy Metrics ####
    def _MAE(self, X, X_true, mask):
        """
    ...
    def _MAR_mask(self, X, p, p_obs):
        ...
        coeffs = generate_missing_data(self.seed)._pick_coeffs(X, idxs_obs, idxs_nas)
        ### Pick the intercepts to have a desired amount of missing values
        intercepts = generate_missing_data(self.seed)._fit_intercepts(X[:, idxs_obs], coeffs, p)
    ...
    def _MNAR_mask_logistic(self, X, p, p_params=.3, exclude_inputs=True):
        ...
        ### Pick coefficients so that W^Tx has unit variance (avoids shrinking)
        coeffs = generate_missing_data(self.seed)._pick_coeffs(X, idxs_params, idxs_nas)
        ### Pick the intercepts to have a desired amount of missing values
        intercepts = generate_missing_data(self.seed)._fit_intercepts(X[:, idxs_params], coeffs, p)
    ...
    def _MNAR_self_mask_logistic(self, X, p):
        ...
        ### Pick coefficients so that W^Tx has unit variance (avoids shrinking)
        coeffs = generate_missing_data(self.seed)._pick_coeffs(X, self_mask=True)
        ### Pick the intercepts to have a desired amount of missing values
        intercepts = generate_missing_data(self.seed)._fit_intercepts(X, coeffs, p, self_mask=True)
    ...
    def _MNAR_mask_quantiles(self, X, p, q, p_params, cut='both', MCAR=False):
        ...
        if cut == 'upper':
            quants = generate_missing_data(self.seed)._quantile(X[:, idxs_na], 1 - q, dim=0)
            m = X[:, idxs_na] >= quants
        elif cut == 'lower':
            quants = generate_missing_data(self.seed)._quantile(X[:, idxs_na], q, dim=0)
            m = X[:, idxs_na] <= quants
        elif cut == 'both':
            u_quants = generate_missing_data(self.seed)._quantile(X[:, idxs_na], 1 - q, dim=0)
            l_quants = generate_missing_data(self.seed)._quantile(X[:, idxs_na], q, dim=0)
