...
import datetime
from openpy_fxts.scores.metrics import calculate_metrics
from time import time

from openpy_fxts.base_tf import tf, tkc, tkloss, tkmtc, tko, tku, value_miss
...
def learning_curve(
    ...
    list_metrics = list(set([item.replace('val_', '') for item in list(dict_hist.keys())]))
    fig, axs = plt.subplots(len(list_metrics), 1, layout='constrained', figsize=(18, 9), )
    for ax, metric in zip(axs.flat, list_metrics):
        _plots_metrics(ax, dict_hist, metric)
    best_epoch = np.argmin(dict_hist['val_loss'])
    fig.suptitle(
        'Learning curve'
        '\n' + 'best epoch:' + str(best_epoch + 1) + '(' + "{:e}".format(np.min(dict_hist['val_loss'])) + ')' +
        '- time_train: ' + str(datetime.timedelta(seconds=time() - time_start))
    )
    if view_plots:
        plt.show()
    if filepath is not None:
        fig.savefig(f'{filepath}/learning_curve_{name_mdl}.png')
    ...
    plt.title(
        'Learning curve'
        '\n' + 'best epoch:' + str(best_epoch + 1) + '(' + "{:e}".format(np.min(hist[monitor_label])) + ')' +
        '\n' + 'time_train: ' + str(datetime.timedelta(seconds=time() - time_start))
    )
    if filepath is not None:
...
def _plots_metrics(
    ...
    if metric == 'lr':
        ax.plot(history[metric], label='Learning Rate', alpha=.8, color='#ff7f0e')
        ax.axvline(x=best_epoch, label='Best epoch', alpha=.3, ls='--', color='#5a9aa5')
        ax.set_xticks(np.arange(len(history['val_loss'])), np.arange(1, len(history['val_loss']) + 1))
        ax.set_title('Learning Rate')
        ax.legend()
        ax.grid(alpha=.3)
    else:
        ax.plot(history[metric], label='Training', alpha=.8, color='#ff7f0e')
        ax.plot(history[f'val_{metric}'], label='Validation', alpha=.9, color='#5a9aa5')
        ax.axvline(x=best_epoch, label='Best epoch', alpha=.3, ls='--', color='#5a9aa5')
        ax.set_xticks(np.arange(len(history['val_loss'])), np.arange(1, len(history['val_loss']) + 1))
...
def _examples_plots(
    ...
    if hide_labels:
        ...
        ax.set_xlabel(f'Feature {i + 1} across all time steps', fontsize=fontsize)
        ax.set_ylabel(f'F_{i + 1}', fontsize=fontsize)
        ax.grid(alpha=.3)
        # ax.set_title('Title', fontsize=fontsize)
    '''
...
def _plot_results(
    ...
    if features == 1:
        ...
        if view_plots:
            plt.show()
        if filepath is not None:
            plt.savefig(f'{filepath}/true_vs_pred_{name_mdl}.png')
    else:
        ...
        if view_plots:
            plt.show()
        if filepath is not None:
            fig.savefig(f'{filepath}/true_vs_pred_{name_mdl}.png')
...
class process_values_preliminary:
    ...
    def reshape_X_test(self):
        ...
        if self.type_mdl == 'Conv2D':
            return X_test.reshape(X_test.shape[0], X_test.shape[1], X_test.shape[2], 1, 1)
        else:
            return X_test

    def reshape_yhat(self, yhat):
        if self.type_mdl == 'MLP':
            return yhat
            # return np.array(yhat).reshape(np.array(yhat).shape[1], np.array(yhat).shape[2], np.array(yhat).shape[0])
        else:
            return yhat
    ...
    def get_values(
        ...
        base = process_values_preliminary(
            self.model,
            self.dict_test,
            self.config_data,
            self.config_sim,
        ...
        X_test = base.reshape_X_test()
        if self.app == 'mdits':
            list_case = ['BiRNN_Bahdanau_Attention_Conv1D', 'BiRNN_Luong_Attention_Conv1D', 'BiRNN_MultiHeadAttention_Conv1D']
            if self.name_mdl in list_case:
                X_test = tf.where(tf.math.is_nan(X_test), value_miss, X_test)
        yhat = self.model.predict(X_test)
        yhat = base.reshape_yhat(yhat)
        yhat_inverse, y_test_inverse = _inverse_transform(yhat, self.dict_test, self.config_data['y_colname'])
        _evaluate_pred_global(y_test_inverse, yhat_inverse, self.name_mdl)
        if self.config_sim['plt_results']:
            _plot_results(
                y_test_inverse,
                yhat_inverse,
                self.name_mdl,
                self.config_sim['view_plots'],
                filepath
            )
            _metrics_n_steps_for_future(
                y_test_inverse,
                yhat_inverse,
                self.name_mdl,
                self.config_data,
        ...
        if self.config_sim['metrics']:
            scorer_dict = dict_metrics(y_test_inverse, yhat_inverse, self.config_data, self.name_mdl)
...
def _metrics_n_steps_for_future(
        y_test_inverse,
        yhat_inverse,
        name_mdl: str = None,
        config: dict = None,
    ...
    for key, values in dict_results.items():
        ...
        axs[aux].set_xticks(np.arange(0, config['n_future']), np.arange(1, config['n_future'] + 1))
        axs[aux].grid(alpha=.3)
        aux += 1

    if view_plots:
        plt.show()
    if filepath is not None:
        fig.savefig(f'{filepath}/n_steps_{name_mdl}.png')
