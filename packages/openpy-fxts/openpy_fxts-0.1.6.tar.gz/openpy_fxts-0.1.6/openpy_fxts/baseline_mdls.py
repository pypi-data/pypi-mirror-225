...
class base_model:
    ...
    def _training_mdl(self, filepath):
        base = base_model(
            self.config_data, self.config_mdl, self.config_sim, self.config_arch, self.name_mdl, self.type_mdl, self.app
        )
        ...
        X_train, y_train, X_valid, y_valid = base.reshape_train_valid_for_fxts(pre_processed)
        history = model.fit(
            x=X_train,
            y=y_train,
            validation_data=(
