...
options:
   ...
   -n, --no_cache   Do not use caching for audio. [default: False].
...
def genmixdb(location: Location,
    ...
    if logging:
        ...
        logger.info(f'Feature shape:        '
                    f'{mixdb.fg.stride} x {mixdb.fg.num_bands} ({mixdb.fg.stride * mixdb.fg.num_bands} total params)')
    ...
    MP_GLOBAL.mixid_width = max_text_width(len(mixdb.mixtures))
    ...
    if logging:
        ...
        logger.info('')
        logger.debug(f'Target audio cache info: {mixdb.raw_target_audio_cache_info}')
        logger.debug(f'Noise audio cache info:  {mixdb.augmented_noise_audio_cache_info}')
        logger.debug('')
...
def _process_mixture(mixid: int) -> MRecord:
    ...
    from sonusai.mixture import initialize_mixture

    mrecord, genmix_data = initialize_mixture(mixdb=MP_GLOBAL.mixdb,
                                              mixid=mixid,
                                              mixid_width=MP_GLOBAL.mixid_width)
