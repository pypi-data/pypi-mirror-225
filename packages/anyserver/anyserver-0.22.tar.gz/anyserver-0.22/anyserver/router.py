...
from anyserver.encoders import DEFAULT_ENCODERS, toJSON
    ...
    def _routes(self):
        ...
        for verb in routes:
            ...
            for sub_path in routes[verb]:
                ...
                http[verb][route] = self.format(action)

        return http

    def format(self, action):
        def formatted(req, resp):
            # Run the handler function to get the raw result
            data = action(req, resp)

            # Try and determine a content type format to respond with
            ctype = resp.head['content-type'] if 'content-type' in resp.head else ''
            for enc in DEFAULT_ENCODERS:
                if ctype == enc.mime:
                    # COntent type was matched
                    return enc.encode(data)

            # Respond with default content type (JSON)
            return toJSON(data)
        return formatted
