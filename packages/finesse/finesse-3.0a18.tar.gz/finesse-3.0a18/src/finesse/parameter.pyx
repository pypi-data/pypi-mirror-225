...
class ParameterRef(Symbol):
    ...
    def __deepcopy__(self, memo):
        ...
        owner = memo.get(owner_id)

        if type(owner) is Model:
            new_model = memo.get(id(self.parameter.owner))
        else:
...
cdef class Parameter:
    ...
    def __init__(self, parameter_info, owner):
        ...
        if self.__owner_type != Model:
            self.__full_name = f"{owner.name}.{parameter_info.name}"
            self.__validator = ModelElement._validators[self.__owner_type][self.name]
        else:
    ...
    cdef __cdeepcopy__(self, Parameter new, dict memo):
        ...
        new.__owner_type = self.__owner_type
    ...
    @property
    def owner(self):
        """The component/element this parameter is associated with, this could
        be a :class:`finesse.element.ModelElement` or a :class:`finesse.model.Model`.
        """
        return self.__owner()
    ...
    def _model(self):
        """
        Returns the model that this owner is connected to. Raises an exception if
    ...
    def is_tunable(self, bint value):
        if not self.changeable_during_simulation:
            raise FinesseException(f"The parameter {self.full_name} cannot be changed during a simulation")
        if self._locked:
            raise ParameterLocked()
        if len(self.external_setters) > 0:
            raise FinesseException(f"{repr(self)} is being externally controlled by {self.external_setters}, so it cannot be tuned directly.")
        if self.state == ParameterState.Symbolic:
    ...
    def __divmod__(A, B):
        if hasattr(A, "value"):
            return divmod(A.value, B)
        else:
            return divmod(A, B.value)

    def __pow__(A, B, order):
        if hasattr(A, "value"):
            if hasattr(B, "value"):
                return pow(A.value, B.value, order)
            return pow(A.value, B, order)
        return pow(A, B.value, order)
