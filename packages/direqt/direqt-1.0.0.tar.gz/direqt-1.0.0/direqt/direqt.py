...
_VALID_AD_FORMATS = [_AD_FORMAT_RAW, _AD_FORMAT_RBM10, _AD_FORMAT_SMS]
...
class DireqtClient(object):
    ...
    def fetch(self, ad_unit, msisdn=None, subscriber_id=None,
              targeting=None, url_shortener=None, fetch_only=False, count=1):
        ...
        Args:
            ...
            count: Optional. Specifies the maximum number of fetch records to be returned. If more
                than one response is included, the result is an array of fetch records.
        """
        targeting = targeting or {}

        if not msisdn and self.fmt == _AD_FORMAT_RBM10:
        ...
        if not subscriber_id and msisdn:
            subscriber_id = hashlib.md5((self.api_key + msisdn).encode('utf-8')).hexdigest()
    ...
    def _process_fetch_result(self, msisdn, body, url_shortener=None, fetch_only=False):
        ...
        logger.info("Received payload: " + (json.dumps(payload) or '<none>'))

        if fetch_only or self.fmt == _AD_FORMAT_RAW:
            response = {
                'payload': payload,
                'message_id': message_id,
                'properties': properties,
