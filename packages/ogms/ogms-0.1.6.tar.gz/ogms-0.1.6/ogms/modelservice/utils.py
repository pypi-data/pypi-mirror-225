...
class HttpHelper:
    @staticmethod
    def Request_get_sync(ip: str, port: int, path: str) -> json:
        try :
    ...
    @staticmethod
    def Request_get_str_sync(ip: str, port: int, path: str):
        try:
            conn = httplib.HTTPConnection(ip, port)
            conn.request("GET", path)
            res = conn.getresponse()
    ...
    @staticmethod
    def Request_post_sync(ip: str, port: int, path: str, params: object = None, files: object = None):
        try :
