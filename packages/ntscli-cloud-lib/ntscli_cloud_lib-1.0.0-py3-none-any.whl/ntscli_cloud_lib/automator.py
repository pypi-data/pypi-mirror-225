...
class DeviceIdentifier(betterproto.Message):
    ...
    adb_serial: Optional[str] = betterproto.message_field(
...
class TestCase(betterproto.Message):
    ...
    # The absolute URL to the test, relative or absolute.
    url: Optional[str] = betterproto.message_field(7, wraps=betterproto.TYPE_STRING)
    # The signature used to verify the testcase is run unmodified.
    signature: Optional[str] = betterproto.message_field(
        8, wraps=betterproto.TYPE_STRING
    )


@dataclass
class TestPlanCommon(betterproto.Message):
    """
    The test plan common object. Every property in this object will get blindly
    merged into each testcase object by automator. This also allows overriding
    properties that we in the testcase object.
    """

    retries: Optional[int] = betterproto.message_field(1, wraps=betterproto.TYPE_INT32)
...
class TestPlan(betterproto.Message):
    ...
    batch_name: Optional[str] = betterproto.message_field(
        7, wraps=betterproto.TYPE_STRING
    )
    # Estimates of run time based on recent executions
    estimated_time: "EstimatedDuration" = betterproto.message_field(8)
    # The test plan common object.
    common: "TestPlanCommon" = betterproto.message_field(9)
...
class GetTestPlanRequestOptions(betterproto.Message):
    ...
    playlist_id: Optional[str] = betterproto.message_field(
        1, wraps=betterproto.TYPE_STRING
    )
    # String. Use this as an enum to indicate what kind of test plan request to
    # make (ie testplan, playlist, calibration).
    test_plan_type: Optional[str] = betterproto.message_field(
        2, wraps=betterproto.TYPE_STRING
    )
...
class TestPlanRunRequest(betterproto.Message):
    ...
    # This object is passed in the request and used to pre-populate the savedData
    # used by a test.
    initial_saved_data: "InitialSavedData" = betterproto.message_field(4)
...
@dataclass
class InitialSavedData(betterproto.Message):
    """
    Data to be injected into saved data. The savedData for each test run will
    be initialized with these values.
    """

    # Whether this test run should be counted against certification.
    for_certification: Optional[bool] = betterproto.message_field(
