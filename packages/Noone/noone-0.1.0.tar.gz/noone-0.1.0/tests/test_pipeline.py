...
from noone_pipeline import NoOneCnPipeline, NoOnePipeline
...
class TestAdPipeline(Base):
    pipe = NoOnePipeline.from_pretrained(counterfeit, torch_dtype=torch.float16)
...
class TestDiffusersPipeline(Base):
    pipe = DiffusionPipeline.from_pretrained(
        ...
        custom_pipeline="NoOne/noone_pipeline",
        # Bingsu/adsd_pipeline",
    )
    pipe.safety_checker = None
    pipe.scheduler = DPMSolverMultistepScheduler.from_config(pipe.scheduler.config)
    pipe.to("cuda")
...
class TestCnPipeline:
    ...
    pipe = NoOneCnPipeline.from_pretrained(
        counterfeit,
