...
def tiktoken_len(text):
    tokens = tokenizer.encode(text, disallow_special=())
    return len(tokens)
...
chunks = []
for idx, record in enumerate(tqdm(pdf_text_dict)):
    pdf_text_dict = text_splitter.split_text(record["text"])
chunks.extend(
    [
        {"id": str(uuid4()), "text": text[i], "chunk": i, "file": record["file_name"]}
